...
class Ajsi20 extends CI_Controller {           //擴展類必须註明由母類擴展而來 (自訂類使用 ci 框架)
	  ...
	  public function index()           //自訂類預設執行函數 流覽資料
	    {                      
	      ...
	      $this->load->model('ajs/ajsi20_model');     // 加載TABLE model 模型		
	      $result= $this->ajsi20_model->search($limit, $offset = 0 , $sort_by  , $sort_order ); //至model 取 mysql 資料 預設 15,0,ta001,desc
	    ...
	    //  $this->pagination->initialize($config);    //分頁初始化 display 3
	      $config['base_url'] = site_url("ajs/ajsi20/display/$sort_by/$sort_order");   //設定分頁url路徑
	   ...
	   //   $this->pagination->initialize($config);
	      ...
	      $data['systitle'] ='分錄底稿維護作業';  //網頁抬頭顯示名稱
	      $data['menu_v'] = 'main_menu_v';
	      $data['content_v'] = 'ajs/ajsi20_brow_v';		
	  ...
	  public function display($sort_by = 'ta002', $sort_order = 'desc', $offset = 0)  //欄位表頭排序
	    {		
	      ...
	      $data['sort_order'] = $sort_order;
	      $this->load->model('ajs/ajsi20_model');// 加載TABLE model 模型		
	      $result= $this->ajsi20_model->search($limit, $offset , $sort_by  , $sort_order ); //至model 取 mysql 資料 預設 15,0,ta001,desc
	  ...
	  public function lookup(){
        ...
        $this->load->model('ajs/ajsi20_model');
        $query = $this->ajsi20_model->lookup(urldecode(urldecode($this->uri->segment(4))),$seq5); //Search DB 
        ...
        if('IS_AJAX')  
         ...
         {  
		    $this->load->view('ajs/ajsi20_model/lookup',$data); 
        ...
        $data['response'] = 'false'; //Set default response 
        $this->load->model('ajs/ajsi20_model');
        $query = $this->ajsi20_model->lookup2(urldecode(urldecode($this->uri->segment(4))),$seq5); //Search DB 
	  ...
	  public function lookupa(){
        ...
        $data['response'] = 'false'; //Set default response 
        $this->load->model('ajs/ajsi20_model');
        $query = $this->ajsi20_model->lookupa(urldecode(urldecode($this->uri->segment(4)))); //Search DB 
        ...
        if('IS_AJAX')  
         ...
         {  
		    $this->load->view('ajs/ajsi20_model/lookupa',$data); 
          // $this->index; //Load html view of search results  
         }  
        }  
		public function dataacpq02a()   //提示改輸入資料如 收款單別   不更新網頁
          {
	        $this->load->model('ajs/ajsi20_model');
	        $data['result'] = $this->ajsi20_model->ajaxacpq02a($this->uri->segment(4));
            $Result = $data['result'];		  
	        $this->load->vars($data);
	        echo  $Result;
          }
	   ...
	   public function datacmsq05a()   //提示改輸入資料如 請購部門 不更新網頁
        {
	      $this->load->model('ajs/ajsi20_model');
	      $data['result'] = $this->ajsi20_model->ajaxcmsq05a($this->uri->segment(4));
          $Result = $data['result'];		  
	      $this->load->vars($data);
	      echo  $Result;
        }
	   public function datacmsq02a()  //提示改輸入資料如 廠別 不更新網頁ta010
        {
	      $this->load->model('ajs/ajsi20_model');
	      $data['result'] = $this->ajsi20_model->ajaxcmsq02a($this->uri->segment(4));
          $Result = $data['result'];		  
	      $this->load->vars($data);
	      echo  $Result;
        }
        ...
        {
	      $this->load->model('ajs/ajsi20_model');
	      $data['result'] = $this->ajsi20_model->ajaxpalq01a($this->uri->segment(4));
          $Result = $data['result'];		  
	      $this->load->vars($data);
	      echo  $Result;
        }
	   public function datachkno1()   //提示改輸入資料如  收款單號 不更新網頁ta012
        {
	      $this->load->model('ajs/ajsi20_model');
	      $data['result'] = $this->ajsi20_model->ajaxchkno1($this->uri->segment(4),$this->uri->segment(5));
          $Result = $data['result'];		  
	      $this->load->vars($data);
	      echo  $Result;
        }
	  ...
	  public function filter1($sort_by = 'ta001', $sort_order = 'desc', $offset = 0)   ////篩選資料
        {
	     ...
	     $data['sort_order'] = $sort_order;
	     $this->load->model('ajs/ajsi20_model','',TRUE);   //true 系統會使用反單引號（`, 在英文鍵盤左上方）來保護你的欄位或資料表名稱
	     $result=$this->ajsi20_model->filterf1($limit, $offset , $sort_by  , $sort_order);
	     $data['message'] = '篩選資料成功!';	
	     $data['results'] = $result['rows'];
	     $data['num_results'] = $result['num_rows'];
	     $this->load->library('pagination');
	     ...
	     $config['base_url'] = site_url("ajs/ajsi20/filter1/$sort_by/$sort_order/$seq6/$seq7");  //設定分頁url路徑
      ...
      public function findform()   //進階查詢輸入資料
        {
	      ...
	      $data['username'] = $this->session->userdata('manager');
	      $data['message'] = '';
	      $data['systitle'] ='分錄底稿維護-進階查詢';
	      $data['menu_v'] = 'main_menuno_v';
	      $data['content_v'] = 'ajs/ajsi20_find_v';
	      $data['foot_v'] ='main_foot_v';
	      $this->load->vars($data);
	  ...
	  public function findsql($sort_by = 'ta001', $sort_order = 'desc', $offset = 0)  //進階查詢流覽資料
	    {		
			...
			session_start();}
		  if(@$_POST['find005']){
			$_SESSION['ajsi20_sql_term'] = $_POST['find005'];
		  }
		  if(@$_POST['find007']){
			$_SESSION['ajsi20_sql_sort'] = $_POST['find007'];
		  }
	      $limit = 15;    //每頁筆數
	      $data['message'] = '資料流覽成功!';
	      $data['sort_by'] = $sort_by;
	      $data['sort_order'] = $sort_order;
	      $this->load->model('ajs/ajsi20_model');// 加載TABLE model 模型		
	      $result= $this->ajsi20_model->findf($limit, $offset , $sort_by  , $sort_order ); //至model 取 mysql 資料 預設 15,0,ta001,desc
	      $data['results'] = $result['rows'];
	      $data['num_results'] = $result['num_rows'];
	      $this->load->library('pagination');
	      $data['numrow']=$result['num_rows'];// 總筆數 
	    ...
	    //$config['NUM_LINKS'] = 2;   //分頁前後出現2個數字
	      ...
	      $config['cur_page'] = $this->uri->segment(6,0);   //當前頁 結合分頁url路徑 +1
	      $this->pagination->initialize($config);    //分頁初始化 display 3
	      $config['base_url'] = site_url("ajs/ajsi20/findsql/$sort_by/$sort_order");   //設定分頁url路徑
	      $config['total_rows'] = $result['num_rows']; // 總筆數
	      $config['per_page'] = $limit;                //每頁筆數
	      $config['uri_segment'] = 6;       //當前頁
	      $this->pagination->initialize($config);
		...
		if (session_status() == PHP_SESSION_NONE) {
		session_start();}
		  if(@$_SESSION["ajsi20_sql_term"]) {unset($_SESSION["ajsi20_sql_term"]);}
		  if(@$_SESSION["ajsi20_sql_sort"]) {unset($_SESSION["ajsi20_sql_sort"]);}
		  $this->display();
	  }
      public function addform()   //新增輸入資料
        {
	     ...
	     $data['message'] = '';
	     $data['username'] = $this->session->userdata('manager');
	     $data['systitle'] ='分錄底稿維護-新增資料';
		   //系統參數
		  $this->load->model('ajs/ajsi20_model','',TRUE);
		//  $result2 = $this->ajsi20_model->funsysf();
	    //  $data['results2'] = $result2['rows2'];
		  
	     $data['menu_v'] = 'main_menuno_v';
	     $data['content_v'] = 'ajs/ajsi20_add_v';
	     $data['foot_v'] ='main_foot_v';
	     $this->load->vars($data);
	     $this->load->view('main_head_v');
        }
    ...
    //  $this->issetFile($filePath,$filename);
		...
		if ($msg2)
		 ...
		 $data['uploadfile'] = $filename;  	
          ...
          $this->load->model('ajs/ajsi20_model','',TRUE);
	      $data['message'] = '新增成功!';
	      $action = $this->ajsi20_model->insertf();
	      if ($action === 'exist')
	       {
	        $data['message'] = '資料重複!';		    
	       }
		     else{
			  $this->ajsi20_model->auto_print();
		  }
      ...
      public function copyform()   //複製資料輸入
        {
	     $data['username'] = $this->session->userdata('manager');
	     $data['message'] = '';
	     $data['systitle'] ='分錄底稿維護-複製資料';
	     $data['menu_v'] = 'main_menuno_v';
	     $data['content_v'] = 'ajs/ajsi20_copy_v';
	     $data['foot_v'] ='main_foot_v';
	     $this->load->vars($data);
	     $this->load->view('main_head_v');
        }
      ...
      public function copysave()   //複製存檔
        {
         ...
         $this->load->model('ajs/ajsi20_model','',TRUE);
	     $data['message'] = '複製成功!';
         $action = $this->ajsi20_model->copyf();
		  ...
		  public function copybefore()   //前置單據存檔
        {
	      ...
	      $data['message'] = '查詢一筆資料!';
	      $this->load->model('ajs/ajsi20_model');
	      $data['result'] = $this->ajsi20_model->selonebefore($this->uri->segment(4),$this->uri->segment(5));
	      $data['username'] = $this->session->userdata('manager');
	      $data['systitle'] ='分錄底稿維護-前置單據資料';
	      $data['menu_v'] = 'main_menuno_v';
	      $data['content_v'] = 'ajs/ajsi20_add_v';
	      $data['foot_v'] ='main_foot_v';
	      $this->load->vars($data);
      ...
      public function exceldetail()   //轉excel明細輸入起迄資料
        {
	     $data['message'] = '';
	     $data['username'] = $this->session->userdata('manager');
	     $data['systitle'] ='分錄底稿維護-轉excel檔';
	     $data['menu_v'] = 'main_menuno_v';
	     $data['content_v'] = 'ajs/ajsi20_excel_v';
	     $data['foot_v'] ='main_foot_v';
	     $this->load->vars($data);
	     $this->load->view('main_head_v');
        }
      ...
      public function write()   //轉excel 部份資料由 print_v call
        {
         ...
         $result1 = $this->ajsi20_model->excelnewf();	
      ...
      public function printdetail()   //印明細起迄資料輸入
        {
	     $data['username'] = $this->session->userdata('manager');
	     $data['message'] = '';
	     $data['systitle'] ='分錄底稿維護-印明細表';
	     $data['menu_v'] = 'main_menuno_v';
	     $data['content_v'] = 'ajs/ajsi20_print_v';
	     $data['foot_v'] ='main_foot_v';
	     $this->load->vars($data);
	     $this->load->view('main_head_v');
        }
		 public function printdetailc()   //印明細起迄資料輸入
        {
	     ...
	     $data['systitle'] ='分錄底稿維護-印明細表';
	     $data['menu_v'] = 'main_menuno_v';
	     $data['content_v'] = 'ajs/ajsi20_print1_v';
	     $data['foot_v'] ='main_foot_v';
	     $this->load->vars($data);
	     $this->load->view('main_head_v');
        }
		public function printc()   //印轉帳傳票
        {
          ...
          $this->load->model('ajs/ajsi20_model','',TRUE);
	      $data['message'] = '列印傳票!';
           $result = $this->ajsi20_model->printfc();
	   ...
	   //   $data['systitle'] ='分錄底稿維護-印明細表';
	      //$data['menu_v'] = 'main_menuno_v';
	   //   $data['content_v'] = 'ajs/ajsi20_printb_v';
	   //  $data['foot_v'] ='main_footno_v';
	     $this->load->vars($data);
	   //  $this->load->view('main_headprint_v');
	     $this->load->view('ajs/ajsi20_printc_v');  
		...
		public function printbb($ta009c)   //印轉帳傳票
        {
	      ...
	      $data['message'] = '列印傳票!';
		      //公司參數
		   $result1 = $this->ajsi20_model->companyf();
	      $data['results1'] = $result1['rows1'];
		  
           $result = $this->ajsi20_model->printfb();
	      $data['results'] = $result['rows'];
	     $this->load->vars($data);
	     $this->load->view('ajs/ajsi20_printb_v');  
        }
      public function printa()   //印明細
        {
          ...
          $this->load->model('ajs/ajsi20_model','',TRUE);
	      $data['message'] = '列印明細成功!';
          $result = $this->ajsi20_model->printfd();
	      $data['results'] = $result['rows'];
	      $data['num_results'] = $result['num_rows'];
	      $this->load->library('pagination');
	      $data['numrow']=$result['num_rows'];// 總筆數 
		  ...
		  $data['paper9'] = $this->input->post('ta009p');
	      //$data['menu_v'] = 'main_menuno_v';
	      $data['content_v'] = 'ajs/ajsi20_printa_v';
	     //$data['foot_v'] ='main_footno_v';
	     $this->load->vars($data);
	     $this->load->view('main_headprint_v');
	     //$this->load->view('ajs/ajsi20_printa_v',$data);  
        }
      ...
      public function updsave()   //修改存檔
		...
		if ($msg2)
         ...
         $this->load->model('ajs/ajsi20_model','',TRUE);
	     $this->load->vars($data);
	     $this->ajsi20_model->updatef();
	     redirect('ajs/ajsi20/'.$this->session->userdata('search1'));
        }
      ...
      public function updform()   //修改輸入資料
        {
	      ...
	      $this->load->model('ajs/ajsi20_model');
	      $data['result'] = $this->ajsi20_model->selone($this->uri->segment(4),$this->uri->segment(5));
	      $data['username'] = $this->session->userdata('manager');
	      $data['systitle'] ='分錄底稿維護-修改資料';
	      $data['menu_v'] = 'main_menuno_v';
	      $data['content_v'] = 'ajs/ajsi20_upd_v';
	      $data['foot_v'] ='main_foot_v';
	      $this->load->vars($data);
	      $this->load->view('main_head_v');
        }
      ...
      public function see()   //看資料
        {      
	      ...
	      $data['result'] = $this->ajsi20_model->selone($this->uri->segment(4),$this->uri->segment(5));
	      $data['username'] = $this->session->userdata('manager');
	      $data['systitle'] ='分錄底稿維護-查看資料';
	      $data['menu_v'] = 'main_menuno_v';
	      $data['content_v'] = 'ajs/ajsi20_see_v';
	      $data['foot_v'] ='main_foot_v';
	      $this->load->vars($data);
	      $this->load->view('main_head_v');
        }
      ...
      public function del()   //刪除單筆 暫存
        {      
	      ...
	      $data['message'] = '刪除資料成功!';
	      $this->load->model('ajs/ajsi20_model','',TRUE);
	      $this->ajsi20_model->deletef($seg1,$seg2);
	      $this->display();
        }
	 ...
	 public function printb()   //印單據選取
        {    
	      ...
	      $this->load->model('ajs/ajsi20_model','',TRUE);
	      $data['message'] = '列印單據成功!';
          $result = $this->ajsi20_model->printfd1($this->uri->segment(4),$this->uri->segment(5));
	      $data['results'] = $result['rows'];
	      $data['num_results'] = $result['num_rows'];
	      $this->load->library('pagination');
	      $data['numrow']=$result['num_rows'];// 總筆數 
	...
	public function delete_detail(){
		$data['message'] = '刪除資料成功!';
		$this->load->model('ajs/ajsi20_model','',TRUE);
		$this->ajsi20_model->del_detail();
	    redirect('ajs/ajsi20/updform/'.$_POST['del_md001'].'/'.$_POST['del_md002']);   //重新整理
	}
}
/* End of file ajsi20.php */
/* Location: ./application/controllers/ajsi20.php */
?>