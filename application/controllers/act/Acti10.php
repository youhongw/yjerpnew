...
class Acti10 extends CI_Controller {           //擴展類必须註明由母類擴展而來 (自訂類使用 ci 框架)
	  ...
	  public function findsql($sort_by = 'ta001', $sort_order = 'desc', $offset = 0)  //進階查詢流覽資料
	    {		
		  ...
		  if(@$_POST['find005']){
			$_SESSION['acti10_sql_term'] = $_POST['find005'];
		  }
		  if(@$_POST['find007']){
			$_SESSION['acti10_sql_sort'] = $_POST['find007'];
		  }
	      $limit = 15;    //每頁筆數
	      $data['message'] = '資料流覽成功!';
		...
		if (@session_status() == PHP_SESSION_NONE) {
		  ...
		  if(@$_SESSION["acti10_sql_term"]) {unset($_SESSION["acti10_sql_term"]);}
		  if(@$_SESSION["acti10_sql_sort"]) {unset($_SESSION["acti10_sql_sort"]);}
		  $this->display();
	  }
