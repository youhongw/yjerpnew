...
class Palb01 extends CI_Controller {           //擴展類必须註明由母類擴展而來 (自訂類使用 ci 框架)
	...
	//自訂類預設執行函數 流覽資料
	  public function index()           
	    {                      
	      ...
	      $this->load->model('pal/palb01_model');     // 加載TABLE model 模型		
	      $result= $this->palb01_model->search($limit, $offset = 0 , $sort_by  , $sort_order ); //至model 取 mysql 資料 預設 15,0,ma001,desc
	    ...
	    //$config['NUM_LINKS'] = 2;   //分頁前後出現2個數字
	      ...
	      $config['base_url'] = site_url("pal/palb01/display/$sort_by/$sort_order");   //設定分頁url路徑
	   ...
	   // $this->load->library('table');//加載table函數
	    ...
	    //$data['find07']=$this->session->userdata('find07');
	      ...
	      $data['systitle'] ='薪資計算作業';  //網頁抬頭顯示名稱
	      $data['menu_v'] = 'main_menu_v';
	      $data['content_v'] = 'pal/palb01_brow_v';		
      ...
      public function batcha()   
        {
		  ...
		  $data['message'] = $this->palb01_model->batchaf();   //invlc 表
	...
	//提示資料重複 類別代號 key 5	
	 public function checkkey()   
        {
	     $this->load->model('pal/palb01_model');
	     $data['result'] = $this->palb01_model->ajaxkey($this->uri->segment(4));
         $Result = $data['result'];		  
	     $this->load->vars($data);
	     echo  $Result;
        }
