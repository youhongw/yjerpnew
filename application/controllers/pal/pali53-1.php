...
class pali53 extends CI_Controller {           //擴展類必须註明由母類擴展而來 (自訂類使用 ci 框架)
	...
	//自訂類預設執行函數 流覽資料	
	 ...
	 //欄位表頭排序 資料流覽 
	  public function display($sort_by = 'tf001', $sort_order = 'desc', $offset = 0)  
	    {		
	      $limit = 15;    //每頁筆數
	      $data['message'] = '資料流覽成功!';
	      $data['sort_by'] = $sort_by;
	...
	public function findsql($sort_by = 'tf001', $sort_order = 'desc', $offset = 0)  
	    {		
	      ...
	      $data['sort_order'] = $sort_order;
	      $this->load->model('pal/pali53_model');// 加載TABLE model 模型		
	      $result= $this->pali53_model->findf($limit, $offset , $sort_by  , $sort_order ); //至model 取 mysql 資料 預設 15,0,tf001,desc
	      $data['results'] = $result['rows'];
	      $data['num_results'] = $result['num_rows'];
	      $this->load->library('pagination');
	      $data['numrow']=$result['num_rows'];// 總筆數 
    ...
    public function updsave()   
        {
	     ...
	     redirect('pal/pali53/'.$this->session->userdata('search'));
        }
