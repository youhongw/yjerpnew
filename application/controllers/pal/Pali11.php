...
class pali11 extends CI_Controller {           //擴展類必须註明由母類擴展而來 (自訂類使用 ci 框架)
	...
	//自訂類預設執行函數 流覽資料	
	  public function index()           
	    {                      
	      ...
	      $this->load->model('pal/pali11_model');     // 加載TABLE model 模型		
	      $result= $this->pali11_model->search($limit, $offset = 0 , $sort_by  , $sort_order ); //至model 取 mysql 資料 預設 15,0,md001,desc
	   ...
	   //   $this->pagination->initialize($config);    //分頁初始化 display 3
	      $config['base_url'] = site_url("pal/pali11/display/$sort_by/$sort_order");   //設定分頁url路徑
	   ...
	   // $this->load->library('table');//加載table函數
	    ...
	    //$data['find07']=$this->session->userdata('find07');
	      ...
	      $data['content_v'] = 'pal/pali11_brow_v';		
	 ...
	 //欄位表頭排序 資料流覽 
	  public function display($sort_by = 'md001', $sort_order = 'desc', $offset = 0)  
		...
		$data['message'] = '資料瀏覽成功!';
		$this->load->model('pal/pali11_model');// 加載TABLE model 模型	
		$select_col = array('`palmt`.*','b.mv002 as mt002disp','c.mo002 as mt001disp','c.mo003','c.mo004');	
		...
		$join = array('b'=>array('table'=>"cmsmv",'method'=>"left",'term'=>"mt002 = b.mv001"),
				'c'=>array('table'=>"palmo",'method'=>"left",'term'=>"mt001 = c.mo001")
				);
		$where = array();
		...
		$data['data'] = $this->pali11_model->get_pali11($select_col,$join,$where);
	...
	public function filter1($sort_by = 'md001', $sort_order = 'desc', $offset = 0)   
       {
	     ...
	     $data['sort_order'] = $sort_order;
	     $this->load->model('pal/pali11_model','',TRUE);   //true 系統會使用反單引號（`, 在英文鍵盤左上方）來保護你的欄位或資料表名稱
	     $result=$this->pali11_model->filterf1($limit, $offset , $sort_by  , $sort_order);
	     $data['message'] = '篩選資料成功!';	
	     ...
	     $config['base_url'] = site_url("pal/pali11/filter1/$sort_by/$sort_order/$seq6/$seq7");  //設定分頁url路徑
	     ...
	     $this->load->vars($data);
	     $this->load->view('main_headbrow_v');
	   //$this->load->view('pal/pali11_v', $data);
       }
    ...
    public function findform()   
        {
	      ...
	      $data['content_v'] = 'pal/pali11_find_v';
	      $data['foot_v'] ='main_foot_v';
	      $this->load->vars($data);
	...
	public function findsql($sort_by = 'md001', $sort_order = 'desc', $offset = 0)  
	    {		
	      ...
	      $data['sort_order'] = $sort_order;
	      $this->load->model('pal/pali11_model');// 加載TABLE model 模型		
	      $result= $this->pali11_model->findf($limit, $offset , $sort_by  , $sort_order ); //至model 取 mysql 資料 預設 15,0,md001,desc
	      $data['results'] = $result['rows'];
	      $data['num_results'] = $result['num_rows'];
	      $this->load->library('pagination');
	      $data['numrow']=$result['num_rows'];// 總筆數 
	    ...
	    //$config['NUM_LINKS'] = 2;   //分頁前後出現2個數字
	      ...
	      $config['cur_page'] = $this->uri->segment(6,0);   //當前頁 結合分頁url路徑 +1
	      $this->pagination->initialize($config);    //分頁初始化 display 3
	      $config['base_url'] = site_url("pal/pali11/findsql/$sort_by/$sort_order");   //設定分頁url路徑
	      $config['total_rows'] = $result['num_rows']; // 總筆數
	      $config['per_page'] = $limit;                //每頁筆數
	      $config['uri_segment'] = 6;       //當前頁
	    //$this->load->library('table');//加載table函數
    ...
    public function addform()   
      {
	   ...
	   $data['menu_v'] = 'main_menuno_v';
	   $data['content_v'] = 'pal/pali11_add_v';
	   $data['foot_v'] ='main_foot_v';
	   $this->load->vars($data);
	   $this->load->view('main_head_v');
      }
    ...
    public function addsave()   
      {
		$data['username'] = $this->session->userdata('manager');
		$this->load->model('pal/pali11_model','',TRUE);
		$data['message'] = '新增資料成功!';
		extract($this->input->post());
		$mt003 = strtotime($mt003);
		$mt003 = date('Y_n_j',$mt003);
		$action = $this->pali11_model->insertf();
		if ($action === 'exist')
		 {
			$data['message'] = '資料重複!';	    
		 }
    ...
    public function copyform()   
      {
	   ...
	   $data['menu_v'] = 'main_menuno_v';
	   $data['content_v'] = 'pal/pali11_copy_v';
	   $data['foot_v'] ='main_foot_v';
	   $this->load->vars($data);
	   $this->load->view('main_head_v');
      }
    ...
    public function copysave()   
	 ...
	 //$date=date("Ymd");
       ...
       $this->load->model('pal/pali11_model','',TRUE);
	   $data['message'] = '複製成功!';
       $action = $this->pali11_model->copyf();
    ...
    public function exceldetail()   
      {
	   ...
	   $data['menu_v'] = 'main_menuno_v';
	   $data['content_v'] = 'pal/pali11_excel_v';
	   $data['foot_v'] ='main_foot_v';
	   $this->load->vars($data);
	   $this->load->view('main_head_v');
      }
    ...
    public function write()   
      {
       ...
       $result1 = $this->pali11_model->excelnewf();	
    ...
    public function printdetail()   
      {
	   ...
	   $data['menu_v'] = 'main_menuno_v';
	   $data['content_v'] = 'pal/pali11_print_v';
	   $data['foot_v'] ='main_foot_v';
	   $this->load->vars($data);
	   $this->load->view('main_head_v');
      }
    ...
    public function printa()   
      {
       ...
       $this->load->model('pal/pali11_model','',TRUE);
	   $data['message'] = '列印明細成功!';
       $result = $this->pali11_model->printfd();
	   $data['results'] = $result['rows'];
	   $data['num_results'] = $result['num_rows'];
	   $this->load->library('pagination');
	   $data['numrow']=$result['num_rows'];// 總筆數 
	   ...
	   $data['systitle'] ='異動薪資-印明細表';
	 //$data['menu_v'] = 'main_menuno_v';
	   $data['content_v'] = 'pal/pali11_printa_v';
	 //$data['foot_v'] ='main_footno_v';
	   $this->load->vars($data);
	   $this->load->view('main_headprint_v');
	 //$this->load->view('pal/pali11_printa_v',$data);  
      }
    ...
    public function updform()   //修改輸入資料
      {
	    ...
	    //$this->db->get('invma');
	    $this->load->model('pal/pali11_model');
	    $data['result'] = $this->pali11_model->selone(urldecode(urldecode($this->uri->segment(4))),$this->uri->segment(5),$this->uri->segment(6));
		if(count($data['result'])!=1){
			...
			$data['menu_v'] = 'main_menuno_v';
			$data['content_v'] = 'pal/pali11_upd_v';
			$data['foot_v'] ='main_foot_v';
			$this->load->vars($data);
			$this->load->view('main_head_v');
		}
    ...
    public function see()   //看資料
      {      
	    ...
	    $data['menu_v'] = 'main_menuno_v';
	    $data['content_v'] = 'pal/pali11_see_v';
	    $data['foot_v'] ='main_foot_v';
	    $this->load->vars($data);
	    $this->load->view('main_head_v');
      }	
	...
	public function display_add($offset = 0,$func = "")  //欄位表頭排序
	  {
		...
		$select_col = array('`palmo`.mo002 as mt001disp');	
		$join = array();
		$where = array();
		if(isset($mt001)){$where[] = array('name'=>"mo001",'method'=>"is",'value'=>$mt001);}
		
		$this->load->model('pal/pali11_model');// 加載TABLE model 模型
		$data['result'] = $this->pali11_model->get_pali16($select_col,$join,$where);
	...
	public function display_upd($offset = 0,$func = "")  //欄位表頭排序
	  {
		...
		$this->load->model('pal/pali11_model');// 加載TABLE model 模型
		$data['result'] = $this->pali11_model->get_pali11($select_col,$join,$where);
    ...
    public function del()
      {      
	    ...
	    $data['message'] = '刪除資料成功!';
	    $this->load->model('pal/pali11_model','',TRUE);
	    $this->pali11_model->deletef($seg1,$seg2);
	    $this->display();
       }
    ...
    public function ajax_del()
      {
	    ...
	    $this->load->model('pal/pali11_model','',TRUE);
		$ret = array();
		$ret['response'] = $this->pali11_model->deletef($mt001,$mt002,$mt003);
		$mt003 = strtotime($mt003);
		$mt003 = date('Y_n_j',$mt003);
		$ret['mt003'] = $mt003;
...
/* End of file pali11.php */
/* Location: ./application/controllers/pali11.php */
?>