...
class Pali05 extends CI_Controller {           //擴展類必须註明由母類擴展而來 (自訂類使用 ci 框架)
	...
	//自訂類預設執行函數 流覽資料  	
	  public function index()           
	    {                      
	      ...
	      $this->load->model('pal/pali05_model');     // 加載TABLE model 模型		
	      $result= $this->pali05_model->search($limit, $offset = 0 , $sort_by  , $sort_order ); //至model 取 mysql 資料 預設 15,0,ma001,desc
	    ...
	    //$config['NUM_LINKS'] = 2;   //分頁前後出現2個數字
	      ...
	      $config['base_url'] = site_url("pal/pali05/display/$sort_by/$sort_order");   //設定分頁url路徑
	   ...
	   // $this->load->library('table');//加載table函數
	    ...
	    //$data['find07']=$this->session->userdata('find07');
	      ...
	      $data['content_v'] = 'pal/pali05_brow_v';		
	  ...
	  public function display($sort_by = 'ma001', $sort_order = 'desc', $offset = 0)  
	    {		
	      ...
	      $data['sort_order'] = $sort_order;
	      $this->load->model('pal/pali05_model');// 加載TABLE model 模型		
	      $result= $this->pali05_model->search($limit, $offset , $sort_by  , $sort_order ); //至model 取 mysql 資料 預設 15,0,ma001,desc
      ...
      public function updform()   
        {
	     $data['message'] = '修改資料!';
	     $this->db->get('palma');
	     $this->load->model('pal/pali05_model');
	     $result = $this->pali05_model->selone();
	     $data['results'] = $result['rows'];
	     $data['username'] = $this->session->userdata('manager');
	     $data['systitle'] ='保費費率-修改資料';
	     $data['menu_v'] = 'main_menuno_v';
	     $data['content_v'] = 'pal/pali05_upd_v';
	     $data['foot_v'] ='main_foot_v';
	     $this->load->vars($data);
	...
	//自動更新保費功能
      public function auto_upd()   
        {
	     $data['message'] = '自動更新保費功能!';
	     $this->db->get('palma');
	     $this->load->model('pal/pali05_model');
	     $result = $this->pali05_model->selone();
	     $data['results'] = $result['rows'];
	     $data['username'] = $this->session->userdata('manager');
	     $data['systitle'] ='保費費率-自動更新保費功能';
	     $data['menu_v'] = 'main_menuno_v';
	     $data['content_v'] = 'pal/pali05_upd_v';
	     $data['foot_v'] ='main_foot_v';
	     $this->load->vars($data);
	...
	public function auto_update_ajax()
		{
			...
			$data['message'] = $this->pali05_model->auto_update_ajax();   //invlc 表
			
			echo json_encode($data['message']);
		
		}
}
/* End of file pali05.php */
/* Location: ./application/controllers/pali05.php */
?>