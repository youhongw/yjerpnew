...
class pali56 extends CI_Controller {           //擴展類必须註明由母類擴展而來 (自訂類使用 ci 框架)
	  ...
	  public function display($sort_by = 'te002', $sort_order = 'desc', $offset = 0)  
	    {
	      ...
	      $data['sort_order'] = $sort_order;
	      $this->load->model('pal/pali56_model');// 加載TABLE model 模型		
	      $result= $this->pali56_model->search($sort_by  , $sort_order,$dateo,$datec,$type,$epyo,$epyc); //至model 取 mysql 資料 預設 15,0,tf001,desc
	  ...
	  public function display_bak($sort_by = 'te002', $sort_order = 'desc', $offset = 0)  
	    {
	      ...
	      $data['sort_order'] = $sort_order;
	      $this->load->model('pal/pali56_model');// 加載TABLE model 模型		
	      $result= $this->pali56_model->search_bak( $sort_by  , $sort_order,$dateo,$datec,$type,$epyo,$epyc); //至model 取 mysql 資料 預設 15,0,tf001,desc
	      $data['results'] = $result['rows'];
		  $data['total_num'] = $result['total_num'];
	      $this->load->library('pagination');
	      $config = array();		
    ...
    public function write()   
      {
		if(!@$this->input->post('dateo')){$dateo = date("Ymd");}else{$dateo = $this->input->post('dateo');}
		if(!@$this->input->post('datec')){$datec = date("Ymd");}else{$datec = $this->input->post('datec');}
		if(!@$this->input->post('epyo')){$epyo = "";}else{$epyo = $this->input->post('epyo');}
		if(!@$this->input->post('epyc')){$epyc = "";}else{$epyc = $this->input->post('epyc');}
		if(!@$this->input->post('type')){$type = "A";}else{$type = $this->input->post('type');}
		if(!@$this->input->post('slice_func')){$slice_func = "1";}else{$slice_func = $this->input->post('slice_func');}
		preg_match_all('/\d/S',$dateo, $matches);  //處理日期字串
		$data['dateo'] = implode('',$matches[0]);
		preg_match_all('/\d/S',$datec, $matches);  //處理日期字串
		$data['datec'] = implode('',$matches[0]);
		...
		$data['username'] = $this->session->userdata('manager');
		$title = array();
		$datas = $this->pali56_model->search("me001"  , "",$dateo,$datec,$type,$epyo,$epyc); //至model 取 mysql 資料 預設 15,0,tf001,desc
		$datas = $datas['rows'];
		$epy_data = $this->pali56_model->get_epy_data(); //至model 取 mysql 資料 預設 15,0,tf001,desc
		//echo "<pre>";var_dump($epy_data);exit;
		if($slice_func==1){
			$title = array('部門名稱','刷卡日期','員工代號','員工姓名','刷卡時間','狀態');  //excel 表頭
			
			foreach($datas as $d_key => $d_val){
				foreach($d_val as $key=>$val){
					$time_str=" ";$status_str=" ";
					foreach($val->te003 as $k=>$v){
						$time_str .= $v." ";
					}
					foreach($val->status as $k=>$v){
						$status_str .= $v." ";
					}
					$final_result[] = array($val->me002,$val->te002,$val->te001,$val->mv002,$time_str,$status_str);
				}
			}
			//echo "<pre>";var_dump($final_result);exit;
		}else{
			$title = array('部門名稱','員工編號','員工姓名');  //excel 表頭
			$used_date = array();
			foreach($datas as $d_key => $d_val){
				//echo "<pre>";var_dump($d_val);exit;
				foreach($epy_data as $epy_key=>$epy_val){
					if(@$d_val[$epy_val->mv001]){
						$val = $d_val[$epy_val->mv001];
						if(!@$used_date[substr($d_key,4,4)]){
							$title[] = substr($d_key,0,4)."年".substr($d_key,4,2)."月".substr($d_key,6,2)."日";
							$title[] = null;
							$used_date[substr($d_key,4,4)] = "haha";
						}
						$result[$val->te001][0] = $val->me002;
						$result[$val->te001][1] = $val->te001;
						$result[$val->te001][2] = $val->mv002;
						...
						$result[$val->te001][] = $before_noon;
						$result[$val->te001][] = $after_noon;
					}else{
						if(!@$used_date[substr($d_key,4,4)]){
							$title[] = substr($d_key,0,4)."年".substr($d_key,4,2)."月".substr($d_key,6,2)."日";
							$title[] = null;
						...
						$result[$epy_val->mv001][0] = $epy_val->me002;
						$result[$epy_val->mv001][1] = $epy_val->mv001;
						$result[$epy_val->mv001][2] = $epy_val->mv002;
						$before_noon = " ";$after_noon = " ";
						
						$result[$epy_val->mv001][] = $before_noon;
						$result[$epy_val->mv001][] = $after_noon;
					}
				}
			}
			$final_result = array();
       ...
       $this->excel->writer($title,$final_result);    //讀取excel  
      }
    ...
    public function printa()   
      {
		...
		$data['message'] = '列印明細成功!';
		$this->load->model('pal/pali56_model');// 加載TABLE model 模型		
		$result= $this->pali56_model->search("me001"  , "",$dateo,$datec,$type,$epyo,$epyc); //至model 取 mysql 資料 預設 15,0,tf001,desc
		$data['results'] = $result['rows'];
		$data['total_num'] = $result['total_num'];
		//$data['num_results'] = $result['num_rows'];
		$this->load->library('pagination');
