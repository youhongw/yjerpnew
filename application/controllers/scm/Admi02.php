...
class Admi02 extends CI_Controller {           //擴展類必须註明由母類擴展而來 (自訂類使用 ci 框架)
	...
	public function display_search($offset = 0,$func = "")  
	  {
		...
		$limit = 15;    //每頁筆數
		$this->load->model('scm/admi02_model');// 加載TABLE model 模型
		$result= $this->admi02_model->construct_sql($limit, $offset ,$func); //至model 取 mysql 資料 預設 15,0,tf001,desc
		$data['results'] = $result['data'];
		$data['num_results'] = $result['num'];
		$data['numrow'] = $result['num'];// 總筆數 
		$data['page'] = $result['num']/$limit; // 總頁數
		//$data['sql'] = $_SESSION['admi02']['search']['sql'];
		...
		$config['base_url'] = site_url("scm/admi02/display_search");   //設定分頁url路徑
		/* 網址去除".html" 字串進行切割 陣列,  */
		...
		$data['systitle'] ='領料建立作業';		  
		$data['menu_v'] = 'main_menu_v';
		//$data['content_v'] = 'scm/admi02_brow_v';		
		//$data['foot_v'] ='main_foot_v';
		$this->load->vars($data);
		//$this->load->view('main_headbrow_v');	
        $this->load->view('scm/admi02_brow_v');			
	  }
	...
	/*  public function construct_sql($offset = 0,$func = ""){
		$limit = 15;
		$this->load->model('scm/admi02_model');// 加載TABLE model 模型
		$this->admi02_model->construct_sql($limit, $offset ,$func);
	  } */
      ...
      public function findform() {  
	      ...
	      $data['username'] = $this->session->userdata('manager');
	      $data['message'] = '';
	      $data['systitle'] ='領料-進階查詢';
	      $data['menu_v'] = 'main_menuno_v';
	      $data['content_v'] = 'scm/admi02_find_v';
	      $data['foot_v'] ='main_foot_v';
	      $this->load->vars($data);
	  ...
	  public function findsql($sort_by = 'mf001', $sort_order = 'desc', $offset = 0)  
	    {		
		  ...
		  else {$_SESSION['admi02_sql_term'] = '(mf001="") ';}
		  
		  if(@$_POST['find007']){
			$_SESSION['admi02_sql_sort'] = $_POST['find007'];
		   }
		  else {$_SESSION['admi02_sql_sort'] = 'mf001';}
	      $limit = 15;    //每頁筆數
	      $data['message'] = '資料流覽成功!';
	      $data['sort_by'] = $sort_by;
	      $data['sort_order'] = $sort_order;
	      $this->load->model('scm/admi02_model');// 加載TABLE model 模型		
	      $result= $this->admi02_model->findf($limit, $offset , $sort_by  , $sort_order ); //至model 取 mysql 資料 預設 15,0,mf001,desc
	    ...
	    //$config['NUM_LINKS'] = 2;   //分頁前後出現2個數字
	      ...
	      $config['base_url'] = site_url("scm/admi02/findsql/$sort_by/$sort_order");   //設定分頁url路徑
	...
	/*    public function clear_sql_term(){  
      ...
      public function addform()   
	   ...
	   $data['systitle'] ='領料-新增';
	  // $data['menu_v'] = 'main_menuno_v';
	  // $data['content_v'] = 'scm/admi02_add_v';
	  // $data['foot_v'] ='main_foot_v';
	   $this->load->vars($data);
	  // $this->load->view('main_head_v');
	     $this->load->view('scm/admi02_add_v');
        }
      ...
      public function addsave()  { 
	   $data['username'] = $this->session->userdata('manager');
       $data['message'] = '新增成功!';
       $this->load->model('scm/admi02_model','',TRUE);
	 ...
	 //  $this->load->view('main_head_v');
      ...
      public function write()  { 
	     ...
	     $data['username'] = $this->session->userdata('manager');
         $this->load->model('scm/admi02_model','',TRUE);
	     $title = array('領料代號','領料名稱','領料密碼','群組代號','超級領料','部門代號','建立日期');  //excel 表頭
      ...
      public function printdetail()    {
	   $data['username'] = $this->session->userdata('manager');
	   $data['message'] = '';
	   $data['systitle'] ='領料-印明細表';
	   $data['menu_v'] = 'main_menuno_v';
	   $data['content_v'] = 'scm/admi02_print_v';
	   $data['foot_v'] ='main_foot_v';
	   $this->load->vars($data);
	   $this->load->view('main_head_v');
      }
      ...
      public function printa()   {
	   ...
	   $data['username'] = $this->session->userdata('manager');
	   $data['systitle'] ='領料-印明細表';
	   $data['content_v'] = 'scm/admi02_printa_v';
	   $this->load->vars($data);
      ...
      public function updsave() {
	      $seq1 = $this->uri->segment(4);
			...
			if(isset($_SESSION['admi02']['search'])){
				$current_index = $_SESSION['admi02']['search']['view'][$seq1];
				if($current_index!=0){
					$data['prev'] = $_SESSION['admi02']['search']['index'][$current_index-1];
				}
				if(isset($_SESSION['admi02']['search']['index'][$current_index+1])){
					$data['next'] = $_SESSION['admi02']['search']['index'][$current_index+1];
				}
				...
				if($this->uri->segment(3)=="display" || $this->uri->segment(3)=="display_leave"){
					$this->session->set_userdata('admi02_search',$this->uri->segment(3)."/".$this->uri->segment(4)."/".$this->uri->segment(5)."/".$offset);
				}
			} 
	...
	//	echo var_dump($seq1);exit;
	       ...
	       $this->load->model('scm/admi02_model');
	       $data['results'] = $this->admi02_model->selone($this->uri->segment(4));
	       $data['username'] = $this->session->userdata('manager');
	       $data['systitle'] ='領料-修改';
	     //  $data['menu_v'] = 'main_menuno_v';
	     //  $data['content_v'] = 'scm/admi02_upd_v';
	      // $data['foot_v'] ='main_foot_v';
	       $this->load->vars($data);
	      // $this->load->view('main_head_v');
	       $this->load->view('scm/admi02_upd_v');
        }
      ...
      public function updform()  { 
		...
		//以下暫存view處理，上一筆下一筆用
			...
			if(isset($_SESSION['admi02']['search'])){
				...
				$offset = floor($current_index/15)*15;
				$temp_ident = explode('/',$this->session->userdata('admi02_search'));
				$this->session->set_userdata('admi02_search',"display_search/".$offset);
				if($temp_ident[0]=="display"){
					$this->session->set_userdata('admi02_search',"display/mf001/asc/".$offset);
				}
				if($temp_ident[0]=="display_leave"){
					$this->session->set_userdata('admi02_search',"display_leave/mf001/asc/".$offset);
				}
			}
	    $data['message'] = '查詢一筆修改資料!';
	    $this->load->model('scm/admi02_model');
	    $data['results'] = $this->admi02_model->selone($seq1);
	    $data['username'] = $this->session->userdata('manager');
	    $data['systitle'] ='領料-修改';
	 //   $data['menu_v'] = 'main_menuno_v';
	  //  $data['content_v'] = 'scm/admi02_upd_v';
	 //   $data['foot_v'] ='main_foot_v';
	  ...
	  //  $this->load->view('main_head_v');
       ...
       public function see()  {   //看資料
	    ...
	    $data['message'] = '查看一筆資料!';
	    $this->load->model('scm/admi02_model');
	    $data['result'] = $this->admi02_model->selone($seq1);
	    $data['username'] = $this->session->userdata('manager');
	    $data['systitle'] ='領料-查看資料';
	  //  $data['menu_v'] = 'main_menuno_v';
	  //  $data['content_v'] = 'scm/admi02_see_v';
	  //  $data['foot_v'] ='main_foot_v';
	    $this->load->vars($data);
	  //  $this->load->view('main_head_v');
		$this->load->view('scm/admi02_upd_v');
	  ...
	  //提示改輸入資料重複 領料代號  mf001
	   public function check_key()  {
	     extract($this->input->get());
		$this->load->model('scm/admi02_model','',TRUE);
		$data = $this->admi02_model->check_key($mf001);
	  ...
	  public function check_title_no(){
		...
		extract($this->input->get());
		$this->load->model('scm/admi02_model','',TRUE);
		$data = $this->admi02_model->check_title_no($seq1);
		//echo "<pre>";var_dump($data);exit;
		echo $data;
	  }
}
