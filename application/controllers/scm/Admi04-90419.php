...
class Admi04 extends CI_Controller {           //擴展類必须註明由母類擴展而來 (自訂類使用 ci 框架)
	...
	/*  public function construct_sql($offset = 0,$func = ""){
	  ...
	  public function findsql($sort_by = 'mf001', $sort_order = 'desc', $offset = 0)  
	    {		
		  ...
		  else {$_SESSION['admi04_sql_term'] = '(mf001="") ';}
		  
		  if(@$_POST['find007']){
			$_SESSION['admi04_sql_sort'] = $_POST['find007'];
		   }
		  else {$_SESSION['admi04_sql_sort'] = 'mf001';}
	      $limit = 15;    //每頁筆數
	      $data['message'] = '資料流覽成功!';
	      $data['sort_by'] = $sort_by;
	      $data['sort_order'] = $sort_order;
	      $this->load->model('scm/admi04_model');// 加載TABLE model 模型		
	      $result= $this->admi04_model->findf($limit, $offset , $sort_by  , $sort_order ); //至model 取 mysql 資料 預設 15,0,mf001,desc
	...
	/*    public function clear_sql_term(){  
	 ...
	 //  $this->load->view('main_head_v');
      ...
      public function write()  { 
	     ...
	     $data['username'] = $this->session->userdata('manager');
         $this->load->model('scm/admi04_model','',TRUE);
	     $title = array('生產入庫代號','生產入庫名稱','生產入庫密碼','群組代號','超級生產入庫','部門代號','建立日期');  //excel 表頭
	      ...
	      $seq1 = $this->uri->segment(4);
			...
			if(isset($_SESSION['admi04']['search'])){
				...
				if($this->uri->segment(3)=="display" || $this->uri->segment(3)=="display_leave"){
					$this->session->set_userdata('admi04_search',$this->uri->segment(3)."/".$this->uri->segment(4)."/".$this->uri->segment(5)."/".$offset);
				}
			} 
	...
	//	echo var_dump($seq1);exit;
	       ...
	       $data['results'] = $this->admi04_model->selone($this->uri->segment(4));
	       $data['username'] = $this->session->userdata('manager');
	       $data['systitle'] ='生產入庫-修改';
	     //  $data['menu_v'] = 'main_menuno_v';
	     //  $data['content_v'] = 'scm/admi04_upd_v';
	      // $data['foot_v'] ='main_foot_v';
	       $this->load->vars($data);
	      // $this->load->view('main_head_v');
		...
		//以下暫存view處理，上一筆下一筆用
			...
			if(isset($_SESSION['admi04']['search'])){
				...
				if($temp_ident[0]=="display"){
					$this->session->set_userdata('admi04_search',"display/mf001/asc/".$offset);
				}
				if($temp_ident[0]=="display_leave"){
					$this->session->set_userdata('admi04_search',"display_leave/mf001/asc/".$offset);
				}
			}
	 ...
	 //   $data['foot_v'] ='main_foot_v';
	  ...
	  //提示改輸入資料重複 生產入庫代號  mf001
	   public function check_key()  {
	     extract($this->input->get());
		$this->load->model('scm/admi04_model','',TRUE);
		$data = $this->admi04_model->check_key($mf001);
