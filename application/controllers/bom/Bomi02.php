...
class Bomi02 extends CI_Controller
{           //擴展類必须註明由母類擴展而來 (自訂類使用 ci 框架)
	...
	public function display_search($offset = 0, $func = "")
	{
		...
		$limit = 15;    //每頁筆數
		$this->load->model('bom/bomi02_model'); // 加載TABLE model 模型
		//echo "<pre>";var_dump($limit);exit;

		$result = $this->bomi02_model->construct_sql($limit, $offset, $func); //至model 取 mysql 資料 預設 15,0,tf001,desc
		$data['results'] = $result['data'];
		$data['num_results'] = $result['num'];
		$data['numrow'] = $result['num']; // 總筆數 
		$data['page'] = $result['num'] / $limit; // 總頁數
		$data['sql'] = $_SESSION['bomi02']['search']['sql'];  //顯示sql語法
		...
		$config['base_url'] = site_url("bom/bomi02/display_search");   //設定分頁url路徑
		...
		$data['systitle'] = 'BOM單建立作業';
		$data['menu_v'] = 'main_menu_v';
		$data['content_v'] = 'bom/bomi02_brow_v';
	...
	public function edit_child()   //開子視窗
	{
		...
		$data['content_v'] = 'funnew/bomi02a_child_v';
	...
	public function lookup()
	{
		...
		$this->load->model('bom/bomi02_model');
		$query = $this->bomi02_model->lookup(urldecode(urldecode($this->uri->segment(4)))); //Search DB 
		...
		if ('IS_AJAX') {
			...
			$this->load->view('bom/bomi02_model/lookup', $data);
	...
	{
		$this->load->model('bom/bomi02_model');
		$data['result'] = $this->bomi02_model->ajaxpurq04a($this->uri->segment(4));
		$Result = $data['result'];
		$this->load->vars($data);
		echo  $Result;
	}
	...
	{
		$this->load->model('bom/bomi02_model');
		$data['result'] = $this->bomi02_model->ajaxcmsq05a($this->uri->segment(4));
		$Result = $data['result'];
		$this->load->vars($data);
		echo  $Result;
	}
	...
	{
		$this->load->model('bom/bomi02_model');
		$data['result'] = $this->bomi02_model->ajaxcmsq02a($this->uri->segment(4));
		$Result = $data['result'];
		$this->load->vars($data);
		echo  $Result;
	}
	...
	{
		$this->load->model('bom/bomi02_model');
		$data['result'] = $this->bomi02_model->ajaxpalq01a($this->uri->segment(4));
		$Result = $data['result'];
		$this->load->vars($data);
		echo  $Result;
	}
	...
	{
		$this->load->model('bom/bomi02_model');
		$data['result'] = $this->bomi02_model->ajaxchkno1($this->uri->segment(4), $this->uri->segment(5));
		$Result = $data['result'];
		$this->load->vars($data);
		echo  $Result;
	}
	...
	public function filter1($sort_by = 'mc001', $sort_order = 'desc', $offset = 0)   ////篩選資料
	{
		...
		$this->load->model('bom/bomi02_model', '', TRUE);   //true 系統會使用反單引號（`, 在英文鍵盤左上方）來保護你的欄位或資料表名稱
		$result = $this->bomi02_model->filterf1($limit, $offset, $sort_by, $sort_order);
		$data['message'] = '篩選資料成功!';
		...
		$config['base_url'] = site_url("bom/bomi02/filter1/$sort_by/$sort_order/$seq6/$seq7");  //設定分頁url路徑
	...
	public function findform()   //進階查詢輸入資料
	{
		...
		$data['menu_v'] = 'main_menuno_v';
		$data['content_v'] = 'bom/bomi02_find_v';
		$data['foot_v'] = 'main_foot_v';
		$this->load->vars($data);
		$this->load->view('main_head_v');
	}
	...
	public function findsql($sort_by = 'mc001', $sort_order = 'desc', $offset = 0)  //進階查詢流覽資料
	{
		...
		if (@$_POST['find005']) {
			$_SESSION['bomi02_sql_term'] = $_POST['find005'];
		} else {
			$_SESSION['bomi02_sql_term'] = '(mc001="") ';
		}
		if (@$_POST['find007']) {
			$_SESSION['bomi02_sql_sort'] = $_POST['find007'];
		} else {
			$_SESSION['bomi02_sql_sort'] = 'mc001';
		}
		...
		$data['sort_order'] = $sort_order;
		$this->load->model('bom/bomi02_model'); // 加載TABLE model 模型		
		$result = $this->bomi02_model->findf($limit, $offset, $sort_by, $sort_order); //至model 取 mysql 資料 預設 15,0,mc001,desc
		$data['results'] = $result['rows'];
		$data['num_results'] = $result['num_rows'];
		$this->load->library('pagination');
		$data['numrow'] = $result['num_rows']; // 總筆數 
		...
		$config['base_url'] = site_url("bom/bomi02/findsql/$sort_by/$sort_order");   //設定分頁url路徑
		$config['total_rows'] = $result['num_rows']; // 總筆數
		$config['per_page'] = $limit;                //每頁筆數
	...
	{  //清除條件
		...
		if (@$_SESSION["bomi02_sql_term"]) {
			unset($_SESSION["bomi02_sql_term"]);
		}
		if (@$_SESSION["bomi02_sql_sort"]) {
			unset($_SESSION["bomi02_sql_sort"]);
		}
		$this->display();
	}
	public function addform()   //新增輸入資料
	{
		...
		$coldata = $this->seti01_model->get_detail_view($this->session->userdata('manager'), 'bomi02');
		if ($coldata == "no_data") {
		...
		$data['message'] = '';
		$data['username'] = $this->session->userdata('manager');
		$data['systitle'] = 'BOM單資料-新增資料';
		$data['menu_v'] = 'main_menuno_v';
		$data['content_v'] = 'bom/bomi02_add_v';
		$data['foot_v'] = 'main_foot_v';
		$this->load->vars($data);
		$this->load->view('main_head_v');
	}
	...
	public function addsave()   //新增存檔
	{
		...
		$data['message'] = '新增成功!';
		$action = $this->bomi02_model->insertf();
		if ($action === 'exist') {
			$data['message'] = '資料重複!';
		} else {
			$this->bomi02_model->auto_print();
		}
		$data['systitle'] = 'BOM單資料-新增資料';
		$data['menu_v'] = 'main_menuno_v';
		$data['content_v'] = 'bom/bomi02_add_v';
	...
	public function copyform()   //複製資料輸入
	{
		...
		$data['menu_v'] = 'main_menuno_v';
		$data['content_v'] = 'bom/bomi02_copy_v';
		$data['foot_v'] = 'main_foot_v';
		$this->load->vars($data);
		$this->load->view('main_head_v');
	}
	...
	public function copysave()   //複製存檔
	{
		...
		$this->load->model('bom/bomi02_model', '', TRUE);
		$data['message'] = '複製成功!';
		$action = $this->bomi02_model->copyf($mc001o, $mc001c);
	...
	public function exceldetail()   //轉excel明細輸入起迄資料
	{
		...
		$data['menu_v'] = 'main_menuno_v';
		$data['content_v'] = 'bom/bomi02_excel_v';
		$data['foot_v'] = 'main_foot_v';
		$this->load->vars($data);
		$this->load->view('main_head_v');
	}
	...
	public function write()   //轉excel 部份資料由 print_v call
	{
		...
		$result1 = $this->bomi02_model->excelnewf();
	...
	public function printdetail()   //印明細起迄資料輸入
	{
		...
		$data['menu_v'] = 'main_menuno_v';
		$data['content_v'] = 'bom/bomi02_print_v';
		$data['foot_v'] = 'main_foot_v';
		$this->load->vars($data);
		$this->load->view('main_head_v');
	}
	public function printdetailc()   //印明細起迄資料輸入
	{
		...
		$data['systitle'] = 'BOM單資料-印明細表';
		$data['menu_v'] = 'main_menuno_v';
		$data['content_v'] = 'bom/bomi02_print1_v';
		$data['foot_v'] = 'main_foot_v';
		$this->load->vars($data);
		$this->load->view('main_head_v');
	}
	public function printc()   //印BOM單
	{
		...
		$this->load->model('bom/bomi02_model', '', TRUE);
		$data['message'] = '列印BOM單!';
		$result = $this->bomi02_model->printfc();
		...
		//   $data['systitle'] ='BOM單資料-印明細表';
		//$data['menu_v'] = 'main_menuno_v';
		//   $data['content_v'] = 'bom/bomi02_printb_v';
		//  $data['foot_v'] ='main_footno_v';
		$this->load->vars($data);
		//  $this->load->view('main_headprint_v');
		$this->load->view('bom/bomi02_printc_v');
	}
	public function printbb($ta009c)   //印BOM單
	{
		...
		$data['message'] = '列印BOM單!';
		//公司參數
		$result1 = $this->bomi02_model->companyf();
		$data['results1'] = $result1['rows1'];

		$result = $this->bomi02_model->printfb();
		$data['results'] = $result['rows'];
		$this->load->vars($data);
		$this->load->view('bom/bomi02_printb_v');
	}
	public function printa()   //印明細
	{
		...
		$this->load->model('bom/bomi02_model', '', TRUE);
		$data['message'] = '列印明細成功!';
		$result = $this->bomi02_model->printfd();
		$data['results'] = $result['rows'];
		$data['num_results'] = $result['num_rows'];
		$this->load->library('pagination');
		$data['numrow'] = $result['num_rows']; // 總筆數 
		...
		$data['systitle'] = 'BOM單資料-印明細表';
		//$data['menu_v'] = 'main_menuno_v';
		$data['content_v'] = 'bom/bomi02_printa_v';
		//$data['foot_v'] ='main_footno_v';
		$this->load->vars($data);
		$this->load->view('main_headprint_v');
		//$this->load->view('bom/bomi02_printa_v',$data);  
	}
	...
	public function updsave()   //修改存檔
	{
		...
		if (isset($_SESSION['bomi02']['search'])) {
			$current_index = @$_SESSION['bomi02']['search']['view'][$seg1];
			if ($current_index != 0) {
				$data['prev'] = $_SESSION['bomi02']['search']['index'][$current_index - 1];
			}
			if (isset($_SESSION['bomi02']['search']['index'][$current_index + 1])) {
				$data['next'] = $_SESSION['bomi02']['search']['index'][$current_index + 1];
			}
			...
			if ($this->uri->segment(3) == "display") {
				$this->session->set_userdata('bomi02_search', $this->uri->segment(3) . "/" . $this->uri->segment(4) . "/" . $this->uri->segment(5) . "/" . $offset);
			}
			...
			$this->load->model('bom/bomi02_model', '', TRUE);
			$this->load->vars($data);
			$this->bomi02_model->updatef();
			// $data['seq1'] = $this->uri->segment(4); 
			$data['message'] = '儲存完畢!';
			$this->load->model('bom/bomi02_model');
			$data['result'] = $this->bomi02_model->selone($seg1);
			...
			$data['menu_v'] = 'main_menuno_v';
			$data['content_v'] = 'bom/bomi02_upd_v';
			$data['foot_v'] = 'main_foot_v';
			$this->load->vars($data);
			$this->load->view('main_head_v');
		}
	...
	public function updform()   //修改輸入資料
	{
		...
		if (isset($_SESSION['bomi02']['search']) && isset($_SESSION['bomi02']['search']['view'][$view_str])) {
			$current_index = $_SESSION['bomi02']['search']['view'][$view_str];
			if ($current_index != 0) {
				$data['prev'] = $_SESSION['bomi02']['search']['index'][$current_index - 1];
			}
			if (isset($_SESSION['bomi02']['search']['index'][$current_index + 1])) {
			...
			$offset = floor($current_index / 15) * 15;
			$temp_ident = explode('/', $this->session->userdata('bomi02_search'));
			$this->session->set_userdata('bomi02_search', "display_search/" . $offset);
			if ($temp_ident[0] == "display") {
				$this->session->set_userdata('bomi02_search', "display/mc002/desc/" . $offset);
			}
		}
		...
		if ($data['result'] == "no_data") {
			redirect('bom/bomi02/' . $this->session->userdata('bomi02_search'));
			exit;
		}
	...
	public function see()   //看資料
	{
		...
		if (isset($_SESSION['bomi02']['search']) && isset($_SESSION['bomi02']['search']['view'][$view_str])) {
			...
			$this->session->set_userdata('bomi02_search', "display_search/" . $offset);
			if ($temp_ident[0] == "display") {
				$this->session->set_userdata('bomi02_search', "display/ta002/desc/" . $offset);
			}
		}
		...
		$data['menu_v'] = 'main_menuno_v';
		$data['content_v'] = 'bom/bomi02_see_v';
		$data['foot_v'] = 'main_foot_v';
		$this->load->vars($data);
		$this->load->view('main_head_v');
	}
	...
	public function del()   //刪除單筆 暫存
	{
		...
		$data['message'] = '刪除資料成功!';
		$this->load->model('bom/bomi02_model', '', TRUE);
		$this->bomi02_model->deletef($seg1, $seg2);
		$this->display();
	}
	...
	public function printb()   //印單據選取
	{
		...
		$this->load->model('bom/bomi02_model', '', TRUE);
		$data['message'] = '列印單據成功!';
		$result = $this->bomi02_model->printfd1($this->uri->segment(4), $this->uri->segment(5));
		$data['results'] = $result['rows'];
		$data['num_results'] = $result['num_rows'];
		$this->load->library('pagination');
		$data['numrow'] = $result['num_rows']; // 總筆數 
	...
	public function delete_detail()
	{
		$data['message'] = '刪除資料成功!';
		$this->load->model('bom/bomi02_model', '', TRUE);
		$this->bomi02_model->del_detail();
		redirect('bom/bomi02/updform/' . $_POST['del_md001'] . '/' . $_POST['del_md002']);   //重新整理
	}
	/*==以下AJAX處理區域==*/
	//抓取最新一筆的編號
	//提示輸入資料 key 是否重複 	
	public function check_key()
	{
		...
		$this->load->model('bom/bomi02_model', '', TRUE);
		$data = $this->bomi02_model->check_key($mc001);
	...
	public function check_title_no()
	{
		extract($this->input->get());
		$this->load->model('bom/bomi02_model', '', TRUE);
		$data = $this->bomi02_model->check_title_no($invi02);
		echo $data;
	}
	//刪除單筆細項AJAX
	public function del_detail_ajax()
	{
		...
		$data['message'] = '刪除資料成功!';
		$this->load->model('bom/bomi02_model', '', TRUE);
		echo $this->bomi02_model->deletedetailf($seg1, $seg2, $seg3);
	}
}
/* End of file bomi02.php */
/* Location: ./application/controllers/bomi02.php */
