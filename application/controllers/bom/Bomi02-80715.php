...
class Bomi02 extends CI_Controller {           //擴展類必须註明由母類擴展而來 (自訂類使用 ci 框架)
	  ...
	  public function index()           //自訂類預設執行函數 流覽資料
	    {                      
	      ...
	      $result= $this->bomi02_model->search($limit, $offset = 0 , $sort_by  , $sort_order ); //至model 取 mysql 資料 預設 15,0,mc001,desc
	   ...
	   //   $this->pagination->initialize($config);    //分頁初始化 display 3
	      $config['base_url'] = site_url("bom/bomi02/display/$sort_by/$sort_order");   //設定分頁url路徑
	  ...
	  public function display($sort_by = 'mc001', $sort_order = 'desc', $offset = 0)  //欄位表頭排序
	    {		
	      ...
	      $data['sort_order'] = $sort_order;
	      $this->load->model('bom/bomi02_model');// 加載TABLE model 模型		
	      $result= $this->bomi02_model->search($limit, $offset , $sort_by  , $sort_order ); //至model 取 mysql 資料 預設 15,0,mc001,desc
	  ...
	  public function findsql($sort_by = 'mc001', $sort_order = 'desc', $offset = 0)  //進階查詢流覽資料
	    {		
		  ...
		  else {$_SESSION['bomi02_sql_term'] = '(mc001="") ';}
		  
		  if(@$_POST['find007']){
			$_SESSION['bomi02_sql_sort'] = $_POST['find007'];
		  }
		  else {$_SESSION['bomi02_sql_sort'] = 'mc001';}
		...
		if (@session_status() == PHP_SESSION_NONE) {
		  ...
		  if(@$_SESSION["bomi02_sql_term"]) {unset($_SESSION["bomi02_sql_term"]);}
		  if(@$_SESSION["bomi02_sql_sort"]) {unset($_SESSION["bomi02_sql_sort"]);}
		  $this->display();
	  }
      ...
      public function copysave()   //複製存檔
        {
         ...
         $this->load->model('bom/bomi02_model','',TRUE);
	     $data['message'] = '複製成功!';
         $action = $this->bomi02_model->copyf();
      ...
      public function updsave()   //修改存檔
        {
	     ...
	     redirect('bom/bomi02/'.$this->session->userdata('search1'));
        }
      ...
      public function updform()   //修改輸入資料
        {
	      ...
	      $this->load->model('bom/bomi02_model');
	      $data['result'] = $this->bomi02_model->selone($this->uri->segment(4),$this->uri->segment(5));
	      $data['username'] = $this->session->userdata('manager');
	...
	public function delete_detail(){
	    ...
	    redirect('bom/bomi02/updform/'.$_POST['del_md001']);   //重新整理
	}
}
