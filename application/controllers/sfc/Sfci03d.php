...
class sfci03d extends CI_Controller
{           //擴展類必须註明由母類擴展而來 (自訂類使用 ci 框架 第一個字母大寫)
	...
	public function display_search($offset = 0, $func = "")
	{
		...
		$result = $this->sfci03_model->constructd_sql($limit, $offset, $func); //至model 取 mysql 資料 預設 15,0,tf001,desc
		$data['results'] = $result['data'];
		$data['num_results'] = $result['num'];
		$data['numrow'] = $result['num']; // 總筆數 
		$data['page'] = $result['num'] / $limit; // 總頁數
		$data['sql'] = $_SESSION['sfci03d']['search']['sql'];  //顯示sql語法
		...
		$config['base_url'] = site_url("sfc/sfci03d/display_search");   //設定分頁url路徑
	...
	public function displayr($offset = 0, $func = "")
	{
		...
		$result = $this->sfci03_model->construct_sqlr($limit, $offset, $func, "D%04"); //至model 取 mysql 資料 預設 15,0,tf001,desc
		$data['results'] = $result['data'];
		$data['num_results'] = $result['num'];
		$data['numrow'] = $result['num']; // 總筆數 
		$data['page'] = $result['num'] / $limit; // 總頁數
		...
		$config['cur_tag_close'] = '</a>'; // 當前頁结束樣式
		$config['cur_page'] = $this->uri->segment(4, 0);   //當前頁 結合分頁url路徑 +1 url第四段無就置放0
		$config['base_url'] = site_url("sfc/sfci03d/displayr");   //設定分頁url路徑
		/* 網址去除".html" explode 字串進行切割 陣列,  */
		$temp_url = explode(".html", $config['base_url']);
		$config['base_url'] = "";
		foreach ($temp_url as $key => $val) {
		...
		$data['curpage'] = $this->uri->segment(4, 1);   //當前頁第4無時顯示 1
		$data['limit'] = $limit;    //每頁筆數
		$data['systitle'] = '注塑-生產工價報表查詢';
		$data['menu_v'] = 'main_menu_v';
		// $data['content_v'] = 'sfc/sfci03d_brow_v';
		// $data['foot_v'] = 'main_foot_v';
		$this->load->vars($data);
		// $this->load->view('main_headbrow_v');
		$this->load->view('sfc/sfci03d_browr_v');
	}
	...
	public function exceldetailr()   //轉excel明細輸入起迄資料, 改報表轉出
	{
		...
		$data['message'] = '';
		$data['username'] = $this->session->userdata('manager');
		$data['systitle'] = '注塑-生產工價報表-列印';
		$this->load->vars($data);
		$this->load->view('sfc/sfci03d_excelr_v');
	}
	...
	public function addform()   //新增輸入資料
	{
		...
		$data['content_v'] = 'sfc/sfci03d_add_v';
		$data['foot_v'] = 'main_foot_v';
		// echo "<pre>";
	...
	public function updsave()   //修改存檔
	{
		...
		if ($data['result'] == "no_data") {
			redirect('sfc/sfci03d/' . $this->session->userdata('sfci03_search'));
			exit;
		}


		$data['systitle'] = '注塑-生產日報單-修改資料';
		$data['menu_v'] = 'main_menuno_v';
		$data['content_v'] = 'sfc/sfci03d_upd_v';
		$data['foot_v'] = 'main_foot_v';
		$this->load->vars($data);
		$this->load->view('main_head_v');
	}
	...
	public function updform()   //修改輸入資料
	{
		...
		$data['result'] = $this->sfci03_model->selone($seg1, $seg2);
		if ($data['result'] == "no_data") {
			redirect('sfc/sfci03d/' . $this->session->userdata('sfci03d_search'));
			exit;
		}
	...
	{  //清除條件
		if (session_status() == PHP_SESSION_NONE) {
			session_start();
			unset($_SESSION["sfci03d_sql_term"]);
		}
