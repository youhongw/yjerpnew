...
class Sfci17a extends CI_Controller
{           //擴展類必须註明由母類擴展而來 (自訂類使用 ci 框架)
	...
	{
		$this->load->model('sfc/sfci17a_model'); // 加載TABLE model 模型
		// $result = $this->sfci17a_model->temp_sql(); //至model 取 mysql 資料 預設 15,0,tf001,desc
		$this->display();
	}
	//欄位表頭排序資料流覽 	  
	...
	public function display_search($offset = 0, $func = "")
	{
		...
		$limit = 10;    //每頁筆數
		$this->load->model('sfc/sfci17a_model'); // 加載TABLE model 模型
		$result = $this->sfci17a_model->construct_sql($limit, $offset, $func); //至model 取 mysql 資料 預設 15,0,tf001,desc
		$data['results'] = $result['data'];
		$data['num_results'] = $result['num'];
		$data['numrow'] = $result['num']; // 總筆數 
		$data['page'] = $result['num'] / $limit; // 總頁數
		//$data['sql'] = $_SESSION['sfci17a']['search']['sql'];
		...
		$config['base_url'] = site_url("sfc/sfci17a/display_search");   //設定分頁url路徑
		/* 網址去除".html" 字串進行切割 陣列,  */
		...
		$data['content_v'] = 'sfc/sfci17a_brow_v';
		$data['foot_v'] = 'main_foot_v';
		$this->load->vars($data);
		$this->load->view('main_headbrow_v');
		// $this->load->view('sfc/sfci17a_brow_v');
	}
	...
	public function addform()
	{
		...
		$this->load->vars($data);
		$this->load->view('sfc/sfci17a_add_v');
	}
	...
	public function addsave()
	{
		...
		$data['username'] = $this->session->userdata('manager');
		$this->load->model('sfc/sfci17a_model', '', TRUE);
		$action = $this->sfci17a_model->selone1(trim($this->input->post('da001')), trim($this->input->post('da013')), trim($this->input->post('da014')));

		if ($action) {
			if ($this->sfci17a_model->insertf()) {
	...
	public function write()
	{
		...
		$result1 = $this->sfci17a_model->excelnewf();
	...
	public function printdetail()
	{
		...
		$data['content_v'] = 'sfc/sfci17a_print_v';
		$data['foot_v'] = 'main_foot_v';
		$this->load->vars($data);
	...
	public function printa()
	{
		...
		$this->load->model('sfc/sfci17a_model', '', TRUE);
		$result = $this->sfci17a_model->printfd();
		$data['results'] = $result['rows'];
		$data['num_results'] = $result['num_rows'];
		$data['numrow'] = $result['num_rows']; // 總筆數 
		$data['username'] = $this->session->userdata('manager');
		$data['systitle'] = '進貨入庫-印明細表';
		$data['content_v'] = 'sfc/sfci17a_printa_v';
		$this->load->vars($data);
	...
	public function updsave()
	{
		...
		$data['result'] = $this->sfci17a_model->selone($seq1, $seq2, $seq3);

		if (count($data['result']) > 0) {
			if ($this->sfci17a_model->updatef()) {
		...
		$this->load->vars($data);
		$this->load->view('sfc/sfci17a_upd_v');
	}
	...
	public function updform()
	{
		...
		$this->load->model('sfc/sfci17a_model');
		$data['result'] = $this->sfci17a_model->selone($seq1, $seq2, $seq3);
		$data['username'] = $this->session->userdata('manager');
		$data['systitle'] = '模具-修改';
	...
	{   //看資料
		...
		if (isset($_SESSION['sfci17a']['search'])) {
			$current_index = $_SESSION['sfci17a']['search']['view'][$seq1];
			if ($current_index != 0) {
				$data['prev'] = $_SESSION['sfci17a']['search']['index'][$current_index - 1];
			}
			if (isset($_SESSION['sfci17a']['search']['index'][$current_index + 1])) {
				$data['next'] = $_SESSION['sfci17a']['search']['index'][$current_index + 1];
			}
			$offset = floor($current_index / 15) * 15;
			$temp_ident = explode('/', $this->session->userdata('sfci17a_search'));
			$this->session->set_userdata('sfci17a_search', "display_search/" . $offset);
			if ($temp_ident[0] == "display") {
				$this->session->set_userdata('sfci17a_search', "display/da001/asc/" . $offset);
			}
			if ($temp_ident[0] == "display_leave") {
				$this->session->set_userdata('sfci17a_search', "display_leave/da001/asc/" . $offset);
			}
		}
		$data['message'] = '查看一筆資料!';
		$this->load->model('sfc/sfci17a_model');
		$data['result'] = $this->sfci17a_model->selone($seq1);
		$data['username'] = $this->session->userdata('manager');
		$data['systitle'] = '進貨入庫-查看資料';
		//  $data['menu_v'] = 'main_menuno_v';
		//  $data['content_v'] = 'sfc/sfci17a_see_v';
		//  $data['foot_v'] ='main_foot_v';
		$this->load->vars($data);
		//  $this->load->view('main_head_v');
		$this->load->view('sfc/sfci17a_upd_v');
	...
	public function del()
	{
		...
		$data['message'] = '刪除資料成功!';
		$this->load->model('sfc/sfci17a_model', '', TRUE);
		$this->sfci17a_model->deletef($seg1, $seg2, $seg3, $seg4);
		$this->display();
	}
	...
	public function clear_sql_term()
	{
		...
		if (@$_SESSION["sfci17a_sql_term"]) {
			unset($_SESSION["sfci17a_sql_term"]);
		}
		if (@$_SESSION["sfci17a_sql_sort"]) {
			unset($_SESSION["sfci17a_sql_sort"]);
		}
		// $this->display_sql();
		$this->display();
	}
	...
	public function check_key()
	{
		...
		if ($seg1 != "") {
			$this->load->model('sfc/sfci17a_model', '', TRUE);
			$data = $this->sfci17a_model->selone2($seg1);
			$relust = $data ? 'Y' : 'N';
	...
	public function lookup_body_check()
	{
		...
		$this->load->model('sfc/sfci17a_model', '', TRUE);
		$data = $this->sfci17a_model->lookupd_body_check($seq1, $seq2, $seq3); //Search DB 
		echo $data;
	}
}

/* End of file sfci17a.php */
/* Location: ./application/controllers/sfci17a.php */