...
class sfci04 extends CI_Controller
{           //擴展類必须註明由母類擴展而來 (自訂類使用 ci 框架)
	...
	public function display_search($offset = 0, $func = "")  //欄位表頭排序
	{
		...
		$this->load->model('sfc/sfci04_model'); // 加載TABLE model 模型		
		$result = $this->sfci04_model->search($limit, $offset, $func); //至model 取 mysql 資料 預設 15,0,ta001,desc
		$data['results'] = $result['data'];
		$data['num_results'] = $result['num'];
		$this->load->library('pagination');
		$data['numrow'] = $result['num']; // 總筆數 
		$data['page'] = $result['num'] / $limit; // 總頁數 
		$data['sql'] = $_SESSION['sfci04']['search']['sql'];
		$config = array();
		...
		$config['base_url'] = site_url("sfc/sfci04/display_search");   //設定分頁url路徑
		...
		$data['systitle'] = '製令製程建立作業';
		$data['menu_v'] = 'main_menu_v';
		$data['content_v'] = 'sfc/sfci04_brow_v';
	...
	public function lookup()
	{
		...
		$this->load->model('sfc/sfci04_model');
		$query = $this->sfci04_model->lookup(urldecode(urldecode($this->uri->segment(4)))); //Search DB 
		...
		if ('IS_AJAX') {
			...
			$this->load->view('sfc/sfci04_model/lookup', $data);
	...
	public function lookupa()
	{
		...
		$data['response'] = 'false'; //Set default response 
		$this->load->model('sfc/sfci04_model');
		$query = $this->sfci04_model->lookupa(urldecode(urldecode($this->uri->segment(4)))); //Search DB 
		...
		if ('IS_AJAX') {
			...
			$this->load->view('sfc/sfci04_model/lookupa', $data);
			// $this->index; //Load html view of search results  
		}
	}
	...
	{
		$this->load->model('sfc/sfci04_model');
		$data['result'] = $this->sfci04_model->ajaxpurq04a($this->uri->segment(4));
		$Result = $data['result'];
		$this->load->vars($data);
		echo  $Result;
	}
	...
	{
		$this->load->model('sfc/sfci04_model');
		$data['result'] = $this->sfci04_model->ajaxcmsq05a($this->uri->segment(4));
		$Result = $data['result'];
		$this->load->vars($data);
		echo  $Result;
	}
	...
	{
		$this->load->model('sfc/sfci04_model');
		$data['result'] = $this->sfci04_model->ajaxcmsq02a($this->uri->segment(4));
		$Result = $data['result'];
		$this->load->vars($data);
		echo  $Result;
	}
	...
	{
		$this->load->model('sfc/sfci04_model');
		$data['result'] = $this->sfci04_model->ajaxpalq01a($this->uri->segment(4));
		$Result = $data['result'];
		$this->load->vars($data);
		echo  $Result;
	}
	...
	{
		$this->load->model('sfc/sfci04_model');
		$data['result'] = $this->sfci04_model->ajaxchkno1($this->uri->segment(4), $this->uri->segment(5));
		$Result = $data['result'];
		$this->load->vars($data);
		echo  $Result;
	}
	...
	public function filter1($sort_by = 'TA002', $sort_order = 'desc', $offset = 0)   ////篩選資料
	{
		if (session_status() == PHP_SESSION_NONE) {
			session_start();
		}
		...
		$seq6 = urldecode($this->uri->segment(6)); 	 //解決亂碼
		$seq7 = '1';
		$sort_order = (substr($sort_order, 0, 3) == 'asc') ? 'asc' : 'desc';  // if else  = ? :
		$data['sort_order'] = $sort_order;
		$this->load->model('sfc/sfci04_model', '', TRUE);   //true 系統會使用反單引號（`, 在英文鍵盤左上方）來保護你的欄位或資料表名稱
		$result = $this->sfci04_model->filterf1($limit, $offset, $sort_by, $sort_order);
		$data['message'] = '篩選資料成功!';
		...
		$config['base_url'] = site_url("sfc/sfci04/filter1/$sort_by/$sort_order/$seq6/$seq7");  //設定分頁url路徑
	...
	public function findform()   //進階查詢輸入資料
	{
		...
		$data['username'] = $this->session->userdata('manager');
		$data['message'] = '';
		$data['systitle'] = '製令製程建立作業-進階查詢';
		$data['menu_v'] = 'main_menuno_v';
		$data['content_v'] = 'sfc/sfci04_find_v';
		$data['foot_v'] = 'main_foot_v';
		$this->load->vars($data);
	...
	public function findsql($sort_by = 'ta001', $sort_order = 'desc', $offset = 0)  //進階查詢流覽資料
	{
		...
		$data['sort_order'] = $sort_order;
		$this->load->model('sfc/sfci04_model'); // 加載TABLE model 模型		
		$result = $this->sfci04_model->findf($limit, $offset, $sort_by, $sort_order); //至model 取 mysql 資料 預設 15,0,ta001,desc
		$data['results'] = $result['rows'];
		$data['num_results'] = $result['num_rows'];
		$this->load->library('pagination');
		$data['numrow'] = $result['num_rows']; // 總筆數 
		...
		$config['base_url'] = site_url("sfc/sfci04/findsql/$sort_by/$sort_order");   //設定分頁url路徑
		$config['total_rows'] = $result['num_rows']; // 總筆數
		$config['per_page'] = $limit;                //每頁筆數
		$config['uri_segment'] = 6;       //當前頁
		$this->pagination->initialize($config);
	...
	public function addform()   //新增輸入資料
	{
		...
		$data['message'] = '';
		$data['username'] = $this->session->userdata('manager');
		$data['systitle'] = '製令製程建立作業-新增資料';
		$data['menu_v'] = 'main_menuno_v';
		$data['content_v'] = 'sfc/sfci04_add_v';
		$data['foot_v'] = 'main_foot_v';
		$this->load->vars($data);
		$this->load->view('main_head_v');
	}
	...
	public function addsave()   //新增存檔
	{
		...
		$data['username'] = $this->session->userdata('manager');
		$this->load->model('sfc/sfci04_model', '', TRUE);
		$data['message'] = '新增成功!';
		$action = $this->sfci04_model->insertf();
		if ($action === 'exist') {
			$data['message'] = '資料重複!';
		}
		$data['systitle'] = '製令製程建立作業-新增資料';
	...
	public function copyform()   //複製資料輸入
	{
		$data['username'] = $this->session->userdata('manager');
		$data['message'] = '';
		$data['systitle'] = '製令製程建立作業-複製資料';
		$data['menu_v'] = 'main_menuno_v';
		$data['content_v'] = 'sfc/sfci04_copy_v';
		$data['foot_v'] = 'main_foot_v';
		$this->load->vars($data);
		$this->load->view('main_head_v');
	}
	...
	public function copysave()   //複製存檔
	{
		...
		$this->load->model('sfc/sfci04_model', '', TRUE);
		$data['message'] = '複製成功!';
		$action = $this->sfci04_model->copyf();
	...
	public function copybefore()   //前置單據存檔 預計生產量,庫別,7 製令別, 單號
	{
		...
		$data['message'] = '查詢一筆資料!';
		$this->load->model('sfc/sfci04_model');
		$data['result'] = $this->sfci04_model->selonebefore($this->uri->segment(4), $this->uri->segment(5));
		$data['username'] = $this->session->userdata('manager');
		$data['systitle'] = '製令製程建立作業-前置單據資料';
		$data['menu_v'] = 'main_menuno_v';
		$data['content_v'] = 'sfc/sfci04_add_v';
		$data['foot_v'] = 'main_foot_v';
		$this->load->vars($data);
	...
	public function exceldetail()   //轉excel明細輸入起迄資料
	{
		$data['message'] = '';
		$data['username'] = $this->session->userdata('manager');
		$data['systitle'] = '製令製程建立作業-轉excel檔';
		$data['menu_v'] = 'main_menuno_v';
		$data['content_v'] = 'sfc/sfci04_excel_v';
		$data['foot_v'] = 'main_foot_v';
		$this->load->vars($data);
		$this->load->view('main_head_v');
	}
	...
	public function write()   //轉excel 部份資料由 print_v call
	{
		...
		$result1 = $this->sfci04_model->excelnewf();
	...
	public function printdetail()   //印明細起迄資料輸入
	{
		$data['username'] = $this->session->userdata('manager');
		$data['message'] = '';
		$data['systitle'] = '製令製程建立作業-印明細表';
		$data['menu_v'] = 'main_menuno_v';
		$data['content_v'] = 'sfc/sfci04_print_v';
		$data['foot_v'] = 'main_foot_v';
		$this->load->vars($data);
		$this->load->view('main_head_v');
	}
	public function printdetailc()   //印明細起迄資料輸入
	{
		...
		$data['systitle'] = '製令製程建立作業-印明細表';
		$data['menu_v'] = 'main_menuno_v';
		$data['content_v'] = 'sfc/sfci04_print1_v';
		$data['foot_v'] = 'main_foot_v';
		$this->load->vars($data);
		$this->load->view('main_head_v');
	}
	public function printc()   //印製造命令
	{
		...
		$this->load->model('sfc/sfci04_model', '', TRUE);
		$data['message'] = '列印製造命令!';
		$result = $this->sfci04_model->printfc();
		...
		//   $data['systitle'] ='製令製程建立作業-印明細表';
		//$data['menu_v'] = 'main_menuno_v';
		//   $data['content_v'] = 'sfc/sfci04_printb_v';
		//  $data['foot_v'] ='main_footno_v';
		$this->load->vars($data);
		//  $this->load->view('main_headprint_v');
		$this->load->view('sfc/sfci04_printc_v');
	}
	public function printbb()   //印製造命令
	{
		$this->load->model('sfc/sfci04_model', '', TRUE);
		$data['message'] = '列印製造命令!';
		$result = $this->sfci04_model->printfb();
		$data['results'] = $result['rows'];
		$this->load->vars($data);
		$this->load->view('sfc/sfci04_printb_v');
	}
	public function printa()   //印明細
	{
		...
		$this->load->model('sfc/sfci04_model', '', TRUE);
		$data['message'] = '列印明細成功!';
		$result = $this->sfci04_model->printfd();
		$data['results'] = $result['rows'];
		$data['num_results'] = $result['num_rows'];
		$this->load->library('pagination');
		$data['numrow'] = $result['num_rows']; // 總筆數 
		...
		$data['systitle'] = '製令製程建立作業-印明細表';
		//$data['menu_v'] = 'main_menuno_v';
		$data['content_v'] = 'sfc/sfci04_printa_v';
		//$data['foot_v'] ='main_footno_v';
		$this->load->vars($data);
		$this->load->view('main_headprint_v');
		//$this->load->view('sfc/sfci04_printa_v',$data);  
	}
	...
	public function updsave()   //修改存檔
	{
		...
		$data['seq1'] = $this->input->post('TA001');
		$data['seq2'] = $this->input->post('TA002');
		...
		$this->load->model('sfc/sfci04_model', '', TRUE);
		$this->load->vars($data);
		$_SESSION['message1'] = $this->sfci04_model->updatef();


		redirect('sfc/sfci04/updform/' . $seq1 . '/' . $seq2);
	}
	...
	public function updform()   //修改輸入資料
	{
		...
		if (isset($_SESSION['sfci04']['search'])) {
			$current_index = $_SESSION['sfci04']['search']['view'][$view_str]; //2個Key使用$view_str
			if ($current_index != 0) {
				$data['prev'] = $_SESSION['sfci04']['search']['index'][$current_index - 1];
			}
			if (isset($_SESSION['sfci04']['search']['index'][$current_index + 1])) {
				$data['next'] = $_SESSION['sfci04']['search']['index'][$current_index + 1];
			}
			$offset = floor($current_index / 15) * 15;
			$temp_ident = explode('/', $this->session->userdata('sfci04_search'));
			$this->session->set_userdata('sfci04_search', "display_search/" . $offset);
			if ($temp_ident[0] == "display") {
				$this->session->set_userdata('sfci04_search', "display/" . $offset);
			}
			if ($temp_ident[0] == "display_leave") {
				$this->session->set_userdata('sfci04_search', "display_leave/" . $offset);
			}
		}
		...
		$this->load->model('sfc/sfci04_model');
		$data['result'] = $this->sfci04_model->selone($seq1, $seq2);
		$data['username'] = $this->session->userdata('manager');
		$data['systitle'] = '製令製程建立作業-修改資料';
		$data['menu_v'] = 'main_menuno_v';
		$data['content_v'] = 'sfc/sfci04_upd_v';
		$data['foot_v'] = 'main_foot_v';
		$this->load->vars($data);
		$this->load->view('main_head_v');
	}
	...
	public function see()   //看資料
	{
		...
		$data['result'] = $this->sfci04_model->selone($seq1, $seq2);
		$data['username'] = $this->session->userdata('manager');
		$data['systitle'] = '製令製程建立作業-查看資料';
		$data['menu_v'] = 'main_menuno_v';
		$data['content_v'] = 'sfc/sfci04_see_v';
		$data['foot_v'] = 'main_foot_v';
		$this->load->vars($data);
		$this->load->view('main_head_v');
	}
	...
	public function del()   //刪除單筆 暫存
	{
		...
		$data['message'] = '刪除資料成功!';
		$this->load->model('sfc/sfci04_model', '', TRUE);
		$this->sfci04_model->deletef($seg1, $seg2);
		$this->display();
	}
	...
	public function delete()   //刪除選取
	{
		$data['message'] = '刪除資料成功!';
		$this->load->model('sfc/sfci04_model', '', TRUE);
		$data['message'] = $this->sfci04_model->delmutif();
		$this->display();
	}
	public function printb()   //印單據選取
	{
		...
		$this->load->model('sfc/sfci04_model', '', TRUE);
		$data['message'] = '列印單據成功!';
		$result = $this->sfci04_model->printfd1($this->uri->segment(4), $this->uri->segment(5));
		$data['results'] = $result['rows'];
		$data['num_results'] = $result['num_rows'];
		$this->load->library('pagination');
		$data['numrow'] = $result['num_rows']; // 總筆數 
	...
	public function check_pnob()
	{
		$mq001 = $this->uri->segment(4);
		$pnob = $this->uri->segment(5);
		// echo "<pre>";var_dump($mq001);var_dump($pnob);exit;

		$this->load->model('sfc/sfci04_model', '', TRUE);
		$data = $this->sfci04_model->check_pnob($mq001, $pnob);
		//echo "<pre>";var_dump($data);exit;
		echo $data;
	}

	public function check_sfcta()
	{
		$mq001 = $this->uri->segment(4);
		$mq002 = $this->uri->segment(5);
		$ta003 = $this->uri->segment(6);
		// echo "<pre>";var_dump($mq001);var_dump($pnob);exit;

		$this->load->model('sfc/sfci04_model', '', TRUE);
		$data = $this->sfci04_model->check_sfcta($mq001, $mq002, $ta003);
		//echo "<pre>";var_dump($data);exit;
		echo $data;
	}
}
/* End of file sfci04.php */
/* Location: ./application/controllers/sfci04.php */
