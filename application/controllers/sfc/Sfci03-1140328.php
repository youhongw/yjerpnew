...
class sfci03 extends CI_Controller
{           //擴展類必须註明由母類擴展而來 (自訂類使用 ci 框架 第一個字母大寫)
	...
	public function display_search($offset = 0, $func = "")
	{
		...
		$result = $this->sfci03_model->construct_sql($limit, $offset, $func); //至model 取 mysql 資料 預設 15,0,tf001,desc
		...
		$config['base_url'] = site_url("sfc/sfci03/display_search");   //設定分頁url路徑
	...
	public function displayr($offset = 0, $func = "")
	{
		...
		$result = $this->sfci03_model->construct_sqlr($limit, $offset, $func); //至model 取 mysql 資料 預設 15,0,tf001,desc
		$data['results'] = $result['data'];
		$data['num_results'] = $result['num'];
		$data['numrow'] = $result['num']; // 總筆數 
		$data['page'] = $result['num'] / $limit; // 總頁數
		...
		$config['cur_tag_close'] = '</a>'; // 當前頁结束樣式
		$config['cur_page'] = $this->uri->segment(4, 0);   //當前頁 結合分頁url路徑 +1 url第四段無就置放0
		$config['base_url'] = site_url("sfc/sfci03/displayr");   //設定分頁url路徑
		/* 網址去除".html" explode 字串進行切割 陣列,  */
		$temp_url = explode(".html", $config['base_url']);
		$config['base_url'] = "";
		foreach ($temp_url as $key => $val) {
		...
		$this->load->vars($data);
		// $this->load->view('main_headbrow_v');
		$this->load->view('sfc/sfci03_browr_v');
	}
	...
	public function display_leave($offset = 0, $func = "")
	{
		...
		$limit = 15;    //每頁筆數
		$this->load->model('sfc/sfci03_model'); // 加載TABLE model 模型
		$result = $this->sfci03_model->construct_sql2($limit, $offset, $func); //至model 取 mysql 資料 預設 15,0,tf001,desc
		$data['results'] = $result['data'];
		$data['num_results'] = $result['num'];
		$data['numrow'] = $result['num']; // 總筆數 
		$data['page'] = $result['num'] / $limit; // 總頁數
		$data['sql'] = $_SESSION['sfci03']['search']['sql'];
		//$data['message'] = '資料瀏覽成功!<br>查詢條件:'.$data['sql']."<br>";
		$data['message'] = '資料瀏覽成功!';
		$data['sort_order'] = "desc";
		$this->load->library('pagination');
		...
		$config['base_url'] = site_url("sfc/sfci03/display_leave");   //設定分頁url路徑
		/* 網址去除".html" */
		$temp_url = explode(".html", $config['base_url']);
		$config['base_url'] = "";
		foreach ($temp_url as $key => $val) {
		...
		$data['menu_v'] = 'main_menu_v';
		$data['content_v'] = 'sfc/sfci03_browl_v';
		$data['foot_v'] = 'main_foot_v';
		$this->load->vars($data);
		$this->load->view('main_headbrow_v');
	}
	...
	public function lookup()
	{
		...
		$query = $this->sfci03_model->lookup(urldecode(urldecode($this->uri->segment(4)))); //Search DB 
		...
		if ('IS_AJAX') {
			...
			$this->load->view('sfc/sfci03_model/lookup', $data);
	...
	public function lookupa()
	{
		...
		$data['response'] = 'false'; //Set default response 
		$this->load->model('sfc/sfci03_model');
		$query = $this->sfci03_model->lookupa(urldecode(urldecode($this->uri->segment(4)))); //Search DB 
		...
		if ('IS_AJAX') {
			...
			$this->load->view('sfc/sfci03_model/lookupa', $data);
			// $this->index; //Load html view of search results  
		}
	}
	...
	/* 不用此功能 1060814	
	  public function datapurq04a()   //提示改輸入資料如 移轉單別   不更新網頁
          {
	        $this->load->model('sfc/sfci03_model');
	        $data['result'] = $this->sfci03_model->ajaxpurq04a($this->uri->segment(4));
            $Result = $data['result'];		  
	        $this->load->vars($data);
	        echo  $Result;
          }
	  ...
	  public function datacmsq05a()   //提示改輸入資料如 請購部門 不更新網頁
        {
	      $this->load->model('sfc/sfci03_model');
	      $data['result'] = $this->sfci03_model->ajaxcmsq05a($this->uri->segment(4));
          $Result = $data['result'];		  
	      $this->load->vars($data);
	      echo  $Result;
        }
	  ...
	  public function datacmsq02a()  //提示改輸入資料如 廠別 不更新網頁td010
        {
	      $this->load->model('sfc/sfci03_model');
	      $data['result'] = $this->sfci03_model->ajaxcmsq02a($this->uri->segment(4));
          $Result = $data['result'];		  
	      $this->load->vars($data);
	      echo  $Result;
        }
	  ...
	  public function datapalq01a()  //提示改輸入資料如 請購人員 不更新網頁td012
        {
	      $this->load->model('sfc/sfci03_model');
	      $data['result'] = $this->sfci03_model->ajaxpalq01a($this->uri->segment(4));
          $Result = $data['result'];		  
	      $this->load->vars($data);
	      echo  $Result;
        }
	  ...
	  public function datachkno1()   //提示改輸入資料如  移轉單號 不更新網頁td012
        {
	      $this->load->model('sfc/sfci03_model');
	      $data['result'] = $this->sfci03_model->ajaxchkno1($this->uri->segment(4),$this->uri->segment(5));
          $Result = $data['result'];		  
	      $this->load->vars($data);
	      echo  $Result;
        }  */
	...
	public function filter1($sort_by = 'td001', $sort_order = 'desc', $offset = 0)
	{
		...
		$this->load->model('sfc/sfci03_model', '', TRUE);   //true 系統會使用反單引號（`, 在英文鍵盤左上方）來保護你的欄位或資料表名稱
		$result = $this->sfci03_model->filterf1($limit, $offset, $sort_by, $sort_order);
		$data['message'] = '篩選資料成功!';
		...
		$config['base_url'] = site_url("sfc/sfci03/filter1/$sort_by/$sort_order/$seq6/$seq7");  //設定分頁url路徑
	...
	public function findform()   //進階查詢輸入資料
	{
		...
		$data['content_v'] = 'sfc/sfci03_find_v';
		$data['foot_v'] = 'main_foot_v';
		$this->load->vars($data);
	...
	public function findsql($sort_by = 'td001', $sort_order = 'desc', $offset = 0)  //進階查詢流覽資料
	{
		...
		if (@$_POST['find005']) {
			$_SESSION['sfci03_sql_term'] = $_POST['find005'];
		}
		if (@$_POST['find007']) {
			$_SESSION['sfci03_sql_sort'] = $_POST['find007'];
		}
		$limit = 15;    //每頁筆數
		$data['message'] = '資料流覽成功!';
		...
		$data['sort_order'] = $sort_order;
		$this->load->model('sfc/sfci03_model'); // 加載TABLE model 模型		
		$result = $this->sfci03_model->findf($limit, $offset, $sort_by, $sort_order); //至model 取 mysql 資料 預設 15,0,td001,desc
		$data['results'] = $result['rows'];
		$data['num_results'] = $result['num_rows'];
		$this->load->library('pagination');
		$data['numrow'] = $result['num_rows']; // 總筆數 
		...
		$config['base_url'] = site_url("sfc/sfci03/findsql/$sort_by/$sort_order");   //設定分頁url路徑
		$config['total_rows'] = $result['num_rows']; // 總筆數
		$config['per_page'] = $limit;                //每頁筆數
	...
	{  //清除條件
		...
		if (@$_SESSION["sfci03_sql_term"]) {
			unset($_SESSION["sfci03_sql_term"]);
		}
		if (@$_SESSION["sfci03_sql_sort"]) {
			unset($_SESSION["sfci03_sql_sort"]);
		}
		//1060809
		unset($_SESSION['sfci03']['search']['where']);
		unset($_SESSION['sfci03']['search']['order']);
		unset($_SESSION['sfci03']['search']['offset']);
		$this->display();
	}
	...
	public function addform()   //新增輸入資料
	{
		...
		$data['menu_v'] = 'main_menuno_v';
		$data['content_v'] = 'sfc/sfci03_add_v';
		$data['foot_v'] = 'main_foot_v';
		// echo "<pre>";
	...
	public function copyform()   //複製資料輸入
	{
		...
		$data['menu_v'] = 'main_menuno_v';
		$data['content_v'] = 'sfc/sfci03_copy_v';
		$data['foot_v'] = 'main_foot_v';
		$this->load->vars($data);
		$this->load->view('main_head_v');
	}
	...
	public function copysave()   //複製存檔
	{
		...
		$this->load->model('sfc/sfci03_model', '', TRUE);
		$data['message'] = '複製成功!';
		$action = $this->sfci03_model->copyf();
	...
	public function exceldetail()   //轉excel明細輸入起迄資料, 改報表轉出
	{
		...
		// $data['menu_v'] = 'main_menuno_v';
		// $data['content_v'] = 'sfc/sfci03_excel_v';
		// $data['foot_v'] = 'main_foot_v';
		// $this->load->vars($data);
		// $this->load->view('main_head_v');
		...
		$data['message'] = '';
		$data['username'] = $this->session->userdata('manager');
		$data['systitle'] = '生產日報單-轉excel';
		$this->load->vars($data);
		$this->load->view('sfc/sfci03_excel_v');
	}
	...
	public function write()   //轉excel 部份資料由 print_v call
	{
		...
		if ($seq2 == '1') { //溶解生產記錄表
			$this->sfci03_model->excelnewf_prs('溶解生產記錄表');
		} else if ($seq2 == '2') { //軸承孔檢查表
			$this->sfci03_model->excelnewf_pcl('軸承孔檢查表');
		} else if ($seq2 == '3') { //月報表
			$this->sfci03_model->excelnewf_month('月報表');
		} else if ($seq2 == '4') { //端面粗糙度檢查
			$this->sfci03_model->excelnewf_pclmn('端面粗糙度檢查');
		} else if ($seq2 == '5') { //健溢CNC日報表			
			$this->sfci03_model->excelnewf_cnc04('健溢CNC日報表');
		} else if ($seq2 == '6') { //機加工生產日報表
			$this->sfci03_model->excelnewf_cnc05('機加工生產日報表');
		} else if ($seq2 == '7') { //拋丸粗糙度測量表
			$this->sfci03_model->excelnewf_pclsa('拋丸粗糙度測量表');
		} else if ($seq2 == '8') { //拋丸打磨不良報表
			$result1 = $this->sfci03_model->excelnewf_cnc06();
		} else if ($seq2 == '9') { //實心胎一次硫化報表
			$this->sfci03_model->excelnewf_rw001('實心胎一次硫化日報表');
		} else if ($seq2 == '10') { //空心胎硫化報表
			$this->sfci03_model->excelnewf_rw002('空心胎硫化日報表');
		} else if ($seq2 == '11') { //萬馬力-日報表
			$this->sfci03_model->excelnewf_rw003('萬馬力日報表');
		} else if ($seq1 == 'D403' || $seq1 == 'D503') { //橡膠------〉從下拉選單：實心胎一次硫化、空心胎硫化

			redirect('/sfc/sfci03/exceldetail/'); //存檔後跳回正常之頁面
		} else {
            ...
            $this->load->model('sfc/sfci03_model');
			$result1 = $this->sfci03_model->excelnewf();
	...
	public function write_month()
	{
		...
		$result1 = $this->sfci03_model->excelnewf_month();
	...
	public function exceldetailr()   //轉excel明細輸入起迄資料, 改報表轉出
	{
		...
		$this->load->vars($data);
		$this->load->view('sfc/sfci03_excelr_v');
	}

	public function writer()   //轉excel 部份資料由 print_v call
	{
		$this->load->model('sfc/sfci03_model', '', TRUE);
		$data['message'] = '轉檔excel成功!';
		$data['username'] = $this->session->userdata('manager');
		...
		$result1 = $this->sfci03_model->excelnewfr();
	...
	//	 from SFCI03GP
		for ($i = 0; $i < count($result1); $i++) {
			foreach ($result1[$i] as $key => $value) {
				if ($key == 'TE02911' or $key == 'TE005disp' or $key == 'TE018' or $key == 'TE019' or $key == 'da002' or $key == 'TE009disp' or $key == 'MD002') {
					// $result1[$i][$key] = iconv("big-5", "utf-8", $value);
				...
				if ($key == 'TE0131' or $key == 'da0051' or $key == 'da0052' or $key == 'TE0132'
                    or $key == 'da002'	or $key == 'da005'			) 
			        {$result1[$i][$key] ='';}
			//	if ($key == 'da0051') {$result1[$i]['TE029'] =$result1[$i][$key];}
             //   if ($key == 'MD013m') {$result1[$i]['da015'] =$result1[$i][$key];}
              //  if ($key == 'da0051') {$result1[$i][$key] ='';}
	...
	public function printdetail()   //印明細起迄資料輸入
	{
		...
		$data['username'] = $this->session->userdata('manager');
		// $data['menu_v'] = 'main_menuno_v';
		// $data['content_v'] = 'sfc/sfci03_print_v';
		// $data['foot_v'] = 'main_foot_v';
		$this->load->vars($data);
		$this->load->view('sfc/sfci03_print_v');
	}
	...
	public function printdetailc()   //印明細起迄資料輸入(訂單一次筆列印)
	{
		...
		// $data['systitle'] = '移轉單建立-印明細表';
		// $data['menu_v'] = 'main_menuno_v';
		// $data['content_v'] = 'sfc/sfci03_print1_v';
		// $data['foot_v'] = 'main_foot_v';
		// $this->load->vars($data);
		// $this->load->view('main_head_v');
		...
		// var_dump($data);		exit;
		$this->load->vars($data);
		$this->load->view('sfc/sfci03_print1_v');
	}
	...
	public function printc()   //印工票單
	{
		...
		$data['title'] = '工票單';
		//公司參數
		$data['companyf'] = $this->sfci03_model->companyf();
		$data['results'] = $this->sfci03_model->printfc();
		// echo "<pre>";		var_dump($data);		exit;

		$this->load->vars($data);
		$this->load->view('sfc/sfci03_printc_v');
	}
	...
	public function printbb($ta009c)   //印移轉單
	{
		...
		$data['message'] = '列印移轉單!';
		//公司參數
		$result1 = $this->sfci03_model->companyf();
		$data['results1'] = $result1['rows1'];

		$result = $this->sfci03_model->printfb();
		$data['results'] = $result['rows'];
		$this->load->vars($data);
		$this->load->view('sfc/sfci03_printb_v');
	}
	...
	public function printa()   //印明細
	{
		...
		$this->load->model('sfc/sfci03_model', '', TRUE);
		$data['message'] = '列印明細成功!';
		$result = $this->sfci03_model->printfd();
		$data['results'] = $result['rows'];
		$data['num_results'] = $result['num_rows'];
		//$this->load->library('pagination');
		$data['numrow'] = $result['num_rows']; // 總筆數 
		$data['username'] = $this->session->userdata('manager');
		$data['systitle'] = '移轉單建立-印明細表';
		$data['content_v'] = 'sfc/sfci03_printa_v';
		$this->load->vars($data);
	...
	public function updsave()   //修改存檔
	{
		...
		if ($data['result'] == "no_data") {
			redirect('sfc/sfci03/' . $this->session->userdata('sfci03_search'));
			exit;
		}
		...
		$data['menu_v'] = 'main_menuno_v';
		$data['content_v'] = 'sfc/sfci03_upd_v';
		$data['foot_v'] = 'main_foot_v';
		$this->load->vars($data);
		$this->load->view('main_head_v');
	}
	...
	public function see()   //看資料
	{
		...
		if (isset($_SESSION['sfci03']['search']) && isset($_SESSION['sfci03']['search']['view'][$view_str])) {
			$current_index = $_SESSION['sfci03']['search']['view'][$view_str];
			if ($current_index != 0) {
				$data['prev'] = $_SESSION['sfci03']['search']['index'][$current_index - 1];
			}
			if (isset($_SESSION['sfci03']['search']['index'][$current_index + 1])) {
				$data['next'] = $_SESSION['sfci03']['search']['index'][$current_index + 1];
			}
			$offset = floor($current_index / 15) * 15;
			$temp_ident = explode('/', $this->session->userdata('sfci03_search'));
			$this->session->set_userdata('sfci03_search', "display_search/" . $offset);
			if ($temp_ident[0] == "display") {
				$this->session->set_userdata('sfci03_search', "display/td002/desc/" . $offset);
			}
		}
		...
		$data['menu_v'] = 'main_menuno_v';
		$data['content_v'] = 'sfc/sfci03_see_v';
		$data['foot_v'] = 'main_foot_v';
		$this->load->vars($data);
		$this->load->view('main_head_v');
	}
	...
	public function printb()   //印單據選取
	{
		$this->load->model('sfc/sfci03_model', '', TRUE);
		$data['message'] = '列印單據成功!';
		$result = $this->sfci03_model->printfd1($this->uri->segment(4), $this->uri->segment(5));
		$data['results'] = $result['rows'];
		$data['num_results'] = $result['num_rows'];
		$this->load->library('pagination');
		$data['numrow'] = $result['num_rows']; // 總筆數 
		$data['username'] = $this->session->userdata('manager');
		$data['systitle'] = '移轉單';
		$data['content_v'] = 'sfc/sfci03_printb_v';
		$this->load->vars($data);
		$this->load->view('main_headprint_v');
	}
	...
	public function display_child($offset = 0, $func = "")
	{
		...
		$data['message'] = '資料流覽成功!';
		$this->load->model('sfc/sfci03_model'); // 加載TABLE model 模型
		$result = $this->sfci03_model->construct_sql_sfcta($limit, $offset, $func); //至model 取 mysql 資料 預設 15,0,ma001,desc
		$data['results'] = $result['data'];
		$data['num_results'] = $result['num'];
		$this->load->library('pagination');
		$data['numrow'] = $result['num']; // 總筆數 
		...
		$config['base_url'] = site_url("sfc/sfci03/display_child");   //設定分頁url路徑
		/* 網址去除".html" */
		$temp_url = explode(".html", $config['base_url']);
		$config['base_url'] = "";
		foreach ($temp_url as $key => $val) {
		...
		$data['menu_v'] = 'main_menu_v';
		$data['content_v'] = 'funnew/sfci03_child_v';
		$data['foot_v'] = 'main_foot_v';
		$this->load->vars($data);
	...
	public function display_childa($offset = 0, $func = "")
	{
		...
		$data['message'] = '資料流覽成功!';
		$this->load->model('sfc/sfci03_model'); // 加載TABLE model 模型
		$result = $this->sfci03_model->construct_sqla($limit, $offset, $func); //至model 取 mysql 資料 預設 15,0,ma001,desc
		$data['results'] = $result['data'];
		$data['num_results'] = $result['num'];
		$this->load->library('pagination');
		$data['numrow'] = $result['num']; // 總筆數 
		...
		$config['cur_tag_close'] = '</a>'; // 當前頁结束樣式
		$config['cur_page'] = $this->uri->segment(4, 0);   //當前頁 結合分頁url路徑 +1  ,分頁初始化 display 3 + 2 + 1 = 6
		$config['base_url'] = site_url("sfc/sfci03/display_childa");   //設定分頁url路徑
		/* 網址去除".html" */
		$temp_url = explode(".html", $config['base_url']);
		$config['base_url'] = "";
		foreach ($temp_url as $key => $val) {
		...
		$data['systitle'] = '廠商基本資料建立作業';
		$data['menu_v'] = 'main_menu_v';
		$data['content_v'] = 'funnew/sfci03_childa_v';
		$data['foot_v'] = 'main_foot_v';
		$this->load->vars($data);
		$this->load->view('main_headchild_v');
	}
