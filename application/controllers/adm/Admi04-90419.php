...
class Admi04 extends CI_Controller {           //擴展類必须註明由母類擴展而來 (自訂類使用 ci 框架)
	...
	//自訂類預設執行函數 流覽資料	
	  public function index()          
	    {                      
	      ...
	      $this->load->model('adm/admi04_model');     // 加載TABLE model 模型		
	      $result= $this->admi04_model->search($limit, $offset = 0 , $sort_by  , $sort_order ); //至model 取 mysql 資料 預設 15,0,me001,desc
	    ...
	    //$config['NUM_LINKS'] = 2;   //分頁前後出現2個數字
	      ...
	      $config['base_url'] = site_url("adm/admi04/display/$sort_by/$sort_order");   //設定分頁url路徑
	   ...
	   // $this->load->library('table');//加載table函數
	    ...
	    //$data['find07']=$this->session->userdata('find07');
	      ...
	      $data['systitle'] ='群組建立作業';  //網頁抬頭顯示名稱
	      $data['menu_v'] = 'main_menu_v';
	      $data['content_v'] = 'adm/admi04_brow_v';		
	 ...
	 //欄位表頭排序 資料流覽 
	  public function display($sort_by = 'me001', $sort_order = 'desc', $offset = 0)  
	    {		
	      ...
	      $data['sort_order'] = $sort_order;
	      $this->load->model('adm/admi04_model');// 加載TABLE model 模型		
	      $result= $this->admi04_model->search($limit, $offset , $sort_by  , $sort_order ); //至model 取 mysql 資料 預設 15,0,me001,desc
	  ...
	  public function filter1($sort_by = 'me001', $sort_order = 'desc', $offset = 0)   
        {
	     ...
	     $data['sort_order'] = $sort_order;
	     $this->load->model('adm/admi04_model','',TRUE);   //true 系統會使用反單引號（`, 在英文鍵盤左上方）來保護你的欄位或資料表名稱
	     $result=$this->admi04_model->filterf1($limit, $offset , $sort_by  , $sort_order);
	     $data['message'] = '篩選資料成功!';	
	     $data['results'] = $result['rows'];
	     $data['num_results'] = $result['num_rows'];
	     $this->load->library('pagination');
	   ...
	   //$this->pagination->initialize($config);//分頁初始化 
	     $config['base_url'] = site_url("adm/admi04/filter1/$sort_by/$sort_order/$seq6/$seq7");  //設定分頁url路徑
	   ...
	   //$this->load->library('table');//加載table函數
	     ...
	     $this->load->vars($data);
	     $this->load->view('main_headbrow_v');
	   //$this->load->view('adm/admi04_v', $data);
          }
    ...
    public function findform()   
        {
	      ...
	      $data['username'] = $this->session->userdata('manager');
	      $data['message'] = '';
	      $data['systitle'] ='群組-進階查詢';
	      $data['menu_v'] = 'main_menuno_v';
	      $data['content_v'] = 'adm/admi04_find_v';
	      $data['foot_v'] ='main_foot_v';
	      $this->load->vars($data);
	  ...
	  public function findsql($sort_by = 'me001', $sort_order = 'desc', $offset = 0)  
	    {		
		  ...
		  if(@$_POST['find005']){
		   ...
		   else {$_SESSION['admi04_sql_term'] = '(me001="") ';}
		  if(@$_POST['find007']){
			$_SESSION['admi04_sql_sort'] = $_POST['find007'];
		  }
		    else {$_SESSION['admi04_sql_sort'] = 'me001';}
	      $limit = 15;    //每頁筆數
	      $data['message'] = '資料流覽成功!';
	      $data['sort_by'] = $sort_by;
	      $data['sort_order'] = $sort_order;
	      $this->load->model('adm/admi04_model');// 加載TABLE model 模型		
	      $result= $this->admi04_model->findf($limit, $offset , $sort_by  , $sort_order ); //至model 取 mysql 資料 預設 15,0,me001,desc
	      $data['results'] = $result['rows'];
	      $data['num_results'] = $result['num_rows'];
	      $this->load->library('pagination');
	      $data['numrow']=$result['num_rows'];// 總筆數 
	    ...
	    //$config['NUM_LINKS'] = 2;   //分頁前後出現2個數字
	      ...
	      $config['cur_page'] = $this->uri->segment(6,0);   //當前頁 結合分頁url路徑 +1
	      $this->pagination->initialize($config);    //分頁初始化 display 3
	      $config['base_url'] = site_url("adm/admi04/findsql/$sort_by/$sort_order");   //設定分頁url路徑
	      $config['total_rows'] = $result['num_rows']; // 總筆數
	      $config['per_page'] = $limit;                //每頁筆數
	      $config['uri_segment'] = 6;       //當前頁
	    //$this->load->library('table');//加載table函數
      ...
      public function addform()   
        {
	     ...
	     $data['message'] = '';
	     $data['username'] = $this->session->userdata('manager');
	     $data['systitle'] ='群組-新增資料';
	     $data['menu_v'] = 'main_menuno_v';
	     $data['content_v'] = 'adm/admi04_add_v';
	     $data['foot_v'] ='main_foot_v';
	     $this->load->vars($data);
	     $this->load->view('main_head_v');
        }
      ...
      public function addsave()   
        {
	     $data['username'] = $this->session->userdata('manager');
         //$this->db->get('invma');	
         $this->load->model('adm/admi04_model','',TRUE);
	     $data['message'] = '新增成功!';
	     $action = $this->admi04_model->insertf();
	     if ($action === 'exist')
	      {
	        $data['message'] = '資料重複!';		    
	      }
	...
	//複製資料輸入	
	 ...
	 //$data['date']= date("Ymd");
	     $data['username'] = $this->session->userdata('manager');
	     $data['message'] = '';
	     $data['systitle'] ='群組-複製資料';
	     $data['menu_v'] = 'main_menuno_v';
	     $data['content_v'] = 'adm/admi04_copy_v';
	     $data['foot_v'] ='main_foot_v';
	     $this->load->vars($data);
	     $this->load->view('main_head_v');
        }
      ...
      public function copysave()   
        {
         ...
         $this->load->model('adm/admi04_model','',TRUE);
	     $data['message'] = '複製成功!';
           $action = $this->admi04_model->copyf();
      ...
      public function exceldetail()   
        {
	     ...
	     $data['message'] = '';
	     $data['username'] = $this->session->userdata('manager');
	     $data['systitle'] ='群組-轉excel檔';
	     $data['menu_v'] = 'main_menuno_v';
	     $data['content_v'] = 'adm/admi04_excel_v';
	     $data['foot_v'] ='main_foot_v';
	     $this->load->vars($data);
	     $this->load->view('main_head_v');
        }
      ...
      public function write()   
        {
	     ...
	     $data['username'] = $this->session->userdata('manager');
	     $title = array('群組代號','群組名稱','群組權限代號','備  註','建立日期');  //excel 表頭
      ...
      public function printdetail()   
        {
	     $data['username'] = $this->session->userdata('manager');
	     $data['message'] = '';
	     $data['systitle'] ='群組-印明細表';
	     $data['menu_v'] = 'main_menuno_v';
	     $data['content_v'] = 'adm/admi04_print_v';
	     $data['foot_v'] ='main_foot_v';
	     $this->load->vars($data);
	     $this->load->view('main_head_v');
        }
      ...
      public function printa()   //印明細
        {
	     ...
	     $data['username'] = $this->session->userdata('manager');
	     $data['systitle'] ='群組-印明細表';
	     $data['content_v'] = 'adm/admi04_printa_v';
	     $this->load->vars($data);
      ...
      public function updsave()   
        {
         ...
         $this->load->model('adm/admi04_model','',TRUE);
	     $this->load->vars($data);
	     $this->admi04_model->updatef();
	     redirect('adm/admi04/'.$this->session->userdata('search'));
        }
      ...
      public function updform()   
        {
	     ...
	     $this->load->model('adm/admi04_model');
	     $data['result'] = $this->admi04_model->selone($this->uri->segment(4),$this->uri->segment(5));
	     $data['username'] = $this->session->userdata('manager');
	     $data['systitle'] ='群組-修改資料';
	     $data['menu_v'] = 'main_menuno_v';
	     $data['content_v'] = 'adm/admi04_upd_v';
	     $data['foot_v'] ='main_foot_v';
	     $this->load->vars($data);
	     $this->load->view('main_head_v');
        }
	...
	//看資料
      public function see()   
        {      
	     ...
	     $data['message'] = '查看一筆資料!';
	     $this->load->model('adm/admi04_model');
	     $data['result'] = $this->admi04_model->selone($this->uri->segment(4));
	     $data['username'] = $this->session->userdata('manager');
	     $data['systitle'] ='群組-查看資料';
	     $data['menu_v'] = 'main_menuno_v';
	     $data['content_v'] = 'adm/admi04_see_v';
	     $data['foot_v'] ='main_foot_v';
	     $this->load->vars($data);
	     $this->load->view('main_head_v');
        }
	...
	public function display_child($offset = 0,$func = "")  
	  {		
		...
		$data['message'] = '資料流覽成功!';
		$this->load->model('adm/admi04_model');// 加載TABLE model 模型		
		$result= $this->admi04_model->construct_sql($limit, $offset ,$func); //至model 取 mysql 資料 預設 15,0,mb001,desc
		...
		$config['base_url'] = site_url("adm/admi04/display_child");   //設定分頁url路徑
		/* 網址去除".html" */
		...
		$data['menu_v'] = 'main_menu_v';
		$data['content_v'] = 'funnew/admi04_child_v.php';		
		$data['foot_v'] ='main_foot_v';
		$this->load->vars($data);
	...
	//提示主鍵資料重複 	
	 public function checkkey()   
        {
	     $this->load->model('adm/admi04_model');
	     $data['result'] = $this->admi04_model->ajaxkey($this->uri->segment(4));
         $Result = $data['result'];		  
	     $this->load->vars($data);
	     echo  $Result;
        }
	//群組代號快速查詢
	public function lookup1_admi04(){
    ...
    /*    $query = $this->cmsi03_model->lookup(
		...
		$query = $this->admi04_model->lookup1(urldecode(urldecode($this->uri->segment(4)))); //Search DB 
        if( ! empty($query) )  
          {  
    ...
    /*    $query = $this->cmsi03_model->lookup(
		...
		$query = $this->admi04_model->lookup2(urldecode(urldecode($this->uri->segment(4)))); //Search DB 
        if( ! empty($query) )  
          {  
            $data['response'] = 'true'; //Set response  
            $data['message'] = array(); //Create array  
