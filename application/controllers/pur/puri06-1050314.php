	...
	//自訂類預設執行函數 流覽資料
	  public function index()           
	    {                      
	      ...
	      $result= $this->puri06_model->search($limit, $offset = 0 , $sort_by  , $sort_order ); //至model 取 mysql 資料 預設 15,0,ma001,desc
	...
	//欄位表頭排序 資料流覽  
	  public function display($sort_by = 'ma001', $sort_order = 'desc', $offset = 0)  
	    {		
	      ...
	      $data['sort_order'] = $sort_order;
	      $this->load->model('pur/puri06_model');// 加載TABLE model 模型		
	      $result= $this->puri06_model->search($limit, $offset , $sort_by  , $sort_order ); //至model 取 mysql 資料 預設 15,0,ma001,desc
    ...
    //進階查詢流覽資料 
	  public function findsql($sort_by = 'ma001', $sort_order = 'desc', $offset = 0)  
	    {		
	      ...
	      $data['sort_order'] = $sort_order;
	      $this->load->model('pur/puri06_model');// 加載TABLE model 模型		
	      $result= $this->puri06_model->findf($limit, $offset , $sort_by  , $sort_order ); //至model 取 mysql 資料 預設 15,0,ma001,desc
	      $data['results'] = $result['rows'];
	      $data['num_results'] = $result['num_rows'];
	      $this->load->library('pagination');
	      $data['numrow']=$result['num_rows'];// 總筆數 
      ...
      public function printa()   
        {
          $this->load->model('pur/puri06_model','',TRUE);
	      $data['message'] = '列印明細成功!';
          $result = $this->puri06_model->printfd();
	      $data['results'] = $result['rows'];
	      $data['num_results'] = $result['num_rows'];
	      $this->load->library('pagination');
	      $data['numrow']=$result['num_rows'];// 總筆數 
	      $data['username'] = $this->session->userdata('manager');
	      $data['systitle'] ='請購資料-印明細表';
	      $data['content_v'] = 'pur/puri06_printa_v';
	      $this->load->vars($data);
