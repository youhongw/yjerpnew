...
class Purq01a extends CI_Controller {           //擴展類必须註明由母類擴展而來 (自訂類使用 ci 框架)
	  ...
	  public function index()           //自訂類預設執行函數 流覽資料
	    {                      
	      ...
	      $this->load->model('pur/purq01a_model');     // 加載TABLE model 模型		
	    ...
	    //  $this->pagination->initialize($config);    //分頁初始化 display 3
	      $config['base_url'] = site_url("pur/purq01a/display/$sort_by/$sort_order");   //設定分頁url路徑
	   ...
	   // $this->load->library('table');//加載table函數
	    ...
	    //$data['find07']=$this->session->userdata('find07');
	      ...
	      $data['content_v'] = 'pur/purq01a_brow_v';		
	  ...
	  public function display($sort_by = 'mb001', $sort_order = 'asc', $offset = 0)  //欄位表頭排序
	    ...
	    //$this->load->library('table');//加載table函數
	      ...
	      $data['systitle'] ='庫別視窗查詢';		  
  	      $this->load->vars($data);
		  $this->load->view('pur/purq01a_child_v');	
	    } 
	   ...
	   $this->load->vars($data);
	    ...
	    //$this->pagination->initialize($config);    //分頁初始化 display 3 + 2 + 1 = 6
	      $config['base_url'] = site_url("pur/purq01a/filter1/$sort_by/$sort_order/$seq6/$seq7");   //設定分頁url路徑
	      $config['total_rows'] = $result['num_rows']; // 總筆數
	      $config['per_page'] = $limit;                //每頁筆數
	   ...
	   //   $this->load->view('main_headquery_v');		
	      ...
	      $data['menu_v'] = 'main_menuno_v';
	      $data['content_v'] = 'pur/purq01a_find_v';
	  ...
	  public function findsql($sort_by = 'ma001', $sort_order = 'desc', $offset = 0)  //進階查詢流覽資料
	    ...
	    //$config['NUM_LINKS'] = 2;   //分頁前後出現2個數字
	      ...
	      $config['cur_page'] = $this->uri->segment(6,0);   //當前頁 結合分頁url路徑 +1
	      $this->pagination->initialize($config);    //分頁初始化 display 3
	      $config['base_url'] = site_url("pur/purq01a/findsql/$sort_by/$sort_order");   //設定分頁url路徑
	      $config['total_rows'] = $result['num_rows']; // 總筆數
	      $config['per_page'] = $limit;                //每頁筆數
	      $config['uri_segment'] = 6;       //當前頁
	    //$this->load->library('table');//加載table函數
	...
	//$this->db->get('invma');
	  ...
	  $data['message'] = '刪除資料成功!';
	  $this->load->model('pur/purq01a_model','',TRUE);
