     ...
     //進階查詢流覽資料
	  public function findsql($sort_by = 'ma001', $sort_order = 'desc', $offset = 0)  
	    {		
		  ...
		  if(@$_POST['find005']){
			$_SESSION['puri01_sql_term'] = $_POST['find005'];
		 }
		  else {$_SESSION['puri01_sql_term'] = '(ma001="") ';}
		  if(@$_POST['find007']){
			$_SESSION['puri01_sql_sort'] = $_POST['find007'];
		 }
		  else {$_SESSION['puri01_sql_sort'] = 'ma001';}
		...
		if (session_status() == PHP_SESSION_NONE) {
		  ...
		  if(@$_SESSION["puri01_sql_term"]) {unset($_SESSION["puri01_sql_term"]);}
		  if(@$_SESSION["puri01_sql_sort"]) {unset($_SESSION["puri01_sql_sort"]);}
		  $this->display();
	  }
	 //新增輸入資料   
	...
	public function display_child($offset = 0,$func = "")  
	  {
		...
		$data['message'] = '資料流覽成功!';
		$this->load->model('pur/puri01_model');// 加載TABLE model 模型
		$result= $this->puri01_model->construct_sql($limit, $offset ,$func); //至model 取 mysql 資料 預設 15,0,ma001,desc
		$data['results'] = $result['data'];
		$data['num_results'] = $result['num'];
		$this->load->library('pagination');
		$data['numrow']=$result['num'];// 總筆數 
	  ...
	  {		
		...
		$data['message'] = '資料流覽成功!';
		$this->load->model('pur/puri01_model');// 加載TABLE model 模型		
		$result= $this->puri01_model->construct_sql($limit, $offset ,$func); //至model 取 mysql 資料 預設 15,0,mb001,desc
		$data['results'] = $result['data'];
		$data['num_results'] = $result['num'];
		$this->load->library('pagination');
		$data['numrow']=$result['num'];// 總筆數 
		...
		$data['menu_v'] = 'main_menu_v';
		$data['content_v'] = 'funnew/puri01a_child_v.php';		
		$data['foot_v'] ='main_foot_v';
		$this->load->vars($data);
		$this->load->view('main_headchild_v');		
	  }
