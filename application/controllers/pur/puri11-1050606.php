...
class Puri11 extends CI_Controller {           //擴展類必须註明由母類擴展而來 (自訂類使用 ci 框架)
	  ...
	  public function index()           //自訂類預設執行函數 流覽資料
	    {                      
	      ...
	      $result= $this->puri11_model->search($limit, $offset = 0 , $sort_by  , $sort_order ); //至model 取 mysql 資料 預設 15,0,ti001,desc
	    ...
	    //  $this->pagination->initialize($config);    //分頁初始化 display 3
	      $config['base_url'] = site_url("pur/puri11/display/$sort_by/$sort_order");   //設定分頁url路徑
	      ...
	      $data['systitle'] ='退貨單資料建立作業';  //網頁抬頭顯示名稱
	  ...
	  public function display($sort_by = 'ti001', $sort_order = 'desc', $offset = 0)  //欄位表頭排序
	    {		
	      ...
	      $data['sort_order'] = $sort_order;
	      $this->load->model('pur/puri11_model');// 加載TABLE model 模型		
	      $result= $this->puri11_model->search($limit, $offset , $sort_by  , $sort_order ); //至model 取 mysql 資料 預設 15,0,ti001,desc
      ...
      public function copysave()   //複製存檔
		  ...
		  public function copybefore()   //前置單據存檔
        {
	      ...
	      $data['message'] = '查詢一筆資料!';
	      $this->load->model('pur/puri11_model');
	      $data['result'] = $this->puri11_model->selonebefore($this->uri->segment(4),$this->uri->segment(5));
	      $data['username'] = $this->session->userdata('manager');
      ...
      public function printdetail()   //印明細起迄資料輸入
	   ...
	   //  $this->load->view('main_headprint_v');
		...
		public function auto_printbb(){    //自動列印
	      ...
	      $data['results'] = $result['rows'];
	     $this->load->vars($data);
	     $this->load->view('cop/puri11_printb_v');	
		}
      ...
      public function updform()   //修改輸入資料
        {
	      ...
	      $this->load->model('pur/puri11_model');
	      $data['result'] = $this->puri11_model->selone($this->uri->segment(4),$this->uri->segment(5));
	      $data['username'] = $this->session->userdata('manager');
