...
class Puri03 extends CI_Controller {           //擴展類必须註明由母類擴展而來 (自訂類使用 ci 框架)
	  ...
	  public function index()           //自訂類預設執行函數 流覽資料
	    {                      
	      ...
	      $result= $this->puri03_model->search($limit, $offset = 0 , $sort_by  , $sort_order ); //至model 取 mysql 資料 預設 15,0,tl001,desc
	   ...
	   //   $this->pagination->initialize($config);    //分頁初始化 display 3
	      $config['base_url'] = site_url("pur/puri03/display/$sort_by/$sort_order");   //設定分頁url路徑
	  ...
	  public function display($sort_by = 'tl002', $sort_order = 'desc', $offset = 0)  //欄位表頭排序
	    {		
	      $limit = 15;    //每頁筆數
	      $data['message'] = '資料流覽成功!';
	      $data['sort_by'] = $sort_by;
	      $data['sort_order'] = $sort_order;
	      $this->load->model('pur/puri03_model');// 加載TABLE model 模型		
	      $result= $this->puri03_model->search($limit, $offset , $sort_by  , $sort_order ); //至model 取 mysql 資料 預設 15,0,tl001,desc
	  ...
	  public function findsql($sort_by = 'tl001', $sort_order = 'desc', $offset = 0)  //進階查詢流覽資料
	    {	
		  ...
		  else {$_SESSION['admi05_sql_term'] = '(tl001="") ';}
		  if(@$_POST['find007']){
			$_SESSION['admi05_sql_sort'] = $_POST['find007'];
		    }
		  else {$_SESSION['admi05_sql_sort'] = 'tl001';}
      ...
      public function updsave()   //修改存檔
        {
	     ...
	     redirect('pur/puri03/'.$this->session->userdata('search1'));
        }
      ...
      public function updform()   //修改輸入資料
        {
	      ...
	      $this->load->model('pur/puri03_model');
	      $data['result'] = $this->puri03_model->selone($this->uri->segment(4),$this->uri->segment(5));
	      $data['username'] = $this->session->userdata('manager');
