...
class ipsi02 extends CI_Controller {           //擴展類必须註明由母類擴展而來 (自訂類使用 ci 框架 第一個字母大寫)
	  ...
	  public function display_search($offset = 0,$func = "")  
		...
		$limit = 15;    //每頁筆數
		$this->load->model('ips/ipsi02_model');// 加載TABLE model 模型
		//echo "<pre>";var_dump($limit);exit;
		
		$result= $this->ipsi02_model->construct_sql($limit, $offset ,$func); //至model 取 mysql 資料 預設 15,0,tf001,desc
		$data['results'] = $result['data'];
		$data['num_results'] = $result['num'];
		$data['numrow'] = $result['num'];// 總筆數 
		$data['page'] = $result['num']/$limit; // 總頁數
		$data['sql'] = $_SESSION['ipsi02']['search']['sql'];  //顯示sql語法
		...
		$config['base_url'] = site_url("ips/ipsi02/display_search");   //設定分頁url路徑
		...
		$data['systitle'] ='預付購料建立作業';		  
		$data['menu_v'] = 'main_menu_v';
		$data['content_v'] = 'ips/ipsi02_brow_v';		
	  ...
	  public function construct_sql($offset = 0,$func = ""){
		$limit = 15;
		$this->load->model('ips/ipsi02_model');// 加載TABLE model 模型
		$this->ipsi02_model->construct_sql($limit, $offset ,$func);
	  }
	  ...
	  public function display_leave($offset = 0,$func = "")  
	    {		
			...
			$limit = 15;    //每頁筆數
			$this->load->model('ips/ipsi02_model');// 加載TABLE model 模型
			$result= $this->ipsi02_model->construct_sql2($limit, $offset ,$func); //至model 取 mysql 資料 預設 15,0,tf001,desc
			$data['results'] = $result['data'];
			$data['num_results'] = $result['num'];
			$data['numrow'] = $result['num'];// 總筆數 
			$data['page'] = $result['num']/$limit; // 總頁數
			$data['sql'] = $_SESSION['ipsi02']['search']['sql'];
			//$data['message'] = '資料瀏覽成功!<br>查詢條件:'.$data['sql']."<br>";
			$data['message'] = '資料瀏覽成功!';
			$data['sort_order'] = "desc";
			$this->load->library('pagination');
			...
			$config['base_url'] = site_url("ips/ipsi02/display_leave");   //設定分頁url路徑
			/* 網址去除".html" */
			$temp_url = explode(".html",$config['base_url']);
			$config['base_url'] = "";
			foreach($temp_url as $key => $val){$config['base_url'].=$val;}
			...
			$data['menu_v'] = 'main_menu_v';
			$data['content_v'] = 'ips/ipsi02_browl_v';		
			$data['foot_v'] ='main_foot_v';
			$this->load->vars($data);
			$this->load->view('main_headbrow_v');	
	    }
	  ...
	  public function lookup(){
        ...
        $this->load->model('ips/ipsi02_model');
        $query = $this->ipsi02_model->lookup(urldecode(urldecode($this->uri->segment(4)))); //Search DB 
        ...
        if('IS_AJAX')  
         ...
         {  
		    $this->load->view('ips/ipsi02_model/lookup',$data); 
	  ...
	  public function lookupa(){
        ...
        $data['response'] = 'false'; //Set default response 
        $this->load->model('ips/ipsi02_model');
        $query = $this->ipsi02_model->lookupa(urldecode(urldecode($this->uri->segment(4)))); //Search DB 
        ...
        if('IS_AJAX')  
         ...
         {  
		    $this->load->view('ips/ipsi02_model/lookupa',$data); 
          // $this->index; //Load html view of search results  
         }  
        }  
	...
	/* 不用此功能 1060814	
	  public function datapurq04a()   //提示改輸入資料如 預付購料別   不更新網頁
          {
	        $this->load->model('ips/ipsi02_model');
	        $data['result'] = $this->ipsi02_model->ajaxpurq04a($this->uri->segment(4));
            $Result = $data['result'];		  
	        $this->load->vars($data);
	        echo  $Result;
          }
	  ...
	  public function datacmsq05a()   //提示改輸入資料如 請購部門 不更新網頁
        {
	      $this->load->model('ips/ipsi02_model');
	      $data['result'] = $this->ipsi02_model->ajaxcmsq05a($this->uri->segment(4));
          $Result = $data['result'];		  
	      $this->load->vars($data);
	      echo  $Result;
        }
	  ...
	  public function datacmsq02a()  //提示改輸入資料如 廠別 不更新網頁ta010
        {
	      $this->load->model('ips/ipsi02_model');
	      $data['result'] = $this->ipsi02_model->ajaxcmsq02a($this->uri->segment(4));
          $Result = $data['result'];		  
	      $this->load->vars($data);
	      echo  $Result;
        }
	  ...
	  public function datapalq01a()  //提示改輸入資料如 請購人員 不更新網頁ta012
        {
	      $this->load->model('ips/ipsi02_model');
	      $data['result'] = $this->ipsi02_model->ajaxpalq01a($this->uri->segment(4));
          $Result = $data['result'];		  
	      $this->load->vars($data);
	      echo  $Result;
        }
		
	  public function datachkno1()   //提示改輸入資料如  預付購料號 不更新網頁ta012
        {
	      $this->load->model('ips/ipsi02_model');
	      $data['result'] = $this->ipsi02_model->ajaxchkno1($this->uri->segment(4),$this->uri->segment(5));
          $Result = $data['result'];		  
	      $this->load->vars($data);
	      echo  $Result;
        }  */
	  ...
	  public function filter1($sort_by = 'ta001', $sort_order = 'desc', $offset = 0)   
        {
	     ...
	     $this->load->model('ips/ipsi02_model','',TRUE);   //true 系統會使用反單引號（`, 在英文鍵盤左上方）來保護你的欄位或資料表名稱
	     $result=$this->ipsi02_model->filterf1($limit, $offset , $sort_by  , $sort_order);
	     $data['message'] = '篩選資料成功!';	
	     ...
	     $config['base_url'] = site_url("ips/ipsi02/filter1/$sort_by/$sort_order/$seq6/$seq7");  //設定分頁url路徑
      ...
      public function findform()   //進階查詢輸入資料
        {
	      ...
	      $data['content_v'] = 'ips/ipsi02_find_v';
	      $data['foot_v'] ='main_foot_v';
	      $this->load->vars($data);
	  ...
	  public function findsql($sort_by = 'ta001', $sort_order = 'desc', $offset = 0)  //進階查詢流覽資料
	    {		
		  ...
		  if(@$_POST['find005']){
			$_SESSION['ipsi02_sql_term'] = $_POST['find005'];
		  }
		  if(@$_POST['find007']){
			$_SESSION['ipsi02_sql_sort'] = $_POST['find007'];
		  }
	      $limit = 15;    //每頁筆數
	      $data['message'] = '資料流覽成功!';
	      ...
	      $data['sort_order'] = $sort_order;
	      $this->load->model('ips/ipsi02_model');// 加載TABLE model 模型		
	      $result= $this->ipsi02_model->findf($limit, $offset , $sort_by  , $sort_order ); //至model 取 mysql 資料 預設 15,0,ta001,desc
	      $data['results'] = $result['rows'];
	      $data['num_results'] = $result['num_rows'];
	      $this->load->library('pagination');
	      $data['numrow']=$result['num_rows'];// 總筆數 
	    ...
	    //$config['NUM_LINKS'] = 2;   //分頁前後出現2個數字
	      ...
	      $config['base_url'] = site_url("ips/ipsi02/findsql/$sort_by/$sort_order");   //設定分頁url路徑
	      $config['total_rows'] = $result['num_rows']; // 總筆數
	      $config['per_page'] = $limit;                //每頁筆數
	  ...
	  public function clear_sql_term(){  //清除條件
		  ...
		  if(@$_SESSION["ipsi02_sql_term"]) {unset($_SESSION["ipsi02_sql_term"]);}
		  if(@$_SESSION["ipsi02_sql_sort"]) {unset($_SESSION["ipsi02_sql_sort"]);}
		  //1060809
		  unset($_SESSION['ipsi02']['search']['where']);
		  unset($_SESSION['ipsi02']['search']['order']);
		  unset($_SESSION['ipsi02']['search']['offset']);
		  $this->display();
	  }
      ...
      public function addform()   //新增輸入資料
        {
		 //Default columns 檢視明細設定
		  ...
		  $coldata = $this->seti01_model->get_detail_view($this->session->userdata('manager'), 'ipsi02');
		  if($coldata=="no_data"){
	     ...
	     $data['systitle'] ='預付購料-新增資料';
		   //系統參數
		// $this->load->model('ips/ipsi02_model','',TRUE);
		// $result2 = $this->ipsi02_model->funsysf();
	   //  $data['results2'] = $result2['rows2'];
		  
	     $data['menu_v'] = 'main_menuno_v';
	     $data['content_v'] = 'ips/ipsi02_add_v';
	     $data['foot_v'] ='main_foot_v';
	     $this->load->vars($data);
	     $this->load->view('main_head_v');
        }
      ...
      public function addsave()   //新增存檔
        {
          ...
          $this->load->model('ips/ipsi02_model','',TRUE);
	      $data['message'] = '新增成功!';
	      $action = $this->ipsi02_model->insertf();
	      if ($action === 'exist')
	       {
	        $data['message'] = '資料重複!';		    
	       }
		  else{
			  $this->ipsi02_model->auto_print();
		  }
      ...
      public function copyform()   //複製資料輸入
        {
	     ...
	     $data['menu_v'] = 'main_menuno_v';
	     $data['content_v'] = 'ips/ipsi02_copy_v';
	     $data['foot_v'] ='main_foot_v';
	     $this->load->vars($data);
	     $this->load->view('main_head_v');
        }
      ...
      public function copysave()   //複製存檔
        {
         ...
         $this->load->model('ips/ipsi02_model','',TRUE);
	     $data['message'] = '複製成功!';
         $action = $this->ipsi02_model->copyf();
     ...
     /*	 
      public function exceldetail()   //轉excel明細輸入起迄資料, 改報表轉出
        {
	     ...
	     $data['menu_v'] = 'main_menuno_v';
	     $data['content_v'] = 'ips/ipsi02_excel_v';
	     $data['foot_v'] ='main_foot_v';
	     $this->load->vars($data);
	     $this->load->view('main_head_v');
        } */
      ...
      public function write()   //轉excel 部份資料由 print_v call
        {
         ...
         $result1 = $this->ipsi02_model->excelnewf();	
      ...
      public function printdetail()   //印明細起迄資料輸入
        {
	     ...
	     $data['menu_v'] = 'main_menuno_v';
	     $data['content_v'] = 'ips/ipsi02_print_v';
	     $data['foot_v'] ='main_foot_v';
	     $this->load->vars($data);
	     $this->load->view('main_head_v');
        }
	  ...
	  public function printdetailc()   //印明細起迄資料輸入(訂單一次筆列印)
        {
	     ...
	     $data['systitle'] ='預付購料-印明細表';
	     $data['menu_v'] = 'main_menuno_v';
	     $data['content_v'] = 'ips/ipsi02_print1_v';
	     $data['foot_v'] ='main_foot_v';
	     $this->load->vars($data);
	     $this->load->view('main_head_v');
        }
	  ...
	  public function printc()   //印預付購料 訂單一次多筆列印
        {
	      ...
	      $data['message'] = '列印預付購料!';
		     //公司參數
		  $result1 = $this->ipsi02_model->companyf();
	      $data['results1'] = $result1['rows1'];
		  
          $result = $this->ipsi02_model->printfc();
	      $data['results'] = $result['rows'];
	      $this->load->vars($data);
	      $this->load->view('ips/ipsi02_printc_v');  
        }
		
		public function printbb($ta009c)   //印預付購料
        {
		  $data['paper9']=$ta009c;
          $this->load->model('ips/ipsi02_model','',TRUE);
	      $data['message'] = '列印預付購料!';
          ...
          $result = $this->ipsi02_model->printfb();
	      $data['results'] = $result['rows'];
	      $this->load->vars($data);
	      $this->load->view('ips/ipsi02_printb_v');
        }
      ...
      public function printa()   //印明細
        {
          ...
          $this->load->model('ips/ipsi02_model','',TRUE);
	      $data['message'] = '列印明細成功!';
          $result = $this->ipsi02_model->printfd();
	      $data['results'] = $result['rows'];
	      $data['num_results'] = $result['num_rows'];
	      //$this->load->library('pagination');
	      $data['numrow']=$result['num_rows'];// 總筆數 
	      $data['username'] = $this->session->userdata('manager');
	      $data['systitle'] ='預付購料-印明細表';
	      $data['content_v'] = 'ips/ipsi02_printa_v';
	      $this->load->vars($data);
      ...
      public function updsave()   //修改存檔
        {
		  ...
		  if(isset($_SESSION['ipsi02']['search'])){
				$current_index = @$_SESSION['ipsi02']['search']['view'][$seq1];
				if($current_index!=0){
					$data['prev'] = $_SESSION['ipsi02']['search']['index'][$current_index-1];
				}
				if(isset($_SESSION['ipsi02']['search']['index'][$current_index+1])){
					$data['next'] = $_SESSION['ipsi02']['search']['index'][$current_index+1];
				}
				$offset = floor($current_index/15)*15;
				$this->session->set_userdata('ipsi02_search',"display_search/".$offset);
				if($this->uri->segment(3)=="display"){
					$this->session->set_userdata('ipsi02_search',$this->uri->segment(3)."/".$this->uri->segment(4)."/".$this->uri->segment(5)."/".$offset);
				}
		   }
		   ...
		   $this->load->model('ips/ipsi02_model','',TRUE);
		   $this->load->vars($data);
		   $this->ipsi02_model->updatef();
		//   $data['seq1'] = $this->uri->segment(4); 
		   $data['message'] = '儲存完畢!';
		   $this->load->model('ips/ipsi02_model');
		   $data['result'] = $this->ipsi02_model->selone($seg1, $seg2);
		  ...
		  //Default columns 檢視明細設定
	       ...
	       $data['username'] = $this->session->userdata('manager');
		   $data['systitle'] ='預付購料建立作業-修改資料';
		   $data['menu_v'] = 'main_menuno_v';
		   $data['content_v'] = 'ips/ipsi02_upd_v';
		   $data['foot_v'] ='main_foot_v';
		   $this->load->vars($data);
		   $this->load->view('main_head_v');
        }
      ...
      public function updform()   //修改輸入資料
        {
		  ...
		  if(isset($_SESSION['ipsi02']['search'])&&isset($_SESSION['ipsi02']['search']['view'][$view_str])){
			  $current_index = $_SESSION['ipsi02']['search']['view'][$view_str];
			  if($current_index!=0){
				 $data['prev'] = $_SESSION['ipsi02']['search']['index'][$current_index-1];
			  }
			  if(isset($_SESSION['ipsi02']['search']['index'][$current_index+1])){
			  ...
			  $offset = floor($current_index/15)*15;
			  $temp_ident = explode('/',$this->session->userdata('ipsi02_search'));
			  $this->session->set_userdata('ipsi02_search',"display_search/".$offset);
			  if($temp_ident[0]=="display"){
				 $this->session->set_userdata('ipsi02_search',"display/ta002/desc/".$offset);
			  }
		  }
		  ...
		  if($data['result']=="no_data"){
			  redirect('ips/ipsi02/'.$this->session->userdata('ipsi02_search'));
			  exit;
		  }
      ...
      public function see()   //看資料
        {  
	      ...
	      $data['menu_v'] = 'main_menuno_v';
	      $data['content_v'] = 'ips/ipsi02_see_v';
	      $data['foot_v'] ='main_foot_v';
	      $this->load->vars($data);
	      $this->load->view('main_head_v');
        }
      ...
      public function del()   //刪除單筆 暫存 (置於修改右按鈕)
        {      
	      ...
	      $data['message'] = '刪除資料成功!';
	      $this->load->model('ips/ipsi02_model','',TRUE);
	      $this->ipsi02_model->deletef($seg1,$seg2);
	      $this->display();
        }
	  ...
	  public function printb()   //印單據選取
        {  
	      $this->load->model('ips/ipsi02_model','',TRUE);
	      $data['message'] = '列印單據成功!';
          $result = $this->ipsi02_model->printfd1($this->uri->segment(4),$this->uri->segment(5));
	      $data['results'] = $result['rows'];
	      $data['num_results'] = $result['num_rows'];
	      $this->load->library('pagination');
	      $data['numrow']=$result['num_rows'];// 總筆數 
	      $data['username'] = $this->session->userdata('manager');
	      $data['systitle'] ='預付購料';
	      $data['content_v'] = 'ips/ipsi02_printb_v';	   
	      $this->load->vars($data);
	      $this->load->view('main_headprint_v');
        }
	  ...
	  public function delete_detail(){
		$data['message'] = '刪除資料成功!';
		$this->load->model('ips/ipsi02_model','',TRUE);
		$this->ipsi02_model->del_detail();
	    redirect('ips/ipsi02/updform/'.$_POST['del_md001'].'/'.$_POST['del_md002']);   //重新整理
	  }
	  ...
	  public function display_child($offset = 0,$func = "") {
		...
		$data['message'] = '資料流覽成功!';
		$this->load->model('ips/ipsi02_model');// 加載TABLE model 模型
		$result= $this->ipsi02_model->construct_sql($limit, $offset ,$func); //至model 取 mysql 資料 預設 15,0,ma001,desc
		$data['results'] = $result['data'];
		$data['num_results'] = $result['num'];
		$this->load->library('pagination');
		$data['numrow']=$result['num'];// 總筆數 
		...
		$config['base_url'] = site_url("ips/ipsi02/display_child");   //設定分頁url路徑
		/* 網址去除".html" */
		$temp_url = explode(".html",$config['base_url']);
		$config['base_url'] = "";
		foreach($temp_url as $key => $val){$config['base_url'].=$val;}
		...
		$data['menu_v'] = 'main_menu_v';
		$data['content_v'] = 'funnew/ipsi02_child_v';		
		$data['foot_v'] ='main_foot_v';
		$this->load->vars($data);
	  ...
	  public function check_title_no(){
		...
		//echo var_dump($ta001.$ta008);exit;
		
		$this->load->model('ips/ipsi02_model','',TRUE);
		$data = $this->ipsi02_model->check_title_no($ipsi06,$ta008);
		echo $data;
	  }
      ...
      public function del_detail_ajax()   
        {
		  ...
		  $data['message'] = '刪除資料成功!';
		  $this->load->model('ips/ipsi02_model','',TRUE);
		  echo $this->ipsi02_model->deletedetailf($seg1,$seg2,$seg3);
        }
}
/* End of file ipsi02.php */
/* Location: ./application/controllers/ipsi02.php */
?>