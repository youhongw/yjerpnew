...
class ipsi05 extends CI_Controller {           //擴展類必须註明由母類擴展而來 (自訂類使用 ci 框架 第一個字母大寫)
	...
	/* 不用此功能 1060814	
	  public function datapurq04a()   //提示改輸入資料如 報關/贖單別   不更新網頁
          {
	        $this->load->model('ips/ipsi05_model');
	  ...
	  public function datachkno1()   //提示改輸入資料如  報關/贖單號 不更新網頁tg012
        {
	      $this->load->model('ips/ipsi05_model');
     ...
     /*	 
      ...
      public function write()   //轉excel 部份資料由 print_v call
        {
	     ...
	     $data['username'] = $this->session->userdata('manager');
	     $title = array('報關/贖單別','報關/贖單號','訂單日期','客戶代號','客戶名稱','序號','品號','品名','規格','單位','數量','單價','金額');  //excel 表頭
	  ...
	  public function printc()   //印報關/贖單 訂單一次多筆列印
        {
		  $data['paper9']=$this->input->post('tg009c');
          $this->load->model('ips/ipsi05_model','',TRUE);
	      $data['message'] = '列印報關/贖單!';
		     //公司參數
		...
		public function printbb($tg009c)   //印報關/贖單
        {
		  $data['paper9']=$tg009c;
          $this->load->model('ips/ipsi05_model','',TRUE);
	      $data['message'] = '列印報關/贖單!';
	  ...
	  public function check_title_no(){
		...
		$this->load->model('ips/ipsi05_model','',TRUE);
		$data = $this->ipsi05_model->check_title_no($ipsi06,$tg003);
		echo $data;
	  }
