...
class Noti13 extends CI_Controller {           //擴展類必须註明由母類擴展而來 (自訂類使用 ci 框架)
//票據科目設定作業	
	  ...
	  public function index()           //自訂類預設執行函數 流覽資料
	    {                      
	      ...
	      $this->load->model('not/noti13_model');     // 加載TABLE model 模型		
	      $result= $this->noti13_model->search($limit, $offset = 0 , $sort_by  , $sort_order ); //至model 取 mysql 資料 預設 15,0,mc001,desc
	    ...
	    //$config['NUM_LINKS'] = 2;   //分頁前後出現2個數字
	      ...
	      $config['base_url'] = site_url("not/noti13/display/$sort_by/$sort_order");   //設定分頁url路徑
	      ...
	      $data['systitle'] ='融資種類建立作業';  //網頁抬頭顯示名稱
	      $data['menu_v'] = 'main_menu_v';
	      $data['content_v'] = 'not/noti13_brow_v';		
	  ...
	  public function display($sort_by = 'mc001', $sort_order = 'desc', $offset = 0)  //欄位表頭排序
	    {
	      ...
	      $data['sort_order'] = $sort_order;
	      $this->load->model('not/noti13_model');// 加載TABLE model 模型		
	      $result= $this->noti13_model->search($limit, $offset , $sort_by  , $sort_order ); //至model 取 mysql 資料 預設 15,0,mc001,desc
	      $data['results'] = $result['rows'];
		  $temp_ary = array(1=>"1:L/C",2=>"2:INVOICE",3=>"3:應付商業本票/承兌匯票",4=>"4:應收票據",5=>"5:資產抵押",9=>"9:其他");
		  foreach($data['results'] as $val){
			  $val->mc003 = $temp_ary[$val->mc003];
		  }
	  ...
	  public function lookup(){
        ...
        $this->load->model('not/noti13_model');
        $query = $this->noti13_model->lookup(urldecode(urldecode($this->uri->segment(4)))); //Search DB 
        ...
        if('IS_AJAX')  
         ...
         {  
		    $this->load->view('not/noti13_model/lookup',$data); 
	  ...
	  public function filter1($sort_by = 'mc001', $sort_order = 'desc', $offset = 0)   ////篩選資料
        {
	     ...
	     $data['sort_order'] = $sort_order;
	     $this->load->model('not/noti13_model','',TRUE);   //true 系統會使用反單引號（`, 在英文鍵盤左上方）來保護你的欄位或資料表名稱
	     $result=$this->noti13_model->filterf1($limit, $offset , $sort_by  , $sort_order);
	     $data['message'] = '篩選資料成功!';	
	     $data['results'] = $result['rows'];
	     $data['num_results'] = $result['num_rows'];
	     $this->load->library('pagination');
	     ...
	     $config['base_url'] = site_url("not/noti13/filter1/$sort_by/$sort_order/$seq6/$seq7");  //設定分頁url路徑
      ...
      public function findform()   //進階查詢輸入資料
        {
	      ...
	      $data['username'] = $this->session->userdata('manager');
	      $data['message'] = '';
	      $data['systitle'] ='融資種類-進階查詢';
	      $data['menu_v'] = 'main_menuno_v';
	      $data['content_v'] = 'not/noti13_find_v';
	      $data['foot_v'] ='main_foot_v';
	      $this->load->vars($data);
	  ...
	  public function findsql($sort_by = 'mc001', $sort_order = 'desc', $offset = 0)  //進階查詢流覽資料
	    {		
	      ...
	      $data['sort_order'] = $sort_order;
	      $this->load->model('not/noti13_model');// 加載TABLE model 模型		
	      $result= $this->noti13_model->findf($limit, $offset , $sort_by  , $sort_order ); //至model 取 mysql 資料 預設 15,0,mc001,desc
	      $data['results'] = $result['rows'];
	      $data['num_results'] = $result['num_rows'];
	      $this->load->library('pagination');
	      $data['numrow']=$result['num_rows'];// 總筆數 
	    ...
	    //$config['NUM_LINKS'] = 2;   //分頁前後出現2個數字
	      ...
	      $config['cur_page'] = $this->uri->segment(6,0);   //當前頁 結合分頁url路徑 +1
	      $this->pagination->initialize($config);    //分頁初始化 display 3
	      $config['base_url'] = site_url("not/noti13/findsql/$sort_by/$sort_order");   //設定分頁url路徑
	      $config['total_rows'] = $result['num_rows']; // 總筆數
	      $config['per_page'] = $limit;                //每頁筆數
	      $config['uri_segment'] = 6;       //當前頁
	      $this->pagination->initialize($config);
      ...
      public function addform()   //新增輸入資料
        {
	     ...
	     $data['message'] = '';
	     $data['username'] = $this->session->userdata('manager');
	     $data['systitle'] ='融資種類-新增資料';
	     $data['menu_v'] = 'main_menuno_v';
	     $data['content_v'] = 'not/noti13_add_v';
	     $data['foot_v'] ='main_foot_v';
	     $this->load->vars($data);
	     $this->load->view('main_head_v');
        }
      ...
      public function addsave()   //新增存檔
        {
	      $data['username'] = $this->session->userdata('manager');
          $this->load->model('not/noti13_model','',TRUE);
	      $data['message'] = '新增成功!';
	      $action = $this->noti13_model->insertf();
	      if ($action === 'exist')
	       {
	        $data['message'] = '資料重複!';		    
	       }
      ...
      public function copyform()   //複製資料輸入
        {
	     $data['username'] = $this->session->userdata('manager');
	     $data['message'] = '';
	     $data['systitle'] ='融資種類-複製資料';
	     $data['menu_v'] = 'main_menuno_v';
	     $data['content_v'] = 'not/noti13_copy_v';
	     $data['foot_v'] ='main_foot_v';
	     $this->load->vars($data);
	     $this->load->view('main_head_v');
        }
      ...
      public function copysave()   //複製存檔
        {
         ...
         $this->load->model('not/noti13_model','',TRUE);
	     $data['message'] = '複製成功!';
         $action = $this->noti13_model->copyf();
      ...
      public function exceldetail()   //轉excel明細輸入起迄資料
        {
	     $data['message'] = '';
	     $data['username'] = $this->session->userdata('manager');
	     $data['systitle'] ='融資種類-轉excel檔';
	     $data['menu_v'] = 'main_menuno_v';
	     $data['content_v'] = 'not/noti13_excel_v';
	     $data['foot_v'] ='main_foot_v';
	     $this->load->vars($data);
	     $this->load->view('main_head_v');
        }
      ...
      public function write()   //轉excel 部份資料由 print_v call
        {
	      ...
	      $data['username'] = $this->session->userdata('manager');
	      $title = array('融資種類','融資名稱','融資性質');  //excel 表頭
          $result1 = $this->noti13_model->excelnewf();
		  $temp_ary = array(1=>"1:L/C",2=>"2:INVOICE",3=>"3:應付商業本票/承兌匯票",4=>"4:應收票據",5=>"5:資產抵押",9=>"9:其他");
		  foreach($result1 as $key => $val){
			  $result1[$key]['mc003'] = $temp_ary[$val['mc003']];
		  }
      ...
      public function printdetail()   //印明細起迄資料輸入
        {
	     $data['username'] = $this->session->userdata('manager');
	     $data['message'] = '';
	     $data['systitle'] ='融資種類-印明細表';
	     $data['menu_v'] = 'main_menuno_v';
	     $data['content_v'] = 'not/noti13_print_v';
	     $data['foot_v'] ='main_foot_v';
	     $this->load->vars($data);
	     $this->load->view('main_head_v');
        }
		 public function printdetailc()   //印明細起迄資料輸入
        {
	     ...
	     $data['systitle'] ='融資種類-印明細表';
	     $data['menu_v'] = 'main_menuno_v';
	     $data['content_v'] = 'not/noti13_print1_v';
	     $data['foot_v'] ='main_foot_v';
	     $this->load->vars($data);
	     $this->load->view('main_head_v');
        }
      public function printa()   //印明細
        {
          ...
          $this->load->model('not/noti13_model','',TRUE);
	      $data['message'] = '列印明細成功!';
          $result = $this->noti13_model->printfd();
	      $data['results'] = $result['rows'];
		  $temp_ary = array(1=>"1:L/C",2=>"2:INVOICE",3=>"3:應付商業本票/承兌匯票",4=>"4:應收票據",5=>"5:資產抵押",9=>"9:其他");
		  foreach($data['results'] as $val){
			  $val->mc003 = $temp_ary[$val->mc003];
	      ...
	      $data['systitle'] ='融資種類-印明細表';
	      //$data['menu_v'] = 'main_menuno_v';
	      $data['content_v'] = 'not/noti13_printa_v';
	     //$data['foot_v'] ='main_footno_v';
	     $this->load->vars($data);
	     $this->load->view('main_headprint_v');
	     //$this->load->view('not/noti13_printa_v',$data);  
        }
      ...
      public function updsave()   //修改存檔
        {
         ...
         $this->load->model('not/noti13_model','',TRUE);
	     $this->load->vars($data);
	     $this->noti13_model->updatef();
	     redirect('not/noti13/'.$this->session->userdata('search1'));
        }
      ...
      public function updform()   //修改輸入資料
        {
	      ...
	      $this->load->model('not/noti13_model');
	      $data['result'] = $this->noti13_model->selone($this->uri->segment(4),$this->uri->segment(5));
	      $data['username'] = $this->session->userdata('manager');
	      $data['systitle'] ='融資種類-修改資料';
	      $data['menu_v'] = 'main_menuno_v';
	      $data['content_v'] = 'not/noti13_upd_v';
	      $data['foot_v'] ='main_foot_v';
	      $this->load->vars($data);
	      $this->load->view('main_head_v');
        }
      ...
      public function see()   //看資料
        {      
	      ...
	      $data['result'] = $this->noti13_model->selone($this->uri->segment(4),$this->uri->segment(5));
	      $data['username'] = $this->session->userdata('manager');
	      $data['systitle'] ='融資種類-查看資料';
	      $data['menu_v'] = 'main_menuno_v';
	      $data['content_v'] = 'not/noti13_see_v';
	      $data['foot_v'] ='main_foot_v';
	      $this->load->vars($data);
	      $this->load->view('main_head_v');
        }
      ...
      public function del()   //刪除單筆 暫存
       ...
       //   $seg2=$this->uri->segment(5); 
	      $data['message'] = '刪除資料成功!';
	      $this->load->model('not/noti13_model','',TRUE);
	      $this->noti13_model->deletef($seg1);
	      $this->display();
        }
	 ...
	 public function printb()   //印單據選取
        {    
	      ...
	      $this->load->model('not/noti13_model','',TRUE);
	      $data['message'] = '列印單據成功!';
          $result = $this->noti13_model->printfd1($this->uri->segment(4),$this->uri->segment(5));
	      $data['results'] = $result['rows'];
	      $data['num_results'] = $result['num_rows'];
	      $this->load->library('pagination');
	      $data['numrow']=$result['num_rows'];// 總筆數 
	      $data['username'] = $this->session->userdata('manager');
	      $data['systitle'] ='請  購  單';	
       //   $this->load->view('not/noti13_printb_v');
          
	      $data['content_v'] = 'not/noti13_printb_v';	   
	      $this->load->vars($data);
	      $this->load->view('main_headprint_v');
	...
	public function display_child($offset = 0,$func = "")  
	  {		
		...
		$data['message'] = '資料流覽成功!';
		$this->load->model('not/noti13_model');// 加載TABLE model 模型		
		$result= $this->noti13_model->construct_sql($limit, $offset ,$func); //至model 取 mysql 資料 預設 15,0,mc001,desc
		...
		$config['base_url'] = site_url("not/noti13/display_child");   //設定分頁url路徑
		/* 網址去除".html" */
		...
		$data['menu_v'] = 'main_menu_v';
		$data['content_v'] = 'funnew/noti13_child_v.php';		
		$data['foot_v'] ='main_foot_v';
		$this->load->vars($data);
		$this->load->view('main_headchild_v');		
	  }
	//廠別快速查詢
	public function lookup1_noti13(){
	    $keyword = urldecode(urldecode($this->uri->segment(4)));
        $data['response'] = 'false'; //Set default response 
        $this->load->model('not/noti13_model');
    /*    $query = $this->noti13_model->lookup(
			array('mc001','mc002'),
			array('and'=>"mc001"),
			array('mc001'=>$keyword),
			10
		); */
		$query = $this->noti13_model->lookup1(urldecode(urldecode($this->uri->segment(4)))); //Search DB 
        if( ! empty($query) )  
          {  
            $data['response'] = 'true'; //Set response  
            $data['message'] = array(); //Create array  
	...
	//廠別快速查詢
	public function lookup2_noti13(){
	    $keyword = urldecode(urldecode($this->uri->segment(4)));
        $data['response'] = 'false'; //Set default response 
        $this->load->model('not/noti13_model');
    /*    $query = $this->noti13_model->lookup(
		...
		$query = $this->noti13_model->lookup2(urldecode(urldecode($this->uri->segment(4)))); //Search DB 
        if( ! empty($query) )  
          {  
            $data['response'] = 'true'; //Set response  
            $data['message'] = array(); //Create array  
	...
	public function clear_sql()
	  {
		if (session_status() == PHP_SESSION_NONE) {
			session_start();
			unset($_SESSION['noti13']['search']);
		}
		$this->display_child();
	  }
	  
	  public function checknoti13()   //不更改網頁 輸入資料 廠別 6明細
        {
	   $this->load->model('not/noti13_model');
	   $data['result'] = $this->noti13_model->ajaxnoti13($this->uri->segment(4));
       $Result = $data['result'];		  
	   $this->load->vars($data);
	   echo  $Result;
        }
}
/* End of file noti13.php */
/* Location: ./application/controllers/noti13.php */
?>