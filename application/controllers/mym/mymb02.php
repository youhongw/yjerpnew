...
class Mymb02 extends CI_Controller {
    ...
    public  function index()
      {
	    ...
	    $data['message1'] = '';
		 ...
		 $data['systitle'] ='刪除 excel上傳資料至資料庫';	
          $data['username'] = $this->session->userdata('manager');		 
  	      $data['menu_v'] = 'main_menu_v';
	      $data['content_v'] = 'mym/mymb02_delete_v';		
	      $data['foot_v'] ='main_foot_v';
	      $this->load->vars($data);
	      $this->load->view('main_headbrow_v');	
      //  $this->load->view('mym/mymb02_delete_v');  
      }
   
	  public  function do_del()
      {
		$seg1=$this->input->post('ta002o');;
        $seg2=$this->input->post('ta002c');; 
	    $data['message'] = '刪除資料成功!';
	    $this->load->model('mym/mymb02_model','',TRUE);
	    $this->mymb02_model->deletef($seg1,$seg2); 
		
	    $data['message1'] = '';
	     
		  $data['systitle'] ='excel上傳資料至資料庫';	
          $data['username'] = $this->session->userdata('manager');		 
  	      $data['menu_v'] = 'main_menu_v';
	      $data['content_v'] = 'mym/mymb02_delete_v';		
	      $data['foot_v'] ='main_foot_v';
    ...
    public function read()
	  {  
        $rows = array('ta001','ta002','ta003','ta004','ta005','ta006','ta007','ta008','ta009','ta010','ta011','ta012','ta013','ta014',
		               'ta015','ta016','ta017','ta018','ta019','ta020','ta021','ta022','ta023','ta024','ta025','ta026','ta027','ta028', 
					   'ta029','ta030','ta031','ta032','ta033','ta034','ta035' );
    //    $KeyWord =  'ta031';    //修改key值
	...
	//	$KeyWord = explode(",",$KeyWord);
      ...
      //  $this->FilterExsel($Exsel_data,$KeyWord);            //檢查資料重複
	    ...
	    $this->load->model('mym/mymb02_model');   //true 系統會使用反單引號（`, 在英文鍵盤左上方）來保護你的欄位或資料表名稱
	       
        $DB_data  =  $this->mymb02_model->selUsers();                //查询DB中的全部數據
       //   return   'kk2';                                       
        if(!empty($DB_data))
          {
    ...
    public  function write()
      {
        ...
        $data = $this->mymb02_model->selUsers();
    ...
    public   function addExcel($value='')
     {  
	    $this->load->model('mym/mymb02_model','',TRUE);   //true 系統會使用反單引號（`, 在英文鍵盤左上方）來保護你的欄位或資料表名稱
        return $this->mymb02_model->add_Excel($value);   
     }
     /**
      * 更新方法
      * 
    ...
    public    function updExcel($value='')
      {  
        return $this->mymb02_model->upd_Excel($value);  
      }
    /**
    *篩選所有的key欄位組成數组    
    *@param   DB數據
