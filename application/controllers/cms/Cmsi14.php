...
class Cmsi14 extends CI_Controller {           //擴展類必须註明由母類擴展而來 (自訂類使用 ci 框架)
	...
	//自訂類預設執行函數 流覽資料  	
	  public function index()           
	    {                      
	      ...
	      $this->load->model('cms/cmsi14_model');     // 加載TABLE model 模型		
	      $result= $this->cmsi14_model->search($limit, $offset = 0 , $sort_by  , $sort_order ); //至model 取 mysql 資料 預設 15,0,ma001,desc
	    ...
	    //$config['NUM_LINKS'] = 2;   //分頁前後出現2個數字
	      ...
	      $config['base_url'] = site_url("cms/cmsi14/display/$sort_by/$sort_order");   //設定分頁url路徑
	   ...
	   // $this->load->library('table');//加載table函數
	    ...
	    //$data['find07']=$this->session->userdata('find07');
	      ...
	      $data['content_v'] = 'cms/cmsi14_brow_v';		
	  ...
	  public function display($sort_by = 'ma001', $sort_order = 'desc', $offset = 0)  
	    {		
	      ...
	      $data['sort_order'] = $sort_order;
	      $this->load->model('cms/cmsi14_model');// 加載TABLE model 模型		
	      $result= $this->cmsi14_model->search($limit, $offset , $sort_by  , $sort_order ); //至model 取 mysql 資料 預設 15,0,ma001,desc
	  ...
	  public function filter1($sort_by = 'ma001', $sort_order = 'desc', $offset = 0)   
        {
	     ...
	     $data['sort_order'] = $sort_order;
	     $this->load->model('cms/cmsi14_model','',TRUE);   //true 系統會使用反單引號（`, 在英文鍵盤左上方）來保護你的欄位或資料表名稱
	     $result=$this->cmsi14_model->filterf1($limit, $offset , $sort_by  , $sort_order);
	     $data['message'] = '篩選資料成功!';	
	     $data['results'] = $result['rows'];
	     $data['num_results'] = $result['num_rows'];
	     $this->load->library('pagination');
	     ...
	     $config['base_url'] = site_url("cms/cmsi14/filter1/$sort_by/$sort_order/$seq6/$seq7");  //設定分頁url路徑
      ...
      public function findform()   
	      ...
	      $data['content_v'] = 'cms/cmsi14_find_v';
	      $data['foot_v'] ='main_foot_v';
	      $this->load->vars($data);
     ...
     //進階查詢流覽資料
	  public function findsql($sort_by = 'ma001', $sort_order = 'desc', $offset = 0)  
	    {		
	      ...
	      $data['sort_order'] = $sort_order;
	      $this->load->model('cms/cmsi14_model');// 加載TABLE model 模型		
	      $result= $this->cmsi14_model->findf($limit, $offset , $sort_by  , $sort_order ); //至model 取 mysql 資料 預設 15,0,ma001,desc
	      $data['results'] = $result['rows'];
	      $data['num_results'] = $result['num_rows'];
	      $this->load->library('pagination');
	      $data['numrow']=$result['num_rows'];// 總筆數 
	    ...
	    //$config['NUM_LINKS'] = 2;   //分頁前後出現2個數字
	      ...
	      $config['cur_page'] = $this->uri->segment(6,0);   //當前頁 結合分頁url路徑 +1
	      $this->pagination->initialize($config);    //分頁初始化 display 3
	      $config['base_url'] = site_url("cms/cmsi14/findsql/$sort_by/$sort_order");   //設定分頁url路徑
	      $config['total_rows'] = $result['num_rows']; // 總筆數
	      $config['per_page'] = $limit;                //每頁筆數
	      $config['uri_segment'] = 6;       //當前頁
	    //$this->load->library('table');//加載table函數
      ...
      public function addform()   
	   ...
	   $data['menu_v'] = 'main_menuno_v';
	   $data['content_v'] = 'cms/cmsi14_add_v';
	   $data['foot_v'] ='main_foot_v';
	   $this->load->vars($data);
	   $this->load->view('main_head_v');
        }
      ...
      public function addsave()   
        {
	     $data['username'] = $this->session->userdata('manager');
         $this->load->model('cms/cmsi14_model','',TRUE);
	     $data['message'] = '新增成功!';
	     $action = $this->cmsi14_model->insertf();
	     if ($action === 'exist')
	      {
	       $data['message'] = '資料重複!';		    
	      }
      ...
      public function copyform()   
        {
	     ...
	     $data['menu_v'] = 'main_menuno_v';
	     $data['content_v'] = 'cms/cmsi14_copy_v';
	     $data['foot_v'] ='main_foot_v';
	     $this->load->vars($data);
	     $this->load->view('main_head_v');
        }
      ...
      public function copysave()   
        {
         ...
         $this->load->model('cms/cmsi14_model','',TRUE);
	     $data['message'] = '複製成功!';
         $action = $this->cmsi14_model->copyf();
      ...
      public function exceldetail()   
        {
	     ...
	     $data['menu_v'] = 'main_menuno_v';
	     $data['content_v'] = 'cms/cmsi14_excel_v';
	     $data['foot_v'] ='main_foot_v';
	     $this->load->vars($data);
	     $this->load->view('main_head_v');
        }
      ...
      public function write()   
        {
         ...
         $result1 = $this->cmsi14_model->excelnewf();	
      ...
      public function printdetail()   
        {
	     ...
	     $data['menu_v'] = 'main_menuno_v';
	     $data['content_v'] = 'cms/cmsi14_print_v';
	     $data['foot_v'] ='main_foot_v';
	     $this->load->vars($data);
	     $this->load->view('main_head_v');
        }
      ...
      public function printa()   
        {
         $this->load->model('cms/cmsi14_model','',TRUE);
	     $data['message'] = '列印明細成功!';
         $result = $this->cmsi14_model->printfd();
	     $data['results'] = $result['rows'];
	     $data['num_results'] = $result['num_rows'];
	     $this->load->library('pagination');
	     $data['numrow']=$result['num_rows'];// 總筆數 
	     $data['username'] = $this->session->userdata('manager');
	     $data['systitle'] ='品號類別-印明細表';
	     $data['content_v'] = 'cms/cmsi14_printa_v';
	     $this->load->vars($data);
      ...
      public function updform()   
        {
	     ...
	     $this->db->get('cmsml');
	     $this->load->model('cms/cmsi14_model');
	     $result = $this->cmsi14_model->selone();
	     $data['results'] = $result['rows'];
	     $data['username'] = $this->session->userdata('manager');
	     $data['systitle'] ='公司名稱-修改資料';
	     $data['menu_v'] = 'main_menuno_v';
	     $data['content_v'] = 'cms/cmsi14_upd_v';
	     $data['foot_v'] ='main_foot_v';
	     $this->load->vars($data);
	     $this->load->view('main_head_v');
        }
	...
	//看資料
      public function see()   
        {      
	     ...
	     $data['message'] = '查看一筆資料!';
	     $this->load->model('cms/cmsi14_model');
	     $data['result'] = $this->cmsi14_model->selone($this->uri->segment(4),$this->uri->segment(5));
	     $data['username'] = $this->session->userdata('manager');
	     $data['systitle'] ='品號類別-查看資料';
	     $data['menu_v'] = 'main_menuno_v';
	     $data['content_v'] = 'cms/cmsi14_see_v';
	     $data['foot_v'] ='main_foot_v';
	     $this->load->vars($data);
	     $this->load->view('main_head_v');
        }
      ...
      public function del()   
        {      
	     ...
	     $data['message'] = '刪除資料成功!';
	     $this->load->model('cms/cmsi14_model','',TRUE);
	     $this->cmsi14_model->deletef($seg1,$seg2);
	     $this->display();
        }
      ...
      public function delete()   
        {    
	     $data['message'] = '刪除資料成功!';
	     $this->load->model('cms/cmsi14_model','',TRUE);
	     $this->cmsi14_model->delmutif();
	     $this->display();
        }
}
/* End of file cmsi14.php */
/* Location: ./application/controllers/cmsi14.php */
?>