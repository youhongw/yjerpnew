...
class Cmsq06a extends CI_Controller {           //擴展類必须註明由母類擴展而來 (自訂類使用 ci 框架)
	  ...
	  public function index()           //自訂類預設執行函數 流覽資料
	    {                      
	      ...
	      $this->load->model('cms/cmsq06a_model');     // 加載TABLE model 模型		
	      $result= $this->cmsq06a_model->search($limit, $offset = 0 , $sort_by  , $sort_order ); //至model 取 mysql 資料 預設 15,0,mb001,desc
	    ...
	    //  $this->pagination->initialize($config);    //分頁初始化 display 3
	      $config['base_url'] = site_url("cms/cmsq06a/display/$sort_by/$sort_order");   //設定分頁url路徑
	   ...
	   // $this->load->library('table');//加載table函數
	    ...
	    //$data['find07']=$this->session->userdata('find07');
	      ...
	      $data['content_v'] = 'cms/cmsq06a_brow_v';		
	  ...
	  public function display($sort_by = 'mb001', $sort_order = 'asc', $offset = 0)  //欄位表頭排序
	    {		
	      ...
	      $data['sort_order'] = $sort_order;
	      $this->load->model('cms/cmsq06a_model');// 加載TABLE model 模型		
	      $result= $this->cmsq06a_model->search($limit, $offset , $sort_by  , $sort_order ); //至model 取 mysql 資料 預設 15,0,mb001,desc
	    ...
	    //$this->load->library('table');//加載table函數
	      ...
	      $data['systitle'] ='幣別視窗查詢';		  
  	      $this->load->vars($data);
		  $this->load->view('cms/cmsq06a_child_v');	
	    } 
	   ...
	   $this->load->vars($data);
	    ...
	    {		
	     ...
	     $seq6 = urldecode(urldecode($this->uri->segment(6))); 	 //解決亂碼
	      ...
	      $data['sort_order'] = $sort_order;
	      $this->load->model('cms/cmsq06a_model');// 加載TABLE model 模型		
	      $result= $this->cmsq06a_model->search1($limit, $offset , $sort_by  , $sort_order ); //至model 取 mysql 資料 預設 15,0,mb001,desc
	      $data['results'] = $result['rows'];
	      $data['num_results'] = $result['num_rows'];
	      $this->load->library('pagination');
	    ...
	    //$this->pagination->initialize($config);    //分頁初始化 display 3 + 2 + 1 = 6
	      $config['base_url'] = site_url("cms/cmsq06a/filter1/$sort_by/$sort_order/$seq6/$seq7");   //設定分頁url路徑
	      $config['total_rows'] = $result['num_rows']; // 總筆數
	      $config['per_page'] = $limit;                //每頁筆數
	   ...
	   //   $this->load->view('main_headquery_v');		
	      ...
	      $data['menu_v'] = 'main_menuno_v';
	      $data['content_v'] = 'cms/cmsq06a_find_v';
	  ...
	  public function findsql($sort_by = 'mb001', $sort_order = 'desc', $offset = 0)  //進階查詢流覽資料
	    {		
	      ...
	      $data['sort_order'] = $sort_order;
	      $this->load->model('cms/cmsq06a_model');// 加載TABLE model 模型		
	      $result= $this->cmsq06a_model->findf($limit, $offset , $sort_by  , $sort_order ); //至model 取 mysql 資料 預設 15,0,mb001,desc
	      $data['results'] = $result['rows'];
	      $data['num_results'] = $result['num_rows'];
	      $this->load->library('pagination');
	      $data['numrow']=$result['num_rows'];// 總筆數 
	    ...
	    //$config['NUM_LINKS'] = 2;   //分頁前後出現2個數字
	      ...
	      $config['cur_page'] = $this->uri->segment(6,0);   //當前頁 結合分頁url路徑 +1
	      $this->pagination->initialize($config);    //分頁初始化 display 3
	      $config['base_url'] = site_url("cms/cmsq06a/findsql/$sort_by/$sort_order");   //設定分頁url路徑
	      $config['total_rows'] = $result['num_rows']; // 總筆數
	      $config['per_page'] = $limit;                //每頁筆數
	      $config['uri_segment'] = 6;       //當前頁
	    //$this->load->library('table');//加載table函數
	...
	//$this->db->get('invma');
	  ...
	  $data['message'] = '刪除資料成功!';
	  $this->load->model('cms/cmsq06a_model','',TRUE);
