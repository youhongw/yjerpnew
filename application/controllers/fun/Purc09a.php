...
class Purc09a extends CI_Controller {           //擴展類必须註明由母類擴展而來 (自訂類使用 ci 框架)
	  ...
	  public function index()           //自訂類預設執行函數 流覽資料
	    {                      
	      ...
	      $this->load->model('fun/purc09a_model');     // 加載TABLE model 模型		
	      $result= $this->purc09a_model->search($limit, $offset = 0 , $sort_by  , $sort_order ); //至model 取 mysql 資料 預設 15,0,tc001,desc
	    ...
	    //  $this->pagination->initialize($config);    //分頁初始化 display 3
	      $config['base_url'] = site_url("fun/purc09a/display/$sort_by/$sort_order");   //設定分頁url路徑
	   ...
	   // $this->load->library('table');//加載table函數
	    ...
	    //$data['find07']=$this->session->userdata('find07');
	      ...
	      $data['content_v'] = 'fun/purc09a_brow_v';		
	  ...
	  public function display($sort_by = 'tc002', $sort_order = 'desc', $offset = 0)  //欄位表頭排序
	    {		
		  //廠商代號
		  $tg005 = $this->uri->segment(6);
		 // echo var_dump($tg005);exit;
	      ...
	      $data['sort_order'] = $sort_order;
	      $this->load->model('fun/purc09a_model');// 加載TABLE model 模型		
	      $result= $this->purc09a_model->search($limit, $offset , $sort_by  , $sort_order,$tg005 ); //至model 取 mysql 資料 預設 15,0,tc001,desc
	    ...
	    //$this->load->library('table');//加載table函數
	      ...
	      $data['systitle'] ='庫別視窗查詢';		  
  	      $this->load->vars($data);
		  $this->load->view('fun/purc09a_child_v');	
	    } 
	   ...
	   $this->load->vars($data);
	    ...
	    {		
	     ...
	     $seq6 = urldecode(urldecode($this->uri->segment(6))); 	 //解決亂碼
	      ...
	      $data['sort_order'] = $sort_order;
	      $this->load->model('fun/purc09a_model');// 加載TABLE model 模型		
	      $result= $this->purc09a_model->search1($limit, $offset , $sort_by  , $sort_order ); //至model 取 mysql 資料 預設 15,0,tc001,desc
	      $data['results'] = $result['rows'];
	      $data['num_results'] = $result['num_rows'];
	      $this->load->library('pagination');
	    ...
	    //$this->pagination->initialize($config);    //分頁初始化 display 3 + 2 + 1 = 6
	      $config['base_url'] = site_url("fun/purc09a/filter1/$sort_by/$sort_order/$seq6/$seq7");   //設定分頁url路徑
	      $config['total_rows'] = $result['num_rows']; // 總筆數
	      $config['per_page'] = $limit;                //每頁筆數
	   ...
	   //   $this->load->view('main_headquery_v');		
	      ...
	      $data['menu_v'] = 'main_menuno_v';
	      $data['content_v'] = 'fun/purc09a_find_v';
	  ...
	  public function findsql($sort_by = 'tc001', $sort_order = 'desc', $offset = 0)  //進階查詢流覽資料
	    {		
	      ...
	      $data['sort_order'] = $sort_order;
	      $this->load->model('fun/purc09a_model');// 加載TABLE model 模型		
	      $result= $this->purc09a_model->findf($limit, $offset , $sort_by  , $sort_order ); //至model 取 mysql 資料 預設 15,0,tc001,desc
	      $data['results'] = $result['rows'];
	      $data['num_results'] = $result['num_rows'];
	      $this->load->library('pagination');
	      $data['numrow']=$result['num_rows'];// 總筆數 
	    ...
	    //$config['NUM_LINKS'] = 2;   //分頁前後出現2個數字
	      ...
	      $config['cur_page'] = $this->uri->segment(6,0);   //當前頁 結合分頁url路徑 +1
	      $this->pagination->initialize($config);    //分頁初始化 display 3
	      $config['base_url'] = site_url("fun/purc09a/findsql/$sort_by/$sort_order");   //設定分頁url路徑
	      $config['total_rows'] = $result['num_rows']; // 總筆數
	      $config['per_page'] = $limit;                //每頁筆數
	      $config['uri_segment'] = 6;       //當前頁
	    //$this->load->library('table');//加載table函數
        ...
        { 
		  $data['message'] = '複製前置單據!';
	        $data['menu_v'] = 'main_menuno_v';
	      $data['content_v'] = 'fun/purc09a_child_v';		
	      $data['foot_v'] ='main_foot_v';
	      $this->load->vars($data);
	      $this->load->view('main_headquery_v');	
	  ...
	  $data['message'] = '查看一筆資料!';
	//$this->db->get('invma');
	  $this->load->model('fun/purc09a_model');
	  $data['result'] = $this->purc09a_model->selone($this->uri->segment(4),$this->uri->segment(5));
	  $data['username'] = $this->session->userdata('manager');
	  $data['systitle'] ='品號類別-查看資料';
	  $data['menu_v'] = 'main_menuno_v';
	  $data['content_v'] = 'fun/purc09a_see_v';
	  $data['foot_v'] ='main_foot_v';
	  $this->load->vars($data);
	  $this->load->view('main_head_v');
        }
	  ...
	  $data['message'] = '刪除資料成功!';
	  $this->load->model('fun/purc09a_model','',TRUE);
	  $this->purc09a_model->deletef($seg1,$seg2);
	  $this->display();
        }
	  ...
	  $data['message'] = '刪除資料成功!';
	  $this->load->model('fun/purc09a_model','',TRUE);
	  $this->purc09a_model->delmutif();
	  $this->display();
        }
	public function datapurc09a()   //不更新網頁輸入資料 廠別 puri05
        {
    
	   $this->load->model('fun/purc09a_model');
	   $data['result'] = $this->purc09a_model->ajaxpurc09a($this->uri->segment(4));
       $Result = $data['result'];		  
	   $this->load->vars($data);
	   echo  $Result;
        }
...
/* End of file purc09a.php */
/* Location: ./application/controllers/purc09a.php */