...
class Copq06a extends CI_Controller {           //擴展類必须註明由母類擴展而來 (自訂類使用 ci 框架)
	...
	public function display_hp($sort_by = 'td001', $sort_order = 'desc', $offset = 0 , $where = '', $where_item = '', $row_count = 0)  //欄位表頭排序  Talence做查價視窗
	    {		
		  session_start();
	      ...
	      $data['sort_order'] = $sort_order;
	      $this->load->model('fun/copq06a_model');// 加載TABLE model 模型		
	      $result= $this->copq06a_model->search_hp($limit, $offset , $sort_by  , $sort_order , $where , $where_item, $row_count); //至model 取 mysql 資料 預設 15,0,td001,desc
	      $data['results'] = $result['rows'];
		  if(@$result['row_count'])
	    ...
	    //$config['NUM_LINKS'] = 2;   //分頁前後出現2個數字, 預設5個
	      ...
	      $config['cur_page'] = $this->uri->segment(6,0);   //當前頁 結合分頁url路徑 +1
	    //$this->pagination->initialize($config);    //分頁初始化 display 3 + 2 + 1 = 6
	      $config['base_url'] = site_url("fun/copq06a/display_hp/$sort_by/$sort_order");   //設定分頁url路徑
	      $config['total_rows'] = $result['num_rows']; // 總筆數
	      $config['per_page'] = $limit;                //每頁筆數
	      $config['uri_segment'] = 6;       //當前頁
	    //$this->load->library('table');//加載table函數
	      ...
	      $data['systitle'] ='歷史價格查詢';		  
  	      $this->load->vars($data);
		  $this->load->view('fun/copq06a_hpchild_v');	
	    }
	...
	public function filter_hp($sort_by = 'create_date', $sort_order = 'asc', $offset = 0, $where = '', $where_item = '', $like = '', $row_count=0)  //欄位表頭排序
	    {		
	     ...
	     $seq6 = urldecode(urldecode($this->uri->segment(6))); 	 //解決亂碼
	      ...
	      $data['sort_order'] = $sort_order;
	      $this->load->model('fun/copq06a_model');// 加載TABLE model 模型		
	      $result= $this->copq06a_model->search_filter_hp($limit, $offset , $sort_by  , $sort_order , $where , $where_item , $like, $row_count); //至model 取 mysql 資料 預設 15,0,td001,desc
	      $data['results'] = $result['rows'];
		  if(@$result['row_count'])
			$data['row_count'] = $result['row_count'];
		  else
	    ...
	    //$config['NUM_LINKS'] = 2;   //分頁前後出現2個數字, 預設5個
	      ...
	      $config['cur_page'] = $this->uri->segment(8,0);   //當前頁 結合分頁url路徑 +1
	    //$this->pagination->initialize($config);    //分頁初始化 display 3 + 2 + 1 = 6
	      $config['base_url'] = site_url("fun/copq06a/filter_hp/$sort_by/$sort_order/$seq6/$seq7");   //設定分頁url路徑
	      $config['total_rows'] = $result['num_rows']; // 總筆數
	      $config['per_page'] = $limit;                //每頁筆數
	      $config['uri_segment'] = 8;       //當前頁
	    //$this->load->library('table');//加載table函數
