...
class Acpi03 extends CI_Controller {           //擴展類必须註明由母類擴展而來 (自訂類使用 ci 框架)
	  ...
	  public function index()           //自訂類預設執行函數 流覽資料
	    {                      
	      ...
	      $result= $this->acpi03_model->search($limit, $offset = 0 , $sort_by  , $sort_order ); //至model 取 mysql 資料 預設 15,0,ta001,desc
	  ...
	  public function display($sort_by = 'ta002', $sort_order = 'desc', $offset = 0)  //欄位表頭排序
	    {		
	      ...
	      $data['sort_order'] = $sort_order;
	      $this->load->model('acp/acpi03_model');// 加載TABLE model 模型		
	      $result= $this->acpi03_model->search($limit, $offset , $sort_by  , $sort_order ); //至model 取 mysql 資料 預設 15,0,ta001,desc
	...
	public function display_child($offset = 0,$func = "")  
	  {		
		...
		$data['message'] = '資料流覽成功!';
		$this->load->model('acp/acpi03_model');// 加載TABLE model 模型		
		$result= $this->acpi03_model->construct_sql($limit, $offset ,$func); //至model 取 mysql 資料 預設 15,0,ma001,desc
		...
		$config['base_url'] = site_url("acp/acpi03/display_child");   //設定分頁url路徑
		/* 網址去除".html" */
		...
		$data['menu_v'] = 'main_menu_v';
		$data['content_v'] = 'funnew/acpi03_child_v.php';		
		$data['foot_v'] ='main_foot_v';
		$this->load->vars($data);
		$this->load->view('main_headchild_v');		
	  }
       //廠別快速查詢
	public function lookup2_acpi03(){
    ...
    /*    $query = $this->noti01_model->lookup(
		...
		$query = $this->acpi03_model->lookup2(urldecode(urldecode($this->uri->segment(4)))); //Search DB 
        if( ! empty($query) )  
          {  
	  ...
	  public function lookup4(){
        ...
        $data['response'] = 'false'; //Set default response 
        $this->load->model('acp/acpi03_model');
        $query = $this->acpi03_model->lookup4(urldecode(urldecode($this->uri->segment(4))),$seq5); //Search DB 
        ...
        if('IS_AJAX')  
         ...
         {  
		    $this->load->view('acp/acpi03_model/lookup4',$data); 
          // $this->index; //Load html view of search results  
         }  
        }
    
	// 下拉視窗不更新網頁查 3待底72 採購預付單
		
	  public function lookup3(){
	    $keyword = urldecode(urldecode($this->uri->segment(4)));
		$seq5 = urldecode(urldecode($this->uri->segment(5)));
        $data['response'] = 'false'; //Set default response 
        $this->load->model('acp/acpi03_model');
        $query = $this->acpi03_model->lookup3(urldecode(urldecode($this->uri->segment(4))),$seq5); //Search DB 
        ...
        if('IS_AJAX')  
         ...
         {  
		    $this->load->view('acp/acpi03_model/lookup3',$data); 
          // $this->index; //Load html view of search results  
         }  
        }
		
	// 下拉視窗不更新網頁查 5溢付72 採購預付單
		
	  public function lookup5(){
	    $keyword = urldecode(urldecode($this->uri->segment(4)));
		$seq5 = urldecode(urldecode($this->uri->segment(5)));
        $data['response'] = 'false'; //Set default response 
        $this->load->model('acp/acpi03_model');
        $query = $this->acpi03_model->lookup5(urldecode(urldecode($this->uri->segment(4))),$seq5); //Search DB 
        ...
        if('IS_AJAX')  
         ...
         {  
		    $this->load->view('acp/acpi03_model/lookup5',$data); 
          // $this->index; //Load html view of search results  
         }  
        }	 
	    // 下拉視窗不更新網頁查 會計科目
	  ...
	  public function lookupa(){
	   ...
	   public function datachkno1()   //提示改輸入資料如  付款單號 不更新網頁ta012
        {
	      $this->load->model('acp/acpi03_model');
	  ...
	  public function findsql($sort_by = 'ta001', $sort_order = 'desc', $offset = 0)  //進階查詢流覽資料
	    {	
		  ...
		  else {$_SESSION['acpi03_sql_term'] = '(ta001="") ';}
		  if(@$_POST['find007']){
			$_SESSION['acpi03_sql_sort'] = $_POST['find007'];
		  }
		  else {$_SESSION['acpi03_sql_sort'] = 'ta001';}
	      ...
	      $data['sort_order'] = $sort_order;
	      $this->load->model('acp/acpi03_model');// 加載TABLE model 模型		
	      $result= $this->acpi03_model->findf($limit, $offset , $sort_by  , $sort_order ); //至model 取 mysql 資料 預設 15,0,ta001,desc
	      $data['results'] = $result['rows'];
	      $data['num_results'] = $result['num_rows'];
	      $this->load->library('pagination');
	      $data['numrow']=$result['num_rows'];// 總筆數 
      ...
      public function printdetail()   //印明細起迄資料輸入
	   ...
	   //  $this->load->view('main_headprint_v');
		...
		public function printbb($ta009c)   //印付款單
        {
		  $data['paper9']=$ta009c;
          $this->load->model('acp/acpi03_model','',TRUE);
	      $data['message'] = '列印付款單!';
