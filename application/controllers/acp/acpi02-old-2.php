...
class Acpi02 extends CI_Controller {           //擴展類必须註明由母類擴展而來 (自訂類使用 ci 框架)
	  ...
	  public function index()           //自訂類預設執行函數 流覽資料
	    {                      
	      ...
	      $result= $this->acpi02_model->search($limit, $offset = 0 , $sort_by  , $sort_order ); //至model 取 mysql 資料 預設 15,0,ta001,desc
	    ...
	    //  $this->pagination->initialize($config);    //分頁初始化 display 3
	      $config['base_url'] = site_url("acp/acpi02/display/$sort_by/$sort_order");   //設定分頁url路徑
	      ...
	      $data['systitle'] ='應付憑單資料建立作業';  //網頁抬頭顯示名稱
	  ...
	  public function display($sort_by = 'ta001', $sort_order = 'desc', $offset = 0)  //欄位表頭排序
	    {		
	      ...
	      $data['sort_order'] = $sort_order;
	      $this->load->model('acp/acpi02_model');// 加載TABLE model 模型		
	      $result= $this->acpi02_model->search($limit, $offset , $sort_by  , $sort_order ); //至model 取 mysql 資料 預設 15,0,ta001,desc
	  ...
	  public function lookup(){
        ...
        $query = $this->acpi02_model->lookup(urldecode(urldecode($this->uri->segment(4))),$this->uri->segment(5)); //Search DB 
          ...
          {  
            ...
            foreach( $query as $row )  
              {  
                $data['message'][] = array(   
                                      'category' => '', 
                                      'value' => $row->th001.','.$row->th002.','.$row->th003.','.$row->th019.','.$row->th046,
									  'value1' => $row->th001,
                                      'value2' => $row->th002,
                                      'value3' => $row->th003,
                                      'value4' => $row->th019,
        ...
        if('IS_AJAX')  
         ...
         {  
		    $this->load->view('acp/acpi02_model/lookup',$data); 
      ...
      public function write()   //轉excel 部份資料由 print_v call
        {
	     ...
	     $data['username'] = $this->session->userdata('manager');
	     $title = array('應付憑單別','應付憑單號','退貨日期','廠商代號','廠商名稱','序號','品號','品名','規格','單位','數量','單價','金額');  //excel 表頭
      ...
      public function updsave()   //修改存檔
        {
	     ...
	     redirect('acp/acpi02/'.$this->session->userdata('search1'));
        }
      ...
      public function updform()   //修改輸入資料
        {
	      ...
	      $this->load->model('acp/acpi02_model');
	      $data['result'] = $this->acpi02_model->selone($this->uri->segment(4),$this->uri->segment(5));
	      $data['username'] = $this->session->userdata('manager');
