...
class Acpi03 extends CI_Controller {           //擴展類必须註明由母類擴展而來 (自訂類使用 ci 框架)
	 ...
	 public function index()           //自訂類預設執行函數 流覽資料
	  ...
	  public function display_search($offset = 0,$func = "")  
		...
		$result= $this->acpi03_model->construct_sql($limit, $offset ,$func); //至model 取 mysql 資料 預設 15,0,tf001,desc
		$data['results'] = $result['data'];
		$data['num_results'] = $result['num'];
		$data['numrow'] = $result['num'];// 總筆數 
		$data['page'] = $result['num']/$limit; // 總頁數
		$data['sql'] = $_SESSION['acpi03']['search']['sql'];  //顯示sql語法
		...
		$config['base_url'] = site_url("acp/acpi03/display_search");   //設定分頁url路徑
	  ...
	  public function construct_sql($offset = 0,$func = ""){
		$limit = 15;
		$this->load->model('acp/acpi03_model');// 加載TABLE model 模型
		$this->acpi03_model->construct_sql($limit, $offset ,$func);
	  }
	  ...
	  public function lookup1(){
        ...
        $query = $this->acpi03_model->lookup1(urldecode(urldecode($this->uri->segment(4))),$seq5); //Search DB 
        ...
        if('IS_AJAX')  
         ...
         {  
		    $this->load->view('acp/acpi03_model/lookup1',$data); 
        ...
        if('IS_AJAX')  
         ...
         {  
		    $this->load->view('acp/acpi03_model/lookup2',$data); 
          // $this->index; //Load html view of search results  
         }  
        }  
	    // 下拉視窗不更新網頁查 交貨庫別
	  ...
	  public function findsql($sort_by = 'tc001', $sort_order = 'desc', $offset = 0)  //進階查詢流覽資料
		...
		//}
		  if(@$_POST['find005']){
			$_SESSION['acpi03_sql_term'] = $_POST['find005'];
		  }
		  else {$_SESSION['acpi03_sql_term'] = '(tc001="") ';}
		  if(@$_POST['find007']){
			$_SESSION['acpi03_sql_sort'] = $_POST['find007'];
		   }
		  else {$_SESSION['acpi03_sql_sort'] = 'tc001';}
		...
		if (@session_status() == PHP_SESSION_NONE) {
		  ...
		  if(@$_SESSION["acpi03_sql_term"]) {unset($_SESSION["acpi03_sql_term"]);}
		  if(@$_SESSION["acpi03_sql_sort"]) {unset($_SESSION["acpi03_sql_sort"]);}
		  $this->display();
	  }
      ...
      public function addsave()   //新增存檔
        {
	      ...
	      if ($action === 'exist')
		   ...
		   $data['message'] = '資料重複!';}		    
			 ...
			 $action1=$this->acpi03_model->auto_print();  //自動列印1080320
			 if (@$action1 =='Y') {			 
        ...
        {		 
	      ...
	      $data['message'] = '列印結帳單!';
		  //公司參數
		   $result1 = $this->acpi032_model->companyf();
	      $data['results1'] = $result1['rows1'];
		  //系統參數
           $result = $this->acpi032_model->auto_printfb($seq1,$seq2);
	      $data['results'] = $result['rows'];
	     $this->load->vars($data);
		   redirect('acp/acpi03/printbb/'.$seq1."/".$seq2);
        }
      ...
      public function write()   //轉excel 部份資料由 print_v call
        {
	     ...
	     $data['username'] = $this->session->userdata('manager');
	     $title = array('付款單別','付款單號','憑單日期','廠商代號','廠商名稱','序號','來源','憑證單別','憑證單號','憑證序號','應付金額','差額','備註');  //excel 表頭
      ...
      public function updsave()   //修改存檔
        {
		  ...
		  if(isset($_SESSION['acpi03']['search'])){
				$current_index = @$_SESSION['acpi03']['search']['view'][$seq1];
				if($current_index!=0){
					$data['prev'] = $_SESSION['acpi03']['search']['index'][$current_index-1];
				}
				if(isset($_SESSION['acpi03']['search']['index'][$current_index+1])){
					$data['next'] = $_SESSION['acpi03']['search']['index'][$current_index+1];
				}
				...
				if($this->uri->segment(3)=="display"){
					$this->session->set_userdata('acpi03_search',$this->uri->segment(3)."/".$this->uri->segment(4)."/".$this->uri->segment(5)."/".$offset);
				}
		   }
		   ...
		   $data['message'] = '儲存完畢!';
		   $this->load->model('acp/acpi03_model');
		   $data['result'] = $this->acpi03_model->selone($seg1, $seg2);
			
		  //Default columns 檢視明細設定
		   $data['no_col'] = $this->no_col;
		   $data['col_array'] = $this->detail_col;
		   $this->load->model('set/seti01_model');
		   $coldata = $this->seti01_model->get_detail_view($this->session->userdata('manager'), 'acpi03');
      ...
      public function updform()   //修改輸入資料
        {
		  ...
		  if(isset($_SESSION['acpi03']['search'])&&isset($_SESSION['acpi03']['search']['view'][$view_str])){
			  $current_index = $_SESSION['acpi03']['search']['view'][$view_str];
			  if($current_index!=0){
				 $data['prev'] = $_SESSION['acpi03']['search']['index'][$current_index-1];
			  }
			  if(isset($_SESSION['acpi03']['search']['index'][$current_index+1])){
			  ...
			  $offset = floor($current_index/15)*15;
			  $temp_ident = explode('/',$this->session->userdata('acpi03_search'));
			  $this->session->set_userdata('acpi03_search',"display_search/".$offset);
			  if($temp_ident[0]=="display"){
				 $this->session->set_userdata('acpi03_search',"display/tc002/desc/".$offset);
			  }
		  }
		  ...
		  if($data['result']=="no_data"){
			  redirect('acp/acpi03/'.$this->session->userdata('acpi03_search'));
			  exit;
		  }
      ...
      public function see()   //看資料
        {      
		  ...
		  if(isset($_SESSION['acpi03']['search'])&&isset($_SESSION['acpi03']['search']['view'][$view_str])){
				...
				$this->session->set_userdata('acpi03_search',"display_search/".$offset);
				if($temp_ident[0]=="display"){
					$this->session->set_userdata('acpi03_search',"display/td002/desc/".$offset);
				}
		  }
	...
	public function delete_detail(){
	 ...
	 public function clear_sql()
	  ...
	  public function check_title_no(){
		...
		$this->load->model('acp/acpi03_model','',TRUE);
		$data = $this->acpi03_model->check_title_no($acpi01,$tc016);
		echo $data;
	  }
      ...
      public function del_detail_ajax()   
        {
		  ...
		  $data['message'] = '刪除資料成功!';
		  $this->load->model('acp/acpi03_model','',TRUE);
		  echo $this->acpi03_model->deletedetailf($seg1,$seg2,$seg3);
        }
	//應付憑單快速查詢
