...
class Asti01 extends CI_Controller {           //擴展類必须註明由母類擴展而來 (自訂類使用 ci 框架)
	...
	//欄位表頭排序   資料流覽
	  public function display_leave($offset = 0,$func = "")  
	    {		
			...
			$limit = 15;    //每頁筆數
			$this->load->model('ast/asti01_model');// 加載TABLE model 模型
			$result= $this->asti01_model->construct_sql2($limit, $offset ,$func); //至model 取 mysql 資料 預設 15,0,tf001,desc
			$data['results'] = $result['data'];
			$data['num_results'] = $result['num'];
			$data['numrow'] = $result['num'];// 總筆數 
			$data['page'] = $result['num']/$limit; // 總頁數
			...
			$config['cur_tag_close'] = '</a>'; // 當前頁结束樣式
			$config['cur_page'] = $this->uri->segment(4,0);   //當前頁 結合分頁url路徑 +1
			$config['base_url'] = site_url("ast/asti01/display_leave");   //設定分頁url路徑
			/* 網址去除".html" */
			$temp_url = explode(".html",$config['base_url']);
			$config['base_url'] = "";
			foreach($temp_url as $key => $val){$config['base_url'].=$val;}
			...
			$data['menu_v'] = 'main_menu_v';
			$data['content_v'] = 'ast/asti01_browl_v';		
			$data['foot_v'] ='main_foot_v';
			$this->load->vars($data);
			$this->load->view('main_headbrow_v');	
	    }
	...
	public function display_child($offset = 0,$func = "")  
	  {
		...
		$data['curpage'] = $this->uri->segment(4,1);   //當前頁第6無時顯示 1
		$data['limit'] = $limit ;    //每頁筆數
		$data['systitle'] ='資產類別資料建立作業';		  
		$data['menu_v'] = 'main_menu_v';
		$data['content_v'] = 'funnew/asti01_child_v';		
		$data['foot_v'] ='main_foot_v';
		$this->load->vars($data);
	  ...
	  public function lookup(){
        ...
        $query = $this->asti01_model->lookup(urldecode(urldecode($this->uri->segment(4)))); //Search DB 
        ...
        if('IS_AJAX')  
         ...
         {  
		    $this->load->view('ast/asti01_model/lookup',$data); 
	  ...
	  public function lookupa(){
        ...
        $data['response'] = 'false'; //Set default response 
        $this->load->model('ast/asti01_model');
        $query = $this->asti01_model->lookupa(urldecode(urldecode($this->uri->segment(4)))); //Search DB 
        ...
        if('IS_AJAX')  
         ...
         {  
		    $this->load->view('ast/asti01_model/lookupa',$data); 
          // $this->index; //Load html view of search results  
         }  
        }  
		public function datapurq04a()   //提示改輸入資料如 客戶訂單別   不更新網頁
          {
	        $this->load->model('ast/asti01_model');
	        $data['result'] = $this->asti01_model->ajaxpurq04a($this->uri->segment(4));
            $Result = $data['result'];		  
	        $this->load->vars($data);
	        echo  $Result;
          }
	   ...
	   public function datacmsq05a()   //提示改輸入資料如 請購部門 不更新網頁
        {
	      $this->load->model('ast/asti01_model');
	      $data['result'] = $this->asti01_model->ajaxcmsq05a($this->uri->segment(4));
          $Result = $data['result'];		  
	      $this->load->vars($data);
	      echo  $Result;
        }
	   public function datacmsq02a()  //提示改輸入資料如 廠別 不更新網頁tc010
        {
	      $this->load->model('ast/asti01_model');
	      $data['result'] = $this->asti01_model->ajaxcmsq02a($this->uri->segment(4));
          $Result = $data['result'];		  
	      $this->load->vars($data);
	      echo  $Result;
        }
        ...
        {
	      $this->load->model('ast/asti01_model');
	      $data['result'] = $this->asti01_model->ajaxpalq01a($this->uri->segment(4));
          $Result = $data['result'];		  
	      $this->load->vars($data);
	      echo  $Result;
        }
	   public function datachkno1()   //提示改輸入資料如  客戶訂單號 不更新網頁tc012
        {
	      $this->load->model('ast/asti01_model');
	      $data['result'] = $this->asti01_model->ajaxchkno1($this->uri->segment(4),$this->uri->segment(5));
          $Result = $data['result'];		  
	      $this->load->vars($data);
	      echo  $Result;
        }
	  ...
	  public function findsql($sort_by = 'ma001', $sort_order = 'desc', $offset = 0)  //進階查詢流覽資料
	    {		
	      ...
	      $data['sort_order'] = $sort_order;
	      $this->load->model('ast/asti01_model');// 加載TABLE model 模型		
	      $result= $this->asti01_model->findf($limit, $offset , $sort_by  , $sort_order ); //至model 取 mysql 資料 預設 15,0,tc001,desc
	      $data['results'] = $result['rows'];
	      $data['num_results'] = $result['num_rows'];
	      $this->load->library('pagination');
	      $data['numrow']=$result['num_rows'];// 總筆數 
		...
		if (@session_status() == PHP_SESSION_NONE) {
		  ...
		  //1060809
		  unset($_SESSION['asti01']['search']['where']);
		  unset($_SESSION['asti01']['search']['order']);
		  unset($_SESSION['asti01']['search']['offset']);
		  $this->display();
	  }
      public function addform()   //新增輸入資料
        {
		 //Default columns 檢視明細設定
		  ...
		  $coldata = $this->seti01_model->get_detail_view($this->session->userdata('manager'), 'asti01');
		  if($coldata=="no_data"){
	     ...
	     $data['systitle'] ='客戶訂單資料-新增資料';
		  ...
		  $result2 = $this->asti01_model->funsysf();
	      ...
	      if ($action === 'exist')
	       {
	        $data['message'] = '資料重複!';		    
			  ...
			  $this->asti01_model->auto_print();
		  }
	      $data['systitle'] ='資產類別建立作業-新增資料';
	      $data['menu_v'] = 'main_menuno_v';
	      $data['content_v'] = 'ast/asti01_add_v';
	      $data['foot_v'] ='main_foot_v';
	      $this->load->vars($data);
      ...
      public function astyform()   //複製資料輸入
        {
	     ...
	     $data['menu_v'] = 'main_menuno_v';
	     $data['content_v'] = 'ast/asti01_asty_v';
	     $data['foot_v'] ='main_foot_v';
	     $this->load->vars($data);
	     $this->load->view('main_head_v');
        }
      ...
      public function astysave()   //複製存檔
        {
         ...
         $this->load->model('ast/asti01_model','',TRUE);
	     $data['message'] = '複製成功!';
         $action = $this->asti01_model->astyf();
      ...
      public function printdetail()   //印明細起迄資料輸入
		 ...
		 public function printdetailc()   //印明細起迄資料輸入
        {
	     ...
	     $data['systitle'] ='客戶訂單資料-印明細表';
	     $data['menu_v'] = 'main_menuno_v';
	     $data['content_v'] = 'ast/asti01_print1_v';
	     $data['foot_v'] ='main_foot_v';
	     $this->load->vars($data);
	     $this->load->view('main_head_v');
        }
		public function printc()   //印客戶訂單
        {
	      ...
	      $data['message'] = '列印客戶訂單!';
		     //公司參數
		   $result1 = $this->asti01_model->companyf();
	      $data['results1'] = $result1['rows1'];
		  
           $result = $this->asti01_model->printfc();
	   ...
	   //   $data['systitle'] ='客戶訂單資料-印明細表';
	      //$data['menu_v'] = 'main_menuno_v';
	   //   $data['content_v'] = 'ast/asti01_printb_v';
	   //  $data['foot_v'] ='main_footno_v';
	     $this->load->vars($data);
	   //  $this->load->view('main_headprint_v');
	     $this->load->view('ast/asti01_printc_v');  
        }
		public function printbb($ta009c)   //印客戶訂單
        {
	      ...
	      $data['results1'] = $result1['rows1'];
           ...
           $result = $this->asti01_model->printfb();
	      $data['results'] = $result['rows'];
	     $this->load->vars($data);
	     $this->load->view('ast/asti01_printb_v');
        }
      ...
      public function printa()   //印明細
        {
	     ...
	     $this->load->vars($data);
	     $this->load->view('main_headprint_v');
	     //$this->load->view('ast/asti01_printa_v',$data);  
        }
		
      public function updsave()   //修改存檔
        {
		    $seg1 = $this->input->post('ma001');
			$seg2 = $this->input->post('ma002');
			...
			if(isset($_SESSION['asti01']['search'])){
				$current_index = @$_SESSION['asti01']['search']['view'][$seq1];
				if($current_index!=0){
				...
				if($this->uri->segment(3)=="display"){
					$this->session->set_userdata('asti01_search',$this->uri->segment(3)."/".$this->uri->segment(4)."/".$offset);
				}
			}
			...
			$data['message'] = '儲存完畢!';
			$this->load->model('ast/asti01_model');			
			$data['result'] = $this->asti01_model->selone($seg1, $seg2);
			//Default columns 檢視明細設定
			$data['no_col'] = $this->no_col;
			$data['col_array'] = $this->detail_col;
			$this->load->model('set/seti01_model');
      ...
      public function updform()   //修改輸入資料
        {
		  ...
		  $data['seq2'] = $this->uri->segment(5);
			...
			if(isset($_SESSION['asti01']['search'])&&isset($_SESSION['asti01']['search']['view'][$view_str])){
				$current_index = $_SESSION['asti01']['search']['view'][$view_str];
				if($current_index!=0){
					$data['prev'] = $_SESSION['asti01']['search']['index'][$current_index-1];
				}
				if(isset($_SESSION['asti01']['search']['index'][$current_index+1])){
				...
				if($temp_ident[0]=="display"){
					$this->session->set_userdata('asti01_search',"display/ma001/desc/".$offset);
				}
			}
	      ...
	      $this->load->model('ast/asti01_model');
	      $data['result'] = $this->asti01_model->selone($seg1);
      ...
      public function see()   //看資料
        {      
		  ...
		  $data['seq2'] = $this->uri->segment(5);
			...
			if(isset($_SESSION['asti01']['search'])&&isset($_SESSION['asti01']['search']['view'][$view_str])){
				...
				$this->session->set_userdata('asti01_search',"display_search/".$offset);
				if($temp_ident[0]=="display"){
					$this->session->set_userdata('asti01_search',"display/tc002/desc/".$offset);
				}
			}
	    ...
	    /*  $data['message'] = '查看一筆資料!';
	      $this->load->model('ast/asti01_model');
	      $data['result'] = $this->asti01_model->selone($this->uri->segment(4),$this->uri->segment(5)); */
	      $data['username'] = $this->session->userdata('manager');
	      $data['systitle'] ='資產類別建立作業-查看資料';
	      $data['menu_v'] = 'main_menuno_v';
	 ...
	 public function printb()   //印單據選取
        {    
	      ...
	      $this->load->model('ast/asti01_model','',TRUE);
	      $data['message'] = '列印單據成功!';
          $result = $this->asti01_model->printfd1($this->uri->segment(4),$this->uri->segment(5));
	      $data['results'] = $result['rows'];
	      $data['num_results'] = $result['num_rows'];
	      $this->load->library('pagination');
	      $data['numrow']=$result['num_rows'];// 總筆數 
	...
	public function delete_detail(){
		$data['message'] = '刪除資料成功!';
		$this->load->model('ast/asti01_model','',TRUE);
		$this->asti01_model->del_detail();
	    redirect('ast/asti01/updform/'.$_POST['del_md001'].'/'.$_POST['del_md002']);   //重新整理
	}
	...
	public function check_title_no(){
		...
		$data = $this->asti01_model->check_title_no($asti03_asti01,$tc027);
    ...
    public function del_detail_ajax()   
        {
			...
			echo $this->asti01_model->deletedetailf($seg1,$seg2,$seg3);
        }
}
