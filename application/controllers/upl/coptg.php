<?php 
class coptg extends CI_Controller {
    ...
    public  function index()
      {
	    ...
	    $data['message1'] = '';
		 ...
		 $data['systitle'] ='excel上傳資料至mysql';	
	      ...
	      $data['content_v'] = 'upl/coptg_upload_v';		
	      $data['foot_v'] ='main_foot_v';
	      $this->load->vars($data);
	      $this->load->view('main_headbrow_v');	
      //  $this->load->view('upl/coptg_upload_v');  
      }
	  ...
	  public  function do_del()
      {
	    ...
	    $data['message'] = '刪除資料成功!';
	    $this->load->model('upl/coptg_model','',TRUE);
	    $this->coptg_model->deletef($seg1,$seg2); 
    ...
    public function read()
	  {  
        $rows = array('company','creator','usr_group','create_date','modifier','modi_date','flag','tg001','tg002','tg003','tg004','tg005','tg006',
		               'tg007','tg008','tg009','tg010','tg011','tg012','tg013','tg014','tg015','tg016','tg017','tg018','tg019','tg020', 
					   'tg021','tg022','tg023','tg024','tg025','tg026','tg027','tg028','tg029','tg030','tg031','tg032','tg033','tg034',
					   'tg035','tg036','tg037','tg038','tg039','tg040','tg041','tg042','tg043','tg044','tg045','tg046','tg047','tg048',
					   'tg049','tg050','tg051','tg052','tg053','tg054','tg055','tg056','tg057','tg058','tg059','tg060','tg061' );
	...
	//	$KeyWord = explode(",",$KeyWord);
	  //  $KeyWord =  array('ta002','ta031');    //修改key值
	     ...
	     $KeyWord1 =  'tg001';    //修改key值
		 $KeyWord2 =  'tg002';    //修改key值
      ...
      //  $this->FilterExsel($Exsel_data,$KeyWord);            //檢查資料重複
	    ...
	    $this->load->model('upl/coptg_model');   //true 系統會使用反單引號（`, 在英文鍵盤左上方）來保護你的欄位或資料表名稱
	       
        $DB_data  =  $this->coptg_model->selUsers();                //查询DB中的全部數據
       //   return   'kk2';    
    ...
    public  function write()
      {
        ...
        $data = $this->coptg_model->selUsers();
    ...
    public   function addExcel($value='')
     {  
	    $this->load->model('upl/coptg_model','',TRUE);   //true 系統會使用反單引號（`, 在英文鍵盤左上方）來保護你的欄位或資料表名稱
        return $this->coptg_model->add_Excel($value);   
     }
     /**
      * 更新方法
      * 
    ...
    public    function updExcel($value='')
      {  
        return $this->coptg_model->upd_Excel($value);  
      }
    /**
    *篩選所有的key欄位組成數组    
    *@param   DB數據
