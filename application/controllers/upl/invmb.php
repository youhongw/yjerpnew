<?php 
class Invmb extends CI_Controller {
    ...
    public  function index()
      {
	    ...
	    $data['message1'] = '';
		 ...
		 $data['systitle'] ='excel上傳資料至mysql';	
	      ...
	      $data['content_v'] = 'upl/invmb_upload_v';		
	      $data['foot_v'] ='main_foot_v';
	      $this->load->vars($data);
	      $this->load->view('main_headbrow_v');	
      //  $this->load->view('upl/invmb_upload_v');  
      }
	  ...
	  public  function do_del()
      {
	    ...
	    $data['message'] = '刪除資料成功!';
	    $this->load->model('upl/invmb_model','',TRUE);
	    $this->invmb_model->deletef($seg1,$seg2); 
    ...
    public function read()
	  {  
        $rows = array('company','creator','usr_group','create_date','modifier','modi_date','flag','mb001','mb002','mb003','mb004','mb005','mb006',
					   ...
					   'mb063','mb064','mb065','mb066','mb067','mb068','mb069','mb070','mb071','mb072','mb073','mb074','mb075','mb076',
					   'mb077','mb078','mb079','mb080','mb081','mb082','mb083','mb084','mb085','mb086','mb087','mb088','mb089','mb090',
					   'mb091','mb092','mb093','mb094','mb095','mb096' );
	...
	//	$KeyWord = explode(",",$KeyWord);
	  //  $KeyWord =  array('ta002','ta031');    //修改key值
	     ...
	     $KeyWord1 =  'mb001';    //修改key值
		 $KeyWord2 =  'mb002';    //修改key值
      ...
      //  $this->FilterExsel($Exsel_data,$KeyWord);            //檢查資料重複
	    ...
	    $this->load->model('upl/invmb_model');   //true 系統會使用反單引號（`, 在英文鍵盤左上方）來保護你的欄位或資料表名稱
	       
        $DB_data  =  $this->invmb_model->selUsers();                //查询DB中的全部數據
       //   return   'kk2';    
    ...
    public  function write()
      {
        ...
        $data = $this->invmb_model->selUsers();
    ...
    public   function addExcel($value='')
     {  
	    $this->load->model('upl/invmb_model','',TRUE);   //true 系統會使用反單引號（`, 在英文鍵盤左上方）來保護你的欄位或資料表名稱
        return $this->invmb_model->add_Excel($value);   
     }
     /**
      * 更新方法
      * 
    ...
    public    function updExcel($value='')
      {  
        return $this->invmb_model->upd_Excel($value);  
      }
    /**
    *篩選所有的key欄位組成數组    
    *@param   DB數據
