<?php 
class Cmsmc extends CI_Controller {
    ...
    public  function index()
      {
	    ...
	    $data['message1'] = '';
		 ...
		 $data['systitle'] ='excel上傳資料至mysql';	
	      ...
	      $data['content_v'] = 'upl/cmsmc_upload_v';		
	      $data['foot_v'] ='main_foot_v';
	      $this->load->vars($data);
	      $this->load->view('main_headbrow_v');	
      //  $this->load->view('upl/cmsmc_upload_v');  
      }
	  ...
	  public  function do_del()
      {
	    ...
	    $data['message'] = '刪除資料成功!';
	    $this->load->model('upl/cmsmc_model','',TRUE);
	    $this->cmsmc_model->deletef($seg1,$seg2); 
	...
	//	$KeyWord = explode(",",$KeyWord);
      ...
      //  $this->FilterExsel($Exsel_data,$KeyWord);            //檢查資料重複
	    ...
	    $this->load->model('upl/cmsmc_model');   //true 系統會使用反單引號（`, 在英文鍵盤左上方）來保護你的欄位或資料表名稱
	       
        $DB_data  =  $this->cmsmc_model->selUsers();                //查询DB中的全部數據
       //   return   'kk2';    
    ...
    public   function addExcel($value='')
     {  
	    $this->load->model('upl/cmsmc_model','',TRUE);   //true 系統會使用反單引號（`, 在英文鍵盤左上方）來保護你的欄位或資料表名稱
        return $this->cmsmc_model->add_Excel($value);   
     }
     /**
      * 更新方法
      * 
    ...
    public    function updExcel($value='')
      {  
        return $this->cmsmc_model->upd_Excel($value);  
      }
    /**
    *篩選所有的key欄位組成數组    
    *@param   DB數據
