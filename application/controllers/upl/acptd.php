<?php 
class acptd extends CI_Controller {
    ...
    public  function index()
      {
	    ...
	    $data['message1'] = '';
		 ...
		 $data['systitle'] ='excel上傳資料至mysql';	
	      ...
	      $data['content_v'] = 'upl/acptd_upload_v';		
	      $data['foot_v'] ='main_foot_v';
	      $this->load->vars($data);
	      $this->load->view('main_headbrow_v');	
      //  $this->load->view('upl/acptd_upload_v');  
      }
	  ...
	  public  function do_del()
      {
	    ...
	    $data['message'] = '刪除資料成功!';
	    $this->load->model('upl/acptd_model','',TRUE);
	    $this->acptd_model->deletef($seg1,$seg2); 
    ...
    public function read()
	  {  
        $rows = array('company','creator','usr_group','create_date','modifier','modi_date','flag','td001','td002','td003','td004','td005','td006',
		               'td007','td008','td009','td010','td011','td012','td013','td014','td015','td016','td017','td018','td019','td020', 
					   'td021','td022' );
	...
	//	$KeyWord = explode(",",$KeyWord);
	  //  $KeyWord =  array('ta002','ta031');    //修改key值
	     ...
	     $KeyWord1 =  'td001';    //修改key值
		 $KeyWord2 =  'td002';    //修改key值
      ...
      //  $this->FilterExsel($Exsel_data,$KeyWord);            //檢查資料重複
	    ...
	    $this->load->model('upl/acptd_model');   //true 系統會使用反單引號（`, 在英文鍵盤左上方）來保護你的欄位或資料表名稱
	       
        $DB_data  =  $this->acptd_model->selUsers();                //查询DB中的全部數據
       //   return   'kk2';    
    ...
    public   function addExcel($value='')
     {  
	    $this->load->model('upl/acptd_model','',TRUE);   //true 系統會使用反單引號（`, 在英文鍵盤左上方）來保護你的欄位或資料表名稱
        return $this->acptd_model->add_Excel($value);   
     }
     /**
      * 更新方法
      * 
    ...
    public    function updExcel($value='')
      {  
        return $this->acptd_model->upd_Excel($value);  
      }
    /**
    *篩選所有的key欄位組成數组    
    *@param   DB數據
