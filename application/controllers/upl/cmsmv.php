<?php 
class cmsmv extends CI_Controller {
    ...
    public  function index()
      {
	    ...
	    $data['message1'] = '';
		 ...
		 $data['systitle'] ='excel上傳資料至mysql';	
	      ...
	      $data['content_v'] = 'upl/cmsmv_upload_v';		
	      $data['foot_v'] ='main_foot_v';
	      $this->load->vars($data);
	      $this->load->view('main_headbrow_v');	
      //  $this->load->view('upl/cmsmv_upload_v');  
      }
	  ...
	  public  function do_del()
      {
	    ...
	    $data['message'] = '刪除資料成功!';
	    $this->load->model('upl/cmsmv_model','',TRUE);
	    $this->cmsmv_model->deletef($seg1,$seg2); 
    ...
    public function read()
	  {  
        $rows = array('company','creator','usr_group','create_date','modifier','modi_date','flag','mv001','mv002','mv003','mv004','mv005','mv006',
		               'mv007','mv008','mv009','mv010','mv011','mv012','mv013','mv014','mv015','mv016','mv017','mv018','mv019','mv020', 
					   'mv021','mv022','mv023','mv024','mv025','mv026','mv027','mv028','mv029','mv030','mv031','mv032','mv033','mv034',
					   'mv035','mv036','mv037','mv038','mv039','mv040','mv041','mv042','mv043',
					   'mv044','mv045','mv046','mv047','mv048','mv049','mv050','mv051','mv052','mv053' );
	...
	//	$KeyWord = explode(",",$KeyWord);
	  //  $KeyWord =  array('ta002','ta031');    //修改key值
	     ...
	     $KeyWord1 =  'mv001';    //修改key值
		 $KeyWord2 =  'mv002';    //修改key值
      ...
      //  $this->FilterExsel($Exsel_data,$KeyWord);            //檢查資料重複
	    ...
	    $this->load->model('upl/cmsmv_model');   //true 系統會使用反單引號（`, 在英文鍵盤左上方）來保護你的欄位或資料表名稱
	       
        $DB_data  =  $this->cmsmv_model->selUsers();                //查询DB中的全部數據
       //   return   'kk2';    
    ...
    public   function addExcel($value='')
     {  
	    $this->load->model('upl/cmsmv_model','',TRUE);   //true 系統會使用反單引號（`, 在英文鍵盤左上方）來保護你的欄位或資料表名稱
        return $this->cmsmv_model->add_Excel($value);   
     }
     /**
      * 更新方法
      * 
    ...
    public    function updExcel($value='')
      {  
        return $this->cmsmv_model->upd_Excel($value);  
      }
    /**
    *篩選所有的key欄位組成數组    
    *@param   DB數據
