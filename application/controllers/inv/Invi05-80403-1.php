...
class Invi05 extends CI_Controller {           //擴展類必须註明由母類擴展而來 (自訂類使用 ci 框架)
	  ...
	  public function index()           //自訂類預設執行函數 流覽資料
	    {                      
	      ...
	      $result= $this->invi05_model->search($limit, $offset = 0 , $sort_by  , $sort_order ); //至model 取 mysql 資料 預設 15,0,ta001,desc
	   ...
	   //   $this->pagination->initialize($config);    //分頁初始化 display 3
	      $config['base_url'] = site_url("inv/invi05/display/$sort_by/$sort_order");   //設定分頁url路徑
	  ...
	  public function display($sort_by = 'ta002', $sort_order = 'desc', $offset = 0)  //欄位表頭排序
	    {		
	      ...
	      $data['sort_order'] = $sort_order;
	      $this->load->model('inv/invi05_model');// 加載TABLE model 模型		
	      $result= $this->invi05_model->search($limit, $offset , $sort_by  , $sort_order ); //至model 取 mysql 資料 預設 15,0,ta001,desc
      ...
      public function updsave()   //修改存檔
        {
         ...
         $this->load->model('inv/invi05_model','',TRUE);
	     $this->load->vars($data);
	     $this->invi05_model->updatef();
	     redirect('inv/invi05/'.$this->session->userdata('search1'));
        }
