...
{           //擴展類必须註明由母類擴展而來 (自訂類使用 ci 框架)
	...
	{  //清除條件
		...
		if (@$_SESSION["invi02_sql_term"]) {
			unset($_SESSION["invi02_sql_term"]);
		}
		if (@$_SESSION["invi02_sql_sort"]) {
			unset($_SESSION["invi02_sql_sort"]);
		}
		$this->display();
	}
	...
	public function display_child($offset = 0, $func = "")
	{
		...
		$data['systitle'] = '品號基本資料建立作業';
		$data['menu_v'] = 'main_menu_v';
		$data['content_v'] = 'funnew/invi02_child_v';
		$data['foot_v'] = 'main_foot_v';
		$this->load->vars($data);
		$this->load->view('main_headchild_v');
	}
	//欄位表頭排序   資料流覽 開視窗用
	public function display_childe($offset = 0, $func = "")
	{
		if (session_status() == PHP_SESSION_NONE) {
			session_start();
		}
		...
		$config['cur_tag_close'] = '</a>'; // 當前頁结束樣式
		$config['cur_page'] = $this->uri->segment(4, 0);   //當前頁 結合分頁url路徑 +1  ,分頁初始化 display 3 + 2 + 1 = 6
		$config['base_url'] = site_url("inv/invi02/display_childe");   //設定分頁url路徑
		/* 網址去除".html" */
		$temp_url = explode(".html", $config['base_url']);
		$config['base_url'] = "";
		foreach ($temp_url as $key => $val) {
		...
		$data['systitle'] = '品號基本資料建立作業';
		$data['menu_v'] = 'main_menu_v';
		$data['content_v'] = 'funnew/invi02e_child_v';
		$data['foot_v'] = 'main_foot_v';
		$this->load->vars($data);
		$this->load->view('main_headchild_v');
	}
	...
	public function display_childa($offset = 0, $func = "")
	{
		...
		$config['cur_tag_close'] = '</a>'; // 當前頁结束樣式
		$config['cur_page'] = $this->uri->segment(4, 0);   //當前頁 結合分頁url路徑 +1  ,分頁初始化 display 3 + 2 + 1 = 6
		$config['base_url'] = site_url("inv/invi02/display_childa");   //設定分頁url路徑
		/* 網址去除".html" */
		$temp_url = explode(".html", $config['base_url']);
		$config['base_url'] = "";
		foreach ($temp_url as $key => $val) {
	...
	public function display_childi($offset = 0, $func = "")
	{
		if (session_status() == PHP_SESSION_NONE) {
			session_start();
		}
		...
		$data['message'] = '資料流覽成功!';
		$this->load->model('inv/invi02_model'); // 加載TABLE model 模型
		$result = $this->invi02_model->construct_sqli($limit, $offset, $func); //至model 取 mysql 資料 預設 15,0,ma001,desc
		$data['results'] = $result['data'];
		$data['num_results'] = $result['num'];
		$this->load->library('pagination');
		$data['numrow'] = $result['num']; // 總筆數 
		...
		$config['cur_tag_close'] = '</a>'; // 當前頁结束樣式
		$config['cur_page'] = $this->uri->segment(4, 0);   //當前頁 結合分頁url路徑 +1  ,分頁初始化 display 3 + 2 + 1 = 6
		$config['base_url'] = site_url("inv/invi02/display_childi");   //設定分頁url路徑
		/* 網址去除".html" */
		$temp_url = explode(".html", $config['base_url']);
		$config['base_url'] = "";
		foreach ($temp_url as $key => $val) {
		...
		$data['systitle'] = '品號基本資料建立作業';
		$data['menu_v'] = 'main_menu_v';
		$data['content_v'] = 'funnew/invi02i_child_v';
		$data['foot_v'] = 'main_foot_v';
		$this->load->vars($data);
		$this->load->view('main_headchild_v');
	}
	public function display_child2($offset = 0, $func = "")
	{
		...
		$config['cur_tag_close'] = '</a>'; // 當前頁结束樣式
		$config['cur_page'] = $this->uri->segment(4, 0);   //當前頁 結合分頁url路徑 +1  ,分頁初始化 display 3 + 2 + 1 = 6
		$config['base_url'] = site_url("inv/invi02/display_child2");   //設定分頁url路徑
		/* 網址去除".html" */
		$temp_url = explode(".html", $config['base_url']);
		$config['base_url'] = "";
		foreach ($temp_url as $key => $val) {
	...
	//品號單頭快速查詢
	public function lookup1_invi02()
	{
		$keyword = urldecode($this->uri->segment(4));
		$data['response'] = 'false'; //Set default response 
		$this->load->model('inv/invi01_model');

		$query = $this->invi01_model->lookup1($keyword); //Search DB 
		if (!empty($query)) {
	...
	//品號單頭快速查詢
	public function lookup1_invi02a()
	{
	...
	public function lookup_check()
	{
		...
		$data  = $this->invi02_model->lookup_check($keyword); //Search DB 
		// if (!empty($query)) {
		// 	$data['response'] = 'true'; //Set response  
		// 	$data['message'] = array(); //Create array  
		// 	foreach ($query as $row) {
		// 		$data['message'][] = array(	//注意引數key值得照規矩來value,value1,...
		// 			'category' => '',
		// 			'value' => $row->mb002 . "," . $row->mb003, //顯示用的欄位
		// 			'value1' => $row->mb001,
		// 			'value2' => $row->mb002,
		// 			'value3' => $row->mb003,
		// 			'value4' => $row->mb004,
	...
	public function lookupd2_invi02()
	{
		...
		$keyword = urldecode($this->uri->segment(4));
		$this->load->model('inv/invi02_model');
		$data = $this->invi02_model->lookupd2($keyword); //Search DB
	...
	public function check_key()
	{
		...
		if ($seg1 != "") {
			$this->load->model('inv/invi02_model', '', TRUE);
			$data = $this->invi02_model->ajaxkey1($seg1);
			// $relust = $data ? 'Y' : 'N';
	...
	public function checkkey2()
	{
		extract($this->input->get());
		// $seg1 = trim($this->uri->segment(4));
		if ($mb001 != "") {
			$this->load->model('inv/invi02_model', '', TRUE);
			$data = $this->invi02_model->ajaxkey2($mb001);
			// $relust = $data ? 'Y' : 'N';
