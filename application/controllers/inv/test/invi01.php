...
class Invi01 extends CI_Controller {
    ...
    public function seeform()   //看資料 
    {
	   ...
	   $data['date']= date("Ymd");
	    $data['username'] = $this->session->userdata('manager');
       $this->load->view('inv/invi01_see_v',$data);
     ...
     public function findform()   //進階查詢輸入資料
    {
	   ...
	   $data['date']= date("Ymd");
	    $data['username'] = $this->session->userdata('manager');
       $this->load->view('inv/invi01_find_v',$data);
    ...
    public function findsql($num = '')   //進階查詢資料
    {
        ...
        $this->load->helper('url'); 		
		$this->load->model('inv/invi01_model','',TRUE);
		$result=$this->invi01_model->findf();
    ...
    public function findsqlold($num = '', $sort_by = 'create_date', $sort_order = 'desc', $offset = 0)   //篩選
    {
		...
		$data['sort_order'] = 'asc' ;		
	     ...
	     $this->load->model('inv/invi01_model','',TRUE);
	     $result=$this->invi01_model->findf($offset,$limit); 
		$data['message'] = '進階查詢資料成功!';	
	    $data['results'] = $result['rows'];
	...
	//	$num = '15';    //總筆數
	    ...
	    $this->pagination->initialize($config);//分頁初始化 
	 //   $config['base_url'] = site_url("inv/invi01/findsql");  //設定分頁url路徑		
        $config['base_url'] = '/index.php/inv/invi01/findsql/';  // 設定分頁url路徑		
  ...
  public function copyform()   //複製
    {
	   ...
	   $data['date']= date("Ymd");
	    $data['username'] = $this->session->userdata('manager');
		$data['message'] = '';
       $this->load->view('inv/invi01_copy_v',$data);
   ...
   public function exceldetail()   //轉excel明細
    {
	   ...
	   $this->load->helper('url');
		$data['date']=date("Ymd");
		 $data['message'] = '';
		 $data['username'] = $this->session->userdata('manager');
	   $this->load->view('inv/invi01_excel_v',$data);
   ...
   public function printdetail()   //印明細
    {
	   ...
	   $this->load->helper('url');
		$data['date']=date("Ymd");
		 $data['username'] = $this->session->userdata('manager');
		 $data['message'] = '';
	   $this->load->view('inv/invi01_print_v',$data);
  ...
  public function printa()   //印明細
    {
	   ...
	   $data['results'] = $result['rows'];
	    $data['username'] = $this->session->userdata('manager');
	   $this->load->view('inv/invi01_printa_v',$data);
