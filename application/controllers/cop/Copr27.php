...
class Copr27 extends CI_Controller {           //擴展類必须註明由母類擴展而來 (自訂類使用 ci 框架)
	  ...
	  public function index()           //自訂類預設執行函數 流覽資料
	    {                      
	      ...
	      $this->load->model('cop/copr27_model');     // 加載TABLE model 模型		
	      $result= $this->copr27_model->search($limit, $offset = 0 , $sort_by  , $sort_order ); //至model 取 mysql 資料 預設 15,0,tg001,desc
	    ...
	    //  $this->pagination->initialize($config);    //分頁初始化 display 3
	      ...
	      $data['content_v'] = 'cop/copr27_brow_v';		
	  ...
	  public function display($sort_by = 'tg001', $sort_order = 'desc', $offset = 0)  //欄位表頭排序
	    {		
	      ...
	      $data['sort_order'] = $sort_order;
	      $this->load->model('cop/copr27_model');// 加載TABLE model 模型		
	      $result= $this->copr27_model->search($limit, $offset , $sort_by  , $sort_order ); //至model 取 mysql 資料 預設 15,0,tg001,desc
      ...
      public function exceldetail()   //轉excel明細輸入起迄資料
        {
	     $data['message'] = '';
	     $data['username'] = $this->session->userdata('manager');
	     $data['systitle'] ='客戶年月未收款統計表-轉excel檔';
	     $data['menu_v'] = 'main_menuno_v';
	     $data['content_v'] = 'cop/copr27_excel_v';
	     $data['foot_v'] ='main_foot_v';
	     $this->load->vars($data);
      ...
      public function printdetail()   //印明細起迄資料輸入
        {
	     $data['username'] = $this->session->userdata('manager');
	     $data['message'] = '';
	     $data['systitle'] ='客戶年月未收款統計表-印明細表';
	     $data['menu_v'] = 'main_menuno_v';
	     $data['content_v'] = 'cop/copr27_print_v';
	     $data['foot_v'] ='main_foot_v';
	     $this->load->vars($data);
	     $this->load->view('main_head_v');
        }
      ...
      public function printa()   //印明細
        {
          ...
          $this->load->model('cop/copr27_model','',TRUE);
	      $data['message'] = '列印明細成功!';
          $result = $this->copr27_model->printfd();
	      $data['results'] = $result['rows'];
	      ...
	      $data['systitle'] ='客戶年月未收款統計表-印明細表';
	      //$data['menu_v'] = 'main_menuno_v';
	      $data['content_v'] = 'cop/copr27_printa_v';
	     //$data['foot_v'] ='main_footno_v';
	     $this->load->vars($data);
	     $this->load->view('main_headprint_v');
	     //$this->load->view('cop/copr27_printa_v',$data);  
        }
	 ...
	 public function printb()   //印單據選取
        {    
	      ...
	      $this->load->model('cop/copr27_model','',TRUE);
	      $data['message'] = '列印單據成功!';
          $result = $this->copr27_model->printfd1($this->uri->segment(4),$this->uri->segment(5));
	      $data['results'] = $result['rows'];
	      $data['num_results'] = $result['num_rows'];
	      $this->load->library('pagination');
	      $data['numrow']=$result['num_rows'];// 總筆數 
	      $data['username'] = $this->session->userdata('manager');
	      $data['systitle'] ='請  購  單';	
       //   $this->load->view('cop/copr27_printb_v');
          
	      $data['content_v'] = 'cop/copr27_printb_v';	   
	      $this->load->vars($data);
	      $this->load->view('main_headprint_v');
