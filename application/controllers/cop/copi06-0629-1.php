...
class Copi06 extends CI_Controller {           //擴展類必须註明由母類擴展而來 (自訂類使用 ci 框架)
	  ...
	  public function index()           //自訂類預設執行函數 流覽資料
	    {                      
	      ...
	      $result= $this->copi06_model->search($limit, $offset = 0 , $sort_by  , $sort_order ); //至model 取 mysql 資料 預設 15,0,tc001,desc
	    ...
	    //$config['NUM_LINKS'] = 2;   //分頁前後出現2個數字
	      ...
	      $config['base_url'] = site_url("cop/copi06/display/$sort_by/$sort_order");   //設定分頁url路徑
	      ...
	      $data['systitle'] ='客戶訂單資料建立作業';  //網頁抬頭顯示名稱
	  ...
	  public function display($sort_by = 'tc002', $sort_order = 'desc', $offset = 0)  //欄位表頭排序
	    {		
		 ...
		 // if !isset($data['message']) {
	      ...
	      $data['sort_order'] = $sort_order;
	      $this->load->model('cop/copi06_model');// 加載TABLE model 模型		
	      $result= $this->copi06_model->search($limit, $offset , $sort_by  , $sort_order ); //至model 取 mysql 資料 預設 15,0,tc001,desc
