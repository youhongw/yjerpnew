...
class Copi05 extends CI_Controller {           //擴展類必须註明由母類擴展而來 (自訂類使用 ci 框架)
	  ...
	  public function index()           //自訂類預設執行函數 流覽資料
	    {                      
	      ...
	      $result= $this->copi05_model->search($limit, $offset = 0 , $sort_by  , $sort_order ); //至model 取 mysql 資料 預設 15,0,ta001,desc
	   ...
	   //   $this->pagination->initialize($config);    //分頁初始化 display 3
	      $config['base_url'] = site_url("cop/copi05/display/$sort_by/$sort_order");   //設定分頁url路徑
	      ...
	      $data['systitle'] ='報價單資料建立作業';  //網頁抬頭顯示名稱
	  ...
	  public function display($sort_by = 'ta002', $sort_order = 'desc', $offset = 0)  //欄位表頭排序
	    {		
	      ...
	      $data['sort_order'] = $sort_order;
	      $this->load->model('cop/copi05_model');// 加載TABLE model 模型		
	      $result= $this->copi05_model->search($limit, $offset , $sort_by  , $sort_order ); //至model 取 mysql 資料 預設 15,0,ta001,desc
      ...
      public function copysave()   //複製存檔
        {
         ...
         $this->load->model('cop/copi05_model','',TRUE);
	     $data['message'] = '複製成功!';
         $action = $this->copi05_model->copyf();
      ...
      public function updsave()   //修改存檔
        {
	     ...
	     redirect('cop/copi05/'.$this->session->userdata('search1'));
        }
      ...
      public function updform()   //修改輸入資料
        {
	      ...
	      $this->load->model('cop/copi05_model');
	      $data['result'] = $this->copi05_model->selone($this->uri->segment(4),$this->uri->segment(5));
	      $data['username'] = $this->session->userdata('manager');
	      $data['systitle'] ='報價單資料-修改資料';
	      $data['menu_v'] = 'main_menuno_v';
