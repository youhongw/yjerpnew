...
class copi08 extends CI_Controller {           //擴展類必须註明由母類擴展而來 (自訂類使用 ci 框架)
	  ...
	  public function index()           //自訂類預設執行函數 流覽資料
	    {                      
	      ...
	      $result= $this->copi08_model->search($limit, $offset = 0 , $sort_by  , $sort_order ); //至model 取 mysql 資料 預設 15,0,tg001,desc
	    ...
	    //  $this->pagination->initialize($config);    //分頁初始化 display 3
	      $config['base_url'] = site_url("cop/copi08/display/$sort_by/$sort_order");   //設定分頁url路徑
	  ...
	  public function display($sort_by = 'tg002', $sort_order = 'desc', $offset = 0)  //欄位表頭排序
	    {		
	      ...
	      $data['sort_order'] = $sort_order;
	      $this->load->model('cop/copi08_model');// 加載TABLE model 模型		
	      $result= $this->copi08_model->search($limit, $offset , $sort_by  , $sort_order ); //至model 取 mysql 資料 預設 15,0,tg001,desc
	  ...
	  public function findsql($sort_by = 'tg001', $sort_order = 'desc', $offset = 0)  //進階查詢流覽資料
	    {		
		  ...
		  else {$_SESSION['admi05_sql_term'] = '(tg001="") ';}
		  if(@$_POST['find007']){
			$_SESSION['admi05_sql_sort'] = $_POST['find007'];
		  }
		  else {$_SESSION['admi05_sql_sort'] = 'tg001';}
      ...
      public function copysave()   //複製存檔
		  ...
		  public function copybefore()   //前置單據存檔
        {
	      ...
	      $data['otg042'] = $this->uri->segment(8); //日期20160822
	      $data['message'] = '查詢一筆資料!';
	      $this->load->model('cop/copi08_model');
	      $data['result'] = $this->copi08_model->selonebefore($this->uri->segment(4),$this->uri->segment(5));
		   //系統參數
		  $result2 = $this->copi08_model->funsysf();
	      $data['results2'] = $result2['rows2'];
	      $data['username'] = $this->session->userdata('manager');
      ...
      public function updsave()   //修改存檔
        {
	     ...
	     redirect('cop/copi08/'.$this->session->userdata('search1'));
        }
      ...
      public function updform()   //修改輸入資料
        {
	      ...
	      $this->load->model('cop/copi08_model');
	      $data['result'] = $this->copi08_model->selone($this->uri->segment(4),$this->uri->segment(5));
	      $data['username'] = $this->session->userdata('manager');
