...
class Copi06 extends CI_Controller {           //擴展類必须註明由母類擴展而來 (自訂類使用 ci 框架)
      ...
      public function printa()   //印明細
        {
	     ...
	     $this->load->vars($data);
	     $this->load->view('main_headprint_v');
	     //$this->load->view('cop/copi06_printa_v',$data);  
        }
        ...
        {      
		 ...
		 //以下暫存view處理，上一筆下一筆用
			...
			if(isset($_SESSION['copi06']['search'])){
				...
				$this->session->set_userdata('copi06_search',"display_search/".$offset);
				if($temp_ident[0]=="display"){
					$this->session->set_userdata('copi06_search',"display/mb001/asc/".$offset);  //品號
				}
				if($temp_ident[0]=="display_leave"){
					$this->session->set_userdata('copi06_search',"display_leave/mb001/asc/".$offset);
				}
			}
	      $data['message'] = '查看一筆資料!';
	      $this->load->model('cop/copi06_model');
	      $data['result'] = $this->copi06_model->selone($this->uri->segment(4),$this->uri->segment(5));
	      $data['username'] = $this->session->userdata('manager');
	...
	public function check_title_no(){
		...
		$this->load->model('cop/copi06_model','',TRUE);
		$data = $this->copi06_model->check_title_no($tc001,$tc039);
