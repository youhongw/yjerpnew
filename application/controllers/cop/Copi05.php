...
class Copi05 extends CI_Controller {           //擴展類必须註明由母類擴展而來 (自訂類使用 ci 框架)
	  ...
	  public function display_search($offset = 0,$func = "")  
		...
		$limit = 15;    //每頁筆數
		$this->load->model('cop/copi05_model');// 加載TABLE model 模型
		//echo "<pre>";var_dump($limit);exit;
		
		$result= $this->copi05_model->construct_sql($limit, $offset ,$func); //至model 取 mysql 資料 預設 15,0,tf001,desc
		$data['results'] = $result['data'];
		$data['num_results'] = $result['num'];
		$data['numrow'] = $result['num'];// 總筆數 
		$data['page'] = $result['num']/$limit; // 總頁數
		$data['sql'] = $_SESSION['copi05']['search']['sql'];  //顯示sql語法
		...
		$config['base_url'] = site_url("cop/copi05/display_search");   //設定分頁url路徑
		...
		$data['systitle'] ='報價單建立作業';		  
		$data['menu_v'] = 'main_menu_v';
		$data['content_v'] = 'cop/copi05_brow_v';		
	  ...
	  public function lookup(){
        ...
        $this->load->model('cop/copi05_model');
        $query = $this->copi05_model->lookup(urldecode(urldecode($this->uri->segment(4)))); //Search DB 
        ...
        if('IS_AJAX')  
         ...
         {  
		    $this->load->view('cop/copi05_model/lookup',$data); 
          // $this->index; //Load html view of search results  
         }  
        }  
		
	
		public function datapurq04a()   //提示改輸入資料如 報價單別   不更新網頁
          {
	        $this->load->model('cop/copi05_model');
	        $data['result'] = $this->copi05_model->ajaxpurq04a($this->uri->segment(4));
            $Result = $data['result'];		  
	        $this->load->vars($data);
	        echo  $Result;
          }
	   ...
	   public function datacmsq05a()   //提示改輸入資料如 請購部門 不更新網頁
        {
	      $this->load->model('cop/copi05_model');
	      $data['result'] = $this->copi05_model->ajaxcmsq05a($this->uri->segment(4));
          $Result = $data['result'];		  
	      $this->load->vars($data);
	      echo  $Result;
        }
	   public function datacmsq02a()  //提示改輸入資料如 廠別 不更新網頁ta010
        {
	      $this->load->model('cop/copi05_model');
	      $data['result'] = $this->copi05_model->ajaxcmsq02a($this->uri->segment(4));
          $Result = $data['result'];		  
	      $this->load->vars($data);
	      echo  $Result;
        }
        ...
        {
	      $this->load->model('cop/copi05_model');
	      $data['result'] = $this->copi05_model->ajaxpalq01a($this->uri->segment(4));
          $Result = $data['result'];		  
	      $this->load->vars($data);
	      echo  $Result;
        }
	   public function datachkno1()   //提示改輸入資料如  報價單號 不更新網頁ta012
        {
	      $this->load->model('cop/copi05_model');
	      $data['result'] = $this->copi05_model->ajaxchkno1($this->uri->segment(4),$this->uri->segment(5));
          $Result = $data['result'];		  
	      $this->load->vars($data);
	      echo  $Result;
        }
	  ...
	  public function filter1($sort_by = 'ta001', $sort_order = 'desc', $offset = 0)   ////篩選資料
        {
	     ...
	     $this->load->model('cop/copi05_model','',TRUE);   //true 系統會使用反單引號（`, 在英文鍵盤左上方）來保護你的欄位或資料表名稱
	     $result=$this->copi05_model->filterf1($limit, $offset , $sort_by  , $sort_order);
	     $data['message'] = '篩選資料成功!';	
	     ...
	     $config['base_url'] = site_url("cop/copi05/filter1/$sort_by/$sort_order/$seq6/$seq7");  //設定分頁url路徑
      ...
      public function findform()   //進階查詢輸入資料
        {
	      ...
	      $data['username'] = $this->session->userdata('manager');
	      $data['message'] = '';
	      $data['systitle'] ='報價單資料-進階查詢';
	      $data['menu_v'] = 'main_menuno_v';
	      $data['content_v'] = 'cop/copi05_find_v';
	      $data['foot_v'] ='main_foot_v';
	      $this->load->vars($data);
	  ...
	  public function findsql($sort_by = 'ta001', $sort_order = 'desc', $offset = 0)  //進階查詢流覽資料
	    {	
		  ...
		  if(@$_POST['find005']){
			$_SESSION['copi05_sql_term'] = $_POST['find005'];
		   }
		  else {$_SESSION['copi05_sql_term'] = '(ta001="") ';}
		  if(@$_POST['find007']){
			$_SESSION['copi05_sql_sort'] = $_POST['find007'];
		   }
		  else {$_SESSION['copi05_sql_sort'] = 'ta001';}
	      ...
	      $data['sort_order'] = $sort_order;
	      $this->load->model('cop/copi05_model');// 加載TABLE model 模型		
	      $result= $this->copi05_model->findf($limit, $offset , $sort_by  , $sort_order ); //至model 取 mysql 資料 預設 15,0,ta001,desc
	      $data['results'] = $result['rows'];
	      $data['num_results'] = $result['num_rows'];
	      $this->load->library('pagination');
	      $data['numrow']=$result['num_rows'];// 總筆數 
	    ...
	    //$config['NUM_LINKS'] = 2;   //分頁前後出現2個數字
	      ...
	      $config['base_url'] = site_url("cop/copi05/findsql/$sort_by/$sort_order");   //設定分頁url路徑
	      $config['total_rows'] = $result['num_rows']; // 總筆數
	      $config['per_page'] = $limit;                //每頁筆數
		...
		if (@session_status() == PHP_SESSION_NONE) {
		  ...
		  if(@$_SESSION["copi05_sql_term"]) {unset($_SESSION["copi05_sql_term"]);}
		  if(@$_SESSION["copi05_sql_sort"]) {unset($_SESSION["copi05_sql_sort"]);}
		  $this->display();
	  }
      ...
      public function addform()   //新增輸入資料
        {
	     ...
	     $data['message'] = '';
	     $data['username'] = $this->session->userdata('manager');
	     $data['systitle'] ='報價單資料-新增資料';
	     $data['menu_v'] = 'main_menuno_v';
	     $data['content_v'] = 'cop/copi05_add_v';
	     $data['foot_v'] ='main_foot_v';
	     $this->load->vars($data);
	     $this->load->view('main_head_v');
        }
      ...
      public function addsave()   //新增存檔
        {
		  ...
		  $coldata = $this->seti01_model->get_detail_view($this->session->userdata('manager'), 'copi05');
			if($coldata=="no_data"){
				...
				$usecol_array = explode(',',$coldata->ta013);
				$data['usecol_array'] = array();
				foreach($usecol_array as $key => $val){
					$data['usecol_array'][$val] = $data['col_array'][$val];
				}
	      ...
	      $data['username'] = $this->session->userdata('manager');
          $this->load->model('cop/copi05_model','',TRUE);
	      $data['message'] = '新增成功!';
	      $action = $this->copi05_model->insertf();
	      if ($action === 'exist')
	       {
	        $data['message'] = '資料重複!';		    
	       }
      ...
      public function copyform()   //複製資料輸入
        {
	     $data['username'] = $this->session->userdata('manager');
	     $data['message'] = '';
	     $data['systitle'] ='報價單資料-複製資料';
	     $data['menu_v'] = 'main_menuno_v';
	     $data['content_v'] = 'cop/copi05_copy_v';
	     $data['foot_v'] ='main_foot_v';
	     $this->load->vars($data);
	     $this->load->view('main_head_v');
        }
      ...
      public function copysave()   //複製存檔
        {
         ...
         $this->load->model('cop/copi05_model','',TRUE);
	     $data['message'] = '複製成功!';
         $action = $this->copi05_model->copyf($ta001o,$ta002o,$ta001c,$ta013);
      ...
      public function exceldetail()   //轉excel明細輸入起迄資料
        {
	     $data['message'] = '';
	     $data['username'] = $this->session->userdata('manager');
	     $data['systitle'] ='報價單資料-轉excel檔';
	     $data['menu_v'] = 'main_menuno_v';
	     $data['content_v'] = 'cop/copi05_excel_v';
	     $data['foot_v'] ='main_foot_v';
	     $this->load->vars($data);
	     $this->load->view('main_head_v');
        }
      ...
      public function write()   //轉excel 部份資料由 print_v call
        {
	     ...
	     $data['username'] = $this->session->userdata('manager');
	     $title = array('報價單別','報價單號','核價日期','客戶代號','客戶名稱','幣別','業務員','付款條件','序號','品號','品名','規格','單位','數量','單價','金額');  //excel 表頭
         $result1 = $this->copi05_model->excelnewf();	
      ...
      public function printdetail()   //印明細起迄資料輸入
        {
	     $data['username'] = $this->session->userdata('manager');
	     $data['message'] = '';
	     $data['systitle'] ='報價單資料-印明細表';
	     $data['menu_v'] = 'main_menuno_v';
	     $data['content_v'] = 'cop/copi05_print_v';
	     $data['foot_v'] ='main_foot_v';
	     $this->load->vars($data);
	     $this->load->view('main_head_v');
        }
		 public function printdetailc()   //印明細起迄資料輸入
        {
	     ...
	     $data['systitle'] ='報價單資料-印明細表';
	     $data['menu_v'] = 'main_menuno_v';
	     $data['content_v'] = 'cop/copi05_print1_v';
	     $data['foot_v'] ='main_foot_v';
	     $this->load->vars($data);
	     $this->load->view('main_head_v');
        }
		public function printc()   //印報價單
        {
          $this->load->model('cop/copi05_model','',TRUE);
	      $data['message'] = '列印報價單!';
           $result = $this->copi05_model->printfc();
		  
	      $data['results'] = $result['rows'];
	     $this->load->vars($data);
	   //  $this->load->view('main_headprint_v');
	     $this->load->view('cop/copi05_printc_v');  
        }
		public function printbb()   //印報價單
        {
          $this->load->model('cop/copi05_model','',TRUE);
	      $data['message'] = '列印報價單!';
		   //公司參數
		   $result1 = $this->copi05_model->companyf();
	      $data['results1'] = $result1['rows1'];
		  
           $result = $this->copi05_model->printfb();
	      $data['results'] = $result['rows'];
	     $this->load->vars($data);
	     $this->load->view('cop/copi05_printb_v');  
        }
      public function printa()   //印明細
        {
          ...
          $this->load->model('cop/copi05_model','',TRUE);
	      $data['message'] = '列印明細成功!';
          $result = $this->copi05_model->printfd();
	      $data['results'] = $result['rows'];
	      $data['num_results'] = $result['num_rows'];
	      $this->load->library('pagination');
	      $data['numrow']=$result['num_rows'];// 總筆數 
	      ...
	      $data['systitle'] ='報價單資料-印明細表';
	      //$data['menu_v'] = 'main_menuno_v';
	      $data['content_v'] = 'cop/copi05_printa_v';
	     //$data['foot_v'] ='main_footno_v';
	     $this->load->vars($data);
	     $this->load->view('main_headprint_v');
	     //$this->load->view('cop/copi05_printa_v',$data);  
        }
      ...
      public function updsave()   //修改存檔
        {
		  ...
		  if(isset($_SESSION['copi05']['search'])){
				$current_index = @$_SESSION['copi05']['search']['view'][$seq1];
				if($current_index!=0){
					$data['prev'] = $_SESSION['copi05']['search']['index'][$current_index-1];
				}
				if(isset($_SESSION['copi05']['search']['index'][$current_index+1])){
					$data['next'] = $_SESSION['copi05']['search']['index'][$current_index+1];
				}
				...
				if($this->uri->segment(3)=="display"){
					$this->session->set_userdata('copi05_search',$this->uri->segment(3)."/".$this->uri->segment(4)."/".$this->uri->segment(5)."/".$offset);
				}
         ...
         $this->load->model('cop/copi05_model','',TRUE);
	     $this->load->vars($data);
	     $this->copi05_model->updatef();
		// $data['seq1'] = $this->uri->segment(4); 
		   $data['message'] = '儲存完畢!';
		   $this->load->model('cop/copi05_model');
		   $data['result'] = $this->copi05_model->selone($seg1, $seg2);
		  ...
		  //Default columns 檢視明細設定
		   ...
		   $data['menu_v'] = 'main_menuno_v';
		   $data['content_v'] = 'cop/copi05_upd_v';
		   $data['foot_v'] ='main_foot_v';
		   $this->load->vars($data);
		   $this->load->view('main_head_v');
        }
      ...
      public function updform()   //修改輸入資料
        {
		  ...
		  if(isset($_SESSION['copi05']['search'])&&isset($_SESSION['copi05']['search']['view'][$view_str])){
			  $current_index = $_SESSION['copi05']['search']['view'][$view_str];
			  if($current_index!=0){
				 $data['prev'] = $_SESSION['copi05']['search']['index'][$current_index-1];
			  }
			  if(isset($_SESSION['copi05']['search']['index'][$current_index+1])){
			  ...
			  $offset = floor($current_index/15)*15;
			  $temp_ident = explode('/',$this->session->userdata('copi05_search'));
			  $this->session->set_userdata('copi05_search',"display_search/".$offset);
			  if($temp_ident[0]=="display"){
				 $this->session->set_userdata('copi05_search',"display/ta002/desc/".$offset);
			  }
		  }
		  ...
		  if($data['result']=="no_data"){
			  redirect('cop/copi05/'.$this->session->userdata('copi05_search'));
			  exit;
		  }
      ...
      public function see()   //看資料
        {      
	      ...
	      $data['username'] = $this->session->userdata('manager');
	      $data['systitle'] ='報價單資料-查看';
	      $data['menu_v'] = 'main_menuno_v';
	      $data['content_v'] = 'cop/copi05_see_v';
	      $data['foot_v'] ='main_foot_v';
	      $this->load->vars($data);
	      $this->load->view('main_head_v');
        }
      ...
      public function del()   //刪除單筆 暫存
        {      
	      ...
	      $data['message'] = '刪除資料成功!';
	      $this->load->model('cop/copi05_model','',TRUE);
	      $this->copi05_model->deletef($seg1,$seg2);
	      $this->display();
        }
	 ...
	 public function printb()   //印單據選取
        {    
	      ...
	      $this->load->model('cop/copi05_model','',TRUE);
	      $data['message'] = '列印單據成功!';
          $result = $this->copi05_model->printfd1($this->uri->segment(4),$this->uri->segment(5));
	      $data['results'] = $result['rows'];
	      $data['num_results'] = $result['num_rows'];
	      $this->load->library('pagination');
	      $data['numrow']=$result['num_rows'];// 總筆數 
       ...
       //   $this->load->view('cop/copi05_printb_v');
	      ...
	      $data['content_v'] = 'cop/copi05_printb_v';	   
	      $this->load->vars($data);
	      $this->load->view('main_headprint_v');
	...
	public function delete_detail(){
		$data['message'] = '刪除資料成功!';
		$this->load->model('cop/copi05_model','',TRUE);
		$this->copi05_model->del_detail();
	    redirect('cop/copi05/updform/'.$_POST['del_md001'].'/'.$_POST['del_md002']);   //重新整理
	}
     /*==以下AJAX處理區域==*/
	  //抓取最新一筆的編號
	  public function check_title_no(){
		...
		$this->load->model('cop/copi05_model','',TRUE);
		$data = $this->copi05_model->check_title_no($copi03,$ta013);
		echo $data;
	  }
	   //刪除單筆細項AJAX
      public function del_detail_ajax()   
        {
		  ...
		  $data['message'] = '刪除資料成功!';
		  $this->load->model('cop/copi05_model','',TRUE);
		  echo $this->copi05_model->deletedetailf($seg1,$seg2,$seg3);
        }
	 //查看bom筆數
...
/* End of file copi05.php */
/* Location: ./application/controllers/copi05.php */
?>