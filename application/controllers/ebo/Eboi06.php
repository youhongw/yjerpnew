...
class Eboi06 extends CI_Controller {           //擴展類必须註明由母類擴展而來 (自訂類使用 ci 框架)
	  ...
	  public function index()           //自訂類預設執行函數 流覽資料
	    {                      
	      ...
	      $this->load->model('ebo/eboi06_model');     // 加載TABLE model 模型		
	      $result= $this->eboi06_model->search($limit, $offset = 0 , $sort_by  , $sort_order ); //至model 取 mysql 資料 預設 15,0,mc001,desc
	   ...
	   //   $this->pagination->initialize($config);    //分頁初始化 display 3
	      $config['base_url'] = site_url("ebo/eboi06/display/$sort_by/$sort_order");   //設定分頁url路徑
	      ...
	      $data['systitle'] ='E-BOM轉BOM維護作業';  //網頁抬頭顯示名稱
	      $data['menu_v'] = 'main_menu_v';
	      $data['content_v'] = 'ebo/eboi06_brow_v';		
	  ...
	  public function display($sort_by = 'mc001', $sort_order = 'desc', $offset = 0)  //欄位表頭排序
	    {		
	      ...
	      $data['sort_order'] = $sort_order;
	      $this->load->model('ebo/eboi06_model');// 加載TABLE model 模型		
	      $result= $this->eboi06_model->search($limit, $offset , $sort_by  , $sort_order ); //至model 取 mysql 資料 預設 15,0,mc001,desc
	  ...
	  public function lookup(){
        ...
        $this->load->model('ebo/eboi06_model');
        $query = $this->eboi06_model->lookup(urldecode(urldecode($this->uri->segment(4)))); //Search DB 
        ...
        if('IS_AJAX')  
         ...
         {  
		    $this->load->view('ebo/eboi06_model/lookup',$data); 
		...
		public function datapurq04a()   //提示改輸入資料如 核價單別   不更新網頁
          {
	        $this->load->model('ebo/eboi06_model');
	        $data['result'] = $this->eboi06_model->ajaxpurq04a($this->uri->segment(4));
            $Result = $data['result'];		  
	        $this->load->vars($data);
	        echo  $Result;
          }
	   ...
	   public function datacmsq05a()   //提示改輸入資料如 請購部門 不更新網頁
        {
	      $this->load->model('ebo/eboi06_model');
	      $data['result'] = $this->eboi06_model->ajaxcmsq05a($this->uri->segment(4));
          $Result = $data['result'];		  
	      $this->load->vars($data);
	      echo  $Result;
        }
	   public function datacmsq02a()  //提示改輸入資料如 廠別 不更新網頁mc010
        {
	      $this->load->model('ebo/eboi06_model');
	      $data['result'] = $this->eboi06_model->ajaxcmsq02a($this->uri->segment(4));
          $Result = $data['result'];		  
	      $this->load->vars($data);
	      echo  $Result;
        }
        ...
        {
	      $this->load->model('ebo/eboi06_model');
	      $data['result'] = $this->eboi06_model->ajaxpalq01a($this->uri->segment(4));
          $Result = $data['result'];		  
	      $this->load->vars($data);
	      echo  $Result;
        }
	   public function datachkno1()   //提示改輸入資料如  核價單號 不更新網頁mc012
        {
	      $this->load->model('ebo/eboi06_model');
	      $data['result'] = $this->eboi06_model->ajaxchkno1($this->uri->segment(4),$this->uri->segment(5));
          $Result = $data['result'];		  
	      $this->load->vars($data);
	      echo  $Result;
        }
	  ...
	  public function filter1($sort_by = 'mc001', $sort_order = 'desc', $offset = 0)   ////篩選資料
        {
	     ...
	     $data['sort_order'] = $sort_order;
	     $this->load->model('ebo/eboi06_model','',TRUE);   //true 系統會使用反單引號（`, 在英文鍵盤左上方）來保護你的欄位或資料表名稱
	     $result=$this->eboi06_model->filterf1($limit, $offset , $sort_by  , $sort_order);
	     $data['message'] = '篩選資料成功!';	
	     $data['results'] = $result['rows'];
	     $data['num_results'] = $result['num_rows'];
	     $this->load->library('pagination');
	     ...
	     $config['base_url'] = site_url("ebo/eboi06/filter1/$sort_by/$sort_order/$seq6/$seq7");  //設定分頁url路徑
      ...
      public function findform()   //進階查詢輸入資料
        {
	      ...
	      $data['content_v'] = 'ebo/eboi06_find_v';
	      $data['foot_v'] ='main_foot_v';
	      $this->load->vars($data);
	  ...
	  public function findsql($sort_by = 'mc001', $sort_order = 'desc', $offset = 0)  //進階查詢流覽資料
	    {		
		  ...
		  if(@$_POST['find005']){
			$_SESSION['eboi06_sql_term'] = $_POST['find005'];
		  }
		  else {$_SESSION['eboi06_sql_term'] = '(mc001="") ';}
		  
		  if(@$_POST['find007']){
			$_SESSION['eboi06_sql_sort'] = $_POST['find007'];
		  }
		  else {$_SESSION['eboi06_sql_sort'] = 'mc001';}
	      ...
	      $data['sort_order'] = $sort_order;
	      $this->load->model('ebo/eboi06_model');// 加載TABLE model 模型		
	      $result= $this->eboi06_model->findf($limit, $offset , $sort_by  , $sort_order ); //至model 取 mysql 資料 預設 15,0,mc001,desc
	      $data['results'] = $result['rows'];
	      $data['num_results'] = $result['num_rows'];
	      $this->load->library('pagination');
	      $data['numrow']=$result['num_rows'];// 總筆數 
	    ...
	    //$config['NUM_LINKS'] = 2;   //分頁前後出現2個數字
	      ...
	      $config['cur_page'] = $this->uri->segment(6,0);   //當前頁 結合分頁url路徑 +1
	      $this->pagination->initialize($config);    //分頁初始化 display 3
	      $config['base_url'] = site_url("ebo/eboi06/findsql/$sort_by/$sort_order");   //設定分頁url路徑
	      $config['total_rows'] = $result['num_rows']; // 總筆數
	      $config['per_page'] = $limit;                //每頁筆數
	      $config['uri_segment'] = 6;       //當前頁
	      $this->pagination->initialize($config);
		...
		if (@session_status() == PHP_SESSION_NONE) {
		  ...
		  if(@$_SESSION["eboi06_sql_term"]) {unset($_SESSION["eboi06_sql_term"]);}
		  if(@$_SESSION["eboi06_sql_sort"]) {unset($_SESSION["eboi06_sql_sort"]);}
		  $this->display();
	  }
      public function addform()   //新增輸入資料
        {
	     ...
	     $data['menu_v'] = 'main_menuno_v';
	     $data['content_v'] = 'ebo/eboi06_add_v';
	     $data['foot_v'] ='main_foot_v';
	     $this->load->vars($data);
	     $this->load->view('main_head_v');
        }
      ...
      public function addsave()   //新增存檔
        {
	      $data['username'] = $this->session->userdata('manager');
          $this->load->model('ebo/eboi06_model','',TRUE);
	      $data['message'] = '新增成功!';
	      $action = $this->eboi06_model->insertf();
	      if ($action === 'exist')
	       {
	        $data['message'] = '資料重複!';		    
	       }
      ...
      public function copyform()   //複製資料輸入
        {
	     ...
	     $data['menu_v'] = 'main_menuno_v';
	     $data['content_v'] = 'ebo/eboi06_copy_v';
	     $data['foot_v'] ='main_foot_v';
	     $this->load->vars($data);
	     $this->load->view('main_head_v');
        }
      ...
      public function copysave()   //複製存檔
        {
         ...
         $this->load->model('ebo/eboi06_model','',TRUE);
	     $data['message'] = '複製成功!';
         $action = $this->eboi06_model->copyf();
      ...
      public function exceldetail()   //轉excel明細輸入起迄資料
        {
	     ...
	     $data['menu_v'] = 'main_menuno_v';
	     $data['content_v'] = 'ebo/eboi06_excel_v';
	     $data['foot_v'] ='main_foot_v';
	     $this->load->vars($data);
	     $this->load->view('main_head_v');
        }
      ...
      public function write()   //轉excel 部份資料由 print_v call
        {
         ...
         $result1 = $this->eboi06_model->excelnewf();	
      ...
      public function printdetail()   //印明細起迄資料輸入
        {
	     ...
	     $data['menu_v'] = 'main_menuno_v';
	     $data['content_v'] = 'ebo/eboi06_print_v';
	     $data['foot_v'] ='main_foot_v';
	     $this->load->vars($data);
	     $this->load->view('main_head_v');
        }
		 public function printdetailc()   //印明細起迄資料輸入
        {
	     ...
	     $data['systitle'] ='核價單資料-印明細表';
	     $data['menu_v'] = 'main_menuno_v';
	     $data['content_v'] = 'ebo/eboi06_print1_v';
	     $data['foot_v'] ='main_foot_v';
	     $this->load->vars($data);
	     $this->load->view('main_head_v');
        }
		public function printc()   //印核價單
        {
          ...
          $this->load->model('ebo/eboi06_model','',TRUE);
	      $data['message'] = '列印核價單!';
           $result = $this->eboi06_model->printfc();
	   ...
	   //   $data['systitle'] ='核價單資料-印明細表';
	      //$data['menu_v'] = 'main_menuno_v';
	   //   $data['content_v'] = 'ebo/eboi06_printb_v';
	   //  $data['foot_v'] ='main_footno_v';
	     $this->load->vars($data);
	   //  $this->load->view('main_headprint_v');
	     $this->load->view('ebo/eboi06_printc_v');  
        }
		public function printbb()   //印核價單
        {
          $this->load->model('ebo/eboi06_model','',TRUE);
	      $data['message'] = '列印核價單!';
           $result = $this->eboi06_model->printfb();
	      $data['results'] = $result['rows'];
	     $this->load->vars($data);
	     $this->load->view('ebo/eboi06_printb_v');  
        }
      public function printa()   //印明細
        {
          ...
          $this->load->model('ebo/eboi06_model','',TRUE);
	      $data['message'] = '列印明細成功!';
          $result = $this->eboi06_model->printfd();
	      $data['results'] = $result['rows'];
	      $data['num_results'] = $result['num_rows'];
	      $this->load->library('pagination');
	      $data['numrow']=$result['num_rows'];// 總筆數 
	      ...
	      $data['systitle'] ='核價單資料-印明細表';
	      //$data['menu_v'] = 'main_menuno_v';
	      $data['content_v'] = 'ebo/eboi06_printa_v';
	     //$data['foot_v'] ='main_footno_v';
	     $this->load->vars($data);
	     $this->load->view('main_headprint_v');
	     //$this->load->view('ebo/eboi06_printa_v',$data);  
        }
      ...
      public function updsave()   //修改存檔
        {
         ...
         $this->load->model('ebo/eboi06_model','',TRUE);
	     $this->load->vars($data);
	     $this->eboi06_model->updatef();
	     redirect('ebo/eboi06/'.$this->session->userdata('search1'));
        }
      ...
      public function updform()   //修改輸入資料
        {
	      ...
	      $this->load->model('ebo/eboi06_model');
	      $data['result'] = $this->eboi06_model->selone($this->uri->segment(4),$this->uri->segment(5));
	      $data['username'] = $this->session->userdata('manager');
	      $data['systitle'] ='核價單資料-修改資料';
	      $data['menu_v'] = 'main_menuno_v';
	      $data['content_v'] = 'ebo/eboi06_upd_v';
	      $data['foot_v'] ='main_foot_v';
	      $this->load->vars($data);
	      $this->load->view('main_head_v');
        }
      ...
      public function see()   //看資料
        {      
	      ...
	      $data['menu_v'] = 'main_menuno_v';
	      $data['content_v'] = 'ebo/eboi06_see_v';
	      $data['foot_v'] ='main_foot_v';
	      $this->load->vars($data);
	      $this->load->view('main_head_v');
        }
      ...
      public function del()   //刪除單筆 暫存
        {      
	      ...
	      $data['message'] = '刪除資料成功!';
	      $this->load->model('ebo/eboi06_model','',TRUE);
	      $this->eboi06_model->deletef($seg1,$seg2);
	      $this->display();
        }
	 ...
	 public function printb()   //印單據選取
        {    
	      ...
	      $this->load->model('ebo/eboi06_model','',TRUE);
	      $data['message'] = '列印單據成功!';
          $result = $this->eboi06_model->printfd1($this->uri->segment(4),$this->uri->segment(5));
	      $data['results'] = $result['rows'];
	      $data['num_results'] = $result['num_rows'];
	      $this->load->library('pagination');
	      $data['numrow']=$result['num_rows'];// 總筆數 
	...
	public function delete_detail(){
		$data['message'] = '刪除資料成功!';
		$this->load->model('ebo/eboi06_model','',TRUE);
		$this->eboi06_model->del_detail();
	    redirect('ebo/eboi06/updform/'.$_POST['del_md001']);   //重新整理
	}
}
/* End of file eboi06.php */
/* Location: ./application/controllers/eboi06.php */
?>