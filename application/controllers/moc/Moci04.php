...
class Moci04 extends CI_Controller {           //擴展類必须註明由母類擴展而來 (自訂類使用 ci 框架)
	  ...
	  public function display($sort_by = 'mv021', $sort_order = 'asc', $offset = 0)  
	    {
	      ...
	      $this->load->model('moc/moci04_model');// 加載TABLE model 模型		
	      $result= $this->moci04_model->search($limit, $offset , $sort_by  , $sort_order ); //至model 取 mysql 資料 預設 15,0,mv001,desc
	    ...
	    //$config['NUM_LINKS'] = 2;   //分頁前後出現2個數字, 預設5個
	      ...
	      $config['base_url'] = site_url("moc/moci04/display/$sort_by/$sort_order");   //設定分頁url路徑
	      ...
	      $data['content_v'] = 'moc/moci04_brow_v';		
	...
	public function display_search($offset = 0,$func = "")  
	  {
		...
		$limit = 15;    //每頁筆數
		$this->load->model('moc/moci04_model');// 加載TABLE model 模型
		$result= $this->moci04_model->construct_sql($limit, $offset ,$func); //至model 取 mysql 資料 預設 15,0,tf001,desc
		$data['results'] = $result['data'];
		$data['num_results'] = $result['num'];
		$data['numrow'] = $result['num'];// 總筆數 
		$data['page'] = $result['num']/$limit; // 總頁數
		$data['sql'] = $_SESSION['moci04']['search']['sql'];
		...
		$config['base_url'] = site_url("moc/moci04/display_search");   //設定分頁url路徑
		/* 網址去除".html" */
		...
		$limit = 15;
		$this->load->model('moc/moci04_model');// 加載TABLE model 模型
		$this->moci04_model->construct_sql($limit, $offset ,$func);
		}
	  ...
	  public function lookup(){
        ...
        $this->load->model('moc/moci04_model');
        $query = $this->moci04_model->lookup(urldecode(urldecode($this->uri->segment(4)))); //Search DB 
        ...
        if('IS_AJAX')  
         ...
         {  
		    $this->load->view('moc/moci04_model/lookup',$data); 
		...
		public function datapurq04a()   //提示改輸入資料如 核價單別   不更新網頁
          {
	        $this->load->model('moc/moci04_model');
	        $data['result'] = $this->moci04_model->ajaxpurq04a($this->uri->segment(4));
            $Result = $data['result'];		  
	        $this->load->vars($data);
	        echo  $Result;
          }
	   ...
	   public function datacmsq05a()   //提示改輸入資料如 請購部門 不更新網頁
        {
	      $this->load->model('moc/moci04_model');
	      $data['result'] = $this->moci04_model->ajaxcmsq05a($this->uri->segment(4));
          $Result = $data['result'];		  
	      $this->load->vars($data);
	      echo  $Result;
        }
	   public function datacmsq02a()  //提示改輸入資料如 廠別 不更新網頁tl010
        {
	      $this->load->model('moc/moci04_model');
	      $data['result'] = $this->moci04_model->ajaxcmsq02a($this->uri->segment(4));
          $Result = $data['result'];		  
	      $this->load->vars($data);
	      echo  $Result;
        }
        ...
        {
	      $this->load->model('moc/moci04_model');
	      $data['result'] = $this->moci04_model->ajaxpalq01a($this->uri->segment(4));
          $Result = $data['result'];		  
	      $this->load->vars($data);
	      echo  $Result;
        }
	   public function datachkno1()   //提示改輸入資料如  核價單號 不更新網頁tl012
        {
	      $this->load->model('moc/moci04_model');
	      $data['result'] = $this->moci04_model->ajaxchkno1($this->uri->segment(4),$this->uri->segment(5));
          $Result = $data['result'];		  
	      $this->load->vars($data);
	      echo  $Result;
        }
	  ...
	  public function filter1($sort_by = 'tc001', $sort_order = 'desc', $offset = 0)   ////篩選資料
        {
	     ...
	     $data['sort_order'] = $sort_order;
	     $this->load->model('moc/moci04_model','',TRUE);   //true 系統會使用反單引號（`, 在英文鍵盤左上方）來保護你的欄位或資料表名稱
	     $result=$this->moci04_model->filterf1($limit, $offset , $sort_by  , $sort_order);
	     $data['message'] = '篩選資料成功!';	
	     $data['results'] = $result['rows'];
	     $data['num_results'] = $result['num_rows'];
	     $this->load->library('pagination');
	     ...
	     $config['base_url'] = site_url("moc/moci04/filter1/$sort_by/$sort_order/$seq6/$seq7");  //設定分頁url路徑
      ...
      public function findform()   //進階查詢輸入資料
        {
	      ...
	      $data['content_v'] = 'moc/moci04_find_v';
	      $data['foot_v'] ='main_foot_v';
	      $this->load->vars($data);
	  ...
	  public function findsql($sort_by = 'tc001', $sort_order = 'desc', $offset = 0)  //進階查詢流覽資料
	    {	
	      ...
	      $data['sort_order'] = $sort_order;
	      $this->load->model('moc/moci04_model');// 加載TABLE model 模型		
	      $result= $this->moci04_model->findf($limit, $offset , $sort_by  , $sort_order ); //至model 取 mysql 資料 預設 15,0,tl001,desc
	      $data['results'] = $result['rows'];
	      $data['num_results'] = $result['num_rows'];
	      $this->load->library('pagination');
	      $data['numrow']=$result['num_rows'];// 總筆數 
	    ...
	    //$config['NUM_LINKS'] = 2;   //分頁前後出現2個數字
	      ...
	      $config['base_url'] = site_url("moc/moci04/findsql/$sort_by/$sort_order");   //設定分頁url路徑
	      $config['total_rows'] = $result['num_rows']; // 總筆數
	      $config['per_page'] = $limit;                //每頁筆數
	      $config['uri_segment'] = 6;       //當前頁
	      $this->pagination->initialize($config);
      ...
      public function addform()   
        {
		  ...
		  $coldata = $this->seti01_model->get_detail_view($this->session->userdata('manager'), 'moci04');
		  if($coldata=="no_data"){
		  ...
		  $this->load->model('moc/moci04_model');// 加載TABLE model 模型		
		  //$result= $this->moci04_model->op1(); //至model 取 mysql 資料 預設 15,0,mv001,desc
	      ...
	      $data['menu_v'] = 'main_menuno_v';
	      $data['content_v'] = 'moc/moci04_add_v';
	      $data['foot_v'] ='main_foot_v';
	      $this->load->vars($data);
	      $this->load->view('main_head_v');
        }
      ...
      public function addsave()   //新增存檔
        {
	      $data['no_col'] = $this->no_col;
			...
			$data['username'] = $this->session->userdata('manager');
			$this->load->model('moc/moci04_model','',TRUE);
			$data['message'] = '新增成功!';
			$action = $this->moci04_model->insertf();
			if ($action === 'exist')
			{
				$data['message'] = '資料重複!';		    
			}
      ...
      public function copyform()   //複製資料輸入
        {
	     ...
	     $data['menu_v'] = 'main_menuno_v';
	     $data['content_v'] = 'moc/moci04_copy_v';
	     $data['foot_v'] ='main_foot_v';
	     $this->load->vars($data);
	     $this->load->view('main_head_v');
        }
      ...
      public function copysave()   //複製存檔
        {
         ...
         $this->load->model('moc/moci04_model','',TRUE);
	     $data['message'] = '複製成功!';
         $action = $this->moci04_model->copyf();
      ...
      public function exceldetail()   //轉excel明細輸入起迄資料
        {
	     ...
	     $data['menu_v'] = 'main_menuno_v';
	     $data['content_v'] = 'moc/moci04_excel_v';
	     $data['foot_v'] ='main_foot_v';
	     $this->load->vars($data);
	     $this->load->view('main_head_v');
        }
      ...
      public function write()   //轉excel 部份資料由 print_v call
		{
	     ...
	     $data['username'] = $this->session->userdata('manager');
	     $title = array('領退料單別','領退料單號','材料品號','領退料數量','單位','庫別','製程代號','批號','製令單別','製令單號','領料說明','備註','材料型態','材料品名','材料規格','確認碼','領退料包裝數量','包裝單位');  //excel 表頭
         $result1 = $this->moci04_model->excelnewf();	
      ...
      public function printdetail()   //印明細起迄資料輸入
        {
	     ...
	     $data['menu_v'] = 'main_menuno_v';
	     $data['content_v'] = 'moc/moci04_print_v';
	     $data['foot_v'] ='main_foot_v';
	     $this->load->vars($data);
	     $this->load->view('main_head_v');
        }
		 public function printdetailc()   //印明細起迄資料輸入
        {
	     ...
	     $data['systitle'] ='退料單資料-印明細表';
	     $data['menu_v'] = 'main_menuno_v';
	     $data['content_v'] = 'moc/moci04_print1_v';
	     $data['foot_v'] ='main_foot_v';
	     $this->load->vars($data);
	     $this->load->view('main_head_v');
        }
		public function printc()   //印核價單
        {
	      ...
	      $data['message'] = '列印退料單!';
		     //公司參數
		   $result1 = $this->moci04_model->companyf();
	      $data['results1'] = $result1['rows1'];
		  
           $result = $this->moci04_model->printfc();
	   ...
	   //  $this->load->view('main_headprint_v');
	     $this->load->view('moc/moci04_printc_v');  
        }
		public function printbb($tm008c)   //印核價單
        {
	      ...
	      $data['results1'] = $result1['rows1'];
           ...
           $result = $this->moci04_model->printfb();
	      $data['results'] = $result['rows'];
	     $this->load->vars($data);
	     $this->load->view('moc/moci04_printb_v');  
        }
      public function printa()   //印明細
        {
          ...
          $this->load->model('moc/moci04_model','',TRUE);
	      $data['message'] = '列印明細成功!';
          $result = $this->moci04_model->printfd();
	      $data['results'] = $result['rows'];
	      $data['num_results'] = $result['num_rows'];
	      $this->load->library('pagination');
	      $data['numrow']=$result['num_rows'];// 總筆數 
	      ...
	      $data['systitle'] ='退料單資料-印明細表';
	      //$data['menu_v'] = 'main_menuno_v';
	      $data['content_v'] = 'moc/moci04_printa_v';
	     //$data['foot_v'] ='main_footno_v';
	     $this->load->vars($data);
	     $this->load->view('main_headprint_v');
	     //$this->load->view('moc/moci04_printa_v',$data);  
        }
      ...
      public function updsave()   //修改存檔
        {	
			...
			if(isset($_SESSION['moci04']['search'])){
				$current_index = @$_SESSION['moci04']['search']['view'][$seq1];
				if($current_index!=0){
					$data['prev'] = $_SESSION['moci04']['search']['index'][$current_index-1];
				}
				if(isset($_SESSION['moci04']['search']['index'][$current_index+1])){
					$data['next'] = $_SESSION['moci04']['search']['index'][$current_index+1];
				}
				...
				if($this->uri->segment(3)=="display"){
					$this->session->set_userdata('moci04_search',$this->uri->segment(3)."/".$this->uri->segment(4)."/".$this->uri->segment(5)."/".$offset);
				}
			}
			...
			$data['message'] = '儲存完畢!';
			$this->load->model('moc/moci04_model');
			$data['result'] = $this->moci04_model->selone($seg1, $seg2);
			//Default columns 檢視明細設定
			$data['no_col'] = $this->no_col;
			$data['col_array'] = $this->detail_col;
			$this->load->model('set/seti01_model');
			...
			$data['menu_v'] = 'main_menuno_v';
			$data['content_v'] = 'moc/moci04_upd_v';
			$data['foot_v'] ='main_foot_v';
			$this->load->vars($data);
			$this->load->view('main_head_v');
      ...
      public function updform()   //修改輸入資料
        {
			...
			if(isset($_SESSION['moci04']['search'])&&isset($_SESSION['moci04']['search']['view'][$view_str])){
				$current_index = $_SESSION['moci04']['search']['view'][$view_str];
				if($current_index!=0){
					$data['prev'] = $_SESSION['moci04']['search']['index'][$current_index-1];
				}
				if(isset($_SESSION['moci04']['search']['index'][$current_index+1])){
				...
				$offset = floor($current_index/15)*15;
				$temp_ident = explode('/',$this->session->userdata('moci04_search'));
				$this->session->set_userdata('moci04_search',"display_search/".$offset);
				if($temp_ident[0]=="display"){
					$this->session->set_userdata('moci04_search',"display/tc002/desc/".$offset);
				}
			}
			$data['seg1'] = $seg1;
			$data['seg2'] = $seg2;
			...
			if($data['result']=="no_data"){
				redirect('moc/moci04/'.$this->session->userdata('moci04_search'));
				exit;
			}
			//Default columns 檢視明細設定
			$data['no_col'] = $this->no_col;
	      ...
	      $data['message'] = '查詢一筆修改資料!';
	      $this->load->model('moc/moci04_model');
	      $data['result'] = $this->moci04_model->selone($this->uri->segment(4),$this->uri->segment(5));
	      $data['username'] = $this->session->userdata('manager');
	  ...
	  public function lookupa(){
        ...
        $data['response'] = 'false'; //Set default response 
        $this->load->model('moc/moci04_model');
        $query = $this->moci04_model->lookupa(urldecode(urldecode($this->uri->segment(4)))); //Search DB 
        ...
        if('IS_AJAX')  
         ...
         {  
		    $this->load->view('moc/moci04_model/lookupa',$data); 
          // $this->index; //Load html view of search results  
         }  
        }
      ...
      public function see()   //看資料
        {      
			...
			$data['menu_v'] = 'main_menuno_v';
			$data['content_v'] = 'moc/moci04_see_v';
			$data['foot_v'] ='main_foot_v';
			$this->load->vars($data);
			$this->load->view('main_head_v');
      ...
      public function del()   //刪除單筆 暫存
        {      
	      ...
	      $data['message'] = '刪除資料成功!';
	      $this->load->model('moc/moci04_model','',TRUE);
	      $this->moci04_model->deletef($seg1,$seg2);
	      $this->display();
        }
	 ...
	 public function printb()   //印單據選取
        {    
	      ...
	      $this->load->model('moc/moci04_model','',TRUE);
	      $data['message'] = '列印單據成功!';
          $result = $this->moci04_model->printfd1($this->uri->segment(4),$this->uri->segment(5));
	      $data['results'] = $result['rows'];
	      $data['num_results'] = $result['num_rows'];
	      $this->load->library('pagination');
	      $data['numrow']=$result['num_rows'];// 總筆數 
       ...
       //   $this->load->view('moc/moci04_printb_v');
	      ...
	      $data['content_v'] = 'moc/moci04_printb_v';	   
	      $this->load->vars($data);
	      $this->load->view('main_headprint_v');
	...
	public function delete_detail(){
		$data['message'] = '刪除資料成功!';
		$this->load->model('moc/moci04_model','',TRUE);
		$this->moci04_model->del_detail();
	    redirect('moc/moci04/updform/'.$_POST['del_md001'].'/'.$_POST['del_md002']);   //重新整理
	}
	...
	public function check_moci01(){
		...
		$this->load->model('moc/moci04_model','',TRUE);
		$data=$this->moci04_model->check_moci01();
	}
	...
	//品號快速查詢(以製令單別(單身)為搜尋條件)
	public function lookup_invi03(){
	    $keyword1 = urldecode(urldecode($this->uri->segment(4)));
	    $keyword2 = urldecode(urldecode($this->uri->segment(5)));
	    $keyword3 = urldecode(urldecode($this->uri->segment(6)));
        $data['response'] = 'false'; //Set default response 
        ...
        /*$query = $this->invi02_model->lookup2(
			array('mb001','mb002','mb003','mb004'),
			array('and'=>"mb001"),
			array('mb001'=>$keyword),
			15
		...
		$query = $this->invi02_model->lookup1($keyword1,$keyword2,$keyword3);
          ...
          {  
            ...
            foreach( $query as $row )  
              {  
                $data['message'][] = array(	//注意引數key值得照規矩來value,value1,...
				  'category' => '', 
				  'value' => $row->tb003.",".$row->mb002.",".$row->mb003,//顯示用的欄位
				  'value1' => $row->tb003,
				  'value2' => $row->mb002,
				  'value3' => $row->mb003,
				  'value4' => $row->tb004,
	...
	//品號快速查詢(以製令單別(單頭)為搜尋條件)
	public function lookup_invi04(){
	    $keyword1 = urldecode(urldecode($this->uri->segment(4)));
	    $keyword2 = urldecode(urldecode($this->uri->segment(5)));
	    $keyword3 = urldecode(urldecode($this->uri->segment(6)));
        $data['response'] = 'false'; //Set default response 
		...
		$query = $this->invi02_model->lookup2($keyword1,$keyword2,$keyword3);
          ...
          {  
            ...
            foreach( $query as $row )  
              {  
                $data['message'][] = array(	//注意引數key值得照規矩來value,value1,...
				  'category' => '', 
				  'value' => $row->ta006.",".$row->mb002.",".$row->mb003,//顯示用的欄位
				  'value1' => $row->ta006, //品號
				  'value2' => $row->mb002, //品名
				  'value3' => $row->mb003, //規格
				  'value4' => $row->mb004, //單位
	...
	public function lookup_moci04(){
        ...
        $query = $this->moci04_model->lookupmoc(
	...
	public function set_detail_view()   
        {
			...
			$this->load->model('set/seti01_model');
			$data['result'] = $this->seti01_model->get_detail_view($this->session->userdata('manager'), 'moci04');
	...
	public function save_detail_view()
		{
			$data = $this->input->get();
			$this->load->model('set/seti01_model');
			$result = $this->seti01_model->save_detail_view('moci04',$data['order']);
	...
	public function check_title_no(){
		extract($this->input->get());
		$this->load->model('moc/moci04_model','',TRUE);
		$data = $this->moci04_model->check_title_no($puri04,$Ddate);
		echo $data;
	}
    ...
    public function del_detail_ajax()   
        {
			...
			echo $this->moci04_model->deletedetailf($seg1,$seg2,$seg3);
        }
}
/* End of file moci04.php */
/* Location: ./application/controllers/moci04.php */
?>