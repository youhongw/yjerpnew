...
class Moci02 extends CI_Controller {           //擴展類必须註明由母類擴展而來 (自訂類使用 ci 框架)
	  ...
	  public function index()           //自訂類預設執行函數 流覽資料
	    {                      
	      ...
	      $result= $this->moci02_model->search($limit, $offset = 0 , $sort_by  , $sort_order ); //至model 取 mysql 資料 預設 15,0,ta001,desc
	    ...
	    //  $this->pagination->initialize($config);    //分頁初始化 display 3
	      $config['base_url'] = site_url("moc/moci02/display/$sort_by/$sort_order");   //設定分頁url路徑
	      ...
	      $data['systitle'] ='製造命令資料建立作業';  //網頁抬頭顯示名稱
	  ...
	  public function display($sort_by = 'ta001', $sort_order = 'desc', $offset = 0)  //欄位表頭排序
	    {		
	      ...
	      $data['sort_order'] = $sort_order;
	      $this->load->model('moc/moci02_model');// 加載TABLE model 模型		
	      $result= $this->moci02_model->search($limit, $offset , $sort_by  , $sort_order ); //至model 取 mysql 資料 預設 15,0,ta001,desc
      ...
      public function copysave()   //複製存檔
        {
	     ...
	     $this->load->view('main_head_v');  
		  ...
		  public function copybefore()   //前置單據存檔 預計生產量,庫別, 單別, 日期, 單號
        {
		...
		//  $this->session->set_userdata('vmocq01a51', $this->uri->segment(7));
		//  $this->session->set_userdata('vta003', $this->uri->segment(8));
		//  $this->session->set_userdata('vta002', $this->uri->segment(9));
	      $data['message'] = '查詢一筆資料!';
	      $this->load->model('moc/moci02_model');
      ...
      public function write()   //轉excel 部份資料由 print_v call
        {
	     ...
	     $data['username'] = $this->session->userdata('manager');
	     $title = array('製造命令別','製造命令號','進貨日期','供應廠商','製造命令序號','品號','品名','規格','單位','數量','單價','金額');  //excel 表頭
      ...
      public function updsave()   //修改存檔
        {
         ...
         $this->load->model('moc/moci02_model','',TRUE);
	     $this->load->vars($data);
	     $this->moci02_model->updatef();
	     redirect('moc/moci02/'.$this->session->userdata('search1'));
        }
      ...
      public function updform()   //修改輸入資料
        {
	      ...
	      $this->load->model('moc/moci02_model');
	      $data['result'] = $this->moci02_model->selone($this->uri->segment(4),$this->uri->segment(5));
	      $data['username'] = $this->session->userdata('manager');
