...
class Moci02 extends CI_Controller {           //擴展類必须註明由母類擴展而來 (自訂類使用 ci 框架)
	 ...
	 public function index()           //自訂類預設執行函數 流覽資料
	  ...
	  public function display_search($offset = 0,$func = "")  
		...
		$limit = 15;    //每頁筆數
		$this->load->model('moc/moci02_model');// 加載TABLE model 模型
		//echo "<pre>";var_dump($limit);exit;
		
		$result= $this->moci02_model->construct_sql($limit, $offset ,$func); //至model 取 mysql 資料 預設 15,0,tf001,desc
		$data['results'] = $result['data'];
		$data['num_results'] = $result['num'];
		$data['numrow'] = $result['num'];// 總筆數 
		$data['page'] = $result['num']/$limit; // 總頁數
		$data['sql'] = $_SESSION['moci02']['search']['sql'];  //顯示sql語法
		...
		$config['base_url'] = site_url("moc/moci02/display_search");   //設定分頁url路徑
		...
		$data['systitle'] ='製令單建立作業';		  
		$data['menu_v'] = 'main_menu_v';
		$data['content_v'] = 'moc/moci02_brow_v';		
	  ...
	  public function lookup(){
        ...
        $this->load->model('moc/moci02_model');
        $query = $this->moci02_model->lookup(urldecode(urldecode($this->uri->segment(4)))); //Search DB 
        ...
        if('IS_AJAX')  
         ...
         {  
		    $this->load->view('moc/moci02_model/lookup',$data); 
	  ...
	  public function lookupa(){
        ...
        $data['response'] = 'false'; //Set default response 
        $this->load->model('moc/moci02_model');
        $query = $this->moci02_model->lookupa(urldecode(urldecode($this->uri->segment(4)))); //Search DB 
        ...
        if('IS_AJAX')  
         ...
         {  
		    $this->load->view('moc/moci02_model/lookupa',$data); 
          // $this->index; //Load html view of search results  
         }  
        }  
		public function datapurq04a()   //提示改輸入資料如 製造命令別   不更新網頁
          {
	        $this->load->model('moc/moci02_model');
	        $data['result'] = $this->moci02_model->ajaxpurq04a($this->uri->segment(4));
            $Result = $data['result'];		  
	        $this->load->vars($data);
	        echo  $Result;
          }
	   ...
	   public function datacmsq05a()   //提示改輸入資料如 請購部門 不更新網頁
        {
	      $this->load->model('moc/moci02_model');
	      $data['result'] = $this->moci02_model->ajaxcmsq05a($this->uri->segment(4));
          $Result = $data['result'];		  
	      $this->load->vars($data);
	      echo  $Result;
        }
	   public function datacmsq02a()  //提示改輸入資料如 廠別 不更新網頁ta010
        {
	      $this->load->model('moc/moci02_model');
	      $data['result'] = $this->moci02_model->ajaxcmsq02a($this->uri->segment(4));
          $Result = $data['result'];		  
	      $this->load->vars($data);
	      echo  $Result;
        }
        ...
        {
	      $this->load->model('moc/moci02_model');
	      $data['result'] = $this->moci02_model->ajaxpalq01a($this->uri->segment(4));
          $Result = $data['result'];		  
	      $this->load->vars($data);
	      echo  $Result;
        }
	   public function datachkno1()   //提示改輸入資料如  製造命令號 不更新網頁ta002
        {
	      $this->load->model('moc/moci02_model');
	      $data['result'] = $this->moci02_model->ajaxchkno1($this->uri->segment(4),$this->uri->segment(5));
          $Result = $data['result'];		  
	      $this->load->vars($data);
	      echo  $Result;
        }
	  ...
	  public function filter1($sort_by = 'ta001', $sort_order = 'desc', $offset = 0)   ////篩選資料
        {
	     ...
	     $this->load->model('moc/moci02_model','',TRUE);   //true 系統會使用反單引號（`, 在英文鍵盤左上方）來保護你的欄位或資料表名稱
	     ...
	     $config['base_url'] = site_url("moc/moci02/filter1/$sort_by/$sort_order/$seq6/$seq7");  //設定分頁url路徑
      ...
      public function findform()   //進階查詢輸入資料
        {
	      ...
	      $data['content_v'] = 'moc/moci02_find_v';
	      $data['foot_v'] ='main_foot_v';
	      $this->load->vars($data);
	  ...
	  public function findsql($sort_by = 'ta001', $sort_order = 'desc', $offset = 0)  //進階查詢流覽資料
	    {		
	      ...
	      $data['sort_order'] = $sort_order;
	      $this->load->model('moc/moci02_model');// 加載TABLE model 模型		
	      $result= $this->moci02_model->findf($limit, $offset , $sort_by  , $sort_order ); //至model 取 mysql 資料 預設 15,0,ta001,desc
	      $data['results'] = $result['rows'];
	      $data['num_results'] = $result['num_rows'];
	      $this->load->library('pagination');
	      $data['numrow']=$result['num_rows'];// 總筆數 
	    ...
	    //$config['NUM_LINKS'] = 2;   //分頁前後出現2個數字
	      ...
	      $config['base_url'] = site_url("moc/moci02/findsql/$sort_by/$sort_order");   //設定分頁url路徑
	      $config['total_rows'] = $result['num_rows']; // 總筆數
	      $config['per_page'] = $limit;                //每頁筆數
      ...
      public function addform()   //新增輸入資料
        {
		 //Default columns 檢視明細設定
		  ...
		  $coldata = $this->seti01_model->get_detail_view($this->session->userdata('manager'), 'moci02');
		  if($coldata=="no_data"){
	     ...
	     $data['message'] = '';
	     $data['username'] = $this->session->userdata('manager');
	     $data['systitle'] ='製造命令資料-新增資料';
	     $data['menu_v'] = 'main_menuno_v';
	     $data['content_v'] = 'moc/moci02_add_v';
	     $data['foot_v'] ='main_foot_v';
	     $this->load->vars($data);
	     $this->load->view('main_head_v');
        }
      ...
      public function addsave()   //新增存檔
        {
	      ...
	      $data['message'] = '新增成功!';
	      $action = $this->moci02_model->insertf();
	      if ($action === 'exist')
	       {
	        $data['message'] = '資料重複!';		    
	       }
		   else{
			  $this->moci02_model->auto_print($seg1,$seg2);
		  }
	      $data['systitle'] ='製造命令資料-新增資料';
	      $data['menu_v'] = 'main_menuno_v';
	      $data['content_v'] = 'moc/moci02_add_v';
      ...
      public function copyform()   //複製資料輸入
        {
	     ...
	     $data['menu_v'] = 'main_menuno_v';
	     $data['content_v'] = 'moc/moci02_copy_v';
	     $data['foot_v'] ='main_foot_v';
	     $this->load->vars($data);
	     $this->load->view('main_head_v');
        }
      ...
      public function copysave()   //複製存檔
        {
         ...
         $this->load->model('moc/moci02_model','',TRUE);
	     $data['message'] = '複製成功!';
         $action = $this->moci02_model->copyf();
		...
		//  $this->session->set_userdata('vta007',  urldecode(urldecode($this->uri->segment(11))));
	       ...
	       $data['message'] = '查詢一筆資料!';
	      $this->load->model('moc/moci02_model');
	      $data['result'] = $this->moci02_model->selonebefore($this->uri->segment(4),$this->uri->segment(5));
	      $data['username'] = $this->session->userdata('manager');
      ...
      public function exceldetail()   //轉excel明細輸入起迄資料
        {
	     ...
	     $data['menu_v'] = 'main_menuno_v';
	     $data['content_v'] = 'moc/moci02_excel_v';
	     $data['foot_v'] ='main_foot_v';
	     $this->load->vars($data);
	     $this->load->view('main_head_v');
        }
      ...
      public function write()   //轉excel 部份資料由 print_v call
        {
         ...
         $result1 = $this->moci02_model->excelnewf();	
      ...
      public function printdetail()   //印明細起迄資料輸入
        {
	     ...
	     $data['menu_v'] = 'main_menuno_v';
	     $data['content_v'] = 'moc/moci02_print_v';
	     $data['foot_v'] ='main_foot_v';
	     $this->load->vars($data);
	     $this->load->view('main_head_v');
        }
		 public function printdetailc()   //印明細起迄資料輸入
        {
	     ...
	     $data['systitle'] ='製造命令資料-印明細表';
	     $data['menu_v'] = 'main_menuno_v';
	     $data['content_v'] = 'moc/moci02_print1_v';
	     $data['foot_v'] ='main_foot_v';
	     $this->load->vars($data);
	     $this->load->view('main_head_v');
        }
		public function printc()   //印製造命令
        {
          ...
          $this->load->model('moc/moci02_model','',TRUE);
	      $data['message'] = '列印製造命令!';
           $result = $this->moci02_model->printfc();
	   ...
	   //   $data['systitle'] ='製造命令資料-印明細表';
	      //$data['menu_v'] = 'main_menuno_v';
	   //   $data['content_v'] = 'moc/moci02_printb_v';
	   //  $data['foot_v'] ='main_footno_v';
	     $this->load->vars($data);
	   //  $this->load->view('main_headprint_v');
	     $this->load->view('moc/moci02_printc_v');  
        }
		public function printbb()   //印製造命令
        {
          $this->load->model('moc/moci02_model','',TRUE);
	      $data['message'] = '列印製造命令!';
           $result = $this->moci02_model->printfb();
	      $data['results'] = $result['rows'];
	     $this->load->vars($data);
	     $this->load->view('moc/moci02_printb_v');  
        }
      public function printa()   //印明細
        {
          ...
          $this->load->model('moc/moci02_model','',TRUE);
	      $data['message'] = '列印明細成功!';
          $result = $this->moci02_model->printfd();
	      $data['results'] = $result['rows'];
	      $data['num_results'] = $result['num_rows'];
	      $this->load->library('pagination');
	      $data['numrow']=$result['num_rows'];// 總筆數 
	      ...
	      $data['systitle'] ='製造命令資料-印明細表';
	      //$data['menu_v'] = 'main_menuno_v';
	      $data['content_v'] = 'moc/moci02_printa_v';
	     //$data['foot_v'] ='main_footno_v';
	     $this->load->vars($data);
	     $this->load->view('main_headprint_v');
	     //$this->load->view('moc/moci02_printa_v',$data);  
        }
      ...
      public function updsave()   //修改存檔
        {
		  ...
		  if(isset($_SESSION['moci02']['search'])){
				$current_index = @$_SESSION['moci02']['search']['view'][$seq1];
				if($current_index!=0){
					$data['prev'] = $_SESSION['moci02']['search']['index'][$current_index-1];
				}
				if(isset($_SESSION['moci02']['search']['index'][$current_index+1])){
					$data['next'] = $_SESSION['moci02']['search']['index'][$current_index+1];
				}
				...
				if($this->uri->segment(3)=="display"){
					$this->session->set_userdata('moci02_search',$this->uri->segment(3)."/".$this->uri->segment(4)."/".$this->uri->segment(5)."/".$offset);
				}
		   }
	     $data['username'] = $this->session->userdata('manager');
	     $data['message'] = '修改資料成功!';
         $this->load->model('moc/moci02_model','',TRUE);
	     $this->load->vars($data);
	     $this->moci02_model->updatef($seg1, $seg2);
		 //  $data['seq1'] = $this->uri->segment(4); 
		   $data['message'] = '儲存完畢!';
		   $this->load->model('moc/moci02_model');
		   $data['result'] = $this->moci02_model->selone($seg1, $seg2);
	     ...
	     $data['message'] = '修改資料成功!';
         $data['menu_v'] = 'main_menuno_v';
		   $data['content_v'] = 'moc/moci02_upd_v';
		   $data['foot_v'] ='main_foot_v';
		   $this->load->vars($data);
		   $this->load->view('main_head_v');
        }
      ...
      public function updform()   //修改輸入資料
        {
		  ...
		  if(isset($_SESSION['moci02']['search'])&&isset($_SESSION['moci02']['search']['view'][$view_str])){
			  $current_index = $_SESSION['moci02']['search']['view'][$view_str];
			  if($current_index!=0){
				 $data['prev'] = $_SESSION['moci02']['search']['index'][$current_index-1];
			  }
			  if(isset($_SESSION['moci02']['search']['index'][$current_index+1])){
			  ...
			  $offset = floor($current_index/15)*15;
			  $temp_ident = explode('/',$this->session->userdata('moci02_search'));
			  $this->session->set_userdata('moci02_search',"display_search/".$offset);
			  if($temp_ident[0]=="display"){
				 $this->session->set_userdata('moci02_search',"display/ta002/desc/".$offset);
			  }
		  }
		  ...
		  if($data['result']=="no_data"){
			  redirect('moc/moci02/'.$this->session->userdata('moci02_search'));
			  exit;
		  }
      ...
      public function see()   //看資料
        {      
	      ...
	      $data['menu_v'] = 'main_menuno_v';
	      $data['content_v'] = 'moc/moci02_see_v';
	      $data['foot_v'] ='main_foot_v';
	      $this->load->vars($data);
	      $this->load->view('main_head_v');
        }
      ...
      public function del()   //刪除單筆 暫存
        {      
	      ...
	      $data['message'] = '刪除資料成功!';
	      $this->load->model('moc/moci02_model','',TRUE);
	      $this->moci02_model->deletef($seg1,$seg2);
	      $this->display();
        }
      ...
      public function delete()   //刪除選取
        {    
	      $data['message'] = '刪除資料成功!';
	      $this->load->model('moc/moci02_model','',TRUE);
	      $data['message'] = $this->moci02_model->delmutif();
	      $this->display();
        }
	 public function printb()   //印單據選取
        {    
	      ...
	      $this->load->model('moc/moci02_model','',TRUE);
	      $data['message'] = '列印單據成功!';
          $result = $this->moci02_model->printfd1($this->uri->segment(4),$this->uri->segment(5));
	      $data['results'] = $result['rows'];
	      $data['num_results'] = $result['num_rows'];
	      $this->load->library('pagination');
	      $data['numrow']=$result['num_rows'];// 總筆數 
	...
	/*==以下AJAX處理區域==*/
	  ...
	  public function check_title_no(){
		...
		$this->load->model('moc/moci02_model','',TRUE);
		$data = $this->moci02_model->check_title_no($moci01,$ta003);
		echo $data;
	  }
      ...
      public function del_detail_ajax()   
        {
		  ...
		  $data['message'] = '刪除資料成功!';
		  $this->load->model('moc/moci02_model','',TRUE);
		  echo $this->moci02_model->deletedetailf($seg1,$seg2,$seg3);
        }
	 //查看bom筆數
...
/* End of file moci02.php */
/* Location: ./application/controllers/moci02.php */
?>