...
class csti04 extends CI_Controller {           //擴展類必须註明由母類擴展而來 (自訂類使用 ci 框架)
	...
	public function display_search($offset = 0,$func = "")  
	  {
		...
		$limit = 15;    //每頁筆數
		$this->load->model('cst/csti04_model');// 加載TABLE model 模型
		$result= $this->csti04_model->construct_sql($limit, $offset ,$func); //至model 取 mysql 資料 預設 15,0,tf001,desc
		$data['results'] = $result['data'];
		$data['num_results'] = $result['num'];
		$data['numrow'] = $result['num'];// 總筆數 
		$data['page'] = $result['num']/$limit; // 總頁數
		$data['sql'] = $_SESSION['csti04']['search']['sql'];
		...
		$config['base_url'] = site_url("cst/csti04/display_search");   //設定分頁url路徑
		/* 網址去除".html" 字串進行切割 陣列,  */
		...
		$data['content_v'] = 'cst/csti04_brow_v';		
	  ...
	  public function construct_sql($offset = 0,$func = ""){
		$limit = 15;
		$this->load->model('cst/csti04_model');// 加載TABLE model 模型
		$this->csti04_model->construct_sql($limit, $offset ,$func);
	}
	...
	public function filter1($sort_by = 'me001', $sort_order = 'desc', $offset = 0)   
        {
	     ...
	     $this->load->model('cst/csti04_model','',TRUE);   //true 系統會使用反單引號（`, 在英文鍵盤左上方）來保護你的欄位或資料表名稱
	     $result=$this->csti04_model->filterf1($limit, $offset , $sort_by  , $sort_order);
	     $data['message'] = '篩選資料成功!';	
	     ...
	     $config['base_url'] = site_url("cst/csti04/filter1/$sort_by/$sort_order/$seq6/$seq7");  //設定分頁url路徑
	     ...
	     $data['limit'] = $limit ;    //每頁筆數
	     $data['systitle'] ='製令成本建立作業';
	     $data['menu_v'] = 'main_menu_v';
	     $data['content_v'] = 'cst/csti04_brow_v';
	     $data['foot_v'] ='main_foot_v';
	     $this->load->vars($data);
	     $this->load->view('main_headbrow_v');
	   //$this->load->view('cst/csti04_v', $data);
        }
    ...
    public function findform()   
        {
	      ...
	      $data['username'] = $this->session->userdata('manager');
	      $data['message'] = '';
	      $data['systitle'] ='製令成本-進階查詢';
	      $data['menu_v'] = 'main_menuno_v';
	      $data['content_v'] = 'cst/csti04_find_v';
	      $data['foot_v'] ='main_foot_v';
	      $this->load->vars($data);
	...
	public function findsql($sort_by = 'me001', $sort_order = 'desc', $offset = 0)  
	    {		
		  ...
		  if(@$_POST['find005']){
			$_SESSION['csti04_sql_term'] = $_POST['find005'];
		  }
		  if(@$_POST['find007']){
			$_SESSION['csti04_sql_sort'] = $_POST['find007'];
		  }
	      $limit = 15;    //每頁筆數
	      $data['message'] = '資料流覽成功!';
	      ...
	      $data['sort_order'] = $sort_order;
	      $this->load->model('cst/csti04_model');// 加載TABLE model 模型		
	      $result= $this->csti04_model->findf($limit, $offset , $sort_by  , $sort_order ); //至model 取 mysql 資料 預設 15,0,me001,desc
	      $data['results'] = $result['rows'];
	      $data['num_results'] = $result['num_rows'];
	      $this->load->library('pagination');
	      $data['numrow']=$result['num_rows'];// 總筆數 
	    ...
	    //$config['NUM_LINKS'] = 2;   //分頁前後出現2個數字
	      ...
	      $config['base_url'] = site_url("cst/csti04/findsql/$sort_by/$sort_order");   //設定分頁url路徑
	      $config['total_rows'] = $result['num_rows']; // 總筆數
	      $config['per_page'] = $limit;                //每頁筆數
	    ...
	    //$this->load->library('table');//加載table函數
		 ...
		 public function clear_sql_term(){  //清除條件
		  ...
		  if(@$_SESSION["csti04_sql_term"]) {unset($_SESSION["csti04_sql_term"]);}
		  if(@$_SESSION["csti04_sql_sort"]) {unset($_SESSION["csti04_sql_sort"]);}
		  $this->display();
	  }
    ...
    public function addform()   
        {
	     ...
	     $data['message'] = '';
	     $data['username'] = $this->session->userdata('manager');
	     $data['systitle'] ='製令成本-新增資料';
	     $data['menu_v'] = 'main_menuno_v';
	     $data['content_v'] = 'cst/csti04_add_v';
	     $data['foot_v'] ='main_foot_v';
	     $this->load->vars($data);
	     $this->load->view('main_head_v');
        }
    ...
    public function addsave()   
        {
	     ...
	     $data['username'] = $this->session->userdata('manager');
         $this->load->model('cst/csti04_model','',TRUE);
	     $data['message'] = '新增成功!';
		 
		// $this->csti04_model->uploadf();   //檔案上傳
	     $action = $this->csti04_model->insertf();
	     if ($action === 'exist')
	      {
	       $data['message'] = '資料重複!';		    
	      }
    ...
    public function copyform()   //複製資料輸入
       {
	    $data['username'] = $this->session->userdata('manager');
	    $data['message'] = '';
	    $data['systitle'] ='製令成本-複製資料';
	    $data['menu_v'] = 'main_menuno_v';
	    $data['content_v'] = 'cst/csti04_copy_v';
	    $data['foot_v'] ='main_foot_v';
	    $this->load->vars($data);
	    $this->load->view('main_head_v');
       }
    ...
    public function copysave()   
       {
         ...
         $this->load->model('cst/csti04_model','',TRUE);
	     $data['message'] = '複製成功!';
         $action = $this->csti04_model->copyf();
    ...
    public function exceldetail()  
        {
	     $data['message'] = '';
	     $data['username'] = $this->session->userdata('manager');
	     $data['systitle'] ='製令成本-轉excel檔';
	     $data['menu_v'] = 'main_menuno_v';
	     $data['content_v'] = 'cst/csti04_excel_v';
	     $data['foot_v'] ='main_foot_v';
	     $this->load->vars($data);
	     $this->load->view('main_head_v');
        }
    ...
    public function write()   
        {
         ...
         $result1 = $this->csti04_model->excelnewf();	
    ...
    public function printdetail()   
        {
	     $data['username'] = $this->session->userdata('manager');
	     $data['message'] = '';
	     $data['systitle'] ='製令成本-印明細表';
	     $data['menu_v'] = 'main_menuno_v';
	     $data['content_v'] = 'cst/csti04_print_v';
	     $data['foot_v'] ='main_foot_v';
	     $this->load->vars($data);
	     $this->load->view('main_head_v');
        }
    ...
    public function printa()   
        {
         ...
         $this->load->model('cst/csti04_model','',TRUE);
	     $data['message'] = '列印明細成功!';
         $result = $this->csti04_model->printfd();
	     $data['results'] = $result['rows'];
	     $data['num_results'] = $result['num_rows'];
	     $this->load->library('pagination');
	     $data['numrow']=$result['num_rows'];// 總筆數 
	     $data['username'] = $this->session->userdata('manager');
	     $data['systitle'] ='製令成本-印明細表';
	     $data['content_v'] = 'cst/csti04_printa_v';
	     $this->load->vars($data);
    ...
    public function updsave()   
        {
		  $seq1 = $this->uri->segment(4);
			...
			if(isset($_SESSION['csti04']['search'])){
				$current_index = $_SESSION['csti04']['search']['view'][$seq1];
				if($current_index!=0){
					$data['prev'] = $_SESSION['csti04']['search']['index'][$current_index-1];
				}
				if(isset($_SESSION['csti04']['search']['index'][$current_index+1])){
					$data['next'] = $_SESSION['csti04']['search']['index'][$current_index+1];
				}
				...
				if($this->uri->segment(3)=="display" || $this->uri->segment(3)=="display_leave"){
					$this->session->set_userdata('csti04_search',$this->uri->segment(3)."/".$this->uri->segment(4)."/".$this->uri->segment(5)."/".$offset);
				}
			}	
         ...
         $this->load->model('cst/csti04_model','',TRUE);
	     $this->load->vars($data);
	     $this->csti04_model->updatef();
		 $data['seq1'] = $this->uri->segment(4); 
	      $data['message'] = '儲存完畢!';
	      $this->load->model('cst/csti04_model');
	      $data['result'] = $this->csti04_model->selone($this->uri->segment(4));
		 $data['username'] = $this->session->userdata('manager');
	      $data['systitle'] ='製令成本-修改';
	      $data['menu_v'] = 'main_menuno_v';
	      $data['content_v'] = 'cst/csti04_upd_v';
	      $data['foot_v'] ='main_foot_v';
	      $this->load->vars($data);
	      $this->load->view('main_head_v'); 
		// redirect('cst/csti04/'.$this->session->userdata('csti04_search'));
	    // redirect('cst/csti04/'.$this->session->userdata('search1'));
        }
    ...
    public function updform()   
        {
		 ...
		 //以下暫存view處理，上一筆下一筆用
			...
			if(isset($_SESSION['csti04']['search'])){
				...
				$offset = floor($current_index/15)*15;
				$temp_ident = explode('/',$this->session->userdata('csti04_search'));
				$this->session->set_userdata('csti04_search',"display_search/".$offset);
				if($temp_ident[0]=="display"){
					$this->session->set_userdata('csti04_search',"display/me001/asc/".$offset);
				}
				if($temp_ident[0]=="display_leave"){
					$this->session->set_userdata('csti04_search',"display_leave/me001/asc/".$offset);
				}
			}
        ...
        {      
		 ...
		 //以下暫存view處理，上一筆下一筆用
			...
			//	echo "<pre>";var_dump($_SESSION['csti04']['search']['view']);exit;
				...
				$this->session->set_userdata('csti04_search',"display_search/".$offset);
				if($temp_ident[0]=="display"){
					$this->session->set_userdata('csti04_search',"display/me001/asc/".$offset);  //品號
				}
				if($temp_ident[0]=="display_leave"){
					$this->session->set_userdata('csti04_search',"display_leave/me001/asc/".$offset);
				}
	     ...
	     $data['result'] = $this->csti04_model->selone($this->uri->segment(4));
	     $data['username'] = $this->session->userdata('manager');
	     $data['systitle'] ='製令成本-查看資料';
	     $data['menu_v'] = 'main_menuno_v';
	     $data['content_v'] = 'cst/csti04_see_v';
	     $data['foot_v'] ='main_foot_v';
	     $this->load->vars($data);
	     $this->load->view('main_head_v');
        }
    ...
    public function del()   
        {      
	     ...
	     $data['message'] = '刪除資料成功!';
	     $this->load->model('cst/csti04_model','',TRUE);
	     $this->csti04_model->deletef($seg1,$seg2);
	     $this->display();
        }
	...
	public function display_child($offset = 0,$func = "")  
	  {
		...
		$data['message'] = '資料流覽成功!';
		$this->load->model('cst/csti04_model');// 加載TABLE model 模型
		$result= $this->csti04_model->construct_sql($limit, $offset ,$func); //至model 取 mysql 資料 預設 15,0,ma001,desc
		$data['results'] = $result['data'];
		$data['num_results'] = $result['num'];
		$this->load->library('pagination');
		$data['numrow']=$result['num'];// 總筆數 
		...
		$config['base_url'] = site_url("cst/csti04/display_child");   //設定分頁url路徑
		/* 網址去除".html" */
		$temp_url = explode(".html",$config['base_url']);
		$config['base_url'] = "";
		foreach($temp_url as $key => $val){$config['base_url'].=$val;}
		...
		$data['systitle'] ='製令成本建立作業';		  
		$data['menu_v'] = 'main_menu_v';
		$data['content_v'] = 'funnew/csti04d_child_v';		
		$data['foot_v'] ='main_foot_v';
		$this->load->vars($data);
	  ...
	  public function display_child2($offset = 0,$func = "")  
	  {
		...
		$config['cur_tag_close'] = '</a>'; // 當前頁结束樣式
		$config['cur_page'] = $this->uri->segment(4,0);   //當前頁 結合分頁url路徑 +1  ,分頁初始化 display 3 + 2 + 1 = 6
		$config['base_url'] = site_url("cst/csti04/display_child2");   //設定分頁url路徑
		/* 網址去除".html" */
		$temp_url = explode(".html",$config['base_url']);
		$config['base_url'] = "";
		foreach($temp_url as $key => $val){$config['base_url'].=$val;}
	...
	//品號快速查詢
	public function lookupd_csti04(){
     ...
     /*   $query = $this->csti04_model->lookupd(
			array('a.me001','a.me002','a.me003','a.me004'),
			array('and'=>"me001"),
			array('me001'=>$keyword),
			15
		); */
      $query = $this->csti04_model->lookupd(urldecode(urldecode($this->uri->segment(4)))); //Search DB 
        if( ! empty($query) )  
          {  
	...
	//品號快速查詢
	public function lookupd2_csti04(){
	    $keyword = urldecode(urldecode($this->uri->segment(4)));
        $data['response'] = 'false'; //Set default response 
     ...
     /*   $query = $this->csti04_model->lookupd(
      ...
      $query = $this->csti04_model->lookupd2(urldecode(urldecode($this->uri->segment(4)))); //Search DB 
        if( ! empty($query) )  
          {  
            $data['response'] = 'true'; //Set response  
            $data['message'] = array(); //Create array  
	...
	public function checkkey()   
        {
	     $this->load->model('cst/csti04_model');
	     $data['result'] = $this->csti04_model->ajaxkey($this->uri->segment(4));
         $Result = $data['result'];		  
	     $this->load->vars($data);
	     echo  $Result;
        }
}
/* End of file csti04.php */
/* Location: ./application/controllers/csti04.php */
?>