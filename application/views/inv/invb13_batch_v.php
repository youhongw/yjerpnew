...
<div id="content"> <!-- div-3 -->
 <div class="box"> <!-- div-4 -->
    <div class="heading">
      <h1><img src="<?php echo base_url()?>assets/image/order.png" alt="" /> 每月全部庫存[換算單位]計算作業 - 計算　　　</h1>
	...
	<form  class="cmxform" id="commentForm"  name="form" action="<?php echo base_url()?>index.php/inv/invb13/batcha"  method="post"  enctype="multipart/form-data" > 
...
//	 if (!$this->input->post('ta003c')) {$ta003c=date("Y/m/d");}	
  //   if (!$this->input->post('tb008c')) {$tb008c='1';}	 
	 ...
	 if (!$this->input->post('ta034c')) {$ta034c=date("Y/m/d");}
...
//	 if (!$this->input->post('ta034o')) {$ta034o=date("Y/m/d");}
	...
	<table class="form14">   <!-- 表格 -->
	  <tr>
	    <td class="normal14y"  width="12%">計算庫存年月：</td>
        <td class="normal14a" width="88%"><input tabIndex="3" id="ta034c"  onfocus="this.select()" class="date-picker" onChange="dateformat_ym(this)"      onKeyPress="keyFunction()"    type="text" name="ta034c"  value="<?php echo $ta034c; ?>"  size="16" style="background-color:#E7EFEF" />
	    <img  onclick="scwShow(ta034c,event);"   src="<?php echo base_url()?>assets/image/png/calendar.png" alt="" align="top"/> 
           </td>
	  </tr>
	<!--  
	 <tr>
	    <td class="start14a"  width="12%">計算庫別：</td>
