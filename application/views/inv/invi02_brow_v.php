<div class="box2">  <!-- div-1 -->
    <div class="heading">
	  ...
	  <?PHP if (substr($this->session->userdata('sysmg006'),1,1)=='Y') { ?>
	    <a onclick="location = '<?php echo base_url()?>index.php/inv/invi02/display'"  style="float:left" accesskey="a" class="button"><span>重整 a </span><img height="12" width="12" src="<?php echo base_url()?>assets/image/delete2.png" /></a>	
      <?PHP } ?>
	  <?PHP if (substr($this->session->userdata('sysmg006'),0,1)=='Y') { ?>
	    <a onclick="location = '<?php echo base_url()?>index.php/inv/invi02/addform'" style="float:left" accesskey="i" class="button"><span>新增 i </span><img src="<?php echo base_url()?>assets/image/png/add.png" /></a>
	  <?PHP } ?>
      ...
      <?PHP } ?>
	  <?PHP if (substr($this->session->userdata('sysmg006'),1,1)=='Y') { ?>
	    <a onclick="location = '<?php echo base_url()?>index.php/inv/invi02/findform'" style="float:left" accesskey="k" class="button"><span>查詢 k </span><img src="<?php echo base_url()?>assets/image/png/find.png" /></a>	
      <?PHP } ?>
	...
	$title_array = array(
		...
		'mb001' => array('sort_name'=>"mb001",'name'=>"品號",'width'=>"7%",'align'=>"left"),
		'mb025' => array('sort_name'=>"mb025",'name'=>"屬性",'width'=>"5%",'align'=>"left"),
		'mb002' => array('sort_name'=>"mb002",'name'=>"品名",'width'=>"16%",'align'=>"left"),
		'mb003' => array('sort_name'=>"mb003",'name'=>"規格",'width'=>"8%",'align'=>"left"),
		'mb004' => array('sort_name'=>"mb004",'name'=>"單位",'width'=>"8%",'align'=>"left"),
		'mb017' => array('sort_name'=>"mb017",'name'=>"主要庫別",'width'=>"8%",'align'=>"left"),
		'mb200' => array('sort_name'=>"mb200",'name'=>"產品圖",'width'=>"7%",'align'=>"left"),
...
?>
  <div class="content"> <!-- div-2 -->
        ...
        <thead>
          <tr>                          <!-- 表格表頭 -->
			...
			foreach($title_array as $key => $val){
				...
				echo anchor("inv/invi02/display_search/".$this->uri->segment(4,0)."/order?val=".$val['sort_name']." asc",$str);
				
				$str = "<img src='".base_url()."assets/image/desc.png' />";
				echo anchor("inv/invi02/display_search/".$this->uri->segment(4,0)."/order?val=".$val['sort_name']." desc",$str);
	...
	$filter_array = array(
		...
		'mb001' => array('filter_name'=>"mb001",'name'=>"品號",'size'=>"10",'align'=>"left"),
		'mb025' => array('filter_name'=>"mb025",'name'=>"屬性",'size'=>"10",'align'=>"left"),
		'mb002' => array('filter_name'=>"mb002",'name'=>"品名",'size'=>"10",'align'=>"left"),
		'mb003' => array('filter_name'=>"mb003",'name'=>"規格",'size'=>"10",'align'=>"left"),
		'mb004' => array('filter_name'=>"mb004",'name'=>"單位",'size'=>"10",'align'=>"left"),
		'mb017' => array('filter_name'=>"mb017",'name'=>"主要庫別",'size'=>"10",'align'=>"left"),
		'mb200' => array('filter_name'=>"mb200",'name'=>"產品圖",'size'=>"10",'align'=>"left",'use'=>"disable"),
	);
?>  
      <!--  <tbody>     <!-- 表格內容輸入篩選查詢 第一,二欄 刪除選項及序號空白 -->
        ...
        <tr>
          ...
          <td class="left"><?php echo  $row->mb001;?></td>
             <?php $mb025='';   ?>		  
             <?php if ($row->mb025=='P') {$mb025='P:採購件'; }  ?>
             <?php if ($row->mb025=='M') {$mb025='M:自製件'; }  ?>	
             <?php if ($row->mb025=='S') {$mb025='S:託外加工件'; }  ?>
             <?php if ($row->mb025=='Y') {$mb025='Y:虛設件'; }  ?>			 
		  <td class="left"><?php echo  $mb025;?></td>			  
		  ...
		  <td class="center"><img src="<?php echo base_url();?>assets/image/jpg/<?php echo $row->mb200;?>"   id="ad" width="90" height="90" border="0" style="padding:5px"/>
		...
		<?php endforeach;?>
		      ...
		      <!-- 修改時 留在原來那一筆資料使用 -->
				<?php $this->session->set_userdata('invi02_search',$this->uri->segment(3)."/".$this->uri->segment(4,0));
				if($this->uri->segment(3)=="display" || $this->uri->segment(3)=="display_leave"){
					$this->session->set_userdata('invi02_search',$this->uri->segment(3)."/".$this->uri->segment(4)."/".$this->uri->segment(5)."/".$this->uri->segment(6,0));
				} ?>
...
function filter() {
	...
	url = '<?php echo base_url() ?>index.php/inv/invi02/display_search/0/and_where?key=' + encodeURIComponent(key) + 
...
function filtera() {
	...
	url = '<?php echo base_url() ?>index.php/inv/invi02/display_search/0/or_where?key=' + encodeURIComponent(key) + 
	'&val=' + encodeURIComponent(val);
	location = url;
}
</script>