...
?>
        ...
        <tr class="row_<?php echo $chkval; ?>">
		  ...
		  <td class="center"><a href='javascript:send_back("<?php echo $row->mb001;?>","<?php echo $row->mb002;?>","<?php echo addslashes($row->mb003);?>");'>[ 選擇 </a><img src="<?php echo base_url()?>assets/image/png/eye.png" />]</td>
		</tr>
		<?php $chkval += 1; ?>
		<?php endforeach;?>
        </tbody>
        </table>
		<div class="mult_add"><input id="btn_mult_add" type="button" style="float:right;" value="多筆加入" onclick="send_mult_back();" /></div>
...
function send_back(mb001, mb002, mb003){
	window.parent.$.unblockUI();
	if(window.parent.addte004disp){	//以此判斷各呼叫此網頁的方法是否存在藉以給予相對應的需求
		window.parent.addte004disp(mb001,mb002,mb003);
		$.ajax({
			method: "POST",
...
function send_mult_back(){
	...
	$('.row_chkbx:checked').each(function(){
		...
		if(window.parent.mult_addte004disp){	//以此判斷各呼叫此網頁的方法是否存在藉以給予相對應的需求
			window.parent.mult_addte004disp(mb001,mb002,mb003);
		}
	});
}
