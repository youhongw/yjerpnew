	...
	<table class="list">     <!-- 列明細 2 判斷 31, 3 判斷 32 4 判斷 32, 31 1050920 tb004 1借-1貸-->
	                         <!-- tb007 傳票金額 科目設定ma007 1借,-1貸  傳票 $vtb072 貸方金額-->
	   <?php $vmba017=0;$vvtb071=0;$vvtb072=0;$kvtb004=0; $vvtb004=0; ?>
	   <?php foreach($results as $row ) : ?>
	      <tr>
		  <?php  $mvtb004=$row->tb004 ?>
		<!-- 1041119 -->
		<?php  if ($row->tb004==1) {$vtb071=$row->tb0071;$vtb072=0;$vvtb004=1;} else {$vtb072=$row->tb0072;$vtb071=0;$vvtb004=-1;} ?>
		<!-- 1041120-1 -->
		<?php // if ($row->tb004==-1) {$vtb072=$row->tb0072;$vtb071=0;$vvtb004=-1;} else {$vtb071=$row->tb0071;$vtb072=0;$vvtb004=1;} ?>
		<!-- 1041119  1114第一筆傳票-->
		<?php  if ($vvtb004==$mvtb004 and $row->tb004==1 and $row->ma007==1) {$vmba017=$vmba017+($row->mba017*$row->tb004);}  ?>
		<?php  if ($vvtb004!=$mvtb004 and $row->tb004==1 and $row->ma007==1) {$vmba017=$vmba017-($row->mba017*$row->tb004*-1);} ?>
		
		<?php  if ($vvtb004==$mvtb004 and $row->tb004==1 and $row->ma007==-1) {$vmba017=$vmba017-($row->mba017*$row->tb004);}  ?>
		<?php  if ($vvtb004!=$mvtb004 and $row->tb004==1 and $row->ma007==-1) {$vmba017=$vmba017+($row->mba017*$row->tb004*-1);} ?>
		<!-- 1041120-1 1050920 1114第一筆傳票  判斷期初-->
		<?php  if ($vvtb004==$mvtb004 and $row->tb004==-1 and $row->ma007==1) {$vmba017=$vmba017-($row->mba017);}  ?>
		      
		<?php  if ($vvtb004!=$mvtb004 and $row->tb004==-1 and $row->ma007==1)  {$vmba017=$vmba017+($row->mba017);} ?>
		        <?php // var_dump($mvtb004);var_dump($vmba017);exit;   ?>
		<?php  if ($vvtb004==$mvtb004 and $row->tb004==-1 and $row->ma007==-1) {$vmba017=$vmba017+($row->mba017);}  ?>
		<?php  if ($vvtb004!=$mvtb004 and $row->tb004==-1 and $row->ma007==-1)  {$vmba017=$vmba017-($row->mba017);} ?>
		<!-- 判斷第一筆  明細 100920-ok 加第二行-->
		<?php  if ( $kvtb004==0 )  {$vmba017=($row->mba017)*1;}  ?>
		<?php  if ( $kvtb004==0 and ( $row->tb004==-1 and $row->ma007==1))  {$vmba017=($row->mba017)*-1;}  ?>
		<!-- 1041119 tb004-->
		<?php  if ($vmba017>=0 and $row->ma007==1) {$vtb004='借餘';$mvtb004=1;} ?> 
		<!-- 1041120-2 -->
        <?php   if ($vmba017>=0 and $row->ma007==-1) {$vtb004='貸餘';$mvtb004=1;} ?> 
		
		<?php  if ($vmba017<0 and $row->ma007==1) {$vtb004='貸餘';$mvtb004=-1;} ?>
		         <?php // var_dump($vtb004);var_dump($vmba017);exit;   ?>
		<!-- 1041120-2 tb004-->
		<?php  if ($vmba017<0 and $row->ma007==-1) {$vtb004='借餘';$mvtb004=-1;} ?>
		...
		<td width="8%" align="right"><?php echo $pvmba017;?></td> 
