...
<div id="content"> <!-- div-3 -->
 <div class="box"> <!-- div-4 --><span>　　　　　　</span>
    <div class="heading">
      <h1><img src="<?php echo base_url()?>assets/image/order.png" alt="" /> 試 算 表 - 列印明細表　　　</h1>
	  ...
	  <button style= "cursor:pointer" form="commentForm" onfocus="$('#vdate').focus();" type='submit' tabIndex="98" accesskey="p" name='submit' class="button"   target="_new" value='&nbsp;列印F8&nbsp;'><span>列 印Alt+p</span><img src="<?php echo base_url()?>assets/image/png/print.png" /></button>&nbsp;&nbsp;
		  ...
		  <button style= "cursor:pointer" form="commentForm" onfocus="$('#vdate').focus();" type='submit' accesskey="l" class="button" name='action' value='excel'>轉EXCEL檔Alt+l </span><img src="<?php echo base_url()?>assets/image/png/excel.png" /></button>&nbsp;&nbsp;
		  <?PHP } ?>
	...
	<form  class="cmxform" id="commentForm"  name="form" action="<?php echo base_url()?>index.php/act/actr20/printa"  method="post"  enctype="multipart/form-data" > 
	...
	<table class="form14">   <!-- 表格 class="date-picker"-->
      <tr>
	    <td class="normal14y" width="12%">選擇試算表年月：</td>
	    <td class="normal14" width="88%"><input tabIndex="3" id="vdate" onclick="fPopCalendar(event,vdate,vdate);" class="date-picker" onChange="dateformat_ym(this)"  onKeyPress="keyFunction()"    type="text" name="vdate"  value="<?php echo $vdate; ?>"  size="16" style="background-color:#E7EFEF" /><span > <?php   echo ''; ?> </span>
        <!-- <img  onclick="fPopCalendar(event,vdate,vdate);"   src="<?php echo base_url()?>assets/image/png/calendar.png" alt="" align="top"/> -->
           </td>
	  </tr>
	   <tr>
	    <td class="normal14z" >選擇列印種類：</td>
	    <td class="normal14" ><select id="mf001c" onKeyPress="keyFunction()" name="mf001c"  tabIndex="12">
            <option <?php if($mf001c == '1') echo 'selected="selected"';?> value='1'>1.統制帳戶</option>                                                                        
		    <option <?php if($mf001c == '2') echo 'selected="selected"';?> value='2'>2.明細帳戶</option>
		  </select></td>  
	  </tr>	
	   <tr>
