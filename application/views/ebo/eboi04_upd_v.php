...
<div id="content"> <!-- div-3 --> 
 <div class="box"> <!-- div-4 --><span>　　　　　　</span>
    <div class="heading">
      <h1><img src="<?php echo base_url()?>assets/image/order.png" alt="" /> E-BOM變更單建立作業 - 修改　　　</h1>
    <div style="float:left;padding-top: 5px; ">
	<button style= "cursor:pointer" form="commentForm" onfocus="$('#ti001').focus();" type='submit' accesskey="s" name='submit' class="button" onKeyPress="keyFunction()" onclick="addItem();" value='&nbsp;儲存F8&nbsp;'><span>儲 存Alt+s</span><img src="<?php echo base_url()?>assets/image/png/save.png" /></button>&nbsp;&nbsp;
	...
	<form  class="cmxform" id="commentForm"  name="form" action="<?php echo base_url()?>index.php/ebo/eboi04/updsave" method="post" enctype="multipart/form-data" >
	...
	<!--	 <td class="left"><input type="text" tabIndex="19"  onfocus="scwShow(this,event);" onclick="scwShow(this,event);"  id="tj011[<?php echo $i ?>]" onKeyPress="keyFunction()" name="order_product[<?php echo $i ?>][tj011]" value="<?php echo  $tj011[$i]; ?>" size="10"  class="date" style="background-color:#E7EFEF" /></td>
      ...
      <!--   <td class="right"><input type="text"  tabIndex="22" id="tj007" onKeyPress="keyFunction()" name="order_product[<?php echo $i ?>][tj007]" value="<?php echo $tj007[$i]; ?>" size="10" style="text-align:right;"  /></td>	
		...
		<script>
		function del_detail(del_tj001,del_tj002,del_tj003){
			if(confirm('是否刪除此筆資料，主件品號:'+del_tj001+'品號:'+del_tj002+'序號:'+del_tj003))
			{
				$('#del_tj001').val(del_tj001);
				$('#del_tj002').val(del_tj002);
				$('#del_tj003').val(del_tj003);
				$('#del_form').submit();
			}
		}
		</script>
 ...
 <?php } ?>		
	...
	<?php include_once("./application/views/fun/eboi04_funjsupdjs_v.php"); ?> 
...
'◎操作說明:[ 品號欄位可輸入部份品號或品名下拉視窗選擇,按Enter鍵或Tab鍵跳下一個欄位,按Shift+Tab鍵跳上一個欄位,Tab鍵停留在有選項欄位按上下鍵可選擇資料. ] ' ?> </div>  <?php } ?>
		...
		<form action="<?php echo base_url()?>index.php/ebo/eboi04/delete_detail" method="post" enctype="multipart/form-data" id="del_form" style="display:none;">
			<input id="del_tj001" name="del_tj001" />
			<input id="del_tj002" name="del_tj002" />
			<input id="del_tj003" name="del_tj003" />
		</form>
 <?php include_once("./application/views/fun/eboi04_funjsupd_v.php"); ?>