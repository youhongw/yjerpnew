 <div class="box2">  <!-- div-1 -->
	...
	<!--  <a style="float:left;cursor:default;" class="button" ><span style="cursor: auto;">進階條件:<?php if(@$_SESSION['admi05_sql_term'] || @$_SESSION['admi05_sql_sort']){if(@$_SESSION['admi05_sql_term']) echo " ".$_SESSION['admi05_sql_term']; if(@$_SESSION['admi05_sql_sort']) echo " ".$_SESSION['admi05_sql_sort'];} else echo"無";?></span></a> -->	
      <?PHP // } ?>
	  <?PHP if ((substr($this->session->userdata('sysmg006'),1,1)=='Y') || ($this->session->userdata('syssuper')=='Y')) { ?>
	  <a onclick="location = '<?php echo base_url()?>index.php/adm/admi05/clear_sql_term'"  style="float:left" accesskey="a" class="button"><span>重整 a </span><img height="12" width="12" src="<?php echo base_url()?>assets/image/delete2.png" /></a>	
      <?PHP } ?>
	  <?PHP if ((substr($this->session->userdata('sysmg006'),0,1)=='Y') || ($this->session->userdata('syssuper')=='Y')) { ?>
	  <a onclick="location = '<?php echo base_url()?>index.php/adm/admi05/addform'"  style="float:left" accesskey="i" class="button"><span>新增 i </span><img src="<?php echo base_url()?>assets/image/png/add.png" /></a>
	  <?PHP } ?>
	  <?PHP if ((substr($this->session->userdata('sysmg006'),9,1)=='Y') || ($this->session->userdata('syssuper')=='Y')) { ?>
      ...
      <?PHP } ?>
	  <?PHP if ((substr($this->session->userdata('sysmg006'),1,1)=='Y') || ($this->session->userdata('syssuper')=='Y')) { ?>
	  <a onclick="location = '<?php echo base_url()?>index.php/adm/admi05/findform'"  style="float:left" accesskey="k" class="button"><span>查詢 k </span><img src="<?php echo base_url()?>assets/image/png/find.png" /></a>	
      <?PHP } ?>
	  <?PHP if ((substr($this->session->userdata('sysmg006'),3,1)=='Y') || ($this->session->userdata('syssuper')=='Y')) { ?>
	  ...
	  <?PHP } ?>
	  <?PHP if ((substr($this->session->userdata('sysmg006'),6,1)=='Y') || ($this->session->userdata('syssuper')=='Y')) { ?>
	  ...
	  <?PHP } ?>
	  <?PHP if ((substr($this->session->userdata('sysmg006'),10,1)=='Y') || ($this->session->userdata('syssuper')=='Y')) { ?>
	  <a onclick="location = '<?php echo base_url()?>index.php/adm/admi05/exceldetail'"  style="float:left"   accesskey="l" class="button"><span>EXCEL l </span><img src="<?php echo base_url()?>assets/image/png/excel.png" /></a>  
	  <?PHP } ?>
    ...
    <form action="<?php echo base_url()?>index.php/adm/admi05/delete" method="post" enctype="multipart/form-data" id="form">
        <table class="list">      <!-- 表格開始 -->
	      ...
	      <td width="5%" class="center"> 
		   ...
		   <td width="5%" class="center"> 
		    <?php echo anchor("adm/admi05/display/b.mb002/" . (($sort_order == 'asc' && $sort_by == 'b.mb002') ? 'desc' : 'asc') ,'程式名稱'); ?>
		    <?php if ($sort_order == 'asc'  ) { ?>  <img src="<?php echo base_url()?>assets/image/desc.png" /> <?php } else { ?>
		    <img src="<?php echo base_url()?>assets/image/asc.png" />  <?php }  ?>
	      </td>
	      <td width="5%" class="center"> 
	     ...
	     <?php $filter_mg001='';$filter_mg002='';$filter_mb002='';$filter_mg004='';$filter_mg006='';$filter_mg007='';$filter_mg008='';$filter_create=''; ?>
	    <tr class="filter">
	      <td class="left"></td>
		  ...
		  <input type="text"  name="filter_mg001" value="" size="12" />
	      </td>
	      ...
	      <td class="left">
		  <div class="button-search"></div>
		  <input type="text" name="filter_mg004" value="" size="12" />
	      </td>
		  
		  <td class="left">
		  <div class="button-search"></div>
			<input type="text" name="filter_mg006" value="" size="12" />
	      </td>
		  
		  <td class="left">
		  <div class="button-search"></div>
			<input type="text" name="filter_mg007" value="" size="12" />
	      </td>
		  
		  <td class="left">
		  <div class="button-search"></div>
			<input type="text" name="filter_mg008" value="" size="12" />
	      </td>	
        ...
        <tr>
		  ...
		  <td class="center"><a href="<?php echo site_url('adm/admi05/see/'.$row->mg001.'/'.$row->mg002)?>">[ 查看 </a><img src="<?php echo base_url()?>assets/image/png/eye.png" />]</td>
...
function filter() {
	...
	if (filter_mb002) {
		url = '<?php echo base_url() ?>index.php/adm/admi05/filter1/b.mb002/desc/' + encodeURIComponent(filter_mb002);
	} 
	...
	if (filter_mg006) {
		url = '<?php echo base_url() ?>index.php/adm/admi05/filter1/mg006/desc/' + encodeURIComponent(filter_mg006);
	}
    ...
    if (!filter_mg001  && !filter_mg002 && !filter_mb002   && !filter_mg004 && !filter_mg006 && !filter_mg007 && !filter_mg008 ) {         
...
function filtera() {
	...
	var filter_mb002 = $('input[name=\'filter_mb002\']').val();
	if (filter_mb002) {
		url = '<?php echo base_url() ?>index.php/adm/admi05/filter1/b.mb002/asc/' + encodeURIComponent(filter_mb002);
	} 
	var filter_mg004 = $('input[name=\'filter_mg004\']').val();
	if (filter_mg004) {
