...
<div id="content"> <!-- div-3 -->
 <div class="box"> <!-- div-4 -->
    <div class="heading">
      <h1><img src="<?php echo base_url()?>assets/image/order.png" alt="" /> 使用者權限模組建立作業 - 查看</h1>
	  <form  class="cmxform" id="commentForm"  name="form" action="<?php echo base_url()?>index.php/adm/admi06/display"  method="post" enctype="multipart/form-data" >
    </div>
	...
	<table class="form14"  >     <!-- 頭部表格 -->
	  <tr>
        ...
        <td class="normal14a" width="39%"><input type="text" tabIndex="2" id="mg005" onKeyPress="keyFunction()" name="mg005" onkeyup="this.value=this.value.replace(/[^A-Z0-9]/gi,'');this.value=this.value.toLocaleUpperCase()"  value="<?php echo  strtoupper($mg005); ?>"  style="background-color:#F0F0F0" readonly="readonly" /></td>
	  </tr>
	  
	  <tr>
		<td class="normal14">執行權限(Y/N)：</td>
        <td class="normal14"><input type="text" tabIndex="3" id="mg004" onKeyPress="keyFunction()" name="mg004" onkeyup="this.value=this.value.replace(/[^A-Z0-9]/gi,'');this.value=this.value.toLocaleUpperCase()"  value="<?php echo  strtoupper($mg004); ?>"  style="background-color:#F0F0F0" readonly="readonly" /></td>
		
		<td class="normal14" >頁次權限(0/1/2)：</td>
		<td class="normal14">
		   <input type="text" tabIndex="4" id="mg003" onKeyPress="keyFunction()" name="mg003" onkeyup="this.value=this.value.replace(/[^A-Z0-9]/gi,'');this.value=this.value.toLocaleUpperCase()"  value="<?php echo  strtoupper($mg003); ?>"  style="background-color:#F0F0F0" readonly="readonly" /><span>(0可見可改,1可見不可改,2不可見)</span>
		</td>
	  </tr>
	</table>
	  ...
	  <div style="width:100%; overflow-x: auto;  ">
        ...
        <thead>
		    ...
		    <?php $current_product_count = 0; //依照模組庫紀錄的明細先列一遍 ?>
			<?php foreach($body_data as $key => $val){
				...
				foreach($usecol_array as $k => $v){
					...
					if($type == "checkbox"){
									echo "<input type='".$type."' id='order_product[".$current_product_count."][".$k."]' name='order_product[".$current_product_count."][".$k."]' class='order_product_".$k."' value='Y' onKeyPress='keyFunction() ' disabled='disabled'";
	...
	<div class="buttons">
	<a  accesskey="x" onKeyPress="keyFunction()" id='cancel' name='cancel' href="<?php echo site_url('adm/admi06/display'); ?>" class="button" ><span>返 回Alt+x</span></a>
	
	    <?php if(isset($prev)){ 
			$prev_str = "";$prev_ary = explode('_',$prev);foreach($prev_ary as $k=>$v){if($prev_str){$prev_str.="/";}$prev_str.=$v;}
		?>
			<a accesskey="," tabIndex="11" id='prev' name='prev' href="<?php echo site_url('adm/admi06/see/'.$prev_str); ?>" class="button" ><span>上一筆Alt+<</span><img src="<?php echo base_url()?>assets/image/png/pre.png" /></a>
		<?php } ?>
		<?php if(isset($next)){ 
			$next_str = "";$next_ary = explode('_',$next);foreach($next_ary as $k=>$v){if($next_str){$next_str.="/";}$next_str.=$v;}
		?>
			<a accesskey="." tabIndex="12" id='next' name='next' href="<?php echo site_url('adm/admi06/see/'.$next_str); ?>" class="button" ><span>下一筆Alt+></span><img src="<?php echo base_url()?>assets/image/png/next.png" /></a>
		<?php } ?>	
		</div> 
      </form>
     </div> <!-- div-6 -->
