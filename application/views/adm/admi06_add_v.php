...
<div id="content"> <!-- div-3 --> 
  <div class="box"> <!-- div-4 -->
	...
	<form   class="cmxform" id="commentForm"  name="form"  method="post" enctype="multipart/form-data"  action="<?php echo base_url()?>index.php/adm/admi06/addsave" >	
	<?php       
		if(!isset($mg001)) { $mg001=$this->input->post('admi10'); }  //使用者代號
		if(!isset($mg001disp)) { $mg001disp=$this->input->post('admi10disp'); } 
		if(!isset($mg003)) { $mg003=$this->input->post('mg003'); } 
		if(!isset($mg004)) { $mg004=$this->input->post('mg004'); } 
		if(!isset($mg005)) { $mg005=$this->input->post('mg005'); } 
	...
	<table class="form14"  >     <!-- 頭部表格 -->
	  <tr>
        ...
        <td class="normal14a"  width="38%"><input tabIndex="1" id="admi10"    onKeyPress="keyFunction()" name="admi10" onfocus="" onchange="check_admi10(this);"  value="<?php echo $mg001; ?>" size="12" type="text" required />
	...
	<table class="form14"  >
	 ...
	 <div style="width:100%; overflow-x: auto;  ">
        ...
        <thead>
           <tr>
		     ...
		     <?php $current_product_count = 0;//依照模組庫紀錄的明細先列一遍 新增只給初值 ?>
          <tfoot>
            <tr>
	 ...
	 <div class="buttons">
	   ...
	   <a  accesskey="x" tabIndex="97" onKeyPress="keyFunction()" id='cancel' name='cancel' href="<?php echo site_url('adm/admi06/display'); ?>" class="button" ><span>返 回Alt+x</span><img src="<?php echo base_url()?>assets/image/png/cancle.png" /></a>
	 </div> 
   </div> 	<!-- end 頁標籤 -->   
   </form>
	   <?php	if ($message!=' ') { ?>
	<div class="success"><?php echo '  提示訊息：'.$message.'<span>'.'</span>'.
'◎操作說明:[ 品號欄位,交貨庫別,可輸入部份品號或品名下拉視窗選擇,圖示1客戶商品計價查詢,欄位淡黃色按2下開視窗查詢,按Enter鍵或Tab鍵跳下一個欄位,Alt+y跳到明細模組, Alt+w新增一筆明細. ] ' ?> </div>  <?php } ?>
...
<?php  include_once("./application/views/funnew/admi10_funmjs_v.php"); ?> <!-- 使用者代號 -->
