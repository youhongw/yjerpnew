...
<div id="content">  <!-- div-3 -->
 <div class="box"> <!-- div-4 --><span>　　　　　　</span>
    <div class="heading">
      <h1><img src="<?=base_url()?>assets/image/order.png" alt="" /> 使用者權限建立作業 - 修改　　　</h1>
      <div style="float:left;padding-top: 5px; ">
	  <button style= "cursor:pointer" form="commentForm" onfocus="$('#mq001').focus();" type='submit'  accesskey="s"  name="submit"  class="button" value='&nbsp;儲存F8&nbsp;'><span>儲 存Alt+s</span><img src="<?=base_url()?>assets/image/png/save.png" /></button>&nbsp;&nbsp;
	    <a  accesskey="x"  onKeyPress="keyFunction()" id="cancel" name="cancel" href="<?php echo site_url('adm/admi05/'.$this->session->userdata('search')); ?>" class="button" ><span>返 回Alt+x</span><img src="<?=base_url()?>assets/image/png/cancle.png" /></a>
	   
	</div>
	</div>
    <div class="content"> <!-- div-5 -->
	<form  class="cmxform" id="commentForm"  name="form" action="<?php echo base_url()?>index.php/adm/admi05/updsave" method="post" enctype="multipart/form-data" >
	<!--<div id="htabs" class="htabs14"><span>編輯項目-修改</span></div> -->
	<div id="tab-general"> <!-- div-6 -->
	<?php foreach($result as $row) { ?>
          <?php   $mg001=$row->mg001;?>
          <?php   $admq02a=$row->mg002;?>
          <?php   $mg003=$row->mg003;?>
		  <?php   $mg004=$row->mg004;?>
		  <?php   $mg005=$row->mg005;?>
		  <?php   $mg006=$row->mg006;?>
          ...
          <?php   $admq02adisp=$row->mg002disp;?>			
	<?php  }?>
     
	<table class="form14">  <!-- 表格 -->
	  <tr>
	    <td class="start14a" width="12%"><span class="required">使用者代號：</span> </td>
        <td class="normal14a" width="38%"><input   tabIndex="1" id="mg001" onKeyPress="keyFunction()" onchange="startCheck5(this)"  name="mg001" onkeyup="this.value=this.value.replace(/[^A-Z0-9]/gi,'');this.value=this.value.toLocaleUpperCase()" value="<?php echo strtoupper($mg001); ?>"  type="text" required readonly="readonly" />
	      <span id="mg001disp" ></span></td>
		<td class="normal14a" width="11%">資料管制(Y/N)：</td>
        <td class="normal14a" width="39%"><input type="text" tabIndex="5" id="mg005" onKeyPress="keyFunction()" name="mg005" onkeyup="this.value=this.value.replace(/[^A-Z0-9]/gi,'');this.value=this.value.toLocaleUpperCase()"  value="<?php echo  strtoupper($mg005); ?>"   /></td>
	  </tr>
	  <tr>
	    <td class="start14a" >程式代碼：</td>
		<td class="normal14"  ><input type="text" tabIndex="2" id="mg002" onKeyPress="keyFunction()"  onchange="startadmq02a(this)" name="admq02a"  onkeyup="this.value=this.value.replace(/[^A-Z0-9]/gi,'');this.value=this.value.toLocaleUpperCase()" value="<?php echo  strtoupper($admq02a); ?>"   /><img id="Showadmq02a" src="<?=base_url()?>assets/image/button-search.png" alt="" align="top" /></a>
	  ...
	  <tr>
		...
		<td class="normal14">
		  <input type="text" tabIndex="3" id="mg003" onKeyPress="keyFunction()" name="mg003" onkeyup="this.value=this.value.replace(/[^A-Z0-9]/gi,'');this.value=this.value.toLocaleUpperCase()"  value="<?php echo  strtoupper($mg003); ?>"   /><span>(0可見可改,1可見不可改,2不可見)</span></td>
