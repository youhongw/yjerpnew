...
<div id="content"> <!-- div-3 -->
 <div class="box"> <!-- div-4 -->
    <div class="heading">
      <h1><img src="<?php echo base_url()?>assets/image/order.png" alt="" /> 資產改良建立作業 - 查看</h1>
	  <form  class="cmxform" id="commentForm"  name="form" action="<?php echo base_url()?>index.php/ast/asti06/display"  method="post" enctype="multipart/form-data" >
    </div>
	...
	<table class="form14"  >     <!-- 頭部表格 -->
	   <tr>
        ...
        <td class="normal14a"  width="24%"><input tabIndex="1" id="asti03_asti06"    onKeyPress="keyFunction()"   name="asti03_asti06" onfocus="" onchange="check_asti03_asti06(this);"  value="<?php echo $tc001; ?>" size="12" type="text" style="background-color:#F0F0F0" readonly="value" required />
	  ...
	  <tr>
        ...
        <td  class="normal14"  ><input type="text" tabIndex="5" onKeyPress="keyFunction()" id="tc007" name="tc007"   value="<?php echo "0"; ?>"  size="12" style="background-color:#F0F0F0" readonly="value" /></td>
        <td class="normal14">增減未攤月數：</td>
        <td  class="normal14"  ><input type="text" tabIndex="5" onKeyPress="keyFunction()" id="tc009" name="tc009"   value="<?php echo "0"; ?>"  size="12" style="background-color:#F0F0F0" readonly="value" /></td>
	...
	<!-- 合計     -->
		...
		<!-- 合計     -->	
	<div class="buttons">
	<a  accesskey="x" onKeyPress="keyFunction()" id='cancel' name='cancel' href="<?php echo site_url('ast/asti06/display'); ?>" class="button" ><span>返 回Alt+x</span></a>
	
	    <?php if(isset($prev)){ 
			$prev_str = "";$prev_ary = explode('_',$prev);foreach($prev_ary as $k=>$v){if($prev_str){$prev_str.="/";}$prev_str.=$v;}
		?>
			<a accesskey="," tabIndex="11" id='prev' name='prev' href="<?php echo site_url('ast/asti06/see/'.$prev_str); ?>" class="button" ><span>上一筆Alt+<</span><img src="<?php echo base_url()?>assets/image/png/pre.png" /></a>
		<?php } ?>
		<?php if(isset($next)){ 
			$next_str = "";$next_ary = explode('_',$next);foreach($next_ary as $k=>$v){if($next_str){$next_str.="/";}$next_str.=$v;}
		?>
			<a accesskey="." tabIndex="12" id='next' name='next' href="<?php echo site_url('ast/asti06/see/'.$next_str); ?>" class="button" ><span>下一筆Alt+></span><img src="<?php echo base_url()?>assets/image/png/next.png" /></a>
		<?php } ?>	
		</div> 
      </form>
     </div> <!-- div-6 -->
