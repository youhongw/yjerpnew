...
<div id="content"> <!-- div-3 --> 
 <div class="box"> <!-- div-4 -->
    <div class="heading">
      <h1><img src="<?php echo base_url()?>assets/image/order.png" alt="" /> 資產重估建立作業 - 修改</h1>
    </div>
	
    <div class="content"> <!-- div-5 -->
	<form  class="cmxform" id="commentForm"  name="form" action="<?php echo base_url()?>index.php/ast/asti07/updsave" method="post" enctype="multipart/form-data" >
	<div id="tab-general"> <!-- div-6 -->
	
	<table class="form14"  >     <!-- 頭部表格 -->
	  <tr>
	    <td class="start14a"  width="10%"><span class="required">單別：</span></td>   <!--onchange="startcopi03(this);check_title_no();"    -->
        <td class="normal14a"  width="24%"><input tabIndex="1" id="asti03_asti07"    onKeyPress="keyFunction()"   name="asti03_asti07"  onchange="check_asti03_asti07(this);"  value="<?php echo $tc001; ?>" size="12" type="text" style="background-color:#F0F0F0" readonly="value" required />
	  ...
	  <tr>
        ...
        <td  class="normal14"  ><input type="text" tabIndex="6" onKeyPress="keyFunction()" id="tc006" name="tc006"   value="<?php echo $tc006; ?>"  size="12"/></td>

        <td class="normal14">增減殘值：</td>
        <td  class="normal14"  ><input type="text" tabIndex="7" onKeyPress="keyFunction()" id="tc010" name="tc010"   value="<?php echo $tc010; ?>"  size="12"/></td>
	  </tr>
	  
	  <tr>
	   	<td class="normal14">異動日期：</td>
        <td  class="normal14"  ><input type="text" tabIndex="8" readonly="value"  onKeyPress="keyFunction()" id="tc003" name="tc003"   value="<?php echo $tc003; ?>" style="background-color:#F0F0F0" size="12" /></td>
        <td class="normal14">列印次數：</td>
        <td  class="normal14"  ><input type="text" tabIndex="9" readonly="value"  onKeyPress="keyFunction()" id="tc016" name="tc016"   value="<?php echo $tc016; ?>" style="background-color:#F0F0F0" size="12" /></td>
        <td class="normal14">確認者：</td>
        <td  class="normal14"  ><input type="text" tabIndex="10" readonly="value"  onKeyPress="keyFunction()" id="tc028" name="tc028"   value="<?php echo $tc028; ?>" style="background-color:#F0F0F0" size="12" /></td>
	  </tr>
	  
	   <tr>
	   	<td class="normal14">產生分錄：</td>
        <td  class="normal14"  ><input type="hidden" name="tc031" value="N" />	
		  <input  type='checkbox' tabIndex="11" id="tc031" onKeyPress="keyFunction()" name="tc031" <?php if($tc031 == 'Y' ) echo 'checked'; ?>  <?php if($tc031 != 'Y' ) echo 'check'; ?> value="Y" size="1"/>
        </td>
        <td class="normal14">簽核狀態：</td>
         <td class="normal14"  ><select id="tc032" tabIndex="12" readonly="value" onKeyPress="keyFunction()" name="tc032"   style="background-color:#F0F0F0" >
        ...
        <td  class="normal14"  ><input type="text" tabIndex="13" onKeyPress="keyFunction()" id="tc013" name="tc013"   value="<?php echo $tc013; ?>" size="12"/></td>
	  </tr>
	</table>
	...
	<?php $current_product_count = 0; ?>
	  ...
	  <a  accesskey="x" onKeyPress="keyFunction()" id='cancel' name='cancel' href="<?php echo site_url('ast/asti07/display'); ?>" class="button" ><span>返 回Alt+x</span><img src="<?php echo base_url()?>assets/image/png/cancle.png" /></a>
	  <?php if(isset($prev)){ 
			$prev_str = "";$prev_ary = explode('_',$prev);foreach($prev_ary as $k=>$v){if($prev_str){$prev_str.="/";}$prev_str.=$v;}
	  ?>
			<a accesskey="," tabIndex="11" id='prev' name='prev' href="<?php echo site_url('ast/asti07/updform/'.$prev_str); ?>" class="button" ><span>上一筆Alt+<</span><img src="<?php echo base_url()?>assets/image/png/pre.png" /></a>
	  <?php } ?>
	  <?php if(isset($next)){ 
			$next_str = "";$next_ary = explode('_',$next);foreach($next_ary as $k=>$v){if($next_str){$next_str.="/";}$next_str.=$v;}
	  ?>
			<a accesskey="." tabIndex="12" id='next' name='next' href="<?php echo site_url('ast/asti07/updform/'.$next_str); ?>" class="button" ><span>下一筆Alt+></span><img src="<?php echo base_url()?>assets/image/png/next.png" /></a>
	  <?php } ?>	
	  </div> 
	  </div> <!-- div-加 -->
    </form>  <!-- end 表單 -->
...
<form action="<?php echo base_url()?>index.php/ast/asti07/delete_detail" method="post" enctype="multipart/form-data" id="del_form" style="display:none;">
...
<?php  include_once("./application/views/funnew/asti07_fundjs_v.php"); ?>  <!-- 明細開視窗 --> 
