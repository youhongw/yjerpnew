...
<div id="content"> <!-- div-3 -->
 <div class="box"> <!-- div-4 -->
    <div class="heading">
      <h1><img src="<?php echo base_url()?>assets/image/order.png" alt="" /> 資產折舊建立作業 - 查看</h1>
	  <form  class="cmxform" id="commentForm"  name="form" action="<?php echo base_url()?>index.php/ast/asti11/display"  method="post" enctype="multipart/form-data" >
    </div>
	...
	<table class="form14"  >     <!-- 頭部表格 -->
	   <tr>
        ...
        <td  class="normal14" width="92%" ><input tabIndex="1" id="asti03_asti11" onKeyPress="keyFunction()" onfocus="check_title_no();"  onchange="check_asti03_asti11(this);check_title_no();" name="asti03_asti11" value="<?php echo $tc001; ?>" size="12" type="text" readonly="value" style="background-color:#F0F0F0" />
	  ...
	  <div style="width:100%; overflow-x: auto;  ">
        ...
        <thead>
			...
			<?php foreach($body_data as $key => $val){
				...
				echo "<tr>";
				echo "<td class='center'><img src='".base_url()."assets/image/delete2.png' title='刪除資料' onclick='del_detail(\"".$tc001."\",\"".$tc002."\",\"".$val->asti02_asti11."\",\"".$val->asti02_asti11_mc003."\",\"".$val->td010."\",\"".$current_product_count."\");' /></td>";
	...
	<div class="buttons">
	<a  accesskey="x" onKeyPress="keyFunction()" id='cancel' name='cancel' href="<?php echo site_url('ast/asti11/display'); ?>" class="button" ><span>返 回Alt+x</span></a>
	
	    <?php if(isset($prev)){ 
			$prev_str = "";$prev_ary = explode('_',$prev);foreach($prev_ary as $k=>$v){if($prev_str){$prev_str.="/";}$prev_str.=$v;}
		?>
			<a accesskey="," tabIndex="11" id='prev' name='prev' href="<?php echo site_url('ast/asti11/see/'.$prev_str); ?>" class="button" ><span>上一筆Alt+<</span><img src="<?php echo base_url()?>assets/image/png/pre.png" /></a>
		<?php } ?>
		<?php if(isset($next)){ 
			$next_str = "";$next_ary = explode('_',$next);foreach($next_ary as $k=>$v){if($next_str){$next_str.="/";}$next_str.=$v;}
		?>
			<a accesskey="." tabIndex="12" id='next' name='next' href="<?php echo site_url('ast/asti11/see/'.$next_str); ?>" class="button" ><span>下一筆Alt+></span><img src="<?php echo base_url()?>assets/image/png/next.png" /></a>
		<?php } ?>	
		</div> 
      </form>
     </div> <!-- div-6 -->
