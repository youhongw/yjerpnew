...
<div id="content"> <!-- div-3 --> 
  <div class="box"> <!-- div-4 -->
    <div class="heading">
      <h1><img src="<?php echo base_url()?>assets/image/order.png" alt="" /> 資產外送建立作業 - 新增</h1>
    </div>
	
    <div class="content"> <!-- div-5 -->
	<form   class="cmxform" id="commentForm"  name="form"  method="post" enctype="multipart/form-data"  action="<?php echo base_url()?>index.php/ast/asti13/addsave" >	
	<?php       
      // 頭部表格  isset 檢查變數 存檔後可保留值
	  if(!isset($te001)) { $te001=$this->input->post('asti03_asti13'); }
	  ...
	  if(!isset($te001disp)) { $te001disp=$this->input->post('asti03_asti13disp'); }
	...
	<table class="form14"  >     <!-- 頭部表格 -->
	  <tr>
        ...
        <td  class="normal14a"  width="25%"  ><input tabIndex="1" id="asti03_asti13" onKeyPress="keyFunction()" onfocus="check_title_no();"  onchange="check_asti03_asti13(this);check_title_no();" name="asti03_asti13" value="<?php echo $te001; ?>" size="12" type="text" required />
		<a href="javascript:;"><img id="Showasti03_asti13disp" src="<?php echo base_url()?>assets/image/png/invoice.png" alt="" align="top"/></a>
	  ...
	  <input type="text" style="display:none" value="0" id="tc004" />
	 ...
	 <div class="buttons">
	   ...
	   <a  accesskey="x" tabIndex="97" onKeyPress="keyFunction()" id='cancel' name='cancel' href="<?php echo site_url('ast/asti13/display'); ?>" class="button" ><span>返 回Alt+x</span><img src="<?php echo base_url()?>assets/image/png/cancle.png" /></a>
	 </div> 
   </div> 	<!-- end 頁標籤 -->   
   </form>
	   <?php	if ($message!=' ') { ?>
...
function check_tg006(tg006){	
	console.log(tg006.value);
	var row = $(tg006).parent().parent().parent()[0].id.substr(12);
	
	
	var asti02 = $('#order_product\\['+row+'\\]\\[asti02\\]').val();
	var asti02_asti13 = $('#order_product\\['+row+'\\]\\[asti02_asti13\\]').val();
	var asti02_asti13_mc003 = $('#order_product\\['+row+'\\]\\[asti02_asti13_mc003\\]').val();
	
	$.ajax({
		url: '<?php echo base_url(); ?>index.php/ast/asti13/check_tg006/'+asti02+'/'+asti02_asti13+'/'+asti02_asti13_mc003+'/'+tg006.value, 
		cache: false, 				
		dataType: 'json',  
		type: 'POST',  
		data: {tg006:tg006.value},
		success:      
		function(data){
			console.log(data);
		}
