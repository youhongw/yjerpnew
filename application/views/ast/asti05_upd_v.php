...
foreach($title_data as $key=>$val){
	$$key = $val;
	if($key == 'mf016' || $key == 'mf017' || $key == 'mf028' || $key == 'mf036' || $key == 'mf038' || $key == 'mf047'){
...
<div id="content"> <!-- div-3 --> 
 <div class="box"> <!-- div-4 -->
    <div class="heading">
      <h1><img src="<?php echo base_url()?>assets/image/order.png" alt="" /> 資產盤點資料建立作業 - 修改</h1>
    </div>
	
    <div class="content"> <!-- div-5 -->
	<form  class="cmxform" id="commentForm"  name="form" action="<?php echo base_url()?>index.php/ast/asti05/updsave" method="post" enctype="multipart/form-data" >
	...
	<table class="form14"  >     <!-- 頭部表格 -->
	  ...
	  <div style="width:100%; overflow-x: auto;  ">
        ...
        <!--  依照資料庫紀錄的明細先列一遍 原0會由1加  -->
			...
			<?php foreach($body_data as $key => $val){
				...
				echo "<tr>";
				echo "<td class='center'><img src='".base_url()."assets/image/delete2.png' title='刪除資料' onclick='del_detail(\"".$val->cmsi05."\",\"".$val->cmsi05_me002."\",\"".$val->cmsi09_asti05."\",\"".$current_product_count."\");' /></td>";
	  ...
	  ?>
			<a accesskey="," tabIndex="11" id='prev' name='prev' href="<?php echo site_url('ast/asti05/updform/'.$prev_str); ?>" class="button" ><span>上一筆Alt+<</span><img src="<?php echo base_url()?>assets/image/png/pre.png" /></a>
	  <?php } ?>
	  <?php if(isset($next)){ 
			$next_str = "";$next_ary = explode('_',$next);foreach($next_ary as $k=>$v){if($next_str){$next_str.="/";}$next_str.=$v;}
	  ?>
			<a accesskey="." tabIndex="12" id='next' name='next' href="<?php echo site_url('ast/asti05/updform/'.$next_str); ?>" class="button" ><span>下一筆Alt+></span><img src="<?php echo base_url()?>assets/image/png/next.png" /></a>
	  <?php } ?>	
	  </div> 
	  </div> <!-- div-加 -->
    </form>  <!-- end 表單 -->
...
<form action="<?php echo base_url()?>index.php/ast/asti05/delete_detail" method="post" enctype="multipart/form-data" id="del_form" style="display:none;">
...
$(document).ready(function(){
	$('#book_value').val("<?php echo ($mf020 + $mf021 - $mf029); ?>");
})
