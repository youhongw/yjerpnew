...
<div id="content"> <!-- div-3 --> 
 <div class="box"> <!-- div-4 -->
    <div class="heading">
      <h1><img src="<?php echo base_url()?>assets/image/order.png" alt="" /> 資產出售建立作業 - 修改</h1>
    </div>
	
    <div class="content"> <!-- div-5 -->
	<form  class="cmxform" id="commentForm"  name="form" action="<?php echo base_url()?>index.php/ast/asti09/updsave" method="post" enctype="multipart/form-data" >
	...
	<table class="form14"  >     <!-- 頭部表格 -->
	  ...
	  <tr>
		...
		<td class="normal14">本幣出售金額：</td>   <!--   $exchange_tc021        -->
        <td class="normal14"><input tabIndex="2" id="exchange_tc021" onKeyPress="keyFunction()"  onchange="" name="exchange_tc021" value="<?php echo $tc021; ?>" size="12" type="text" readonly="value" style="background-color:#F0F0F0"/></td>
	  ...
	  <div style="width:100%; overflow-x: auto;  ">
        ...
        <!--  依照資料庫紀錄的明細先列一遍 原0會由1加  -->
			...
			<?php foreach($body_data as $key => $val){
				...
				echo "<tr>";
				echo "<td class='center'><img src='".base_url()."assets/image/delete.png' title='刪除資料' onclick='del_detail(\"".$tc001."\",\"".$tc002."\",\"".$val->asti02_asti09."\",\"".$val->asti02_asti09_mc003."\",\"".$current_product_count."\");' /></td>";
	  ...
	  <a  accesskey="x" onKeyPress="keyFunction()" id='cancel' name='cancel' href="<?php echo site_url('ast/asti09/display'); ?>" class="button" ><span>返 回Alt+x</span><img src="<?php echo base_url()?>assets/image/png/cancle.png" /></a>
	  <?php if(isset($prev)){ 
			$prev_str = "";$prev_ary = explode('_',$prev);foreach($prev_ary as $k=>$v){if($prev_str){$prev_str.="/";}$prev_str.=$v;}
	  ?>
			<a accesskey="," tabIndex="11" id='prev' name='prev' href="<?php echo site_url('ast/asti09/updform/'.$prev_str); ?>" class="button" ><span>上一筆Alt+<</span><img src="<?php echo base_url()?>assets/image/png/pre.png" /></a>
	  <?php } ?>
	  <?php if(isset($next)){ 
			$next_str = "";$next_ary = explode('_',$next);foreach($next_ary as $k=>$v){if($next_str){$next_str.="/";}$next_str.=$v;}
	  ?>
			<a accesskey="." tabIndex="12" id='next' name='next' href="<?php echo site_url('ast/asti09/updform/'.$next_str); ?>" class="button" ><span>下一筆Alt+></span><img src="<?php echo base_url()?>assets/image/png/next.png" /></a>
	  <?php } ?>	
	  </div> 
	  </div> <!-- div-加 -->
    </form>  <!-- end 表單 -->
...
<form action="<?php echo base_url()?>index.php/ast/asti09/delete_detail" method="post" enctype="multipart/form-data" id="del_form" style="display:none;">
