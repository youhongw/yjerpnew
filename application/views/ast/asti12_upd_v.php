...
<div id="content"> <!-- div-3 --> 
 <div class="box"> <!-- div-4 -->
    <div class="heading">
      <h1><img src="<?php echo base_url()?>assets/image/order.png" alt="" /> 資產移轉建立作業 - 修改</h1>
    </div>
	
    <div class="content"> <!-- div-5 -->
	<form  class="cmxform" id="commentForm"  name="form" action="<?php echo base_url()?>index.php/ast/asti12/updsave" method="post" enctype="multipart/form-data" >
	<div id="tab-general"> <!-- div-6 -->
	
	<table class="form14"  >     <!-- 頭部表格 -->
	   <tr>
	    <td class="start14a" width="9%"><span class="required">單別：</span></td>
        <td  class="normal14a" width="25%""  ><input tabIndex="1" id="asti03_asti12" onKeyPress="keyFunction()" onfocus=""  onchange="check_asti03_asti12(this);check_title_no();" name="asti03_asti12" value="<?php echo $te001; ?>" size="12" type="text" readonly="value" style="background-color:#F0F0F0" required />
		<a href="javascript:;"><img id="" src="<?php echo base_url()?>assets/image/png/invoice.png" alt="" align="top"/></a>
        <span id="asti03_asti12disp"> <?php   echo $te001disp; ?> </span></td>
		  
		  
	    <td class="normal14a" width="8%" >異動日期：</td>  
        <td  class="normal14a" width="25%"><input tabIndex="2"  ondblclick="scwShow(this,event);" id="te003" onKeyPress="keyFunction()"  onchange="dateformat_ymd(this);check_title_no();" name="te003"  value="<?php echo $te003; ?>"  size="12" type="text" readonly="value" style="background-color:#F0F0F0"/></td>
	  </tr>
	  
	  <tr>	    
		<td class="normal14"><span class="required">單號：</span></td>
        <td class="normal14a"><input tabIndex="2" id="te002" onKeyPress="keyFunction()" readonly="value" name="te002" onfocus="" value="<?php echo $te002; ?>" size="12" type="text" required /></td>
	    
		<td class="normal14a">確認者：</td>
		<td  class="normal14"  ><input type="text" tabIndex="8" readonly="value"  onKeyPress="keyFunction()" id="te009" name="te009"  value="<?php echo $te009; ?>" style="background-color:#F0F0F0" size="12"/></td>
	  </tr>
	  
	  <tr>
	    <td class="normal14">單據日期：</td>
        <td class="normal14a"  width="25%" ><input tabIndex="2"  ondblclick="scwShow(this,event);"   id="te008" onKeyPress="keyFunction()"  onchange="dateformat_ymd(this);check_title_no();" name="te008"  value="<?php echo $te008; ?>"  size="12" type="text" style="background-color:#FFFFE4"  /></td>
        
		<td class="normal14">列印次數：</td>
        <td  class="normal14"><input tabIndex="2" id="te007" onKeyPress="keyFunction()"  onchange="" name="te007"  value="<?php echo $te007; ?>"  size="12" type="text" readonly="value" style="background-color:#F0F0F0" /></td>
	  </tr>
 ...
 <!-- <input type="text" style="display:none" value="0" id="asti02temp12" />
	  ...
	  <div style="width:100%; overflow-x: auto;  ">
        ...
        <!--  依照資料庫紀錄的明細先列一遍 原0會由1加  -->
			...
			<?php foreach($body_data as $key => $val){
				...
				echo "<tr>";
				echo "<td class='center'><img src='".base_url()."assets/image/delete.png' title='刪除資料' onclick='del_detail(\"".$te001."\",\"".$te002."\",\"".$val->asti02."\",\"".$val->asti02_asti12."\",\"".$val->asti02_asti12_mc003."\",\"".$current_product_count."\");' /></td>";
	  ...
	  <a  accesskey="x" onKeyPress="keyFunction()" id='cancel' name='cancel' href="<?php echo site_url('ast/asti12/display'); ?>" class="button" ><span>返 回Alt+x</span><img src="<?php echo base_url()?>assets/image/png/cancle.png" /></a>
	  <?php if(isset($prev)){ 
			$prev_str = "";$prev_ary = explode('_',$prev);foreach($prev_ary as $k=>$v){if($prev_str){$prev_str.="/";}$prev_str.=$v;}
	  ?>
			<a accesskey="," tabIndex="11" id='prev' name='prev' href="<?php echo site_url('ast/asti12/updform/'.$prev_str); ?>" class="button" ><span>上一筆Alt+<</span><img src="<?php echo base_url()?>assets/image/png/pre.png" /></a>
	  <?php } ?>
	  <?php if(isset($next)){ 
			$next_str = "";$next_ary = explode('_',$next);foreach($next_ary as $k=>$v){if($next_str){$next_str.="/";}$next_str.=$v;}
	  ?>
			<a accesskey="." tabIndex="12" id='next' name='next' href="<?php echo site_url('ast/asti12/updform/'.$next_str); ?>" class="button" ><span>下一筆Alt+></span><img src="<?php echo base_url()?>assets/image/png/next.png" /></a>
	  <?php } ?>	
	  </div> 
	  </div> <!-- div-加 -->
    </form>  <!-- end 表單 -->
...
<form action="<?php echo base_url()?>index.php/ast/asti12/delete_detail" method="post" enctype="multipart/form-data" id="del_form" style="display:none;">
...
<?php  include_once("./application/views/funnew/puri01_funmjs_v.php"); ?> <!-- 外送地點 -->
<?php  include_once("./application/views/funnew/erp_funjs_v.php"); ?>      <!-- 全域變數 -->
<?php  include_once("./application/views/funnew/asti12_fundjs_v.php"); ?>  <!-- 明細開視窗 --> 
...
function check_tf006(row_obj){	
	if($.isNumeric(row_obj)){row = row_obj;}
	else{var row = $(row_obj).parent().parent().parent()[0].id.substr(12);}	
	var tf006 = $('#order_product\\['+row+'\\]\\[tf006\\]').val();

	if (tf006 <0){
		alert("外送數量需大於零！");
		$('#order_product\\['+row+'\\]\\[tf006\\]').val('');
	}
}	
</script>
