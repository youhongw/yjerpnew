  <?php if (!$results)  { ?>
        <script> alert("無資料可列印!"); url = '<?=base_url() ?>index.php/cop/copr08/printdetail';location = url; </script> 
  <?php exit;} ?>
<!--   頁直行數  -->
...
?>       
	...
	<table class="thead">    <!-- 列表頭 -->
	  <tr>
	    ...
	    <td width="12%" align="right">銷貨金額</td>
		<td width="12%" align="right">銷貨成本</td>
		<td width="12%" align="right">銷貨毛利<br>毛利率</td>
	...
	<table class="list">     <!-- 列明細 2 判斷 31, 3 判斷 32 4 判斷 32, 31-->
		...
		<td width="34%" align="left"><?php if ($val->th005!='') {  echo wordwrap($val->th005,75,"<br />\n",TRUE);} else {echo "_";}?> 
        ...
        <td width="12%" align="right"><?php echo  round($val->th013);?></td> 
		<?php if (round($val->th013)==0) {$th013=1;} else {$th013=round($val->th013);} ?>
		<?php if (round($val->th013a)==0) {$th013a=1;} else {$th013a=round($val->th013a);} ?>
		<?php if (round($val->th013b)==0) {$th013b=1;} else {$th013b=round($val->th013b);} ?>
		<td width="12%" align="right"><?php echo  round($val->th013a);?></td> 
		<td width="12%" align="right"><?php echo  round($val->th013b);?><br><?php echo  round(($th013b)/round($th013)*100,2).'%';?></td> 
