<? //echo "<pre>";var_dump($results);exit; ?>
	...
	<?php foreach($results as $row ) : ?>
      <?php $numrow1 +=2;  ?> 
	    <?php if ($row['mb002'] =='')  { ?>
	      <script> alert("無資料可列印!"); url = '<?=base_url() ?>index.php/bom/bomr05/printdetail';location = url; </script> 
	   <?php } ?>
	  <?php endforeach;?>
	  <!-- 表頭3行   -->
       <?php $numrow1 +=4;  ?> 
	...
	<table class="thead">    <!-- 列表頭 -->
	  <tr>
	    <td width="10%" align="center">主件品號</td>
	...
	<table class="list">     <!-- 列明細 2 判斷 31, 3 判斷 32 4 判斷 32, 31-->
	   ...
	   <?php foreach($results as $row ) : ?>
	      <tr>
		 <td width="10%" align="center"><?php if($row['level'] == "0") echo $row['md003'];?></td>
		<td width="10%" align="center"><?php echo $row['level'];?></td>
        <td width="10%" align="center"><?php if($row['level'] != "0") echo  $row['md001'];?></td>			
        <td width="10%" align="center"><?php echo  $row['mb025'];?></td>		
		<td width="30%" align="left"><?php if ($row['mb002']!='') {  echo wordwrap($row['mb002'],75,"<br />\n",TRUE);} else {echo "_";}?> 
