...
?>
        ...
        <tr>
			...
			<td class="center"><a href="javascript:send_back('<?php echo $row->ma001;?>','<?php echo $row->ma002;?>');">[ 選擇 </a><img src="<?php echo base_url()?>assets/image/png/eye.png" />]</td>  
		</tr>
		  <?php $chkval += 1; ?>
		  <?php endforeach;?>
        </tbody>		 
...
function send_back(ma001, ma002){
	window.parent.$.unblockUI();
	if(window.parent.addtc006disp){	//以此判斷各呼叫此網頁的方法是否存在藉以給予相對應的需求
		window.parent.addtc006disp(ma001,ma002);
		$.ajax({
			method: "POST",
