...
<div id="content"> <!-- div-3 --> 
 <div class="box"> <!-- div-4 -->
    <div class="heading">
      <h1><img src="<?php echo base_url()?>assets/image/order.png" alt="" /> 進貨單資料建立作業 - 修改</h1>
    </div>
	
    <div class="content"> <!-- div-5 -->
	<form  class="cmxform" id="commentForm"  name="form" action="<?php echo base_url()?>index.php/pur/puri09/updsave" method="post" enctype="multipart/form-data" >
	...
	<?php foreach($result as $row) { ?>
		  ...
		  <?php   $tg025=$row->tg025;?>
	      <?php   $tg026=$row->tg026;?>
		  <?php if ($row->tg027 > '') {$tg027=substr($row->tg027,0,4).'/'.substr($row->tg027,4,2).'/'.substr($row->tg027,6,2);} else {$tg027='';} ?>
		  <?php  // $tg027=$row->tg027;?>
		  <?php   $tg028=$row->tg028;?>
		  <?php if ($row->tg029 > '') {$tg029=substr($row->tg029,0,4).'/'.substr($row->tg029,4,2);} else {$tg029='';} ?>
		  <?php //  $tg029=substr($row->tg029,0,4).'/'.substr($row->tg029,4,2);?>
		  <?php   $tg030=$row->tg030;?>
		  <?php   $tg031=$row->tg031;?>
		  <?php   $tg032=$row->tg032;?>
		  <?php   $tg033=$row->tg033;?>
		 ...
		 <!-- 明細 -->
		   ...
		   <?php   $th009disp[]=$row->th009disp;?>
		    <?php  if ($row->th014=='') {$th014[]=$row->th014;} else  {$th014[]=substr($row->th014,0,4).'/'.substr($row->th014,4,2).'/'.substr($row->th014,6,2);} ?>
		   <?php  //  $th014[]=$row->th014;?>  
		   <?php   $th007[]=$row->th007;?>  
		   <?php   $th015[]=$row->th015;?>   
		   <?php   $th017[]=$row->th017;?>   
		   <?php   $th016[]=$row->th016;?> 
		   <?php   $th016a[]=$row->th016;?> 
	...
	<table class="form14"  >     <!-- 頭部表格 -->
	 <tr>
        ...
        <td class="normal14a"  width="40%" ><input tabIndex="2"  ondblclick="scwShow(this,event);"   onchange="dataymd1(this)"  id="tg014" onKeyPress="keyFunction()"   name="tg014"  value="<?php echo $tg014; ?>"  size="12" type="text"  style="background-color:#E7EFEF" /></td>
	  </tr>	
	  <tr>
	    <td class="start14a" >進貨單號：</td>
        <td class="normal14a" ><input tabIndex="3" id="tg002" readonly="value" onKeyPress="keyFunction()"  name="tg002" value="<?php echo $tg002; ?>" size="30" type="text" required /><span id="tg002disp" ></span></td>
		 <td class="normal14a">供應廠商：</td>
        <td  class="normal14"  ><input tabIndex="4" id="tg005" onKeyPress="keyFunction()"  onchange="startpurq01a(this)" name="purq01a" value="<?php echo $purq01a; ?>" size="10" type="text"  /><img id="Showpurq01a" src="<?php echo base_url()?>assets/image/png/factory.png" alt="" align="top"/></a>
	  ...
	  <tr>
		...
		<td class="normal14">確認碼：</td>
          <td  class="normal14"  ><select id="tg013" onKeyPress="keyFunction()" name="tg013" onChange="selappr(this)" tabIndex="6">
		    ...
		    <td class="start14"><input type='hidden' name='tg015' id='tg015' value="<?php echo $tg015; ?>" /></td>
	...
	<table class="form14">     <!-- 表格 -->
	 <tr>
	  ...
	  <tr>
        ...
        <td  class="normal14"  ><input type="text" tabIndex="20"   onKeyPress="keyFunction()"  onclick="scwShow(this,event);" id="tg027" name="tg027"   value="<?php echo $tg027; ?>"   style="background-color:#E7EFEF" /></td>
	...
	<!--     <td class="center"><img src="<?php echo base_url()?>assets/image/delete.png" title="移除" onclick="$(\'#product-row0\').remove();" /></td> -->
		...
		<input type="hidden"  name="order_product[<?php echo $i ?>][th001]" value="<?php echo $th001[$i]; ?>" />
	     ...
	     <td class="left"><input type="text"  <?php echo 'id='.'th004'.$i ?> onchange="startinvq02a(this,product_row)"   name="order_product[<?php echo $i ?>][th004]" value="<?php echo $th004[$i]; ?>" size="20" style="background-color:#E7EFEF"  /></td>
	     ...
	     <td class="left"><input  type="text"  onKeyPress="keyFunction()"    id="th008"   name="order_product[<?php echo $i ?>][th008]" value="<?php echo $th008[$i]; ?>" size="6"  /></td>
		 ...
		 <td class="left"><input type="text"   <?php echo 'id='.'th009'.$i ?>  onchange="startcmsq03a(this,product_row)"  name="order_product[<?php echo $i ?>][th009]" value="<?php echo $th009[$i]; ?>" size="10" style="background-color:#E7EFEF"  /></td>
		...
		<td class="left"><input type="text"   ondblclick="scwShow(this,event);"  id="th014[<?php echo $i ?>]" onKeyPress="keyFunction()" name="order_product[<?php echo $i ?>][th014]" value="<?php echo  $th014[$i]; ?>" size="10"  /></td>
	     <td class="center"><input type="text"  class="total_qty" id="th007" onKeyPress="keyFunction()" onchange="pricepurq02a(this,product_row)" name="order_product[<?php echo $i ?>][th007]" value="<?php echo $th007[$i]; ?>" size="10" style="text-align:right;" /></td>
		 <td class="center"><input type="text"  id="th015" onKeyPress="keyFunction()" name="order_product[<?php echo $i ?>][th015]" value="<?php echo $th015[$i]; ?>" size="10" style="text-align:right;" /></td>
		 <td class="center"><input type="text"  id="th017" onKeyPress="keyFunction()" name="order_product[<?php echo $i ?>][th017]" value="<?php echo $th017[$i]; ?>" size="10" style="text-align:right;" /></td>
		 <td class="center"><input type="text"  id="th016" onKeyPress="keyFunction()" name="order_product[<?php echo $i ?>][th016]" value="<?php echo $th016[$i]; ?>" size="10" style="text-align:right;" /></td>
		 <input type="hidden"  id="th016a"  name="order_product[<?php echo $i ?>][th016a]" value="<?php echo $th016a[$i]; ?>"  />
		 
         <td class="center"><input type="text"   id="th018" onKeyPress="keyFunction()" name="order_product[<?php echo $i ?>][th018]" value="<?php echo $th018[$i]; ?>" size="10" style="text-align:right;"  /></td>	
		 ...
		 <td class="right"><input readonly="value" type="text" class="total_price1" name="order_product[<?php echo $i ?>][th047]" value="<?php echo $th047[$i]; ?>" size="10" style="text-align:right;background-color:#EBEBE4;" /></td>
	     ...
	     <td class="left"><input type="text" id="th011"  onKeyPress="keyFunction()" name="order_product[<?php echo $i ?>][th011]" value="<?php echo $th011[$i]; ?>" size="10"  /></td>
		 ...
		 <td class="left"><input type="text" id="th028"  onKeyPress="keyFunction()" name="order_product[<?php echo $i ?>][th028]" value="<?php echo $th028[$i]; ?>" size="10"  /></td>
 ...
 <?php } ?>		 
	...
	<?php include("./application/views/fun/puri09_funjsupdjs_v.php"); ?> 
	 ...
	 <!-- 合計     -->
	  ...
	  <a  accesskey="x" onKeyPress="keyFunction()" id='cancel' name='cancel' href="<?php echo site_url('pur/puri09/display'); ?>" class="button" ><span>返 回Alt+x</span><img src="<?php echo base_url()?>assets/image/png/cancle.png" /></a>
	  </div> 
	  </div> <!-- div-加 -->
    </form>
	<?php	if ($message!=' ') { ?>
	<div class="success"><?php echo '  提示訊息：'.$message.'<span>'.'</span>'.
'◎操作說明:[ 品號欄位可輸入部份品號或品名下拉視窗選擇,Alt+y跳明細欄位, Alt+w 新增一筆明細,Tab鍵停留在有選項欄位按上下鍵可選擇資料. ] ' ?> </div>  <?php } ?>
    </div> <!-- div-6 -->
  </div> <!-- div-5 -->
</div> <!-- div-4 -->
 ...
 <form action="<?php echo base_url()?>index.php/pur/puri09/delete_detail" method="post" enctype="multipart/form-data" id="del_form" style="display:none;">
			<input id="del_md001" name="del_md001" />
			<input id="del_md002" name="del_md002" />
			<input id="del_md003" name="del_md003" />
		</form>
 <?php include("./application/views/fun/puri09_funjsupd_v.php"); ?>