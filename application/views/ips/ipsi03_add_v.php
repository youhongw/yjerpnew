...
<div id="content"> <!-- div-3 --> 
  <div class="box"> <!-- div-4 -->
    <div class="heading">
      <h1><img src="<?php echo base_url()?>assets/image/order.png" alt="" /> 預付購料變更建立作業 - 新增</h1>
    </div>
	
    <div class="content"> <!-- div-5 -->
	<form   class="cmxform" id="commentForm"  name="form"  method="post" enctype="multipart/form-data"  action="<?php echo base_url()?>index.php/ips/ipsi03/addsave" >	
	<?php       
	  ...
	  if(!isset($tc005)) { $tc005=$this->input->post('tc005'); }
	  ...
	  if(!isset($tc009disp)) { $tc009disp=$this->input->post('tc009disp'); }
	  
	  if(!isset($tc010)) { $tc010="Y"; }
	  ...
	  if(!isset($tc014)) { $tc014=$this->input->post('tc014'); } 
	  if(!isset($tc015)) { $tc015=$this->input->post('tc015'); }
	  if(!isset($tc016)) { $tc016=$this->input->post('tc016'); }
	  if(!isset($tc017)) { $tc017=$this->input->post('tc017'); }
	  if(!isset($tc018)) { $tc018=$this->input->post('tc018'); }
	  if(!isset($tc019)) { $tc019=$this->input->post('tc019'); }
	  if(!isset($tc020)) { $tc020=$this->input->post('tc020'); }
	  if(!isset($tc021)) { $tc021=$this->input->post('tc021'); }
	  if(!isset($tc022)) { $tc022=$this->input->post('tc022'); }
	  if(!isset($tc023)) { $tc023=$this->input->post('tc023'); }
	  if(!isset($tc024)) { $tc024=$this->input->post('tc024'); }
	  if(!isset($tc025)) { $tc025=date("Y/m/d"); } 
	  if(!isset($tc026)) { $tc026=$username; }
	  if(!isset($tc027))  { $tc027="N"; }
	  if(!isset($tc028)) { $tc028=$this->session->userdata('sysma003'); } else  {$tc028=$this->input->post('tc028');}
	  if(!isset($tc028disp)) { $tc028disp=$this->input->post('tc028disp'); }
	  if(!isset($tc029)) { $tc029=$this->input->post('tc029'); }
	  if(!isset($tc029disp)) { $tc029disp=$this->input->post('tc029disp'); }
	  
	  if(!isset($tc111)) { $tc111=$this->input->post('tc111'); }
	  if(!isset($tc112)) { $tc112=$this->input->post('tc112'); }
	  if(!isset($tc113)) { $tc113=$this->input->post('tc113'); }
	  if(!isset($tc114)) { $tc114=$this->input->post('tc114'); }
	  if(!isset($tc115)) { $tc115=$this->input->post('tc115'); }
	  if(!isset($tc116)) { $tc116=$this->input->post('tc116'); }
	  if(!isset($tc117)) { $tc117=$this->input->post('tc117'); }
	  if(!isset($tc118)) { $tc118=$this->input->post('tc118'); }
	  if(!isset($tc119)) { $tc119=$this->input->post('tc119'); }
	  if(!isset($tc120)) { $tc120=$this->input->post('tc120'); }
	  if(!isset($tc121)) { $tc121=$this->input->post('tc121'); }
	  if(!isset($tc122)) { $tc122=$this->input->post('tc122'); }
	  if(!isset($tc123)) { $tc123=$this->input->post('tc123'); }
	  if(!isset($tc124)) { $tc124=$this->input->post('tc124'); }
	  if(!isset($tc125)) { $tc125=$this->input->post('tc125'); }
	  if(!isset($tc126)) { $tc126=$this->input->post('tc126'); }
	  if(!isset($tc127)) { $tc127=$this->input->post('tc127'); }
	  if(!isset($tc127disp)) { $tc127disp=$this->input->post('tc127disp'); }
	  if(!isset($tc128)) { $tc128=$this->input->post('tc128'); }
	  if(!isset($tc129)) { $tc129=$this->input->post('tc129'); }
	  if(!isset($tc130)) { $tc130=$this->input->post('tc130'); }
	  if(!isset($tc131)) { $tc131=$this->input->post('tc131'); }
	  if(!isset($tc132)) { $tc132=$this->input->post('tc132'); }
	  if(!isset($tc133)) { $tc133=$this->input->post('tc133'); }
	  if(!isset($tc134)) { $tc134=$this->input->post('tc134'); }
	  if(!isset($tc135)) { $tc135=$this->input->post('tc135'); }
	  if(!isset($tc136)) { $tc136=$this->input->post('tc136'); }
	  if(!isset($tc137)) { $tc137=$this->input->post('tc137'); }
	  if(!isset($tc138)) { $tc138=$this->input->post('tc138'); }
	...
	<table class="form14"  >     <!-- 頭部表格 -->
	  <tr>
        ...
        <td class="normal14a"  width="24%"><input tabIndex="1" id="ipsi02"  ondblclick="search_ipsi02_window()"  onKeyPress="keyFunction()"   name="tc001" onfocus="selverify();" onchange="check_ipsi02(this);check_title_no();"  value="<?php echo $tc001; ?>" size="12" type="text" required />
		...
		<input tabIndex="3" id="tc003" onKeyPress="keyFunction()"  name="tc003" onfocus="check_title_no1();" value="<?php echo $tc003; ?>" size="6" type="text" style="text-align:right;"  /></td>
	  </tr>
	...
	<table class="form14">     <!-- 表格 -->
	 ...
	 <div class="buttons">
	   ...
	   <a  accesskey="x" tabIndex="97" onKeyPress="keyFunction()" id='cancel' name='cancel' href="<?php echo site_url('ips/ipsi03/display'); ?>" class="button" ><span>返 回Alt+x</span><img src="<?php echo base_url()?>assets/image/png/cancle.png" /></a>
	 </div> 
   </div> 	<!-- end 頁標籤 -->   
   </form>
	   <?php	if ($message!=' ') { ?>
