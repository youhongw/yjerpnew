      ...
      <table class="list">      <!-- 表格開始 -->
        <thead>
          <tr>                          <!-- 表格表頭 -->
			...
			<td width="5%" class="center"> 
		      <?php echo anchor("pal/pali46/display/b.mv002/" . (($sort_order == 'asc' && $sort_by == 'b.mv002') ? 'desc' : 'asc') ,'員工姓名'); ?>
		      <?php if ($sort_order == 'asc'  ) { ?>  <img src="<?php echo base_url()?>assets/image/desc.png" /> <?php } else { ?>
		      <img src="<?php echo base_url()?>assets/image/asc.png" />  <?php }  ?>
	        </td>
	        <td width="5%" class="center"> 
	        ...
	        <td width="3%" class="center">
		      <?php echo anchor("pal/pali46/display/a.create_date/" . (($sort_order == 'asc' && $sort_by == 'a.create_date') ? 'desc' : 'asc') ,'建立日期'); ?>
		      <?php if ($sort_order == 'asc'  ) { ?>  <img src="<?php echo base_url()?>assets/image/desc.png" /> <?php } else { ?>
		      <img src="<?php echo base_url()?>assets/image/asc.png" />  <?php }  ?>
	        </td>
			...
			<td width="20%" class="center">&nbsp刪除&nbsp</td>
          </tr>
        </thead>
		  
        <tbody>     <!-- 表格內容輸入篩選查詢 第一,二欄 刪除選項及序號空白 -->
	      <?php $filter_yg001='';$filter_yg002='';$filter_yg002disp='';$filter_yg003='';$filter_yg004='';$filter_create=''; ?>
	      <tr class="filter">
	        <td class="left"></td>
		...
		<!--session 變數取消 onclick="open_winexcel()"	  -->
		<?php $this->session->unset_userdata('yg002'); ?> 
	    <?php $chkval=1; ?>               
	    <?php foreach($results as $row ) : ?>
        <tr  >		
          <td style="text-align: center;"> <input type="checkbox" name="selected[]"   id="cbbox"  value="<?php echo $row->yg001."/".$row->yg002;?>" onclick="$('input[name=\'selected\']').attr('checked', this.checked);" /></td>
		...
		<td class="center"><a href="<?php echo site_url('pal/pali46/see/'.$row->yg001."/".$row->yg002)?>">[ 查看 </a><img src="<?php echo base_url()?>assets/image/png/eye.png" />]</td>
		  ...
		  <td class="center"><a href="<?php echo site_url('pal/pali46/updform/'.$row->yg001."/".$row->yg002)?>">[ 修改 </a><img src="<?php echo base_url()?>assets/image/png/modi.png" />]</td>
	        <?PHP } ?>  
			 <td class="center"> <a onclick="return CheckForm();" href="<?php echo site_url('pal/pali46/del/'.$row->yg001."/".$row->yg002)?>" id="delete1"  >[ 刪除 ]</a></td> 
		</tr>
		<?php $chkval += 1; ?>
		<?php endforeach;?>
        </tbody>		 
...
function filter() {
	...
	var filter_yg002disp = $('input[name=\'filter_yg002disp\']').val();
	if (filter_yg002disp) {
		url = '<?php echo base_url() ?>index.php/pal/pali46/filter1/b.mv002/desc/' + encodeURIComponent(filter_yg002disp);
	} 
	...
	if (filter_create) {
		url = '<?php echo base_url() ?>index.php/pal/pali46/filter1/a.create_date/desc/' + encodeURIComponent(filter_create); 
	}
	
    if (!filter_yg001 && !filter_yg002 && !filter_yg002disp && !filter_yg003 && !filter_yg004 && !filter_create) {         
...
function filtera() {
	...
	var filter_yg002disp = $('input[name=\'filter_yg002disp\']').val();
	if (filter_yg002disp) {
		url = '<?php echo base_url() ?>index.php/pal/pali46/filter1/b.mv002/asc/' + encodeURIComponent(filter_yg002disp);
	} 
	...
	var filter_create = $('input[name=\'filter_create\']').val();
	if (filter_create) {
		url = '<?php echo base_url() ?>index.php/pal/pali46/filter1/a.create_date/asc/' + encodeURIComponent(filter_create); 
	}
