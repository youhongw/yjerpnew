...
<div id="content">  <!-- div-3 -->
  <div class="box"> <!-- div-4 -->
	...
	<?php
	  $date=date("Y/m/d");	  
	?>
<!--hidden data--><input id="tf002" name="tf002" value="<?php echo $te002?>" style="display:none;" /><input id="tf003" name="tf003" value="<?php echo date("w",mktime(0,0,0,$arrDate[1],$arrDate[2],$arrDate[0]));?>" style="display:none;" />
	...
	<tr>
		...
		<?php 
		if($tf003 == 6){?>六加班8小時內　六加班8小時外<?php }else if($tf003 == 0 || $holiday){?>日加班8小時內　日加班8小時外<?php }else{?>平時加班2小時內　平時加班2小時外<?php }
		?></td>
...
<!--備註--><td class='start14'><?php foreach($val->times as $k => $v){echo $v." ";} ?><input id="first_time_<?php echo $origin_num;?>" value="<?php echo $val->times[0] ?>" style="display:none;" /><input id="last_time_<?php echo $origin_num;?>" value="<?php echo $v ?>" style="display:none;" /></td>
		...
		if($tf003 == 6){?><input id='tf012_<?php echo $origin_num;?>' name="tf012[<?php echo $origin_num-1;?>]" value="<?php echo $val->eight_hr1 ?>" size='2' />小時　　　　　<input id='tf013_<?php echo $origin_num;?>' name="tf013[<?php echo $origin_num-1;?>]" value="<?php echo $val->eight_hr2 ?>" size='2' />小時
		<?php }else if($tf003 == 7 || $holiday){?><input id='tf014_<?php echo $origin_num;?>' name="tf014[<?php echo $origin_num-1;?>]" value="<?php echo $val->eight_hr1 ?>" size='2' />小時　　　　　<input id='tf015_<?php echo $origin_num;?>' name="tf015[<?php echo $origin_num-1;?>]" value="<?php echo $val->eight_hr2 ?>" size='2' />小時
...
$('.ipt_add1').change(function(){
	...
	if(week == 6){
		if($('#tf069_'+temp_id).val()>8){
			$('#tf012_'+temp_id).val(8);$('#tf013_'+temp_id).val($('#tf069_'+temp_id).val()-8);
		}else{
