...
<div id="content">  <!-- div-3 -->
  <div class="box"> <!-- div-4 -->
    <div class="heading">
      <h1><img src="<?php echo base_url()?>assets/image/order.png" alt="" /> 勞退級距建立作業 - 新增</h1>
    </div>
	
    <div class="content"> <!-- div-5 -->
	<form   class="cmxform" id="commentForm"  name="form"  method="post" enctype="multipart/form-data"  action="<?php echo base_url()?>index.php/pal/pali18/addsave" >	
	<!--<div id="htabs" class="htabs14"  ><span>編輯項目-新增</span></div>-->
	<div id="tab-general"> <!-- div-6 -->
	<?php
	  $date=date("Ymd");
	  if (!isset($mw001)) { $mw001='';} else { $mw001=$this->input->post('mw001');}
	  if (!isset($mw002)) { $mw002=0;} else { $mw002=$this->input->post('mw002');}
	  if (!isset($mw003)) { $mw003=0;} else { $mw003=$this->input->post('mw003');}
	  if (!isset($mw004)) { $mw004=0;} else { $mw004=$this->input->post('mw004');}
	  if (!isset($mw005)) { $mw005=0;} else { $mw005=$this->input->post('mw005');}
	  if (!isset($mw006)) { $mw006=0;} else { $mw006=$this->input->post('mw006');}
	  if (!isset($mw007)) { $mw007=0;} else { $mw007=$this->input->post('mw007');}
	  if (!isset($mw008)) { $mw008=0;} else { $mw008=$this->input->post('mw008');}
	  if (!isset($mw009)) { $mw009=0;} else { $mw009=$this->input->post('mw009');}
	  if (!isset($mw010)) { $mw010='';} else { $mw010=$this->input->post('mw010');}
	...
	<a  accesskey="x"  tabIndex="9" id='cancel' name='cancel' href="<?php echo site_url('pal/pali18/display'); ?>" class="button" ><span>返 回Alt+x</span><img src="<?php echo base_url()?>assets/image/png/cancle.png" /></a>
	</div> 
...
$('#mw002').change(function(){
	...
	$('#mw005').val(<?php echo $rates->mr003;?>*insurance/100);
	$('#mw006').val(<?php echo $rates->mr004;?>*insurance/100);
