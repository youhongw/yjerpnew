...
<div id="content">  <!-- div-3 -->
  <div class="box"> <!-- div-4 -->
	...
	<?php
	  $date=date("Y/m/d");	  
	?>
<!--hidden data--><input id="me001" name="me001" value="<?php echo $result[$what_ever]->me001 ?>" style="display:none;" /><input id="tf002" name="tf002" value="<?php echo $tf002?>" style="display:none;" /><input id="tf003" name="tf003" value="<?php echo date("w",mktime(0,0,0,$arrDate[1],$arrDate[2],$arrDate[0]));?>" style="display:none;" />
	...
	<tr>
		...
		<?php 
		if($tf003 == 6){?>六加班2內　六加班2外　六加班8<?php }else if($tf003 == 7 || $holiday){?>日加班8內　日加班8外2內　日加班8外2外<?php }else{?>平時加班2小時內　平時加班2小時外<?php }
		?></td>
...
<!--備註--><td class='start14'><?php if(@$times[$val->tf002][$val->tf001]){foreach($times[$val->tf002][$val->tf001] as $k => $v){echo $v." ";}}else{echo "無刷卡資料!!";} ?><input id="first_time_<?php echo $origin_num;?>" value="<?php if(@$times[$val->tf002][$val->tf001]){echo $times[$val->tf002][$val->tf001][0];} ?>" style="display:none;" /><input id="last_time_<?php echo $origin_num;?>" value="<?php if(@$times[$val->tf002][$val->tf001]){echo $v;} ?>" style="display:none;" /></td>
		...
		if($tf003 == 6){?><input id='tf012_<?php echo $origin_num;?>' name="tf012[<?php echo $origin_num-1;?>]" value="<?php if(@$val->tf012) echo $val->tf012; else echo $val->sat_hr1 ?>" size='4' />小時　 <input id='tf013_<?php echo $origin_num;?>' name="tf013[<?php echo $origin_num-1;?>]" value="<?php if(@$val->tf013) echo $val->tf013;else echo $val->sat_hr2 ?>" size='4' />小時　 <input id='tf018_<?php echo $origin_num;?>' name="tf018[<?php echo $origin_num-1;?>]" value="<?php if(@$val->tf018) echo $val->tf018;else echo $val->sat_hr3 ?>" size='4' />小時
		<?php }else if($tf003 == 0 || $holiday){?><input id='tf014_<?php echo $origin_num;?>' name="tf014[<?php echo $origin_num-1;?>]" value="<?php if(@$val->tf014) echo $val->tf014;else echo $val->eight_hr1; ?>" size='4' />小時　 <input id='tf015_<?php echo $origin_num;?>' name="tf015[<?php echo $origin_num-1;?>]" value="<?php if(@$val->tf015) echo $val->tf015; else echo $val->eight_hr2 ?>" size='4' />小時　　　 <input id='tf019_<?php echo $origin_num;?>' name="tf019[<?php echo $origin_num-1;?>]" value="<?php if(@$val->tf019) echo $val->tf019; else echo $val->eight_hr3 ?>" size='4' />小時
