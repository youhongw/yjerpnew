...
<div>
<input type='text' id="mt003o" name="mt003o" class="mt003o" size="12" value="<?php isset($mt003o) ? $mt003o : ''; ?>"  /> 
查詢人事區間:<input id="mt001o" name="mt001o" class="mt001o" size="3" onchange="search_event();" />~<input id="mt001c" name="mt001c" size="3" onchange="search_event();" />　　
...
function view_days_of_month(current_year_month){
	...
	$.ajax({
		method: "POST",
		url: "<?php echo base_url()?>index.php/pal/pali29/get_pali29",
		dataType: "json",
		async: false,
		data: { 
			mt001o:$('#mt001o').val(),
	...
	$.ajax({
		method: "POST",
		url: "<?php echo base_url()?>index.php/pal/pali29/get_pali16",
		dataType: "json",
		data: { 
			mo001o:$('#mt001o').val(),
			mo001c:$('#mt001c').val(),
	...
	.done(function( msg ) {
		...
		for(var j=1;j<=12;j++){
			...
			<!--	append_str += '<br><div class="show_event">正常上班: '; <!--
			    if (week==0 || week==6 ) {append_str += '<br><div class="show_event">休假: ';}
				    else {append_str += '<br><div class="show_event">正常上班: ';}
				//for(var dw_key in day_work_ary)
				//{var dw_val = day_work_ary[dw_key];append_str += "'"+dw_val+"' ";}
			
			    vdate=temp_year+j+k;
				     jj=padLeft(j.toString(),2);
					 kk=padLeft(k.toString(),2);
				 vadate=temp_year+jj+kk;
				 
				    vmonth=padLeft(_current_month.toString(),2);
				vvdate=_current_year+vmonth;
				
				kkdata='1';
				data2=' ';
			  //  console.log(vadate);
			//	console.log(vadate.substr(0,6));
			    console.log(_current_year+vmonth);
			//	console.log(_current_month);
				/*append_str += '<br>半:';
				for(var dh_key in day_half_ary){var dh_val = day_half_ary[dh_key];append_str += dh_val+" ";}*/
				<!-- 1070511休:   1070514 _current_date-->
				//console.log(_real_date);
				if  ( vadate.substr(0,6) == vvdate ) {
				$.ajax({
		method: "POST",
	 ...
	 .done(function( data1 ) {
		if( data1 > 'z' ){
			...
			document.cookie =  "myname="+data1 ;
			data1="";
		//	alert(getCookie(myname="myname"));
		//	vmt006=$("#result").text(data1);
		//	 console.log(vmt006);
			//console.log('msg.response');
		}else{
		   //  $('#mt003o').val(msg);
			 console.log(data2);
			 document.cookie =  "myname="+data2 ;
			// alert(document.cookie);
			 data1="";
			// vmt006=$("#searchResult").html(''+data1.msg+'');
			// console.log(vmt006);
			// request.getSession().setAttribute("sname", 'a');
		}
	//	console.log('國定假日test');
		//$('#mt003o').val(msg);
		// console.log(msg);
			...
			// append_str += '<input type="text" id="mt003o" name="mt003o" class="mt003o" size="12" value=""  />' ;
			// $('#mt003o').before(append_str);
             //  console.log(zstr);
			 
           //   if  ( vadate.substr(0,6) == _current_date.substr(0,6) ) {append_str += '<br>'+getCookie(myname="myname");}
           //      else 	{append_str += '<br>' ;}	
				append_str += '<br>'+getCookie(myname="myname"); 
               document.cookie =  "myname="+data2 ;				 
			//	 if ( @val['mt006'] ) {append_str += val['mt006'] ;}
				    
			/*	IF ( vdate.substr(0,6) == '201805' ) {
				$.ajax({
		method: "POST",
		url: "<?php echo base_url()?>index.php/pal/pali29/get_pali29",
		dataType: "json",
	...
	.done(function( msg ) {
		...
		for(var key in msg.data){
			...
			append_str += "<br> <span class='mt006' >"+val['mt006']+"</span> ";
			console.log(val['mt006']);
		}
				}); 
				} */
			//	append_str += '<br>休假: ';
...
function view_detail_of_day(current_day){
	...
	.done(function( msg ) {
		...
		for(var key in msg.data){
			...
			append_str += "人事 : <span class='mt001'>"+val['mt001']+"."+val['mt001disp']+"</span> <br>"
	...
	if(msg.response == true){
		...
		for(var key in msg.data){
			...
			append_str += "</div>";
			append_str += "<div class='work_block' >";
			append_str += "人事 : <span>"+val['mo001']+"."+val['mo002']+"</span> <br>";
...
function add_event(mt001,mt003,mt004){
	...
	$('#divApali29 iframe').attr('src', '<?php echo base_url()?>index.php/pal/pali29/display_add?mt001='+mt001+"&mt003="+mt003+"&mt004="+mt004);
	$.blockUI({
		css: {
			top: '5%',
			left: '20%',
...
function edit_event(mt001,mt002,mt003){
	...
	$('#divEpali29 iframe').attr('src', '<?php echo base_url()?>index.php/pal/pali29/display_upd?mt001='+mt001+"&mt002="+mt002+"&mt003="+mt003);
	$.blockUI({
		css: {
			top: '10%',
			left: '15%',
...
function del_event(mt001,mt002,mt003,mt001disp,mt002disp){
	if(confirm("請問是否確定要刪除資料:"+mt001+"."+mt001disp+","+mt002+"."+mt002disp+","+mt003)){
		$.ajax({
			method: "POST",
			url: "<?php echo base_url()?>index.php/pal/pali29/ajax_del",
			dataType: "json",
			data: { 
				mt001:mt001,
				mt002:mt002,
