     ...
     <!--  <div class="buttons"> -->
	   <div style="float:left; ">
		...
		<?php if($this->uri->segment(3)=="display_leave"){ ?>
			...
			<?PHP } } ?>
	    <?PHP if (substr($this->session->userdata('sysmg006'),1,1)=='Y') { ?>
	  <a onclick="location = '<?php echo base_url()?>index.php/pal/pali01/display'"  style="float:left" accesskey="d" class="button"><span>重整 a </span><img height="12" width="12" src="<?php echo base_url()?>assets/image/delete2.png" /></a>	
      <?PHP } ?>
        ...
        <tr>                          <!-- 表格表頭 -->
		  ...
		  <?php
			foreach($title_array as $key => $val){
				...
				echo anchor("pal/pali01/display_search/".$this->uri->segment(4,0)."/order?val=".$val['sort_name']." asc",$str);
				
				$str = "<img src='".base_url()."assets/image/desc.png' />";
				echo anchor("pal/pali01/display_search/".$this->uri->segment(4,0)."/order?val=".$val['sort_name']." desc",$str);
...
<?php 
	$filter_array = array(
		...
		'mv001' => array('filter_name'=>"mv001",'name'=>"員工代號",'size'=>"10",'align'=>"left"),
		'mv002' => array('filter_name'=>"mv002",'name'=>"員工姓名",'size'=>"10",'align'=>"left",'color'=>"#F5F5F5"),
		'mv004' => array('filter_name'=>"mv004",'name'=>"部門別",'size'=>"10",'align'=>"left"),
		'mv006' => array('filter_name'=>"mv006",'name'=>"職務",'size'=>"10",'align'=>"left"),
		'mv021' => array('filter_name'=>"mv021",'name'=>"到職日",'size'=>"10",'align'=>"left"),
		'mv022' => array('filter_name'=>"mv022",'name'=>"離職日",'size'=>"10",'align'=>"left"),
		'mv015' => array('filter_name'=>"mv015",'name'=>"電話",'size'=>"10",'align'=>"left")
	);
?>
...
function filter() {
	...
	url = '<?php echo base_url() ?>index.php/pal/pali01/display_search/0/and_where?key=' + encodeURIComponent(key) + 
...
function filtera() {
	...
	url = '<?php echo base_url() ?>index.php/pal/pali01/display_search/0/or_where?key=' + encodeURIComponent(key) + 
	'&val=' + encodeURIComponent(val);
	location = url;
}
</script>