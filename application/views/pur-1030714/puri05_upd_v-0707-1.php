...
<div id="content">  
 <div class="box">
	...
	<?php 
	  ...
	  //開視窗及不更新網頁直接輸入出現中文	 
	  if($this->uri->segment(4) ) { $ta001=urldecode(urldecode($this->uri->segment(4)));  $this->session->set_userdata('ta001', $ta001);$ta001 = $this->session->userdata('ta001'); }
	  if($this->uri->segment(5) ) { $ta001disp=urldecode(urldecode($this->uri->segment(5))); $this->session->set_userdata('ta001disp', $ta001disp); $ta001disp = $this->session->userdata('ta001disp');} 
	 ...
	 <table class="form12"  >     <!-- 頭部表格 -->
	  <tr>
	     ...
	     <td class="start12a"  width="10%"><span class="required">請購單別：</span> </td>
            <td class="normal12a"  width="50%"><input tabIndex="1" readonly="value" id="ta001"    onKeyPress="keyFunction()"  onchange="startdata1(this)"  name="ta001" value="<?php echo strtoupper($ta001); ?>"  type="text"  style="background-color:#EBEBE4" required /><a href="javascript:;"><img id="Showpurq04a31" src="<?=base_url()?>assets/image/button-search.png" alt="" align="top"/></a>
	  ...
	  <tr>
	    <td class="start12a" ><span class="required">講購單號：</span> </td>
            <td class="normal12a" ><input type="text" tabIndex="2" readonly="value" id="ta002" onKeyPress="keyFunction()"  name="ta002"  value="<?php echo $ta002; ?>" size="30"  style="background-color:#EBEBE4" required /><span id="ta002disp" ></span></td>
	  ...
	  <tr>
	    <td  class="normal12" >單據日期：</td>
            <td  class="normal12"  ><input tabIndex="3"  class="date" id="ta013" onKeyPress="keyFunction()"   name="ta013"  value="<?php echo substr($ta013,0,4).'/'.substr($ta013,4,2).'/'.substr($ta013,6,2); ?>"  size="10" type="text"  /></td>
	  ...
	  <div>
		...
		<tbody id="product-row0">		
	     ...
	     <td class="center"><img src="<?=base_url()?>assets/image/delete.png" title="移除" onclick="$(\'#product-row0\').remove();" /></td>
  	     <input type="hidden" readonly="value" name="order_product[0][tb001]" value="<?php echo $tb001[$i]; ?>" />
	     <input type="hidden" readonly="value" name="order_product[0][tb002]" value="<?php echo $tb002[$i]; ?>" />	
	     <input type="hidden" readonly="value"  name="order_product[0][tb003]" value="<?php echo $tb003[$i]; ?>"  />
		...
		function(req, add){ 
            ...
            select:   
               function(event, ui) { 
                 ...
                 $('input[name=\'order_product['0'][tb004]\']').val(ui.item.value1);				   
			     $('input[name=\'order_product['0'][tb005]\']').val(ui.item.value2);
			     $('input[name=\'order_product['0'][tb006]\']').val(ui.item.value3);
			     $('input[name=\'order_product['0'][tb007]\']').val(ui.item.value4);
	...
	$('input[name=\'order_product[0][tb009]\'],input[name=\'order_product[0][tb017]\']').focusout(function() { 
		var n = this.name.replace(/order_product\[(\d+)].*/, '$1'); 
		var input_1=$('input[name=\'order_product['0'][tb009]\']').val()*1;  
		var input_2=$('input[name=\'order_product['0'][tb017]\']').val()*1;  
		var get_total=input_1*input_2;  
		$('input[name=\'order_product['0'][tb018]\']').val(get_total); 
		alert(get_total);
		//流水號
	//	var num_1 = 1000;
...
//--></script>
		...
		<tbody id="product-row1">		
	     ...
	     <td class="center"><img src="<?=base_url()?>assets/image/delete.png" title="移除" onclick="$(\'#product-row1\').remove();" /></td>
  	     <input type="hidden"  readonly="value" name="order_product[1][tb001]" value="<?php echo $tb001[$i]; ?>" />
	     <input type="hidden"  readonly="value" name="order_product[1][tb002]" value="<?php echo $tb002[$i]; ?>" />	
	     <input type="hidden"  readonly="value" name="order_product[1][tb003]" value="<?php echo $tb003[$i]; ?>"  />
	...
	_renderMenu: function(ul, items) {
		...
		function(req, add){ 	
            ...
            select:   
               function(event, ui) { 
			     var n = this.name.replace(/order_product\[(\d+)].*/, '$1');
                 $('input[name=\'order_product['1'][tb004]\']').val(ui.item.value1);				   
			     $('input[name=\'order_product['1'][tb005]\']').val(ui.item.value2);
			     $('input[name=\'order_product['1'][tb006]\']').val(ui.item.value3);
			     $('input[name=\'order_product['1'][tb007]\']').val(ui.item.value4);
	...
	$('input[name=\'order_product[1][tb009]\'],input[name=\'order_product[1][tb017]\']').focusout(function() { 
		var 1 = this.name.replace(/order_product\[(\d+)].*/, '$1'); 
		var input_1=$('input[name=\'order_product['1'][tb009]\']').val()*1;  
		var input_2=$('input[name=\'order_product['1'][tb017]\']').val()*1;  
		var get_total=input_1*input_2;  
		$('input[name=\'order_product['1'][tb018]\']').val(get_total); 
	...
	$('input[name=\'order_product[1][tb009]\']').focus(function(){
		var real_value = $(this)[1].defaultValue;
		if ($(this).val()==real_value)
		     $(this).val(real_value);
