...
<script type="text/javascript"> 	   
	...
	<div id="divFcopq03a24" style="display:none"> 	   
	<div style="float:right;"><input type="button" class="close" value="close" /></div> 	   
	<iframe src="<?php echo base_url()?>index.php/fun/copq03a/display4" allowTransparency="flase" name="ifmain" width="640" height="480" marginwidth="0" marginheight="0" frameborder="0"></iframe> 	   
	</div> 	  
<script type="text/javascript"><!--
  function addcopq03a24(sta001,sta002) {	
	form.copq03a24.value=sta001;
	var oSpan = document.getElementById("copq03a24disp");
		oSpan.innerHTML = sta002;
	document.form.copq03a24.focus();    
	return copq03a24;	
}
//--></script>
<!-- 開視窗 invq81a 單位 -->	
...
<script type="text/javascript"><!--
  function addinvq81a(stc001,stc002,stc003) {
	...
	$(".total_dc").each(function() {		
	 ...
	 $('input[name=\'order_product[' + product_row1  + '][tj008]\']').val(stc002);
	return $("#tj008").html(stc002);
...
<script type="text/javascript"><!--
  function addcopc08a(sma001,sma002) {
	...
	url = '<?=base_url() ?>index.php/cop/copi09/copybefore/'+encodeURIComponent(sma001)+'/'+encodeURIComponent(sma002); 
	location = url;
	return true;
}
//--></script>
...
function showcopq03a24(sText){                 //不更新網頁 32  銷退單別 cop/copi0501
	var oSpan = document.getElementById("copq03a24disp");
      //   chkno1();
 ...
 <!-- 不更新網頁帶出資料 copq03a21 銷退單別 -->        
function startcopq03a24(oInput){            
  //	if(!oInput.value){
...
//	}
	...
	createXMLHttpRequest();
   	...
   	var sUrl = "<?php echo base_url()?>index.php/fun/copq03a/datacopq03a24/" + encodeURIComponent(oInput.value)+ "/" + new Date().getTime();   
	...
	xmlHttp.onreadystatechange = function(){	    
		if(xmlHttp.readyState == 4 && xmlHttp.status == 200)					
		  showcopq03a24(xmlHttp.responseText);	//顯示服務器結果
...
function showchkno1(sText){   
       ...
       if  (!sText) { sText=$('input[name=\'ti034\']').val();   
...
//	}
	//建立非同步請求
	 ...
	 var zno=$('input[name=\'copq03a24\']').val();
	 var zstr=$('input[name=\'ti034\']').val();
	...
	createXMLHttpRequest();
   	var sUrl = "<?php echo base_url()?>index.php/cop/copi09/datachkno1/" + encodeURIComponent(zno)+ "/" + encodeURIComponent(zymd)+ "/" + new Date().getTime();   
	var QueryString = encodeURIComponent(oInput.value);
...
function showcopq02a(sText,n){   //不更新網頁 6  單價 
	...
	// alert(sText);	
	 if (sText>0){
	 $('input[name=\'order_product[' + nn + '][tj011]\']').val(sText)};
}
...
//	}
      ...
      var td010=$('input[name=\'order_product[' + nn + '][tj008]\']').val();  //單位
...
function showcmsq03a(sText,n){   //不更新網頁 6  庫別 
	...
	// alert(sysma200);
	 if (num>sysma200){
          $('input[name=\'order_product[' + nn + '][tj013disp]\']').val(sText); }  
}
...
<script type="text/javascript"><!--       
 function selappr(){
	//首先判斷是否有輸入，沒有輸入直接返回，並提示
	  var selval = document.getElementById('ti019').selectedIndex;
	 //  alert(selval);
...
<script type="text/javascript"><!--       
 function seltax(){
	//首先判斷是否有輸入，沒有輸入直接返回，並提示
	  var selval = document.getElementById('ti036').selectedIndex;
	 //  alert(selval);
	   var oSpan = document.getElementById("taxdisp");
	  if (selval==0) {form.ti044.value=0.05;oSpan.innerHTML = "<span style='color:red'> 應稅外加</span>";}	    
      else if (selval==1){  form.ti044.value=0.05;oSpan.innerHTML = "<span style='color:red'> 應稅內含</span>";}
	  else {  form.ti044.value=0;oSpan.innerHTML = "<span style='color:red'> 不計稅</span>";}
}
...
<script type="text/javascript"><!--       
 function seleall(oInput){
	//首先判斷是否有輸入，沒有輸入直接返回，並提示	
           document.getElementById('ti034').select();		   
}
//--></script>
...
<script type="text/javascript"><!--        
 function dataymd1(oInput){
	//首先判斷是否有輸入，沒有輸入直接返回，並提示
		   document.form.ti034.value=oInput.value.substring(0,4)+'/'+oInput.value.substring(4,6)+'/'+oInput.value.substring(6,8); 
		   var zstr=$('input[name=\'ti034\']').val();		 
		if  ( zstr.length != 10) { 
		    document.form.ti034.focus(); return ti034;}	
}
//--></script>
 <!-- 不更新網頁 下一個定位的索引 --> 	
<script type="text/javascript"> 
...
<script type="text/javascript"><!--    // 明細 新增 15 因前置單据
var product_row = 0; 
var vtj0= -1;
...
//	html += '    <td class="left"><input type="text"  tabIndex="20"  name="order_product[' + product_row + '][tj003]" value="0" size="6" /></td>';
	html += '    <td class="left"><input type="text"  tabIndex="61"  class="total_dc" onchange="startinvq02a(this,product_row)" id="tj004'+ product_row+'"   name="order_product[' + product_row + '][tj004]" value="" size="20" style="background-color:#E7EFEF"  /></td>';	
	html += '    <td class="left"><input readonly="value"  tabIndex="62"  onKeyPress="keyFunction()" type="text" id="tj005"  name="order_product[' + product_row + '][tj005]" value=""  style="background-color:#EBEBE4" /></td>';
	html += '    <td class="left"><input readonly="value" tabIndex="63" onKeyPress="keyFunction()" type="text" id="tj006"   name="order_product[' + product_row + '][tj006]" value=""  size="30" style="background-color:#EBEBE4" /></td>';
	html += '    <td class="left"><input  tabIndex="64" ondblclick="startinvq81a();"  onKeyPress="keyFunction()"  type="text" id="tj008"   name="order_product[' + product_row + '][tj008]" value="" size="6"  /></td>';
	html += '    <td class="left"><input readonly="value" tabIndex="65" onKeyPress="keyFunction()" type="text"  id="tj003" name="order_product[' + product_row + '][tj003]" value="0" size="6" style="background-color:#EBEBE4" /></td>';
	html += '    <td class="left"><input type="text"  tabIndex="66" onchange="startcmsq03a(this,product_row)" id="tj013'+ product_row+'"   name="order_product[' + product_row + '][tj013]" value="" size="10"  /></td>';	
	html += '    <td class="left"><input readonly="value" type="text"  tabIndex="67"  id="tj013disp'+ product_row+'"   name="order_product[' + product_row + '][tj013disp]" value="" size="10" style="background-color:#EBEBE4"  /></td>';
	
	html += '    <td class="center"><input type="text" tabIndex="71" onchange="startcopq02a(this,product_row)"  class="total_qty" id="tj007" onKeyPress="keyFunction()" name="order_product[' + product_row + '][tj007]" value="0" size="10" style="text-align:right" style="background-color:#E7EFEF" /></td>';
    html += '    <td class="center"><input type="text" tabIndex="72" id="tj011" onKeyPress="keyFunction()" name="order_product[' + product_row + '][tj011]" value="0" size="10" style="text-align:right" /></td>';	
    html += '    <td class="right"><input readonly="value" tabIndex="73" type="text"  name="order_product[' + product_row + '][tj012]" value="0" size="10" style="text-align:right;background-color:#EBEBE4" /></td>';
	html += '    <td class="right"><input readonly="value"  tabIndex="74" type="text" class="total_price"  name="order_product[' + product_row + '][tj031]" value="0" size="10" style="text-align:right;background-color:#EBEBE4" /></td>';
	html += '    <td class="center"><input readonly="value" tabIndex="75" type="text"  id="tj036" onKeyPress="keyFunction()" name="order_product[' + product_row + '][tj032]" value="0" size="10" style="text-align:right;background-color:#EBEBE4"/></td>';	
    html += '    <td class="right"><input readonly="value" tabIndex="76" type="text" class="total_price1"  name="order_product[' + product_row + '][tj033]" value="0" size="10" style="text-align:right;background-color:#EBEBE4" /></td>';
	html += '    <td class="center"><input type="text" readonly="value" tabIndex="77"  id="tj038" onKeyPress="keyFunction()" name="order_product[' + product_row + '][tj034]" value="0" size="10" style="text-align:right;background-color:#EBEBE4" /></td>';
	
	html += '    <td class="left"><input type="text"  tabIndex="78" id="tj015" onKeyPress="keyFunction()" name="order_product[' + product_row + '][tj015]" value="" size="10"  /></td>';
	html += '    <td class="left"><input type="text"  tabIndex="79" id="tj016" onKeyPress="keyFunction()" name="order_product[' + product_row + '][tj016]" value="" size="10"  /></td>';
	html += '    <td class="left"><input type="text"  tabIndex="80" id="tj017" onKeyPress="keyFunction()" name="order_product[' + product_row + '][tj017]" value="" size="10"  /></td>';
	html += '    <td class="left"><input type="text"  tabIndex="81" id="tj018" onKeyPress="keyFunction()" name="order_product[' + product_row + '][tj018]" value="" size="10"  /></td>';
	html += '    <td class="left"><input type="text"  tabIndex="82" id="tj019" onKeyPress="keyFunction()" name="order_product[' + product_row + '][tj019]" value="" size="10"  /></td>';
	html += '    <td class="left"><input type="text"  tabIndex="83" id="tj020" onKeyPress="keyFunction()" name="order_product[' + product_row + '][tj020]" value="" size="10"  /></td>';
	
	html += '    <td class="left"><select  tabIndex="84" id="tj030" onKeyPress="keyFunction()" name="order_product[' + product_row + '][tj030]" ><option  value="1">1.銷退</option><option value="2">2.折讓</option></select></td>';
	html += '    <td class="left"><input type="text"  tabIndex="85" id="tj014" onKeyPress="keyFunction()" name="order_product[' + product_row + '][tj014]" value="" size="10"  /></td>';
	html += '    <td class="left"><input type="text"  tabIndex="86" id="tj029" onKeyPress="keyFunction()" name="order_product[' + product_row + '][tj029]" value="" size="10"  /></td>';
	html += '    <td class="left"><input type="text"  tabIndex="87" id="tj028" onKeyPress="keyFunction()" name="order_product[' + product_row + '][tj028]" value="" size="10"  /></td>';

	html += '    <td class="left"><input type="text" id="tj023" tabIndex="88"  onKeyPress="keyFunction()" name="order_product[' + product_row + '][tj023]" value="" size="20" /></td>';
	html += '  </tr>';	
    html += '</tbody>';
    ...
    $('input[name=\'order_product[' + product_row + '][tj013]\']').catcomplete({
        autoFocus: true,
		delay: 0,
        minLength: 1,  		
		source: 
		function(req, add){ 	
          ...
          //  var smb001=document.getElementById('tj007').value;
			   vtj0= product_row - 1;
	...
	$('input[name=\'order_product[' + product_row + '][tj007]\'],input[name=\'order_product[' + product_row + '][tj011]\']').focusout(function() { 
		var n = this.name.replace(/order_product\[(\d+)].*/, '$1'); 
	...
	//訂單單號
	$('input[name=\'order_product[' + product_row + '][tj015]\']').focus(function(){
		totalSum1();
	});
...
function totalSum1() {
    ...
    $("#sum_total").html(sumTotal.toFixed(1));
  	...
  	form.ti010.value=Math.round(sumTotal);	  //原幣貨款
	 var tax=$('input[name=\'ti036\']').val();  //稅率
	 var rate=$('input[name=\'ti009\']').val();  //匯率
	form.ti011.value=Math.round(sumTotal*tax);  //原幣稅額
	
	form.ti037.value=Math.round(sumTotal1*rate);	  //本幣貨款
	form.ti038.value=Math.round(sumTotal1*rate*tax);  //本幣稅額
	var sumTax =Math.round(sumTotal*tax);          //原幣稅額
	var sumTax1 =Math.round(sumTotal1*rate*tax);   //本幣稅額
	//課稅別
	//if ($('select[name=\'ti017\']').val()=='1') {form.ti013.value=Math.round(sumTotal-sumTax);form.ti025.value=Math.round(sumTax*1);}
	//if ($('select[name=\'ti017\']').val()=='1') {form.ti045.value=Math.round(sumTotal1-sumTax1);form.ti046.value=Math.round(sumTax1*1);}
	...
	form.ti029.value=Math.round(sumQty);	
}
//--></script>
 <!-- 明細 品號開視窗   -->  