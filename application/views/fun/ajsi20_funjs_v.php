...
<script type="text/javascript"> 	   
  function startcmsq12a() {
	...
	<div id="divFcmsq12a" style="display:none"> 	   
	<div style="float:right;"><input type="button" class="close" value="close" /></div> 	   
	<iframe src="<?php echo base_url()?>index.php/fun/cmsq12a/display" allowTransparency="flase" name="ifmain" width="640" height="480" marginwidth="0" marginheight="0" frameborder="0"></iframe> 	   
	</div> 

<script type="text/javascript"><!--
  function addcmsq12a(stc001,stc002) {
	var product_row1 = 0; 
	$(".total_dc").each(function() {		
		product_row1++;
    });
	 product_row1=product_row1-1;
	 $('input[name=\'order_product[' + product_row1  + '][tb010]\']').val(stc002);
	return $("#tb010").html(stc002);
}
//--></script>
...
function showactq02a(sText){                 //不更新網頁 actq02a  傳票單別 tc001
	var oSpan = document.getElementById("actq02adisp");
 ...
 <!-- 不更新網頁帶出資料 actq02a 傳票單別 -->        
function startactq02a(oInput){            
  	if(!oInput.value){
		oInput.focus();    //聚焦到用戶名的輸入框
		$("#actq02adisp").html("<span style='color:red'>不可空白.</span>");			
		return;
	}
	//建立非同步請求
	createXMLHttpRequest();
   
   	var sUrl = "<?php echo base_url()?>index.php/fun/actq02a/dataactq02a/" + encodeURIComponent(oInput.value)+ "/" + new Date().getTime();   
...
function showactq03a(sText){   //不更新網頁 actq03a 客戶代號 
	var oSpan = document.getElementById("actq03adisp");
	    oSpan.innerHTML = "<span style='color:black'>"+sText+"</span>"; 	
	 if (!sText) { 
	    oSpan.innerHTML = "<span style='color:red'>無此資料!</span>"; 	
...
function startactq03a(oInput){         //不更新網頁actq03a 客戶代號
	createXMLHttpRequest();
...
function showcmsq02a(sText){   //不更新網頁 cmsq02a 廠別
	var oSpan = document.getElementById("cmsq02adisp");
	 oSpan.innerHTML = "<span style='color:black'>"+sText+"</span>"; 	
	 if (!sText) { 
	    oSpan.innerHTML = "<span style='color:red'>無此資料!</span>"; 	
...
<!-- 不更新網頁,收款業務人員 -->
function showcmsq09a31(sText){   //不更新網頁 cmsq09a31  業務人員
	var oSpan = document.getElementById("cmsq09a31disp");
	    oSpan.innerHTML = "<span style='color:black'>"+sText+"</span>"; 	
	 if (!sText) { 
	    oSpan.innerHTML = "<span style='color:red'>無此資料!</span>"; 	
...
<script type="text/javascript"><!--       
 function dataapp(oInput){
	//首先判斷是否有輸入，沒有輸入直接返回，並提示
		   document.form.ta003.value=oInput.value.substring(0,4)+'/'+oInput.value.substring(4,6)+'/'+oInput.value.substring(6,8); 
		   var zstr=$('input[name=\'ta003\']').val();
		if  ( zstr.length != 10) { 
		    document.form.ta003.focus(); return ta003;}	
}
...
<script type="text/javascript"><!--       
 function selappr(){
	//首先判斷是否有輸入，沒有輸入直接返回，並提示
	  var selval = document.getElementById('ta010').selectedIndex;
	 //  alert(selval);
	   var oSpan = document.getElementById("approved");
		...
		else { oSpan.innerHTML = "<span style='color:black'> 作廢</span>";}
}
//--></script>
...
<script type="text/javascript"><!--       
 function seleall(oInput){
	//首先判斷是否有輸入，沒有輸入直接返回，並提示	
           document.getElementById('ta003').select();		   
}
//--></script>
...
//	html += '    <td class="left"><input type="text"  tabIndex="20"  name="order_product[' + product_row + '][tb003]" value="0" size="6" /></td>';
	...
	html += '    <td class="left"><input type="text"  tabIndex="52" id="tb005'+ product_row+'"   name="order_product[' + product_row + '][tb005]" value="" size="12" style="background-color:#E7EFEF" /></td>';
	...
	html += '    <td class="left"><input tabIndex="56"  onKeyPress="keyFunction()"  type="text" id="tb006disp"   name="order_product[' + product_row + '][tb006disp]" value="" size="12" style="background-color:#EBEBE4" /></td>';
     ...
     html += '    <td class="right"><input  type="text" tabIndex="59"  id="tb014" name="order_product[' + product_row + '][tb014]" value="1" size="10" style="text-align:right" /></td>';
	...
	html += '    <td class="left"><input type="text" id="tb010" tabIndex="62" ondblclick="startcmsq12a();" class="Showcmsq12a"  onKeyPress="keyFunction()" name="order_product[' + product_row + '][tb010]" value="" size="30" /></td>';
...
function totalSum1() {
  ...
  //	form.ta007.value=Math.round(sumTotal-sumTotal2);	  //本幣借方金額
