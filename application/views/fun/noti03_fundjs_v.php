...
function del_detail(tb004,row){
	...
	if(tb004 != ta008){
		...
		if(confirm("確定刪除細項:"+ta001+"-"+tb004+"-"+ta008+"?")){
		$.ajax({
			method: "POST",
...
//刪除一筆明細 (noti03退票)
function del_detail2(tb004,row){
	var ta008 = $('#ta008').val();
	if(tb004 != ta008){
		alert('刪除的項目與目前票況不符!');
	}else{
		$("#product_row_"+row+"").remove();
		current_count --;
		temp = $('#order_product\\['+current_count+'\\]\\[tb004\\]').val();
		$('#ta008').val(temp);
				
		$('#Shownoti03adisp').attr('class','button');
		$('#Shownoti03adisp2').css('display','');		
				
		$('#Shownoti03bdisp').attr('class','button');
		$('#Shownoti03bdisp2').css('display','');
		//$('#tc025').val('');	
		//$('#tc025disp').val('');		
		//$('#tc025disp2').val('');		
    };
...
//刪除一筆明細 (noti03兌現)
function del_detail3(tb004,row){
	var ta008 = $('#ta008').val();
	if(tb004 != ta008){
		alert('刪除的項目與目前票況不符!');
	}else{
...
//新增一筆明細 (noti03退票新增)
function addItem2(smb001,smb002){
	//current_count = 0;
	...
	append_str += "<tr>";
	append_str += "<td class='center'><img src='<?php echo base_url()?>assets/image/delete2.png' title='刪除資料' onclick='del_detail2(\""+'5:退票\"'+',\"'+current_count+"\");' /></td>";
...
//新增一筆明細 (noti03兌現新增)
function addItem3(smb001,smb002){
	//current_count = 0;
	var max_no = get_max_no();
	...
	append_str += "<tbody id='product_row_"+current_count+"' class='product_row' >";
	append_str += "<tr>";
	append_str += "<td class='center'><img src='<?php echo base_url()?>assets/image/delete2.png' title='刪除資料' onclick='del_detail3(\""+'6:兌現\"'+',\"'+current_count+"\");' /></td>";
		...
		if(type == "text"){
			switch(key){
				case 'tb003':
					...
					break;
				case 'tb004':
					append_str += "<input type='"+type+"' value='6:兌現'  id='order_product["+current_count+"]["+key+"]' name='order_product["+current_count+"]["+key+"]' class='order_product_"+key+"'  onKeyPress='keyFunction()' ";
					break;
				case 'tb005':
					append_str += "<input type='"+type+"' value='"+smb002+"'  id='order_product["+current_count+"]["+key+"]' name='order_product["+current_count+"]["+key+"]' class='order_product_"+key+"'  onKeyPress='keyFunction()' ";
					break;
