...
<script type="text/javascript"><!--
  function addinvq81a(stc001,stc002,stc003) {
	...
	$(".total_dc").each(function() {		
	 ...
	 $('input[name=\'order_product[' + product_row1  + '][th008]\']').val(stc002);
	return $("#th008").html(stc002);
...
<script type="text/javascript"><!--
  function addpurc09a(sma001,sma002) {
	...
	url = '<?=base_url() ?>index.php/pur/puri09/copybefore/'+encodeURIComponent(sma001)+'/'+encodeURIComponent(sma002); 
	location = url;
	return true;
}
//--></script>
...
function showpurq04a34(sText){                 //不更新網頁 31  請購單別 ta001
	var oSpan = document.getElementById("purq04a34disp");
      //   chkno1();
     //   alert('test');		
...
//	}
	//建立非同步請求
	 ...
	 var zno=$('input[name=\'purq04a34\']').val();
	...
	createXMLHttpRequest();
   	var sUrl = "<?php echo base_url()?>index.php/pur/puri09/datachkno1/" + encodeURIComponent(zno)+ "/" + encodeURIComponent(zymd)+ "/" + new Date().getTime();   
	var QueryString = encodeURIComponent(oInput.value);
...
function showcmsq03a(sText,n){   //不更新網頁 6  庫別 
	...
	// alert(sysma200);
	 if (num>sysma200){
          $('input[name=\'order_product[' + nn + '][th009disp]\']').val(sText); }  
}
...
function showinvq02a(sText,n){   //不更新網頁 6  品號 
	...
	// alert(str[1]);
	 ...
	 $('input[name=\'order_product[' + nn + '][th008]\']').val(str[2]);}
...
function showpurq02a(sText,n){   //不更新網頁 6  單價 
	...
	// alert(sText);	
	 if (sText>0){
	 $('input[name=\'order_product[' + nn + '][th018]\']').val(sText)};
}
...
//	}
    //   alert('test');
      ...
      var td010=$('input[name=\'order_product[' + nn + '][th008]\']').val();  //單位
	//  alert(nn);
...
<script type="text/javascript"><!--       
 function dataymd1(oInput){
	//首先判斷是否有輸入，沒有輸入直接返回，並提示
	       // alert(test);
		   document.form.tg014.value=oInput.value.substring(0,4)+'/'+oInput.value.substring(4,6)+'/'+oInput.value.substring(6,8); 
		   var zstr=$('input[name=\'tg014\']').val();
		 //  alert(zstr.length);
		 
		if  ( zstr.length == 8 ) { 
		    document.form.tg014.focus(); return tg014;}	
}
...
$(document).keydown(function(event){
	...
	if(event.altKey && (keycode == '72')){  //tab5 h
		...
		//跳明細 alt+y 89  銷售及進貨250 其他0
	if(event.altKey && (keycode == '89')){  //tab6 y
		setTimeout(function() {
			$('input[name=\'order_product[250][th004]\']').focus();
		}, 100);	
	}
	//新增一筆明細 alt+w
...
//	html += '    <td class="left"><input type="text"  tabIndex="20"  name="order_product[' + product_row + '][th003]" value="0" size="6" /></td>';
	html += '    <td class="left"><input type="text" class="total_dc" tabIndex="41" id="th004'+ product_row+'"  onchange="startinvq02a(this,product_row)"  name="order_product[' + product_row + '][th004]" value="" size="20" style="background-color:#E7EFEF" /></td>';	
	html += '    <td class="left"><input readonly="value"    onKeyPress="keyFunction()" type="text" id="th005"  name="order_product[' + product_row + '][th005]" value=""  style="background-color:#EBEBE4" /></td>';
	html += '    <td class="left"><input readonly="value"  onKeyPress="keyFunction()" type="text" id="th006"   name="order_product[' + product_row + '][th006]" value=""  size="30" style="background-color:#EBEBE4" /></td>';
	html += '    <td class="left"><input  onKeyPress="keyFunction()" ondblclick="startinvq81a();" type="text" id="th008"   name="order_product[' + product_row + '][th008]" value="" size="6"  /></td>';
	html += '    <td class="left"><input readonly="value" onKeyPress="keyFunction()" type="text"  id="th003" name="order_product[' + product_row + '][th003]" value="0" size="6" style="background-color:#EBEBE4" /></td>';
	html += '    <td class="left"><input type="text"  tabIndex="43" id="th009'+ product_row+'"  onchange="startcmsq03a(this,product_row)"   name="order_product[' + product_row + '][th009]" value="" size="10" style="background-color:#E7EFEF" /></td>';	
	html += '    <td class="left"><input readonly="value" type="text"   id="th009disp'+ product_row+'"   name="order_product[' + product_row + '][th009disp]" value="" size="10" style="background-color:#EBEBE4"  /></td>';	
	html += '    <td class="left"><input type="text"  tabIndex="45"  ondblclick="scwShow(this,event);"   id="th014" onKeyPress="keyFunction()" name="order_product[' + product_row + '][th014]" value="<?php echo date("Y/m/d"); ?>" size="10"  style="background-color:#E7EFEF"/></td>';

	html += '    <td class="center"><input type="text" tabIndex="46"  id="th007" onKeyPress="keyFunction()" onchange="pricepurq02a(this,product_row)" name="order_product[' + product_row + '][th007]" value="0" size="10" style="text-align:right" /></td>';
	html += '    <td class="center"><input type="text" tabIndex="47"  id="th015" onKeyPress="keyFunction()" name="order_product[' + product_row + '][th015]" value="0" size="10" style="text-align:right" /></td>';
	html += '    <td class="center"><input type="text" tabIndex="48"  id="th017" onKeyPress="keyFunction()" name="order_product[' + product_row + '][th017]" value="0" size="10" style="text-align:right" /></td>';
	html += '    <td class="center"><input type="text" tabIndex="49" class="total_qty" id="th016" onKeyPress="keyFunction()" name="order_product[' + product_row + '][th016]" value="0" size="10" style="text-align:right" /></td>';
    html += '    <td class="center"><input type="text" tabIndex="50" id="th018" onKeyPress="keyFunction()" name="order_product[' + product_row + '][th018]" value="0" size="10" style="text-align:right" /></td>';	
    html += '    <td class="right"><input readonly="value" type="text" class="total_price"  name="order_product[' + product_row + '][th045]" value="0" size="10" style="text-align:right;background-color:#EBEBE4" /></td>';
	html += '    <td class="center"><input readonly="value" type="text"  id="th046" onKeyPress="keyFunction()" name="order_product[' + product_row + '][th046]" value="0" size="10" style="text-align:right;background-color:#EBEBE4"/></td>';	
    html += '    <td class="right"><input readonly="value"  type="text" class="total_price1"  name="order_product[' + product_row + '][th047]" value="0" size="10" style="text-align:right;background-color:#EBEBE4" /></td>';
	html += '    <td class="center"><input type="text" readonly="value"   id="th048" onKeyPress="keyFunction()" name="order_product[' + product_row + '][th048]" value="0" size="10" style="text-align:right;background-color:#EBEBE4" /></td>';
	
	html += '    <td class="left"><input type="text"  tabIndex="55" id="th011" onKeyPress="keyFunction()" name="order_product[' + product_row + '][th011]" value="" size="10"  /></td>';
	html += '    <td class="left"><input type="text"  tabIndex="56" id="th012" onKeyPress="keyFunction()" name="order_product[' + product_row + '][th012]" value="" size="10"  /></td>';
	html += '    <td class="left"><input type="text"  tabIndex="57" id="th013" onKeyPress="keyFunction()" name="order_product[' + product_row + '][th013]" value="" size="10"  /></td>';
	//html += '    <td class="left"><input type="text"  tabIndex="58" id="th028" onKeyPress="keyFunction()" name="order_product[' + product_row + '][th028]" value="" size="10"  /></td>';
//	html += '    <td class="left"><input type="text"  tabIndex="58" id="th028" onKeyPress="keyFunction()" name="order_product[' + product_row + '][th028]" value="" size="10"  /></td>';
	html += '    <td class="left"><select id="th028" tabIndex="58" onKeyPress="keyFunction()" name="order_product[' + product_row + '][th028]" ><option  value="0">0.免檢</option><option value="1">1.待驗</option><option  value="2">2.合格</option><option  value="3">3.不良</option><option  value="4">4.特採</option></select></td>'; 
		
	html += '    <td class="left"><input type="text" id="th033" tabIndex="59"  onKeyPress="keyFunction()" name="order_product[' + product_row + '][th033]" value="" size="20" /></td>';
	html += '  </tr>';	
    html += '</tbody>';
    ...
    $('input[name=\'order_product[' + product_row + '][th009]\']').catcomplete({
        autoFocus: true,
		delay: 0,
        minLength: 1,  		
		source: 
	...
	$('input[name=\'order_product[' + product_row + '][th016]\'],input[name=\'order_product[' + product_row + '][th018]\']').focusout(function() { 
		var n = this.name.replace(/order_product\[(\d+)].*/, '$1'); 
	//    var n = vproduct_row;
     ...
     //稅額
	   ...
	   if ($('select[name=\'tg010\']').val()=='1') {$('input[name=\'order_product[' + n + '][th048]\']').val(amt3);}
		...
		var num_1 = 1000;
		var n = this.name.replace(/order_product\[(\d+)].*/, '$1'); 
	 //	if ($('input[name=\'order_product[' + n + '][th003]\']').val() == '0' ) { num_1 = 1000; } else { num_1 = 0; }
	 ...
	 //	if ( num_2 >= 2000 ) { num_2=num_2-1000;$('input[name=\'order_product[' + n + '][th003]\']').val(num_2)=$('input[name=\'order_product[' + n + '][th003]\']').val(num_2)-1000; }
	 //	alert(num_2);
	...
	//進貨數量
	$('input[name=\'order_product[' + product_row + '][th007]\']').focus(function(){
	...
	//進貨數量copy 驗收數量
	$('input[name=\'order_product[' + product_row + '][th015]\']').focus(function(){
	    var n = this.name.replace(/order_product\[(\d+)].*/, '$1'); 
		var input_1=$('input[name=\'order_product[' + n + '][th007]\']').val()*1; 
	     $('input[name=\'order_product[' + n + '][th015]\']').val(input_1); 
	});
	   //計價數量
	$('input[name=\'order_product[' + product_row + '][th016]\']').focus(function(){
		var n = this.name.replace(/order_product\[(\d+)].*/, '$1'); 
	   ...
	   //單價  
	$('input[name=\'order_product[' + product_row + '][th018]\']').focus(function(){
		var real_value = $(this)[0].defaultValue;
		if ($(this).val()==real_value)
		    $(this).val(real_value);
		     if ($(this).val()=='0')
	...
	//採購單別
	$('input[name=\'order_product[' + product_row + '][th011]\']').focus(function(){
		totalSum1();
	});
