...
function showpalq01a(sText){   //不更新網頁 32 員工代號 
    var str=(sText.split(";",4));
	    console.log(sText);
	  $('input[name=\'tf016\']').val(str[3]);
	 
	var oSpan = document.getElementById("palq01adisp");
	 oSpan.innerHTML = "<span style='color:black'>"+str[0]+','+str[1]+','+str[2]+"</span>"; 	
	 if (!sText) { 
...
//	}
	...
	createXMLHttpRequest();
   	//var sUrl = "<?php echo base_url()?>index.php/fun/palq01a/datapalq01a1/" + encodeURIComponent(oInput.value)+ "/" + new Date().getTime();   
	var sUrl = "<?php echo base_url()?>index.php/fun/palq01a/datapalq01a1/" + palq01a + "/" + date;   
	var QueryString = encodeURIComponent(palq01a);
...
<script type="text/javascript"><!--       //檢查 星期 時間
 ...
 function timecal(oInput){         //時間相減  
	...
	if (week == '6' && str6>2 ) {form.tf012.value=2;form.tf013.value=str6-2;form.tf010.value=0;form.tf011.value=0;form.tf014.value=0;form.tf015.value=0;}
	if (week == '6' && str6<=2 ) {form.tf012.value=str6;form.tf013.value=0;form.tf010.value=0;form.tf011.value=0;form.tf014.value=0;form.tf015.value=0;}
