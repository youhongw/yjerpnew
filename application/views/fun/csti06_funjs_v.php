...
function addItem() {
	...
	html += '    <td class="center"><img src="<?php echo base_url()?>assets/image/delete.png" title="移除" onclick="$(\'#product-row' + product_row + '\').remove();" /></td>';
  	html += '    <input type="hidden"  name="order_product[' + product_row + '][md001]" value="" />';
	html += '    <input type="hidden"  name="order_product[' + product_row + '][md014]" value="Y" />';
//	html += '    <td class="left"><input type="text"  tabIndex="13"  name="order_product[' + product_row + '][md002]" value="0" size="6" /></td>';
	html += '    <td class="left"><input type="text"  tabIndex="51" id="md003'+ product_row+'"   name="order_product[' + product_row + '][md003]" value="" size="20"  style="background-color:#E7EFEF"/></td>';	
	html += '    <td class="left"><input readonly="value"   tabIndex="52" onKeyPress="keyFunction()" type="text" id="md003disp"  name="order_product[' + product_row + '][md003disp]" value=""  style="background-color:#EBEBE4"/></td>';
	html += '    <td class="left"><input readonly="value"  tabIndex="53" onKeyPress="keyFunction()" type="text" id="md003disp1"   name="order_product[' + product_row + '][md003disp1]" value=""  size="30" style="background-color:#EBEBE4"/></td>';
	html += '    <td class="left"><input readonly="value"  tabIndex="54"  onKeyPress="keyFunction()"   type="text" id="md003disp2"   name="order_product[' + product_row + '][md003disp2]" value="" size="6" style="background-color:#EBEBE4"/></td>';
	html += '    <td class="left"><input type="text"   readonly="value" tabIndex="52"  name="order_product[' + product_row + '][md002]" value="0" size="6" style="background-color:#EBEBE4" /></td>';
//	html += '    <td class="left"><input type="text"  ondblclick="scwShow(this,event);"  tabIndex="55" id="md011['+ product_row +']" onKeyPress="keyFunction()" name="order_product[' + product_row + '][md011]" value="<?php echo date("Y/m/d"); ?>" size="10"  style="background-color:#E7EFEF"/></td>';
//	html += '    <td class="left"><input type="text"  ondblclick="scwShow(this,event);"  tabIndex="56" id="md012['+ product_row +']" onKeyPress="keyFunction()" name="order_product[' + product_row + '][md012]" value="" size="10"  style="background-color:#E7EFEF"/></td>';
//	html += '    <td class="left"><input type="text" id="md006" tabIndex="57"  onKeyPress="keyFunction()" name="order_product[' + product_row + '][md006]" value="1" size="10" style="text-align:right" /></td>';
//	html += '    <td class="left"><input type="text" id="md006" tabIndex="58"  onKeyPress="keyFunction()" name="order_product[' + product_row + '][md007]" value="1" size="10" style="text-align:right" /></td>';
	html += '    <td class="right"><input type="text" id="md006" tabIndex="59"  onKeyPress="keyFunction()" name="order_product[' + product_row + '][md008]" value="0.00" size="10" style="text-align:right" /></td>';
  //  html += '    <td class="center" ><select id="md014" tabIndex="60" onKeyPress="keyFunction()" name="order_product[' + product_row + '][md014]" ><option  value="Y">Y:計算</option><option value="N">N:不計算</option></select></td>'; 
	// html += '    <td class="center"><select id="md017" tabIndex="61" onKeyPress="keyFunction()" name="order_product[' + product_row + '][md017]" ><option  value="1">1.直接材料</option><option value="2">2.間接材料</option><option  value="3">3.廠商供料</option><option  value="4">4.不發料</option><option  value="5">5.客戶供料</option></select></td>'; 
//	html += '    <td class="left"><input type="text" id="md016" tabIndex="62"  onKeyPress="keyFunction()" name="order_product[' + product_row + '][md016]" value="" size="20" /></td>';
	html += '  </tr>';	
    html += '</tbody>';
	 
	 $('#order_product tfoot').before(html); 
	   	   //下拉視窗 網頁不更新  mb001 md003 品號品名輸入
	
    $('input[name=\'order_product[' + product_row + '][md003]\']').catcomplete({
        autoFocus: true,
		delay: 0,
        minLength: 1,  		
		source: 
		function(req, add){ 	
          ...
          //  var smb001=document.getElementById('md003').value;
			    vtj0= product_row - 1;
		       smb001= $('#md003'+vtj0 ).val();
			
			//   smb001=$("#md003"+(product_row-1)).val();
            ...
            select:   
               function(event, ui) { 
                 ...
                 $('input[name=\'order_product[' + n + '][md003]\']').val(ui.item.value1);				   
			     $('input[name=\'order_product[' + n + '][md003disp]\']').val(ui.item.value2);
			     $('input[name=\'order_product[' + n + '][md003disp1]\']').val(ui.item.value3);
			     $('input[name=\'order_product[' + n + '][md003disp2]\']').val(ui.item.value4);
	...
	$('input[name=\'order_product[' + product_row + '][md011]\'],input[name=\'order_product[' + product_row + '][md006]\']').focusout(function() { 
		var n = this.name.replace(/order_product\[(\d+)].*/, '$1'); 		
		
		//流水號
		var num_1 = 1000;
	 //	if ($('input[name=\'order_product[' + n + '][md002]\']').val() == '0' ) { num_1 = 1000; } else { num_1 = 0; }
		var num_2=(parseInt($('input[name=\'order_product[' + n + '][md002]\']').val())*1)+(n*2)+num_1; 
		if ( num_2 >= 2000 ) { num_2=num_2-1000; }
	    $('input[name=\'order_product[' + n + '][md002]\']').val(num_2); 
	 //	if ( num_2 >= 2000 ) { num_2=num_2-1000;$('input[name=\'order_product[' + n + '][md002]\']').val(num_2)=$('input[name=\'order_product[' + n + '][md002]\']').val(num_2)-1000; }
	 //	alert(num_2);
		
	});
	 
	   //單價  
	$('input[name=\'order_product[' + product_row + '][md006]\']').focus(function(){
		var real_value = $(this)[0].defaultValue;
		if ($(this).val()==real_value)
		    $(this).val(real_value);
			$(this).val('');
	});
 /*	$('input[name=\'order_product[' + product_row + '][md010]\']').focusout(function(){
		var real_value = $(this)[0].defaultValue;
		if ($(this).val()==real_value)
		    $(this).val(real_value);
			//$(this).val('');
	...
	$('input[name=\'order_product[' + product_row + '][md016]\']').blur(function(){
		$('input[name=\'order_product[' + product_row + '][md003]\']').focus();
	});
