...
function addItem() {
	...
	html += '    <td class="left"><input type="text"   tabIndex="18" id="mf003['+ product_row +']" onKeyPress="keyFunction()" name="order_product[' + product_row + '][mf003]" onchange="startnoti13a(this)" ondblclick="noti13a(this,'+ product_row +');" value="" size="20" style="text-align:left" /><span id="shownoti13a"></span></td>';
    html += '    <td class="left"><input type="text"  tabIndex="17" id="mf005'+ product_row +'" onKeyPress="keyFunction()" name="order_product[' + product_row + '][mf005]" size="20"   style="text-align:right;background-color:#F0F0F0" /></td>';
	html += '    <td class="left"><input type="text" class="mf004" tabIndex="18" id="mf004['+ product_row +']" onKeyPress="keyFunction()" name="order_product[' + product_row + '][mf004]" value="" size="20" style="text-align:right" /></td>';
	html += '  </tr>';	
    html += '</tbody>';
	 
	 $('#order_product tfoot').before(html); 
	//明細計算
	$('input[name=\'order_product[' + row + '][mf003]\'],input[name=\'order_product[' + row + '][mf005]\'],input[name=\'order_product[' + row + '][mf004]\']').focusout(function() { 
	...
	$('input[name=\'order_product[' + product_row + '][mf003]\']').blur(function(){
...
<!-- 不更新網頁帶出資料 noti01a 銀行機構 -->
<script type="text/javascript">
function startnoti01a(oInput){ 
	//首先判斷是否有輸入，沒有輸入直接返回，並提示
//	if(!oInput.value){
//		oInput.focus();    //聚焦到用戶名的輸入框
...
//	}
	...
	createXMLHttpRequest();
   	var sUrl = "<?php echo base_url()?>index.php/fun/noti01a/datanoti01a/" + encodeURIComponent(oInput.value)+ "/" + new Date().getTime();   
	var QueryString = encodeURIComponent(oInput.value);
	...
	xmlHttp.onreadystatechange = function(){	    
		if(xmlHttp.readyState == 4 && xmlHttp.status == 200)					
		  shownoti01a(xmlHttp.responseText);	//顯示服務器結果		
	}
...
function shownoti01a(sText){   //不更新網頁 ta012  銀行機構
	var oSpan = document.getElementById("Shownoti01a_str");
	$("#ma002").val(sText);
...
<!-- 不更新網頁帶出資料 cmsi06a 幣別匯率 -->
<script type="text/javascript">
function startcmsi06a(oInput){ 
	//首先判斷是否有輸入，沒有輸入直接返回，並提示
//	if(!oInput.value){
//		oInput.focus();    //聚焦到用戶名的輸入框
//		$("#ta006disp").html("此欄位不可空白.");	
...
//	}
	...
	createXMLHttpRequest();
   	var sUrl = "<?php echo base_url()?>index.php/fun/cmsi06a/datacmsi06a/" + encodeURIComponent(oInput.value)+ "/" + new Date().getTime();   
	var QueryString = encodeURIComponent(oInput.value);
	...
	xmlHttp.onreadystatechange = function(){	    
		if(xmlHttp.readyState == 4 && xmlHttp.status == 200)					
		  showcmsi06a(xmlHttp.responseText);	//顯示服務器結果		
	}
...
function showcmsi06a(sText){   //不更新網頁 ta012  銀行機構
	var oSpan = document.getElementById("Showcmsi06a_str");
	 oSpan.innerHTML = "<span style='color:black'>"+sText+"</span>";
	 if (!sText) { 
	    oSpan.innerHTML = "<span style='color:red'>無此資料!</span>"; 	
	 }	
...
<script type="text/javascript">
	...
	function select_noti01a(no,name){
		$('#me001').val(no);
		$('#ma002').val(name);
	}
</script> 	    	
	...
	<iframe src="<?php echo base_url()?>index.php/fun/noti01a/display" allowTransparency="flase" name="ifmain" width="650" height="480" marginwidth="0" marginheight="0" frameborder="0"></iframe> 	   
	</div>
...
<script type="text/javascript">
	...
	function select_cmsi06a(no,name){
		$('#me002').val(no);
	}
</script> 	    	
		   
	<div id="divFcmsi06a" style="display:none"> 	   
	<div style="float:right;"><input type="button" class="close" value="close" /></div> 	   
	<iframe src="<?php echo base_url()?>index.php/fun/cmsi06a/display" allowTransparency="flase" name="ifmain" width="640" height="480" marginwidth="0" marginheight="0" frameborder="0"></iframe> 	   
	</div> 
...
function totalSum(){
	...
	//投保金額合計
	$(".mf004").each(function(index, element) {
		index1 = index+1;
		sum1 = 0;
		console.log(index1);
    });	
		...
		if(index1 >=1){
			for(i = 1; i<=index1; i++){
				while (typeof($('input[name=\'order_product[' + num1 + '][mf004]\']').val()) == 'undefined'){
					num1++;	
				}
				if(!($('input[name=\'order_product[' + num1 + '][mf004]\']').val())){
					sum1 +=0;
					num1++;
				}else{
					sum1 += parseFloat($('input[name=\'order_product[' + num1 + '][mf004]\']').val());
					num1++;	
				}
			}
		}
