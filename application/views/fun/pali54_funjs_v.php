...
function showpalq01a(sText){   //不更新網頁 32 員工代號 
    var str=(sText.split(";",11));
	 
	  $('input[name=\'cmsq05a\']').val(str[1]);
	  
	  $('input[name=\'tg301\']').val(str[3]);
	  $('input[name=\'tg304\']').val(str[4]);
	  $('input[name=\'tg305\']').val(str[5]);
	  $('input[name=\'tg306\']').val(str[6]);
	  
	   $('input[name=\'tg307\']').val(str[7]);
	  $('input[name=\'tg309\']').val(str[8]);
	  $('input[name=\'tg310\']').val(str[9]);
	  $('input[name=\'tg311\']').val(str[10]);
	var oSpan = document.getElementById("palq01adisp");
	 oSpan.innerHTML = "<span style='color:black'>"+str[0]+"</span>"; 	
	 var oSpan1 = document.getElementById("cmsq05adisp");
	 oSpan1.innerHTML = "<span style='color:black'>"+str[2]+"</span>"; 	
	 if (!sText) { 
...
//	}
	...
	createXMLHttpRequest();
   	var sUrl = "<?php echo base_url()?>index.php/fun/palq01a/datapalq01a2/" + encodeURIComponent(oInput.value)+ "/" + new Date().getTime();   
	var QueryString = encodeURIComponent(oInput.value);
...
//--></script>
<!-- 不更新網頁,檢查欄位  時間是否有誤  -->
<script type="text/javascript"><!--       //檢查 星期 時間
 
 function timeday(oInput){         //星期   
    var str=$('input[name=\'tf002\']').val();
	var d = new Date(str);
	form.tf003.value=d.getDay();
	return form.tf003.value;
 }
 function timecal(oInput){         //時間相減  
    var week=$('input[name=\'tf003\']').val();
    var str9=$('input[name=\'tf009\']').val();
	 
    var str1=$('input[name=\'tf004\']').val();
	var str2=$('input[name=\'tf005\']').val();
	
	var str11=str1.substr(0,2);str12=str1.substr(2,2);
	var str21=str2.substr(0,2);str22=str2.substr(2,2);	
	var str3=parseInt(str21)-parseInt(str11);str4=parseInt(str22)-parseInt(str12);
	
	if (str4 < 0) {str3=parseInt(str3)-1;str4=parseInt(str4)*-1;}
	if (str4 >0 && str4 <30) {str4=0;}
	if (str4 >=30 && str4 <60) {str4=0.5;}
	
	var str5=str3+str4;	
	
	var str6=str3+str4+parseInt(str9);
	if (week == '0' && str6>8 ) {form.tf014.value=8;form.tf015.value=str6-8;form.tf010.value=0;form.tf011.value=0;form.tf012.value=0;form.tf013.value=0;}
	if (week == '0' && str6<=8 ) {form.tf014.value=str6;form.tf015.value=0;form.tf010.value=0;form.tf011.value=0;form.tf012.value=0;form.tf013.value=0;}
	// alert(str6);
	
	if (week == '6' && str6>8 ) {form.tf012.value=8;form.tf013.value=str6-8;form.tf010.value=0;form.tf011.value=0;form.tf014.value=0;form.tf015.value=0;}
	if (week == '6' && str6<=8 ) {form.tf012.value=str6;form.tf013.value=0;form.tf010.value=0;form.tf011.value=0;form.tf014.value=0;form.tf015.value=0;}
	
	if ( (week >= '1') && (week <='5') && (str6>2) )   {form.tf010.value=2;form.tf011.value=str6-2;form.tf012.value=0;form.tf013.value=0;form.tf014.value=0;form.tf015.value=0;}
  	if ( (week >= '1') && (week <='5') && (str6<=2) )  {form.tf010.value=str6;form.tf011.value=0;form.tf012.value=0;form.tf013.value=0;form.tf014.value=0;form.tf015.value=0;}
	//alert(str6);
	form.tf006.value=str5;  
	return form.tf006.value;
 }
  function timecal2(oInput){         //時間相減2  
     var week=$('input[name=\'tf003\']').val();
    var str9=$('input[name=\'tf006\']').val();
	 
    var str1=$('input[name=\'tf007\']').val();
	var str2=$('input[name=\'tf008\']').val();
	...
	if ( (week >= '1') && (week <='5') && (str6>2) )   {form.tf010.value=2;form.tf011.value=str6-2;form.tf012.value=0;form.tf013.value=0;form.tf014.value=0;form.tf015.value=0;}
  	if ( (week >= '1') && (week <='5') && (str6<=2) )  {form.tf010.value=str6;form.tf011.value=0;form.tf012.value=0;form.tf013.value=0;form.tf014.value=0;form.tf015.value=0;}
	//alert(str6);
	form.tf009.value=str5;  
	return form.tf009.value;
 }
//--></script>
