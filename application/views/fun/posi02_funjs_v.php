...
function showcmsq03a(sText){                 //不更新網頁 3 庫別   
	var oSpan = document.getElementById("cmsq03adisp");	
...
function startcmsq03a(oInput){            //不更新網頁 3  庫別
	 
	//首先判斷是否有輸入，沒有輸入直接返回，並提示
	
	if(!oInput.value){
		oInput.focus();    //聚焦到用戶名的輸入框
		$("#cmsq03adisp").html("<span style='color:red'>欄位不可空白.</span>");			
		return;
	}
	//建立非同步請求
 ...
 function dataymd1(oInput){
	//首先判斷是否有輸入，沒有輸入直接返回，並提示
	        //alert(test);
		   document.form.ma004.value=oInput.value.substring(0,4)+'/'+oInput.value.substring(4,6)+'/'+oInput.value.substring(6,8); 
		   var zstr=$('input[name=\'ma004\']').val();
		 //  alert(zstr.length);
		 
		if  ( zstr.length != 10) { 
		    document.form.ma004.focus(); return ma004;}	
}
