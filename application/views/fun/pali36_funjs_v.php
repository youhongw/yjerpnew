...
function showpalq01a(sText){   //不更新網頁 32 員工代號parse() 用于解析JSON字符串。从 JSON 字符串创建一个对象
	sText = JSON.parse(sText);
	var oSpan = document.getElementById("palq01adisp");
	 oSpan.innerHTML = "<span style='color:black'>"+sText['mv002']+"</span>"; 	
	var ipt_tk002 = document.getElementById("tk002");
	 ipt_tk002.value = sText['mv004']; 
	 if (!sText) { 
...
<script type="text/javascript"><!--       
 function dataym1(oInput){
	//首先判斷是否有輸入，沒有輸入直接返回，並提示
	     //   alert('test');
		   document.form.tk003.value=oInput.value.substring(0,4)+'/'+oInput.value.substring(4,6); 
		   var zstr=$('input[name=\'tk003\']').val();
		 //  alert(zstr.length);
		 
		if  ( zstr.length != 7) { 
		    document.form.tk003.focus(); return tk003;}	
}
//--></script>
