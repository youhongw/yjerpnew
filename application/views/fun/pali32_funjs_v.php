...
function showpalq01a(sText){   //不更新網頁 32 員工代號
	...
	var oSpan = document.getElementById("palq01adisp");
	 oSpan.innerHTML = "<span style='color:black'>"+sText['mv002']+"</span>";
	 
  	 
	var ipt_tb002 = document.getElementById("tb002");
	 ipt_tb002.value = sText['mv004']; 
...
<script type="text/javascript"><!--       
 function dataym1(oInput){
	//首先判斷是否有輸入，沒有輸入直接返回，並提示
	     //   alert('test');
		   document.form.tb003.value=oInput.value.substring(0,4)+'/'+oInput.value.substring(4,6); 
		   var zstr=$('input[name=\'tb003\']').val();
		 //  alert(zstr.length);
		 
		if  ( zstr.length != 7) { 
		    document.form.tb003.focus(); return tb003;}	
}
//--></script>
