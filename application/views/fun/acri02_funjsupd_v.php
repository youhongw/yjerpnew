...
<script type="text/javascript"><!--       
 function dataymd1(oInput){
	//首先判斷是否有輸入，沒有輸入直接返回，並提示
	       // alert(test);
		   document.form.tc016.value=oInput.value.substring(0,4)+'/'+oInput.value.substring(4,6)+'/'+oInput.value.substring(6,8); 
		   var zstr=$('input[name=\'tc016\']').val();
		 //  alert(zstr.length);
		 
		if  ( zstr.length == 8) { 
		    document.form.tc016.focus(); return tc016;}	
}
...
//	html += '    <td class="left"><input type="text"  tabIndex="20"  name="order_product[' + product_row + '][tb003]" value="0" size="6" /></td>';
	...
	html += '    <td class="left"><input type="text" id="tb008" tabIndex="76" onclick="scwShow(this,event);"  onKeyPress="keyFunction()" name="order_product[' + product_row + '][tb008]" value="" size="10" style="background-color:#E7EFEF" /></td>';
	...
	Enterkey();
	$('input[name=\'order_product[' + product_row + '][tb005]\']').focus();   //新增一筆
	product_row++;
}
//-->
</script>
