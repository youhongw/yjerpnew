...
<script type="text/javascript"><!--
  function addpalq01a(sma001,sma002,sma003) {
	...
	$('#tc002').val(sma003);startcmsq05a($('#tc002').get(0));
...
function showpalq01a(sText){   //不更新網頁 32 員工代號 
	var data = jQuery.parseJSON(sText);
	var oSpan = document.getElementById("palq01adisp");
	 oSpan.innerHTML = "<span style='color:black'>"+data['mv002']+"</span>";
	 $('#tc002').val(data['mv004']);startcmsq05a($('#tc002').get(0));
	 if (!sText) { 
...
<!-- 不更新網頁  bzero   -->	
<script type="text/javascript"><!--       
 function bzero(oInput){
	     //   alert(oInput.value);
		     oInput.select();
		   //  if (oInput.value == '0')
			//     document.form.tc004.value='';
}
//--></script>
<!-- 不更新網頁  按每個欄位會全部反白0  -->	
<script type="text/javascript"><!--       
...
<script type="text/javascript"><!--       
 function dataym1(oInput){
	//首先判斷是否有輸入，沒有輸入直接返回，並提示
	     //   alert('test');
		   document.form.tc003.value=oInput.value.substring(0,4)+'/'+oInput.value.substring(4,6); 
		   var zstr=$('input[name=\'tc003\']').val();
		 //  alert(zstr.length);
		 
		if  ( zstr.length != 7) { 
		    document.form.tc003.focus(); return tc003;}	
}
//--></script>
