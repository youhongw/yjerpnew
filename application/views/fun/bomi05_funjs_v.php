...
<script type="text/javascript"> 	   
	...
	<div id="divFbomq03a42" style="display:none"> 	   
	<div style="float:right;"><input type="button" class="close" value="close" /></div> 	   
	<iframe src="<?php echo base_url()?>index.php/fun/bomq03a/display2" allowTransparency="flase" name="ifmain" width="640" height="480" marginwidth="0" marginheight="0" frameborder="0"></iframe> 	   
	</div> 	  
<script type="text/javascript"><!--
  function addbomq03a42(std001,std002) {	
	form.bomq03a42.value=std001;
	var oSpan = document.getElementById("bomq03a42disp");
		oSpan.innerHTML = std002;
	document.form.bomq03a42.focus();    
	return bomq03a42;	
}
//--></script>
...
<script type="text/javascript"> 	 
	...
	var zno=$('input[name=\'invq02a1\']').val();
	 ...
	 var ifm = document.getElementById('ifm');
	     ifm.innerHTML = $('input[name=\'invq02a1\']').val();	
		
  //   ifm.contentDocument.getElementById("td004").value = zno;
  //  alert(ifm.innerHTML);	
	$.blockUI({ 	   
	css: { 	   
	top: '20%', 	   
	...
	<iframe src="<?php echo base_url()?>index.php/fun/bomc02a/display/"+<span id="ifm"><?php echo $ifm; ?></span>  allowTransparency="flase" name="ifmain" width="620" height="460" marginwidth="0" marginheight="0" frameborder="0"></iframe> 	   
	</div>

<!--1主件品號,2組成用量,  std007 成品數量, sma003庫別,std001 組合別  sma001 主件品號   std0014 日期         -->
<script type="text/javascript">
  function addbomc02a(sma001,sma002,sma003) {
  //  var oSpan = document.getElementById("cmsq06adisp");
	//	oSpan.innerHTML = 'testtesttest';
    var std007=$('input[name=\'td007\']').val();
    var std001=$('input[name=\'bomq03a42\']').val();
    var std002=$('input[name=\'td002\']').val();
	var ste007=sma003;
	var std004=$('input[name=\'invq02a1\']').val();
	var sss=$('input[name=\'td014\']').val();
	var std014=sss.substring(0,4)+sss.substring(5,7)+sss.substring(8,10);
    //   form.td004.value=sma001;		
	//   alert(sma001+std007+sma003+std001);
	url = '<?php echo base_url()?>index.php/bom/bomi05/copybefore/'+encodeURIComponent(std004)+'/'+std007+'/'+sma003+'/'+std001+'/'+std002+'/'+std004+'/'+std014; 
	location = url;
	return true;
}
</script>
...
function shbomq03a42(sText){                 //不更新網頁 31  組合單別 td001
	var oSpan = document.getElementById("bomq03a42disp");
      //   chkno1();
     //   alert('test');		
 ...
 <!-- 不更新網頁帶出資料 bomq03a42  組合單別 -->        
function stbomq03a42(oInput){            
  //	if(!oInput.value){
...
//	}
  ...
  //  alert(test);
	createXMLHttpRequest();
   	...
   	var sUrl = "<?php echo base_url()?>index.php/fun/bomq03a/databomq03a42/" + encodeURIComponent(oInput.value)+ "/" + new Date().getTime();   
	...
	xmlHttp.onreadystatechange = function(){	    
		if(xmlHttp.readyState == 4 && xmlHttp.status == 200)					
		  shbomq03a42(xmlHttp.responseText);	//顯示服務器結果		
	}
...
function showchkno1(sText){   
       ...
       if  (!sText) { sText=$('input[name=\'td014\']').val();   
	  ...
	  // alert(zno1);
	   ...
	   document.getElementById("td002").value=zno3;
...
//	}
	//建立非同步請求
	 ...
	 var zno=$('input[name=\'bomq03a42\']').val();
	 var zstr=$('input[name=\'td014\']').val();
	...
	createXMLHttpRequest();
   	var sUrl = "<?php echo base_url()?>index.php/bom/bomi05/datachkno1/" + encodeURIComponent(zno)+ "/" + encodeURIComponent(zymd)+ "/" + new Date().getTime();   
	var QueryString = encodeURIComponent(oInput.value);
...
<script type="text/javascript"><!--       
 function selappr(){
	//首先判斷是否有輸入，沒有輸入直接返回，並提示
	  var selval = document.getElementById('td012').selectedIndex;
	 //  alert(selval);
...
function addItem() {
    ...
    html += '    <input type="hidden" name="order_product[' + product_row + '][te200]" value="0" />';
     html += '    <input type="hidden" name="order_product[' + product_row + '][te201]" value="0" />';	
//	html += '    <td class="left"><input type="text"  tabIndex="20"  name="order_product[' + product_row + '][te003]" value="0" size="6" /></td>';
	html += '    <td class="left"><input type="text"  tabIndex="41" onchange="startinvq02a(this,product_row)" id="te004'+ product_row+'"   name="order_product[' + product_row + '][te004]" value="" size="20"  style="background-color:#E7EFEF"/></td>';	
	...
	html += '    <td class="center"><input type="text" tabIndex="44"   id="te008" onKeyPress="keyFunction()" name="order_product[' + product_row + '][te008]" value="1" size="10" style="text-align:right" /></td>';
    html += '    <td class="center"><input type="text" tabIndex="45" id="te011" onKeyPress="keyFunction()" name="order_product[' + product_row + '][te011]" value="0" size="10" style="text-align:right" /></td>';	
    html += '    <td class="center"><input type="text" tabIndex="46" id="te012" onKeyPress="keyFunction()" name="order_product[' + product_row + '][te012]" value="0" size="10" style="text-align:right" /></td>';
	html += '    <td class="left"><input type="text" id="te009" tabIndex="47"  onKeyPress="keyFunction()" name="order_product[' + product_row + '][te009]" value="" size="20" /></td>';
	html += '  </tr>';	
    html += '</tbody>';
    ...
    $('input[name=\'order_product[' + product_row + '][te004]\']').catcomplete({
		...
		function(req, add){ 	
          ...
          //  var smb001=document.getElementById('te004').value;
			...
			//   smb001=$("#te003"+(product_row-1)).val();
			   //  alert(smb001);
			 
            $.ajax({  
                url: '<?php echo base_url(); ?>index.php/bom/bomi05/lookup/'+encodeURIComponent(smb001), 
                cache: false, 				
                dataType: 'json',  
                type: 'POST',  
                data: req,  
    ...
    $('input[name=\'order_product[' + product_row + '][te007]\']').catcomplete({
		...
		function(req, add){ 	
          ...
          //  var smb001=document.getElementById('te007').value;
            ...
            $.ajax({  
                url: '<?php echo base_url(); ?>index.php/bom/bomi05/lookupa/'+encodeURIComponent(smb001), 
                cache: false, 				
                dataType: 'json',  
                type: 'POST',  
                data: req,  
	...
	$('input[name=\'order_product[' + product_row + '][te008]\']').focusout(function() { 
		var n = this.name.replace(/order_product\[(\d+)].*/, '$1'); 
		var input_1=$('input[name=\'order_product[' + n + '][te008]\']').val()*1;  
		var input_2=$('input[name=\'order_product[' + n + '][te011]\']').val()*1;  
		var get_total=input_1*input_2;  
		$('input[name=\'order_product[' + n + '][te012]\']').val(get_total); 
	...
	//元件用量
	$('input[name=\'order_product[' + product_row + '][te008]\']').focus(function(){
	   ...
	   //損耗量
	$('input[name=\'order_product[' + product_row + '][te012]\']').focus(function(){
		var n = this.name.replace(/order_product\[(\d+)].*/, '$1'); 
		var input_3=$('input[name=\'order_product[' + n + '][te008]\']').val()*1;  
		var input_4=$('input[name=\'order_product[' + n + '][te011]\']').val()*1;  
		var get_total1=input_3*input_4;  
		$('input[name=\'order_product[' + n + '][te012]\']').val(get_total1); 		
	}); 
