...
<script type="text/javascript"> 	   
	...
	<iframe src="<?php echo base_url()?>index.php/fun/posq02a/display" allowTransparency="flase" name="ifmain" width="640" height="480" marginwidth="0" marginheight="0" frameborder="0"></iframe> 	   
	</div> 	  
<script type="text/javascript"><!--
...
<script type="text/javascript"> 	   
	...
	<div id="divFcmsq032a" style="display:none"> 	   
	<div style="float:right;"><input type="button" class="close" value="close" /></div> 	   
...
<script type="text/javascript"><!--
  function addcmsq03a(sta001,sta002) {
	  	  form.cmsq032a.value=sta001;
	var oSpan = document.getElementById("cmsq032adisp");
		oSpan.innerHTML = sta002;
	document.form.cmsq032a.focus();    
	return cmsq032a;
...
<script type="text/javascript"> 	   
	...
	<div id="divFcmsq033a" style="display:none"> 	   
	<div style="float:right;"><input type="button" class="close" value="close" /></div> 	   
...
<script type="text/javascript"><!--
  function addcmsq033a(sta001,sta002) {
	  	  form.cmsq033a.value=sta001;
	var oSpan = document.getElementById("cmsq033adisp");
		oSpan.innerHTML = sta002;
	document.form.cmsq033a.focus();    
	return cmsq033a;
...
function showposq02a(sText){                 //不更新網頁 31  核價單別 pos/posi1101
	var oSpan = document.getElementById("posq02adisp");
 ...
 <!-- 不更新網頁帶出資料 posq02a 單別 -->        
function startposq02a(oInput){            
...
function showcmsq03a(sText,n){   //不更新網頁 6  庫別 下拉視窗輸入
	...
	// alert(sysma200);
	 if (num>sysma200){
          $('input[name=\'order_product[' + nn + '][td012disp]\']').val(sText); }  
}
...
function showinvq02a(sText,n){   //不更新網頁 6  品號 下拉視窗輸入 品名規格單位
	...
	// alert(str[1]);
	 ...
	 $('input[name=\'order_product[' + nn + '][td008]\']').val(str[2]);}
...
function showchkno1(sText){ 
      //  alert('test');
       if  (!sText) { sText=$('input[name=\'tc003\']').val();   
...
function chkno1(oInput){  
     // alert('test');
	 var zno=$('input[name=\'posq02a\']').val();
	// alert(zno);
	 var zstr=$('input[name=\'tc003\']').val();
	 //  alert(zstr);
	...
	createXMLHttpRequest();
   	var sUrl = "<?php echo base_url()?>index.php/pos/posi11/datachkno1/" + encodeURIComponent(zno)+ "/" + encodeURIComponent(zymd)+ "/" + new Date().getTime();   
	var QueryString = encodeURIComponent(oInput.value);
...
function dataymd1(oInput){
	//首先判斷是否有輸入，沒有輸入直接返回，並提示
		   ...
		   document.form.tc004.value=oInput.value.substring(0,4)+'/'+oInput.value.substring(4,6)+'/'+oInput.value.substring(6,8); 
		   var zstr=$('input[name=\'tc004\']').val();
		 //  alert(zstr.length
		if  ( zstr.length == 8) { 
		    document.form.tc004.focus(); return tc004;}	
}
function dataymd2(oInput){
	//首先判斷是否有輸入，沒有輸入直接返回，並提示
	
		   document.form.tc006.value=oInput.value.substring(0,4)+'/'+oInput.value.substring(4,6)+'/'+oInput.value.substring(6,8); 
		   var zstr=$('input[name=\'tc006\']').val();
		 //  alert(zstr.length
		if  ( zstr.length == 8) { 
		    document.form.tc006.focus(); return tc006;}	
}
function datahm1(oInput){
	//首先判斷是否有輸入，沒有輸入直接返回，並提示
	
		   document.form.tc005.value=oInput.value.substring(0,2)+':'+oInput.value.substring(2,4); 
		   var zstr=$('input[name=\'tc005\']').val();
		 //  alert(zstr.length
		if  ( zstr.length == 4) { 
		    document.form.tc005.focus(); return tc005;}	
}
function datahm2(oInput){
	//首先判斷是否有輸入，沒有輸入直接返回，並提示
	
		   document.form.tc007.value=oInput.value.substring(0,2)+':'+oInput.value.substring(2,4); 
		   var zstr=$('input[name=\'tc007\']').val();
		 //  alert(zstr.length
		if  ( zstr.length == 4) { 
		    document.form.tc007.focus(); return tc007;}	
}
//--></script>
...
function addItem() {
    ...
    html += '    <input type="hidden" name="order_product[' + product_row + '][td009b]" value="" />';
	html += '    <td class="left"><input type="text"  tabIndex="51" id="td004'+ product_row+'" onchange="startinvq02a(this,product_row)"   name="order_product[' + product_row + '][td004]" value="" size="20" style="background-color:#E7EFEF" /></td>';	
	...
	html += '    <td class="left"><input    onKeyPress="keyFunction()"   type="text" id="td007"   name="order_product[' + product_row + '][td007]" value="" size="6" /></td>';
	html += '    <td class="left"><input type="text"   readonly="value" tabIndex="52"  name="order_product[' + product_row + '][td003]" value="0" size="6" style="background-color:#EBEBE4" /></td>';
	
	html += '    <td class="center"><input type="text" tabIndex="53"  class="total_qty" id="td008" onKeyPress="keyFunction()" name="order_product[' + product_row + '][td008]" value="0" size="10" style="text-align:right" /></td>';
	html += '    <td class="left"><input type="text" id="td009"  tabIndex="54" onKeyPress="keyFunction()" name="order_product[' + product_row + '][td009]" value="0" size="10" style="text-align:right"  /></td>';
	html += '    <td class="left"><input type="text" id="td010"  tabIndex="55" class="total_price" onKeyPress="keyFunction()" name="order_product[' + product_row + '][td010]" value="0" size="10" style="text-align:right"  /></td>';
    
	html += '    <td class="left"><input type="text" id="td011" tabIndex="61"  onKeyPress="keyFunction()" name="order_product[' + product_row + '][td011]" value="" size="20" /></td>';
	html += '  </tr>';	
    html += '</tbody>';
	 ...
	 $('#order_product tfoot').before(html);  
	   	   ...
	   	   //下拉視窗 網頁不更新  mb001 td004 品號品名輸入  預設轉出庫
	   //  $('input[name=\'order_product[' + product_row + '][td012]\']').val()=document.getElementById('ta018').value;
    ...
    $('input[name=\'order_product[' + product_row + '][td004]\']').catcomplete({
		...
		function(req, add){ 	
          ...
          //  var smb001=document.getElementById('td004').value;
            ...
            $.ajax({  
                url: '<?php echo base_url(); ?>index.php/pos/posi11/lookup/'+encodeURIComponent(smb001), 
                cache: false, 				
                dataType: 'json',  
                type: 'POST',  
                data: req,  
            ...
            select:   
               function(event, ui) { 
			     ...
			     $('input[name=\'order_product[' + n + '][td007]\']').val(ui.item.value4);
				 $('input[name=\'order_product[' + n + '][td008]\']').val(ui.item.value5);
				 $('input[name=\'order_product[' + n + '][td011]\']').val(ui.item.value6);
	...
	$('input[name=\'order_product[' + product_row + '][td008]\'],input[name=\'order_product[' + product_row + '][td009]\']').focusout(function() { 
		var n = this.name.replace(/order_product\[(\d+)].*/, '$1'); 
		var input_1=$('input[name=\'order_product[' + n + '][td009]\']').val()*1;  
	 ...
	 // 數量游標停在 0 之後 
	$('input[name=\'order_product[' + product_row + '][td009]\']').focus(function(){
	  //  var n = this.name.replace(/order_product\[(\d+)].*/, '$1'); 
		...
		$('input[name=\'order_product[' + product_row + '][td009]\']').focusout(function(){
	  //  var n = this.name.replace(/order_product\[(\d+)].*/, '$1'); 
		var real_value = $(this)[0].defaultValue;
		if ($(this).val()==real_value)
		    $(this).val(real_value);
	...
	//備註,品號
  	$('input[name=\'order_product[' + product_row + '][td011]\']').blur(function(){
	    var n = this.name.replace(/order_product\[(\d+)].*/, '$1'); 
