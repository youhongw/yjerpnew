...
//	html += '    <td class="left"><input type="text"  tabIndex="20"  name="order_product[' + product_row + '][td003]" value="0" size="6" /></td>';
	...
	html += '    <td class="left"><input readonly="value" ondblclick="invq81a(this,'+ product_row +')"  onKeyPress="keyFunction()"   type="text" id="td010"   name="order_product[' + product_row + '][td010]" value="" size="6" style="background-color:#E7EFEF" /></td>';
	...
	html += '    <td class="left"><input type="text"  tabIndex="52" onchange="startcmsq03a(this,product_row)" id="td007'+ product_row+'" ondblclick="cmsq03a(this,'+ product_row +')" name="order_product[' + product_row + '][td007]" value="" size="10" style="background-color:#E7EFEF" /></td>';	
	...
	html += '    <td class="center"><input type="text" tabIndex="56" id="td011" ondblclick="copq06a(this,'+ product_row +');" onKeyPress="keyFunction()" name="order_product[' + product_row + '][td011]" value="0" size="8" style="text-align:right;background-color:#E7EFEF" /></td>';
...
function totalSum() {
   ...
   /*  var taxamt11=Math.round(amt1/(1+parseFloat(taxp))); //內含稅  出貨單
	   ...
	   if ($('select[name=\'tg017\']').val()=='1') {$('input[name=\'order_product[' + n + '][th036]\']').val(amt2);} */
	     taxp=$('input[name=\'tc041\']').val();
		  amt1=Math.round(sumTotal);	
	     taxamt11=Math.round(amt1/(1+parseFloat(taxp)));  //內含稅 金額
		 amt2=amt1-taxamt11;   //內含稅  稅額
		 if ($('select[name=\'tc016\']').val()=='1') {form.tc029.value=Math.round(taxamt11);form.tc030.value=Math.round(amt2);}
		 
		 tax=$('input[name=\'tc041\']').val();
		 if ($('select[name=\'tc016\']').val()!='1') {form.tc029.value=Math.round(sumTotal);form.tc030.value=Math.round(sumTotal*tax);}
  ...
  /*	form.tc029.value=Math.round(sumTotal);	
    ...
    var sumTax =parseFloat(form.tc030.value);
...
//--></script>
 ...
 <!-- 開啟單位 -->
<script type="text/javascript"> 	   
	function invq81a(thisobj,row_count) {	
	// var n = this.name.replace(/order_product\[(\d+)].*/, '$1');    
	// var ifm = document.getElementById('ifm');
	//    ifm.innerHTML = $('input[name=\'order_product[' + n + '][tb004]\']').val(ui.item.value1);
	
	var str = thisobj.name;
	if(!$("input[name$='"+str.replace(/td010/,"td004")+"']").val() ) {
		alert("請先選擇該欄位的產品品號");
		return;
	}
	$('#hp_ifmain').attr('src','<?php echo base_url()?>index.php/fun/invq81a/display_hp/md001/desc/0/'+$("#tc004").val()+'/'+$("input[name$='"+str.replace(/td010/,"td004")+"']").val()+'/'+row_count);
	...
	function select_price(price,row){
		$('input[name=\'order_product[' + row + '][td010]\']').val(price);
		$('.close').click();
	}
	...
	<div style="float:right;"><input type="button" class="close" value="close" /></div> 	   
	<iframe src="<?php echo base_url()?>index.php/fun/invq81a/display_hp/md001td001/desc/0/cn00001" allowTransparency="flase" id="hp_ifmain" name="ifmain" width="640" height="480" marginwidth="0" marginheight="0" frameborder="0"></iframe> 	   
	</div>
<script type="text/javascript"><!--
  function addinvq81a1(stc001,stc002,stc003) {
		 //   alert(stc002);
	     var product_row1 = 0; 
	$(".total_dc").each(function() {		
		product_row1++;
