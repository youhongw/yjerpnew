...
function showcmsq02a(sText){                 //不更新網頁 3 廠別    
	var oSpan = document.getElementById("cmsq02adisp");	
...
function startcmsq02a(oInput){            //不更新網頁 3  廠別
	 
	//首先判斷是否有輸入，沒有輸入直接返回，並提示
	
	if(!oInput.value){
		oInput.focus();    //聚焦到用戶名的輸入框
		$("#cmsq02adisp").html("<span style='color:red'>欄位不可空白.</span>");			
		return;
	}
	//建立非同步請求
	...
	//首先判斷是否有輸入，沒有輸入直接返回，並提示
	 // alert(oInput.value);
	   //  zamt3=$('input[name=\'md003\']').val();
	      zamt4=$('input[name=\'mp005\']').val();
		  zamt5=$('input[name=\'mp008\']').val();
		  zamt7=parseInt(zamt4)+parseInt(zamt5);
	      form.mp007.value=Math.round(zamt7);
	 return mp007;
}
