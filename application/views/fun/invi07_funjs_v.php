...
<script type="text/javascript"> 	   
	...
	<div id="divFinvq04a17" style="display:none"> 	   
	<div style="float:right;"><input type="button" class="close" value="close" /></div> 	   
	<iframe src="<?php echo base_url()?>index.php/fun/invq04a/display7" allowTransparency="flase" name="ifmain" width="640" height="480" marginwidth="0" marginheight="0" frameborder="0"></iframe> 	   
	</div> 	  
<script type="text/javascript"><!--
  function addinvq04a17(sma001,sma002) {	
	form.invq04a17.value=sma001;
	var oSpan = document.getElementById("invq04a17disp");
		oSpan.innerHTML = sma002;
	document.form.invq04a17.focus();    
	return invq04a17;	
}
//--></script>
<!-- 開視窗 cmsq05a 部門別 -->
...
<script type="text/javascript"><!--
  function addcmsq02a(sta001,stj002) {
...
function showinvq04a17(sText){                 //不更新網頁 31  核價單別 inv/invi0701
	var oSpan = document.getElementById("invq04a17disp");
      //   chkno1();
     //   alert('test');		
 ...
 <!-- 不更新網頁帶出資料 invq04a17 核價單別 -->        
function startinvq04a17(oInput){            
  //	if(!oInput.value){
	//	oInput.focus();    //聚焦到用戶名的輸入框
	//	$("#inv/invi0701disp").html("<span style='color:red'>不可空白.</span>");			
	//	return;
//	}
	//建立非同步請求
	...
	createXMLHttpRequest();
   	...
   	var sUrl = "<?php echo base_url()?>index.php/fun/invq04a/datainvq04a17/" + encodeURIComponent(oInput.value)+ "/" + new Date().getTime();   
...
function showcmsq03a(sText,n){   //不更新網頁 6  庫別 
	...
	// alert(sysma200);
	 if (num>sysma200){
          $('input[name=\'order_product[' + nn + '][tk017disp]\']').val(sText); }  
}
...
function showinvq02a(sText,n){   //不更新網頁 6  品號 
	...
	// alert(str[1]);
	 if (num>sysma201){
     $('input[name=\'order_product[' + nn + '][tk005]\']').val(str[0]);  
     $('input[name=\'order_product[' + nn + '][tk006]\']').val(str[1]);
	 $('input[name=\'order_product[' + nn + '][tk006disp]\']').val(str[2]);}
...
function showchkno1(sText){   
       ...
       if  (!sText) { sText=$('input[name=\'tj012\']').val();   
	   ...
	   document.getElementById("tj002").value=zno3;
	//   alert(zno3);
//	   var oSpan = document.getElementById("invq04a17disp");
//	 oSpan.innerHTML = "<span style='color:black'>"+sText+"</span>"; 	
//	 if (!sText) { 
...
//	 }	
	...
	//	alert(zno3);
	//var oSpan = document.getElementById("inv/invi0702");	 
	// oSpan.innerHTML = "<span style='color:black'>"+sText+"</span>"; 	
	// if (!sText) { 
	 //   oSpan.innerHTML = "<span style='color:red'>無此資料!</span>"; 	
	 }	
...
//	if(!oInput.value){
//		oInput.focus();    //聚焦到用戶名的輸入框
//		$("#inv/invi0706disp").html("此欄位不可空白.");	
//		return;
//	}
	//建立非同步請求
	
	 var zno=$('input[name=\'invq04a17\']').val();
	 var zstr=$('input[name=\'tj012\']').val();
	...
	createXMLHttpRequest();
   	var sUrl = "<?php echo base_url()?>index.php/inv/invi07/datachkno1/" + encodeURIComponent(zno)+ "/" + encodeURIComponent(zymd)+ "/" + new Date().getTime();   
	var QueryString = encodeURIComponent(oInput.value);
...
<script type="text/javascript"><!--       
 function selappr(){
	//首先判斷是否有輸入，沒有輸入直接返回，並提示
	  var selval = document.getElementById('tj010').selectedIndex;
	 //  alert(selval);
...
function addItem() {
	...
	html += '    <td class="center"><img src="<?php echo base_url()?>assets/image/delete.png" title="移除" onclick="$(\'#product-row' + product_row + '\').remove();" /></td>';
  	html += '    <input type="hidden"  name="order_product[' + product_row + '][tk001]" value="" />';
	html += '    <input type="hidden" name="order_product[' + product_row + '][tk002]" value="" />';	
//	html += '    <td class="left"><input type="text"  tabIndex="13"  name="order_product[' + product_row + '][tk003]" value="0" size="6" /></td>';
	html += '    <td class="left"><input type="text"  tabIndex="51" id="tk004'+ product_row+'" onchange="startinvq02a(this,product_row)"   name="order_product[' + product_row + '][tk004]" value="" size="20" style="background-color:#E7EFEF" /></td>';	
	html += '    <td class="left"><input readonly="value"   onKeyPress="keyFunction()" type="text" id="tk005"  name="order_product[' + product_row + '][tk005]" value=""  /></td>';
	html += '    <td class="left"><input readonly="value"  onKeyPress="keyFunction()" type="text" id="tk006"   name="order_product[' + product_row + '][tk006]" value=""  size="30" /></td>';
	html += '    <td class="left"><input    onKeyPress="keyFunction()"   type="text" id="tk006disp"   name="order_product[' + product_row + '][tk006disp]" value="" size="6" /></td>';
	html += '    <td class="left"><input type="text"   readonly="value" tabIndex="52"  name="order_product[' + product_row + '][tk003]" value="0" size="6" style="background-color:#EBEBE4" /></td>';
	html += '    <td class="left"><input type="text"  ondblclick="scwShow(this,event);"  tabIndex="53" id="tk019['+ product_row +']" onKeyPress="keyFunction()" name="order_product[' + product_row + '][tk019]" value="<?php echo date("Y/m/d"); ?>" size="10"  style="background-color:#E7EFEF"/></td>';
	html += '    <td class="center"><input type="text" id="tk007" tabIndex="54"   class="total_qty"  onKeyPress="keyFunction()" name="order_product[' + product_row + '][tk007]" value="0" size="12" style="text-align:right" /></td>';
	
	html += '    <td class="left"><input type="text" id="tk016"  tabIndex="55" class="total_price" onKeyPress="keyFunction()" name="order_product[' + product_row + '][tk016]" value="0" size="6" style="text-align:right"  /></td>';
    html += '    <td class="left"><input type="text"  tabIndex="56" onchange="startcmsq03a(this,product_row)" id="tk017'+ product_row+'"   name="order_product[' + product_row + '][tk017]" value="" size="10"  /></td>';	
	html += '    <td class="left"><input readonly="value" type="text"  tabIndex="57"  id="tk017disp'+ product_row+'"   name="order_product[' + product_row + '][tk017disp]" value="" size="10" style="background-color:#EBEBE4"  /></td>';   
   
	html += '    <td class="left"><input type="text" id="tk021" tabIndex="58"  onKeyPress="keyFunction()" name="order_product[' + product_row + '][tk021]" value="" size="10" /></td>';
	html += '    <td class="left"><input type="text" id="tk022" tabIndex="59"  onKeyPress="keyFunction()" name="order_product[' + product_row + '][tk022]" value="" size="20" /></td>';
	html += '  </tr>';	
    html += '</tbody>';
	 ...
	 $('#order_product tfoot').before(html);  
	   	   ...
	   	   //下拉視窗 網頁不更新  mb001 tk004 品號品名輸入
	
    $('input[name=\'order_product[' + product_row + '][tk004]\']').catcomplete({
        autoFocus: true,
		delay: 0,
        minLength: 1,  		
		source: 
		function(req, add){ 	
          ...
          //  var smb001=document.getElementById('tk004').value;
			   vtj0= product_row - 1;
    ...
    $('input[name=\'order_product[' + product_row + '][tk017]\']').catcomplete({
        autoFocus: true,
		delay: 0,
        minLength: 1,  		
		source: 
	...
	$('input[name=\'order_product[' + product_row + '][tk007]\'],input[name=\'order_product[' + product_row + '][tk016]\']').focusout(function() { 
		var n = this.name.replace(/order_product\[(\d+)].*/, '$1'); 
	...
	//	$('input[name=\'order_product[' + n + '][tk010]\']').val(get_total);
	 ...
	 // 數量游標停在 0 之後 
	$('input[name=\'order_product[' + product_row + '][tk007]\']').focus(function(){
	    var n = this.name.replace(/order_product\[(\d+)].*/, '$1'); 
	...
	$('input[name=\'order_product[' + product_row + '][tk007]\']').focusout(function(){
	    var n = this.name.replace(/order_product\[(\d+)].*/, '$1'); 
		var real_value = $(this)[0].defaultValue;
		if ($(this).val()==real_value)
		    $(this).val(real_value);
	...
	$('input[name=\'order_product[' + product_row + '][tk016]\']').focusout(function(){
	    var n = this.name.replace(/order_product\[(\d+)].*/, '$1'); 
		var real_value = $(this)[0].defaultValue;
		if ($(this).val()==real_value)
		    $(this).val(real_value);
	...
	//庫別 
	$('input[name=\'order_product[' + product_row + '][tk017]\']').focus(function(){
	    var n = this.name.replace(/order_product\[(\d+)].*/, '$1'); 
	...
	//專案
	$('input[name=\'order_product[' + product_row + '][tk021]\']').focus(function(){
		totalSum1();
	});
	//備註,品號
  	$('input[name=\'order_product[' + product_row + '][tk022]\']').blur(function(){
		$('input[name=\'order_product[' + product_row + '][tk004]\']').focus();
	});
...
function totalSum1() {
	...
	form.tj007.value=Math.round(sumQty);	  //; 總數量
	form.tj008.value=Math.round(sumTotal);	  //; 總金額
