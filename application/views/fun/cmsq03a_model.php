...
class Cmsq03a_model extends CI_Model {
	...
	function selbrowse($num,$offset)   //查詢 table 表所有資料
      {            
        ...
        $this->db->from('cmsmc');
        $this->db->order_by('mc001', 'ASC');                //排序  單欄
	   // $this->db->order_by('mc001 asc, mc002 asc');    //排序  單欄以上 asc 由小至大 desc預設由大至小
	...
	//	$array = array('mc001' => '1', 'mc002 >=' => $seq6, 'mc002 <=' => $seq6 );
	//	$this->db->like('title', 'match', 'after'); 
	 ...
	 //    $array = array('mc001' => '1', 'mc002' => $seq6 );
	    $query = $this->db->select('COUNT(*) as count', FALSE)  //筆數查詢,如果設為FALSE不會使用反引號保護你的字段或者表名
	                      ->from('cmsmc')
						   ->like($seq4, $seq6, 'after');  
					//	  ->where('mc001','1');
	 ...
	 function ajaxcmsq03a1($seg1)    //ajax 查詢一筆 顯示用 庫別6
          { 	              
	  ...
	  function ajaxcmsq03a($seg1)    //ajax 查詢一筆 顯示用 庫別6
          { 	              
	    $this->db->set('mc001', $this->uri->segment(4));
	    $this->db->where('mc001', $this->uri->segment(4));	
	    $query = $this->db->get('cmsmc');
	...
	function findf($limit, $offset, $sort_by, $sort_order)    //查詢多筆進階查詢 mysql 
	  ...
	  //$seq5='';$seq51='';$seq7='';$seq71='';		  
	    ...
	    $seq1 = " mc001, mc002, mc003, mc004, mc005, mc006, create_date FROM `invma` ";
	...
	function insertf()    //新增一筆
	    ...
	    $exist = $this->invq01a_model->selone1($this->input->post('mc001'),$this->input->post('mc002'));
	    if ($exist)
	         {
		    return 'exist';
		 } 
	    ...
	    $data = array( 
            ...
            $exist = $this->invq01a_model->selone2($this->input->post('mc003c'),$this->input->post('mc004c'));
		    if ($exist)
		        {
			  return 'exist';
		        }         
	...
	function excelnewf()           //轉excel檔,一筆以上
	    ...
	    $seq4=$this->input->post('mc004c'); 
	    $sql = " SELECT mc001,mc002,mc003,mc004,mc005,mc006,create_date FROM invma WHERE mc001 >= '$seq1'  AND mc001 <= '$seq2' AND mc002 >= '$seq3' AND mc002 <= '$seq4' "; 
	...
	function printfd()           //印明細表一筆以上
	    ...
	    $seq3=$this->input->post('mc003c'); 
	    $seq4=$this->input->post('mc004c'); 
	    $sql = " SELECT * FROM invma WHERE mc001 >= '$seq1'  AND mc001 <= '$seq2' AND mc002 >= '$seq3' AND mc002 <= '$seq4' "; 
            $query = $this->db->query($sql);
	    $ret['rows'] = $query->result();
