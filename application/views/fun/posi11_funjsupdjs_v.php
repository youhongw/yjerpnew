...
//--></script>
		...
		function(req, add){ 
            ...
            select:   
               function(event, ui) { 
			     ...
			     $('input[name=\'order_product[' + n + '][td008]\']').val(ui.item.value4);
				// $('input[name=\'order_product[' + n + '][td012]\']').val(ui.item.value5);
				// $('input[name=\'order_product[' + n + '][td012disp]\']').val(ui.item.value6);
	...
	$('input[name=\'order_product['  + vrow +  '][td008]\'],input[name=\'order_product['  + vrow +  '][td009]\']').focusout(function() { 
		var n = this.name.replace(/order_product\[(\d+)].*/, '$1'); 
		var input_1=$('input[name=\'order_product[' + n + '][td008]\']').val()*1;  
		var input_2=$('input[name=\'order_product[' + n + '][td009]\']').val()*1;  
	...
	$('input[name=\'order_product[' + vrow +  '][td008]\']').focus(function(){
	 ...
	 //金額 數量游標停在 0 之後 
	$('input[name=\'order_product['  + vrow +  '][td009]\']').focus(function(){
	    var n = this.name.replace(/order_product\[(\d+)].*/, '$1'); 
		var real_value = $(this)[0].defaultValue;
		if ($(this).val()==real_value)
		    $(this).val(real_value);
	...
	//備註,品號
  	$('input[name=\'order_product[' + vrow + '][td011]\']').blur(function(){
		$('input[name=\'order_product['  + vrow +  '][td004]\']').focus();
	});
