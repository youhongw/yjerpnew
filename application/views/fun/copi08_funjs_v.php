...
<script type="text/javascript"> 	   //開視窗  客戶代號 invi02
	...
	<!-- 開視窗客戶代號2 簡稱 3電話4連絡人,5幣別,6付款條件,7課稅別 8 送貨地址 b帳單地址---->
<script type="text/javascript"><!--
...
<script type="text/javascript"><!--
  function addcopc08a(sma001,sma002,otg001,otg002,otg042) {
    ...
    var oSpan = document.getElementById("tg049");
      ...
      //    alert(ztg042);	   
	url = '<?=base_url() ?>index.php/cop/copi08/copybefore/'+encodeURIComponent(sma001)+'/'+encodeURIComponent(sma002)+'/'+otg001+'/'+otg002+'/'+ztg042; 
	location = url;
	return true;
}
//--></script>
...
//	}
	...
	createXMLHttpRequest();
   	var sUrl = "<?php echo base_url()?>index.php/fun/copq01a/checkcopq01a1/" + encodeURIComponent(oInput.value)+ "/" + new Date().getTime();   
	var QueryString = encodeURIComponent(oInput.value);
...
<script type="text/javascript"><!--    // 明細 新增 15 因前置單据30 因考量前置單據才給250,249 INSERT 250否則0,-1 INSERT 0
...
//	if ($('select[name=\'tg017\']').val()=='1') {form.tg013.value=Math.round(sumTotal/parseFloat(vtax));form.tg025.value=Math.round(sumTotal-parseFloat(form.tg013.value));}
//	if ($('select[name=\'tg017\']').val()=='1') {form.tg045.value=Math.round(parseFloat(form.tg013.value)*parseFloat(rate));form.tg046.value=Math.round(sumTotal1-parseFloat(form.tg045.value));}
	
	var sumTot =Math.round(sumTotal+sumTax);
	$("#sum_tot").html(sumTot.toFixed(1));	//合計原幣金額
//	 $("#sum_tot").html(parseFloat(form.tg013.value)+parseFloat(form.tg025.value));	
	var sumTot1 =Math.round(sumTotal1+sumTax1);
	$("#sum_tot1").html(sumTot1.toFixed(1));	//合計本幣金額
//	 $("#sum_tot1").html(parseFloat(form.tg045.value)+parseFloat(form.tg046.value));	
