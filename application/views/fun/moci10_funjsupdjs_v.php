...
//--></script>
	...
	function look_up_show(product_row,vrow,vtj0){
		 $('input[name=\'order_product['+vrow+'][tn004]\']').catcomplete({
        autoFocus: true,
		delay: 0,
        minLength: 1,  		
		source: 
	...
	$('input[name=\'order_product['  + vrow +  '][tm010]\'],input[name=\'order_product['  + vrow +  '][tm014]\']').focusout(function() { 
	  //	var n = this.name.replace(/order_product\[(\d+)].*/, '$1'); 
	 ...
	 //	alert(num_2);
	   ...
	   //單價  
	$('input[name=\'order_product['+ vrow + '][tm010]\']').focus(function(){
	...
	$('input[name=\'order_product[' + vrow + '][tm010]\']').blur(function(){
		var real_value = $(this)[0].defaultValue;
		if ($(this).val()=='')
			$(this).val(real_value);
	});
	
	$('input[name=\'order_product['  + vrow + '][tm011]\']').blur(function(){
		var vrow=vrow+1;
		$('input[name=\'order_product['  + vrow + '][tm004]\']').focus();
	});
