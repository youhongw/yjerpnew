...
<!-- 不更新網頁,客戶類別 -->
function showcopq04a(sText){   //不更新網頁 18  客戶類別 
	var oSpan = document.getElementById("copq04adisp");
		 oSpan.innerHTML = "<span style='color:black'>"+sText+"</span>"; 	
	 if (!sText) { 
	    oSpan.innerHTML = "<span style='color:red'>無此資料!</span>"; 	
	 }	
...
//	}
	...
	createXMLHttpRequest();
   	var sUrl = "<?php echo base_url()?>index.php/fun/copq04a/checkcopq04a/" + encodeURIComponent(oInput.value)+ "/" + new Date().getTime();   
	var QueryString = encodeURIComponent(oInput.value);
	...
	xmlHttp.onreadystatechange = function(){	    
		if(xmlHttp.readyState == 4 && xmlHttp.status == 200)					
		  showcopq04a(xmlHttp.responseText);	//顯示服務器結果		
	}
	xmlHttp.send(null);
}
<!-- 不更新網頁,付款銀行1 -->
...
function showcmsq16a1(sText){   //不更新網頁 18  付款銀行2
...
//	}
	...
	createXMLHttpRequest();
   	var sUrl = "<?php echo base_url()?>index.php/fun/cmsq16a/datacmsq16a1/" + encodeURIComponent(oInput.value)+ "/" + new Date().getTime();   
	var QueryString = encodeURIComponent(oInput.value);
	...
	xmlHttp.onreadystatechange = function(){	    
		if(xmlHttp.readyState == 4 && xmlHttp.status == 200)					
		  showcmsq16a1(xmlHttp.responseText);	//顯示服務器結果		
	}
	xmlHttp.send(null);
}
function showcmsq16a2(sText){   //不更新網頁 18  付款銀行3
...
//	}
	...
	createXMLHttpRequest();
   	var sUrl = "<?php echo base_url()?>index.php/fun/cmsq16a/datacmsq16a2/" + encodeURIComponent(oInput.value)+ "/" + new Date().getTime();   
	var QueryString = encodeURIComponent(oInput.value);
	...
	xmlHttp.onreadystatechange = function(){	    
		if(xmlHttp.readyState == 4 && xmlHttp.status == 200)					
		  showcmsq16a2(xmlHttp.responseText);	//顯示服務器結果		
	}
	xmlHttp.send(null);
}
...
<script type="text/javascript"><!--       
 function dataymd2(oInput){
	//首先判斷是否有輸入，沒有輸入直接返回，並提示
	       // alert(test);
		   document.form.ma068.value=oInput.value.substring(0,4)+'/'+oInput.value.substring(4,6)+'/'+oInput.value.substring(6,8); 
		   var zstr=$('input[name=\'ma068\']').val();
		 //  alert(zstr.length);
		 
		if  ( zstr.length != 10) { 
		    document.form.ma068.focus(); return ma068;}	
}
...
<script type="text/javascript"><!--       
 function dataymd3(oInput){
	//首先判斷是否有輸入，沒有輸入直接返回，並提示
	       // alert(test);
		   document.form.ma021.value=oInput.value.substring(0,4)+'/'+oInput.value.substring(4,6)+'/'+oInput.value.substring(6,8); 
...
<script type="text/javascript"><!--       
 function dataymd4(oInput){
	//首先判斷是否有輸入，沒有輸入直接返回，並提示
	       // alert(test);
		   document.form.ma022.value=oInput.value.substring(0,4)+'/'+oInput.value.substring(4,6)+'/'+oInput.value.substring(6,8); 
...
<script type="text/javascript"> 	   
	...
	<div id="divFcopq04a" style="display:none"> 	   
	<div style="float:right;"><input type="button" class="close" value="close" /></div> 	   
	<iframe src="<?php echo base_url()?>index.php/fun/copq04a/display" allowTransparency="flase" name="ifmain" width="640" height="480" marginwidth="0" marginheight="0" frameborder="0"></iframe> 	   
	</div> 

<script type="text/javascript"><!--
  function addcopq04a(sta001,sta002) {
	  	  form.copq04a.value=sta001;
	var oSpan = document.getElementById("copq04adisp");
		oSpan.innerHTML = sta002;
	document.form.copq04a.focus();    
	return copq04a;
...
<script type="text/javascript"> 	   
	...
	<div id="divFcopq82a" style="display:none"> 	   
	<div style="float:right;"><input type="button" class="close" value="close" /></div> 	   
	<iframe src="<?php echo base_url()?>index.php/fun/copq82a/display" allowTransparency="flase" name="ifmain" width="640" height="480" marginwidth="0" marginheight="0" frameborder="0"></iframe> 	   
	</div> 

<script type="text/javascript"><!--
  function addcopq82a(sta001,sta002) {
	  	  form.copq82a.value=sta001;
	var oSpan = document.getElementById("copq82adisp");
		oSpan.innerHTML = sta002;
	document.form.copq82a.focus();    
	return copq82a;
