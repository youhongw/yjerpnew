...
<script type="text/javascript"><!--
  function addinvq02a(sma001,sma002,sma003) {	
...
function showcopq01a(sText){   //不更新網頁 32 主供應商 
	var oSpan = document.getElementById("copq01adisp");
...
function startcopq01a(oInput){         //不更新網頁 32 主供應商
...
<script type="text/javascript"><!--       
 function dataymd2(oInput){
	//首先判斷是否有輸入，沒有輸入直接返回，並提示
		   ...
		   document.form.mb017.value=oInput.value.substring(0,4)+'/'+oInput.value.substring(4,6)+'/'+oInput.value.substring(6,8); 
		   var zstr=$('input[name=\'mb017\']').val();
		 //  alert(zstr.length
		if  ( zstr.length == 8) { 
		    document.form.mb017.focus(); return mb017;}	
}
