...
function showactq07a(sText,n){   //不更新網頁 6  品號 
	...
	// alert(str[1]);
	 if (num>sysma201){
     $('input[name=\'order_product[' + nn + '][dt002]\']').val(str[0]);  
     $('input[name=\'order_product[' + nn + '][dt003]\']').val(str[1]);
	 $('input[name=\'order_product[' + nn + '][dt004]\']').val(str[2]);}
	 
}

function startactq07a(oInput,n){         //不更新網頁 6 品號
...
//	}
    ...
    var nn=n;
	createXMLHttpRequest();
   	var sUrl = "<?php echo base_url()?>index.php/fun/actq07a/checkactq07a/" + encodeURIComponent(oInput.value)+ "/" + new Date().getTime();   
	var QueryString = encodeURIComponent(oInput.value);
	...
	xmlHttp.onreadystatechange = function(){	    
		if(xmlHttp.readyState == 4 && xmlHttp.status == 200)					
		  showactq07a(xmlHttp.responseText,nn);	//顯示服務器結果
...
function addItem() {
    ...
    html += '    <td class="left"><input type="text"  tabIndex="51" id="mh002'+ product_row+'"  onchange="startactq07a(this,product_row)"  name="order_product[' + product_row + '][mh002]" value="" size="6"  /></td>';	
