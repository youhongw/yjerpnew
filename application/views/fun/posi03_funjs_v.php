...
function showcmsq11a(sText){                 //不更新網頁 3 庫別   
	var oSpan = document.getElementById("cmsq11adisp");	
...
function startcmsq11a(oInput){            //不更新網頁 3  庫別
	...
	createXMLHttpRequest();
   	var sUrl = "<?php echo base_url()?>index.php/fun/cmsq11a/datacmsq11a/" + encodeURIComponent(oInput.value)+ "/" + new Date().getTime();   
	var QueryString = encodeURIComponent(oInput.value);
 ...
 function dataym1(oInput){
	//首先判斷是否有輸入，沒有輸入直接返回，並提示
	        //alert(test);
		   document.form.mb002.value=oInput.value.substring(0,4)+'/'+oInput.value.substring(4,6); 
		   var zstr=$('input[name=\'mb002\']').val();
		 //  alert(zstr.length);
		 
		if  ( zstr.length != 10) { 
		    document.form.mb002.focus(); return mb002;}	
}
function dataym2(oInput){
	//首先判斷是否有輸入，沒有輸入直接返回，並提示
	        //alert(test);
		   document.form.mb003.value=oInput.value.substring(0,4)+'/'+oInput.value.substring(4,6); 
		   var zstr=$('input[name=\'mb003\']').val();
		 //  alert(zstr.length);
		 
		if  ( zstr.length != 10) { 
		    document.form.mb003.focus(); return mb003;}	
}
