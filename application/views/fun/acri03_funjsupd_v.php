...
<script type="text/javascript"><!--
  function addacrq01a63(sma001,sma002) {	
	form.acrq01a63.value=sma001;
	var oSpan = document.getElementById("acrq01a63disp");
		oSpan.innerHTML = sma002;
	document.form.acrq01a63.focus();    
	return acrq01a63;	
}
//--></script>
...
<script type="text/javascript"><!--
  function addpurc09a(sma001,sma002) {
	...
	url = '<?=base_url() ?>index.php/acr/acri03/copybefore/'+encodeURIComponent(sma001)+'/'+encodeURIComponent(sma002); 
	location = url;
	return true;
}
//--></script>
...
function showacrq01a63(sText){                 //不更新網頁 31  請購單別 tc001
	var oSpan = document.getElementById("acrq01a63disp");
      //   chkno1();
     //   alert('test');		
...
function startacrq01a63(oInput){            
  //	if(!oInput.value){
...
//	}
	...
	createXMLHttpRequest();
   	...
   	var sUrl = "<?php echo base_url()?>index.php/fun/acrq01a/dataacrq01a63/" + encodeURIComponent(oInput.value)+ "/" + new Date().getTime();   
	...
	xmlHttp.onreadystatechange = function(){	    
		if(xmlHttp.readyState == 4 && xmlHttp.status == 200)					
		  showacrq01a63(xmlHttp.responseText);	//顯示服務器結果		
	}
...
function showchkno1(sText){   
       ...
       if  (!sText) { sText=$('input[name=\'tc017\']').val();   
...
//	}
	//建立非同步請求
	 ...
	 var zno=$('input[name=\'acrq01a63\']').val();
	 var zstr=$('input[name=\'tc017\']').val();
   ...
   //   alert(zstr);
	...
	createXMLHttpRequest();
   	var sUrl = "<?php echo base_url()?>index.php/acr/acri03/datachkno1/" + encodeURIComponent(zno)+ "/" + encodeURIComponent(zymd)+ "/" + new Date().getTime();   
	var QueryString = encodeURIComponent(oInput.value);
...
$(document).keydown(function(event){
	...
	if(event.altKey && (keycode == '89')){  //tab6 y
		setTimeout(function() {
			$('select[name=\'order_product[0][td004]\']').focus();
		}, 100);	
	}
	//新增一筆明細 alt+y 
...
//	html += '    <td class="left"><input type="text"  tabIndex="20"  name="order_product[' + product_row + '][td003]" value="0" size="6" /></td>';
	...
	html += '    <td class="center"><input type="text" tabIndex="58"   id="td010" onKeyPress="keyFunction()" name="order_product[' + product_row + '][td010]" value="" size="5" style="text-align:right;background-color:#EBEBE4" /></td>';
	 html += '    <td class="right"><input readonly="value" type="text" tabIndex="59"  id="td011" name="order_product[' + product_row + '][td011]" value="1" size="6" /></td>';
	...
	//本幣貨幣 游標停在 0 之後  
	$('input[name=\'order_product[' + product_row + '][td014]\']').focusout(function(){
	     var n = this.name.replace(/order_product\[(\d+)].*/, '$1'); 
	 //	totalSum1();
	});
	//本幣貨幣游標停在 0 之後 
    $('input[name=\'order_product[' + product_row + '][td015]\']').focusout(function(){
	    var n = this.name.replace(/order_product\[(\d+)].*/, '$1'); 
	 //	totalSum1();
	});
	//參考單號
	...
	Enterkey();
     $('select[name=\'order_product[' + product_row + '][td004]\']').focus();   //新增一筆
	product_row++;
}
//-->
</script>
