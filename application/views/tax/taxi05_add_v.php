...
<div id="content">  <!-- div-3 -->
  <div class="box"> <!-- div-4 -->
    <div class="heading">
      <h1><img src="<?php echo base_url()?>assets/image/order.png" alt="" /> 彚總申報資料建立作業 - 新增　　　</h1>
	...
	<a accesskey="x" id='cancel' name='cancel' href="<?php echo site_url('tax/taxi05/display'); ?>" class="button" ><span>返 回Alt+x</span><img src="<?php echo base_url()?>assets/image/png/back.png" /></a>
	
	</div>
	</div>
    <div class="content"> <!-- div-5 -->
	<form   class="cmxform" id="commentForm"  name="form"  method="post" enctype="multipart/form-data"  action="<?php echo base_url()?>index.php/tax/taxi05/addsave" >	
	
	<?php       
      // 頭部表格  isset 檢查變數
	  $date=date("Y/m/d");
	  if(!isset($mf001)) { $mf001=$this->input->post('mf001'); }
	  if(!isset($mf001disp)) { $mf001disp=$this->input->post('mf001disp'); }
	  $mf002=$this->input->post('mf002');
	  $mf003=$this->input->post('mf003');
	  $mf004=$this->input->post('mf004');
	  $mf005=$this->input->post('mf005');
	   if(!isset($mf006)) { $mf006=$date; }  else {$mf006=$this->input->post('mf006');}
	  $mf007=$this->input->post('mf007');
	  $mf008=$this->input->post('mf008');
	  $mf009=$this->input->post('mf009');
	  $mf010=$this->input->post('mf010');
	  if(!isset($mf011)) { $mf011='1'; }  else {$mf011=$this->input->post('mf011');}
	  ...
	  if(!isset($mf018)) { $mf018='2'; }  else {$mf018=$this->input->post('mf018');}
	?>
