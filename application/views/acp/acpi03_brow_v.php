<div class="box2"> <!-- div-1 -->
    <div class="heading">
     ...
     <!--  <div class="buttons"> -->
      ...
      <!-- <a onclick="$('form').submitb();"   class="button">印付款憑單</a>   -->
	  ...
	  <?PHP } ?>
	  <?PHP if (substr($this->session->userdata('sysmg006'),6,1)=='Y') { ?>
	  <a onclick="location = '<?php echo base_url()?>index.php/acp/acpi03/printdetailc'"  style="float:left"  accesskey="o"  class="button"><span>印付款憑單 o </span><img src="<?php echo base_url()?>assets/image/png/print1.png" /></a> 
	  <?PHP } ?>
	  <?PHP if (substr($this->session->userdata('sysmg006'),10,1)=='Y') { ?>
	  <!--<a onclick="location = '<?php echo base_url()?>index.php/acp/acpi03/exceldetail'"  style="float:left"  accesskey="l" class="button"><span>轉EXCEL檔 l </span><img src="<?php echo base_url()?>assets/image/png/excel.png" /></a>  -->
	  <?PHP } ?>
...
<?php 
	  $title_array = array(
		...
		'tc001' => array('sort_name'=>"tc001",'name'=>"付款單別",'width'=>"6%",'align'=>"left"),
		'mq002' => array('sort_name'=>"mq002",'name'=>"單別名稱",'width'=>"6%",'align'=>"left"),
		'tc002' => array('sort_name'=>"tc002",'name'=>"付款單號",'width'=>"6%",'align'=>"left"),
		'tc016' => array('sort_name'=>"tc016",'name'=>"單據日期",'width'=>"6%",'align'=>"left"),
		...
		//'tc015' => array('sort_name'=>"tc015",'name'=>"分錄碼",'width'=>"5%",'align'=>"left"),
		'see' => array('sort_name'=>"",'name'=>"查看管理",'width'=>"12%",'align'=>"center"),
		'edit' => array('sort_name'=>"",'name'=>"修改管理",'width'=>"12%",'align'=>"center"),
		'print' => array('sort_name'=>"",'name'=>"印付款憑單",'width'=>"12%",'align'=>"center")
	  );
    ?>	
  <div class="content"> <!-- div-2 -->
    <form action="<?php echo base_url()?>index.php/acp/acpi03/delete" method="post" enctype="multipart/form-data" id="form">
       <table class="list">      <!-- 表格開始 -->
	      ...
	      <?php
		    foreach($title_array as $key => $val){
			  ...
			  echo anchor("acp/acpi03/display_search/".$this->uri->segment(4,0)."/order?val=".$val['sort_name']." asc",$str);
			  $str = "<img src='".base_url()."assets/image/desc.png' />";
			  echo anchor("acp/acpi03/display_search/".$this->uri->segment(4,0)."/order?val=".$val['sort_name']." desc",$str);
	  ...
	  $filter_array = array(
		...
		'tc001' => array('filter_name'=>"tc001",'name'=>"付款單別",'size'=>"10",'align'=>"left"),
		'mq002' => array('filter_name'=>"mq002",'name'=>"單別名稱",'size'=>"10",'align'=>"left"),
		'tc002' => array('filter_name'=>"tc002",'name'=>"付款單號",'size'=>"10",'align'=>"left"),
		'tc016' => array('filter_name'=>"tc016",'name'=>"單據日期",'size'=>"10",'align'=>"left"),
		...
		//'tc015' => array('filter_name'=>"tc015",'name'=>"分錄碼",'size'=>"10",'align'=>"left"),
	  );
    ?> 
        ...
        <tr>
          ...
          <td class="left"><?php echo  $row->mq002.' '.$row->tc015;?></td>		  
		  <td class="left"><?php echo  $row->tc002;?></td>
		  <td class="left"><?php echo  substr($row->tc016,0,4).'/'.substr($row->tc016,4,2).'/'.substr($row->tc016,6,2);?></td>
		 ...
		 <!-- <td class="left"><?php echo  $row->tc015;?></td> -->	                 			
...
function filter() {
	...
	url = '<?php echo base_url() ?>index.php/acp/acpi03/display_search/0/and_where?key=' + encodeURIComponent(key) + 
...
function filtera() {
	...
	url = '<?php echo base_url() ?>index.php/acp/acpi03/display_search/0/or_where?key=' + encodeURIComponent(key) + 
	'&val=' + encodeURIComponent(val);
	location = url;
}
</script>