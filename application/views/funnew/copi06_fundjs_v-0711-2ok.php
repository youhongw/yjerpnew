...
function set_catcomplete(row){
	...
	//明細計算
	$('input[name=\'order_product[' + row + '][td008]\'],input[name=\'order_product[' + row + '][td011]\']').focusout(function() { 
		...
		var sumQty=0;index1=0;
        $(".total_qty").each(function(index, element) {
			qty1=$('input[name=\'order_product[' + index1 + '][td008]\']').val();
			console.log(qty1);
			console.log(index1);
			index1=index1+1;
			if (isNaN(qty1)) {qty1=0;}
			   sumQty +=parseFloat(qty1);			
			   console.log(sumQty);
    });	 	
		//totalSum();		
		console.log(index1);
		if (typeof($('input[name=\'order_product[' + index1 + '][td008]\']').val()) == 'undefined' ) {qty1=0;} else 
		    { qty1=$('input[name=\'order_product[' + index1 + '][td008]\']').val(); }
...
function totalSum() {
	...
	$(".total_qty").each(function(index, element) {
			qty1=$('input[name=\'order_product[' + index + '][td008]\']').val();
			if (isNaN(qty1)) {qty1=0;}
			   sumQty +=parseFloat(qty1);			
			   console.log(sumQty);
    });
