...
function check_title_no(){
	...
	$.ajax({
		method: "POST",
		url: "<?php echo base_url() ?>index.php/acp/acpi02/check_title_no",
		data: {
			acpi01: acpi01, 
			ta034: ta034
		}
	...
	.done(function( msg ) {
		if($('#acpi01disp').text()!=""&&$('#acpi01disp').text()!="查無資料")
		$('#ta002').val(msg);
	});
}

//查詢出貨單開視窗 puri09 //下拉選單$('.close').click($.unblockUI);
function set_catcomplete(row){
    $('#order_product\\['+row+'\\]\\[tb005\\]').catcomplete({
		...
		function(req, add){
			...
			console.log(smb001);
            $.ajax({
                url: '<?php echo base_url(); ?>index.php/pur/puri09/lookupd_puri09/'+encodeURIComponent(smb001), 
                cache: false, 				
                dataType: 'json',  
                type: 'POST',  
                data: req,
		...
		select: function(event, ui) {
			...
			if(ui.item.value!="查無資料"){
				...
				$('#order_product\\['+row+'\\]\\[tb006\\]').val(ui.item.value2);
				$('#order_product\\['+row+'\\]\\[tb007\\]').val(row);
				$('#order_product\\['+row+'\\]\\[tb008\\]').val(ui.item.value3);
				$('#order_product\\['+row+'\\]\\[tb009\\]').val(ui.item.value6);
				$('#order_product\\['+row+'\\]\\[tb015\\]').val(ui.item.value7);
				$('#order_product\\['+row+'\\]\\[tb016\\]').val(ui.item.value8);
				$('#order_product\\['+row+'\\]\\[tb017\\]').val(ui.item.value9);
				$('#order_product\\['+row+'\\]\\[tb018\\]').val(ui.item.value10);
			}
			return false;
		},
		
		change: function(event, ui) {
			$('#order_product\\['+row+'\\]\\[tb005\\]').attr('onchange','check_puri092d(this)');
			check_puri09d(row);  //1060713 新增
			//check_puri09d($('#order_product\\['+row+'\\]\\[tb004\\]').val());
			return false;
		}
	...
	//明細計算
	$('input[name=\'order_product[' + row + '][tb009]\'],input[name=\'order_product[' + row + '][tb010]\'],input[name=\'order_product[' + row + '][tb017]\'],input[name=\'order_product[' + row + '][tb018]\'],input[name=\'order_product[' + row + '][tb019]\'],input[name=\'order_product[' + row + '][tb020]\']').focusout(function() { 
		...
		$('input[name=\'order_product[' + n + '][tb009]\']').val(get_total); 
       //合計資料
		totalSum();		
	
	});
	//數量游標停在 0 之後 
	$('input[name=\'order_product[' + row + '][tb009]\']').focus(function(){
	...
	$('input[name=\'order_product[' + row + '][tb011]\']').focus(function() {
		...
		$('input[name=\'order_product[' + n + '][tb004]\']').val(); 
	});
	...
	$('#order'+row).click(function(){
		...
		if($('#copi01').val()=='') {alert('請先選擇廠商代號!');return;}
		
		$('#hp_ifmain').attr('src',"<?php echo base_url()?>index.php/pur/copi02/display_child/"+$("#copi01").val() );
		$.blockUI({
		css: {
			top: '15%',
			left: '25%',
...
function addcopi02disp(me001, me002, me003, me004, me005, me006, me007){
	...
	$('#order_product\\['+selected_row+'\\]\\[tb010\\]').val(me004); //單位
	$('#order_product\\['+selected_row+'\\]\\[tb011\\]').val(me005); //單價
	...
	$.ajax({
		method: "POST",
		url: "<?php echo base_url() ?>index.php/pur/copi02/clear_sql"
	});
}
...
function set_catcomplete2(row){
	console.log(row);
    $('#order_product\\['+row+'\\]\\[tb007\\]').catcomplete({
        autoFocus: false,
		delay: 1000,
        minLength: 1,  		
		source:
		function(req, add){
			var smb002= $('#order_product\\['+row+'\\]\\[tb007\\]').val();
			$('#order_product\\['+row+'\\]\\[tb007\\]').attr('onchange','');
            $.ajax({
		...
		select: function(event, ui) {
			clear_row(row);
			if(ui.item.value!="查無資料"){
				$('#order_product\\['+row+'\\]\\[tb007\\]').val(ui.item.value1);
				$('#order_product\\['+row+'\\]\\[tb007disp\\]').val(ui.item.value2);
			}
			return false;
		},
		change: function(event, ui) {
			...
			//check_cmsi05d($('#order_product\\['+row+'\\]\\[tb007\\]').val());
			return false;
		}
		//focus: function(event, ui) {
		//	return false;
...
function totalSum() {
    ...
    $(".total_price").each(function(index, element) {
			price=$('input[name=\'order_product[' + index1 + '][tb015]\']').val();
			index1=index1+1;
			if (isNaN(price)) {price=0;}
			   sumamt +=parseFloat(price);			
			//   console.log(sumamt);
    });
		if (typeof($('input[name=\'order_product[' + index1 + '][tb015]\']').val()) == 'undefined' ) {price=0;} else 
		    { price=$('input[name=\'order_product[' + index1 + '][tb015]\']').val(); }
    ...
    //end 訂單金額合計
	//原幣稅額 ta018
    $(".total_price1").each(function(index, element) {
			price=$('input[name=\'order_product[' + index2 + '][tb016]\']').val();
			index2=index2+1;
			if (isNaN(price)) {price=0;}
			   sumTax +=parseFloat(price);			
			//   console.log(sumamt);
    });
		if (typeof($('input[name=\'order_product[' + index2 + '][tb016]\']').val()) == 'undefined' ) {price=0;} else 
		    { price=$('input[name=\'order_product[' + index2 + '][tb016]\']').val(); }
		if (isNaN(price) ||   price == null || price=='' ) {price=0;}
			 sumTax +=parseFloat(price);
		 $('#ta029').val(sumTax);
	...
	$('#ta028').val(sumamt);
	$('#ta029').val(sumTax);
	$('#ta2829').val(Math.round(sumtot));  //合計金額
	  //  console.log(sumtot);
	  
	   //數量合計 ta041
	$(".total_qty").each(function(index, element) {
		    if (isNaN($('input[name=\'order_product[' + index3 + '][tb017]\']').val()) ) {qty=0;} else 
		    { qty=$('input[name=\'order_product[' + index3 + '][tb017]\']').val(); }
			index3=index3+1;
			if ( isNaN(qty)  ||   qty == null || qty=='' ) {qty=0;}
			   sumQty +=parseFloat(qty);			
			  // console.log(sumQty);
    });
		if (typeof($('input[name=\'order_product[' + index3 + '][tb017]\']').val()) == 'undefined' ) {qty=0;} else 
		    { qty=$('input[name=\'order_product[' + index3 + '][tb017]\']').val(); }
		if (isNaN(qty) ||   qty == null || qty=='' ) {qty=0;}
			 sumQty +=parseFloat(qty);
		 $('#ta037').val(sumQty);
   ...
   //總毛重合計 ta042
	$(".total_qty1").each(function(index, element) {
		    if (isNaN($('input[name=\'order_product[' + index4 + '][tb018]\']').val()) ) {qty1=0;} else 
		    { qty1=$('input[name=\'order_product[' + index4 + '][tb018]\']').val(); }
			index4=index4+1;
			if ( isNaN(qty1)  ||   qty1 == null || qty1=='' ) {qty1=0;}
			   sumQty1 +=parseFloat(qty1);			
			 //  console.log(sumQty1);
    });
		if (typeof($('input[name=\'order_product[' + index4 + '][tb018]\']').val()) == 'undefined' ) {qty1=0;} else 
		    { qty1=$('input[name=\'order_product[' + index4 + '][tb018]\']').val(); }
		if (isNaN(qty1) ||   qty1 == null || qty1=='' ) {qty1=0;}
			 sumQty1 +=parseFloat(qty1);
		 $('#ta038').val(sumQty1);
		// console.log(sumQty1);
    //end 總毛重合計
	var sumtot1 =Math.round(sumQty+sumQty1);
	$('#ta037').val(sumQty);
	$('#ta038').val(sumQty1);
	$('#ta3738').val(Math.round(sumtot1));  //合計金額
...
function del_detail(tb001,tb002,tb003,row){
	...
	$.ajax({
		method: "POST",
		url: "<?php echo base_url() ?>index.php/acp/acpi02/del_detail_ajax",
		data: { 
			tb001: tb001, 
			tb002: tb002,
			tb003: tb003
...
<script>
//查詢銷貨憑證視窗
function search_puri09d_window(row_obj){
	if($.isNumeric(row_obj)){row = row_obj;}
	else{var row = $(row_obj).parent().parent().parent()[0].id.substr(12);}
	console.log(row);
	selected_row = row;
	$.blockUI({
		...
		message: $('#divFpuri09d'),
		onOverlayClick: clear_puri09disp_sql
	});
	  $('.close').click($.unblockUI);
}
function addpuri09ddisp(tg001,tg002,tg014,tg1928,tg028,tg019,tg031,tg032){
	clear_row(selected_row);
	$('#order_product\\['+selected_row+'\\]\\[tb005\\]').val(tg001);
	$('#order_product\\['+selected_row+'\\]\\[tb006\\]').val(tg002);
	$('#order_product\\['+selected_row+'\\]\\[tb007\\]').val(selected_row);
	$('#order_product\\['+selected_row+'\\]\\[tb008\\]').val(tg014);
	$('#order_product\\['+selected_row+'\\]\\[tb009\\]').val(tg1928);
	$('#order_product\\['+selected_row+'\\]\\[tb015\\]').val(tg028);
	$('#order_product\\['+selected_row+'\\]\\[tb016\\]').val(tg019);
	$('#order_product\\['+selected_row+'\\]\\[tb017\\]').val(tg031);
	$('#order_product\\['+selected_row+'\\]\\[tb018\\]').val(tg032);
...
function mult_addpuri09ddisp(tg001,tg002,tg014,tg1928,tg028,tg019,tg031,tg032){
	console.log(mb001);
	console.log(current_count);
	$('#order_product\\['+current_count+'\\]\\[tb005\\]').val(tg001);
	$('#order_product\\['+current_count+'\\]\\[tb006\\]').val(tg002);
	$('#order_product\\['+current_count+'\\]\\[tb007\\]').val(current_count);
	$('#order_product\\['+current_count+'\\]\\[tb008\\]').val(tg014);
	$('#order_product\\['+current_count+'\\]\\[tb009\\]').val(tg1928);
	$('#order_product\\['+current_count+'\\]\\[tb015\\]').val(tg028);
	$('#order_product\\['+current_count+'\\]\\[tb016\\]').val(tg019);
	$('#order_product\\['+current_count+'\\]\\[tb017\\]').val(tg031);
	$('#order_product\\['+current_count+'\\]\\[tb018\\]').val(tg032);
	addItem();
}
function clear_puri09disp_sql(){
	$.unblockUI();
	$.ajax({
		method: "POST",
		url: "<?php echo base_url() ?>index.php/pur/puri09/clear_sql"
...
function mult_addacti03ddisp(mb001, mb002 ){
	console.log(mb001);
	console.log(current_count);
	$('#order_product\\['+current_count+'\\]\\[tb013\\]').val(mb001);
	$('#order_product\\['+current_count+'\\]\\[tb013disp\\]').val(mb002);
...
function addcmsi05ddisp(mb001, mb002){
	clear_row(selected_row);
	$('#order_product\\['+selected_row+'\\]\\[tb014\\]').val(mb001);
	$('#order_product\\['+selected_row+'\\]\\[tb014disp\\]').val(mb002);
	$('#order_product\\['+selected_row+'\\]\\[tb014\\]').focus();
	$.ajax({
		method: "POST",
...
function check_puri09d(row_obj){
	if($.isNumeric(row_obj)){row = row_obj;}
	else{var row = $(row_obj).parent().parent().parent()[0].id.substr(12);}
	var smb001 = $('#order_product\\['+row+'\\]\\[tb005\\]').val();
	if(!smb001){clear_row(row);return;}
	$.ajax({
		url: '<?php echo base_url(); ?>index.php/pur/puri09/lookupd2_puri09/'+encodeURIComponent(smb001), 
		cache: false, 				
		dataType: 'json',  
		type: 'POST',  
		...
		function(data){
			if(data.response =="true"){
				...
				$('#order_product\\['+row+'\\]\\[tb006\\]').val(data.message[0].value2);
				$('#order_product\\['+row+'\\]\\[tb008\\]').val(data.message[0].value3);
				$('#order_product\\['+row+'\\]\\[tb009\\]').val(data.message[0].value4);
				$('#order_product\\['+row+'\\]\\[tb017\\]').val(data.message[0].value5);
				$('#order_product\\['+row+'\\]\\[tb018\\]').val(data.message[0].value6);
				$('#order_product\\['+row+'\\]\\[tb019\\]').val(data.message[0].value7);
				$('#order_product\\['+row+'\\]\\[tb020\\]').val(data.message[0].value8);
			}else{
...
function check_cmsi05d(row_obj){
	...
	var smb001 = $('#order_product\\['+row+'\\]\\[tb014\\]').val();
	if(!smb001){clear_row(row);return;}
	$.ajax({
		...
		function(data){
			if(data.response =="true"){
				$('#order_product\\['+row+'\\]\\[tb014\\]').val(data.message[0].value1);
				$('#order_product\\['+row+'\\]\\[tb014disp\\]').val(data.message[0].value2);
			}else{
				$('#order_product\\['+row+'\\]\\[tb014\\]').val("查無資料");
			}
		}
	});
}
...
<!--開視窗 出貨憑證    -->
<div id="divFpuri09d" style="display:none;width:100%;height:100%;">
<div style="float:right;"><input type="button" class="close" value="close" /></div> 
<iframe src="<?php echo base_url()?>index.php/pur/puri09/display_child" allowTransparency="flase" name="ifmain" width="100%" height="100%" marginwidth="0" marginheight="0" frameborder="0"></iframe> 	   
</div>
<!--開視窗 科目代號    -->
