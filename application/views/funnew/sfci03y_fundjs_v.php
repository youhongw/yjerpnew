...
function check_title_no(){
	...
	$.ajax({
		method: "POST",
		url: "<?php echo base_url() ?>index.php/sfc/sfci03y/check_title_no",
		data: {
			sfci01: sfci01, 
			td008: td008
		}
	...
	.done(function( msg ) {
		if($('#sfci01disp').text()!=""&&$('#sfci01disp').text()!="查無資料")
		$('#td002').val(msg);
	});
}
...
function totalSum() {
    ...
    $(".total_price").each(function(index, element) {
		...
		if (typeof($('input[name=\'order_product[' + index1 + '][tc012]\']').val()) == 'undefined' ) {price=0;} else 
		    { price=$('input[name=\'order_product[' + index1 + '][tc012]\']').val(); }
...
//	if ($('select[name=\'tb016\']').val()=='1') {$('#tb029').val()=Math.round(sumamt/parseFloat(vtax));$('#tb030').val()=Math.round(sumamt-$('#tb029').val());}
    if ($('select[name=\'tb016\']').val()=='1') {$('#tb029').val(Math.round(sumamt/parseFloat(vtax)));temp1=Math.round(sumamt-$('#tb029').val());$('#tb030').val(temp1);}
	...
	$(".total_qty").each(function(index, element) {
		    if (isNaN($('input[name=\'order_product[' + index2 + '][tc008]\']').val()) ) {qty=0;} else 
		...
		if (typeof($('input[name=\'order_product[' + index2 + '][tc008]\']').val()) == 'undefined' ) {qty=0;} else 
		    { qty=$('input[name=\'order_product[' + index2 + '][tc008]\']').val(); }
		if (isNaN(qty) ||   qty == null || qty=='' ) {qty=0;}
			 sumQty +=parseFloat(qty);
		 $('#tb031').val(sumQty);
   ...
   //總毛重合計 tb043
	$(".total_qty1").each(function(index, element) {
		    if (isNaN($('input[name=\'order_product[' + index3 + '][tc030]\']').val()) ) {qty1=0;} else 
		...
		if (typeof($('input[name=\'order_product[' + index3 + '][tc030]\']').val()) == 'undefined' ) {qty1=0;} else 
		    { qty1=$('input[name=\'order_product[' + index3 + '][tc030]\']').val(); }
		if (isNaN(qty1) ||   qty1 == null || qty1=='' ) {qty1=0;}
			 sumQty1 +=parseFloat(qty1);
		 $('#tb043').val(sumQty1);
	...
	$(".total_qty2").each(function(index, element) {
		    if (isNaN($('input[name=\'order_product[' + index4 + '][tc031]\']').val()) ) {qty2=0;} else 
		...
		if (typeof($('input[name=\'order_product[' + index4 + '][tc031]\']').val()) == 'undefined' ) {qty2=0;} else 
		    { qty2=$('input[name=\'order_product[' + index4 + '][tc031]\']').val(); }
		if (isNaN(qty2) ||   qty2 == null || qty2=='' ) {qty2=0;}
			 sumQty2 +=parseFloat(qty2);
		 $('#tb044').val(sumQty2);
...
function del_detail(tc001,tc002,tc003,row){
	...
	.done(function( msg ) {
		...
		else{  alert ( "刪除細項:"+tc001+"-"+tc002+"-"+tc003+" 失敗!");}
	});
	}
}
