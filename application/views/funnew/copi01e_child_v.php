        ...
        <thead>
		  ...
		  <?php
			foreach($title_array as $key => $val){
				...
				echo anchor("cop/copi01/displaye_child/".$this->uri->segment(4,0)."/order?val=".$val['sort_name']." asc",$str);
				
				$str = "<img src='".base_url()."assets/image/desc.png' />";
				echo anchor("cop/copi01/displaye_child/".$this->uri->segment(4,0)."/order?val=".$val['sort_name']." desc",$str);
...
?>
        ...
        <tr onclick="parent.addcopi01edisp('<?php echo $row->ma001;?>','<?php echo $row->ma002;?>');parent.$.unblockUI();" >
		  ...
		  <td class="center"><a href="javascript:send_back_copi01e('<?php echo $row->ma001;?>','<?php echo $row->ma002;?>');">[ 選擇</a><img src="<?php echo base_url()?>assets/image/png/ok.png" />]</td>
		</tr>
		<?php $chkval += 1; ?>
		<?php endforeach;?>
        </tbody>		 
...
function send_back_copi01e(ma001, ma002){
	//alert('test1');
	window.parent.$.unblockUI();
	if(window.parent.addcopi01edisp){	//以此判斷各呼叫此網頁的方法是否存在藉以給予相對應的需求
		window.parent.addcopi01edisp(ma001,ma002);
		$.ajax({
			method: "POST",
...
function filter() {
	...
	url = '<?php echo base_url() ?>index.php/cop/copi01/displaye_child/0/and_where?key=' + encodeURIComponent(key) + 
