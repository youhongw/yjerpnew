...
function check_title_no(){
	...
	var tj012 = $('#tj012').val();
	var tj012 = tj012.substr(0,4)+tj012.substr(5,2)+tj012.substr(8,2);
	//alert(invi04);
	console.log(invi04);
	console.log(tj012);
	$.ajax({
		method: "POST",
		url: "<?php echo base_url() ?>index.php/inv/invi07/check_title_no",
		data: {
			invi04: invi04, 
			tj012: tj012
		}
...
function set_catcomplete(row){
    $('#order_product\\['+row+'\\]\\[tk004\\]').catcomplete({
        autoFocus: false,
		delay: 1000,
        minLength: 1,  		
		source:
		function(req, add){
			var smb001= $('#order_product\\['+row+'\\]\\[tk004\\]').val();
			$('#order_product\\['+row+'\\]\\[tk004\\]').attr('onchange','');
			console.log(smb001);
            $.ajax({
		...
		select: function(event, ui) {
			...
			if(ui.item.value!="查無資料"){
				$('#order_product\\['+row+'\\]\\[tk004\\]').val(ui.item.value1);
				$('#order_product\\['+row+'\\]\\[tk005\\]').val(ui.item.value2);
				$('#order_product\\['+row+'\\]\\[tk006\\]').val(ui.item.value3);
				$('#order_product\\['+row+'\\]\\[tk018\\]').val(ui.item.value4);
				$('#order_product\\['+row+'\\]\\[tk017\\]').val(ui.item.value5);
				$('#order_product\\['+row+'\\]\\[tk017disp\\]').val(ui.item.value6);
			}
			return false;
		},
		
		change: function(event, ui) {
			$('#order_product\\['+row+'\\]\\[tk004\\]').attr('onchange','check_invi02d(this)');
			check_invi02d(row);  //1060713 新增
			//check_invi02d($('#order_product\\['+row+'\\]\\[tk004\\]').val());
			return false;
		}
	...
	//明細計算
	$('input[name=\'order_product[' + row + '][tk007]\'],input[name=\'order_product[' + row + '][tk016]\'],input[name=\'order_product[' + row + '][tk019]\']').focusout(function() { 
      ...
      //  var input_3=$('input[name=\'order_product[' + n + '][tk026]\']').val()/100; 		
		var get_total=input_1*input_2;  
		//$('input[name=\'order_product[' + n + '][tk011]\']').val(get_total); 
       //合計資料
		totalSum();		
	
	});
	//數量游標停在 0 之後 
	$('input[name=\'order_product[' + row + '][tk007]\']').focus(function(){
	 ...
	 //單價  游標停在 0 之後
	$('input[name=\'order_product[' + row + '][tk016]\']').focus(function(){
		var real_value = $(this)[0].defaultValue;
		if ($(this).val()==real_value)
		    $(this).val(real_value);
		     if ($(this).val()=='0')
	...
	//預設預交日期
	$('input[name=\'order_product[' + row + '][tk019]\']').focus(function() {
		var n = this.name.replace(/order_product\[(\d+)].*/, '$1'); 
		var today = new Date();
       ...
       if(mm<10) {
        ...
        today =  yyyy + '/' + mm + '/' + dd;
		if ($('input[name=\'order_product[' + n + '][tk019]\']').val()=='') {
		$('input[name=\'order_product[' + n + '][tk019]\']').val(today); }
	});
	//備註
	$('input[name=\'order_product[' + row + '][tk022]\']').blur(function() {
		//$('input[name=\'order_product[' + row + '][tk004]\']').focus(); 
	});
...
function addpuri02disp(me001, me002, me003, me004, me005, me006, me007){
	...
	$('#order_product\\['+selected_row+'\\]\\[tk004\\]').val(me001); //品號
	$('#order_product\\['+selected_row+'\\]\\[tk005\\]').val(me002); //品名
	$('#order_product\\['+selected_row+'\\]\\[tk006\\]').val(me003); //規格
	$('#order_product\\['+selected_row+'\\]\\[tk008\\]').val(me004); //單位
	$('#order_product\\['+selected_row+'\\]\\[tk010\\]').val(me005); //單價
	$('#order_product\\['+selected_row+'\\]\\[tk012\\]').val(me006); //庫別
	$('#order_product\\['+selected_row+'\\]\\[tk012disp\\]').val(me007); //庫別名稱
	
	$('#order_product\\['+selected_row+'\\]\\[tk004\\]').focus();
	$.ajax({
		method: "POST",
...
function set_catcomplete2(row){
	console.log(row);
    $('#order_product\\['+row+'\\]\\[tk012\\]').catcomplete({
        autoFocus: false,
		delay: 1000,
        minLength: 1,  		
		source:
		function(req, add){
			var smb002= $('#order_product\\['+row+'\\]\\[tk012\\]').val();
			$('#order_product\\['+row+'\\]\\[tk012\\]').attr('onchange','');
            $.ajax({
		...
		select: function(event, ui) {
			clear_row(row);
			if(ui.item.value!="查無資料"){
				$('#order_product\\['+row+'\\]\\[tk017\\]').val(ui.item.value1);
				$('#order_product\\['+row+'\\]\\[tk017disp\\]').val(ui.item.value2);
			}
			return false;
		},
		change: function(event, ui) {
			...
			//check_cmsi03d($('#order_product\\['+row+'\\]\\[tk007\\]').val());
			return false;
		}
		//focus: function(event, ui) {
		//	return false;
...
function totalSum() {
    ...
    $(".total_price").each(function(index, element) {
			//price=$('input[name=\'order_product[' + index1 + '][tk016]\']').val();
			if (isNaN($('input[name=\'order_product[' + index1 + '][tk016]\']').val()) ) {price=0;} else 
		    { price=$('input[name=\'order_product[' + index1 + '][tk016]\']').val(); }
			index1=index1+1;
		...
		if (typeof($('input[name=\'order_product[' + index1 + '][tk016]\']').val()) == 'undefined' ) {price=0;} else 
		    { price=$('input[name=\'order_product[' + index1 + '][tk016]\']').val(); }
	...
	$(".total_qty").each(function(index, element) {
		    if (isNaN($('input[name=\'order_product[' + index2 + '][tk007]\']').val()) ) {qty=0;} else 
		    { qty=$('input[name=\'order_product[' + index2 + '][tk007]\']').val(); }
			index2=index2+1;
			if ( isNaN(qty)  ||   qty == null || qty=='' ) {qty=0;}
			   sumQty +=parseFloat(qty);			
			  // console.log(sumQty);
    });
		if (typeof($('input[name=\'order_product[' + index2 + '][tk007]\']').val()) == 'undefined' ) {qty=0;} else 
		    { qty=$('input[name=\'order_product[' + index2 + '][tk007]\']').val(); }
		if (isNaN(qty) ||   qty == null || qty=='' ) {qty=0;}
			 sumQty +=parseFloat(qty);
		 $('#tj007').val(sumQty);
...
<script>
function del_detail(tk001,tk002,tk003,row){
	if(confirm("確定刪除細項:"+tk001+"-"+tk002+"-"+tk003+"?")){
	$.ajax({
		method: "POST",
		url: "<?php echo base_url() ?>index.php/inv/invi07/del_detail_ajax",
		data: { 
			tk001: tk001, 
			tk002: tk002,
			tk003: tk003
	...
	.done(function( msg ) {
		if(msg){
		//	alert( "刪除細項:"+tk001+"-"+tk002+"-"+tk003+" 成功!");
			$("#product_row_"+row).remove();
			totalSum();
		//	current_count -=1;
		//	addItem();
		}
		else{  alert ( "刪除細項:"+tk001+"-"+tk002+"-"+tk003+" 失敗!");}
	});
	}
}
function clear_row(row_obj){
	...
	for(var k=4;k<=22;k++){//k的最大值請依照實際情況去調整，通常設為欄位數字最大者(即最後一個欄位)
		$('#product-row'+row+' input.order_product_tk00'+k).val("");
		$('#product-row'+row+' input.order_product_tk0'+k).val("");
...
$(document).keydown(function(event){
	...
	if(event.altKey && (keycode == '89')){  //tab6 y
		setTimeout(function() {
			$('input[name=\'order_product[1][tk004]\']').focus();
		}, 100);	
	}
	//新增一筆明細 alt+w 
...
function addinvi02ddisp(mb001, mb002, mb003, mb004, mb005, mb006){
	clear_row(selected_row);
	$('#order_product\\['+selected_row+'\\]\\[tk004\\]').val(mb001);
	$('#order_product\\['+selected_row+'\\]\\[tk005\\]').val(mb002);
	$('#order_product\\['+selected_row+'\\]\\[tk006\\]').val(mb003);
	$('#order_product\\['+selected_row+'\\]\\[tk018\\]').val(mb004);
	$('#order_product\\['+selected_row+'\\]\\[tk017\\]').val(mb005);
	$('#order_product\\['+selected_row+'\\]\\[tk017disp\\]').val(mb006);
	$('#order_product\\['+selected_row+'\\]\\[tk004\\]').focus();
	$.ajax({
		method: "POST",
...
function mult_addinvi02ddisp(mb001, mb002, mb003, mb004, mb005, mb006){
	...
	$('#order_product\\['+current_count+'\\]\\[tk004\\]').val(mb001);
	$('#order_product\\['+current_count+'\\]\\[tk005\\]').val(mb002);
	$('#order_product\\['+current_count+'\\]\\[tk006\\]').val(mb003);
	$('#order_product\\['+current_count+'\\]\\[tk018\\]').val(mb004);
	$('#order_product\\['+current_count+'\\]\\[tk017\\]').val(mb005);
	$('#order_product\\['+current_count+'\\]\\[tk017disp\\]').val(mb006);
	addItem();
}
...
function addcmsi03ddisp(mb001, mb002){
	clear_row(selected_row);
	$('#order_product\\['+selected_row+'\\]\\[tk017\\]').val(mb001);
	$('#order_product\\['+selected_row+'\\]\\[tk017disp\\]').val(mb002);
	$('#order_product\\['+selected_row+'\\]\\[tk017\\]').focus();
	$.ajax({
		method: "POST",
...
function check_invi02d(row_obj){
	...
	$.ajax({
		...
		function(data){
			if(data.response =="true"){
				$('#order_product\\['+row+'\\]\\[tk004\\]').val(data.message[0].value1);
				$('#order_product\\['+row+'\\]\\[tk005\\]').val(data.message[0].value2);
				$('#order_product\\['+row+'\\]\\[tk006\\]').val(data.message[0].value3);
				$('#order_product\\['+row+'\\]\\[tk018\\]').val(data.message[0].value4);
				$('#order_product\\['+row+'\\]\\[tk017\\]').val(data.message[0].value5);
				$('#order_product\\['+row+'\\]\\[tk017disp\\]').val(data.message[0].value6);
			}else{
				$('#order_product\\['+row+'\\]\\[tk004\\]').val("查無資料");
			}
		}
	});
}
...
function check_cmsi03d(row_obj){
	...
	var smb001 = $('#order_product\\['+row+'\\]\\[tk017\\]').val();
	if(!smb001){clear_row(row);return;}
	$.ajax({
		...
		function(data){
			if(data.response =="true"){
				$('#order_product\\['+row+'\\]\\[tk017\\]').val(data.message[0].value1);
				$('#order_product\\['+row+'\\]\\[tk017disp\\]').val(data.message[0].value2);
			}else{
				$('#order_product\\['+row+'\\]\\[tk017\\]').val("查無資料");
			}
		}
	});
}
