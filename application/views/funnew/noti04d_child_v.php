    ...
    <form method="post" enctype="multipart/form-data" id="form">
        <table class="list"> <!-- 表格開始 -->						
			<tr>
				...
				<input tabIndex="2"  ondblclick="scwShow(this,event);"   id="td003" onKeyPress="keyFunction()"  onchange="dateformat_ymd(this);" name="td003"  value="<?php echo $td003; ?>"  size="12" type="text" style="background-color:#FFFFE4" required="requierd"  />
				</td>
			</tr>
			<tr>
				<td class="normal14a" width="8%" >匯率： </td>  <!-- dateformat_ymd(this); -->
				<td class="normal14"  >
				<input type="text" id="exchange_rate"   tabIndex="12"   onKeyPress="keyFunction()"    name="tc009" value=""  size="12"  required="requierd"/>				
...
function check_submit(){
	...
	if(td003 == "" || exchange_rate == ""){
		alert('有欄位空值');
	}else{
		send_back_noti04d(td003, exchange_rate);
	}
}
function send_back_noti04d(smb001, smb002){
	window.parent.$.unblockUI();
	if(window.parent.addnoti04ddisp){
		window.parent.addnoti04ddisp(smb001,smb002);
		/*a.ajax({
			method:"POST",
...
<?php include_once("puri01_funmjs_v.php"); ?> <!-- 廠商 -->
