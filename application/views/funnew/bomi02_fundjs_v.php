...
function check_key(oInput){
	...
	$.ajax({
		method: "POST",
		url: "<?php echo base_url() ?>index.php/bom/bomi02/check_key",
		data: {
			mc001: mc001
		}
	})
...
function check_title_no(){
	...
	$.ajax({
		method: "POST",
		url: "<?php echo base_url() ?>index.php/bom/bomi02/check_title_no",
		data: {
			invi02: invi02
		}
	})
...
function import_copi05(mz001,mz002,mz003,mz004){
	...
	$.ajax({
		method: "POST",
		dataType: "json",
		url: "<?php echo base_url() ?>index.php/bom/bomi02/import_bomi02",
		data: {
			mz001: mz001,
            mz002: mz002,
            mz003: mz003,			
...
function totalSum() {
    ...
    $(".total_price").each(function(index, element) {
		...
		if (typeof($('input[name=\'order_product[' + index1 + '][md010]\']').val()) == 'undefined' ) {price=0;} else 
		    { price=$('input[name=\'order_product[' + index1 + '][md010]\']').val(); }
...
//	if ($('select[name=\'mc016\']').val()=='1') {$('#mc029').val()=Math.round(sumamt/parseFloat(vtax));$('#mc030').val()=Math.round(sumamt-$('#mc029').val());}
    if ($('select[name=\'mc022\']').val()=='1') {$('#mc009').val(Math.round(sumamt/parseFloat(vtax)));temp1=Math.round(sumamt-$('#mc009').val());$('#mc023').val(temp1);}
	...
	$(".total_qty").each(function(index, element) {
		    if (isNaN($('input[name=\'order_product[' + index2 + '][md007]\']').val()) ) {qty=0;} else 
		...
		if (typeof($('input[name=\'order_product[' + index2 + '][md007]\']').val()) == 'undefined' ) {qty=0;} else 
		    { qty=$('input[name=\'order_product[' + index2 + '][md007]\']').val(); }
		if (isNaN(qty) ||   qty == null || qty=='' ) {qty=0;}
			 sumQty +=parseFloat(qty);
		 $('#mc025').val(sumQty);
   ...
   //總毛重合計 mc043 modi 27 毛重30 modi 20
	$(".total_qty1").each(function(index, element) {
		    if (isNaN($('input[name=\'order_product[' + index3 + '][md020]\']').val()) ) {qty1=0;} else 
		...
		if (typeof($('input[name=\'order_product[' + index3 + '][md020]\']').val()) == 'undefined' ) {qty1=0;} else 
		    { qty1=$('input[name=\'order_product[' + index3 + '][md020]\']').val(); }
		if (isNaN(qty1) ||   qty1 == null || qty1=='' ) {qty1=0;}
			 sumQty1 +=parseFloat(qty1);
		 $('#mc027').val(sumQty1);
	...
	$(".total_qty2").each(function(index, element) {
		    if (isNaN($('input[name=\'order_product[' + index4 + '][md021]\']').val()) ) {qty2=0;} else 
		...
		if (typeof($('input[name=\'order_product[' + index4 + '][md021]\']').val()) == 'undefined' ) {qty2=0;} else 
		    { qty2=$('input[name=\'order_product[' + index4 + '][md021]\']').val(); }
		if (isNaN(qty2) ||   qty2 == null || qty2=='' ) {qty2=0;}
			 sumQty2 +=parseFloat(qty2);
		 $('#mc028').val(sumQty2);
...
function del_detail(md001,md002,md003,row){
	...
	$.ajax({
		method: "POST",
		url: "<?php echo base_url() ?>index.php/bom/bomi02/del_detail_ajax",
		data: { 
			md001: md001, 
			md002: md002,
			md003: md003
...
function addinvi02ddisp(mb001, mb002, mb003, mb004, mb005, mb006){
	...
	$('#order_product\\['+selected_row+'\\]\\[md003\\]').focus();
	$.ajax({
		method: "POST",
...
function check_invi02d(row_obj){
	...
	$.ajax({
		...
		function(data){
			if(data.response =="true"){
				$('#order_product\\['+row+'\\]\\[md003\\]').val(data.message[0].value1);
				$('#order_product\\['+row+'\\]\\[md003disp\\]').val(data.message[0].value2);
				$('#order_product\\['+row+'\\]\\[md003disp1\\]').val(data.message[0].value3);
				$('#order_product\\['+row+'\\]\\[md004\\]').val(data.message[0].value4);
			//	$('#order_product\\['+row+'\\]\\[md007\\]').val(data.message[0].value5);
			//	$('#order_product\\['+row+'\\]\\[md007disp\\]').val(data.message[0].value6);
			}else{
				$('#order_product\\['+row+'\\]\\[md003\\]').val("查無資料");
			}
		}
	});
}
