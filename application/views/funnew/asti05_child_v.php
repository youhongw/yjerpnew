        ...
        <thead>
		  ...
		  <?php
			foreach($title_array as $key => $val){
				...
				echo anchor("ast/asti02/display_child/".$this->uri->segment(4,0)."/order?val=".$val['sort_name']." asc",$str);
				
				$str = "<img src='".base_url()."assets/image/desc.png' />";
				echo anchor("ast/asti02/display_child/".$this->uri->segment(4,0)."/order?val=".$val['sort_name']." desc",$str);
...
?>
		...
		<!--  <td class="center"><a href="javascript:send_back_asti02('<?php echo $row->mq001;?>','<?php echo $row->mq002;?>');">[ 選擇</a><img src="<?php echo base_url()?>assets/image/png/ok.png" />]</td> -->
		   <td class="center"><a href="javascript:send_back_asti02('<?php echo $row->mb001;?>','<?php echo $row->mb002;?>','<?php echo $row->mb003;?>','<?php echo $row->mb011;?>','<?php echo $row->mb012;?>','<?php echo $row->mb020;?>','<?php echo $row->mb021;?>','<?php echo $row->mb029;?>');">[ 選擇</a><img src="<?php echo base_url()?>assets/image/png/ok.png" />]</td>
		</tr>
		<?php $chkval += 1; ?>
		<?php endforeach;?>
        </tbody>		 
...
function send_back_asti02(mb001, mb002, mb003, mb011, mb012, mb020, mb021, mb029){
	
	window.parent.$.unblockUI();
	if(window.parent.addasti02disp){	//以此判斷各呼叫此網頁的方法是否存在藉以給予相對應的需求
		window.parent.addasti02disp(mb001,mb002,mb003,mb011,mb012,mb020,mb021,mb029);
		$.ajax({
			method: "POST",
...
function filter() {
	...
	url = '<?php echo base_url() ?>index.php/ast/asti02/display_child/0/and_where?key=' + encodeURIComponent(key) + 
