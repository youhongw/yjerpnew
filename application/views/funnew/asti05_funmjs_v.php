...
//以下為單身function (asti08使用:部門會因資產影響)
//查詢因資產影響的部門代號視窗
function search_asti02_asti13_body_window(row_obj){
	if($.isNumeric(row_obj)){row = row_obj;}
	else{var row = $(row_obj).parent().parent().parent()[0].id.substr(12);}
	console.log(row);
	selected_row = row;
	...
	$("#iframe_asti02_asti13").attr('src','<?php echo base_url()?>index.php/ast/asti02/display_child_body_asti13/'+temp);
	...
	$.blockUI({
		...
		message: $('#divFasti02_asti13_body'),
		onOverlayClick: clear_asti02_asti13_body_sql
	});
	  $('.close').click($.unblockUI);
}
function addasti02_asti13_body(mc002, mc002disp, mc003, mc003disp, mc004){
	clear_row(selected_row);
	
	$('#order_product\\['+selected_row+'\\]\\[asti02_asti13\\]').val(mc002);
	$('#order_product\\['+selected_row+'\\]\\[asti02_asti13_mc002disp\\]').val(mc002disp);
	$('#order_product\\['+selected_row+'\\]\\[asti02_asti13_mc003\\]').val(mc003);
	$('#order_product\\['+selected_row+'\\]\\[asti02_asti13_mc003disp\\]').val(mc003disp);
	$('#order_product\\['+selected_row+'\\]\\[tg006\\]').val(mc004);
	
	
	$('#order_product\\['+selected_row+'\\]\\[asti02_asti13\\]').focus();
	$.ajax({
		method: "POST",
		url: "<?php echo base_url() ?>index.php/ast/asti02/clear_body_sql_asti13"
	});
}
function clear_asti02_asti13_body_sql(){
	$.unblockUI();
	$.ajax({
		method: "POST",
		url: "<?php echo base_url() ?>index.php/ast/asti02/clear_body_sql_asti13"
...
//直接輸入跳出中文 id.substr(12) 看几個欄位13 序號不算
function check_asti02_asti13_body(row_obj){
	if($.isNumeric(row_obj)){row = row_obj;}
	else{var row = $(row_obj).parent().parent().parent()[0].id.substr(12);}
	var smb001 = $('#order_product\\['+row+'\\]\\[asti02_asti13\\]').val();
	var smb002 = $('#order_product\\['+row+'\\]\\[asti02\\]').val();

	if(!smb001){
		$('#order_product\\['+row+'\\]\\[asti02_asti13\\]').val('');
		$('#order_product\\['+row+'\\]\\[asti02_asti13_mc002disp\\]').val('');
		$('#order_product\\['+row+'\\]\\[asti02_asti13_mc003\\]').val('');
		$('#order_product\\['+row+'\\]\\[asti02_asti13_mc003disp\\]').val('');
		$('#order_product\\['+row+'\\]\\[tg006\\]').val('');
		clear_row(row);return;
		}
	
	$.ajax({
		url: '<?php echo base_url(); ?>index.php/ast/asti02/lookup_body_check_asti13/'+encodeURIComponent(smb001)+'/'+encodeURIComponent(smb002), 
		cache: false, 				
		dataType: 'json',  
		type: 'POST',  
		data: {me001:row_obj.value},
		...
		function(data){
			if(data.response =="true"){
				$('#order_product\\['+row+'\\]\\[asti02_asti13\\]').val(data.message[0].value1);
				$('#order_product\\['+row+'\\]\\[asti02_asti13_mc002disp\\]').val(data.message[0].value2);
				$('#order_product\\['+row+'\\]\\[asti02_asti13_mc003\\]').val(data.message[0].value3);
				$('#order_product\\['+row+'\\]\\[asti02_asti13_mc003disp\\]').val(data.message[0].value4);
				$('#order_product\\['+row+'\\]\\[tg006\\]').val(data.message[0].value5);
				$('#order_product\\['+row+'\\]\\[asti02_asti13\\]').focus();
			}else{
				$('#order_product\\['+row+'\\]\\[asti02_asti13\\]').val("查無資料");
				$('#order_product\\['+row+'\\]\\[asti02_asti13\\]').focus();
			}
		}
	});
...
<div id="divFasti02_asti13_body" style="display:none;width:100%;height:100%;">
<div style="float:right;"><input type="button" class="close" value="close" /></div> 
<iframe id="iframe_asti02_asti13" src="" allowTransparency="flase" name="ifmain" width="100%" height="100%" marginwidth="0" marginheight="0" frameborder="0"></iframe> 	   
</div>
