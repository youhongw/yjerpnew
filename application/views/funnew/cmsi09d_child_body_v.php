	...
	<div class="content">
		...
		<?php $this->session->unset_userdata('mv002'); ?> -->
					...
					<?php foreach ($results as $row) : ?>
						<tr ondblclick="javascript:send_back_cmsi09('<?php echo trim($row->mv001); ?>','<?php echo trim($row->mv002); ?>');">
							<td class="center"><?php echo $chkval; ?></td>
							<td class="center"><?php echo $row->mv001; ?></td>
							<input id='mv001_<?php echo $chkval ?>' value='<?php echo $row->mv001; ?>' style='display:none;' />
							<td class="center"><?php echo trim($row->mv002); ?></td>
...
<script type="text/javascript">
	...
	function send_back_cmsi09(mv001, mv002) {
		...
		if (window.parent.addcmsi09d_body) { //以此判斷各呼叫此網頁的方法是否存在藉以給予相對應的需求

			window.parent.addcmsi09d_body(mv001, mv002);
			$.ajax({
				method: "POST",
	...
	function filter() {
		...
		url = '<?php echo base_url() ?>index.php/cms/cmsi09/display_child_bodyd/0/and_where?key=' + encodeURIComponent(key) +
