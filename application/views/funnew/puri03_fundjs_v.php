...
function check_title_no(){
	...
	$.ajax({
		method: "POST",
		url: "<?php echo base_url() ?>index.php/pur/puri03/check_title_no",
		data: {
			puri04: puri04, 
			tl010: tl010
		}
...
function set_catcomplete(row){
    $('#order_product\\['+row+'\\]\\[tm004\\]').catcomplete({
        autoFocus: false,
		delay: 1000,
        minLength: 1,  		
		source:
		function(req, add){
			var smb001= $('#order_product\\['+row+'\\]\\[tm004\\]').val();
		...
		select: function(event, ui) {
			...
			if(ui.item.value!="查無資料"){
				$('#order_product\\['+row+'\\]\\[tm004\\]').val(ui.item.value1);
				$('#order_product\\['+row+'\\]\\[tm005\\]').val(ui.item.value2);
				$('#order_product\\['+row+'\\]\\[tm006\\]').val(ui.item.value3);
				$('#order_product\\['+row+'\\]\\[tm009\\]').val(ui.item.value4);
				//$('#order_product\\['+row+'\\]\\[tm007\\]').val(ui.item.value5);
				//$('#order_product\\['+row+'\\]\\[tm007disp\\]').val(ui.item.value6);
			}
			return false;
		},
		
		change: function(event, ui) {
			$('#order_product\\['+row+'\\]\\[tm004\\]').attr('onchange','check_invi02d(this)');
			check_invi02d(row);  //1060713 新增
			//check_invi02d($('#order_product\\['+row+'\\]\\[tm004\\]').val());
			return false;
		}
	...
	$('input[name=\'order_product[' + row + '][tm007]\'],input[name=\'order_product[' + row + '][tm010]\']').focusout(function() { 
		var n = this.name.replace(/order_product\[(\d+)].*/, '$1'); 
		var input_1=$('input[name=\'order_product[' + n + '][tm010]\']').val()*1; 
      //  var input_3=$('input[name=\'order_product[' + n + '][tm026]\']').val()/100; 
        var input_3=1;	  
		var get_total=input_1;  
		$('input[name=\'order_product[' + n + '][tm010]\']').val(get_total); 
       //合計資料
		//totalSum();		
	 ...
	 //單價  游標停在 0 之後
	$('input[name=\'order_product[' + row + '][tm010]\']').focus(function(){
	...
	//預設預交日期13 改16生效日期
	$('input[name=\'order_product[' + row + '][tm014]\']').focus(function() {
		var n = this.name.replace(/order_product\[(\d+)].*/, '$1'); 
		var today = new Date();
       ...
       if(mm<10) {
        ...
        today =  yyyy + '/' + mm + '/' + dd;
		if ($('input[name=\'order_product[' + n + '][tm014]\']').val()=='') {
		$('input[name=\'order_product[' + n + '][tm014]\']').val(today); }
	});
...
function import_copi05(mz001,mz002,mz003,mz004){
	...
	$.ajax({
		method: "POST",
		dataType: "json",
		url: "<?php echo base_url() ?>index.php/pur/puri03/import_bomi02",
		data: {
			mz001: mz001,
            mz002: mz002,
            mz003: mz003,			
		...
		if(msg==0){
			...
			//if(confirm("共計"+msg.length+"筆，是否匯入?")){
				for(var key in msg){
					var val = msg[key];
					//addItem();
					$('#order_product\\['+current_count+'\\]\\[tm004\\]').val(val['md003']);
					$('#order_product\\['+current_count+'\\]\\[tm005\\]').val(val['mb002']);
					$('#order_product\\['+current_count+'\\]\\[tm006\\]').val(val['mb003']);
					$('#order_product\\['+current_count+'\\]\\[tm008\\]').val(val['md004']);
					$('#order_product\\['+current_count+'\\]\\[tm007\\]').val(val['md006']*mz002);
				    addItem();
				}
...
function addcopi02disp(me001, me002, me003, me004, me005, me006, me007){
	...
	$('#order_product\\['+selected_row+'\\]\\[tm004\\]').val(me001); //品號
	$('#order_product\\['+selected_row+'\\]\\[tm005\\]').val(me002); //品名
	$('#order_product\\['+selected_row+'\\]\\[tm006\\]').val(me003); //規格
	$('#order_product\\['+selected_row+'\\]\\[tm008\\]').val(me004); //單位
	$('#order_product\\['+selected_row+'\\]\\[tm009\\]').val(me005); //單價
//	$('#order_product\\['+selected_row+'\\]\\[tm007\\]').val(me006); //庫別
//	$('#order_product\\['+selected_row+'\\]\\[tm007disp\\]').val(me007); //庫別名稱
	
	$('#order_product\\['+selected_row+'\\]\\[tm004\\]').focus();
	$.ajax({
		method: "POST",
...
function set_catcomplete222(row){
	console.log(row);
    $('#order_product\\['+row+'\\]\\[tm007\\]').catcomplete({
        autoFocus: false,
		delay: 1000,
        minLength: 1,  		
		source:
		function(req, add){
			var smb002= $('#order_product\\['+row+'\\]\\[tm007\\]').val();
			$('#order_product\\['+row+'\\]\\[tm007\\]').attr('onchange','');
            $.ajax({
		...
		select: function(event, ui) {
			clear_row(row);
			if(ui.item.value!="查無資料"){
				$('#order_product\\['+row+'\\]\\[tm007\\]').val(ui.item.value1);
				$('#order_product\\['+row+'\\]\\[tm007disp\\]').val(ui.item.value2);
			}
			return false;
		},
		change: function(event, ui) {
			...
			//check_cmsi03d($('#order_product\\['+row+'\\]\\[tm007\\]').val());
			return false;
		}
		//focus: function(event, ui) {
		//	return false;
...
function totalSum() {
	...
	//訂單金額 tl029 modi tl009
    $(".total_price").each(function(index, element) {
			price=$('input[name=\'order_product[' + index1 + '][tm010]\']').val();
			index1=index1+1;
			if (isNaN(price)) {price=0;}
			   sumamt +=parseFloat(price);			
			//   console.log(sumamt);
    });
		if (typeof($('input[name=\'order_product[' + index1 + '][tm010]\']').val()) == 'undefined' ) {price=0;} else 
		    { price=$('input[name=\'order_product[' + index1 + '][tm010]\']').val(); }
	...
	//稅金 tl030 modi tl023  41稅率modi 24  課稅別16 modi 22
	tax=$('input[name=\'tl024\']').val();
	$('#tl023').val(Math.round(sumamt*tax));
	var sumTax =Math.round(sumamt*tax);
	var vtax=1+parseFloat(tax);
//	if ($('select[name=\'tl016\']').val()=='1') {$('#tl029').val()=Math.round(sumamt/parseFloat(vtax));$('#tl030').val()=Math.round(sumamt-$('#tl029').val());}
    if ($('select[name=\'tl022\']').val()=='1') {$('#tl009').val(Math.round(sumamt/parseFloat(vtax)));temp1=Math.round(sumamt-$('#tl009').val());$('#tl023').val(temp1);}
	var sumtot =Math.round(sumamt+sumTax);
	$('#tl009').val(sumamt);
	$('#tl023').val(sumTax);
	$('#tl0923').val(Math.round(sumtot));  //合計金額
	  //  console.log(sumtot);
	   //數量合計 tl031 modi 25   數量8modi 7
	$(".total_qty").each(function(index, element) {
		    if (isNaN($('input[name=\'order_product[' + index2 + '][tm007]\']').val()) ) {qty=0;} else 
		    { qty=$('input[name=\'order_product[' + index2 + '][tm007]\']').val(); }
			index2=index2+1;
			if ( isNaN(qty)  ||   qty == null || qty=='' ) {qty=0;}
			   sumQty +=parseFloat(qty);			
			   console.log(sumQty);
    });
		if (typeof($('input[name=\'order_product[' + index2 + '][tm007]\']').val()) == 'undefined' ) {qty=0;} else 
		    { qty=$('input[name=\'order_product[' + index2 + '][tm007]\']').val(); }
		if (isNaN(qty) ||   qty == null || qty=='' ) {qty=0;}
			 sumQty +=parseFloat(qty);
		 $('#tl025').val(sumQty);
   ...
   //總毛重合計 tl043 modi 27 毛重30 modi 20
	$(".total_qty1").each(function(index, element) {
		    if (isNaN($('input[name=\'order_product[' + index3 + '][tm020]\']').val()) ) {qty1=0;} else 
		    { qty1=$('input[name=\'order_product[' + index3 + '][tm020]\']').val(); }
			index3=index3+1;
			if ( isNaN(qty1)  ||   qty1 == null || qty1=='' ) {qty1=0;}
			   sumQty1 +=parseFloat(qty1);			
			 //  console.log(sumQty1);
    });
		if (typeof($('input[name=\'order_product[' + index3 + '][tm020]\']').val()) == 'undefined' ) {qty1=0;} else 
		    { qty1=$('input[name=\'order_product[' + index3 + '][tm020]\']').val(); }
		if (isNaN(qty1) ||   qty1 == null || qty1=='' ) {qty1=0;}
			 sumQty1 +=parseFloat(qty1);
		 $('#tl027').val(sumQty1);
	...
	//總材積合計 tl044 modi 28 31 modi 21
	$(".total_qty2").each(function(index, element) {
		    if (isNaN($('input[name=\'order_product[' + index4 + '][tm021]\']').val()) ) {qty2=0;} else 
		    { qty2=$('input[name=\'order_product[' + index4 + '][tm021]\']').val(); }
			index4=index4+1;
			if ( isNaN(qty2)  ||   qty2 == null || qty2=='' ) {qty2=0;}
			   sumQty2 +=parseFloat(qty2);			
			//   console.log(sumQty2);
    });
		if (typeof($('input[name=\'order_product[' + index4 + '][tm021]\']').val()) == 'undefined' ) {qty2=0;} else 
		    { qty2=$('input[name=\'order_product[' + index4 + '][tm021]\']').val(); }
		if (isNaN(qty2) ||   qty2 == null || qty2=='' ) {qty2=0;}
			 sumQty2 +=parseFloat(qty2);
		 $('#tl028').val(sumQty2);
...
<script>
function del_detail(tm001,tm002,tm003,row){
	if(confirm("確定刪除細項:"+tm001+"-"+tm002+"-"+tm003+"?")){
	$.ajax({
		method: "POST",
		url: "<?php echo base_url() ?>index.php/pur/puri03/del_detail_ajax",
		data: { 
			tm001: tm001, 
			tm002: tm002,
			tm003: tm003
	...
	.done(function( msg ) {
		if(msg){
		//	alert( "刪除細項:"+tm001+"-"+tm002+"-"+tm003+" 成功!");
			$("#product_row_"+row).remove();
			totalSum();
		//	current_count -=1;
		//	addItem();
		}
		else{  alert ( "刪除細項:"+tm001+"-"+tm002+"-"+tm003+" 失敗!");}
	});
	}
}
function clear_row(row_obj){
	...
	for(var k=4;k<=22;k++){//k的最大值請依照實際情況去調整，通常設為欄位數字最大者(即最後一個欄位)
		$('#product-row'+row+' input.order_product_tm00'+k).val("");
		$('#product-row'+row+' input.order_product_tm0'+k).val("");
...
$(document).keydown(function(event){
	...
	if(event.altKey && (keycode == '89')){  //tab6 y
		setTimeout(function() {
			$('input[name=\'order_product[1][tm004]\']').focus();
		}, 100);	
	}
	//新增一筆明細 alt+w 
...
function addinvi02ddisp(mb001, mb002, mb003, mb004, mb005, mb006){
	clear_row(selected_row);
	$('#order_product\\['+selected_row+'\\]\\[tm004\\]').val(mb001);
	$('#order_product\\['+selected_row+'\\]\\[tm005\\]').val(mb002);
	$('#order_product\\['+selected_row+'\\]\\[tm006\\]').val(mb003);
	$('#order_product\\['+selected_row+'\\]\\[tm009\\]').val(mb004);
	//$('#order_product\\['+selected_row+'\\]\\[tm007\\]').val(mb005);
	//$('#order_product\\['+selected_row+'\\]\\[tm007disp\\]').val(mb006);
	$('#order_product\\['+selected_row+'\\]\\[tm004\\]').focus();
	$.ajax({
		method: "POST",
...
function mult_addinvi02ddisp(mb001, mb002, mb003, mb004, mb005, mb006){
	...
	$('#order_product\\['+current_count+'\\]\\[tm004\\]').val(mb001);
	$('#order_product\\['+current_count+'\\]\\[tm005\\]').val(mb002);
	$('#order_product\\['+current_count+'\\]\\[tm006\\]').val(mb003);
	$('#order_product\\['+current_count+'\\]\\[tm009\\]').val(mb004);
	//$('#order_product\\['+current_count+'\\]\\[tm007\\]').val(mb005);
	//$('#order_product\\['+current_count+'\\]\\[tm007disp\\]').val(mb006);
	addItem();
}
...
function addcmsi03ddisp(mb001, mb002){
	clear_row(selected_row);
	$('#order_product\\['+selected_row+'\\]\\[tm007\\]').val(mb001);
	$('#order_product\\['+selected_row+'\\]\\[tm007disp\\]').val(mb002);
	$('#order_product\\['+selected_row+'\\]\\[tm007\\]').focus();
	$.ajax({
		method: "POST",
...
function check_invi02d(row_obj){
	...
	$.ajax({
		...
		function(data){
			if(data.response =="true"){
				$('#order_product\\['+row+'\\]\\[tm004\\]').val(data.message[0].value1);
				$('#order_product\\['+row+'\\]\\[tm005\\]').val(data.message[0].value2);
				$('#order_product\\['+row+'\\]\\[tm006\\]').val(data.message[0].value3);
				$('#order_product\\['+row+'\\]\\[tm009\\]').val(data.message[0].value4);
			//	$('#order_product\\['+row+'\\]\\[tm007\\]').val(data.message[0].value5);
			//	$('#order_product\\['+row+'\\]\\[tm007disp\\]').val(data.message[0].value6);
			}else{
				$('#order_product\\['+row+'\\]\\[tm004\\]').val("查無資料");
			}
		}
	});
}
...
function check_cmsi03d(row_obj){
	...
	var smb001 = $('#order_product\\['+row+'\\]\\[tm007\\]').val();
	if(!smb001){clear_row(row);return;}
	$.ajax({
		...
		function(data){
			if(data.response =="true"){
				$('#order_product\\['+row+'\\]\\[tm007\\]').val(data.message[0].value1);
				$('#order_product\\['+row+'\\]\\[tm007disp\\]').val(data.message[0].value2);
			}else{
				$('#order_product\\['+row+'\\]\\[tm007\\]').val("查無資料");
			}
		}
	});
}
