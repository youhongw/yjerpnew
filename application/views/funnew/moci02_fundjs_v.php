...
function check_title_no(){
	...
	$.ajax({
		method: "POST",
		url: "<?php echo base_url() ?>index.php/moc/moci02/check_title_no",
		data: {
			moci01: moci01, 
			ta003: ta003
		}
...
function set_catcomplete(row){
	...
	$('#order'+row).click(function(){
		...
		$('#hpa_ifmain').attr('src',"<?php echo base_url()?>index.php/bom/bomi02/editb_child/"+$("#invi02").val()+'/'+$('#ta015').val() );
		$.blockUI({
		css: {
			top: '15%',
			left: '25%',
...
//匯入明細圖2 
function import_moci02(mz001,mz002,mz003,mz004){
	console.log('mz001');
	console.log(mz001);
	$.ajax({
		method: "POST",
		dataType: "json",
		url: "<?php echo base_url() ?>index.php/moc/moci02/import_bomi02",
		data: {
			mz001: mz001,
            mz002: mz002,
            mz003: mz003,			
...
//	if ($('select[name=\'ta016\']').val()=='1') {$('#ta029').val()=Math.round(sumamt/parseFloat(vtax));$('#ta030').val()=Math.round(sumamt-$('#ta029').val());}
    if ($('select[name=\'ta018\']').val()=='1') {$('#ta019').val(Math.round(sumamt/parseFloat(vtax)));temp1=Math.round(sumamt-$('#ta019').val());$('#ta030').val(temp1);}
	...
	$(".total_qty").each(function(index, element) {
		    if (isNaN($('input[name=\'order_product[' + index2 + '][tb008]\']').val()) ) {qty=0;} else 
		...
		if (typeof($('input[name=\'order_product[' + index2 + '][tb008]\']').val()) == 'undefined' ) {qty=0;} else 
		    { qty=$('input[name=\'order_product[' + index2 + '][tb008]\']').val(); }
		if (isNaN(qty) ||   qty == null || qty=='' ) {qty=0;}
			 sumQty +=parseFloat(qty);
		 $('#ta023').val(sumQty);
   ...
   //總毛重合計 ta043
	$(".total_qty1").each(function(index, element) {
		    if (isNaN($('input[name=\'order_product[' + index3 + '][tb030]\']').val()) ) {qty1=0;} else 
		...
		if (typeof($('input[name=\'order_product[' + index3 + '][tb030]\']').val()) == 'undefined' ) {qty1=0;} else 
		    { qty1=$('input[name=\'order_product[' + index3 + '][tb030]\']').val(); }
		if (isNaN(qty1) ||   qty1 == null || qty1=='' ) {qty1=0;}
			 sumQty1 +=parseFloat(qty1);
		 $('#ta043').val(sumQty1);
	...
	$(".total_qty2").each(function(index, element) {
		    if (isNaN($('input[name=\'order_product[' + index4 + '][tb031]\']').val()) ) {qty2=0;} else 
		...
		if (typeof($('input[name=\'order_product[' + index4 + '][tb031]\']').val()) == 'undefined' ) {qty2=0;} else 
		    { qty2=$('input[name=\'order_product[' + index4 + '][tb031]\']').val(); }
		if (isNaN(qty2) ||   qty2 == null || qty2=='' ) {qty2=0;}
			 sumQty2 +=parseFloat(qty2);
		 $('#ta044').val(sumQty2);
...
function del_detail(tb001,tb002,tb003,row){
	...
	$.ajax({
		method: "POST",
		url: "<?php echo base_url() ?>index.php/moc/moci02/del_detail_ajax",
		data: { 
			tb001: tb001, 
			tb002: tb002,
			tb003: tb003
...
function addinvi02ddisp(mb001, mb002, mb003, mb004, mb005, mb006){
	...
	$('#order_product\\['+selected_row+'\\]\\[tb003\\]').focus();
	$.ajax({
		method: "POST",
...
function addcmsi03ddisp(mb001, mb002){
	...
	$('#order_product\\['+selected_row+'\\]\\[tb009\\]').focus();
	$.ajax({
		method: "POST",
	...
	$.ajax({
		...
		function(data){
			if(data.response =="true"){
				...
				$('#order_product\\['+row+'\\]\\[tb010\\]').val(data.message[0].value4);
