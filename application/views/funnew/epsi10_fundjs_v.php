...
function check_title_no(){
	...
	$.ajax({
		method: "POST",
		url: "<?php echo base_url() ?>index.php/eps/epsi10/check_title_no",
		data: {
			epsi01: epsi01, 
			th017: th017
		}
	...
	.done(function( msg ) {
		 console.log('kkkk');
		if($('#epsi01disp').text()!=""&&$('#epsi01disp').text()!="查無資料")
		$('#th002').val(msg);
	});
}
...
function totalSum() {
    ...
    $(".total_price").each(function(index, element) {
			price=$('input[name=\'order_product[' + index1 + '][tb013]\']').val();
			index1=index1+1;
			if (isNaN(price)) {price=0;}
			   sumamt +=parseFloat(price);			
			//   console.log(sumamt);
    });
		if (typeof($('input[name=\'order_product[' + index1 + '][tb013]\']').val()) == 'undefined' ) {price=0;} else 
		    { price=$('input[name=\'order_product[' + index1 + '][tb013]\']').val(); }
	...
	//稅金 tc030
	tax=$('input[name=\'ta020\']').val();
	tax=0.05;
	$('#ta068').val(Math.round(sumamt*tax));
...
//	if ($('select[name=\'tc016\']').val()=='1') {$('#tc029').val()=Math.round(sumamt/parseFloat(vtax));$('#tc030').val()=Math.round(sumamt-$('#tc029').val());}
    if ($('select[name=\'ta020\']').val()=='1') {$('#ta029').val(Math.round(sumamt/parseFloat(vtax)));temp1=Math.round(sumamt-$('#ta029').val());$('#ta068').val(temp1);}
	var sumtot =Math.round(sumamt+sumTax);
	$('#ta029').val(sumamt);
	$('#ta068').val(sumTax);
	$('#ta2968').val(Math.round(sumtot));  //合計金額
	  //  console.log(sumtot);
	   //數量合計 tc031
...
function del_detail(td001,td002,td003,row){
	...
	$.ajax({
		method: "POST",
		url: "<?php echo base_url() ?>index.php/eps/epsi10/del_detail_ajax",
		data: { 
			td001: td001, 
			td002: td002,
			td003: td003
