...
function check_title_no(){
	$('#te002').val("");
	var asti03_asti12 = $('#asti03_asti12').val();
	var te008 = $('#te008').val();
	//alert(copi03);
	$.ajax({
		method: "POST",
		url: "<?php echo base_url() ?>index.php/ast/asti12/check_title_no",
		data: {
			asti03_asti12: asti03_asti12, 
			te008: te008
		}
	})
	.done(function( msg ) {
		if($('#asti03_asti12disp').text()!=""&&$('#asti03_asti12disp').text()!="查無資料")
		$('#te002').val(msg);
	});
}
</script>
...
<script>
function del_detail(tf001,tf002,tf003,tf004,tf005,row){
	if(confirm("確定刪除細項:"+tf001+"-"+tf002+"-"+tf003+"?")){
	$.ajax({
		method: "POST",
		url: "<?php echo base_url() ?>index.php/ast/asti12/del_detail_ajax",
		data: { 
			tf001: tf001, 
			tf002: tf002,
			tf003: tf003,
	...
	.done(function( msg ) {
		...
		else{  alert ( "刪除細項:"+tf001+"-"+tf002+"-"+tf003+" 失敗!");}
	});
	}
}
...
function check_body_repeat(body){
	...
	var body_value2 = $('#order_product\\['+body_num+'\\]\\[asti02_asti12\\]').val();
	var body_value3 = $('#order_product\\['+body_num+'\\]\\[asti02_asti12_mc003\\]').val();
	var body_value4 = $('#order_product\\['+body_num+'\\]\\[tf006\\]').val();
	...
	$(".check_repeat").each(function(index, element) {	
		...
		if(body_value1 == $('#order_product\\['+temp_num+'\\]\\[asti02\\]').val() && body_value1 != '' && body_value2 == $('#order_product\\['+temp_num+'\\]\\[asti02_asti12\\]').val() && body_value2 != '' && body_value3 == $('#order_product\\['+temp_num+'\\]\\[asti02_asti12_mc003\\]').val() && body_value3 != '' ){
			repeat_num += 1;
		...
		if(repeat_num >= 2 && body_value1 != '' && body_value2 !='' && body_value3 !=''){
			$('#order_product\\['+body_num+'\\]\\[asti02_asti12\\]').val('');
			$('#order_product\\['+body_num+'\\]\\[asti02_asti12_mc002disp\\]').val('');
			$('#order_product\\['+body_num+'\\]\\[asti02_asti12_mc003\\]').val('');
			$('#order_product\\['+body_num+'\\]\\[asti02_asti12_mc003disp\\]').val('');
			$('#order_product\\['+body_num+'\\]\\[tf006\\]').val('');
			//1070815 check_asti12_asti14_body(body);
