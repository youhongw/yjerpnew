...
/* title欄位設定區域 */
	$title_array = array(
		...
		'tg001' => array('sort_name'=>"tg001",'name'=>"單別",'width'=>"8%",'align'=>"left"),
		'tg002' => array('sort_name'=>"tg002",'name'=>"單號",'width'=>"8%",'align'=>"left"),
	...
	//	'tg004' => array('sort_name'=>"tg004",'name'=>"部門代號",'width'=>"8%",'align'=>"left"),
		...
		'tg006' => array('sort_name'=>"tg006",'name'=>"外送數量",'width'=>"8%",'align'=>"left"),
		'tg008' => array('sort_name'=>"tg008",'name'=>"收回數量",'width'=>"8%",'align'=>"left"),
        ...
        <thead>
		  ...
		  <?php
			foreach($title_array as $key => $val){
				...
				echo anchor("ast/asti13/display_child_body_asti14/".$this->uri->segment(4,0)."/".$this->uri->segment(5,0)."/order?val=".$val['sort_name']." asc",$str);
				
				$str = "<img src='".base_url()."assets/image/desc.png' />";
				echo anchor("ast/asti13/display_child_body_asti14/".$this->uri->segment(4,0)."/".$this->uri->segment(5,0)."/order?val=".$val['sort_name']." desc",$str);
...
/* filter欄位設定區域 */
	$filter_array = array(
		...
		'tg001' => array('filter_name'=>"tg001",'name'=>"單別",'size'=>"8",'align'=>"left"),
		'tg002' => array('filter_name'=>"tg002",'name'=>"單號",'size'=>"8",'align'=>"left"),
		'te008' => array('filter_name'=>"te008",'name'=>"日期",'size'=>"8",'align'=>"left"),
		'tg003' => array('filter_name'=>"tg003",'name'=>"資產編號",'size'=>"9",'align'=>"left"),
		//'tg004' => array('filter_name'=>"tg004",'name'=>"部門代號",'size'=>"9",'align'=>"left"),
		'tg005' => array('filter_name'=>"tg005",'name'=>"保管人",'size'=>"8",'align'=>"left"),
		'tg006' => array('filter_name'=>"tg006",'name'=>"外送數量",'size'=>"9",'align'=>"left"),
		'tg008' => array('filter_name'=>"tg008",'name'=>"收回數量",'size'=>"9",'align'=>"left")
	);
?>
		...
		<!--session 變數取消 	  
		<?php $this->session->unset_userdata('tg001'); ?> -->
	    <?php $chkval=1; ?>               
		...
		$(document).ready(function(){
			...
			$('#go_clear_sql').attr('onclick','location='+"\'"+'<?php echo base_url()?>index.php/ast/asti13/clear_body_sql_asti14/'+temp_location+"/'");
		});
		</script>
        
		<tr onclick="parent.addasti13_asti14_body('<?php echo $row->tg001;?>','<?php echo $row->tg002;?>','<?php echo $row->tg003;?>','<?php echo $row->mb002;?>','<?php echo $row->mb003;?>','<?php echo $row->tg004;?>','<?php echo $row->me002;?>','<?php echo $row->tg005;?>','<?php echo $row->mv002;?>','<?php echo $row->tg006;?>','<?php echo $row->tg008;?>');parent.$.unblockUI();" >
		  ...
		  <input id='tg003_<?php echo $chkval?>' value='<?php echo $row->tg003;?>' style='display:none;' />
	<!--	  <td class="left"><?php echo $row->tg004;?></td>
		  ...
		  <input id='tg006_<?php echo $chkval?>' value='<?php echo $row->tg006;?>' style='display:none;' />
		  <td class="left"><?php echo $row->tg008;?></td>
		  <input id='tg008_<?php echo $chkval?>' value='<?php echo $row->tg008;?>' style='display:none;' />		  
		  
		  <input id='smb002' value='<?php echo $row->tg001;?>' style='display:none;' />
		<!--  <td class="center"><a href="javascript:send_back_cmsi09('<?php echo $row->tg001;?>','<?php echo $row->tg002;?>');">[ 選擇</a><img src="<?php echo base_url()?>assets/image/png/ok.png" />]</td> -->
		   <td class="center"><a href="javascript:send_back_asti13_asti14('<?php echo $row->tg001;?>','<?php echo $row->tg002;?>','<?php echo $row->tg003;?>','<?php echo $row->mb002;?>','<?php echo $row->mb003;?>','<?php echo $row->tg004;?>','<?php echo $row->me002;?>','<?php echo $row->tg005;?>','<?php echo $row->mv002;?>','<?php echo $row->tg006;?>','<?php echo $row->tg008;?>');">[ 選擇</a><img src="<?php echo base_url()?>assets/image/png/ok.png" />]</td>
		</tr>
		<?php $chkval += 1; ?>
		<?php endforeach;?>
        </tbody>		 
...
function send_back_asti13_asti14(tg001, tg002, tg003, mb002, mb003, tg004, me002, tg005, mv002, tg006, tg008){
	window.parent.$.unblockUI();
	if(window.parent.addasti13_asti14_body){	//以此判斷各呼叫此網頁的方法是否存在藉以給予相對應的需求
	  
		window.parent.addasti13_asti14_body(tg001, tg002, tg003, mb002, mb003, tg004, me002, tg005, mv002, tg006, tg008);
		$.ajax({
			method: "POST",
...
function filter() {
	...
	url = '<?php echo base_url() ?>index.php/ast/asti13/display_child_body_asti14/0/and_where?key=' + encodeURIComponent(key) + 
