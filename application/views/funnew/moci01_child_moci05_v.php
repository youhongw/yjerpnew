<div class="box2" style="height:95%"> <!-- div-1 -->
    <div class="heading"> 
      <h1><img src="<?php echo base_url()?>assets/image/order.png" alt="" /> 製令性質建立作業 - 瀏覽</h1>
      <a onclick="location = '<?php echo base_url()?>index.php/moc/moci01/clear_sql_moci05'"  style="float:left" accesskey="d" class="button"><span>清除查詢條件 d </span><img height="12" width="12" src="<?php echo base_url()?>assets/image/delete2.png" /></a>
	</div>
<?php 
/* title欄位設定區域 */
	$title_array = array(
		'rowid' => array('sort_name'=>"mq001",'name'=>"序號",'width'=>"",'align'=>"left",'use'=>"disable"),
		'mq001' => array('sort_name'=>"mq001",'name'=>"單據代號",'width'=>"",'align'=>"left"),
        ...
        <thead>
		  ...
		  <?php
			foreach($title_array as $key => $val){
				...
				echo anchor("moc/moci01/display_child_moci05/".$this->uri->segment(4,0)."/order?val=".$val['sort_name']." asc",$str);
				
				$str = "<img src='".base_url()."assets/image/desc.png' />";
				echo anchor("moc/moci01/display_child_moci05/".$this->uri->segment(4,0)."/order?val=".$val['sort_name']." desc",$str);
...
/* filter欄位設定區域 */
	$filter_array = array(
		...
		'mq001' => array('filter_name'=>"mq001",'name'=>"單據代號",'size'=>"8",'align'=>"left"),
...
?>
		...
		<!--  <td class="center"><a href="javascript:send_back_moci01('<?php echo $row->mq001;?>','<?php echo $row->mq002;?>');">[ 選擇</a><img src="<?php echo base_url()?>assets/image/png/eye.png" />]</td> -->
		   <td class="center"><a href="javascript:send_back_moci01_moci05('<?php echo $row->mq001;?>','<?php echo $row->mq002;?>');">[ 選擇</a><img src="<?php echo base_url()?>assets/image/png/eye.png" />]</td>
		</tr>
		<?php $chkval += 1; ?>
		<?php endforeach;?>
        </tbody>		 
...
function send_back_moci01_moci05(mq001, mq002){
	
	window.parent.$.unblockUI();
	if(window.parent.addmoci01_moci05disp){	//以此判斷各呼叫此網頁的方法是否存在藉以給予相對應的需求
		window.parent.addmoci01_moci05disp(mq001,mq002);
		$.ajax({
			method: "POST",
...
function filter() {
	...
	url = '<?php echo base_url() ?>index.php/moc/moci01/display_child_moci05/0/and_where?key=' + encodeURIComponent(key) + 
