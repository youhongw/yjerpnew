<div class="box2" style="height:95%">
	<!-- div-1 -->
	<div class="heading">
		<h1><img src="<?php echo base_url() ?>assets/image/order.png" alt="" />製令製程查詢作業m - 瀏覽　　　</h1>
		<a onclick="location = '<?php echo base_url() ?>index.php/sfc/sfci03m/clear_sql_sfcta'" style="float:left" accesskey="d" class="button"><span>清除查詢條件 d </span><img height="12" width="12" src="<?php echo base_url() ?>assets/image/delete2.png" /></a>
	</div>
	<?php
	/* title欄位設定區域 */
	$title_array = array(
		...
		<form method="post" enctype="multipart/form-data" id="form">
			<table class="list">
				...
				<thead>
					<tr>
						...
						foreach ($title_array as $key => $val) {
							...
							// echo anchor("sfc/sfci03m/display_child/" . $this->uri->segment(4, 0) . "/order?val=" . $val['sort_name'] . " asc", $str);

							// $str = "<img src='" . base_url() . "assets/image/desc.png' />";
							// echo anchor("sfc/sfci03m/display_child/" . $this->uri->segment(4, 0) . "/order?val=" . $val['sort_name'] . " desc", $str);
				...
				$filter_array = array(
					...
					'MW003' => array('filter_name' => "MW003", 'name' => "製程名稱", 'size' => "8", 'align' => "center", 'disabled' => "disabled"),
...
<script type="text/javascript">
	$(document).ready(function() {
		// 頁面加載完成後自動執行一次 clear_sql_sfcta
    $.ajax({
        url: '<?php echo base_url() ?>index.php/sfc/sfci03m/clear_sql_sfcta',
        type: 'GET',
        success: function(response) {
            console.log('clear_sql_sfcta 執行成功');
        },
        error: function(xhr, status, error) {
            console.log('clear_sql_sfcta 執行失敗: ' + error);
        }
    });
	...
	function send_back_sfci03m(TA001, TA002, TA003, TA004, MW003, TA006, MB002, MB003, MB004) {

		window.parent.$.unblockUI();
		if (window.parent.addsfci03mdisp) { //以此判斷各呼叫此網頁的方法是否存在藉以給予相對應的需求
			window.parent.addsfci03mdisp(TA001, TA002, TA003, TA004, MW003, TA006, MB002, MB003, MB004);
			$.ajax({
				method: "POST",
				url: "<?php echo base_url() ?>index.php/sfc/sfci03m/clear_sql_sfcta"
			});
	...
	function filter() {
		...
		url = '<?php echo base_url() ?>index.php/sfc/sfci03m/display_child/0/and_where?key=' + encodeURIComponent(key) +
