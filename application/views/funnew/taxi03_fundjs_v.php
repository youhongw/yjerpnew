...
var vform=$('select[name=\'mc004\']').attr('value');
    console.log(vform);
	mc008=$('#mc008').val();mc009=$('#mc009').val();
  if ((vform>=21 && vform <=28) && $('#mc009').val()!='') {$('#mc008').val(mc009);$('#mc009').val("");} 
  if ((vform>=31 && vform <=36) && $('#mc008').val()!='') {$('#mc009').val(mc008);$('#mc008').val("");}
}

function check_length(vinput){
	mc010=$('#mc010').val();
	console.log(mc010);
	if ( mc010.length!=10 ) {alert('發票號碼須輸入10碼');}
	$('#mc010').focus;
}
//檢查發票號碼
function check_vno(){
	...
	$.ajax({
		method: "POST",
		url: "<?php echo base_url() ?>index.php/tax/taxi03/check_vno",
		data: {
			mc010: mc010
		}
	})
	.done(function( msg ) {
		 console.log(msg);
		// console.log($('#mc021').val(msg));
		// console.log(kk);
		 if( mc010 == msg ) {alert('發票號碼重複');}
		 // kk=$('#mc214').val(str[0]);
		 // console.log($(kk);
...
function totalSum() {
	...
	mc012=$('select[name=\'mc012\']').val();
	if (mc012<=1) {tax=0.05} else {tax=0}
	//tax=$('input[name=\'mc226\']').val();
	$('#mc013').val(Math.round(sumamt*tax));
