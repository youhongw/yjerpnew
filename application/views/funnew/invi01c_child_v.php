        ...
        <thead>
		  ...
		  <?php
			foreach($title_array as $key => $val){
				...
				echo anchor("inv/invi01/displayc_child/".$this->uri->segment(4,0)."/order?val=".$val['sort_name']." asc",$str);
				
				$str = "<img src='".base_url()."assets/image/desc.png' />";
				echo anchor("inv/invi01/displayc_child/".$this->uri->segment(4,0)."/order?val=".$val['sort_name']." desc",$str);
...
?>
		...
		<!--  <td class="center"><a href="javascript:send_back_invi01('<?php echo $row->ma001;?>','<?php echo $row->ma002;?>');">[ 選擇</a><img src="<?php echo base_url()?>assets/image/png/ok.png" />]</td> -->
		   <td class="center"><a href="javascript:send_back_invi01c('<?php echo $row->ma002;?>','<?php echo $row->ma003;?>');">[ 選擇</a><img src="<?php echo base_url()?>assets/image/png/ok.png" />]</td>
		</tr>
		<?php $chkval += 1; ?>
		<?php endforeach;?>
        </tbody>		 
...
function send_back_invi01c(ma002, ma003){
	
	window.parent.$.unblockUI();
	if(window.parent.addinvi01cdisp){	//以此判斷各呼叫此網頁的方法是否存在藉以給予相對應的需求
		window.parent.addinvi01cdisp(ma002,ma003);
		$.ajax({
			method: "POST",
...
function filter() {
	...
	url = '<?php echo base_url() ?>index.php/inv/invi01/displayc_child/0/and_where?key=' + encodeURIComponent(key) + 
