<div class="box2" style="height:95%"> <!-- div-1 -->
    <div class="heading"> 
      <h1><img src="<?php echo base_url()?>assets/image/order.png" alt="" /> 保管人建立作業 - 瀏覽</h1>
...
/* title欄位設定區域 */
	$title_array = array(
		...
		'mc001' => array('sort_name'=>"mc001",'name'=>"部門代號",'width'=>"",'align'=>"left"),
		'mc002' => array('sort_name'=>"mc002",'name'=>"部門名稱",'width'=>"",'align'=>"left"),
		'mc003' => array('sort_name'=>"mc003",'name'=>"科目",'width'=>"",'align'=>"left"),
        ...
        <thead>
		  ...
		  <?php
			foreach($title_array as $key => $val){
				...
				echo anchor("ast/asti02/display_child_body_asti08a/".$this->uri->segment(4,0)."/".$this->uri->segment(5,0)."/order?val=".$val['sort_name']." asc",$str);
				
				$str = "<img src='".base_url()."assets/image/desc.png' />";
				echo anchor("ast/asti02/display_child_body_asti08a/".$this->uri->segment(4,0)."/".$this->uri->segment(5,0)."/order?val=".$val['sort_name']." desc",$str);
...
/* filter欄位設定區域 */
	$filter_array = array(
		...
		'mc001' => array('filter_name'=>"mc001",'name'=>"部門代號",'size'=>"8",'align'=>"left"),
		'mc002' => array('filter_name'=>"mc002",'name'=>"部門名稱",'size'=>"12",'align'=>"left"),
...
?>
		...
		$(document).ready(function(){
			...
			$('#go_clear_sql').attr('onclick','location='+"\'"+'<?php echo base_url()?>index.php/ast/asti02/clear_body_sql_asti08a/'+temp_location+"/'");
		});
		</script>
        
		<tr onclick="parent.addasti02_asti08a_body('<?php echo $row->mc001;?>','<?php echo $row->mc002;?>');parent.$.unblockUI();" >
		  ...
		  <input id='smb002' value='<?php echo $row->mv001;?>' style='display:none;' />
		<!--  <td class="center"><a href="javascript:send_back_cmsi09('<?php echo $row->mc001;?>','<?php echo $row->mc002;?>');">[ 選擇</a><img src="<?php echo base_url()?>assets/image/png/ok.png" />]</td> -->
		   <td class="center"><a href="javascript:send_back_asti02_asti08a('<?php echo $row->mc001;?>','<?php echo $row->mc002;?>');">[ 選擇</a><img src="<?php echo base_url()?>assets/image/png/ok.png" />]</td>
		</tr>
		<?php $chkval += 1; ?>
		<?php endforeach;?>
        </tbody>		 
...
function send_back_asti02_asti08a(mc001, mc002){
	window.parent.$.unblockUI();
	if(window.parent.addasti02_asti08a_body){	//以此判斷各呼叫此網頁的方法是否存在藉以給予相對應的需求
	  
		window.parent.addasti02_asti08a_body(mc001,mc002);
		$.ajax({
			method: "POST",
			url: "<?php echo base_url() ?>index.php/ast/asti02/clear_body_sql_asti08a"
		});
	}
}
