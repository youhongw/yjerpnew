...
//查詢訂單性質開視窗asti03_12 //下拉選單$('.close').click($.unblockUI);
$(document).ready(function(){
	$("#Showasti03_asti12disp").click(function() {
	$.blockUI({
		css: {
			top: '15%',
			left: '25%',
		...
		message: $('#divFasti03_asti12'),
		onOverlayClick: clear_asti03_asti13disp_sql
	});
	  $('.close').click($.unblockUI);
	});
    $('#asti03_asti12').catcomplete({
        autoFocus: true,
		delay: 1000,
        minLength: 1,  		
		source:
		function(req, add){
			var smb001= $('#asti03_asti12').val();
			console.log(smb001);
            $.ajax({
                url: '<?php echo base_url(); ?>index.php/ast/asti03/lookup_catcomplete_asti12/'+encodeURIComponent(smb001), 
                cache: false, 				
                dataType: 'json',  
                type: 'POST',  
                data: req,
		...
		select: function(event, ui) {
			if(ui.item.value!="查無資料"){
				$('#asti03_asti12').val(ui.item.value1);
				$('#asti03_asti12disp').text(ui.item.value2);
				//console.log($('#asti03').val(ui.item.value1));
				return false;
			}else{
				$('#asti03_asti12').val("");
				$('#asti03_asti12disp').text("查無資料");
				return false;
			}
...
function addasti03_asti12disp(smb001,smb002){
	$('#asti03_asti12').val(smb001);
	$('#asti03_asti12disp').text(smb002);
	$('#asti03_asti12').focus();
	//check_asti03(smb001);
	//console.log(smb001);
  	$.ajax({
...
//不更新網頁 輸入直接跳出中文
function check_asti03_asti12(row_obj){
	var smb001= $('#asti03_asti12').val();
	if(!smb001){$('#asti03_asti12disp').text("");return;}
	$.ajax({
		...
		function(data){
			if(data.response == "true"){
				if(data.message[0].value=="查無資料"){
					...
					$('#asti03_asti12').val(smb001);
					$('#asti03_asti12disp').text(data.message[0].value2);
				}
			}else{
				$('#asti03_asti12').val("");
				$('#asti03_asti12disp').text("查無資料");
...
<div id="divFasti03_asti12" style="display:none;width:100%;height:100%;">
<div style="float:right;"><input type="button" class="close" value="close" /></div> 
<iframe src="<?php echo base_url()?>index.php/ast/asti03/display_child_asti12" allowTransparency="flase" name="ifmain" width="100%" height="100%" marginwidth="0" marginheight="0" frameborder="0"></iframe> 	   
</div>

<script type="text/javascript"> 	
//查詢訂單性質開視窗asti03_13 //下拉選單$('.close').click($.unblockUI);
$(document).ready(function(){
...
<div style="float:right;"><input type="button" class="close" value="close" /></div> 
<iframe src="<?php echo base_url()?>index.php/ast/asti03/display_child_asti13" allowTransparency="flase" name="ifmain" width="100%" height="100%" marginwidth="0" marginheight="0" frameborder="0"></iframe> 	   
</div>
