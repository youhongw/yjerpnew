<div class="box2" style="height:95%">
	...
	<div class="heading">
		...
		<a onclick="location = '<?php echo base_url() ?>index.php/cms/cmsi03/clear_sqla'" style="float:left" accesskey="d" class="button"><span>清除查詢條件 d </span><img height="12" width="12" src="<?php echo base_url() ?>assets/image/delete2.png" /></a>
	</div>
	<?php
	/* title欄位設定區域 */
	$title_array = array(
	...
	<div class="content">
		...
		<form method="post" enctype="multipart/form-data" id="form">
			<table class="list">
				...
				<thead>
					<tr>
						...
						foreach ($title_array as $key => $val) {
							...
							echo anchor("cms/cmsi03/display_childa/" . $this->uri->segment(4, 0) . "/order?val=" . $val['sort_name'] . " asc", $str);

							$str = "<img src='" . base_url() . "assets/image/desc.png' />";
							echo anchor("cms/cmsi03/display_childa/" . $this->uri->segment(4, 0) . "/order?val=" . $val['sort_name'] . " desc", $str);
				...
				$filter_array = array(
					...
					'MC002' => array('filter_name' => "MC002", 'name' => "庫別名稱", 'size' => "12", 'align' => "center", 'disabled' => "disabled"),
		...
		<?php $this->session->unset_userdata('MC002'); ?> -->
					...
					<?php foreach ($results as $row) : ?>
						<tr ondblclick="javascript:send_back_cmsi03('<?php echo trim($row->MC001); ?>','<?php echo mb_convert_encoding($row->MC002, 'utf-8', 'big-5'); ?>');">
							...
							<td class="center"><?php echo mb_convert_encoding($row->MC002, 'utf-8', 'big-5'); ?></td>
							<input id='MC002_<?php echo $chkval ?>' value='<?php echo mb_convert_encoding($row->MC002, 'utf-8', 'big-5'); ?>' style='display:none;' />
							<!-- <td class="center"><?php echo $row->MC003; ?></td>
							<input id='MC003_<?php echo $chkval ?>' value='<?php echo $row->MC003; ?>' style='display:none;' /> -->
							<td class="center"><a href="javascript:send_back_cmsi03('<?php echo trim($row->MC001); ?>','<?php echo mb_convert_encoding($row->MC002, 'utf-8', 'big-5'); ?>');">[ 選擇</a><img src="<?php echo base_url() ?>assets/image/png/ok.png" />]</td>
						</tr>
						<?php $chkval += 1; ?>
					<?php endforeach; ?>
				</tbody>
...
<script type="text/javascript">
	...
	function send_back_cmsi03(MC001, MC002) {

		window.parent.$.unblockUI();
		if (window.parent.addcmsi03disp) { //以此判斷各呼叫此網頁的方法是否存在藉以給予相對應的需求
			window.parent.addcmsi03disp(MC001, MC002);
			$.ajax({
				method: "POST",
				url: "<?php echo base_url() ?>index.php/cms/cmsi03/clear_sqla"
			});
		}
	}
	...
	function filter() {
		...
		url = '<?php echo base_url() ?>index.php/cms/cmsi03/display_childa/0/and_where?key=' + encodeURIComponent(key) +
