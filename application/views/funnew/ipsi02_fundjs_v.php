...
function check_title_no(){
	...
	$.ajax({
		method: "POST",
		url: "<?php echo base_url() ?>index.php/ips/ipsi02/check_title_no",
		data: {
			ipsi06: ipsi06, 
			ta008: ta008
		}
	...
	.done(function( msg ) {
		 console.log('kkkk');
		if($('#ipsi06disp').text()!=""&&$('#ipsi06disp').text()!="查無資料")
		$('#ta002').val(msg);
	});
}
...
function del_detail(td001,td002,td003,row){
	...
	$.ajax({
		method: "POST",
		url: "<?php echo base_url() ?>index.php/ips/ipsi02/del_detail_ajax",
		data: { 
			td001: td001, 
			td002: td002,
			td003: td003
