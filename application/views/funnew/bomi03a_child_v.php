<div class="box2" style="height:95%"> <!-- div-1 -->
    <div class="heading"> 
      <h1><img src="<?php echo base_url()?>assets/image/order.png" alt="" /> 產品結構性質建立作業 - 瀏覽</h1>
      <a onclick="location = '<?php echo base_url()?>index.php/bom/bomi03/clear_sql'"  style="float:left" accesskey="d" class="button"><span>清除查詢條件 d </span><img height="12" width="12" src="<?php echo base_url()?>assets/image/delete2.png" /></a>
	</div>
<?php 
/* title欄位設定區域 */
	$title_array = array(
        ...
        <thead>
		  ...
		  <?php
			foreach($title_array as $key => $val){
				...
				echo anchor("bom/bomi03/displaya_child/".$this->uri->segment(4,0)."/order?val=".$val['sort_name']." asc",$str);
				
				$str = "<img src='".base_url()."assets/image/desc.png' />";
				echo anchor("bom/bomi03/displaya_child/".$this->uri->segment(4,0)."/order?val=".$val['sort_name']." desc",$str);
...
function send_back_bomi03(mc001, mc002){
	...
	if(window.parent.addbomi03disp){	//以此判斷各呼叫此網頁的方法是否存在藉以給予相對應的需求
		...
		$.ajax({
			method: "POST",
			url: "<?php echo base_url() ?>index.php/bom/bomi03/clear_sql"
		});
	}
}
...
function filter() {
	...
	url = '<?php echo base_url() ?>index.php/bom/bomi03/displaya_child/0/and_where?key=' + encodeURIComponent(key) + 
