...
function check_title_no(){
	...
	var td014 = $('#td014').val();
	var td014 = td014.substr(0,4)+td014.substr(5,2)+td014.substr(8,2);
	//alert(bomi03);
	console.log(bomi03);
	console.log(td014);
	$.ajax({
		method: "POST",
		url: "<?php echo base_url() ?>index.php/bom/bomi05/check_title_no",
		data: {
			bomi03: bomi03, 
			td014: td014
		}
...
function set_catcomplete(row){
    $('#order_product\\['+row+'\\]\\[te004\\]').catcomplete({
		...
		select: function(event, ui) {
			...
			if(ui.item.value!="查無資料"){
				...
				$('#order_product\\['+row+'\\]\\[te004disp\\]').val(ui.item.value2);
				$('#order_product\\['+row+'\\]\\[te004disp\\]').val(ui.item.value3);
				$('#order_product\\['+row+'\\]\\[te005\\]').val(ui.item.value4);
				//$('#order_product\\['+row+'\\]\\[te007\\]').val(ui.item.value5);
				//$('#order_product\\['+row+'\\]\\[te007disp\\]').val(ui.item.value6);
			}
			return false;
		},
		
		change: function(event, ui) {
			$('#order_product\\['+row+'\\]\\[te004\\]').attr('onchange','check_invi02d(this)');
			check_invi02d(row);  //1060713 新增
			//check_invi02d($('#order_product\\['+row+'\\]\\[te004\\]').val());
			return false;
		}
	...
	$('input[name=\'order_product[' + row + '][te008]\'],input[name=\'order_product[' + row + '][te011]\'],input[name=\'order_product[' + row + '][te012]\'],input[name=\'order_product[' + row + '][te200]\'],input[name=\'order_product[' + row + '][te201]\']').focusout(function() { 
		var n = this.name.replace(/order_product\[(\d+)].*/, '$1'); 
		var input_1=$('input[name=\'order_product[' + n + '][te008]\']').val()*1; 
        var input_3=$('input[name=\'order_product[' + n + '][te200]\']').val()*1; 
        	  
		var get_total=input_1*input_3;  
		$('input[name=\'order_product[' + n + '][te201]\']').val(get_total); 
       //合計資料
		totalSum();		
	 ...
	 //單價  游標停在 0 之後
	$('input[name=\'order_product[' + row + '][te008]\']').focus(function(){
	...
	//單價  游標停在 0 之後
	$('input[name=\'order_product[' + row + '][te200]\']').focus(function(){
		var real_value = $(this)[0].defaultValue;
		if ($(this).val()==real_value)
		    $(this).val(real_value);
		     if ($(this).val()=='0')
...
$(document).ready(function(){
	...
	//	$('#order'+row).click(function(){
		...
		$('#hpa_ifmain').attr('src',"<?php echo base_url()?>index.php/bom/bomi02/editb_child/"+$("#invi02").val()+'/'+$('#td007').val() );
		$.blockUI({
		css: {
			top: '15%',
			left: '25%',
...
function import_moci02(mz001,mz002,mz003,mz004,mz006,mz006disp){
	...
	$.ajax({
		method: "POST",
		dataType: "json",
		url: "<?php echo base_url() ?>index.php/bom/bomi05/import_bomi02",
		data: {
			mz001: mz001,
            mz002: mz002,
            mz003: mz003,			
	...
	.done(function( msg ) {
		...
		//	if(confirm("共計"+msg.length+"筆，是否匯入?")){
				for(var key in msg){
					var val = msg[key];
					//addItem();
					$('#order_product\\['+current_count+'\\]\\[te004\\]').val(val['md003']);
					$('#order_product\\['+current_count+'\\]\\[te004disp\\]').val(val['mb002']);
					$('#order_product\\['+current_count+'\\]\\[te004disp\\]').val(val['mb003']);
					$('#order_product\\['+current_count+'\\]\\[te005\\]').val(val['md004']);
					$('#order_product\\['+current_count+'\\]\\[te008\\]').val(val['md006']*mz002);
					$('#order_product\\['+current_count+'\\]\\[te007\\]').val(mz006);
					$('#order_product\\['+current_count+'\\]\\[te007disp\\]').val(mz006disp);
				    addItem();
				}
...
function addcopi02disp(me001, me002, me003, me004, me005, me006, me007){
	...
	$('#order_product\\['+selected_row+'\\]\\[te004\\]').val(me001); //品號
	$('#order_product\\['+selected_row+'\\]\\[te004disp\\]').val(me002); //品名
	$('#order_product\\['+selected_row+'\\]\\[te004disp1\\]').val(me003); //規格
	$('#order_product\\['+selected_row+'\\]\\[te005\\]').val(me004); //單位
//	$('#order_product\\['+selected_row+'\\]\\[te009\\]').val(me005); //單價
//	$('#order_product\\['+selected_row+'\\]\\[te007\\]').val(me006); //庫別
//	$('#order_product\\['+selected_row+'\\]\\[te007disp\\]').val(me007); //庫別名稱
	
	$('#order_product\\['+selected_row+'\\]\\[te004\\]').focus();
	$.ajax({
		method: "POST",
...
function set_catcomplete222(row){
	console.log(row);
    $('#order_product\\['+row+'\\]\\[te007\\]').catcomplete({
        autoFocus: false,
		delay: 1000,
        minLength: 1,  		
		source:
		function(req, add){
			var smb002= $('#order_product\\['+row+'\\]\\[te007\\]').val();
			$('#order_product\\['+row+'\\]\\[te007\\]').attr('onchange','');
            $.ajax({
		...
		select: function(event, ui) {
			clear_row(row);
			if(ui.item.value!="查無資料"){
				$('#order_product\\['+row+'\\]\\[te007\\]').val(ui.item.value1);
				$('#order_product\\['+row+'\\]\\[te007disp\\]').val(ui.item.value2);
			}
			return false;
		},
		change: function(event, ui) {
			...
			//check_cmsi03d($('#order_product\\['+row+'\\]\\[te007\\]').val());
			return false;
		}
		//focus: function(event, ui) {
		//	return false;
...
function totalSum() {
    ...
    $(".total_price").each(function(index, element) {
			price=$('input[name=\'order_product[' + index1 + '][te201]\']').val();
			index1=index1+1;
			if (isNaN(price)) {price=0;}
			   sumamt +=parseFloat(price);			
			   console.log(sumamt);
    });
		if (typeof($('input[name=\'order_product[' + index1 + '][te201]\']').val()) == 'undefined' ) {price=0;} else 
		    { price=$('input[name=\'order_product[' + index1 + '][te201]\']').val(); }
		if (isNaN(price) ||   price == null || price=='' ) {price=0;}
			 sumamt +=parseFloat(price);
		 $('#td201').val(sumamt);
		 var vtd007 = $('#td007').val();
		 sumamt1 = sumamt/parseFloat(vtd007);
		 $('#td200').val(sumamt1);
		  console.log(sumamt);
    //end 訂單金額合計
...
function del_detail(te001,te002,te003,row){
	...
	$.ajax({
		method: "POST",
		url: "<?php echo base_url() ?>index.php/bom/bomi05/del_detail_ajax",
		data: { 
			te001: te001, 
			te002: te002,
			te003: te003
...
$(document).keydown(function(event){
	...
	if(event.altKey && (keycode == '89')){  //tab6 y
		setTimeout(function() {
			$('input[name=\'order_product[1][te004]\']').focus();
		}, 100);	
	}
	//新增一筆明細 alt+w 
...
function addinvi02ddisp(mb001, mb002, mb003, mb004, mb005, mb006){
	...
	$('#order_product\\['+selected_row+'\\]\\[te004disp\\]').val(mb002);
	$('#order_product\\['+selected_row+'\\]\\[te004disp1\\]').val(mb003);
	$('#order_product\\['+selected_row+'\\]\\[te005\\]').val(mb004);
	//$('#order_product\\['+selected_row+'\\]\\[te007\\]').val(mb005);
	//$('#order_product\\['+selected_row+'\\]\\[te007disp\\]').val(mb006);
	$('#order_product\\['+selected_row+'\\]\\[te004\\]').focus();
	$.ajax({
		method: "POST",
...
function mult_addinvi02ddisp(mb001, mb002, mb003, mb004, mb005, mb006){
	...
	$('#order_product\\['+current_count+'\\]\\[te004disp\\]').val(mb002);
	$('#order_product\\['+current_count+'\\]\\[te004disp1\\]').val(mb003);
	$('#order_product\\['+current_count+'\\]\\[te005\\]').val(mb004);
	//$('#order_product\\['+current_count+'\\]\\[te007\\]').val(mb005);
	//$('#order_product\\['+current_count+'\\]\\[te007disp\\]').val(mb006);
	addItem();
}
...
function addcmsi03ddisp(mb001, mb002){
	clear_row(selected_row);
	$('#order_product\\['+selected_row+'\\]\\[te007\\]').val(mb001);
	$('#order_product\\['+selected_row+'\\]\\[te007disp\\]').val(mb002);
	...
	$.ajax({
		...
		function(data){
			if(data.response =="true"){
				...
				$('#order_product\\['+row+'\\]\\[te004disp\\]').val(data.message[0].value2);
				$('#order_product\\['+row+'\\]\\[te004disp\\]').val(data.message[0].value3);
				$('#order_product\\['+row+'\\]\\[te005\\]').val(data.message[0].value4);
			//	$('#order_product\\['+row+'\\]\\[te007\\]').val(data.message[0].value5);
			//	$('#order_product\\['+row+'\\]\\[te007disp\\]').val(data.message[0].value6);
			}else{
				$('#order_product\\['+row+'\\]\\[te004\\]').val("查無資料");
			}
		}
	});
}
...
function check_cmsi03d(row_obj){
	...
	$.ajax({
		...
		function(data){
			if(data.response =="true"){
				$('#order_product\\['+row+'\\]\\[te007\\]').val(data.message[0].value1);
				$('#order_product\\['+row+'\\]\\[te007disp\\]').val(data.message[0].value2);
			}else{
				$('#order_product\\['+row+'\\]\\[te007\\]').val("查無資料");
			}
		}
	});
}
