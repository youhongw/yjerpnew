...
function check_title_no(){
	...
	$.ajax({
		method: "POST",
		url: "<?php echo base_url() ?>index.php/ips/ipsi05/check_title_no",
		data: {
			ipsi06: ipsi06, 
			tg003: tg003
		}
...
function totalSum() {
    ...
    $(".total_price").each(function(index, element) {
			price=$('input[name=\'order_product[' + index1 + '][ti013]\']').val();
			index1=index1+1;
			if (isNaN(price)) {price=0;}
			   sumamt +=parseFloat(price);			
			//   console.log(sumamt);
    });
		if (typeof($('input[name=\'order_product[' + index1 + '][ti013]\']').val()) == 'undefined' ) {price=0;} else 
		    { price=$('input[name=\'order_product[' + index1 + '][ti013]\']').val(); }
	...
	//稅金 tc030
	tax=$('input[name=\'tg020\']').val();
	tax=0.05;
	$('#tg068').val(Math.round(sumamt*tax));
...
//	if ($('select[name=\'tc016\']').val()=='1') {$('#tc029').val()=Math.round(sumamt/parseFloat(vtax));$('#tc030').val()=Math.round(sumamt-$('#tc029').val());}
    if ($('select[name=\'tg020\']').val()=='1') {$('#tg029').val(Math.round(sumamt/parseFloat(vtax)));temp1=Math.round(sumamt-$('#tg029').val());$('#tg068').val(temp1);}
...
function del_detail(td001,td002,td003,row){
	...
	$.ajax({
		method: "POST",
		url: "<?php echo base_url() ?>index.php/ips/ipsi05/del_detail_ajax",
		data: { 
			td001: td001, 
			td002: td002,
			td003: td003
