...
function check_title_no(){
	...
	$.ajax({
		method: "POST",
		url: "<?php echo base_url() ?>index.php/cop/copi05/check_title_no",
		data: {
			copi03: copi03, 
			ta013: ta013
		}
...
function set_catcomplete(row){
	...
	$('input[name=\'order_product[' + row + '][tb007]\'],input[name=\'order_product[' + row + '][tb009]\'],input[name=\'order_product[' + row + '][tb010]\'],input[name=\'order_product[' + row + '][tb020]\'],input[name=\'order_product[' + row + '][tb021]\']').focusout(function() { 
	...
	$('input[name=\'order_product[' + row + '][tb009]\']').focus(function(){
	 ...
	 //毛重  游標停在 0 之後
	$('input[name=\'order_product[' + row + '][tb020]\']').focus(function(){
		var real_value = $(this)[0].defaultValue;
		if ($(this).val()==real_value)
		    $(this).val(real_value);
		     if ($(this).val()=='0')
	...
	//材積  游標停在 0 之後
	$('input[name=\'order_product[' + row + '][tb021]\']').focus(function(){
		var real_value = $(this)[0].defaultValue;
		if ($(this).val()==real_value)
		    $(this).val(real_value);
		     if ($(this).val()=='0')
	...
	//預設預交日期13 改16生效日期
	$('input[name=\'order_product[' + row + '][tb016]\']').focus(function() {
		var n = this.name.replace(/order_product\[(\d+)].*/, '$1'); 
		var today = new Date();
...
function import_copi05(mz001,mz002,mz003,mz004){
	...
	$.ajax({
		method: "POST",
		dataType: "json",
		url: "<?php echo base_url() ?>index.php/cop/copi05/import_bomi02",
		data: {
			mz001: mz001,
            mz002: mz002,
            mz003: mz003,			
		...
		if(msg==0){
			...
			//if(confirm("共計"+msg.length+"筆，是否匯入?")){
				for(var key in msg){
					...
					$('#order_product\\['+current_count+'\\]\\[tb008\\]').val(val['md004']);
...
function del_detail(tb001,tb002,tb003,row){
	...
	$.ajax({
		method: "POST",
		url: "<?php echo base_url() ?>index.php/cop/copi05/del_detail_ajax",
		data: { 
			tb001: tb001, 
			tb002: tb002,
			tb003: tb003
