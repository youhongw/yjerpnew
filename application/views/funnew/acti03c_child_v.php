...
?>
        ...
        <tr onclick="parent.addacti03cdisp('<?php echo $row->ma001;?>','<?php echo $row->ma003;?>');parent.$.unblockUI();" >
		  ...
		  <td class="center"><a href="javascript:send_back_acti03c('<?php echo $row->ma001;?>','<?php echo $row->ma003;?>');">[ 選擇</a><img src="<?php echo base_url()?>assets/image/png/ok.png" />]</td>
		</tr>
		<?php $chkval += 1; ?>
		<?php endforeach;?>
        </tbody>		 
...
function send_back_acti03c(ma001, ma003){
	//alert('test1');
	window.parent.$.unblockUI();
	if(window.parent.addacti03cdisp){	//以此判斷各呼叫此網頁的方法是否存在藉以給予相對應的需求
		window.parent.addacti03cdisp(ma001,ma003);
		$.ajax({
			method: "POST",
