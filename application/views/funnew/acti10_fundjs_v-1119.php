...
//查詢品名規格開視窗 acti03 //下拉選單$('.close').click($.unblockUI);
...
function totalSum() {
	...
	var index0=1;index1=0;index2=0;index3=0;index4=0;
	var price=0;qty=0;qty1=0;qty2=0;temp1=0;
	//借方金額 
   /* $(".total_qty").each(function(index, element) {
		 var acctdc=$('select[name=\'order_product[' + index0 + '][tb004]\']').attr('value');
		// console.log(acctdc);
		if  (acctdc=='1') {
			price=$('input[name=\'order_product[' + index1 + '][tb007]\']').val();
			index0=index0+1;
			index1=index1+1;
		...
		if (typeof($('input[name=\'order_product[' + index1 + '][tb007]\']').val()) == 'undefined' ) {price=0;} else 
		    { price=$('input[name=\'order_product[' + index1 + '][tb007]\']').val(); }
	...
	$(".total_qty").each(function(index, element) {
		...
		if (isNaN(qty) ||   qty == null || qty=='' ) {qty=0;}
		 ...
		 console.log(sumQty);
    //end 數量合計
	   if (typeof($('input[name=\'order_product[' + index2 + '][tb007]\']').val()) == 'undefined' ) {qty1=0;} else 
		    { qty1=$('input[name=\'order_product[' + index2 + '][tb007]\']').val(); }
		if (isNaN(qty1) ||   qty1 == null || qty1=='' ) {qty1=0;}
		//	 sumQty1 +=parseFloat(qty1);
		 $('#ta008').val(sumQty1);
...
function check_acti03d(row_obj){
	...
	$.ajax({
		url: '<?php echo base_url(); ?>index.php/act/acti03/lookupd2_acti03/'+encodeURIComponent(smb001), 
		cache: false, 				
		dataType: 'json',  
		type: 'POST',  
