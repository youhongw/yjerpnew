<div class="box2" style="height:95%">
	...
	<div class="heading">
		...
		<a onclick="location = '<?php echo base_url() ?>index.php/inv/invi02/clear_sql'" style="float:left" accesskey="d" class="button"><span>清除查詢條件 d </span><img height="12" width="12" src="<?php echo base_url() ?>assets/image/delete2.png" /></a>
	</div>
	<?php
	/* title欄位設定區域 */
	$title_array = array(
					...
					if ($numrow > 0) {
						foreach ($results as $row) : ?>
							<tr ondblclick="javascript:send_back_invi02('<?php echo trim($row->MB001); ?>','<?php echo stripslashes(htmlspecialchars(mb_convert_encoding(trim($row->MB002), 'utf-8', 'big-5'), ENT_QUOTES)); ?>','<?php echo stripslashes(htmlspecialchars(mb_convert_encoding(trim($row->MB003), 'utf-8', 'big-5'), ENT_QUOTES)); ?>','<?php echo mb_convert_encoding(trim($row->MB004), 'utf-8', 'big-5'); ?>','<?php echo trim($row->MB017); ?>','<?php echo mb_convert_encoding(trim($row->MB017disp), 'utf-8', 'big-5'); ?>');">
								...
								<td class="center"><?php echo stripslashes(htmlspecialchars(mb_convert_encoding($row->MB002, "utf-8", "big-5"), ENT_QUOTES)); ?></td>
								<input id='MB002_<?php echo $chkval ?>' value='<?php echo stripslashes(htmlspecialchars(mb_convert_encoding($row->MB002, "utf-8", "big-5"), ENT_QUOTES)); ?>' style='display:none;' />
								<td class="center"><?php echo stripslashes(htmlspecialchars(mb_convert_encoding($row->MB003, "utf-8", "big-5"), ENT_QUOTES)); ?></td>
								<input id='MB003_<?php echo $chkval ?>' value='<?php echo stripslashes(htmlspecialchars(mb_convert_encoding($row->MB003, "utf-8", "big-5"), ENT_QUOTES)); ?>' style='display:none;' />
								<td class="center"><?php echo stripslashes(htmlspecialchars(mb_convert_encoding($row->MB004, "utf-8", "big-5"), ENT_QUOTES)); ?></td>
								<input id='MB004_<?php echo $chkval ?>' value='<?php echo stripslashes(htmlspecialchars(mb_convert_encoding($row->MB004, "utf-8", "big-5"), ENT_QUOTES)); ?>' style='display:none;' />
...
<script type="text/javascript">
	...
	function send_back_invi02(MB001, MB002, MB003, MB004, MB017, MB017disp) {
		...
		if (window.parent.addinvi02adisp) { //以此判斷各呼叫此網頁的方法是否存在藉以給予相對應的需求
			window.parent.addinvi02adisp(MB001, MB002, MB003, MB004, MB017, MB017disp);
			$.ajax({
				method: "POST",
