...
function check_title_no(){
	...
	$.ajax({
		method: "POST",
		url: "<?php echo base_url() ?>index.php/eps/epsi08/check_title_no",
		data: {
			copi03: copi03, 
			tf039: tf039
		}
...
//查詢品名規格開視窗 epsi08 //下拉選單$('.close').click($.unblockUI);
function set_catcomplete(row){
    $('#order_product\\['+row+'\\]\\[tg004\\]').catcomplete({
		...
		select: function(event, ui) {
			...
			if(ui.item.value!="查無資料"){
				...
				$('#order_product\\['+row+'\\]\\[tg010\\]').val(ui.item.value4);
	...
	//明細計算
	$('input[name=\'order_product[' + row + '][tg008]\'],input[name=\'order_product[' + row + '][tg011]\'],input[name=\'order_product[' + row + '][tg026]\'],input[name=\'order_product[' + row + '][tg030]\'],input[name=\'order_product[' + row + '][tg031]\']').focusout(function() { 
        ...
        var input_3=$('input[name=\'order_product[' + n + '][tg026]\']').val()/100; 		
	...
	$('input[name=\'order_product[' + row + '][tg008]\']').focus(function(){
	 ...
	 //單價  游標停在 0 之後
	$('input[name=\'order_product[' + row + '][tg011]\']').focus(function(){
		var real_value = $(this)[0].defaultValue;
		if ($(this).val()==real_value)
		    $(this).val(real_value);
		     if ($(this).val()=='0')
	...
	//預設預交日期
	$('input[name=\'order_product[' + row + '][tg013]\']').focus(function() {
		var n = this.name.replace(/order_product\[(\d+)].*/, '$1'); 
		var today = new Date();
       ...
       if(mm<10) {
        ...
        today =  yyyy + '/' + mm + '/' + dd;
		if ($('input[name=\'order_product[' + n + '][tg013]\']').val()=='') {
		$('input[name=\'order_product[' + n + '][tg013]\']').val(today); }
	});
	...
	$('#order'+row).click(function(){
		...
		$('#hp_ifmain').attr('src',"<?php echo base_url()?>index.php/eps/copi02/display_child/"+$("#copi01").val() );
		$.blockUI({
		css: {
			top: '15%',
			left: '25%',
...
function addcopi02disp(me001, me002, me003, me004, me005, me006, me007){
	...
	$('#order_product\\['+selected_row+'\\]\\[tg005\\]').val(me002); //品名
	$('#order_product\\['+selected_row+'\\]\\[tg006\\]').val(me003); //規格
	$('#order_product\\['+selected_row+'\\]\\[tg010\\]').val(me004); //單位
	$('#order_product\\['+selected_row+'\\]\\[tg011\\]').val(me005); //單價
	...
	$.ajax({
		method: "POST",
		url: "<?php echo base_url() ?>index.php/eps/copi02/clear_sql"
	});
}
...
function totalSum() {
    ...
    $(".total_price").each(function(index, element) {
			price=$('input[name=\'order_product[' + index1 + '][tg012]\']').val();
			index1=index1+1;
			if (isNaN(price)) {price=0;}
			   sumamt +=parseFloat(price);			
			//   console.log(sumamt);
    });
		if (typeof($('input[name=\'order_product[' + index1 + '][tg012]\']').val()) == 'undefined' ) {price=0;} else 
		    { price=$('input[name=\'order_product[' + index1 + '][tg012]\']').val(); }
	...
	//稅金 tf030
	tax=$('input[name=\'tf041\']').val();
	$('#tf030').val(Math.round(sumamt*tax));
	var sumTax =Math.round(sumamt*tax);
	var vtax=1+parseFloat(tax);
//	if ($('select[name=\'tf016\']').val()=='1') {$('#tf029').val()=Math.round(sumamt/parseFloat(vtax));$('#tf030').val()=Math.round(sumamt-$('#tf029').val());}
    if ($('select[name=\'tf016\']').val()=='1') {$('#tf029').val(Math.round(sumamt/parseFloat(vtax)));temp1=Math.round(sumamt-$('#tf029').val());$('#tf030').val(temp1);}
	...
	$(".total_qty").each(function(index, element) {
		    if (isNaN($('input[name=\'order_product[' + index2 + '][tg008]\']').val()) ) {qty=0;} else 
		    { qty=$('input[name=\'order_product[' + index2 + '][tg008]\']').val(); }
			index2=index2+1;
			if ( isNaN(qty)  ||   qty == null || qty=='' ) {qty=0;}
			   sumQty +=parseFloat(qty);			
			  // console.log(sumQty);
    });
		if (typeof($('input[name=\'order_product[' + index2 + '][tg008]\']').val()) == 'undefined' ) {qty=0;} else 
		    { qty=$('input[name=\'order_product[' + index2 + '][tg008]\']').val(); }
		if (isNaN(qty) ||   qty == null || qty=='' ) {qty=0;}
			 sumQty +=parseFloat(qty);
		 $('#tf031').val(sumQty);
   ...
   //總毛重合計 tf043
	$(".total_qty1").each(function(index, element) {
		    if (isNaN($('input[name=\'order_product[' + index3 + '][tg030]\']').val()) ) {qty1=0;} else 
		    { qty1=$('input[name=\'order_product[' + index3 + '][tg030]\']').val(); }
			index3=index3+1;
			if ( isNaN(qty1)  ||   qty1 == null || qty1=='' ) {qty1=0;}
			   sumQty1 +=parseFloat(qty1);			
			 //  console.log(sumQty1);
    });
		if (typeof($('input[name=\'order_product[' + index3 + '][tg030]\']').val()) == 'undefined' ) {qty1=0;} else 
		    { qty1=$('input[name=\'order_product[' + index3 + '][tg030]\']').val(); }
		if (isNaN(qty1) ||   qty1 == null || qty1=='' ) {qty1=0;}
			 sumQty1 +=parseFloat(qty1);
		 $('#tf043').val(sumQty1);
	...
	$(".total_qty2").each(function(index, element) {
		    if (isNaN($('input[name=\'order_product[' + index4 + '][tg031]\']').val()) ) {qty2=0;} else 
		    { qty2=$('input[name=\'order_product[' + index4 + '][tg031]\']').val(); }
			index4=index4+1;
			if ( isNaN(qty2)  ||   qty2 == null || qty2=='' ) {qty2=0;}
			   sumQty2 +=parseFloat(qty2);			
			//   console.log(sumQty2);
    });
		if (typeof($('input[name=\'order_product[' + index4 + '][tg031]\']').val()) == 'undefined' ) {qty2=0;} else 
		    { qty2=$('input[name=\'order_product[' + index4 + '][tg031]\']').val(); }
		if (isNaN(qty2) ||   qty2 == null || qty2=='' ) {qty2=0;}
			 sumQty2 +=parseFloat(qty2);
		 $('#tf044').val(sumQty2);
...
function del_detail(tg001,tg002,tg003,row){
	...
	$.ajax({
		method: "POST",
		url: "<?php echo base_url() ?>index.php/eps/epsi08/del_detail_ajax",
		data: { 
			tg001: tg001, 
			tg002: tg002,
			tg003: tg003
...
function addinvi02ddisp(mb001, mb002, mb003, mb004, mb005, mb006){
	...
	$('#order_product\\['+selected_row+'\\]\\[tg010\\]').val(mb004);
...
function mult_addinvi02ddisp(mb001, mb002, mb003, mb004, mb005, mb006){
	...
	$('#order_product\\['+current_count+'\\]\\[tg005\\]').val(mb002);
	$('#order_product\\['+current_count+'\\]\\[tg006\\]').val(mb003);
	$('#order_product\\['+current_count+'\\]\\[tg010\\]').val(mb004);
	...
	$.ajax({
		...
		function(data){
			if(data.response =="true"){
				...
				$('#order_product\\['+row+'\\]\\[tg010\\]').val(data.message[0].value4);
...
<!-- <iframe src="<?php echo base_url()?>index.php/eps/copi02/display_child/"+$("#copi01").val() allowTransparency="flase" id="hp_ifmain" name="ifmain" width="100%" height="100%" marginwidth="0" marginheight="0" frameborder="0"></iframe> -->	   
</div>