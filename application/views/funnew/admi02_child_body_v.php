<div class="box2" style="height:95%"> <!-- div-1 -->
    <div class="heading"> 
      ...
      <a onclick="location = '<?php echo base_url()?>index.php/adm/admi02/clear_body_sql'"  style="float:left" accesskey="d" class="button"><span>清除查詢條件 d </span><img height="12" width="12" src="<?php echo base_url()?>assets/image/delete2.png" /></a>
	</div>
<?php 
/* title欄位設定區域 */
	$title_array = array(
		'rowid' => array('sort_name'=>"mb001",'name'=>"序號",'width'=>"",'align'=>"left",'use'=>"disable"),
		'mb001' => array('sort_name'=>"mb001",'name'=>"程式代號",'width'=>"",'align'=>"left"),
		'mb002' => array('sort_name'=>"mb002",'name'=>"程式名稱",'width'=>"",'align'=>"left"),
        ...
        <thead>
		  ...
		  <?php
			foreach($title_array as $key => $val){
				...
				echo anchor("adm/admi02/display_child_body/".$this->uri->segment(4,0)."/order?val=".$val['sort_name']." asc",$str);
				
				$str = "<img src='".base_url()."assets/image/desc.png' />";
				echo anchor("adm/admi02/display_child_body/".$this->uri->segment(4,0)."/order?val=".$val['sort_name']." desc",$str);
...
/* filter欄位設定區域 */
	$filter_array = array(
		...
		'mb001' => array('filter_name'=>"mb001",'name'=>"程式代號",'size'=>"8",'align'=>"left"),
		'mb002' => array('filter_name'=>"mb002",'name'=>"程式名稱",'size'=>"12",'align'=>"left")
	);
?>
	    ...
	    <?php foreach($results as $row ) : ?>
         <tr onclick="parent.addadmi02_body('<?php echo $row->mb001;?>','<?php echo $row->mb002;?>');parent.$.unblockUI();" >
		  ...
		  <td class="center"><a href="javascript:send_back_admi02('<?php echo $row->mb001;?>','<?php echo $row->mb002;?>');">[ 選擇</a><img src="<?php echo base_url()?>assets/image/png/ok.png" />]</td>
		</tr>
		<?php $chkval += 1; ?>
		<?php endforeach;?>
        </tbody>		 
...
function send_back_admi02(mb001, mb002){
	//alert('test1');
	window.parent.$.unblockUI();
	if(window.parent.addadmi02_body){	//以此判斷各呼叫此網頁的方法是否存在藉以給予相對應的需求
		window.parent.addadmi02_body(mb001,mb002);
		$.ajax({
			method: "POST",
...
function filter() {
	...
	url = '<?php echo base_url() ?>index.php/adm/admi02/display_child_body/0/and_where?key=' + encodeURIComponent(key) + 
