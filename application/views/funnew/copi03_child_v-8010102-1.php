<div class="box2" style="height:95%"> <!-- div-1 -->
    <div class="heading"> 
      ...
      <a onclick="location = '<?php echo base_url()?>index.php/cop/copi03/clear_sql'"  style="float:left" accesskey="d" class="button"><span>清除查詢條件 d </span><img height="12" width="12" src="<?php echo base_url()?>assets/image/delete2.png" /></a>
	</div>
<?php 
/* title欄位設定區域 */
	$title_array = array(
        ...
        <thead>
		  ...
		  <?php
			foreach($title_array as $key => $val){
				...
				echo anchor("cop/copi03/display_child/".$this->uri->segment(4,0)."/order?val=".$val['sort_name']." asc",$str);
				
				$str = "<img src='".base_url()."assets/image/desc.png' />";
				echo anchor("cop/copi03/display_child/".$this->uri->segment(4,0)."/order?val=".$val['sort_name']." desc",$str);
...
?>
        ...
        <tr onclick="parent.addcopi03disp('<?php echo $row->mq001;?>','<?php echo $row->mq002;?>');parent.$.unblockUI();" >
		...
		<!--  <td class="center"><a href="javascript:send_back_copi03('<?php echo $row->mq001;?>','<?php echo $row->mq002;?>');">[ 選擇</a><img src="<?php echo base_url()?>assets/image/png/ok.png" />]</td> -->
		   <td class="center"><a href="javascript:send_back_copi03('<?php echo $row->mq001;?>','<?php echo $row->mq002;?>');">[ 選擇</a><img src="<?php echo base_url()?>assets/image/png/ok.png" />]</td>
		</tr>
		<?php $chkval += 1; ?>
		<?php endforeach;?>
...
function send_back_copi03(mc001, mc002){
	
	window.parent.$.unblockUI();
	if(window.parent.addcopi03disp){	//以此判斷各呼叫此網頁的方法是否存在藉以給予相對應的需求
		window.parent.addcopi03disp(mc001,mc002);
		$.ajax({
			method: "POST",
...
function filter() {
	...
	url = '<?php echo base_url() ?>index.php/cop/copi03/display_child/0/and_where?key=' + encodeURIComponent(key) + 
