...
function check_cmsi09_body(row_obj){
	...
	$.ajax({
		...
		function(data){
			if(data.response =="true"){
				$('#order_product\\['+row+'\\]\\[cmsi09\\]').val(data.message[0].value1);
				$('#order_product\\['+row+'\\]\\[cmsi09_mv002\\]').val(data.message[0].value2);

				$('#order_product\\['+row+'\\]\\[cmsi09\\]').focus();
			}else{
...
function addcmsi09d_body(mv001, mv002){
	...
	$('#order_product\\['+selected_row+'\\]\\[cmsi09d\\]').focus();
	$.ajax({
		method: "POST",
		url: "<?php echo base_url() ?>index.php/cms/cmsi09/clear_body_sql"
	});
...
function check_cmsi09d(row_obj){
	...
	$.ajax({
		url: '<?php echo base_url(); ?>index.php/cms/cmsi09/lookup_body_check/'+encodeURIComponent(smb001)+'/'+encodeURIComponent(smb002), 
		cache: false, 				
		dataType: 'json',  
		type: 'POST',  
		data: {me001:row_obj.value},
		...
		function(data){
			if(data.response =="true"){
				console.log('ok');
				console.log(data.message[0].value2);
				$('#order_product\\['+row+'\\]\\[cmsi09d\\]').val(data.message[0].value1);
				$('#order_product\\['+row+'\\]\\[cmsi09ddisp\\]').val(data.message[0].value2);

				$('#order_product\\['+row+'\\]\\[cmsi09d\\]').focus();
			}else{
				$('#order_product\\['+row+'\\]\\[cmsi09d\\]').val("查無資料");
				$('#order_product\\['+row+'\\]\\[cmsi09d\\]').focus();
			}
		}
	});
