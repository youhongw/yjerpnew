<div class="box2" style="height:95%"> <!-- div-1 -->
    <div class="heading"> 
      ...
      <a onclick="location = '<?php echo base_url()?>index.php/acp/acpi03/clear_sql'"  style="float:left" accesskey="d" class="button"><span>清除查詢條件 d </span><img height="12" width="12" src="<?php echo base_url()?>assets/image/delete2.png" /></a>
	</div>
<?php 
/* title欄位設定區域 */
	$title_array = array(
		'rowid' => array('sort_name'=>"td001",'name'=>"序號",'width'=>"",'align'=>"left",'use'=>"disable"),
		'td001' => array('sort_name'=>"td001",'name'=>"付款單別",'width'=>"",'align'=>"left"),
		'td002' => array('sort_name'=>"td002",'name'=>"付款單號",'width'=>"",'align'=>"left"),
        ...
        <thead>
		  ...
		  <?php
			foreach($title_array as $key => $val){
				...
				echo anchor("acp/acpi03/display_child/".$this->uri->segment(4,0)."/order?val=".$val['sort_name']." asc",$str);
				
				$str = "<img src='".base_url()."assets/image/desc.png' />";
				echo anchor("acp/acpi03/display_child/".$this->uri->segment(4,0)."/order?val=".$val['sort_name']." desc",$str);
...
/* filter欄位設定區域 */
	$filter_array = array(
		...
		'td001' => array('filter_name'=>"td001",'name'=>"付款單別",'size'=>"8",'align'=>"left"),
		'td002' => array('filter_name'=>"td002",'name'=>"付款單號",'size'=>"8",'align'=>"left"),
...
?>
		...
		<!--session 變數取消 	  
		<?php $this->session->unset_userdata('ma003'); ?> -->
	    <?php $chkval=1; ?>               
	    <?php foreach($results as $row ) : ?>
        <tr onclick="parent.addacpi03disp('<?php echo $row->td001;?>','<?php echo $row->td002;?>','<?php echo $row->td003;?>');parent.$.unblockUI();" >
		  ...
		  <td class="center"><a href="javascript:send_back_acpi03('<?php echo $row->td001;?>','<?php echo $row->td002;?>','<?php echo $row->td003;?>');">[ 選擇</a><img src="<?php echo base_url()?>assets/image/png/ok.png" />]</td>
		</tr>
		<?php $chkval += 1; ?>
		<?php endforeach;?>
        </tbody>		 
...
function send_back_acpi03(td001, td002, td003){
	//alert('test1');
	window.parent.$.unblockUI();
	if(window.parent.addacpi03disp){	//以此判斷各呼叫此網頁的方法是否存在藉以給予相對應的需求
		window.parent.addacpi03disp(td001,td002,td003);
		$.ajax({
			method: "POST",
...
function filter() {
	...
	url = '<?php echo base_url() ?>index.php/acp/acpi03/display_child/0/and_where?key=' + encodeURIComponent(key) + 
