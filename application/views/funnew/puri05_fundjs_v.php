...
function check_title_no(){
	...
	var ta013 = $('#ta013').val();
	var ta013 = ta013.substr(0,4)+ta013.substr(5,2)+ta013.substr(8,2);
	//alert(puri04);
	console.log(puri04);
	console.log(ta013);
	$.ajax({
		method: "POST",
		url: "<?php echo base_url() ?>index.php/pur/puri05/check_title_no",
		data: {
			puri04: puri04, 
			ta013: ta013
		}
...
//查詢品名規格開視窗 puri05 //下拉選單$('.close').click($.unblockUI);
function set_catcomplete(row){
    $('#order_product\\['+row+'\\]\\[tb004\\]').catcomplete({
		...
		select: function(event, ui) {
			...
			if(ui.item.value!="查無資料"){
				...
				$('#order_product\\['+row+'\\]\\[tb007\\]').val(ui.item.value4);
	...
	$('input[name=\'order_product[' + row + '][tb009]\'],input[name=\'order_product[' + row + '][tb017]\']').focusout(function() { 
	...
	$('input[name=\'order_product[' + row + '][tb011]\']').focus(function() {
       ...
       if(mm<10) {
        ...
        today =  yyyy + '/' + mm + '/' + dd;
		if ($('input[name=\'order_product[' + n + '][tb011]\']').val()=='') {
		$('input[name=\'order_product[' + n + '][tb011]\']').val(today); }
	});
	...
	$('#order'+row).click(function(){
		...
		$('#hp_ifmain').attr('src',"<?php echo base_url()?>index.php/inv/invi02/display_child/" );
...
//匯入明細圖2 
function import_puri05(mz001,mz002,mz003,mz004){
	console.log('mz001');
	console.log(mz001);
	$.ajax({
		method: "POST",
		dataType: "json",
		url: "<?php echo base_url() ?>index.php/pur/puri05/import_bomi02",
		data: {
			mz001: mz001,
            mz002: mz002,
            mz003: mz003,			
		...
		if(msg==0){
			...
			//if(confirm("共計"+msg.length+"筆，是否匯入?")){
				for(var key in msg){
					var val = msg[key];
					//addItem();
					$('#order_product\\['+current_count+'\\]\\[tb004\\]').val(val['md003']);
					$('#order_product\\['+current_count+'\\]\\[tb005\\]').val(val['mb002']);
					$('#order_product\\['+current_count+'\\]\\[tb006\\]').val(val['mb003']);
					$('#order_product\\['+current_count+'\\]\\[tb007\\]').val(val['md004']);
				//	$('#order_product\\['+current_count+'\\]\\[tb007\\]').val(val['md006']*mz002);
				    addItem();
				}
...
function addcopi02disp(me001, me002, me003, me004, me005, me006, me007){
	...
	$('#order_product\\['+selected_row+'\\]\\[tb007\\]').val(me004); //單位
...
function set_catcomplete222(row){
	console.log(row);
    $('#order_product\\['+row+'\\]\\[tb022\\]').catcomplete({
        autoFocus: false,
		delay: 1000,
        minLength: 1,  		
		source:
...
function totalSum() {
	...
	//訂單金額 ta029 modi ta009
    $(".total_price").each(function(index, element) {
			price=$('input[name=\'order_product[' + index1 + '][tb010]\']').val();
			index1=index1+1;
			if (isNaN(price)) {price=0;}
			   sumamt +=parseFloat(price);			
			//   console.log(sumamt);
    });
		if (typeof($('input[name=\'order_product[' + index1 + '][tb010]\']').val()) == 'undefined' ) {price=0;} else 
		    { price=$('input[name=\'order_product[' + index1 + '][tb010]\']').val(); }
	...
	//稅金 ta030 modi ta023  41稅率modi 24  課稅別16 modi 22
	tax=$('input[name=\'ta024\']').val();
	$('#ta023').val(Math.round(sumamt*tax));
...
//	if ($('select[name=\'ta016\']').val()=='1') {$('#ta029').val()=Math.round(sumamt/parseFloat(vtax));$('#ta030').val()=Math.round(sumamt-$('#ta029').val());}
    if ($('select[name=\'ta022\']').val()=='1') {$('#ta009').val(Math.round(sumamt/parseFloat(vtax)));temp1=Math.round(sumamt-$('#ta009').val());$('#ta023').val(temp1);}
	var sumtot =Math.round(sumamt+sumTax);
	$('#ta009').val(sumamt);
	$('#ta023').val(sumTax);
	$('#ta0923').val(Math.round(sumtot));  //合計金額
	  //  console.log(sumtot);
	   //數量合計 ta031 modi 25   數量8modi 7
   ...
   //總毛重合計 ta043 modi 27 毛重30 modi 20
	$(".total_qty1").each(function(index, element) {
		    if (isNaN($('input[name=\'order_product[' + index3 + '][tb020]\']').val()) ) {qty1=0;} else 
		    { qty1=$('input[name=\'order_product[' + index3 + '][tb020]\']').val(); }
			index3=index3+1;
			if ( isNaN(qty1)  ||   qty1 == null || qty1=='' ) {qty1=0;}
			   sumQty1 +=parseFloat(qty1);			
			 //  console.log(sumQty1);
    });
		if (typeof($('input[name=\'order_product[' + index3 + '][tb020]\']').val()) == 'undefined' ) {qty1=0;} else 
		    { qty1=$('input[name=\'order_product[' + index3 + '][tb020]\']').val(); }
		if (isNaN(qty1) ||   qty1 == null || qty1=='' ) {qty1=0;}
			 sumQty1 +=parseFloat(qty1);
		 $('#ta027').val(sumQty1);
	...
	//總材積合計 ta044 modi 28 31 modi 21
	$(".total_qty2").each(function(index, element) {
		    if (isNaN($('input[name=\'order_product[' + index4 + '][tb021]\']').val()) ) {qty2=0;} else 
		    { qty2=$('input[name=\'order_product[' + index4 + '][tb021]\']').val(); }
			index4=index4+1;
			if ( isNaN(qty2)  ||   qty2 == null || qty2=='' ) {qty2=0;}
			   sumQty2 +=parseFloat(qty2);			
			//   console.log(sumQty2);
    });
		if (typeof($('input[name=\'order_product[' + index4 + '][tb021]\']').val()) == 'undefined' ) {qty2=0;} else 
		    { qty2=$('input[name=\'order_product[' + index4 + '][tb021]\']').val(); }
		if (isNaN(qty2) ||   qty2 == null || qty2=='' ) {qty2=0;}
			 sumQty2 +=parseFloat(qty2);
		 $('#ta028').val(sumQty2);
...
function del_detail(tb001,tb002,tb003,row){
	...
	$.ajax({
		method: "POST",
		url: "<?php echo base_url() ?>index.php/pur/puri05/del_detail_ajax",
		data: { 
			tb001: tb001, 
			tb002: tb002,
			tb003: tb003
...
function addinvi02ddisp(mb001, mb002, mb003, mb004, mb005, mb006){
	...
	//$('#order_product\\['+selected_row+'\\]\\[tb007\\]').val(mb005);
	//$('#order_product\\['+selected_row+'\\]\\[tb007disp\\]').val(mb006);
	$('#order_product\\['+selected_row+'\\]\\[tb004\\]').focus();
	$.ajax({
		method: "POST",
...
function mult_addinvi02ddisp(mb001, mb002, mb003, mb004, mb005, mb006){
	...
	//$('#order_product\\['+current_count+'\\]\\[tb007\\]').val(mb005);
	//$('#order_product\\['+current_count+'\\]\\[tb007disp\\]').val(mb006);
	addItem();
}
...
function addcmsi03ddisp(mb001, mb002){
	clear_row(selected_row);
	$('#order_product\\['+selected_row+'\\]\\[tb007\\]').val(mb001);
	$('#order_product\\['+selected_row+'\\]\\[tb007disp\\]').val(mb002);
	$('#order_product\\['+selected_row+'\\]\\[tb007\\]').focus();
	$.ajax({
		method: "POST",
	...
	$.ajax({
		...
		function(data){
			if(data.response =="true"){
				...
				$('#order_product\\['+row+'\\]\\[tb007\\]').val(data.message[0].value4);
			//	$('#order_product\\['+row+'\\]\\[tb007\\]').val(data.message[0].value5);
			//	$('#order_product\\['+row+'\\]\\[tb007disp\\]').val(data.message[0].value6);
			}else{
...
function check_cmsi03d(row_obj){
	...
	var smb001 = $('#order_product\\['+row+'\\]\\[tb007\\]').val();
	if(!smb001){clear_row(row);return;}
	$.ajax({
		...
		function(data){
			if(data.response =="true"){
				$('#order_product\\['+row+'\\]\\[tb007\\]').val(data.message[0].value1);
				$('#order_product\\['+row+'\\]\\[tb007disp\\]').val(data.message[0].value2);
			}else{
				$('#order_product\\['+row+'\\]\\[tb007\\]').val("查無資料");
			}
		}
	});
}
