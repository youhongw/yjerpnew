        ...
        <thead>
		  ...
		  <?php
			foreach($title_array as $key => $val){
				...
				echo anchor("cop/copi01/display_child/".$this->uri->segment(4,0)."/order?val=".$val['sort_name']." asc",$str);
				
				$str = "<img src='".base_url()."assets/image/desc.png' />";
				echo anchor("cop/copi01/display_child/".$this->uri->segment(4,0)."/order?val=".$val['sort_name']." desc",$str);
...
?>
		...
		<!-- 品號1,品名2,部門代號15,業務人員16,交易幣別14,價格條件30,付款條件31,課稅別38-->	
	    <?php $chkval=1; ?>               
	    <?php foreach($results as $row ) : ?>
        <tr onclick="parent.addcopi01disp('<?php echo $row->ma001;?>','<?php echo $row->ma002;?>','<?php echo $row->ma015;?>','<?php echo $row->ma015disp;?>','<?php echo $row->ma016;?>','<?php echo $row->ma016disp;?>','<?php echo $row->ma014;?>','<?php echo $row->ma014disp;?>','<?php echo $row->ma030;?>','<?php echo $row->ma031;?>','<?php echo $row->ma031disp;?>','<?php echo $row->ma038;?>');parent.$.unblockUI();" >
		  ...
		  <td class="center"><a href="javascript:send_back_copi01('<?php echo $row->ma001;?>','<?php echo $row->ma002;?>','<?php echo $row->ma015;?>','<?php echo $row->ma015disp;?>','<?php echo $row->ma016;?>','<?php echo $row->ma016disp;?>','<?php echo $row->ma014;?>','<?php echo $row->ma014disp;?>','<?php echo $row->ma030;?>','<?php echo $row->ma031;?>','<?php echo $row->ma031disp;?>','<?php echo $row->ma038;?>');">[ 選擇</a><img src="<?php echo base_url()?>assets/image/png/eye.png" />]</td>
		</tr>
		<?php $chkval += 1; ?>
		<?php endforeach;?>
        </tbody>		 
...
function send_back_copi01(ma001, ma002, ma015,ma015disp, ma016,ma016disp, ma014,ma014disp, ma030, ma031,ma031disp, ma038){
	//alert('test1');
	window.parent.$.unblockUI();
	if(window.parent.addcopi01disp){	//以此判斷各呼叫此網頁的方法是否存在藉以給予相對應的需求
		window.parent.addcopi01disp(ma001,ma002, ma015,ma015disp, ma016,ma016disp, ma014,ma014disp, ma030, ma031,ma031disp, ma038);
		$.ajax({
			method: "POST",
