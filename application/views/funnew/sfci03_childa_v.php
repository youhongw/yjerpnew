<div class="box2" style="height:95%">
	...
	<div class="heading">
		...
		<a onclick="location = '<?php echo base_url() ?>index.php/sfc/sfci03/clear_sqla'" style="float:left" accesskey="d" class="button"><span>清除查詢條件 d </span><img height="12" width="12" src="<?php echo base_url() ?>assets/image/delete2.png" /></a>
	</div>
	<?php
	/* title欄位設定區域 */
	$title_array = array(
					...
					if ($num_results > 0) {
						foreach ($results as $row) : ?>
							<tr ondblclick="javascript:send_back_sfci03('<?php echo trim($row->TA001); ?>','<?php echo trim($row->TA002); ?>','<?php echo trim($row->TA003); ?>','<?php echo trim($row->TA004); ?>','<?php echo mb_convert_encoding(trim($row->MW003), 'utf-8', 'big-5'); ?>','<?php echo trim($row->TA006); ?>','<?php echo stripslashes(htmlspecialchars(mb_convert_encoding(trim($row->MB002), 'utf-8', 'big-5'), ENT_QUOTES)); ?>','<?php echo stripslashes(htmlspecialchars(mb_convert_encoding(trim($row->MB003), 'utf-8', 'big-5'), ENT_QUOTES)); ?>','<?php echo mb_convert_encoding(trim($row->MB004), 'utf-8', 'big-5'); ?>','<?php echo intval(trim($row->TA0101)); ?>');">
								...
								<td class="center"><?php echo mb_convert_encoding(trim($row->MB003), "utf-8", "big-5"); ?></td>
								...
								<td class="center"><?php echo mb_convert_encoding(trim($row->MW003), "utf-8", "big-5"); ?></td>
								...
								<td class="center">
									<!--	<a href="javascript:send_back_sfci03('<?php echo trim($row->TA001); ?>','<?php echo trim($row->TA002); ?>','<?php echo trim($row->TA003); ?>','<?php echo trim($row->TA004); ?>','<?php echo mb_convert_encoding(trim($row->MW003), 'utf-8', 'big-5'); ?>','<?php echo trim($row->TA006); ?>','<?php echo stripslashes(htmlspecialchars(mb_convert_encoding(trim($row->MB002), 'utf-8', 'big-5'), ENT_QUOTES)); ?>','<?php echo addslashes(mb_convert_encoding(trim($row->MB003), 'utf-8', 'big-5')); ?>','<?php echo mb_convert_encoding(trim($row->MB004), 'utf-8', 'big-5'); ?>');">[ 選擇</a><img src="<?php echo base_url() ?>assets/image/png/ok.png" />] -->
									<a href="javascript:send_back_sfci03('<?php echo trim($row->TA001); ?>','<?php echo trim($row->TA002); ?>','<?php echo trim($row->TA003); ?>','<?php echo trim($row->TA004); ?>','<?php echo mb_convert_encoding(trim($row->MW003), 'utf-8', 'big-5'); ?>','<?php echo trim($row->TA006); ?>','<?php echo stripslashes(htmlspecialchars(mb_convert_encoding(trim($row->MB002), 'utf-8', 'big-5'), ENT_QUOTES)); ?>','<?php echo stripslashes(htmlspecialchars(mb_convert_encoding(trim($row->MB003), 'utf-8', 'big-5'), ENT_QUOTES)); ?>','<?php echo mb_convert_encoding(trim($row->MB004), 'utf-8', 'big-5'); ?>','<?php echo intval(trim($row->TA0101)); ?>');">[ 選擇</a><img src="<?php echo base_url() ?>assets/image/png/ok.png" />]
								</td>
							</tr>
							<?php $chkval += 1; ?>
					<?php endforeach;
...
<script type="text/javascript">
	...
	function send_back_sfci03(TA001, TA002, TA003, TA004, MW003, TA006, MB002, MB003, MB004, TA0101) {

		window.parent.$.unblockUI();
		if (window.parent.addsfci03disp) { //以此判斷各呼叫此網頁的方法是否存在藉以給予相對應的需求
			window.parent.addsfci03disp(TA001, TA002, TA003, TA004, MW003, TA006, MB002, MB003, MB004, TA0101);
			$.ajax({
				method: "POST",
				url: "<?php echo base_url() ?>index.php/sfc/sfci03/clear_sqla"
			});
		}
	}
	...
	function filter() {
		...
		url = '<?php echo base_url() ?>index.php/sfc/sfci03/display_childa/0/and_where?key=' + encodeURIComponent(key) +
