...
function check_title_no(){
	...
	$.ajax({
		method: "POST",
		url: "<?php echo base_url() ?>index.php/acp/acpi03/check_title_no",
		data: {
			acpi01: acpi01, 
			tc016: tc016
		}
...
//查詢應付憑單開視窗 acpi02 //下拉選單$('.close').click($.unblockUI);
function set_catcomplete(row){
    $('#order_product\\['+row+'\\]\\[td006\\]').catcomplete({
		...
		function(req, add){
			...
			console.log(smb001);
            $.ajax({
                url: '<?php echo base_url(); ?>index.php/acp/acpi02/lookupd_acpi02/'+encodeURIComponent(smb001), 
                cache: false, 				
                dataType: 'json',  
                type: 'POST',  
                data: req,
		...
		change: function(event, ui) {
			$('#order_product\\['+row+'\\]\\[td005\\]').attr('onchange','check_acpi022d(this)');
			check_acpi02d(row);  //1060713 新增
			//check_acpi02d($('#order_product\\['+row+'\\]\\[td004\\]').val());
			return false;
		}
	...
	$('#order'+row).click(function(){
		...
		if($('#puri01a').val()=='') {alert('請先選擇廠商代號!');return;}
		
		$('#hp_ifmain').attr('src',"<?php echo base_url()?>index.php/acp/acpi02/display_child/"+$("#puri01a").val() );
		$.blockUI({
		css: {
			top: '15%',
			left: '25%',
		...
		message: $('#divFacpi02'),
		onOverlayClick: clear_acpi02disp_sql
	});
	 $('.close').click($.unblockUI);
	});
}
//開圖1視窗(廠商單價計價檔copi02)回傳值
function addacpi02disp(me001, me002, me003, me004, me005, me006, me007){
...
function del_detail(td001,td002,td003,row){
	...
	$.ajax({
		method: "POST",
		url: "<?php echo base_url() ?>index.php/acp/acpi03/del_detail_ajax",
		data: { 
			td001: td001, 
			td002: td002,
			td003: td003
...
<script>
//查詢進貨憑證視窗
function search_acpi02d_window(row_obj){
	if($.isNumeric(row_obj)){row = row_obj;}
	else{var row = $(row_obj).parent().parent().parent()[0].id.substr(12);}
	console.log(row);
	selected_row = row;
...
function addacpi02ddisp(ta001,ta002,ta034,tg2829,ta028,ta029,ta030){
	clear_row(selected_row);
	...
	$('#order_product\\['+selected_row+'\\]\\[td012\\]').val(ta2829);
	$('#order_product\\['+selected_row+'\\]\\[td013\\]').val(ta2829-ta030);
...
function mult_addacpi02ddisp(ta001,ta002,ta034,tg2829,ta028,ta029,ta030){
	console.log(mb001);
...
function addcmsi05ddisp(mb001, mb002){
	clear_row(selected_row);
	$('#order_product\\['+selected_row+'\\]\\[td022\\]').val(mb001);
	$('#order_product\\['+selected_row+'\\]\\[td022disp\\]').val(mb002);
	$('#order_product\\['+selected_row+'\\]\\[td022\\]').focus();
	$.ajax({
		method: "POST",
...
function check_acpi02d(row_obj){
	if($.isNumeric(row_obj)){row = row_obj;}
	else{var row = $(row_obj).parent().parent().parent()[0].id.substr(12);}
	var smb001 = $('#order_product\\['+row+'\\]\\[td006\\]').val();
	if(!smb001){clear_row(row);return;}
	$.ajax({
		url: '<?php echo base_url(); ?>index.php/acp/acpi02/lookupd2_acpi02/'+encodeURIComponent(smb001), 
		cache: false, 				
		dataType: 'json',  
		type: 'POST',  
...
function check_cmsi05d(row_obj){
	...
	var smb001 = $('#order_product\\['+row+'\\]\\[td022\\]').val();
	if(!smb001){clear_row(row);return;}
	$.ajax({
		...
		function(data){
			if(data.response =="true"){
				$('#order_product\\['+row+'\\]\\[td022\\]').val(data.message[0].value1);
				$('#order_product\\['+row+'\\]\\[td022disp\\]').val(data.message[0].value2);
			}else{
				$('#order_product\\['+row+'\\]\\[td022\\]').val("查無資料");
			}
		}
	});
}
...
<!--開視窗 應付憑證    -->
<div id="divFacpi02d" style="display:none;width:100%;height:100%;">
<div style="float:right;"><input type="button" class="close" value="close" /></div> 
<iframe src="<?php echo base_url()?>index.php/acp/acpi02/display_child" allowTransparency="flase" name="ifmain" width="100%" height="100%" marginwidth="0" marginheight="0" frameborder="0"></iframe> 	   
</div>
<!--開視窗 科目代號    -->
...
<div id="divFacpi02" style="display:none;width:100%;height:100%;">
<div style="float:right;"><input type="button" class="close" value="close" /></div> 
