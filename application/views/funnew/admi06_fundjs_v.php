...
function check_title_no(){
	$('#tc002').val("");
	var admi03_admi06 = $('#admi03_admi06').val();
	var tc027 = $('#tc027').val();
	//alert(copi03);
	$.ajax({
		method: "POST",
		url: "<?php echo base_url() ?>index.php/adm/admi06/check_title_no",
		data: {
			admi03_admi06: admi03_admi06, 
			tc027: tc027
		}
	})
	.done(function( msg ) {
		if($('#admi03_admi06disp').text()!=""&&$('#admi03_admi06disp').text()!="查無資料")
		$('#tc002').val(msg);
	});
}
...
<script>
function del_detail(mg001,mg002,row){
	if(confirm("確定刪除細項:"+mg001+"-"+mg002+"?")){
	$.ajax({
		method: "POST",
		url: "<?php echo base_url() ?>index.php/adm/admi06/del_detail_ajax",
		data: { 
			mg001: mg001, 
			mg002: mg002
		}
	...
	.done(function( msg ) {
		...
		else{  alert ( "刪除細項:"+mg001+"-"+mg002+"失敗!");}
	});
	}
}
...
function check_body_repeat(body){
	...
	var body_value1 = $('#order_product\\['+body_num+'\\]\\[admi02\\]').val();
	...
	$(".check_repeat").each(function(index, element) {	
		...
		if(body_value1 == $('#order_product\\['+temp_num+'\\]\\[admi02\\]').val() && body_value1 != ''){
			repeat_num += 1;
			console.log(element.lastChild.value);
		}
		console.log(repeat_num);
			
		if(repeat_num >= 2 && body_value1 != ''){
			$('#order_product\\['+body_num+'\\]\\[admi02\\]').val('');
			$('#order_product\\['+body_num+'\\]\\[admi02_mb002\\]').val('');
			check_admi02_body(body);
			alert('輸入的程式編號已經存在，請重新輸入');
			return;
		}
...
function check_addsystem_repeat(system_id){
	var repeat_num = 0;
	var check = "";
	$(".check_repeat").each(function(index, element) {	
		temp_num = element.lastChild.id.substr(13,3).match(/\d+/)[0];
		
		if(system_id == $('#order_product\\['+temp_num+'\\]\\[admi02\\]').val() && system_id != ''){
			repeat_num += 1;
			console.log(element.lastChild.value);
		}
		if(repeat_num >= 1 && system_id != ''){
			check="repeat";
		}else{
			check="no_repeat";
		}
