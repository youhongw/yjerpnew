...
function check_title_no(){
	$('#tc002').val("");
	var asti03_asti09 = $('#asti03_asti09').val();
	var tc027 = $('#tc027').val();
	//alert(copi03);
	$.ajax({
		method: "POST",
		url: "<?php echo base_url() ?>index.php/ast/asti09/check_title_no",
		data: {
			asti03_asti09: asti03_asti09, 
			tc027: tc027
		}
	})
	.done(function( msg ) {
		if($('#asti03_asti09disp').text()!=""&&$('#asti03_asti09disp').text()!="查無資料")
		$('#tc002').val(msg);
	});
}
...
function del_detail(td001,td002,td003,td004,row){
	...
	$.ajax({
		method: "POST",
		url: "<?php echo base_url() ?>index.php/ast/asti09/del_detail_ajax",
		data: { 
			td001: td001, 
			td002: td002,
			td003: td003,
...
function check_body_repeat(body){
	...
	var body_value1 = $('#order_product\\['+body_num+'\\]\\[asti02_asti09\\]').val();
	var body_value2 = $('#order_product\\['+body_num+'\\]\\[asti02_asti09_mc003\\]').val();
	...
	$(".check_repeat").each(function(index, element) {	
		...
		if(body_value1 == $('#order_product\\['+temp_num+'\\]\\[asti02_asti09\\]').val() && body_value1 != '' && body_value2 == $('#order_product\\['+temp_num+'\\]\\[asti02_asti09_mc003\\]').val() && body_value2 != ''){
			repeat_num += 1;
		...
		if(repeat_num >= 2 && body_value1 != '' && body_value2 !=''){
			$('#order_product\\['+body_num+'\\]\\[asti02_asti09\\]').val('');
			$('#order_product\\['+body_num+'\\]\\[asti02_asti09_mc002disp\\]').val('');
			$('#order_product\\['+body_num+'\\]\\[asti02_asti09_mc003\\]').val('');
			$('#order_product\\['+body_num+'\\]\\[asti02_asti09_mc003disp\\]').val('');
