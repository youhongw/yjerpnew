...
echo "</pre>";*/
 ...
 <div id="container">
 	...
 	<div id="header">
 		...
 		<div id="content">
 			...
 			<div class="box">
 				<!-- div-4 -->
 				<div class="heading">
 					<h1><img src="<?php echo base_url() ?>assets/image/order.png" alt="" /> 報工單 - 修改　　　</h1>

 					<div style="float:left;padding-top: 5px;">
 						<button type="sumit"  style="border: 0px solid;box-shadow: 2px 0.5px 1px 1px #88A70E;"  form="commentForm" accesskey="s" name='submit' class="button" onKeyPress="keyFunction()" value='&nbsp;儲存F8&nbsp;'><span>儲 存Alt+s</span><img src="<?php echo base_url() ?>assets/image/png/save.png" /></button>&nbsp;&nbsp;&nbsp;&nbsp;
 						<a accesskey="x" onKeyPress="keyFunction()" id='cancel' name='cancel' href="<?php echo site_url('sfc/sfci03m/display'); ?>" class="button"><span>返 回Alt+x</span><img src="<?php echo base_url() ?>assets/image/png/cancle.png" /></a>
 				...
 				<div class="content">
 					<!-- div-5 -->
 					<form class="cmxform" id="commentForm" name="form" action="<?php echo base_url() ?>index.php/sfc/sfci03m/updsave" method="post" style="width: 100%;" enctype="multipart/form-data">
 						<div id="tab-general">
 							<!-- div-6 -->
								...
								<!--	<td class="normal14">確認碼：</td>
 								...
 								<tr>
									<td class="normal14">備註：</td>
 									...
 									<td class="normal14" colspan="4" style="color: red; font-size: 20px; font-weight: bold;" id="message" ></td>
 								...
 								<table style="width:100%;" id="order_product" class="list1">
									...
									<?php echo 'Alt+d：複製指定列(游標所在列)'; ?>
 									<?php foreach ($body_data as $key => $val) {
											...
											/*	 if ($k != 'TE012' || $k == 'TE013') {
        echo "<!-- DEBUG $k: 原始值=[" . $val->$k . "] 型態=[" . gettype($val->$k) . "] -->";
       
   } */
   
	// === 診斷程式碼 END ===											//   $seconds = 4200;   // 1 小時 10 分鐘
    ...
    if (empty($seconds) || $seconds < 0) {
        ...
        $s = str_pad($hours, 2, '0', STR_PAD_LEFT) . str_pad($minutes, 2, '0', STR_PAD_LEFT);
    // echo var_dump($s);
	}
} else if ($k == 'TE013') {
    // TE013 秒數轉 HHMM
    $seconds = abs($val->TE013);
    if (empty($seconds) || $seconds < 0) {
        $s = '0000';
    } else {
		//echo var_dump($seconds);
     ...
     // echo var_dump($s);
												...
												if ($type == "text") {
													...
													//1141119 add
												/*	echo "<input type='" . $type . "' id='order_product[" . $current_product_count . "][" . $k . "]' 
      name='order_product[" . $current_product_count . "][" . $k . "]' 
      class='order_product_" . $k . "' 
      value='" . $s . "'  
      ...
      data-field='".$k."' ";   */// ← 加入這行
 									...
 									<tfoot>
 										...
 										<tr>
 											<td class="center" valign="top"><img src="<?php echo base_url() ?>assets/image/add.png" title="增加" style="cursor: pointer;" onclick="addItem();" ondblclick="addItemCopy();" /></td>
 ...
 <form action="<?php echo base_url() ?>index.php/sfc/sfci03m/delete_detail" method="post" enctype="multipart/form-data" id="del_form" style="display:none;">
 ...
 <!-- 全域變數 -->
 <?php include_once("./application/views/funnew/sfci03m_fundjs_v.php"); ?>
 <!-- 明細開視窗 -->
<script type="text/javascript">
$(document).ready(function() {
    $('#cmsi04').focus();
    
	// TE005 focus 就啟用下拉（委派，動態列也吃得到）
$(document).on('focus', 'input[id$="\\[TE005\\]"]', function () {
    set_machine_catcomplete(this);
});
    // 統一使用事件委派處理所有雙擊事件
    // 製令單別
    $(document).on('dblclick', 'input[data-field="TE006"]', function() {
        if (typeof search_sfci03ma_window === 'function') {
            search_sfci03ma_window(this);
        }
    });
    
    // 製令單號
    $(document).on('dblclick', 'input[data-field="TE007"]', function() {
        if (typeof search_sfci03m_window === 'function') {
            search_sfci03m_window(this);
        }
    });
    
    // 員工代號
    $(document).on('dblclick', 'input[data-field="cmsi09d"]', function() {
        if (typeof search_cmsi09d_window === 'function') {
            search_cmsi09d_window(this);
        }
    });
    
    // 機台代號
    $(document).on('dblclick', 'input[data-field="TE005"]', function() {
        if (typeof search_cmsi03d_window === 'function') {
            search_cmsi03d_window(this);
        }
    });
    ...
    $(document).on('keydown', function(e) {
        ...
        if (e.altKey && e.keyCode == 68) {
            ...
            // 确保 addItemCopy() 函数存在
            if (typeof addItemCopy === 'function') {

    // 取得目前游標所在的欄位
    var el = document.activeElement;

    // 從 name 取出 row index：order_product[3][TE023]
    var row = -1;
    if (el && el.name) {
        var m = el.name.match(/order_product\[(\d+)\]/);
        if (m) row = parseInt(m[1], 10);
    }

    // ✅ 有抓到 row 就複製該列；抓不到就退回原本行為
    if (row >= 0) {
        addItemCopy(row);
    } else {
        addItemCopy(); // fallback
    }

} else {
    console.error("addItemCopy function is not defined.");
}
