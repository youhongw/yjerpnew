...
<div id="content"> <!-- div-3 --> 
 <div class="box"> <!-- div-4 -->
    <div class="heading">
      <h1><img src="<?php echo base_url()?>assets/image/order.png" alt="" /> 報工單建立作業 - 修改</h1>
    </div>
	
    <div class="content"> <!-- div-5 -->
	<form  class="cmxform" id="commentForm"  name="form" action="<?php echo base_url()?>index.php/sfc/sfci03y/updsave" method="post" enctype="multipart/form-data" >
	<div id="tab-general"> <!-- div-6 -->
	
	<table class="form14"  >     <!-- 頭部表格 -->
	 <tr>
	    <td class="start14a"  width="9%"><span class="required">報工單別：</span></td>   <!--onchange="startsfci01(this);check_title_no();"    -->
        <td class="normal14a"  width="25%"><input tabIndex="1" id="sfci01"    onKeyPress="keyFunction()"   name="td001"  onchange="check_sfci01(this);check_title_no();"  value="<?php echo $td001; ?>" size="12" type="text" required />
		  <a href="javascript:;"><img id="Showsfci01disp" src="<?php echo base_url()?>assets/image/png/invoice.png" alt="" align="top"/></a>
		  <span id="sfci01disp"> <?php    echo mb_convert_encoding($td001disp, "utf-8", "big-5"); ?> </span></td>
	    <td class="normal14a" width="8%" >單據日期： </td>  <!-- dateformat_ymd(this); -->
        <td class="normal14a"  width="25%" ><input tabIndex="2"  ondblclick="scwShow(this,event);"   id="td008" onKeyPress="keyFunction()"  onchange="dateformat_ymd(this);check_title_no();" name="td008"  value="<?php echo $td008; ?>"  size="12" type="text" style="background-color:#FFFFE4"  /> 
        ...
        <td class="normal14a" width="25%"><input tabIndex="3" id="td002" onKeyPress="keyFunction()" readonly="value" name="td002" onfocus="check_title_no()" value="<?php echo $td002; ?>" size="12" type="text" required /></td>
	  </tr>
	  
	  <tr>	    
		<td class="normal14a">生產線別：</td>
        <td  class="normal14"  ><input type="text" tabIndex="4" onKeyPress="keyFunction()" id="cmsi04"  onblur="check_cmsi04(this)" name="td004"   value="<?php echo  $td004; ?>"   size="12"   />
	      <a href="javascript:;"><img id="Showcmsi04disp" src="<?php echo base_url()?>assets/image/png/linedo.png" alt="" align="top"/></a>
	      <span id="cmsi04disp"> <?php    echo mb_convert_encoding($td004disp, "utf-8", "big-5"); ?> </span></td>
        ...
        <td  class="normal14"  ><select id="verify" onKeyPress="keyFunction()" name="td005" onChange="selverify(this)" tabIndex="6">
        ...
        <td class="normal14"  ><select id="td010" tabIndex="9" readonly="value" onKeyPress="keyFunction()" name="td010"   style="background-color:#F0F0F0" >
        ...
        <!--  依照資料庫紀錄的明細先列一遍 原0會由1加  -->
			...
			<?php foreach($body_data as $key => $val){
				...
				foreach($usecol_array as $k => $v){
					 ...
					 if($k=="te005disp" or $k=="te009disp" or $k=="cmsi09ddisp" or $k=="te015" or $k=="te018" or $k=="te019" or $k=="te020" ){						
	...
	<!-- 合計     -->
	  ...
	  <a  accesskey="x" onKeyPress="keyFunction()" id='cancel' name='cancel' href="<?php echo site_url('sfc/sfci03y/display'); ?>" class="button" ><span>返 回Alt+x</span><img src="<?php echo base_url()?>assets/image/png/cancle.png" /></a>
	  <?php if(isset($prev)){ 
			$prev_str = "";$prev_ary = explode('_',$prev);foreach($prev_ary as $k=>$v){if($prev_str){$prev_str.="/";}$prev_str.=$v;}
	  ?>
			<a accesskey="," tabIndex="11" id='prev' name='prev' href="<?php echo site_url('sfc/sfci03y/updform/'.$prev_str); ?>" class="button" ><span>上一筆Alt+<</span><img src="<?php echo base_url()?>assets/image/png/pre.png" /></a>
	  <?php } ?>
	  <?php if(isset($next)){ 
			$next_str = "";$next_ary = explode('_',$next);foreach($next_ary as $k=>$v){if($next_str){$next_str.="/";}$next_str.=$v;}
	  ?>
			<a accesskey="." tabIndex="12" id='next' name='next' href="<?php echo site_url('sfc/sfci03y/updform/'.$next_str); ?>" class="button" ><span>下一筆Alt+></span><img src="<?php echo base_url()?>assets/image/png/next.png" /></a>
	  <?php } ?>	
	  </div> 
	  </div> <!-- div-加 -->
    </form>  <!-- end 表單 -->
...
<form action="<?php echo base_url()?>index.php/sfc/sfci03y/delete_detail" method="post" enctype="multipart/form-data" id="del_form" style="display:none;">
			<input id="del_md001" name="del_md001" />
			<input id="del_md002" name="del_md002" />
			<input id="del_md003" name="del_md003" />
		</form>
		
<?php  include_once("./application/views/funnew/sfci01_funmjs_v.php"); ?> <!-- 報工單別 -->
<?php  include_once("./application/views/funnew/cmsi04_funmjs_v.php"); ?>  <!-- 生產線別 -->
<?php  include_once("./application/views/funnew/cmsi09_funmjs_v.php"); ?>  <!-- 人員 -->

<?php  include_once("./application/views/funnew/erp_funjs_v.php"); ?>      <!-- 全域變數 -->
<?php  include_once("./application/views/funnew/sfci03y_fundjs_v.php"); ?>  <!-- 明細開視窗 --> 