...
<div id="content"> <!-- div-3 --> 
  <div class="box"> <!-- div-4 -->
    <div class="heading">
      <h1><img src="<?php echo base_url()?>assets/image/order.png" alt="" /> 報工單建立作業 - 新增</h1>
    </div>
	
    <div class="content"> <!-- div-5 -->
	<form   class="cmxform" id="commentForm"  name="form"  method="post" enctype="multipart/form-data"  action="<?php echo base_url()?>index.php/sfc/sfci03y/addsave" >	
	<?php       
	  ...
	  if(!isset($td004)) { $td004=$this->input->post('td004'); } 
	  if(!isset($td004disp)) { $td004disp=$this->input->post('td004disp'); }
	  if(!isset($td005)) { $td005=$this->input->post('td005'); }
	  if(!isset($td006)) { $td006=$this->input->post('td006'); }
	 //  $td025=$this->input->post('td025');  一筆存檔清空白
	  if(!isset($td007)) { $td007=0; }
	  if(!isset($td008)) { $td008=date("Y/m/d"); }
	  if(!isset($td009)) { $td009=$username; }
	  if(!isset($td010)) { $td010="N"; }
	?>
	...
	<table class="form14"  >     <!-- 頭部表格 -->
	  <tr>
        ...
        <td class="normal14a" width="25%"><input tabIndex="3" id="td002" onKeyPress="keyFunction()"  name="td002" onfocus="check_title_no()" value="<?php echo $td002; ?>" size="12" type="text" required /></td>
	  </tr>
	  
	  <tr>	    
		<td class="normal14a">生產線別：</td>
        <td  class="normal14"  ><input type="text" tabIndex="4" onKeyPress="keyFunction()" id="cmsi04" onfocus="check_title_no()" onblur="check_cmsi04(this)" name="td004"   value="<?php echo  $td004; ?>"   size="12"   />
	 ...
	 <div class="buttons">
	   ...
	   <a  accesskey="x" tabIndex="97" onKeyPress="keyFunction()" id='cancel' name='cancel' href="<?php echo site_url('sfc/sfci03y/display'); ?>" class="button" ><span>返 回Alt+x</span><img src="<?php echo base_url()?>assets/image/png/cancle.png" /></a>
	 </div> 
   </div> 	<!-- end 頁標籤 -->   
   </form>
	   <?php	if ($message!=' ') { ?>
