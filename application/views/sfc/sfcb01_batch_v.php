...
<div id="content"> <!-- div-3 -->
 <div class="box"> <!-- div-4 -->
    <div class="heading">
      <h1><img src="<?php echo base_url()?>assets/image/order.png" alt="" /> 移轉單產生領料單 - 產生</h1>
    </div>
	
	<div class="content"> <!-- div-5 -->
	<form  class="cmxform" id="commentForm"  name="form" action="<?php echo base_url()?>index.php/sfc/sfcb01/batcha"  method="post"  enctype="multipart/form-data" > 
	...
	<?php
	 ...
	 // if(!isset($td002c)) { $td002c=date("Y/m/d"); } else {$td002c=$this->input->post('td002c');}
	  if(!isset($td003o)) { $td003o='1'; } else {$td003o=$this->input->post('td003o');}
	...
	<table class="form14">   <!-- 表格 -->
	  <tr>
	    <td class="start14a"  width="14%">起製令單號：</td>
	  ...
	  <tr>
	 <td class="normal14">領料碼：</td>
        <td  class="normal14"  ><select id="td003o" onKeyPress="keyFunction()" name="td003o"  tabIndex="9">
            <option <?php if($td003o == '1') echo 'selected="selected"';?> value='1'>逐批領料</option>                                                                        
		    <option <?php if($td003o == '2') echo 'selected="selected"';?> value='2'>自動扣料</option>
			<option <?php if($td003o == '3') echo 'selected="selected"';?> value='3'>單獨領料</option>
	...
	<!--	<a onclick="return doit();" class="button"><span>計算&nbsp</span><img src="<?php echo base_url()?>assets/image/png/add.png" /></a> -->
		...
		<div class="buttons">
	      ...
	      <a  accesskey="x" tabIndex="97" onKeyPress="keyFunction()"  id='cancel' name='cancel' href="<?php echo site_url('main/index/171'); ?>" class="button" >返 回Alt+x</span><img src="<?php echo base_url()?>assets/image/png/back.png" /></a>
	    </div>
        </form>
    </div> <!-- div-6 -->
  </div> <!-- div-5 -->
...
<?php include("./application/views/fun/sfcb01_funjs_v.php"); ?>
<script>
  $(function() {
