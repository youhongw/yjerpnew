 <?php if (!$results)  { ?>
        <script> alert("無資料可列印!"); url = '<?=base_url() ?>index.php/cst/cstr11/printdetail';location = url; </script> 
  <?php exit;} ?>
<!--   頁直行數  -->
...
foreach($results as $key=>$value){
	...
	//$pur_date = substr($value->mv003,0,4).'/'.substr($value->mv003,4,2).'/'.substr($value->mv003,6,2);
}
$totle_page = $page;
...
?>       
	...
	<table class="list">     <!-- 列明細 2 判斷 31, 3 判斷 32 4 判斷 32, 31-->
	      ...
	      <tr>
	    <td width="15%" align="center"><?php echo  $val->mv001;?></td>
		<td width="15%" align="center"><?php echo  $val->mv002;?></td>		
		<td width="14%" align="right"><?php echo  $val->mv003;?></td>		
        <td width="14%" align="right"><?php echo  $val->mv004;?></td>
        <td width="14%" align="right"><?php echo  $val->mv005;?></td>		
        <td width="14%" align="right"><?php echo  $val->mv006;?></td> 
        <td width="14%" align="right"><?php echo  $val->mv007;?></td>	
        </tr>		
		 <?php $rownum++;$rownum1++;$vqty=$vqty+$val->mv005;$vnoqty=$vnoqty+$val->mv006;$vamt=$vamt+$val->mv005; ?>
