 <?php if (!$results)  { ?>
        <script> alert("無資料可列印!"); url = '<?=base_url() ?>index.php/cst/cstr07/printdetail';location = url; </script> 
  <?php exit;} ?>
<!--   頁直行數  -->
...
foreach($results as $key=>$value){
	...
	//$pur_date = substr($value->mc003,0,4).'/'.substr($value->mc003,4,2).'/'.substr($value->mc003,6,2);
}
$totle_page = $page;
...
?>       
	...
	<table class="list">     <!-- 列明細 2 判斷 31, 3 判斷 32 4 判斷 32, 31-->
		...
		<td width="14%" align="right"><?php echo  $val->mc003;?></td>		
        <td width="14%" align="right"><?php echo  $val->mc004;?></td>
        <td width="14%" align="right"><?php echo  $val->mc005;?></td>		
        <td width="14%" align="right"><?php echo  $val->mc006;?></td> 
        <td width="14%" align="right"><?php echo  $val->mc007;?></td>	
        </tr>		
		 <?php $rownum++;$rownum1++;$vqty=$vqty+$val->mc005;$vnoqty=$vnoqty+$val->mc006;$vamt=$vamt+$val->mc005; ?>
