...
<div id="content"> <!-- div-3 --> 
 <div class="box"> <!-- div-4 --><span>　　　　　　</span>
    <div class="heading">
      <h1><img src="<?php echo base_url()?>assets/image/order.png" alt="" /> 借/還款建立作業 - 修改　　　</h1>
      <div style="float:left;padding-top: 5px; ">
	  <button style= "cursor:pointer" form="commentForm" onfocus="$('#noti06').focus();" type='submit' accesskey="s" name='submit' class="button" onKeyPress="keyFunction()" onclick="" value='&nbsp;儲存F8&nbsp;'><span>儲 存Alt+s</span><img src="<?php echo base_url()?>assets/image/png/save.png" /></button>&nbsp;&nbsp;
	  <a  accesskey="x" onKeyPress="keyFunction()" id='cancel' name='cancel' href="<?php echo site_url('not/noti09/display'); ?>" class="button" ><span>返 回Alt+x</span><img src="<?php echo base_url()?>assets/image/png/cancle.png" /></a>
	  <?php if(isset($prev)){ 
			$prev_str = "";$prev_ary = explode('_',$prev);foreach($prev_ary as $k=>$v){if($prev_str){$prev_str.="/";}$prev_str.=$v;}
	  ?>
			<a accesskey="," tabIndex="11" id='prev' name='prev' href="<?php echo site_url('not/noti09/updform/'.$prev_str); ?>" class="button" ><span>上一筆Alt+<</span><img src="<?php echo base_url()?>assets/image/png/pre.png" /></a>
	  <?php } ?>
	  <?php if(isset($next)){ 
			$next_str = "";$next_ary = explode('_',$next);foreach($next_ary as $k=>$v){if($next_str){$next_str.="/";}$next_str.=$v;}
	  ?>
			<a accesskey="." tabIndex="12" id='next' name='next' href="<?php echo site_url('not/noti09/updform/'.$next_str); ?>" class="button" ><span>下一筆Alt+></span><img src="<?php echo base_url()?>assets/image/png/next.png" /></a>
	  <?php } ?>
	</div>
	</div>
    <div class="content"> <!-- div-5 -->
	<form  class="cmxform" id="commentForm"  name="form" action="<?php echo base_url()?>index.php/not/noti09/updsave" method="post" enctype="multipart/form-data" >
	<div id="tab-general"> <!-- div-6 -->
	
	<table class="form14"  >     <!-- 頭部表格 -->
	  <tr>
	    <td class="normal14y"  width="12%"><span class="required">借/還款單別：</span></td>   <!--onchange="startnoti06(this);check_title_no();"    -->
        <td class="normal14a"  width="38%"><input tabIndex="1" id="noti06"    onKeyPress="keyFunction()"   name="noti06"  onblur="turn_tl008(this);" onchange="check_noti06(this);"  value="<?php echo $tk001; ?>" size="12" type="text" required />
        ...
        <td class="normal14a"  width="40%" ><input tabIndex="2"  ondblclick="scwShow(this,event);"   id="tk003" onKeyPress="keyFunction()"  onchange="dateformat_ymd(this);" name="tk003"  value="<?php echo $tk003; ?>"  size="12" type="text" style="background-color:#FFFFE4"  /></td>
	    
	  </tr>
	  
	  <tr>	    
		<td class="normal14z" ><span class="required">借/還款單號：</span></td>
        <td class="normal14a" ><input tabIndex="3" id="tk002" onKeyPress="keyFunction()"  name="tk002"  value="<?php echo $tk002; ?>" size="12" type="text"  /></td>
...
<form action="<?php echo base_url()?>index.php/not/noti09/delete_detail" method="post" enctype="multipart/form-data" id="del_form" style="display:none;">
...
$(document).ready(function(){ 	   
	$('.order_product_tl008').each(function(index,element){
			...
			if(temp_value == '1'){
				$('#order_product\\['+temp3+'\\]\\[tl010\\]').removeAttr('ondblclick');
				$('#order_product\\['+temp3+'\\]\\[tl009\\]').removeAttr('ondblclick');
				$('#order_product\\['+temp3+'\\]\\[tl010\\]').removeAttr('onchange');
				$('#order_product\\['+temp3+'\\]\\[tl009\\]').removeAttr('onchange');
				...
				$('#order_product\\['+temp3+'\\]\\[tl009\\]').attr('ondblclick','search_noti01_body_window(this)');
				$('#order_product\\['+temp3+'\\]\\[tl010\\]').removeAttr('ondblclick');
				$('#order_product\\['+temp3+'\\]\\[tl009\\]').attr('onchange','check_noti01_body(this)');
				$('#order_product\\['+temp3+'\\]\\[tl010\\]').removeAttr('onchange');
		
				$('#order_product\\['+temp3+'\\]\\[tl009\\]').removeAttr('readonly');
				$('#order_product\\['+temp3+'\\]\\[tl010\\]').attr('readonly','value');
				$('#order_product\\['+temp3+'\\]\\[tl009\\]').attr('style','background-color:#FFFFE4');
				$('#order_product\\['+temp3+'\\]\\[tl010\\]').attr('style','background-color:#F0F0F0');
			}else if(temp_value == '3'){
				$('#order_product\\['+temp3+'\\]\\[tl010\\]').attr('ondblclick','search_noti03_body_window(this)');
				$('#order_product\\['+temp3+'\\]\\[tl009\\]').removeAttr('ondblclick');
				$('#order_product\\['+temp3+'\\]\\[tl010\\]').attr('onchange','check_noti03_body(this)');
				$('#order_product\\['+temp3+'\\]\\[tl009\\]').removeAttr('onchange');
		
				$('#order_product\\['+temp3+'\\]\\[tl010\\]').removeAttr('readonly');
				$('#order_product\\['+temp3+'\\]\\[tl009\\]').attr('readonly','value');
				$('#order_product\\['+temp3+'\\]\\[tl010\\]').attr('style','background-color:#FFFFE4');
				$('#order_product\\['+temp3+'\\]\\[tl009\\]').attr('style','background-color:#F0F0F0');
			}else{
				$('#order_product\\['+temp3+'\\]\\[tl010\\]').attr('ondblclick','search_noti04_body_window(this)');
				$('#order_product\\['+temp3+'\\]\\[tl009\\]').removeAttr('ondblclick');
				$('#order_product\\['+temp3+'\\]\\[tl010\\]').attr('onchange','check_noti04_body(this)');
				$('#order_product\\['+temp3+'\\]\\[tl009\\]').removeAttr('onchange');
