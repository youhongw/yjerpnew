	...
	<div id="content"> <!-- div-3 --> 
		<div class="box"> <!-- div-4 --><span>　　　　　　</span>
			<div class="heading">
				...
				<a accesskey="x" onKeyPress="keyFunction()" id="cancel" name="cancel" href="<?php echo site_url('moc/moci07/'.$this->session->userdata('moci07_search')); ?>" class="button"><span>返回Alt+x</span><img src="<?php echo base_url()?>assets/image/png/cancle.png" /></a>&nbsp;&nbsp;
		...
		<div class="content"> <!-- div-5 -->
			<form   class="cmxform" id="commentForm"  name="form"  method="post" enctype="multipart/form-data"  action="<?php echo base_url()?>index.php/moc/moci07/updsave" >	
			<div id="tab-general"> <!-- div-6 -->
				<table class="form14"  >     <!-- 頭部表格 -->
					<tr style='display:none;'><td><input name="flag" value="<?php echo $flag;?>" /></td></tr>
					<tr>
						<td class="normal14y"  width="11%"><span class="required">託外退貨單別：</span> </td>
						<td class="normal14a"  width="21%">
						...
						<td class="normal14y" width="11%" ><span class="required">託外退貨單號：</span> </td>
				...
				<div class="abgne_tab"> <!--div-6-->
						...
						<div id="tab2" class="tab_content">
							<table class="form14">
								<tr>
									...
									<td class="normal14y" width="11%" >匯率： </td>
									<td class="normal14a"  width="21%" >
									...
									<td class="normal14y" width="12%" >件數：</td>
									<td class="normal14a"  width="22%" >
								...
								<tr>
									<td class="normal14z"  width="11%">付款條件代號：</td>
									<td class="normal14a"  width="23%">
					...
					<table id="order_product" class="list1">
						...
						foreach($body_data as $key => $val){
							...
							echo "<tr>";
							echo "<td class='center'><img src='".base_url()."assets/image/delete2.png' title='刪除資料' onclick='del_detail(\"".$val->tl001."\",\"".$val->tl002."\",\"".$val->tl003."\",\"".$val->tl015."\",\"".$val->tl016."\",\"".$val->tl004."\",\"".$val->tl013."\",\"".$val->tl007."\",\"".$current_product_count."\");'/></td>";
...
function addmoci05disp(ta006, mb002, mb003, mb004, mb017, mc002, ta001, ta002){
	...
	$('#order_product\\['+selected_row+'\\]\\[tl015\\]').val(ta001); //製令單號
	$('#order_product\\['+selected_row+'\\]\\[tl016\\]').val(ta002); //製令單別
...
//退貨單別,退貨單號,序號,製令單別,製令單號,品號,庫別,退貨數量
function del_detail(tl001,tl002,tl003,tl015,tl016,tl004,tl013,tl007,row){
	if(confirm("確定刪除細項:"+tl001+"-"+tl002+"-"+tl003+"?")){
	$.ajax({
		method: "POST",
	...
	.done(function( msg ) {
		if(msg){
			alert( "刪除細項:"+tl001+"-"+tl002+"-"+tl003+" 成功!");
			$("#product_row_"+row).remove();
		}
	else{alert( "刪除細項:"+tl001+"-"+tl002+"-"+tl003+" 失敗!");}
	});
	}
}
function clear_row(row_obj){
	...
	for(var k=4;k<=22;k++){//k的最大值請依照實際情況去調整，通常設為欄位數字最大者(即最後一個欄位)
		$('#product_row'+row+' input.order_product_tl00'+k).val("");
		$('#product_row'+row+' input.order_product_tl0'+k).val("");
		$('#product_row'+row+' input.order_product_tl'+k).val("");
	}
}
</script>
<script>
...
$(document).ready(function(){
    ...
    $('#cmsi02').catcomplete({
		...
		function(req, add){
            ...
            $.ajax({
                url: '<?php echo base_url(); ?>index.php/moc/moci07/lookup_cmsi02/'+encodeURIComponent(smb001), 
                cache: false, 				
                dataType: 'json',  
                type: 'POST',  
                data: req,
...
function check_cmsi02(row_obj){
	...
	$.ajax({
		url: '<?php echo base_url(); ?>index.php/moc/moci07/lookup2_cmsi02/'+encodeURIComponent(smb001), 
		cache: false, 				
		dataType: 'json',  
		type: 'POST',  
...
$(document).ready(function(){
    ...
    $('#cmsi04').catcomplete({
		...
		function(req, add){
            ...
            $.ajax({
                url: '<?php echo base_url(); ?>index.php/moc/moci07/lookup_cmsi04/'+encodeURIComponent(smb001), 
                cache: false, 				
                dataType: 'json',  
                type: 'POST',  
                data: req,
...
function check_cmsi04(row_obj){
	...
	$.ajax({
		url: '<?php echo base_url(); ?>index.php/moc/moci07/lookup2_cmsi04/'+encodeURIComponent(smb001), 
		cache: false, 				
		dataType: 'json',  
		type: 'POST',  
		data: {mb001:row_obj.value},
...
function check_tl011(row_obj){
	...
	$('#order_product\\['+row+'\\]\\[tl012\\]').val(tl007 * tl009);
	totalSum();
}
