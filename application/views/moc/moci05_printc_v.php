...
<?php if (!$results)  { ?>
	<script> alert("無資料可列印!"); url = '<?=base_url() ?>index.php/moc/moci05/printdetailc';location = url; </script> 
<?php exit;}
//$data		按單裝資料
$data = Array();
...
foreach($print_data as $index => $value){
	if($temp_order != $value[0]->tf002){  //判斷是否同一張單  不是就重新計算頁數
		$temp_order = $value[0]->tf002;
		$temp_page = 1;
	}
?>
	<table class="store">
					...
					<tr>
					  ...
					  <td width="30%" align="left" valign="top">
						<table class="company">
							...
							<tr>
							  <td width="600" align="left" valign="top"><b>確認者：</b><?php echo $results[0]->tf013 ?></td>
							  <td align="left" valign="top"></td>
							</tr>
						</table>
					  </td>
			...
			<tr><td valign="top">
				<table class="product" border=1 cellspacing=0 cellpadding=0>
					<tr class="heading">
					  ...
					  <td align="left" width="10%"><b>入/出別</b></td>
