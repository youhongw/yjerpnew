<div id="container"> <!-- div-1 -->
	...
	<div id="content"> <!-- div-3 --> 
		<div class="box"> <!-- div-4 --><span>　　　　　　</span>
			<div class="heading">
				<h1><img src="<?php echo base_url()?>assets/image/order.png" alt="" /> 託外進貨單建立作業 - 新增　　　</h1>
				<div style="float:left;padding-top: 5px; ">
				<button style= "cursor:pointer" form="commentForm" onfocus="$('#puri04').focus();" type='submit' accesskey="s" name='submit' class="button" onKeyPress="keyFunction()" onclick="" value='&nbsp;儲存F8&nbsp;'><span>儲 存Alt+s</span><img src="<?php echo base_url()?>assets/image/png/save.png" /></button>&nbsp;&nbsp;
				...
				<table class="form14"  >     <!-- 頭部表格 -->
					...
					<tr>
						...
						<td class="normal14a"  width="22%" >
							<input tabIndex="3" id="th002" onKeyPress="keyFunction()" name="th002" value="<?php echo $th002; ?>" size="20" type="text" required />
						</td>
					</tr>		
					...
					<tr>
						...
						<td class="normal14" >
							<select tabIndex="13" id="th023" name="th023" onKeyPress="keyFunction()">
						...
						<div id="tab1" class="tab_content">
							<table class="form14">   
								<tr>
									...
									<td class="normal14a"  width="24%" >
										<input tabIndex="3" id="th028" onKeyPress="keyFunction()" name="th028" value="<?php echo $th028; ?>" ondblclick="scwShow(this,event);" onChange="dateformat_ymd(this);" type="text" style="background-color:#E7EFEF" />
									</td>
								</tr>
								...
								<tr>
									...
									<td class="normal14a"  width="22%">
										<select id="th012" name="th012" tabindex="13" onKeyPress="keyFunction()">
									...
									<script>$('#th012').val('2');</script>
			
									<td class="normal14z" width="10%" >課稅別： </td>
									<td class="normal14a"  width="24%" >
										<select id="th015" name="th015" tabindex="13" onKeyPress="keyFunction()" onchange="exchange_tax2()" />
									...
									<script>$('#th015').val('2');</script>
			
									<td class="normal14z" width="10%" >營業稅率：</td>
									<td class="normal14a"  width="24%" >
										<input tabIndex="3" id="th030" onKeyPress="keyFunction()" name="th030" value="<?php if($th030 != ""){echo $th030;}else{echo $this->session->userdata('sysma004');}; ?>" size="20" type="text" />
									</td>
								</tr>
								<tr>
									<td class="normal14z"  width="10%">發票日期：</td>
									<td class="normal14a"  width="22%">
										<input tabIndex="1" id="th013" name="th013" value="<?php echo $th013; ?>"  onKeyPress="keyFunction()"  ondblclick="scwShow(this,event);" onChange="dateformat_ymd(this);" type="text" style="background-color:#E7EFEF"/>
									</td>
			
									<td class="normal14z" width="10%" >扣抵區分： </td>
									<td class="normal14a"  width="24%" >
										<select name="th016" id="th016" onKeyPress="keyFunction()">
						...
						<div id="tab2" class="tab_content">
							<table class="form14">
								<tr>
									...
									<td class="normal14a"  width="24%">
										<input tabIndex="1" id="cmsi06" name="cmsi06" onKeyPress="keyFunction()"  value="<?php if($cmsi06 != ""){echo $cmsi06;}else{echo "NTD";}; ?>" onchange="check_cmsi06(this);check_cmsi07(this);" type="text"  />
...
$(document).ready(function(){
    ...
    $('#puri01').catcomplete({
		...
		function(req, add){
			...
			$('#puri01').attr('onchange','');
            $.ajax({
                url: '<?php echo base_url(); ?>index.php/moc/moci06/lookup_puri01/'+encodeURIComponent(smb001), 
                cache: false, 				
                dataType: 'json',  
                type: 'POST',  
                data: req,
...
function check_puri01(row_obj){
	...
	if(!smb001){$('#puri01disp').text("");return;}
	$.ajax({
		url: '<?php echo base_url(); ?>index.php/moc/moci06/lookup2_puri01/'+encodeURIComponent(smb001), 
		cache: false, 				
		dataType: 'json',  
		type: 'POST',  
		...
		function(data){
			if(data.response == "true"){
				...
				$('#puri01').val(smb001);
				$('#puri01disp').text(data.message[0].value2);
				th015_change(data.message[0].value3);
			}else{
				$('#puri01').val(smb001);
				$('#puri01disp').text("查無資料");
			}
...
function clear_row(row_obj){
	...
	for(var k=4;k<=22;k++){//k的最大值請依照實際情況去調整，通常設為欄位數字最大者(即最後一個欄位)
		...
		$('#product-row'+row+' input.order_product_te'+k).val("");
	}
}
...
function check_title_no(){
	...
	$.ajax({
		method: "POST",
		url: "<?php echo base_url() ?>index.php/moc/moci06/check_title_no",
		data: {
			puri04: puri04, 
			Ddate: Ddate
		}
