<div class="box2" style="height:95%"> <!-- div-1 -->
    <div class="heading"> 
      ...
      <a onclick="location = '<?php echo base_url()?>index.php/ast/asti03/clear_sql_asti07'"  style="float:left" accesskey="d" class="button"><span>清除查詢條件 d </span><img height="12" width="12" src="<?php echo base_url()?>assets/image/delete2.png" /></a>
	</div>
<?php 
/* title欄位設定區域 */
	$title_array = array(
        ...
        <thead>
		  ...
		  <?php
			foreach($title_array as $key => $val){
				...
				echo anchor("ast/asti03/display_child_asti07/".$this->uri->segment(4,0)."/order?val=".$val['sort_name']." asc",$str);
				
				$str = "<img src='".base_url()."assets/image/desc.png' />";
				echo anchor("ast/asti03/display_child_asti07/".$this->uri->segment(4,0)."/order?val=".$val['sort_name']." desc",$str);
...
function send_back_asti03(mc001, mc002){
	...
	if(window.parent.addasti03_asti07disp){	//以此判斷各呼叫此網頁的方法是否存在藉以給予相對應的需求
		window.parent.addasti03_asti07disp(mc001,mc002);
		$.ajax({
			method: "POST",
...
function filter() {
	...
	url = '<?php echo base_url() ?>index.php/ast/asti03/display_child_asti07/0/and_where?key=' + encodeURIComponent(key) + 
