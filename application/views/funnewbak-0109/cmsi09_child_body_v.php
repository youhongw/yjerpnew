<div class="box2" style="height:95%"> <!-- div-1 -->
    <div class="heading"> 
      ...
      <a onclick="location = '<?php echo base_url()?>index.php/cms/cmsi09/clear_body_sql'"  style="float:left" accesskey="d" class="button"><span>清除查詢條件 d </span><img height="12" width="12" src="<?php echo base_url()?>assets/image/delete2.png" /></a>
	</div>
<?php 
/* title欄位設定區域 */
	$title_array = array(
...
?>
	    ...
	    <?php foreach($results as $row ) : ?>
         <tr onclick="parent.addcmsi09_body('<?php echo $row->mv001;?>','<?php echo $row->mv002;?>');parent.$.unblockUI();" >
...
function send_back_cmsi09(mv001, mv002){
	...
	if(window.parent.addcmsi09_body){	//以此判斷各呼叫此網頁的方法是否存在藉以給予相對應的需求
	  
		window.parent.addcmsi09_body(mv001,mv002);
		$.ajax({
			method: "POST",
...
function filter() {
	...
	url = '<?php echo base_url() ?>index.php/cms/cmsi09/display_child_body/0/and_where?key=' + encodeURIComponent(key) + 
