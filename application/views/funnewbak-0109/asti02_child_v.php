...
?>
		...
		<!--  <td class="center"><a href="javascript:send_back_asti02('<?php echo $row->mq001;?>','<?php echo $row->mq002;?>');">[ 選擇</a><img src="<?php echo base_url()?>assets/image/png/eye.png" />]</td> -->
		   <td class="center"><a href="javascript:send_back_asti02('<?php echo $row->mb001;?>','<?php echo $row->mb002;?>','<?php echo $row->mb003;?>');">[ 選擇</a><img src="<?php echo base_url()?>assets/image/png/eye.png" />]</td>
		</tr>
		<?php $chkval += 1; ?>
		<?php endforeach;?>
        </tbody>		 
...
function send_back_asti02(mb001, mb002, mb003){
	
	window.parent.$.unblockUI();
	if(window.parent.addasti02disp){	//以此判斷各呼叫此網頁的方法是否存在藉以給予相對應的需求
		window.parent.addasti02disp(mb001,mb002,mb003);
		$.ajax({
			method: "POST",
