...
function check_title_no(){
	$('#tc002').val("");
	var asti03_asti07 = $('#asti03_asti07').val();
	var tc027 = $('#tc027').val();
	//alert(copi03);
	console.log(asti03_asti07);
	console.log(tc027);
	$.ajax({
		method: "POST",
		url: "<?php echo base_url() ?>index.php/ast/asti07/check_title_no",
		data: {
			asti03_asti07: asti03_asti07, 
			tc027: tc027
		}
	})
	.done(function( msg ) {
		if($('#asti03_asti07disp').text()!=""&&$('#asti03_asti07disp').text()!="查無資料")
		$('#tc002').val(msg);
	});
}
...
function clear_row(row_obj){
	...
	for(var k=4;k<=22;k++){//k的最大值請依照實際情況去調整，通常設為欄位數字最大者(即最後一個欄位)
		...
		$('#product-row'+row+' input.order_product_tb'+k).val("");
	}
}
</script>
<script>
...
function check_body_repeat(body){
	...
	var asti02 = body.value;
	var asti02_id = body.id;
	var asti02_num = asti02_id.substr(13,3).match(/\d+/)[0];
	var repeat_num = 0;
	
	$(".check_repeat").each(function(index, element) {
		if(asti02 == element.lastChild.value && asti02 != ''){
			repeat_num += 1;
			console.log(element.lastChild.value);
		}
		console.log(repeat_num);
			
		if(repeat_num >= 2 && asti02 != ''){
			$('#order_product\\['+asti02_num+'\\]\\[asti02\\]').val('');
