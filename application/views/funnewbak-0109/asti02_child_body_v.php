<div class="box2" style="height:95%"> <!-- div-1 -->
    <div class="heading"> 
      ...
      <a onclick="location = '<?php echo base_url()?>index.php/ast/asti02/clear_body_sql'"  style="float:left" accesskey="d" class="button"><span>清除查詢條件 d </span><img height="12" width="12" src="<?php echo base_url()?>assets/image/delete2.png" /></a>
	</div>
<?php 
/* title欄位設定區域 */
	$title_array = array(
        ...
        <thead>
		  ...
		  <?php
			foreach($title_array as $key => $val){
				...
				echo anchor("ast/asti02/display_child_body/".$this->uri->segment(4,0)."/order?val=".$val['sort_name']." asc",$str);
				
				$str = "<img src='".base_url()."assets/image/desc.png' />";
				echo anchor("ast/asti02/display_child_body/".$this->uri->segment(4,0)."/order?val=".$val['sort_name']." desc",$str);
...
?>
        ...
        <tr onclick="parent.addasti02_body('<?php echo $row->mb001;?>','<?php echo $row->mb002;?>','<?php echo $row->mb003;?>','<?php echo $row->mb012;?>','<?php echo $row->mb011;?>','<?php echo $row->mb016;?>','<?php echo $row->mb020;?>');parent.$.unblockUI();" >
          <td class="left"><?php echo $chkval;?></td>
		  <td class="left"><?php echo $row->mb001;?></td>
		  <input id='ma001_<?php echo $chkval?>' value='<?php echo $row->mb001;?>' style='display:none;' />
		  <td class="left"><?php echo $row->mb002;?></td>
		  <input id='ma003_<?php echo $chkval?>' value='<?php echo $row->mb002;?>' style='display:none;' />
		  <td class="left"><?php echo $row->mb003;?></td>
		  <input id='ma008_<?php echo $chkval?>' value='<?php echo $row->mb003;?>' style='display:none;' />
		  <td class="center"><a href="javascript:send_back_asti02('<?php echo $row->mb001;?>','<?php echo $row->mb002;?>','<?php echo $row->mb003;?>','<?php echo $row->mb012;?>','<?php echo $row->mb011;?>','<?php echo $row->mb016;?>','<?php echo $row->mb020;?>');">[ 選擇</a><img src="<?php echo base_url()?>assets/image/png/eye.png" />]</td>
		</tr>
		<?php $chkval += 1; ?>
		<?php endforeach;?>
        </tbody>		 
...
function send_back_asti02(mb001, mb002, mb003, mb012, mb011, mb016, mb020){
	//alert('test1');
	window.parent.$.unblockUI();
	if(window.parent.addasti02_body){	//以此判斷各呼叫此網頁的方法是否存在藉以給予相對應的需求
		window.parent.addasti02_body(mb001, mb002, mb003, mb012, mb011, mb016, mb020);
		$.ajax({
			method: "POST",
...
function filter() {
	...
	url = '<?php echo base_url() ?>index.php/ast/asti02/display_child_body/0/and_where?key=' + encodeURIComponent(key) + 
