        ...
        <thead>
		  ...
		  <?php
			foreach($title_array as $key => $val){
				...
				echo anchor("cms/cmsi06/display_child/".$this->uri->segment(4,0)."/order?val=".$val['sort_name']." asc",$str);
				
				$str = "<img src='".base_url()."assets/image/desc.png' />";
				echo anchor("cms/cmsi06/display_child/".$this->uri->segment(4,0)."/order?val=".$val['sort_name']." desc",$str);
...
?>
        ...
        <tr onclick="parent.addcmsi06disp('<?php echo $row->mf001;?>','<?php echo $row->mf002;?>');parent.$.unblockUI();" >
		  ...
		  <td class="center"><a href="javascript:send_back_cmsi06('<?php echo $row->mf001;?>','<?php echo $row->mf002;?>');">[ 選擇</a><img src="<?php echo base_url()?>assets/image/png/eye.png" />]</td>
		</tr>
		<?php $chkval += 1; ?>
		<?php endforeach;?>
        </tbody>		 
...
function send_back_cmsi06(mf001, mf002){
	//alert('test1');
	window.parent.$.unblockUI();
	if(window.parent.addcmsi06disp){	//以此判斷各呼叫此網頁的方法是否存在藉以給予相對應的需求
		window.parent.addcmsi06disp(mf001,mf002);
		$.ajax({
			method: "POST",
...
function filter() {
	...
	url = '<?php echo base_url() ?>index.php/cms/cmsi06/display_child/0/and_where?key=' + encodeURIComponent(key) + 
