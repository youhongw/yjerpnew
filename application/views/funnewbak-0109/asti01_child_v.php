<div class="box2" style="height:95%"> <!-- div-1 -->
    <div class="heading"> 
      ...
      <a onclick="location = '<?php echo base_url()?>index.php/ast/asti01/clear_sql'"  style="float:left" accesskey="d" class="button"><span>清除查詢條件 d </span><img height="12" width="12" src="<?php echo base_url()?>assets/image/delete2.png" /></a>
	</div>
<?php 
/* title欄位設定區域 */
	$title_array = array(
		'rowid' => array('sort_name'=>"ma001",'name'=>"序號",'width'=>"",'align'=>"left",'use'=>"disable"),
		'ma001' => array('sort_name'=>"ma001",'name'=>"資產類別代號",'width'=>"",'align'=>"left"),
		'ma002' => array('sort_name'=>"ma002",'name'=>"資產類別名稱",'width'=>"",'align'=>"left"),
        ...
        <thead>
		  ...
		  <?php
			foreach($title_array as $key => $val){
				...
				echo anchor("ast/asti01/display_child/".$this->uri->segment(4,0)."/order?val=".$val['sort_name']." asc",$str);
				
				$str = "<img src='".base_url()."assets/image/desc.png' />";
				echo anchor("ast/asti01/display_child/".$this->uri->segment(4,0)."/order?val=".$val['sort_name']." desc",$str);
...
/* filter欄位設定區域 */
	$filter_array = array(
		...
		'ma001' => array('filter_name'=>"ma001",'name'=>"資產類別代號",'size'=>"8",'align'=>"left"),
		'ma002' => array('filter_name'=>"ma002",'name'=>"資產類別名稱",'size'=>"12",'align'=>"left")
	);
?>
        ...
        <tr>
		  ...
		  <input id='ma002_<?php echo $chkval?>' value='<?php echo $row->ma002;?>' style='display:none;' />
		   <td class="center"><a href="javascript:send_back_asti01('<?php echo $row->ma001;?>','<?php echo $row->ma002;?>');">[ 選擇</a><img src="<?php echo base_url()?>assets/image/png/eye.png" />]</td>
		</tr>
		<?php $chkval += 1; ?>
		<?php endforeach;?>
        </tbody>		 
...
function send_back_asti01(ma001, ma002){
	
	window.parent.$.unblockUI();
	if(window.parent.addasti01disp){	//以此判斷各呼叫此網頁的方法是否存在藉以給予相對應的需求
		window.parent.addasti01disp(ma001,ma002);
		$.ajax({
			method: "POST",
...
function filter() {
	...
	url = '<?php echo base_url() ?>index.php/ast/asti01/display_child/0/and_where?key=' + encodeURIComponent(key) + 
