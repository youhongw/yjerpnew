...
<div class="box2"> <!-- div-1 -->
    <div class="heading"> 
      <h1><img src="<?php echo base_url()?>assets/image/order.png" alt="" /> 應付票據建立作業</h1>
	...
	<?php 
		if(!isset($tb003)) { $tb003=date("Y/m/d"); }
	?>
  ...
  <div class="content"> <!-- div-2 -->
    <form method="post" enctype="multipart/form-data" id="form">
        <table class="list"> <!-- 表格開始 -->						
			<tr>
				<td class="normal14a" width="8%" >退票日期： </td>  <!-- dateformat_ymd(this); -->
				<td class="normal14a"  width="25%" >
				<input tabIndex="2"  ondblclick="scwShow(this,event);"   id="tb003" onKeyPress="keyFunction()"  onchange="dateformat_ymd(this);" name="tb003"  value="<?php echo $tb003; ?>"  size="12" type="text" style="background-color:#FFFFE4" required="requierd"  />
				</td>
			</tr>
			<tr>
				<td class="normal14a" width="8%" >匯率： </td>  <!-- dateformat_ymd(this); -->
				<td class="normal14"  >
				<input type="text" id="exchange_rate"   tabIndex="12"   onKeyPress="keyFunction()"    name="exchange_rate" value=""  size="12"  required="requierd"/>				
...
function check_submit(){
	...
	if(tb003 == "" || exchange_rate == ""){
		alert('有欄位空值');
	}else{
		send_back_noti03b(tb003, exchange_rate);
	}
}
function send_back_noti03b(smb001, smb002){
	window.parent.$.unblockUI();
	if(window.parent.addnoti03bdisp){
		window.parent.addnoti03bdisp(smb001,smb002);
		/*a.ajax({
			method:"POST",
			url: "<?php echo base_url() ?>index.php/not/noti04/clear_sql3"
		});*/
	}
}
