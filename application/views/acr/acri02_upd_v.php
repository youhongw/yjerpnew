...
foreach($title_data as $key=>$val){
	$$key = $val;
	if($key == 'ta003' || $key == 'ta038' || $key == 'ta016' || $key == 'ta020' || $key == 'ta021' || $key == 'ta044' || $key == 'ta045'){
...
<div id="content"> <!-- div-3 --> 
 <div class="box"> <!-- div-4 -->
    <div class="heading">
      <h1><img src="<?php echo base_url()?>assets/image/order.png" alt="" /> 結帳單資料建立作業 - 修改　　　</h1>
    <div style="float:left;padding-top: 5px; ">
	<button style= "cursor:pointer" form="commentForm" onfocus="$('#acri01').focus();" type='submit' accesskey="s" name='submit' class="button" onKeyPress="keyFunction()" onclick="addItem();" value='&nbsp;儲存F8&nbsp;'><span>儲 存Alt+s</span><img src="<?php echo base_url()?>assets/image/png/save.png" /></button>&nbsp;&nbsp;
	  <a  accesskey="x" onKeyPress="keyFunction()" id='cancel' name='cancel' href="<?php echo site_url('acr/acri02/display'); ?>" class="button" ><span>返 回Alt+x</span><img src="<?php echo base_url()?>assets/image/png/cancle.png" /></a>
	   <?php if(isset($prev)){ 
			$prev_str = "";$prev_ary = explode('_',$prev);foreach($prev_ary as $k=>$v){if($prev_str){$prev_str.="/";}$prev_str.=$v;}
	  ?>
			<a accesskey="," tabIndex="11" id='prev' name='prev' href="<?php echo site_url('acr/acri02/updform/'.$prev_str); ?>" class="button" ><span>上一筆Alt+<</span><img src="<?php echo base_url()?>assets/image/png/pre.png" /></a>
	  <?php } ?>
	  <?php if(isset($next)){ 
			$next_str = "";$next_ary = explode('_',$next);foreach($next_ary as $k=>$v){if($next_str){$next_str.="/";}$next_str.=$v;}
	  ?>
			<a accesskey="." tabIndex="12" id='next' name='next' href="<?php echo site_url('acr/acri02/updform/'.$next_str); ?>" class="button" ><span>下一筆Alt+></span><img src="<?php echo base_url()?>assets/image/png/next.png" /></a>
	  <?php } ?>
	</div>
	</div>
    <div class="content"> <!-- div-5 -->
	<form  class="cmxform" id="commentForm"  name="form" action="<?php echo base_url()?>index.php/acr/acri02/updsave" method="post" enctype="multipart/form-data" >
	...
	<table class="form14"  >     <!-- 頭部表格 -->
	 <tr>
        ...
        <td class="normal14a"  width="25%"><input tabIndex="1" id="acri01"    onKeyPress="keyFunction()"   name="ta001"  onchange="check_acri01(this);"  value="<?php echo $ta001; ?>" size="12" type="text" required />
        ...
        <td class="normal14a"  width="25%" ><input tabIndex="2"  ondblclick="scwShow(this,event);"   id="ta038" onKeyPress="keyFunction()"  onchange="dateformat_ymd(this);" name="ta038"  value="<?php echo $ta038; ?>"  size="12" type="text" style="background-color:#FFFFE4"  /> 
	...
	<table class="form14">     <!-- 表格 -->
	 <tr>
	  ...
	  <tr>
        ...
        <td  class="normal14"  ><input type="text" tabIndex="31"   onKeyPress="keyFunction()"  onclick="scwShow(this,event);" id="ta016" name="ta016"   value="<?php echo $ta016; ?>" style="background-color:#E7EFEF"   /></td>
...
<form action="<?php echo base_url()?>index.php/acr/acri02/delete_detail" method="post" enctype="multipart/form-data" id="del_form" style="display:none;">
			<input id="del_md001" name="del_md001" />
			<input id="del_md002" name="del_md002" />
			<input id="del_md003" name="del_md003" />
		</form>
<?php  include_once("./application/views/funnew/acri01a_funmjs_v.php"); ?> <!-- 訂單單別 -->
