...
<div id="content">  <!-- div-3 -->
 <div class="box">  <!-- div-4 --><span>　　　　　　</span>
    <div class="heading">
      <h1><img src="<?php echo base_url()?>assets/image/order.png" alt="" /> 分錄性質設定(庫存異動單) - -設定查詢條件　　　</h1>
	  ...
	  <button style= "cursor:pointer" form="commentForm" onfocus="$('#find003').focus();" type='submit' tabIndex="98" accesskey="k" name='submit'  class="button" value='&nbsp;查詢F8&nbsp;'><span>查詢Alt+k</span><img src="<?php echo base_url()?>assets/image/png/find.png" /></button>&nbsp;&nbsp;
	    <a  accesskey="x" tabIndex="97" onKeyPress="keyFunction()"  id='cancel' name='cancel' href="<?php echo site_url('ajs/ajsi02/display'); ?>" class="button" ><span>返 回Alt+x</span><img src="<?php echo base_url()?>assets/image/png/cancle.png" /></a>
	  
	  </div>
	  <form  class="cmxform" id="commentForm"  name="form"  action="<?php echo base_url()?>index.php/ajs/ajsi02/findsql" method="post" enctype="multipart/form-data" >
    </div>
      ...
      <tr>
	    <td class="start14a" width="18">
	       <select name="find001" id="find001" tabIndex="1" >
             <option <?php if($find001 == '1') echo 'selected="selected"';?> value='mb001'> mb001 庫存異動單性質代號 </option>                                                                        
		     <option <?php if($find001 == '2') echo 'selected="selected"';?> value='mb002'> mb002 庫存異動單單別 </option>
	  ...
	  <tr>
	    <td class="start14" width="04">&nbsp;&nbsp;排序欄位：
	      <select name="find006" id="find006" " tabIndex="5" >
            <option <?php if($find006 == '1') echo 'selected="selected"';?> value='mb001'> mb001 庫存異動單性質代號 </option>                                                                        
		    <option <?php if($find006 == '2') echo 'selected="selected"';?> value='mb002'> mb002 庫存異動單單別 </option>
...
function trim(strvalue) {
   ...
   function OnBlur2()
    ...
    function OnBlur9()
     {
	url = '<?php echo base_url()?>/index.php/ajs/ajsi02/findsql/' + find005.value +'/'+ find007.value ; 
        location = url;
     }
</script>