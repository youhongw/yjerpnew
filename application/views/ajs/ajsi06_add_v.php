...
<div id="content">    <!-- div-3 -->
  <div class="box">   <!-- div-4 -->
    <div class="heading">
      <h1><img src="<?php echo base_url()?>assets/image/order.png" alt="" /> 分錄性質設定(銷退成本) - 新增　　　</h1>
	...
	<button style= "cursor:pointer" form="commentForm" onfocus="$('#mb001').focus();" type='submit' accesskey="s" name='submit' class="button" value='&nbsp;儲存F8&nbsp;'><span>儲 存Alt+s</span><img src="<?php echo base_url()?>assets/image/png/save.png" /></button>&nbsp;&nbsp;
	  <a  accesskey="x"   id='cancel' name='cancel' href="<?php echo site_url('ajs/ajsi06/display'); ?>" class="button" ><span>返 回Alt+x</span><img src="<?php echo base_url()?>assets/image/png/cancle.png" /></a>
	
	</div>
	</div>
    <div class="content">  <!-- div-5 -->
	<form   class="cmxform" id="commentForm"  name="form"  method="post" enctype="multipart/form-data"  action="<?php echo base_url()?>index.php/ajs/ajsi06/addsave" >	
	<div id="tab-general">  <!-- div-6 -->
	<?php
	  $date=date("Ymd");
	  $mb001='24';
	  if(!isset($copq03a24)) { $copq03a24=$this->input->post('copq03a24'); }
	  if(!isset($acrq0163disp)) { $copq03a24disp=$this->input->post('copq03a24'); }
	  ...
	  if(!isset($ajsi31disp)) { $ajsi31disp=$this->input->post('ajsi31'); }
	  if(!isset($ajsi31a)) { $ajsi31a=$this->input->post('ajsi31a'); }
	  if(!isset($ajsi31adisp)) { $ajsi31adisp=$this->input->post('ajsi31a'); }
	...
	<table class="form14">     <!-- 表格 -->
	  <tr>
	    <td class="normal14y"  width="14%" ><span class="required">銷退成本性質代號：</span> </td>
	  ...
	  <tr>
	    <td class="normal14z" >銷退成本單別：</td>
		<td class="normal14"  ><input tabIndex="1" id="copq03a24"    onKeyPress="keyFunction()" onfocus="selappr()" onChange="startcopq03a24(this)"  name="copq03a24" value="<?php echo strtoupper($copq03a24); ?>"  type="text" required />
		<a href="javascript:;"><img id="Showcopq03a24" src="<?=base_url()?>assets/image/png/invoice.png" alt="" align="top"/></a>
	 ...
	 <tr>
	  ...
	  <tr>
	    <td class="normal14z" >借方會計科目銷退：</td>		
        <td class="normal14"  >
...
'◎操作說明:[ 欄位名稱紅色代表必需輸入欄位,按Tab鍵或Enter鍵跳下一個欄位,按Shift+Tab鍵跳上一個欄位,Tab鍵停留在有選項欄位按上下鍵可選擇資料. ] ' ?> </div>  <?php } ?>
 ...
 <?php  include_once("./application/views/fun/ajsi06_funjs_v.php"); ?> 
