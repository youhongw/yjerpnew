...
class Invb13_model extends CI_Model {
	...
	function batchaf()           
        {
		  ...
		  //invmc 統計檔 
		   ...
		   // 入庫進貨 purth 品號,庫別, 單位, 數量, 金額,單價mc014 Y 確認碼 th030='Y'
		     $sql11 = " UPDATE  `invmc` AS A,  
       (SELECT th004,th009,th008,IFNULL(sum(th016),0) as th016, IFNULL(sum(th047),0) as th047  FROM `purtg` as c, `purth` as d WHERE tg001=th001 and tg002=th002 
	       and th016>0 and th030='Y'  GROUP BY th004,th009,th008 ) AS B  
    ...
    WHERE A.`mc001`=B.`th004` and A.`mc002`=B.`th009`  and A.`mc003`=B.`th008` "; 
       ...
       (SELECT tj004,tj011,tj007,IFNULL(sum(tj009),0) as tj009, IFNULL(sum(tj010),0) as tj010  FROM `purti` as c, `purtj` as d WHERE ti001=tj001 and ti002=tj002 
	       and tj009>0 and tj020='Y'  GROUP BY tj004,tj011,tj007 ) AS B  
    SET A.`mc007`=A.`mc007`-B.`tj009` ,A.`mc008`=A.`mc008`-B.`tj010` 
    WHERE A.`mc001`=B.`tj004` and A.`mc002`=B.`tj011`  and A.`mc003`=B.`tj007` "; 
	    $this->db->query($sql21); 
	
		   //銷貨 copth 品號,庫別, 單位, 數量, x金額
		     $sql31 = " UPDATE  `invmc` AS A,  
       (SELECT th004,th007,th009,IFNULL(sum(th008),0) as th008, IFNULL(sum(th013),0) as th013  FROM `coptg` as c, `copth` as d WHERE tg001=th001 and tg002=th002 
	       and th008>0 and th020='Y' GROUP BY th004,th007,th009 ) AS B  
    SET A.`mc007`=A.`mc007`-B.`th008` WHERE A.`mc001`=B.`th004` and A.`mc002`=B.`th007` and A.`mc003`=B.`th009`  ";
		    $this->db->query($sql31); 
			
		 //銷退 coptj 品號,庫別, 單位, 數量, x金額
		     $sql32 = " UPDATE  `invmc` AS A,  
       (SELECT tj004,tj013,tj008,IFNULL(sum(tj007),0) as tj007, IFNULL(sum(tj012),0) as tj012  FROM `copti` as c, `coptj` as d WHERE ti001=tj001 and ti002=tj002 
	       and tj007>0 and tj021='Y' GROUP BY tj004,tj013,tj008 ) AS B  
    SET A.`mc007`=A.`mc007`+B.`tj007`  
    WHERE A.`mc001`=B.`tj004` and A.`mc002`=B.`tj013` and A.`mc003`=B.`tj008`  ";
		    $this->db->query($sql32); 
			
		 //轉撥單   轉出庫 - invtb 品號,庫別, 單位, 數量 
		     $sql41 = " UPDATE  `invmc` AS A,  
       (SELECT tb004,tb012,tb008,IFNULL(sum(tb007),0) as tb007, IFNULL(sum(tb011),0) as tb011  FROM `invta` as c, `invtb` as d WHERE ta001=tb001 and ta002=tb002 
	       and tb007>0 and tb018='Y' GROUP BY tb004,tb012,tb008 ) AS B  
    SET A.`mc007`=A.`mc007`-B.`tb007` 
    WHERE A.`mc001`=B.`tb004` and A.`mc002`=B.`tb012` and A.`mc003`=B.`tb008`  ";
		    $this->db->query($sql41); 	
			
		//轉撥單   轉入庫 + invtb 品號,庫別, 單位, 數量 
		     $sql42 = " UPDATE  `invmc` AS A,  
       (SELECT tb004,tb013,tb008,IFNULL(sum(tb007),0) as tb007, IFNULL(sum(tb011),0) as tb011  FROM `invta` as c, `invtb` as d WHERE ta001=tb001 and ta002=tb002 
	       and tb007>0 and tb018='Y' GROUP BY tb004,tb013,tb008 ) AS B  
    SET A.`mc007`=A.`mc007`+B.`tb007` 
    WHERE A.`mc001`=B.`tb004` and A.`mc002`=B.`tb013` and A.`mc003`=B.`tb008`  ";
	 ...
	 //換算單位 invmd
		   // 入庫進貨 purth 品號,庫別, 單位, 數量, 金額,單價mc014 Y 確認碼,換算單位 數量
		     $sqla1 = " UPDATE  `invmc` AS A,  
       (SELECT th004,th009,th008,IFNULL(sum(th016),0) as th016, IFNULL(sum(th047),0) as th047, IFNULL(sum(md003),0) as md003  FROM `invmd` as c, `purth` as d WHERE md001=th004 and md002=th008 
	       and th016>0 and th030='Y'  GROUP BY th004,th009,th008 ) AS B  
    ...
    WHERE A.`mc001`=B.`th004` and A.`mc002`=B.`th009`  and A.`mc003`!=B.`th008` "; 
      ...
      // 退庫退貨 purtj
       ...
       (SELECT tj004,tj011,tj007,IFNULL(sum(tj009),0) as tj009, IFNULL(sum(tj010),0) as tj010, IFNULL(sum(md003),0) as md003  FROM `invmd` as c, `purtj` as d WHERE md001=tj004 and md002=tj007 
	       and tj009>0 and tj020='Y'  GROUP BY tj004,tj011,tj007 ) AS B  
    ...
    WHERE A.`mc001`=B.`tj004` and A.`mc002`=B.`tj011`  and A.`mc003`!=B.`tj007` "; 
       ...
       (SELECT th004,th007,th009,IFNULL(sum(th008),0) as th008, IFNULL(sum(th013),0) as th013, IFNULL(sum(md003),0) as md003  FROM `invmd` as c, `copth` as d WHERE md001=th004 and md002=th009 
	       and th008>0 and th020='Y' GROUP BY th004,th007,th009 ) AS B  
    ...
    WHERE A.`mc001`=B.`th004` and A.`mc002`=B.`th007` and A.`mc003`!=B.`th009`  ";
         ...
         //銷退 coptj 品號,庫別, 單位, 數量, x金額
		     $sql32 = " UPDATE  `invmc` AS A,  
       (SELECT tj004,tj013,tj008,IFNULL(sum(tj007),0) as tj007, IFNULL(sum(tj012),0) as tj012, IFNULL(sum(md003),0) as md003  FROM `invmd` as c, `coptj` as d WHERE md001=tj004 and md002=tj008 
	       and tj007>0 and tj021='Y' GROUP BY tj004,tj013,tj008 ) AS B  
