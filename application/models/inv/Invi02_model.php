...
class Invi02_model extends CI_Model
{
	...
	function construct_sql($limit = 15, $offset = 0, $func = "")
	{
		...
		if ($this->uri->segment(6)) {
			...
			$_SESSION['invi02']['search']['seq1'] = trim($this->uri->segment(6));
		} else {
			if (isset($_SESSION['invi02']['search']['seq1']))
				$seq1 = $_SESSION['invi02']['search']['seq1'];
			else
				$seq1 = '';
		}
		...
		// /* Data SQL */
		// $query = $this->db->select('mb001, mb002, mb003, mb004, mb005, mb006,mz002,mz003, mb017,b.mc002 as mb017disp,mb072, mb025, mb069, mb200, a.create_date')
		// 	->from('invmb as a')
		// 	->join('cmsmc as b', 'mb017 = b.mc001', 'left')
		// 	->join('invmz as c', 'mb001 = c.mz001', 'left')
		// 	->order_by($order);
		// if ($where) {
		// 	$query->where($where);
		// }
		...
		$sql = " SELECT MB001, MB002, MB003, MB004, MB017,MC002 as MB017disp  FROM  INVMB 
		 				left join CMSMC on MC001 = MB017
				WHERE  $where and MB001 IN 
			(SELECT TOP $limit MB001 FROM INVMB WHERE $where and MB001 NOT IN
				(SELECT TOP $offset  MB001 FROM INVMB WHERE $where ORDER BY MB001)
			ORDER BY MB001)
		 ORDER BY MB001 
		 ";
		if ($where == "") {
			$sql = " SELECT MB001, MB002, MB003, MB004, MB017,MC002 as MB017disp  FROM  INVMB 
			 			left join CMSMC on MC001 = MB017
					WHERE MB001 IN 
				(SELECT TOP $limit MB001 FROM INVMB WHERE MB001 NOT IN
					(SELECT TOP $offset MB001 FROM INVMB ORDER BY MB001)
				ORDER BY MB001)
			ORDER BY MB001
			";
		}
		...
		if ($this->uri->segment(3) == "display_child" && ($seq1)) {
			$sql = " SELECT MB001, MB002, MB003, MB004, MB017,MC002 as MB017disp  FROM  INVMB 
							left join CMSMC on MC001 = MB017
					WHERE MB001 like '%$seq1%' and  $where and MB001 IN 
			(SELECT TOP $limit MB001 FROM INVMB WHERE MB001 like '%$seq1%' and $where and MB001 NOT IN
				(SELECT TOP $offset  MB001 FROM INVMB WHERE MB001 like '%$seq1%' and $where ORDER BY MB001)
			ORDER BY MB001)
		 ORDER BY MB001 
		 ";
			if ($where == "") {
				$sql = " SELECT MB001, MB002, MB003, MB004, MB017,MC002 as MB017disp  FROM  INVMB 
				 				left join CMSMC on MC001 = MB017
						WHERE MB001 like '%$seq1%' and  MB001 IN 
				(SELECT TOP $limit MB001 FROM INVMB WHERE MB001 like '%$seq1%' and  MB001 NOT IN
					(SELECT TOP $offset MB001 FROM INVMB WHERE MB001 like '%$seq1%' ORDER BY MB001)
				ORDER BY MB001)
			ORDER BY MB001
			";
			}
		...
		$sql = " SELECT count(*) as total_num from INVMB WHERE $where ";
		if ($where == "") {
			$sql = " SELECT count(*) as total_num from INVMB ";
		}
		if ($this->uri->segment(3) == "display_child" && ($seq1)) {
			$sql = " SELECT count(*) as total_num from INVMB WHERE MB001 like '%$seq1%' and $where ";
			if ($where == "") {
				$sql = " SELECT count(*) as total_num from INVMB WHERE MB001 like '%$seq1%' ";
			}
		}
		$query = $this->db->query($sql);
	...
	function construct_sqli($limit = 15, $offset = 0, $func = "")
	{
		$this->session->set_userdata('invi02_search', "display_search/" . $offset);
		if (session_status() == PHP_SESSION_NONE) {
			session_start();
		...
		if ($this->uri->segment(3, 0) == "display_childi") {
			unset($_SESSION['invi02']['search']);
		}
		...
		$sql = " SELECT MB001, MB002, MB003, MB004, MB017,MC002 as MB017disp  FROM  INVMB 
		 				left join CMSMC on MC001 = MB017
				WHERE  $where and (SUBSTRING(MB001, 1, 2) = 'IM' or SUBSTRING(MB001, 1, 2) = 'CR'  or SUBSTRING(MB001, 1, 2) = 'RW' or SUBSTRING(MB001, 1, 2) = 'PU' 
				or SUBSTRING(MB001, 1, 2) = 'FW' or SUBSTRING(MB001, 1, 2) = 'AW' or SUBSTRING(MB001, 1, 2) = 'DS') and MB001 IN 
			(SELECT TOP $limit MB001 FROM INVMB WHERE $where and (SUBSTRING(MB001, 1, 2) = 'IM' or SUBSTRING(MB001, 1, 2) = 'CR'  or SUBSTRING(MB001, 1, 2) = 'RW' or SUBSTRING(MB001, 1, 2) = 'PU' 
				or SUBSTRING(MB001, 1, 2) = 'FW' or SUBSTRING(MB001, 1, 2) = 'AW' or SUBSTRING(MB001, 1, 2) = 'DS') and MB001 NOT IN
				(SELECT TOP $offset  MB001 FROM INVMB WHERE $where and (SUBSTRING(MB001, 1, 2) = 'IM' or SUBSTRING(MB001, 1, 2) = 'CR'  or SUBSTRING(MB001, 1, 2) = 'RW' or SUBSTRING(MB001, 1, 2) = 'PU' 
				or SUBSTRING(MB001, 1, 2) = 'FW' or SUBSTRING(MB001, 1, 2) = 'AW' or SUBSTRING(MB001, 1, 2) = 'DS') ORDER BY MB001)
			ORDER BY MB001)
		 ORDER BY MB001 
		 ";
		if ($where == "") {
			$sql = " SELECT MB001, MB002, MB003, MB004, MB017,MC002 as MB017disp  FROM  INVMB 
			 			left join CMSMC on MC001 = MB017
					WHERE (SUBSTRING(MB001, 1, 2) = 'IM' or SUBSTRING(MB001, 1, 2) = 'CR'  or SUBSTRING(MB001, 1, 2) = 'RW' or SUBSTRING(MB001, 1, 2) = 'PU' 
				or SUBSTRING(MB001, 1, 2) = 'FW' or SUBSTRING(MB001, 1, 2) = 'AW' or SUBSTRING(MB001, 1, 2) = 'DS') and MB001 IN 
				(SELECT TOP $limit MB001 FROM INVMB WHERE (SUBSTRING(MB001, 1, 2) = 'IM' or SUBSTRING(MB001, 1, 2) = 'CR'  or SUBSTRING(MB001, 1, 2) = 'RW' or SUBSTRING(MB001, 1, 2) = 'PU' 
				or SUBSTRING(MB001, 1, 2) = 'FW' or SUBSTRING(MB001, 1, 2) = 'AW' or SUBSTRING(MB001, 1, 2) = 'DS') and MB001 NOT IN
					(SELECT TOP $offset MB001 FROM INVMB where (SUBSTRING(MB001, 1, 2) = 'IM' or SUBSTRING(MB001, 1, 2) = 'CR'  or SUBSTRING(MB001, 1, 2) = 'RW' or SUBSTRING(MB001, 1, 2) = 'PU' 
				or SUBSTRING(MB001, 1, 2) = 'FW' or SUBSTRING(MB001, 1, 2) = 'AW' or SUBSTRING(MB001, 1, 2) = 'DS') ORDER BY MB001)
				ORDER BY MB001)
			ORDER BY MB001
			";
		...
		// $ret['num'] = $query->get()->result();
		// $ret['num'] = $ret['num'][0]->total_num;
		$sql = " SELECT count(*) as total_num from INVMB WHERE $where and (SUBSTRING(MB001, 1, 2) = 'IM' or SUBSTRING(MB001, 1, 2) = 'CR'  or SUBSTRING(MB001, 1, 2) = 'RW' or SUBSTRING(MB001, 1, 2) = 'PU' 
		or SUBSTRING(MB001, 1, 2) = 'FW' or SUBSTRING(MB001, 1, 2) = 'AW' or SUBSTRING(MB001, 1, 2) = 'DS') ";
		if ($where == "") {
			$sql = " SELECT count(*) as total_num from INVMB 
			WHERE (SUBSTRING(MB001, 1, 2) = 'IM' or SUBSTRING(MB001, 1, 2) = 'CR'  or SUBSTRING(MB001, 1, 2) = 'RW' or SUBSTRING(MB001, 1, 2) = 'PU' 
				or SUBSTRING(MB001, 1, 2) = 'FW' or SUBSTRING(MB001, 1, 2) = 'AW' or SUBSTRING(MB001, 1, 2) = 'DS')";
	...
	function findf($limit, $offset, $sort_by, $sort_order)
	{
		...
		//下一頁不會亂跳
		if (@$_SESSION['invi02_sql_term']) {
			$seq32 = $_SESSION['invi02_sql_term'];
		}
		if (@$_SESSION['invi02_sql_sort']) {
			$seq33 = $_SESSION['invi02_sql_sort'];
		}
	...
	{
		$sql98 = " select MC002 as MB017disp,* from dbo.INVMB 
						left join CMSMC on MC001 = MB017
					where MB001 = '$keyword' 
					";
		...
		if ($query->num_rows() > 0) {
			...
			foreach ($result as $key => $val) {
				$data['response'] = true;
				$data['mc001disp'] = stripslashes(htmlspecialchars(mb_convert_encoding(trim($val->MB002), "utf-8", "big-5"), ENT_QUOTES));
				...
				$data['tb012'] = stripslashes(htmlspecialchars(mb_convert_encoding(trim($val->MB002), "utf-8", "big-5"), ENT_QUOTES));
				$data['tb013'] = stripslashes(htmlspecialchars(mb_convert_encoding(trim($val->MB003), 'utf-8', 'big-5'), ENT_QUOTES));
				$data['tb007'] = mb_convert_encoding(trim($val->MB004), "utf-8", "big-5");
				$data['tb009'] = trim($val->MB017);
				$data['tb009disp'] = stripslashes(htmlspecialchars(mb_convert_encoding(trim($val->MB017disp), 'utf-8', 'big-5'), ENT_QUOTES));
			}
		} else {
	...
	function lookup_check($keyword)
	{
		// $this->db->select('mb001, mb002, mb003, mb004,mb072,mb025, mb017, b.mc002 as mb017disp,mz002,mz003');
		// $this->db->from('invmb');
		// $this->db->join('cmsmc as b', 'mb017 = b.mc001', 'left');
		// $this->db->join('invmz as c', 'mb001 = c.mz001', 'left');
		// $this->db->where('mb001', $keyword);
		// $query = $this->db->get();
		// return $query->result();

		$sql98 = " select MB001,MB002,MB003,MB004,MB017 from dbo.INVMB where MB001 = '$keyword' ";
		$query = $this->db->query($sql98);
		...
		if ($query->num_rows() > 0) {
			...
			foreach ($result as $key => $val) {
				$data['response'] = true;
				$data['invi02disp'] = stripslashes(htmlspecialchars(mb_convert_encoding(trim($val->MB002), "utf-8", "big-5"), ENT_QUOTES));
				$data['ta035'] = stripslashes(htmlspecialchars(mb_convert_encoding(trim($val->MB003), 'utf-8', 'big-5'), ENT_QUOTES));
				$data['ta007'] = mb_convert_encoding(trim($val->MB004), "utf-8", "big-5");
				$data['cmsi03'] = trim($val->MB017);
			}
		} else {
			$data['invi02disp'] = "資料找不到!請重新輸入";
		}
	...
	function ajaxkey1($seq1)
	{
		$sql98 = " select * from dbo.INVMB where MB001 = '$seq1' ";
		$query = $this->db->query($sql98);
		$result = $query->result();

		if (count($result) > 0) {
			return mb_convert_encoding($result[0]->MB002, "utf-8", "big-5");
		} else {
			return 'N';
		}
	}

	function ajaxkey2($seq1)
	{
		$sql98 = " select MB001, MB002, MB003, MB004 from dbo.INVMB where MB001 = '$seq1' ";
		$query = $this->db->query($sql98);
		$result = $query->result();
		...
		foreach ($result as $key => $val) {
			$data['response'] = true;
			$data['MB001'] = trim($val->MB001);
			// $data .= trim($val->MB001) . '_';
			$data['MB002']  = mb_convert_encoding(trim($val->MB002), "utf-8", "big-5");
			// $data .= mb_convert_encoding(trim($val->MB002), "utf-8", "big-5") . '_';
			$data['MB003']  = mb_convert_encoding(trim($val->MB003), 'utf-8', 'big-5');
			// $data .= stripslashes(htmlspecialchars(mb_convert_encoding(trim($val->MB003), 'utf-8', 'big-5'), ENT_QUOTES)) . '_';
			$data['MB004']  = mb_convert_encoding(trim($val->MB004), "utf-8", "big-5");
			// $data .= mb_convert_encoding(trim($val->MB004), "utf-8", "big-5");
		}
