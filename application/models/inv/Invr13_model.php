...
class invr13_model extends CI_Model {
	...
	function printfd()          
        {
		  ...
		  $seq5=$this->input->post('td001');  //盤點底稿
	      $seq6=substr($this->input->post('td002'),0,4).substr($this->input->post('td002'),5,2).substr(rtrim($this->input->post('td002')),8,2);   //盤點日期
		  $this->db->select('a.*,c.mc002 as td005disp,b.mb002,b.mb003,b.mb004 '); 
		 
        $this->db->from('invtd as a');	
        $this->db->join('invmb as b', 'a.td003 = b.mb001 ','left');	//品號	
		$this->db->join('cmsmc as c', 'a.td005 = c.mc001 ','left');   //庫別
	
		$this->db->where('a.td003 >=', $seq1); 
	    $this->db->where('a.td003 <=', $seq2); 
		$this->db->where('a.td005 >=', $seq3); 
	    $this->db->where('a.td005 <=', $seq4); 
		$this->db->where('a.td001', $seq5); 
	    $this->db->where('a.td002', $seq6);
		$this->db->order_by('a.td001,a.td005,a.td003');
		
		$query = $this->db->get();
		
       $ret['rows'] = $query->result();  
          $seq32 = "td003 >= '$seq1'  AND td003 <= '$seq2' AND td005 >= '$seq3'  AND td005 <= '$seq4' AND td001 = '$seq5'  AND td002 = '$seq6' ";	
