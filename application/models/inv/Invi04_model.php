...
class Invi04_model extends CI_Model {
	...
	{
		$this->session->set_userdata('invi04_search',"display_search/".$offset);
		...
		if(isset($_SESSION['invi04']['search']['where'])){
			if($where){$where .= " and ";}
			$where .= $_SESSION['invi04']['search']['where'];
		}
		...
		if(isset($_SESSION['invi04']['search']['order'])){
			if($order){$order .= " , ";}
			$order .= $_SESSION['invi04']['search']['order'];
		}
		
		if(!isset($_SESSION['invi04']['search']['order']) && $default_order){
			if($order){$order .= " , ";}
			$order .= $default_order;
		}
		/* order end */
		...
		$ret['data'] = $query->get()->result();
		//儲存sql
		$_SESSION['invi04']['search']['sql'] = $this->db->last_query();
		...
		//儲存where與order
		$_SESSION['invi04']['search']['where'] = $where;
		$_SESSION['invi04']['search']['order'] = $order;
		$_SESSION['invi04']['search']['offset'] = $offset;
	...
	function findf($limit, $offset, $sort_by, $sort_order)     
       {            		
		 ...
		 //下一頁不會亂跳
		if(@$_SESSION['invi04_sql_term']){$seq32 = $_SESSION['invi04_sql_term'];}
		if(@$_SESSION['invi04_sql_sort']){$seq33 = $_SESSION['invi04_sql_sort'];}
	...
	function insertf()   
       ...
       //    }
	    ...
	    $exist = $this->invi04_model->selone1($this->input->post('mq001'));
	    if ($exist)
	      {
		    return 'exist';
		  } 
    ...
    function copyf()           //複製一筆
       {
	    ...
	    $data = array( 
            ...
            $exist = $this->invi04_model->selone2($this->input->post('mq002c'));
		    if ($exist)
		      {
			   return 'exist';
		      }         
