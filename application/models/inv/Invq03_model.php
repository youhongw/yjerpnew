...
class Invq03_model extends CI_Model {
	...
	function search($limit, $offset, $sort_by, $sort_order)     
	    { 
	     ...
	     $sort_columns = array('mc001', 'mc002', 'mc003', 'mc004', 'mc017', 'mc064','mc065','create_date');
	     $sort_by = (in_array($sort_by, $sort_columns)) ? $sort_by : 'mc001';  //檢查排序欄位是否在 table 內
	     $query = $this->db->select('mc001, mc002, mc003, mc004,mc007,mc008,b.mb002 as mc001disp, b.mb003 as mc001disp1,b.mb004 as mc001disp2 ,a.create_date')
	                       ->from('invmc as a')
						   ->join('invmb as b', 'a.mc001 = b.mb001 ','left')   //品號
	...
	function selone($seq1)    
        {
		  $this->db->select('a.* ,d.mc002 AS mc017disp,d.mc002 as mc002disp,
        ...
        $this->db->join('invmc as b', 'a.mc001 = b.mc001   ','left');		
	//	$this->db->join('cmsmc as c', 'a.mc017 = c.mc001  ','left');
		$this->db->join('cmsmc as d', 'b.mc002 = d.mc001  ','left');
		$this->db->where('a.mc001', $this->uri->segment(4)); 
		$this->db->order_by('a.mc001 ');
	...
	function lookup($keyword){     
      $this->db->select('mc001, mc002, mc003,mc004')->from('invmc');  
	...
	function ajaxchkno1($seg1)    
        { 	              
		  ...
		  $this->db->where('mc001', $this->uri->segment(4));
	      $this->db->where('mc013', $this->uri->segment(5));
	...
	function findf($limit, $offset, $sort_by, $sort_order)     
         {            		
	      ...
	      $seq11 = "SELECT COUNT(*) as count  FROM `invmc` ";
	      $seq1 = "mc001, mc002, mc003, mc004, mc08,mc010,mc011,mc013,mc012, create_date FROM `invmc` ";
        ...
        if ($seq7!='') {$seq9=" ORDER BY ".$seq7;$seq33=$seq7;}
		 //下一頁不會亂跳
		if(@$_SESSION['invq03_sql_term']){$seq32 = $_SESSION['invq03_sql_term'];}
		if(@$_SESSION['invq03_sql_sort']){$seq33 = $_SESSION['invq03_sql_sort'];}
		
         $sort_order = (substr($sort_order,0,3) == 'asc') ? 'asc' : 'desc';
	     $sort_columns = array('mc001', 'mc002', 'mc003', 'mc004', 'mc005', 'mc006','mc007','mc008','mc010','mc011','mc013','mc012','create_date');
	     $sort_by = (in_array($sort_by, $sort_columns)) ? $sort_by : 'mc001';  //檢查排序欄位是否在 table 內
	 //   $query = $this->db->select('mc001, mc002, mc003, mc004, , mc017, mc064,mc065,mc010,mc011,mc013,mc012,mc016, create_date')
	 //                      ->from('invmc')
		 $query = $this->db->select('mc001, mc002, mc003, mc004,mc007,mc008,b.mb002 as mc001disp, b.mb003 as mc001disp1,b.mb004 as mc001disp2 ,a.create_date')
	                       ->from('invmc as a')
						   ->join('invmb as b', 'a.mc001 = b.mb001 ','left')   //品號
	...
	function filterf1($limit, $offset , $sort_by  , $sort_order)          
	    ...
	    //  $this->db->select('mc001, mc002, mc003, mc004, mc017, mc064,mc065, create_date');
	    //  $this->db->from('invmc');
		  $this->db->select('mc001, mc002, mc003, mc004,mc007,mc008,b.mb002 as mc001disp, b.mb003 as mc001disp1,b.mb004 as mc001disp2 ,a.create_date');
	      $this->db->from('invmc as a');
	...
	//新增一筆 檔頭  invmc	
	function insertf()    //新增一筆 檔頭  invmc
        {
	     $data = array( 
		         ...
		         'mc004' => $this->input->post('cmsq05a'),
		         'mc005' => $this->input->post('mc005'),
		         'mc006' => $this->input->post('mc006'),
                 'mc007' => 'Y',
                 'mc008' => 0,
                 'mc009' => '9',
                 'mc010' => strtoupper($this->input->post('cmsq02a')),		
                 'mc011' => $this->input->post('mc011'),		
                 'mc012' => $this->input->post('palq01a'),
                 'mc013' => substr($this->input->post('mc013'),0,4).substr($this->input->post('mc013'),5,2).substr(rtrim($this->input->post('mc013')),8,2),
                 'mc014' => $this->session->userdata('manager'),		
                 'mc015' => $this->input->post('mc015'),		
                 'mc016' => 'N'		
                 
                );
         
	      $exist = $this->invq03_model->selone1($this->input->post('purq04a31'),$this->input->post('mc002'));
	      if ($exist)
	         {
		      return 'exist';
		     } 
             $this->db->insert('invmc', $data);
		...
		//	while (($_POST['order_product'][  $n  ]['mc004']) > '0' ) {
	      ...
	      $exist = $this->invq03_model->selone1d($this->input->post('purq04a31'),$this->input->post('mc002'));
    ...
    function copyf()           
        {
         ...
         //   if ($query->num_rows() != 1) { return 'exist'; }
            ...
            $seq1=$this->input->post('mc001c');    //主鍵一筆檔頭invmc
	        ...
	        $data = array( 
		           ...
		           'mc011' => $mc011,'mc012' => $mc012,'mc013' => $mc013,'mc014' => $mc014,'mc015' => $mc015,'mc016' => $mc016
                   );
				   
            $exist = $this->invq03_model->selone2($this->input->post('mc001c'),$this->input->post('mc002c'));
		    if ($exist)
		      {
			    return 'exist';
		      }         
             $this->db->insert('invmc', $data);      //複製一筆  
          ...
          //  if ($query->num_rows() != 1) { return 'exist'; }
			...
			$seq1=$this->input->post('mc001c');    //主鍵一筆明細invmc
			$seq2=$this->input->post('mc002c'); 
              $i=0;
            while ($i<$num) {	
			  $data_array = array( 
	...
	function excelnewf()           
	    ...
	    //  $seq4=$this->input->post('mc002c');
	      $sql = " SELECT mc001, b.mb002 as mc001disp, b.mb003 as mc001disp1,b.mb004 as mc001disp2,mc002, mc003, mc004,mc007,mc008,a.create_date FROM invmc as a left join  invmb as b on a.mc001=b.mb001 WHERE mc001 >= '$seq1'  AND mc001 <= '$seq2'  "; 
	...
	function printfd()          
	 ...
	 //  $query = $this->db->select('mc001, mc002, mc003, mc004,mc007,mc008,b.mb002 as mc001disp, b.mb003 as mc001disp1,b.mb004 as mc001disp2 ,a.create_date')
	   //                    ->from('invmc as a')
		//				   ->join('invmb as b', 'a.mc001 = b.mb001 ','left')   //品號
	      $sql = " SELECT mc001, mc002, mc003, mc004,mc007,mc008,b.mb002 as mc001disp, b.mb003 as mc001disp1,b.mb004 as mc001disp2 ,a.create_date FROM invmc as a  left join  invmb as b on a.mc001=b.mb001 WHERE mc001 >= '$seq1'  AND mc001 <= '$seq2' and mc002 >= '$seq3'  AND mc002 <= '$seq4'  "; 
          $query = $this->db->query($sql);
	      $ret['rows'] = $query->result();
	...
	function printfd1()   
       {           
           $this->db->select('a.* ,c.mq002 AS mc001disp, d.me002 AS mc004disp, e.mc002 AS mc010disp, f.mv002 AS mc012disp,
        ...
        $this->db->from('invmc as a');	
        $this->db->join('invmc as b', 'a.mc001 = b.mc001  and a.mc002=b.mc002 ','left');		
		...
		$this->db->order_by('mc001 , mc002 ,b.mc003');
	...
	function updatef()   
        {
          $data = array(			
		        ...
		        'mc003' => substr($this->input->post('mc003'),0,4).substr($this->input->post('mc003'),5,2).substr($this->input->post('mc003'),8,2),
			    'mc004' => $this->input->post('cmsq05a'),
		        'mc005' => $this->input->post('mc005'),'mc006' => $this->input->post('mc006'),'mc007' => $this->input->post('mc007'),
                'mc008' => $this->input->post('mc008'),'mc009' => $this->input->post('mc009'),'mc010' => $this->input->post('cmsq02a'),
                'mc011' => $this->input->post('mc011'),'mc012' => $this->input->post('palq01a'),
				'mc013' => substr($this->input->post('mc013'),0,4).substr($this->input->post('mc013'),5,2).substr($this->input->post('mc013'),8,2),
                'mc014' => $this->input->post('mc014'),'mc015' => $this->input->post('mc015'),'mc016' => $this->input->post('mc016')
                );
            $this->db->where('mc001', $this->input->post('purq04a31'));
			$this->db->where('mc002', $this->input->post('mc002'));
            $this->db->update('invmc',$data);                   //更改一筆
