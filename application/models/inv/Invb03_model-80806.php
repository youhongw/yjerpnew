	...
	function batchaf()           
        {
		  ...
		  //invlc 統計檔 
			...
			$sql = " INSERT INTO  invlc (lc001,lc002,lc003)
			            SELECT mc001,$vyymm,mc002  from invmc ";
            $this->db->query($sql); 
       ...
       (SELECT th004,th009,IFNULL(sum(th016),0) as th016, IFNULL(sum(th047),0) as th047  FROM `purtg` as c, `purth` as d WHERE tg001=th001 and tg002=th002 
	       and th016>0 and substring(tg003,1,6)=$vyymm GROUP BY th004,th009 ) AS B  
    SET A.`lc006`=B.`th016` ,A.`lc003`=B.`th047` ,A.`lc030`=B.`th047`/B.`th016`
    WHERE A.`lc001`=B.`th004` and A.`lc003`=B.`th009` and A.`lc002`=$vyymm  "; 
	    $this->db->query($sql11); 
	
		   //銷貨 copth 
		     $sql12 = " UPDATE  `invlc` AS A,  
       (SELECT th004,th007,IFNULL(sum(th008),0) as th008, IFNULL(sum(th013),0) as th013  FROM `coptg` as c, `copth` as d WHERE tg001=th001 and tg002=th002 
	       and th008>0 and substring(tg003,1,6)=$vyymm GROUP BY th004,th007 ) AS B  
    SET A.`lc008`=B.`th008` ,A.`lc009`=B.`th013` 
