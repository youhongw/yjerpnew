...
class Invr20_model extends CI_Model {
	...
	function printfd()          
        {
		 ...
		 $seq3=substr($this->input->post('mc003'),0,4).substr($this->input->post('mc003'),5,2).substr(rtrim($this->input->post('mc003')),8,2);  //異動日期
	     $seq4=substr($this->input->post('mc0031'),0,4).substr($this->input->post('mc0031'),5,2).substr(rtrim($this->input->post('mc0031')),8,2);
		  
		  
		if($seq3 =='') { $q3="";} else {$q3=" and LA004>= '$seq3'";}
	    if($seq4 =='') { $q4="";} else {$q4=" and LA004<= '$seq4'";}
		 
		 
        $sql5 = "SELECT LA001,LA004,LA005,LA006,LA007,LA008,LA009,MC002,LA010,LA011*LA005 AS 'LA011',LA012,LA013  FROM INVLA ,CMSMC
			WHERE LA001 = '$seq1'  AND LA009 = '111' AND LA009 = MC001  $q3$q4
			GROUP BY LA001,LA004,LA005,LA006,LA007,LA008,LA009,LA010,LA011,LA012,LA013,MC002
			ORDER BY LA004";
		  
		$query = $this->db->query($sql5);
		$ret['rows'] = $query->result();

		$sql_chk = "SELECT COUNT(*) AS NUM FROM INVLA ,CMSMC
			WHERE LA001 = '$seq1'  AND LA009 = '111' AND LA009 = MC001  $q3$q4
			GROUP BY LA001,LA004,LA005,LA006,LA007,LA008,LA009,LA010,LA011,LA012,LA013,MC002";
		...
		$sql6 = "SELECT LA001,MB002,MB003,SUM(LA011*LA005) QTY  
			 FROM INVLA  A, INVMB B
			WHERE    LA001=MB001 AND LA001 = '$seq1'  AND LA009 = '111' $q3$q4     
			GROUP BY LA001,MB002,MB003";
			
		$query2 = $this->db->query($sql6);
        $ret['rows1'] = $query2->result();  
