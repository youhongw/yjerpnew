...
class Invi05_model extends CI_Model {
	...
	function construct_sql($limit = 15, $offset = 0, $func = "")
	  {
		...
		if(isset($_SESSION['invi05']['search']['where'])){
			if($where){$where .= " and ";}
			$where .= $_SESSION['invi05']['search']['where'];
		}
		...
		if(isset($_SESSION['invi05']['search']['order'])){
			if($order){$order .= " , ";}
			$order .= $_SESSION['invi05']['search']['order'];
		}
		
		if(!isset($_SESSION['invi05']['search']['order']) && $default_order){
			if($order){$order .= " , ";}
			$order .= $default_order;
		}
		/* order end */
		...
		$ret['data'] = $query->get()->result();
		//儲存sql 語法
		$_SESSION['invi05']['search']['sql'] = $this->db->last_query();
		...
		//儲存where與order
		$_SESSION['invi05']['search']['where'] = $where;
		$_SESSION['invi05']['search']['order'] = $order;
		$_SESSION['invi05']['search']['offset'] = $offset;
	...
	function construct_view($data){
		...
		$_SESSION['invi05']['search']['view'] = $view_array;
		$_SESSION['invi05']['search']['index'] = $index_array;
		//echo "<pre>";var_dump($_SESSION['invi05']['search']['view']);exit;
	}	
	//查詢修改用 (看資料用)   
	...
	function findf($limit, $offset, $sort_by, $sort_order)     
		...
		//下一頁不會亂跳
		if(@$_SESSION['invi05_sql_term']){$seq32 = $_SESSION['invi05_sql_term'];}
		if(@$_SESSION['invi05_sql_sort']){$seq33 = $_SESSION['invi05_sql_sort'];}
	...
	function insertf()    //新增一筆 檔頭  invta 
        {
			 ...
			 //檢查資料是否已存在 若存在加1
			  while($this->invi05_model->selone1($ta001,$ta002)>0){
		   ...
		   foreach($order_product as $key => $val){
		        if($val['tb003'] && $val['tb004']){
			           ...
			           // $tb012 = implode('',$matches[0]);
						...
						foreach($val as $k=>$v){
							if($k!="tb001"&&$k!="tb002"&&$k!="tb012disp"){//主鍵不用更改以及其他外來鍵庫別名稱
	...
	function updatef()   
        {
          ...
          $data = array(			
			...
			$sql="select tb001,tb002,tb004,tb012,tb007,tb011 from invtb where tb001='$ta001' and tb002='$ta002' ";
			$query = $this->db->query($sql) ;
		    foreach ($query->result() as $row) {
            foreach($row as $i=>$v){
            $$i=$v;
			...
			foreach($order_product as $key => $val){
			   ...
			   // $tb012 = implode('',$matches[0]);
				if($this->seldetail($ta001,$ta002,$val['tb003'])>0){
					...
					foreach($val as $k=>$v){
						if($k!="tb001"&&$k!="tb002"&&$k!="tb012disp" ){//主鍵不用更改以及其他外來鍵庫別名稱 tb013日期等別處理
							if($k=="tb003") {$data[$k] = $vtb003;} else {$data[$k] = $v;}
						}
					}
					$this->db->where('tb001', $ta001);
	    ...
	    if (!empty($_POST['selected'])) 
	         {
			    ...
			    $query82 = $this->db->query("SELECT tb004,tb012,tb007,tb011   FROM invtb as a 
