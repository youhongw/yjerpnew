...
class Invi09_model extends CI_Model {
	...
	function selbrowse($num,$offset)   
       {            
	    $this->db->select('mf001, mf002, mf003, create_date');
        $this->db->from('invmf');
	...
	function search($limit, $offset, $sort_by, $sort_order)     
	  { 
	    ...
	    $sort_columns = array('mf001', 'mf002', 'mf003', 'create_date');
	    $sort_by = (in_array($sort_by, $sort_columns)) ? $sort_by : 'mf001';  //檢查排序欄位是否在 table 內
	    $query = $this->db->select('mf001, mf002, mf003, create_date')
	                      ->from('invmf')
	...
	function selone()    
       { 
		 $this->db->select('mf001,mf002,mf003,flag');	
		 $this->db->from('invmf');
	     //$this->db->set('me001', $this->uri->segment(4)); 
	...
	function findf($limit, $offset, $sort_by, $sort_order)     
       {            		
	     ...
	     $seq11 = "SELECT COUNT(*) as count  FROM `invmf` ";
	     $seq1 = " mf001, mf002, mf003, create_date FROM `invmf` ";
	...
	function insertf()   
       {
	    ...
	    $exist = $this->invi09_model->selone1($this->input->post('mf001'));
	    if ($exist)
	      {
		    return 'exist';
		  } 
           return  $this->db->insert('invmf', $data);
       }
	   
	//查複製資料是否重複	 
    function selone2($seg1)    
       { 	
	    $this->db->set('mf001', $this->input->post('mf002c')); 
    ...
    function copyf()           //複製一筆
       {
	    ...
	    $data = array( 
            ...
            $exist = $this->invi09_model->selone2($this->input->post('mf002c'));
		    if ($exist)
		      {
			   return 'exist';
		      }         
              return $this->db->insert('invmf', $data);      //複製一筆  
       }	
	...
	function excelnewf()           
       {			
	    ...
	    $seq2=$this->input->post('mf002c');
	    $sql = " SELECT mf001,mf002,mf003,create_date FROM invmf WHERE mf001 >= '$seq1'  AND mf001 <= '$seq2' ORDER BY mf001 "; 
	...
	function printfd()          
       {
	    $seq1=$this->input->post('mf001c');    //查詢一筆以上
	    $seq2=$this->input->post('mf002c'); 
	    $sql = " SELECT * FROM invmf WHERE mf001 >= '$seq1'  AND mf001 <= '$seq2'  "; 
        $query = $this->db->query($sql);
	    $ret['rows'] = $query->result();
	...
	function updatef()   //更改一筆
          {
            ...
            $this->db->update('invmf',$data);                   //更改一筆
