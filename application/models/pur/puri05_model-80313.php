...
class Puri05_model extends CI_Model {
	...
	function insertf()    //新增一筆 檔頭  purta
		...
		//	while ($_POST['order_product'][  $n  ]['tb004']) {
			...
			$seq3=$_POST['order_product'][ $n  ]['tb003'];	 
	      $exist = $this->puri05_model->selone1d($this->input->post('purq04a31'),$this->input->post('ta002'),$seq3);
		...
		$this->db->from('cmsmq');
		$this->db->where("mq001",$this->input->post('purq04a31'));	
		$query = $this->db->get();
		$tmp = $query->result();
		if($tmp[0]->mq016=="Y"){
			echo "<script>window.open('printbb/".$this->input->post('purq04a31')."/".$this->input->post('ta002').".html', '_blank','menubar=no,status=no,scrollbars=no,top=0,left=0,toolbar=no,width=800,height=600');</script>";
		}
	}	 
	//查複製資料是否重複	 
