...
class Puri02_model extends CI_Model {
	...
	function search($limit, $offset, $sort_by, $sort_order)     
	  { 
	    ...
	    $sort_columns = array('mb001', 'mb002', 'mb003', 'mb004', 'mb005', 'mb006','mb006','a.create_date');
	...
	{
		$this->session->set_userdata('puri02_search',"display_search/".$offset);
		...
		if(isset($_SESSION['puri02']['search']['where'])){
			if($where){$where .= " and ";}
			$where .= $_SESSION['puri02']['search']['where'];
		}
		...
		if(isset($_SESSION['puri02']['search']['order'])){
			if($order){$order .= " , ";}
			$order .= $_SESSION['puri02']['search']['order'];
		}
		
		if(!isset($_SESSION['puri02']['search']['order']) && $default_order){
			if($order){$order .= " , ";}
			$order .= $default_order;
		}
		/* order end */
	//	echo "<pre>";var_dump($offset);exit;
		/* Data SQL */
		$query = $this->db->select('a.mb001 as a_mb001,a.mb002 as a_mb002,b.mb002 as b_mb002,b.mb003 as b_mb003,a.mb003 as a_mb003,a.mb004 as a_mb004,a.mb008 as a_mb008,a.mb010 as a_mb010,b.mb017 as b_mb017,d.mc002 as b_mb017disp ')
			 ->from('purmb as a')
			 ->join('invmb as b', 'a.mb001 = b.mb001 ','left')
			 ->join('purma as c', 'a.mb002 = c.ma001 ','left')
		...
		//建構暫存view 175 193 where 1080411 先//
		//$this->construct_view($ret['data']);
		// echo "<pre>";var_dump('test2');exit;
		  //   $vmb001=$this->session->userdata('vmb001');
		    //   $where = "("."mb001=".$sql_where.")"; 1060803
		 // if ($where==""){$where="1" } 
		  if ($where==""  ) {$where = "("."a.mb002=".$this->session->userdata('vmb001').")";}
		  if ($this->uri->segment(5)=="cn00000" ) {$where = $where." and "."("."a.mb002=".$this->session->userdata('vmb001').")" ;}
		$query = $this->db->select('a.mb001 as a_mb001,a.mb002 as a_mb002,b.mb002 as b_mb002,b.mb003 as b_mb003,a.mb003 as a_mb003,a.mb004 as a_mb004,a.mb011 as a_mb011,a.mb009 as a_mb009,b.mb017 as b_mb017,d.mc002 as b_mb017disp ')
			 ->from('purmb as a')
			 ->join('invmb as b', 'a.mb001 = b.mb001 ','left')
			 ->join('purma as c', 'a.mb002 = c.ma001 ','left')
			  ->join('cmsmc as d', 'b.mb017 = d.mc001 ','left')
	...
	//	{echo "<pre>";var_dump($this->db->last_query());exit;}
		$_SESSION['puri02']['search']['sql'] = $this->db->last_query();
		...
		//儲存where與order
		$_SESSION['puri02']['search']['where'] = $where;
		$_SESSION['puri02']['search']['order'] = $order;
		$_SESSION['puri02']['search']['offset'] = $offset;
	...
	***/
	function construct_view($data){
		...
		$_SESSION['puri02']['search']['view'] = $view_array;
		$_SESSION['puri02']['search']['index'] = $index_array;
	}
	 //ajax 查詢資料重複
	...
	function selone($seq1,$seq2,$seq3)    
       { 
		 $this->db->select('a.*, d.mf002 as mb003disp,b.mb002 as mb001disp,b.mb003 as mb001disp1,c.ma002 as mb002disp');	
		 $this->db->from('purmb as a');
	     //$this->db->set('mb001', $this->uri->segment(4)); 
	    // $this->db->where('mb001', $this->uri->segment(4));
		 $this->db->join('invmb as b', 'a.mb001 = b.mb001 ','left');
		 $this->db->join('purma as c', 'a.mb002 = c.ma001 ','left'); 
		 $this->db->join('cmsmf as d', 'a.mb003 = d.mf001','left');
		// $this->db->where('mb001', $this->uri->segment(4));
	...
	function findf($limit, $offset, $sort_by, $sort_order)     
       {            		
	     ...
	     $seq1 = " a.mb001, a.mb002, a.mb003, a.mb004, a.mb005, a.mb008,a.mb014,a.mb011, a.create_date FROM `purmb` as a ";
         ...
         if ($seq7!='') {$seq9=" ORDER BY ".$seq7;$seq33=$seq7;}
		 	 //下一頁不會亂跳
		if(@$_SESSION['puri02_sql_term']){$seq32 = $_SESSION['puri02_sql_term'];}
		if(@$_SESSION['puri02_sql_sort']){$seq33 = $_SESSION['puri02_sql_sort'];}
		
         $sort_order = (substr($sort_order,0,3) == 'asc') ? 'asc' : 'desc';
	   //  $sort_columns = array('mb001', 'mb002', 'mb003', 'mb004', 'mb005', 'mb008','mb014','mb011','create_date');
		  $sort_columns = array('a.mb001', 'a.mb002','b.mb002', 'b.mb003', 'a.mb004', 'c.ma002', 'a,mb011','a.mb014','a.create_date');
	     $sort_by = (in_array($sort_by, $sort_columns)) ? $sort_by : 'a.mb001';  //檢查排序欄位是否在 table 內
	     $query = $this->db->select('a.mb001,b.mb002,b.mb002 as mb001disp,b.mb003,b.mb003 as mb001disp1,c.ma002,c.ma002 as mb002disp, a.mb002, a.mb003,a.mb014,a.mb011, a.create_date')
	                       ->from('purmb as a')
						   ->join('invmb as b', 'a.mb001 = b.mb001 ','left')
						   ->join('purma as c', 'a.mb002 = c.ma001 ','left')
		                   ->where($seq32)
	...
	function filterf1($limit, $offset , $sort_by  , $sort_order)    //篩選多筆        
	   {    
		...
		$this->db->select('a.mb001,b.mb002 ,b.mb003 ,b.mb002 as mb001disp,b.mb003 as mb001disp1,  a.mb002, c.ma002, c.ma002 as mb002disp,a.mb003, a.mb011, a.mb014, a.create_date');
	       $this->db->from('purmb as a');
			$this->db->join('invmb as b', 'a.mb001 = b.mb001 ','left');
			$this->db->join('purma as c', 'a.mb002 = c.ma001 ','left');
	...
	function insertf()   
       {
		  ...
		  if ($mb005>'') {$mb005=substr($this->input->post('mb005'),0,4).substr($this->input->post('mb005'),5,2).substr(rtrim($this->input->post('mb005')),8,2);} else {$mb005='';}
		  if ($mb008>'') {$mb008=substr($this->input->post('mb008'),0,4).substr($this->input->post('mb008'),5,2).substr(rtrim($this->input->post('mb008')),8,2);} else {$mb008='';}
		  if ($mb009>'') {$mb009=substr($this->input->post('mb009'),0,4).substr($this->input->post('mb009'),5,2).substr(rtrim($this->input->post('mb009')),8,2);} else {$mb009='';}
		  if ($mb014>'') {$mb014=substr($this->input->post('mb014'),0,4).substr($this->input->post('mb014'),5,2).substr(rtrim($this->input->post('mb014')),8,2);} else {$mb014='';}
		  if ($mb015>'') {$mb015=substr($this->input->post('mb015'),0,4).substr($this->input->post('mb015'),5,2).substr(rtrim($this->input->post('mb015')),8,2);} else {$mb015='';}
          if ($smb013 != 'Y') {
          $smb013 = 'N';
           }
    ...
    function copyf()           //複製一筆
       {
	    ...
	    $data = array( 
            ...
            $exist = $this->puri02_model->selone2($this->input->post('mb001c'),$this->input->post('mb002c'));
		    if ($exist)
		      {
			   return 'exist';
		      }         
	...
	function excelnewf()           
	  ...
	  $seq1=$this->input->post('mb001o');    //查詢一筆以上
	    ...
	    $sql1 = " SELECT a.mb001,b.mb002 as mb001disp,b.mb003 as mb001disp1, a.mb004, a.mb002, c.ma002 as mb002disp,a.mb011,a.mb003, a.mb014, a.create_date "; 
	...
	function printfd()          
	   ...
	   $seq1=$this->input->post('mb001o');    //查詢一筆以上
	    ...
	    $sql1 = " SELECT a.mb008,a.mb001,a.mb004,b.mb002 as mb001disp,b.mb003 as mb001disp1,  a.mb002, c.ma002 as mb002disp,a.mb003, a.mb011, a.mb014, a.create_date "; 
		$sql2 = " FROM purmb as a LEFT JOIN invmb as b ON  a.mb001=b.mb001 LEFT JOIN purma as c ON a.mb002=c.ma001 "; 
		$sql3 = " WHERE a.mb001 >= '$seq1'  AND a.mb001 <= '$seq2' AND  a.mb002 >= '$seq3'  AND a.mb002 <= '$seq4' "; 
		$sql=$sql1.$sql2.$sql3;
        $query = $this->db->query($sql);
