...
class purb13_model extends CI_Model {
	...
	function batchaf()           
        {
			...
			if ($vmm=='01') {$mmm='12';}  //上月
		  //invmc 統計檔 
		          //刪除 品號廠商檔 purmb  mb004,mb011 th008,th018
		...
		//	$sql = " INSERT INTO  copmb (company,creator,usr_group,create_date,modifier,modi_date,flag,mb001,mb002,mb003,mb004,mb008,mb009,mb010,mb012,mb017)
          ...
          $sql = " INSERT INTO  purmb (mb001,mb002,mb003,mb004,mb012,mb008,mb009,mb014)
		  SELECT DISTINCT  b.th004, a.tg005, a.tg007, b.th008, 'Y', CURDATE()+0,CURDATE()+0,CURDATE()+0  FROM purtg AS a, purth AS b WHERE tg001=th001 and tg002=th002   ";         
		  $this->db->query($sql); 
			
			  // update copmb test and A.`mb011`=B.`th018`
		     $sql11 = " UPDATE  `purmb` AS A,  
       (SELECT c.company,c.creator,c.usr_group,c.create_date,c.modifier,c.modi_date,c.flag,c.tg003,c.tg005,c.tg007,c.tg011,d.th004,d.th008,d.th018  FROM purtg AS c, purth AS d WHERE tg001=th001 and tg002=th002 
	       and c.tg013='Y' ORDER BY c.tg003 desc  ) AS B  
    SET A.`company`=B.`company` ,A.`creator`=B.`creator`,A.`usr_group`=B.`usr_group`,A.`create_date`=B.`create_date`,A.`modifier`=B.`modifier`,
        A.`modi_date`=B.`modi_date`,A.`flag`=B.`flag`,A.`mb008`=B.`tg003`,A.`mb009`=B.`tg003`,A.`mb014`=B.`tg003`,A.`mb011`=B.`th018`	
    WHERE A.`mb001`=B.`th004` and A.`mb002`=B.`tg005`  and A.`mb003`=B.`tg007`    and A.`mb003`=B.`tg007` "; 
	    $this->db->query($sql11); 
