...
class Puri05_model extends CI_Model {
	...
	function construct_sql($limit = 15, $offset = 0, $func = "")
	  {
		...
		if(isset($_SESSION['puri05']['search']['where'])){
			if($where){$where .= " and ";}
			$where .= $_SESSION['puri05']['search']['where'];
		}
		...
		if(isset($_SESSION['puri05']['search']['order'])){
			if($order){$order .= " , ";}
			$order .= $_SESSION['puri05']['search']['order'];
		}
		
		if(!isset($_SESSION['puri05']['search']['order']) && $default_order){
			if($order){$order .= " , ";}
			$order .= $default_order;
		}
		/* order end */
		...
		/* Data SQL */
		$query = $this->db->select('a.*,c.mq002,b.me002,c.mq002 as ta001disp,b.me002 as ta004disp')
	                       ->from('purta as a')
		...
		$ret['data'] = $query->get()->result();
		//儲存sql 語法
		$_SESSION['puri05']['search']['sql'] = $this->db->last_query();
		...
		//儲存where與order
		$_SESSION['puri05']['search']['where'] = $where;
		$_SESSION['puri05']['search']['order'] = $order;
		$_SESSION['puri05']['search']['offset'] = $offset;
	...
	function construct_view($data){
		...
		$_SESSION['puri05']['search']['view'] = $view_array;
		$_SESSION['puri05']['search']['index'] = $index_array;
		//echo "<pre>";var_dump($_SESSION['puri05']['search']['view']);exit;
	}			
	//查詢修改用 (看資料用)   
	...
	function findf($limit, $offset, $sort_by, $sort_order)     
        ...
        if ($seq7!='') {$seq9=" ORDER BY ".$seq7;$seq33=$seq7;}
			 //下一頁不會亂跳
		if(@$_SESSION['puri05_sql_term']){$seq32 = $_SESSION['puri05_sql_term'];}
		if(@$_SESSION['puri05_sql_sort']){$seq33 = $_SESSION['puri05_sql_sort'];}
         $sort_order = (substr($sort_order,0,3) == 'asc') ? 'asc' : 'desc';
	     $sort_columns = array('ta001', 'ta002', 'ta003', 'ta004', 'ta005', 'ta006','ta007','ta008','ta010','ta011','ta013','ta012','ta016','create_date');
	     $sort_by = (in_array($sort_by, $sort_columns)) ? $sort_by : 'ta001';  //檢查排序欄位是否在 table 內
	     $query = $this->db->select('ta001,c.mq002, ta002, ta003, ta004, ta005, ta006,ta007,ta008,ta010,ta011,ta013,ta012,ta016, a.create_date')
	                       ->from('purta as a')
						   ->join('cmsmq as c', 'a.ta001 = c.mq001  ','left')
		                   ->where($seq32)
			               ->order_by($seq33)
	...
	function filterf1($limit, $offset , $sort_by  , $sort_order)          
	    {    
	      ...
	      $this->db->select('ta001, ta002,c.mq002, ta003, ta004, ta012, ta013,ta016,ta007,a.create_date');
	      $this->db->from('purta as a');
	...
	//查新增資料是否重複 (單身)	
    function selone1d($seg1,$seq2,$seq3)    
        {
	      $this->db->where('tb001', $seg1);
		  $this->db->where('tb002', $seg2);
		   $this->db->where('tb003', $seq3);
	...
	function insertf()    //新增一筆 檔頭  purta
        {
		  preg_match_all('/\d/S',$this->input->post('ta003'), $matches);  //處理日期字串
           ...
           //檢查資料是否已存在 若存在加1
			  while($this->puri05_model->selone1($ta001,$ta002)>0){
				$ta002 = $this->check_title_no($ta001,$ta010);
				$ta002no=$ta002;
			}
	     $data = array( 
		...
		// 新增明細 purtb
		 ...
		 foreach($order_product as $key => $val){
		        if($val['tb003'] && $val['tb004']){
				        extract($val);
						preg_match_all('/\d/S',$tb011, $matches);  //處理日期字串
			            $tb011 = implode('',$matches[0]);
						$data = array( 
							'company' => $this->session->userdata('syscompany'),
							'creator' => $this->session->userdata('manager'),
							'usr_group' => 'A100',
						...
						foreach($val as $k=>$v){
							if($k!="tb001"&&$k!="tb002"&&$k!="tb011"){//主鍵不用更改以及其他外來鍵庫別名稱
							    if($k=="tb003") {$data[$k] = $vtb003;} else {$data[$k] = $v;} //流水號
							}
						}
					$this->db->insert('purtb', $data);
	...
	//複製一筆	
    function copyf($ta001o,$ta001c,$ta002o,$ta013)           
        {
			$user = $this->session->userdata('sysuser');
			
			$sql_chk = "SELECT COUNT(*) AS NUM FROM PURTA WHERE TA001 = '$ta001o' AND TA002 = '$ta002o'";
			$query_chk = $this->db->query($sql_chk);
			$num = $query_chk->result();
			$NUM1 = $num[0] -> NUM; //輸入的單號是否存在
			$ta013_d = substr($ta013,0,4)-1911 .substr($ta013,5,2);
		    ...
		    FROM PURTA WHERE TA001 = '$ta001c' AND CREATE_DATE > '20210101' AND TA002 LIKE '$ta013_d%'";
			...
			SELECT COMPANY,'$user','A100',convert(varchar, getdate(), 112),'','','1','$ta001c','$TB002_N','$ta013_1',TA004,TA005,TA006,'N','0',TA009,TA010,TA011,TA012,'$ta013_1','',TA015,TA016 FROM PURTA WHERE TA001 = '$ta001o' AND TA002 = '$ta002o'
			";
		...
		//	echo var_dump($sql1);
			...
			SELECT COMPANY,'$user','A100',convert(varchar, getdate(), 112),'','','1','$ta001c','$TB002_N',TB003,TB004,TB005,TB006,TB007,TB008,TB009,TB010,'$ta013_1',TB012,'',TB014,TB015,'','0','0','','N','N','',TB023,TB024,'N',TB026,TB027,TB028,TB029,TB030,TB031,TB032,TB033,TB034,TB035,TB036,TB037,TB038,'N' FROM PURTB WHERE TB001 = '$ta001o' AND TB002 = '$ta002o'
			";
	...
	function printfd()          
        {
		  ...
		  $seq3=$this->input->post('ta002o');    
	      $seq4=$this->input->post('ta002c');
		  $seq5=$this->input->post('ta003o');    
	      $seq6=$this->input->post('ta003c');
		  $seq7=$this->input->post('ta004o');    
	      $seq8=$this->input->post('ta004c');
       ...
       //   $query = $this->db->query($sql);
        ...
        $this->db->where('a.ta013 >=', $seq5); 
	    $this->db->where('a.ta013 <=', $seq6);
        $this->db->where('b.tb004 >=', $seq7); 
	    $this->db->where('b.tb004 <=', $seq8);		
		$this->db->order_by('ta001 , ta002 ,b.tb003');
		
		$query = $this->db->get();
		  
	      $ret['rows'] = $query->result();
          $seq32 = "ta001 >= '$seq1'  AND ta001 <= '$seq2' AND ta002 >= '$seq3'  AND ta002 <= '$seq4' AND ta013 >= '$seq5'  AND ta013 <= '$seq6' AND tb004 >= '$seq7'  AND tb004 <= '$seq8' ";	
	      $query = $this->db->select('COUNT(*) as count', FALSE)  //筆數查詢,如果設為FALSE不會使用反引號保護你的字段或者表名
		              ->from('purta as a')
					  ->join('purtb as b', 'a.ta001 = b.tb001  and a.ta002=b.tb002 ','left')
		              ->where($seq32);
	      $num = $query->get()->result();		
	      $ret['num_rows'] = $num[0]->count;		
	      return $ret;
	...
	//印單據筆  
		...
		foreach($result['rows'] as $val){
			$hp_query = $this->db->select('a.th014, b.tg005, a.th015, a.th008, a.th018,c.ma002')
					->from('purth as a')
					->join('purtg as b', 'a.th002 = b.tg002','left')
					->join('purma as c', 'b.tg005 = c.ma001','left')//抓廠商名
					->order_by('a.th014', 'desc')
					->limit(1, 0)
					->where('a.th004',$val->tb004);
			$temp_data = $hp_query->get()->result();
			if(@$temp_data[0])
				$result['hp_rows'][] = $temp_data[0];
			else
				$result['hp_rows'][] = (object)array();
		}
	    if ($query->num_rows() > 0) 
		 {
		 return $result;
        ...
        { 
          ...
          $data = array(			
			...
			if(!is_array($order_product)){$order_product=array();}
					...
					$this->db->delete('purtb'); //刪除明細 1060809
			...
			foreach($order_product as $key => $val){
				...
				if($this->seldetail($ta001,$ta002,$val['tb003'])>0){
					...
					foreach($val as $k=>$v){
						if($k!="tb001"&&$k!="tb002"&& $k!="tb011"){//主鍵不用更改以及其他外來鍵庫別名稱 td013日期等別處理
					...
					$this->db->update('purtb',$data);//更改一筆
					$mtb003 = (int) $vtb003+10;
			        $vtb003 =  (string)$mtb003;
				}else{
					if($val['tb003'] && $val['tb004']){
	...
	function delmutif()   
	 ...
	 //取單號 最大值加1
	function check_title_no($puri04,$ta013){
