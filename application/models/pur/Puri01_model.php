...
class Puri01_model extends CI_Model {
	...
	{
		$this->session->set_userdata('puri01_search',"display_search/".$offset);
		...
		if(isset($_SESSION['puri01']['search']['where'])){
			if($where){$where .= " and ";}
			$where .= $_SESSION['puri01']['search']['where'];
		}
		...
		if(isset($_SESSION['puri01']['search']['order'])){
			if($order){$order .= " , ";}
			$order .= $_SESSION['puri01']['search']['order'];
		}
		
		if(!isset($_SESSION['puri01']['search']['order']) && $default_order){
			if($order){$order .= " , ";}
			$order .= $default_order;
		}
		/* order end */
		...
		$ret['data'] = $query->get()->result();
		//儲存sql
		$_SESSION['puri01']['search']['sql'] = $this->db->last_query();
		...
		//儲存where與order
		$_SESSION['puri01']['search']['where'] = $where;
		$_SESSION['puri01']['search']['order'] = $order;
		$_SESSION['puri01']['search']['offset'] = $offset;
	...
	function selonea($seg1)    
       {
		  $this->db->select('purma1.*');
         $this->db->from('purma1');
		 $this->db->where('purma1.ma001', $this->uri->segment(4));
         $query = $this->db->get();		 
		 if ($query->num_rows() > 0) 
		 {
	...
	function findf($limit, $offset, $sort_by, $sort_order)     
        ...
        if ($seq7!='') {$seq9=" ORDER BY ".$seq7;$seq33=$seq7;}
			 //下一頁不會亂跳
		if(@$_SESSION['puri01_sql_term']){$seq32 = $_SESSION['puri01_sql_term'];}
		if(@$_SESSION['puri01_sql_sort']){$seq33 = $_SESSION['puri01_sql_sort'];}
	...
	function insertf()    
        {
		     preg_match_all('/\d/S',$this->input->post('ma017'), $matches);  //處理日期字串
			 $ma017 = implode('',$matches[0]);
			 preg_match_all('/\d/S',$this->input->post('ma022'), $matches);  //處理日期字串
			 $ma022 = implode('',$matches[0]);
			 preg_match_all('/\d/S',$this->input->post('ma023'), $matches);  //處理日期字串
			 $ma023 = implode('',$matches[0]);
		  $data = array( 
		         ...
		         'ma004' => $this->input->post('cmsq15a9'),
	...
	function updatef()   
        {
			...
			$data = array(			
                  ...
                  'ma008' => $this->input->post('ma008'),
				  'ma009' => $this->input->post('ma009'),'ma010' => $this->input->post('ma010'),
                  ...
                  'ma017' => $ma017,'ma018' => $this->input->post('ma018'),'ma019' => $this->input->post('ma019'),
                  'ma020' => $this->input->post('ma020'),'ma021' => $this->input->post('cmsq06a'),'ma022' => $ma022,
                  'ma023' => $ma023,'ma024' => $this->input->post('ma024'),'ma025' => $this->input->post('cmsq21a1'),
			...
			/* $vma008=$_POST['ma008'];	
			 $vma001=$this->input->post('ma001');
			 $this->db->where('ma001', $vma001);
             $this->db->delete('purma1'); 
		//	echo '<pre>';var_dump($_POST['ma008']);exit;
			 for ($i=0;$i<count($vma008);$i++){
			//	 echo '<pre>';var_dump($_POST['ma008']);exit;
				$tmp=$_POST['ma008'][$i];
	...
	/*==以下AJAX處理區域==*/
	 function ajaxpuri01($seg1)    //ajax 查詢一筆 顯示用 庫別6
          { 	              
	    $this->db->set('ma001', $this->uri->segment(4));
	    $this->db->where('ma001', $this->uri->segment(4));	
	    $query = $this->db->get('purma');
