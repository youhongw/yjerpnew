...
class Puri04_model extends CI_Model {
	...
	{
		$this->session->set_userdata('puri04_search',"display_search/".$offset);
		...
		if(isset($_SESSION['puri04']['search']['where'])){
			if($where){$where .= " and ";}
			$where .= $_SESSION['puri04']['search']['where'];
		}
		...
		if(isset($_SESSION['puri04']['search']['order'])){
			if($order){$order .= " , ";}
			$order .= $_SESSION['puri04']['search']['order'];
		}
		
		if(!isset($_SESSION['puri04']['search']['order']) && $default_order){
			if($order){$order .= " , ";}
			$order .= $default_order;
		}
		/* order end */
		...
		$ret['data'] = $query->get()->result();
		//儲存sql
		$_SESSION['puri04']['search']['sql'] = $this->db->last_query();
		...
		//儲存where與order
		$_SESSION['puri04']['search']['where'] = $where;
		$_SESSION['puri04']['search']['order'] = $order;
		$_SESSION['puri04']['search']['offset'] = $offset;
		
		return $ret;
	}
	  //建構SQL字串 單據
	function constructa_sql($limit = 15, $offset = 0, $func = "",$docnum)
	{
		$this->session->set_userdata('puri04_search',"display_search/".$offset);
		if (session_status() == PHP_SESSION_NONE) {
			session_start();
	...
	//建構SQL字串 單據
	function constructb_sql($limit = 15, $offset = 0, $func = "",$docnum)
	{
		$this->session->set_userdata('puri04_search',"display_search/".$offset);
		if (session_status() == PHP_SESSION_NONE) {
			session_start();
  ...
  if ($this->uri->segment(3,0)=="clear_sql_term")
	...
	//建構SQL字串 單據 34
	function constructc_sql($limit = 15, $offset = 0, $func = "",$docnum)
	{
		$this->session->set_userdata('puri04_search',"display_search/".$offset);
		if (session_status() == PHP_SESSION_NONE) {
			session_start();
  ...
  if ($this->uri->segment(3,0)=="clear_sql_term")
	...
	//建構SQL字串 單據 35
	function constructd_sql($limit = 15, $offset = 0, $func = "",$docnum)
	{
		$this->session->set_userdata('puri04_search',"display_search/".$offset);
		if (session_status() == PHP_SESSION_NONE) {
			session_start();
  ...
  if ($this->uri->segment(3,0)=="clear_sql_term")
	...
	//建構SQL字串 單據 31
	function constructe_sql($limit = 15, $offset = 0, $func = "",$docnum)
	{
		$this->session->set_userdata('puri04_search',"display_search/".$offset);
		if (session_status() == PHP_SESSION_NONE) {
			session_start();
  ...
  if ($this->uri->segment(3,0)=="clear_sql_term")
	...
	function insertf()   
       ...
       //    }
	    ...
	    $exist = $this->puri04_model->selone1($this->input->post('mq001'));
	    if ($exist)
	      {
		    return 'exist';
		  } 
    ...
    function copyf()           //複製一筆
       {
	    ...
	    $data = array( 
            ...
            $exist = $this->puri04_model->selone2($this->input->post('mq002c'));
		    if ($exist)
		      {
			   return 'exist';
		      }         
