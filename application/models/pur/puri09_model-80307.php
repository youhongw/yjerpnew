...
class Puri09_model extends CI_Model {
	...
	function search($limit, $offset, $sort_by, $sort_order)     
	    { 
	     ...
	     $query = $this->db->select('tg001, tg002, tg003,tg014, tg005,b.ma002 , tg021, tg031, tg019,tg015,a.create_date')
	                       ->from('purtg as a')
	...
	function selonebefore($seq1,$seq2)    
        ...
        $this->db->join('purtd as b', 'a.tc001 = b.td001  and a.tc002=b.td002 and b.td016="N" ','left'); //沒有不要出現		
	...
	function findf($limit, $offset, $sort_by, $sort_order)     
        ...
        if ($seq7!='') {$seq9=" ORDER BY ".$seq7;$seq33=$seq7;}
	     ...
	     $sort_by = (in_array($sort_by, $sort_columns)) ? $sort_by : 'tg001';  //檢查排序欄位是否在 table 內
	     $query = $this->db->select('a.tg001, a.tg002, a.tg003,a.tg014, a.tg004, a.tg005,b.ma002, a.tg006,a.tg007,a.tg008,a.tg010,a.tg011,a.tg021,a.tg031,a.tg019,a.tg015, a.create_date')
	                       ->from('purtg as a')
						  ->join('purma as b', 'a.tg005 = b.ma001 ','left')
		                   ->where($seq32)
			               ->order_by($seq33)
	...
	function filterf1($limit, $offset , $sort_by  , $sort_order)          
	    {    
	      ...
	      $this->db->select('a.tg001, a.tg002, a.tg003, a.tg014, a.tg005,b.ma002 , a.tg021, a.tg031,a.tg019,a.tg015, a.create_date');
	      $this->db->from('purtg as a');
	...
	function insertf()    //新增一筆 檔頭  purtg
        {
		 //    $tax=round($this->input->post('tg019')*$this->input->post('tg026'));
		  //   if ($this->input->post('tg018')=='1') {$tg019=round($this->input->post('tg019')-$tax);}
		  	 if ($this->input->post('tg027') >'') {$vtg027=substr($this->input->post('tg027'),0,4).substr($this->input->post('tg027'),5,2).substr(rtrim($this->input->post('tg027')),8,2);}
                 else {$vtg027='';}	
              if ($this->input->post('tg029') >'') {$vtg029=substr($this->input->post('tg029'),0,4).substr($this->input->post('tg029'),5,2);}
                 else {$vtg029='';}	  				 
		$data = array( 
		...
		//	while ($_POST['order_product'][  $n  ]['th004']) {		
		    while (isset($_POST['order_product'][  $n  ]['th004'])) {
			   ...
			   $data_array = array( 
				 ...
				 'th019' =>  $_POST['order_product'][ $n  ]['th045'],
	     ...
	     // $exist = $this->puri09_model->selone1d($this->input->post('purq04a34'),$this->input->post('tg002'));
		  ...
		  $this->db->insert('purth', $data_array); }
		   // 採購已交數量 th011,12,13   16 結案碼
		     $th011=$_POST['order_product'][ $n  ]['th011'];
			 $th012=$_POST['order_product'][ $n  ]['th012'];
			 $th013=$_POST['order_product'][ $n  ]['th013'];
			 $th016=$_POST['order_product'][ $n  ]['th016'];
			 $existb = $this->puri09_model->selone3d($th011,$th012,$th013);
			  
			   if ($_POST['order_product'][  $n  ]['th011']!='') {
			 if ($existb){			  
         $sql = " UPDATE purtd set td015=td015+'$th016' WHERE td001 = '$th011'  AND td002 = '$th012'  AND td003 = '$th013' "; 
		 $query = $this->db->query($sql);
         $sql9 = " UPDATE purtd set td016='Y' WHERE td001 = '$th011'  AND td002 = '$th012'  AND td003 = '$th013' AND Td015>=td008 "; 
		...
		$this->db->from('cmsmq');
		$this->db->where("mq001",$this->input->post('purq04a34'));	
		$query = $this->db->get();
		$tmp = $query->result();
		if($tmp[0]->mq016=="Y"){
			echo "<script>window.open('printbb/".$this->input->post('purq04a34')."/".$this->input->post('tg002').".html', '_blank','menubar=no,status=no,scrollbars=no,top=0,left=0,toolbar=no,width=800,height=600');</script>";
		}
	}	 
	//查複製資料是否重複	 
		...
		//	while ($_POST['order_product'][  $n  ]['th004']) {
			while (isset($_POST['order_product'][  $n  ]['th004'])) {
			 ...
			 $existb = $this->puri09_model->selone3d($th011,$th012,$th013);
			   ...
			   if ($_POST['order_product'][  $n  ]['th011']!='') {
			 if ($existb){			  
         $sql = " UPDATE purtd set td015=td015+'$th016'-'$th016b' WHERE td001 = '$th011'  AND td002 = '$th012'  AND td003 = '$th013' "; 
		 $query = $this->db->query($sql);	
		$sql9 = " UPDATE purtd set td016='Y' WHERE td001 = '$th011'  AND td002 = '$th012'  AND td003 = '$th013' AND Td015>=td008 "; 
			 $query = $this->db->query($sql9); }
			  }
	...
	$sql95 =" update purtd as c,(select th011,th012,th013,td015,td016,th016 from purth as b,purtd as c
			...
			$this->db->query($sql95);   $num =  (int)$i + 1;
			 $i =  (string)$num; 
			   ...
			   //庫存增加減少  (找本張進貨單刪除時庫存-減回)
			    $query82 = $this->db->query("SELECT th004,th009,th016   FROM purth as a 
	   ...
	   foreach ($query82->result() as $row)
			...
			while (isset($th004[$i])) {
                        ...
                        $vth009=$th009[$i];
                        $vth016=$th016[$i];
         $sql83 = " UPDATE invmc set mc007=mc007-'$vth016' WHERE mc001 = '$vth004'  AND mc002 = '$vth009'  "; 
