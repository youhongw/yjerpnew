...
class Puri07_model extends CI_Model {
    ...
    function selone1d($seg1,$seg2,$seg3)    
        {
	      $this->db->where('td001', $this->input->post('purq04a33'));
	...
	function insertf()    //新增一筆 檔頭  purtc
		...
		if (!isset($_POST['order_product'][  $n  ]['td004']) ) { $n='250'; } 
		 ...
		 while (isset($_POST['order_product'][  $n  ]['td004'])) {		
			   ...
			   $data_array = array( 
                 ...
                 'td012' =>  substr($_POST['order_product'][ $n  ]['td012'],0,4).substr($_POST['order_product'][ $n ]['td012'],5,2).substr($_POST['order_product'][ $n ]['td012'],8,2),
                 'td014' =>  $_POST['order_product'][ $n  ]['td014'],
				 'td016' =>  $_POST['order_product'][ $n  ]['td016']
                );   
						 
	      $exist = $this->puri07_model->selone1d($this->input->post('purq04a33'),$this->input->post('tc002'),$td003);
		  if ($_POST['order_product'][  $n  ]['td004']>'0') {
		     $this->db->insert('purtd', $data_array); }
		//  $this->db->insert('purtd', $data_array);
		  $tc019=$tc019+ $_POST['order_product'][ $n  ]['td011'];
			  $tc023=$tc023+ $_POST['order_product'][ $n  ]['td008'];
		     $mtd003 = (int) $td003+10;
			 $td003 =  (string)$mtd003;
		...
		$this->db->from('cmsmq');
		$this->db->where("mq001",$this->input->post('purq04a33'));	
		$query = $this->db->get();
		$tmp = $query->result();
		if($tmp[0]->mq016=="Y"){
			echo "<script>window.open('printbb/".$this->input->post('purq04a33')."/".$this->input->post('tc002').".html', '_blank','menubar=no,status=no,scrollbars=no,top=0,left=0,toolbar=no,width=800,height=600');</script>";
		}
	}	  
	//查複製資料是否重複	 
