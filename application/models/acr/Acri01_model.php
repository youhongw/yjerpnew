...
class Acri01_model extends CI_Model {
	...
	{
		$this->session->set_userdata('acri01_search',"display_search/".$offset);
		...
		if(isset($_SESSION['acri01']['search']['where'])){
			if($where){$where .= " and ";}
			$where .= $_SESSION['acri01']['search']['where'];
		}
		...
		if(isset($_SESSION['acri01']['search']['order'])){
			if($order){$order .= " , ";}
			$order .= $_SESSION['acri01']['search']['order'];
		}
		
		if(!isset($_SESSION['acri01']['search']['order']) && $default_order){
			if($order){$order .= " , ";}
			$order .= $default_order;
		}
		/* order end */
		...
		$ret['data'] = $query->get()->result();
		//儲存sql
		$_SESSION['acri01']['search']['sql'] = $this->db->last_query();
		...
		//儲存where與order
		$_SESSION['acri01']['search']['where'] = $where;
		$_SESSION['acri01']['search']['order'] = $order;
		$_SESSION['acri01']['search']['offset'] = $offset;
	...
	function selone()    
       { 
		 $this->db->select('cmsmq.*, d.ma003 as mq021disp , a.ms003 as mq025disp, b.ms003 as mq027disp, c.mb005 as mq023disp');	
		 $this->db->from('cmsmq');
	     //$this->db->set('mq001', $this->uri->segment(4)); 
	     $this->db->where('mq001', $this->uri->segment(4));
	...
	function findf($limit, $offset, $sort_by, $sort_order)     
       {            		
         ...
         if ($seq7!='') {$seq9=" ORDER BY ".$seq7;$seq33=$seq7;}
		 	 //下一頁不會亂跳
		if(@$_SESSION['acri01_sql_term']){$seq32 = $_SESSION['acri01_sql_term'];}
		if(@$_SESSION['acri01_sql_sort']){$seq33 = $_SESSION['acri01_sql_sort'];}
	...
	function insertf()   
       ...
       //    }
	    ...
	    $exist = $this->acri01_model->selone1($this->input->post('mq001'));
	    if ($exist)
	      {
		    return 'exist';
		  } 
    ...
    function copyf()           //複製一筆
       {
	    ...
	    $data = array( 
            ...
            $exist = $this->acri01_model->selone2($this->input->post('mq002c'));
		    if ($exist)
		      {
			   return 'exist';
		      }         
