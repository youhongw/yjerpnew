...
class Acri03_model extends CI_Model {
	...
	function construct_sql($limit = 15, $offset = 0, $func = "")
	  {
		...
		/* Data SQL */
		$query = $this->db->select('a.tc001,c.mq002, a.tc002,a.tc016,a.tc017, a.tc003, a.tc004, b.ma002,a.tc006, a.tc013, a.tc014, a.tc015, a.create_date')
	                       ->from('acrtc as a')
						   ->join('copma as b', 'a.tc004 = b.ma001','left')
	...
	function construct_view($data){
		...
		$_SESSION['acri03']['search']['view'] = $view_array;
		$_SESSION['acri03']['search']['index'] = $index_array;
		//echo "<pre>";var_dump($_SESSION['acri03']['search']['view']);exit;
	}	
	//查詢前置單據用 (看資料用)   
	function selonebefore($seq1,$seq2)    
		...
		$this->db->join('copma as h', 'a.td004 = h.ma001 ','left');
	...
	function selone($seq1,$seq2)    
		...
		$this->db->join('cmsmq as c', 'a.tc001 = c.mq001 and c.mq003="63" ','left');
	    $this->db->join('cmsmb as d', 'a.tc010 = d.mb001 ','left');
		$this->db->join('cmsmf as e', 'a.tc005 = e.mf001 ','left');
		$this->db->join('copma as h', 'a.tc004 = h.ma001 ','left');
		...
		$this->db->select('b.*,c.ma002 as td008disp,d.me002 as td021disp,e.mf002 as td010disp')
			->from('acrtd as b')
			->join('actma as c', 'b.td008 = c.ma001 ','left')   //科目
			->join('cmsme as d', 'b.td021 = d.me001 ','left')   //部門
	...
	function findf($limit, $offset, $sort_by, $sort_order)     
         {            		
	      ...
	      $seq1 = "tc001, tc002, tc003, tc004, tc013, tc018,tc016,tc015,tc010,tc011,tc021,tc028,tc029,tc026, create_date FROM `acrtc` ";
		...
		if(@$_SESSION['acri03_sql_sort']){$seq33 = $_SESSION['acri03_sql_sort'];}
	     ...
	     $sort_by = (in_array($sort_by, $sort_columns)) ? $sort_by : 'tc001';  //檢查排序欄位是否在 table 內
	     $query = $this->db->select('tc001, tc002, tc003, tc004,b.ma002, tc004,b.ma002 as tc004disp, tc013,tc018,tc016,tc015,tc011, a.create_date')
	                       ->from('acrtc as a')
						   ->join('copma as b', 'a.tc004 = b.ma001 ','left')
		                   ->where($seq32)
			               ->order_by($seq33)
	...
	function insertf()    //新增一筆 檔頭  acrtc
        { 
			 ...
			 preg_match_all('/\d/S',$this->input->post('tc017'), $matches);  //處理日期字串
			 $tc017 = implode('',$matches[0]);
			 ...
			 //檢查資料是否已存在 若存在加1
			  while($this->acri03_model->selone1($tc001,$tc002)>0){
		...
		// 新增明細 acrtd
		   ...
		   foreach($order_product as $key => $val){
		        if($val['td003'] && $val['td004']&& $val['td005']&& $val['td008']){
						...
						foreach($val as $k=>$v){
							if($k!="td001"&&$k!="td002"&&$k!="td008disp"&&$k!="td021disp"&&$k!="td010disp"){//主鍵不用更改以及其他外來鍵庫別名稱
							    if($k=="td003") {$data[$k] = $vtd003;} else {$data[$k] = $v;}
							}
						}
					$this->db->insert('acrtd', $data);
	...
	function excelnewf()           
	  ...
	  //    $sql = " SELECT tc001,tc002,tc024,tc004,tc011,tc003,create_date FROM acrtc WHERE tc001 >= '$seq1'  AND tc001 <= '$seq2' AND  tc002 >= '$seq3'  AND tc002 <= '$seq4'  "; 
	   ...
	   $sql = " SELECT a.tc001,a.tc002,a.tc003,a.tc004,c.ma002 as tc004disp,b.td004,b.td005,b.td003,b.td012,b.td013,b.td014,
		  b.td017
	...
	function printfd()          
        {
		  ...
		  $seq3=$this->input->post('tc002o');    
	      $seq4=$this->input->post('tc002c');
	      $sql = " SELECT a.tc001,a.tc002,a.tc003,a.tc004,ma002 as tc004disp,b.td001,b.td002,b.td003,b.td004,b.td005,b.td006,b.td007,b.td008,b.td009,
		  b.td010,b.td011,b.td012,b.td014,b.td015,b.td016,b.td018
		  FROM acrtc as a, acrtd as b,copma as c WHERE tc001=td001 and tc002=td002 and tc004=ma001 and   tc001 >= '$seq1'  AND tc001 <= '$seq2' AND tc002 >= '$seq3'  AND tc002 <= '$seq4'  "; 
          $query = $this->db->query($sql);
	      $ret['rows'] = $query->result();
	...
	function printfc()   
      {           
        $this->db->select('a.* ,c.mq002 AS tc001disp, d.mb002 AS tc010disp,e.mf002 AS tc008disp, 
		  ,h.ma002 AS tc004disp,b.company, b.creator, b.usr_group, b.create_date, b.modifier, b.modi_date, b.flag, b.td001, b.td002, b.td003, b.td004, b.td005,
	...
	function updatef()   
        {
          ...
          $data = array(			
            ...
            $this->db->update('acrtc',$data);  
			//刪除明細 先調整己付
			$sql="select * from acrtd where td001='$tc001' and td002='$tc002' ";
			$query = $this->db->query($sql) ;
		    foreach ($query->result() as $row) {
            foreach($row as $i=>$v){
            $$i=$v;
			...
			if(!is_array($order_product)){$order_product=array();}
					...
					$this->db->delete('acrtd'); //刪除明細 1060809
		...
		//	$this->db->flush_cache();  
			...
			foreach($order_product as $key => $val){
				...
				if($this->seldetail($tc001,$tc002,$val['td003'])>0){
					...
					foreach($val as $k=>$v){
						if($k!="td001"&&$k!="td002"&&$k!="td008disp"&&$k!="td010disp" && $k!="td021disp"){//主鍵不用更改以及其他外來鍵庫別名稱 td013日期等別處理
							if($k=="td003") {$data[$k] = $vtd003;} else {$data[$k] = $v;}
						}
					}
					$this->db->where('td001', $tc001);
					...
					$this->db->update('acrtd',$data);//更改一筆
					$mtd003 = (int) $vtd003+10;
			        $vtd003 =  (string)$mtd003;
				}else{
					if($val['td003'] && $val['td004']&& $val['td005']&& $val['td008']){
						...
						foreach($val as $k=>$v){
							if($k!="td001"&&$k!="td002"&&$k!="td008disp"&&$k!="td010disp" && $k!="td021disp"){//主鍵不用更改以及其他外來鍵庫別名稱
								if($k=="td003") {$data[$k] = $vtd003;} else {$data[$k] = $v;}
							}
						}
						$this->db->insert('acrtd', $data);
	    ...
	    if (!empty($_POST['selected'])) 
	         {
                foreach($_POST['selected'] as $check) 
			    {
				 ...
				 if ($seq3 != 'Y') { 
				 //進貨單更新碼 Y 改 N TG015 
	$sql9 =" update purtg as c,(select td001,td002,td005,td006,tg015 from acrtd as b,purtg as c
...
$this->db->query($sql9);
		//退貨單更新碼 Y 改 N Ti020
	$sql8 =" update purti as c,(select td001,td002,td005,td006,ti020 from acrtd as b,purti as c
...
$this->db->query($sql8);
                  ...
                  $this->db->delete('acrtd'); $this->session->set_userdata('msg1',"未收款已刪除"); }
	...
	//取單號 最大值加1
	function check_title_no($tc001,$tc017){
		preg_match_all('/\d/S',$tc017, $matches);  //處理日期字串
		$tc017 = implode('',$matches[0]);
		$this->db->select('MAX(tc002) as max_no')
			->from('acrtc')
			->where('tc001', $tc001)
		//	->where('tc016', $tc016);
			->like('tc017', $tc017, "after");
			
		$query = $this->db->get();
		$result = $query->result();
		
	    if (!$result[0]->max_no){return $tc017."001";}
	...
	function lookupd($keyword){     
	  ...
	  $this->db->from('acpta');
	  $this->db->join('copma as b', 'ta004 = b.ma001','left');
