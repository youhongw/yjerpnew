...
class Acri02_model extends CI_Model {
	...
	function construct_sql($limit = 15, $offset = 0, $func = "")
	  {
		...
		if(isset($_SESSION['acri02']['search']['where'])){
			if($where){$where .= " and ";}
			$where .= $_SESSION['acri02']['search']['where'];
		}
		...
		if(isset($_SESSION['acri02']['search']['order'])){
			if($order){$order .= " , ";}
			$order .= $_SESSION['acri02']['search']['order'];
		}
		
		if(!isset($_SESSION['acri02']['search']['order']) && $default_order){
			if($order){$order .= " , ";}
			$order .= $default_order;
		}
		/* order end */
		...
		/* Data SQL */
		$query = $this->db->select('a.ta001,c.mq002, a.ta002,a.ta038, a.ta003, a.ta004, b.ma002,a.ta006,(ta029+ta030) as ta2930, a.ta029, a.ta030, a.ta031,a.create_date')
	                       ->from('acrta as a')
						   ->join('copma as b', 'a.ta004 = b.ma001','left')
		...
		$ret['data'] = $query->get()->result();
		//儲存sql 語法
		$_SESSION['acri02']['search']['sql'] = $this->db->last_query();
		...
		//儲存where與order
		$_SESSION['acri02']['search']['where'] = $where;
		$_SESSION['acri02']['search']['order'] = $order;
		$_SESSION['acri02']['search']['offset'] = $offset;
	...
	function construct_view($data){
		...
		$_SESSION['acri02']['search']['view'] = $view_array;
		$_SESSION['acri02']['search']['index'] = $index_array;
		//echo "<pre>";var_dump($_SESSION['acri02']['search']['view']);exit;
	}	
	//查詢修改用 (看資料用)   
	function selone($seq1,$seq2)    
		...
		$this->db->select('b.*,c.ma002 as tb013disp,d.me002 as tb021disp')
			->from('acrtb as b')
			->join('actma as c', 'b.tb013 = c.ma001 ','left')   //科目
			->join('cmsme as d', 'b.tb021 = d.me001 ','left')   //部門
			->where('b.tb001', $seq1)
			->where('b.tb002', $seq2);
		$query = $this->db->get();
	...
	function findf($limit, $offset, $sort_by, $sort_order)     
		...
		if(@$_SESSION['acri02_sql_sort']){$seq33 = $_SESSION['acri02_sql_sort'];}
	     ...
	     $sort_by = (in_array($sort_by, $sort_columns)) ? $sort_by : 'ta001';  //檢查排序欄位是否在 table 內
	     $query = $this->db->select('ta001, ta002,b.ma002,ta038, ta003, ta004, ta004,b.ma002 as ta004disp, ta013,ta018,ta016,ta015,ta011,ta025,ta029,ta030,ta027, a.create_date')
	                       ->from('acrta as a')
						   ->join('copma as b', 'a.ta004 = b.ma001 ','left')
		                   ->where($seq32)
			               ->order_by($seq33)
	...
	function filterf1($limit, $offset , $sort_by  , $sort_order)          
	    {    
	      ...
	      $this->db->select('ta001, ta002,b.ma002,ta038, ta003, ta004,b.ma002 as  ta004disp, ta029,ta030,ta027, a.create_date');
	      $this->db->from('acrta as a');
    ...
    function selone1d($seg1)    
        {
	      $this->db->where('tb001', $this->input->post('ta001'));
	...
	function insertf()    //新增一筆 檔頭  acrta
		...
		//刪日期 / 符號
			 ...
			 preg_match_all('/\d/S',$this->input->post('ta038'), $matches);  //處理日期字串
			 $ta038 = implode('',$matches[0]);
			 ...
			 preg_match_all('/\d/S',$this->input->post('ta021'), $matches);  //處理日期字串
			 $ta021 = implode('',$matches[0]);
			 
			 preg_match_all('/\d/S',$this->input->post('ta032'), $matches);  //處理日期字串
			 $ta032 = implode('',$matches[0]);
			 preg_match_all('/\d/S',$this->input->post('ta044'), $matches);  //處理日期字串
			 $ta044 = implode('',$matches[0]);
			 ...
			 //檢查資料是否已存在 若存在加1
			  while($this->acri02_model->selone1($ta001,$ta002)>0){
				$ta002 = $this->check_title_no($ta001,$ta038);
				$ta002no=$ta002;
			}
		...
		// 新增明細 acrtb
		   ...
		   foreach($order_product as $key => $val){
		        if($val['tb003'] && $val['tb004']&& $val['tb005']){
						...
						foreach($val as $k=>$v){
							if($k!="tb001"&&$k!="tb002"&&$k!="tb013disp"&&$k!="tb021disp"){//主鍵不用更改以及其他外來鍵庫別名稱
							    if($k=="tb003") {$data[$k] = $vtb003;} else {$data[$k] = $v;}
							}
						}
					$this->db->insert('acrtb', $data);
	...
	function updatef()   
        {
          ...
          $data = array(			
			...
			if(!is_array($order_product)){$order_product=array();}
					...
					$this->db->delete('acrtb'); //刪除明細 1060809
		...
		//	$this->db->flush_cache();  
			...
			foreach($order_product as $key => $val){
				...
				if($this->seldetail($ta001,$ta002,$val['tb003'])>0){
					...
					foreach($val as $k=>$v){
						if($k!="tb001"&&$k!="tb002"&&$k!="tb013disp" && $k!="tb021disp"){//主鍵不用更改以及其他外來鍵庫別名稱 tb013日期等別處理
							if($k=="tb003") {$data[$k] = $vtb003;} else {$data[$k] = $v;}
						}
					}
					$this->db->where('tb001', $ta001);
					...
					$this->db->update('acrtb',$data);//更改一筆
					$mtb003 = (int) $vtb003+10;
			        $vtb003 =  (string)$mtb003;
				}else{
					if($val['tb003'] && $val['tb004']&& $val['tb005']){
...
$this->db->query($sql8);
	...
	//取單號 最大值加1
	function check_title_no($acri01,$ta038){
		preg_match_all('/\d/S',$ta038, $matches);  //處理日期字串
		$ta038 = implode('',$matches[0]);
		$this->db->select('MAX(ta002) as max_no')
			->from('acrta')
			->where('ta001', $acri01)
		//	->where('ta038', $ta038);
			->like('ta038', $ta038, "after");
			
		$query = $this->db->get();
		$result = $query->result();
		
	    if (!$result[0]->max_no){return $ta038."001";}
