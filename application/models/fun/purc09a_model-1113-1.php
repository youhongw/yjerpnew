...
class Purc09a_model extends CI_Model {
	...
	function selbrowse($num,$offset)   //查詢 table 表所有資料
      {            
	    $this->db->select('tc001, tc002, tc003, tc004, tc004 as tc004disp, tc006, tc007, tc008, tc009, tc010, create_date');
	...
	function search($limit, $offset, $sort_by, $sort_order)     //欄位表頭排序流覽資料
	  ...
	  //  $sort_columns = array('tc001', 'tc002', 'tc003', 'tc004', 'tc004disp', 'tc006', 'tc007', 'tc008', 'tc009', 'tc010','create_date');
	   // $sort_by = (in_array($sort_by, $sort_columns)) ? $sort_by : 'tc001';  //檢查排序欄位是否在 table 內
	   $query = $this->db->select('tc001, tc002, tc003, tc004')
	                      ->from('purtc as a')
						   ->where('a.tc014', 'Y')                 					 
	  ...
	  function searchkkk($limit, $offset, $sort_by, $sort_order)     //欄位表頭排序流覽資料
	  { 
	  //  $sort_order = (substr($sort_order,0,3) == 'asc') ? 'asc' : 'desc';
	  //  $sort_columns = array('tc001', 'tc002', 'tc003', 'tc004', 'tc004disp', 'tc006', 'tc007', 'tc008', 'tc009', 'tc010','create_date');
	   // $sort_by = (in_array($sort_by, $sort_columns)) ? $sort_by : 'tc001';  //檢查排序欄位是否在 table 內
		...
		$this->db->join('purma as c', 'a.tc004 = b.ma001 ','left'); 
	    ...
	    $this->db->select('COUNT(*) as count', FALSE);  //筆數查詢,如果設為FALSE不會使用反引號保護你的字段或者表名
	    $this->db->from('purtc as a');
		$this->db->join('purtd as b', 'a.tc001 = b.td001 and a.tc002=b.td002 and td016='N' ','left'); 
		$this->db->join('purma as c', 'a.tc004 = b.ma001 ','left'); 
		$this->db->where('a.tc014', 'Y'); 
	...
	//	$this->db->like('title', 'match', 'after'); 
	    ...
	    $sort_by = (in_array($sort_by, $sort_columns)) ? $sort_by : 'tc001';  //檢查排序欄位是否在 table 內
	    $query = $this->db->select('tc001, tc002, tc003, tc004, tc005, tc006, tc007, tc008, tc009, tc010 ,create_date')
	                      ->from('purtc')
