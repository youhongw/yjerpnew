...
class Cmsi16a_model extends CI_Model {
	...
	function selbrowse($num,$offset)   //查詢 table 表所有資料
      {            
	    $this->db->select('mo001, mo006');
        $this->db->from('cmsmo');
        $this->db->order_by('mo001', 'ASC');                //排序  單欄
	   // $this->db->order_by('mo001 asc, mb002 asc');    //排序  單欄以上 asc 由小至大 desc預設由大至小
	...
	//	$array = array('mo001' => '1', 'mb002 >=' => $seq6, 'mb002 <=' => $seq6 );
	//	$this->db->like('title', 'match', 'after'); 
	    ...
	    $ret['rows'] = $query->get()->result();
	     ...
	     $array = array('mo001' => '1', 'mb002' => $seq6 );
	    $query = $this->db->select('COUNT(*) as count', FALSE)  //筆數查詢,如果設為FALSE不會使用反引號保護你的字段或者表名
	                      ->from('cmsmo')
						   ->like($seq4, $seq6, 'after');  
					//	  ->where('mo001','1');
	...
	function filterf1($limit, $offset , $sort_by  , $sort_order)    //篩選多筆        
	  {    
	    ...
	    $offset=$this->uri->segment(8,0);
	    $sort_order = (substr($sort_order,0,3) == 'asc') ? 'asc' : 'desc';
	    $sort_columns = array('mo001', 'mb002', 'mb003', 'mb004', 'mb005', 'mb006','create_date');
            $sort_by = (in_array($sort_by, $sort_columns)) ? $sort_by : 'mo001';  //檢查排序欄位是否為 table
			
	    $this->db->select('mo001, mb002, mb003, mb004, mb005, mb006, create_date');
	    $this->db->from('cmsmo');
		$this->db->where('mo001','1');
	    $this->db->like($sort_by, $seq4, 'after');
	    $this->db->order_by($sort_by, $sort_order);
	  //$this->db->order_by('mo001 asc, mb002 asc');
      ...
      function ajaxcmsi16a($seg1)    //ajax 查詢一筆 不更新網頁 廠別 
          { 	              
