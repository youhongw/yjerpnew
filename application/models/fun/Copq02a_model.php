...
class Copq02a_model extends CI_Model {
	...
	function selbrowse($num,$offset)   //查詢 table 表所有資料
      {            
        ...
        $this->db->from('copmb');
        $this->db->order_by('mb001', 'ASC');                //排序  單欄
	   // $this->db->order_by('mb001 asc, mb002 asc');    //排序  單欄以上 asc 由小至大 desc預設由大至小
	...
	function search($limit, $offset, $sort_by, $sort_order)     //欄位表頭排序流覽資料
	  { 
	    ...
	    $sort_columns = array('mb001', 'mb002', 'mb003', 'mb004', 'mb005', 'mb006','mb008','mb013','create_date');
	    $sort_by = (in_array($sort_by, $sort_columns)) ? $sort_by : 'mb001';  //檢查排序欄位是否在 table 內幣別,付款條件,課稅別
	    $query = $this->db->select('mb001, mb002, mb003, mb008, mb009, mb017,create_date')
	                      ->from('copmb')
	...
	//	$array = array('mb001' => '1', 'mb002 >=' => $seq6, 'mb002 <=' => $seq6 );
	//	$this->db->like('title', 'match', 'after'); 
	    $sort_order = (substr($sort_order,0,3) == 'asc') ? 'asc' : 'desc';
	    $sort_columns = array('mb001', 'mb002', 'mb003', 'mb004', 'mb005', 'mb006','mb008','mb013','create_date');
	    $sort_by = (in_array($sort_by, $sort_columns)) ? $sort_by : 'mb001';  //檢查排序欄位是否在 table 內
	    $query = $this->db->select('mb001, mb002, mb008, mb013,mb014,mb031,mb038')
	                      ->from('copmb')
	    ...
	    // $array = array('mb001' => '1', 'mb002' => $seq6 );
	    $query = $this->db->select('COUNT(*) as count', FALSE)  //筆數查詢,如果設為FALSE不會使用反引號保護你的字段或者表名
	                      ->from('copmb')
						   ->like($seq4, $seq6, 'after');  
					//	  ->where('mb001','1');
	...
	function findf($limit, $offset, $sort_by, $sort_order)    //查詢多筆進階查詢 mysql 
	  ...
	  //$seq5='';$seq51='';$seq7='';$seq71='';		  
	    ...
	    $seq1 = " mb001, mb002, mb003, mb004, mb005, mb006, create_date FROM `copmb` ";
	...
	function selone1($seg1,$seg2)    //查新增資料是否重複
	    ...
	    $this->db->set('mb002', $this->input->post('mb002'));
	...
	function insertf()    //新增一筆
	    ...
	    $exist = $this->invq01a_model->selone1($this->input->post('mb001'),$this->input->post('mb002'));
	    if ($exist)
	         {
		    return 'exist';
		 } 
        ...
        function selone2($seg1,$seg2)    //查copy複製資料是否重複
          { 	
	    $this->db->set('mb001', $this->input->post('mb003c'));              
	    $this->db->set('mb002', $this->input->post('mb004c'));
	    $this->db->where('mb001', $this->input->post('mb003c'));     
	    $this->db->where('mb002', $this->input->post('mb004c'));	
	    $query = $this->db->get('copmb');
	    return $query->num_rows() ; 
	  }
	    ...
	    $this->db->set('mb002', $this->input->post('mb002c'));
	    ...
	    $exist = $query->num_rows();
		  ...
		  if ($query->num_rows() == 1) 
            ...
            $seq3=$this->input->post('mb003c');    //主鍵一筆
	    $seq4=$this->input->post('mb004c'); 
	    $data = array( 
	                  'company' => $this->session->userdata('syscompany'),
	                  'creator' => $this->session->userdata('manager'),
		          'usr_group' => 'A100',
            ...
            $exist = $this->invq01a_model->selone2($this->input->post('mb003c'),$this->input->post('mb004c'));
		    if ($exist)
		        {
			  return 'exist';
		        }         
	...
	function excelnewf()           //轉excel檔,一筆以上
	    ...
	    $seq2=$this->input->post('mb002c'); 
	    $seq3=$this->input->post('mb003c'); 
	    $seq4=$this->input->post('mb004c'); 
	    $sql = " SELECT mb001,mb002,mb003,mb004,mb005,mb006,create_date FROM copmb WHERE mb001 >= '$seq1'  AND mb001 <= '$seq2' AND mb002 >= '$seq3' AND mb002 <= '$seq4' "; 
	...
	function printfd()           //印明細表一筆以上
	    ...
	    $seq3=$this->input->post('mb003c'); 
	    $seq4=$this->input->post('mb004c'); 
	    $sql = " SELECT * FROM copmb WHERE mb001 >= '$seq1'  AND mb001 <= '$seq2' AND mb002 >= '$seq3' AND mb002 <= '$seq4' "; 
            $query = $this->db->query($sql);
	    $ret['rows'] = $query->result();
    ...
    function ajaxcopq02a($seg1,$seg2,$seg3)    //ajax 查詢一筆 不更新網頁 供應商 解決亂碼
          { 	              
	 //   $this->db->set('mb001', $seg1);
	//	$this->db->set('mb002', $seg2);
	//	$this->db->set('mb003', $seg3);
	    $this->db->where('mb001', $seg1);	
		$this->db->where('mb002', $seg2);	
		$this->db->where('mb003', urldecode(urldecode($this->uri->segment(6))));	
	    $query = $this->db->get('copmb');
