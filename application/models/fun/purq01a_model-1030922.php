...
class Purq01a_model extends CI_Model {
	...
	function search($limit, $offset, $sort_by, $sort_order)     //欄位表頭排序流覽資料
	  { 
	    ...
	    $query = $this->db->select('ma001, ma002, ma003, ma004, ma005, ma006,ma008,ma013, create_date')
	                      ->from('purma')
      ...
      function searcha($limit, $offset, $sort_by, $sort_order)     //欄位表頭排序流覽資料
	  { 
	    $sort_order = (substr($sort_order,0,3) == 'asc') ? 'asc' : 'desc';
	    $sort_columns = array('ma001', 'ma002', 'ma003', 'ma004', 'ma005', 'ma006','ma008','ma013','create_date');
	    $sort_by = (in_array($sort_by, $sort_columns)) ? $sort_by : 'ma001';  //檢查排序欄位是否在 table 內
	...
	//	$this->db->like('title', 'match', 'after'); 
	    ...
	    $sort_columns = array('ma001', 'ma002', 'ma003', 'ma004', 'ma005', 'ma006','ma008','ma013','create_date');
	    $sort_by = (in_array($sort_by, $sort_columns)) ? $sort_by : 'ma001';  //檢查排序欄位是否在 table 內
	    $query = $this->db->select('ma001, ma002, ma008, ma013')
	                      ->from('purma')
    ...
    function ajaxcpurq01a($seg1)    //ajax 查詢一筆 不更新網頁 供應商 
          { 	              
