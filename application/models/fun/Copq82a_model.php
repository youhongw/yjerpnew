...
class Copq82a_model extends CI_Model {
	...
	function search($limit, $offset, $sort_by, $sort_order)     //欄位表頭排序流覽資料
	  { 
	     $key4 = trim($this->uri->segment(4));    //欄位
	    ...
	    $query = $this->db->select('mm001, mm002, mm003,mm004,mm005, create_date')
	                      ->from('copmm')
						  ->where('mm003',$key4)
	...
	//	$this->db->like('title', 'match', 'after'); 
	 ...
	 function ajaxcopq82a1($seg1)    //ajax 查詢一筆 顯示用 客戶類別6
          { 	              
	  ...
	  function ajaxcopq82a($seg1)    //ajax 查詢一筆 顯示用 客戶類別6
          { 	              
	    $this->db->set('mm001', $this->uri->segment(4));
	    $this->db->where('mm001', $this->uri->segment(4));	
	    $query = $this->db->get('copmm');
	...
	function findf($limit, $offset, $sort_by, $sort_order)    //查詢多筆進階查詢 mysql 
	  ...
	  //$seq5='';$seq51='';$seq7='';$seq71='';		  
	    ...
	    $seq1 = " mm001, mm002, mm003, mm004, mm005, mm006, create_date FROM `invma` ";
	...
	function excelnewf()           //轉excel檔,一筆以上
	    ...
	    $seq4=$this->input->post('mm004c'); 
	    $sql = " SELECT mm001,mm002,mm003,mm004,mm005,mm006,create_date FROM invma WHERE mm001 >= '$seq1'  AND mm001 <= '$seq2' AND mm002 >= '$seq3' AND mm002 <= '$seq4' "; 
	...
	function printfd()           //印明細表一筆以上
	    ...
	    $seq3=$this->input->post('mm003c'); 
	    $seq4=$this->input->post('mm004c'); 
	    $sql = " SELECT * FROM invma WHERE mm001 >= '$seq1'  AND mm001 <= '$seq2' AND mm002 >= '$seq3' AND mm002 <= '$seq4' "; 
            $query = $this->db->query($sql);
	    $ret['rows'] = $query->result();
