...
class Copq01a_model extends CI_Model {
	...
	function selbrowse($num,$offset)   //查詢 table 表所有資料
      {            
        ...
        $this->db->from('copma');
        $this->db->order_by('ma001', 'ASC');                //排序  單欄
	...
	function search($limit, $offset, $sort_by, $sort_order)     //欄位表頭排序流覽資料
	  { 
	    ...
	    $sort_columns = array('ma001', 'ma002', 'ma003', 'ma004', 'ma005', 'ma006','ma008','ma013','create_date');
	    $sort_by = (in_array($sort_by, $sort_columns)) ? $sort_by : 'ma001';  //檢查排序欄位是否在 table 內幣別,付款條件,課稅別,業務員,收款業務
	    $query = $this->db->select('ma001, ma002, ma003, ma004, ma005, ma006,ma008,ma013,ma014,ma031,ma038,ma027,ma016,ma085,ma064,ma015,create_date')
	                      ->from('copma')
	...
	//	if ($this->uri->segment(4) == "ma002") { $seq6 = $this->uri->segment(6); }
	...
	function findf($limit, $offset, $sort_by, $sort_order)    //查詢多筆進階查詢 mysql 
	  ...
	  //$seq5='';$seq51='';$seq7='';$seq71='';		  
	    ...
	    $seq1 = " ma001, ma002, ma003, ma004, ma005, ma006, create_date FROM `copma` ";
	...
	function filterf1($limit, $offset , $sort_by  , $sort_order)    //篩選多筆        
	  {    
	    ...
	    $this->db->select('ma001, ma002, ma003, ma004, ma005, ma006,ma008,ma013,ma014,ma031,ma038,ma027,ma016,ma085,ma064,ma015,create_date');
	    $this->db->from('copma');
	...
	function excelnewf()           //轉excel檔,一筆以上
	    ...
	    $seq4=$this->input->post('ma004c'); 
	    $sql = " SELECT ma001,ma002,ma003,ma004,ma005,ma006,create_date FROM copma WHERE ma001 >= '$seq1'  AND ma001 <= '$seq2' AND ma002 >= '$seq3' AND ma002 <= '$seq4' "; 
	...
	function printfd()           //印明細表一筆以上
	    ...
	    $seq3=$this->input->post('ma003c'); 
	    $seq4=$this->input->post('ma004c'); 
	    $sql = " SELECT * FROM copma WHERE ma001 >= '$seq1'  AND ma001 <= '$seq2' AND ma002 >= '$seq3' AND ma002 <= '$seq4' "; 
            $query = $this->db->query($sql);
	    $ret['rows'] = $query->result();
    ...
    function ajaxcopq01a($seg1)    //ajax 查詢一筆 不更新網頁 供應商 
          { 	              
    ...
    function ajaxcopq01a1($seg1)    //ajax 查詢一筆 不更新網頁 2客戶,38課稅,27送貨1 
          { 	              
	    $this->db->set('ma001', $this->uri->segment(4));
	    $this->db->where('ma001', $this->uri->segment(4));	
	    $query = $this->db->get('copma');
	    ...
	    if ($query->num_rows() > 0) 
		 {
		   ...
		   foreach ($query->result() as $row)
          {
               $result=$row->ma002.';'.$row->ma038.';'.$row->ma027.';'.$row->ma014.';'.$row->ma031.';'.$row->ma016.';'.$row->ma085.';'.$row->ma064.';'.$row->ma015;
            }
		   return $result;   
		 }
	  }  
