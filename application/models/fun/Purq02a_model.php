...
class Purq02a_model extends CI_Model {
	...
	function search($limit, $offset, $sort_by, $sort_order)     //欄位表頭排序流覽資料
	  { 
	    ...
	    $query = $this->db->select('mb002, mb001, mb004, mb011, mb008, mb014,create_date')
	                      ->from('purmb')
	...
	function findf($limit, $offset, $sort_by, $sort_order)    //查詢多筆進階查詢 mysql 
	  ...
	  //$seq5='';$seq51='';$seq7='';$seq71='';		  
	    ...
	    $seq1 = " mb001, mb002, mb003, mb004, mb005, mb006, create_date FROM `purmb` ";
	...
	function excelnewf()           //轉excel檔,一筆以上
	    ...
	    $seq4=$this->input->post('mb004c'); 
	    $sql = " SELECT mb001,mb002,mb003,mb004,mb005,mb006,create_date FROM purmb WHERE mb001 >= '$seq1'  AND mb001 <= '$seq2' AND mb002 >= '$seq3' AND mb002 <= '$seq4' "; 
	...
	function printfd()           //印明細表一筆以上
	    ...
	    $seq3=$this->input->post('mb003c'); 
	    $seq4=$this->input->post('mb004c'); 
	    $sql = " SELECT * FROM purmb WHERE mb001 >= '$seq1'  AND mb001 <= '$seq2' AND mb002 >= '$seq3' AND mb002 <= '$seq4' "; 
            $query = $this->db->query($sql);
	    $ret['rows'] = $query->result();
    ...
    function ajaxpurq02a($seg1,$seg2,$seg3)    //ajax 查詢一筆 不更新網頁 供應商 
          { 	              
	   // $this->db->set('mb001', $this->uri->segment(4));
	    $this->db->where('mb002', $this->uri->segment(4));	
		$this->db->where('mb001', $this->uri->segment(5));	
		$this->db->where('mb004', urldecode(urldecode($this->uri->segment(6))));
	    $query = $this->db->get('purmb');
