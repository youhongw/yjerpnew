...
class Admq02a_model extends CI_Model {
	...
	function selbrowse($num,$offset)   //查詢 table 表所有資料
      {            
	    $this->db->select('mb001, mb002, mb003, mb004,  create_date');
        $this->db->from('admmb');
	...
	function search($limit, $offset, $sort_by, $sort_order)     //欄位表頭排序流覽資料
	  { 
	    ...
	    $sort_columns = array('mb001', 'mb002', 'mb003', 'mb004', 'create_date');
	    $sort_by = (in_array($sort_by, $sort_columns)) ? $sort_by : 'mb001';  //檢查排序欄位是否在 table 內
	    $query = $this->db->select('mb001, mb002, mb003, mb004, create_date')
	                      ->from('admmb')
	...
	function findf($limit, $offset, $sort_by, $sort_order)    //查詢多筆進階查詢 mysql 
	  ...
	  //$seq5='';$seq51='';$seq7='';$seq71='';		  
	    ...
	    $seq1 = " mb001, mb002, mb003, mb004, create_date FROM `admmb` ";
	...
	function excelnewf()           //轉excel檔,一筆以上
	    ...
	    $seq4=$this->input->post('mb004c'); 
	    $sql = " SELECT mb001,mb002,mb003,mb004,create_date FROM admmb WHERE mb001 >= '$seq1'  AND mb001 <= '$seq2' AND mb002 >= '$seq3' AND mb002 <= '$seq4' "; 
	...
	function printfd()           //印明細表一筆以上
	    ...
	    $seq3=$this->input->post('mb003c'); 
	    $seq4=$this->input->post('mb004c'); 
	    $sql = " SELECT * FROM admmb WHERE mb001 >= '$seq1'  AND mb001 <= '$seq2' AND mb002 >= '$seq3' AND mb002 <= '$seq4' "; 
            $query = $this->db->query($sql);
	    $ret['rows'] = $query->result();
