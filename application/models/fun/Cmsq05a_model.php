...
class Cmsq05a_model extends CI_Model {
	...
	function selbrowse($num,$offset)   //查詢 table 表所有資料
      {            
        ...
        $this->db->from('cmsme');
        $this->db->order_by('me001', 'ASC');                //排序  單欄
	   // $this->db->order_by('me001 asc, me002 asc');    //排序  單欄以上 asc 由小至大 desc預設由大至小
	...
	//	if ($this->uri->segment(4) == "me002") { $seq6 = $this->uri->segment(6); }
	    $seq6 = urldecode(urldecode($this->uri->segment(6))); //輸入資料
		  $sort_by = $this->uri->segment(4);			
            $sort_order = $this->uri->segment(5);	
	    $offset=$this->uri->segment(8,0);
	//	$array = array('me001' => '1', 'me002 >=' => $seq6, 'me002 <=' => $seq6 );
	//	$this->db->like('title', 'match', 'after'); 
	    ...
	    $ret['rows'] = $query->get()->result();
	     ...
	     $array = array('me001' => '1', 'me002' => $seq6 );
	    $query = $this->db->select('COUNT(*) as count', FALSE)  //筆數查詢,如果設為FALSE不會使用反引號保護你的字段或者表名
	                      ->from('cmsme')
						   ->like($seq4, $seq6, 'after');  
					//	  ->where('me001','1');
	...
	function findf($limit, $offset, $sort_by, $sort_order)    //查詢多筆進階查詢 mysql 
	  ...
	  //$seq5='';$seq51='';$seq7='';$seq71='';		  
	    ...
	    $seq1 = " me001, me002, me003, me004, me005, me006, me007, me008, me009, me010, create_date FROM `cmsme` ";
	    ...
	    if (trim($this->input->post('find007'))!='') 
		    ...
		    $seq33=$seq7;
             ...
             $sort_order = (substr($sort_order,0,3) == 'asc') ? 'asc' : 'desc';
	     $sort_columns = array('me001', 'me002', 'me003', 'me004', 'me005', 'me006','create_date');
	     $sort_by = (in_array($sort_by, $sort_columns)) ? $sort_by : 'me001';  //檢查排序欄位是否在 table 內
	     $query = $this->db->select('me001, me002, me003, me004, me005, me006, create_date')
	                       ->from('invma')
		               ->where($seq32)
			       ->order_by($seq33)
			     //->order_by($sort_by, $sort_order)
	...
	function selone1($seg1,$seg2)    //查新增資料是否重複
	    ...
	    $this->db->set('me002', $this->input->post('me002'));
	...
	function insertf()    //新增一筆
	    ...
	    $exist = $this->invq01a_model->selone1($this->input->post('me001'),$this->input->post('me002'));
	    if ($exist)
	         {
		    return 'exist';
		 } 
        ...
        function selone2($seg1,$seg2)    //查copy複製資料是否重複
          { 	
	    $this->db->set('me001', $this->input->post('me003c'));              
	    $this->db->set('me002', $this->input->post('me004c'));
	    $this->db->where('me001', $this->input->post('me003c'));     
	    $this->db->where('me002', $this->input->post('me004c'));	
	    $query = $this->db->get('invma');
	    return $query->num_rows() ; 
	  }
	    ...
	    $this->db->set('me002', $this->input->post('me002c'));
	    ...
	    $exist = $query->num_rows();
		  ...
		  if ($query->num_rows() == 1) 
            ...
            $seq3=$this->input->post('me003c');    //主鍵一筆
	    $seq4=$this->input->post('me004c'); 
	    $data = array( 
	                  'company' => $this->session->userdata('syscompany'),
	                  'creator' => $this->session->userdata('manager'),
		          'usr_group' => 'A100',
            ...
            $exist = $this->invq01a_model->selone2($this->input->post('me003c'),$this->input->post('me004c'));
		    if ($exist)
		        {
			  return 'exist';
		        }         
	...
	function excelnewf()           //轉excel檔,一筆以上
	    ...
	    $seq2=$this->input->post('me002c'); 
	    $seq3=$this->input->post('me003c'); 
	    $seq4=$this->input->post('me004c'); 
	    $sql = " SELECT me001,me002,me003,me004,me005,me006,create_date FROM invma WHERE me001 >= '$seq1'  AND me001 <= '$seq2' AND me002 >= '$seq3' AND me002 <= '$seq4' "; 
	...
	function printfd()           //印明細表一筆以上
	    ...
	    $seq3=$this->input->post('me003c'); 
	    $seq4=$this->input->post('me004c'); 
	    $sql = " SELECT * FROM invma WHERE me001 >= '$seq1'  AND me001 <= '$seq2' AND me002 >= '$seq3' AND me002 <= '$seq4' "; 
            $query = $this->db->query($sql);
	    $ret['rows'] = $query->result();
		
            $seq32 = "me001 >= '$seq1'  AND me001 <= '$seq2' AND me002 >= '$seq3' AND me002 <= '$seq4' ";	
	    $query = $this->db->select('COUNT(*) as count', FALSE)  //筆數查詢,如果設為FALSE不會使用反引號保護你的字段或者表名
		              ->from('invma')
		              ->where($seq32);
	    $num = $query->get()->result();		
