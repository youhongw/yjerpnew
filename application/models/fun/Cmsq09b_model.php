...
class Cmsq09b_model extends CI_Model {
	...
	function insertf()    //新增一筆
	    ...
	    $data = array( 
            ...
            return $this->db->insert('cmsmj', $data);      //複製一筆  
          }		
	...
	function excelnewf()           //轉excel檔,一筆以上
	    ...
	    $seq4=$this->input->post('mj004c'); 
	    $sql = " SELECT mj001,mj002,mj003,mj004,create_date FROM cmsmj WHERE mj001 >= '$seq1'  AND mj001 <= '$seq2' AND mj002 >= '$seq3' AND mj002 <= '$seq4' "; 
	...
	function printfd()           //印明細表一筆以上
	    ...
	    $seq3=$this->input->post('mj003c'); 
	    $seq4=$this->input->post('mj004c'); 
	    $sql = " SELECT * FROM cmsmj WHERE mj001 >= '$seq1'  AND mj001 <= '$seq2' AND mj002 >= '$seq3' AND mj002 <= '$seq4' "; 
            $query = $this->db->query($sql);
	    $ret['rows'] = $query->result();
	...
	function ajaxcmsq09b($seg1)    //ajax 查詢一筆 顯示用 程式代號
          { 	              
