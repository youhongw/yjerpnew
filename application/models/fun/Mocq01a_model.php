...
class Mocq01a_model extends CI_Model {
	...
	//	$this->db->like('title', 'match', 'after'); 
	    ...
	    $ret['rows'] = $query->get()->result();
	     ...
	     $array = array('mq001' => '5', 'mq002' => $seq6 );
	    $query = $this->db->select('COUNT(*) as count', FALSE)  //筆數查詢,如果設為FALSE不會使用反引號保護你的字段或者表名
	                      ->from('cmsmq')
						   ->like($seq4, $seq6, 'after');  
					//	  ->where('mq001','1');
    ...
    function filterf35($limit, $offset , $sort_by  , $sort_order)    //篩選多筆        
      ...
      function ajaxmocq01a($seg1)    //ajax 查詢一筆 顯示用 請購單別 tb001
        { 
	...
	function amocq01a51($seg1)    //ajax 查詢一筆 顯示用 製令單別 tb001
        { 
	      $this->db->set('mq001', $this->uri->segment(4));
	...
	function ajaxmocq01a32($seg1)    //ajax 查詢一筆 顯示用 核價單別 tb001
        { 
	      $this->db->set('mq001', $this->uri->segment(4));
	      $this->db->where('mq003', '32');
          $this->db->where('mq001', $this->uri->segment(4));		  
