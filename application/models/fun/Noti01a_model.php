...
class Noti01a_model extends CI_Model {
	...
	function selbrowse($num,$offset)   //查詢 table 表所有資料
      {            
	    $this->db->select('ma001, ma002, ma004, ma006, ma005, ma011');
        $this->db->from('notma');
        $this->db->order_by('ma001', 'ASC');                //排序  單欄
	   // $this->db->order_by('ma001 asc, mb002 asc');    //排序  單欄以上 asc 由小至大 desc預設由大至小
	...
	function search($limit, $offset, $sort_by, $sort_order)     //欄位表頭排序流覽資料
	  { 
	    ...
	    $sort_columns = array('ma001', 'ma002');
	    $sort_by = (in_array($sort_by, $sort_columns)) ? $sort_by : 'ma001';  //檢查排序欄位是否在 table 內
	    $query = $this->db->select('ma001, ma002, ma004, ma006, ma005, ma011')
	                      ->from('notma')
	...
	//	$array = array('ma001' => '1', 'mb002 >=' => $seq6, 'mb002 <=' => $seq6 );
	//	$this->db->like('title', 'match', 'after'); 
	    $sort_order = (substr($sort_order,0,3) == 'asc') ? 'asc' : 'desc';
	    $sort_columns = array('ma001', 'ma002', 'ma004', 'ma006', 'ma005', 'ma011');
	    $sort_by = (in_array($sort_by, $sort_columns)) ? $sort_by : 'ma001';  //檢查排序欄位是否在 table 內
	    $query = $this->db->select('ma001, ma002, ma004, ma006, ma005, ma011')
	                      ->from('notma')
	    ...
	    $ret['rows'] = $query->get()->result();
	     ...
	     $array = array('ma001' => '1', 'mb002' => $seq6 );
	    $query = $this->db->select('COUNT(*) as count', FALSE)  //筆數查詢,如果設為FALSE不會使用反引號保護你的字段或者表名
	                      ->from('notma')
						   ->like($seq4, $seq6, 'after');  
					//	  ->where('ma001','1');
	...
	function findf($limit, $offset, $sort_by, $sort_order)    //查詢多筆進階查詢 mysql 
	  ...
	  //$seq5='';$seq51='';$seq7='';$seq71='';		  
	    ...
	    if (trim($this->input->post('find007'))!='') 
	     ...
	     $sort_columns = array('ma001', 'ma002');
	     $sort_by = (in_array($sort_by, $sort_columns)) ? $sort_by : 'ma001';  //檢查排序欄位是否在 table 內
	     $query = $this->db->select('ma001, ma002')
	                       ->from('notma')
		               ->where($seq32)
			       ->order_by($seq33)
			     //->order_by($sort_by, $sort_order)
	...
	function filterf1($limit, $offset , $sort_by  , $sort_order)    //篩選多筆        
	  {    
	    ...
	    $offset=$this->uri->segment(8,0);
	    $sort_order = (substr($sort_order,0,3) == 'asc') ? 'asc' : 'desc';
	    $sort_columns = array('ma001', 'mb002', 'mb003', 'mb004', 'mb005', 'mb006','create_date');
            $sort_by = (in_array($sort_by, $sort_columns)) ? $sort_by : 'ma001';  //檢查排序欄位是否為 table
			
	    $this->db->select('ma001, mb002, mb003, mb004, mb005, mb006, create_date');
	    $this->db->from('notma');
		$this->db->where('ma001','1');
	    $this->db->like($sort_by, $seq4, 'after');
	    $this->db->order_by($sort_by, $sort_order);
	  //$this->db->order_by('ma001 asc, mb002 asc');
      ...
      function ajaxnoti01a($seg1)    //ajax 查詢一筆 不更新網頁 廠別 
          { 	              
