...
class Purq04a_model extends CI_Model {
	...
	function __construct()
	  ...
	  { 
	   ...
	   function searchb36($limit, $offset, $sort_by, $sort_order)     //欄位表頭排序流覽資料 (全部)
	  { 
	    $sort_order = (substr($sort_order,0,3) == 'asc') ? 'asc' : 'desc';
	    $sort_columns = array('mq001', 'mq002', 'mq003', 'mq004', 'mq005', 'create_date');
	    $sort_by = (in_array($sort_by, $sort_columns)) ? $sort_by : 'mq001';  //檢查排序欄位是否在 table 內
	...
	//	$this->db->like('title', 'match', 'after'); 
	  ...
	  function search36($limit, $offset, $sort_by, $sort_order)     //欄位表頭排序流覽資料 (部份條件)
	  { 
	    $seq4 = trim($this->uri->segment(4));    //欄位
	//	 $seq6 = trim($this->uri->segment(7)); //輸入資料
	//	if ($this->uri->segment(4) == "mb002") { $seq6 = $this->uri->segment(6); }
    ...
    function filterf35($limit, $offset , $sort_by  , $sort_order)    //篩選多筆        
	  ...
	  //$this->db->order_by('mq001 asc, mq002 asc');
	    ...
	    return $ret;					 
			...
			function filterf36($limit, $offset , $sort_by  , $sort_order)    //篩選多筆        
	  {    
	    $seq4 = urldecode(urldecode($this->uri->segment(6))); 	 //解決亂碼          
            $sort_by = $this->uri->segment(4);			
            $sort_order = $this->uri->segment(5);	
      ...
      function ajaxpurq04a($seg1)    //ajax 查詢一筆 顯示用 請購單別 tb001
        { 
	...
	function ajaxpurq04a33($seg1)    //ajax 查詢一筆 顯示用 採購單別 tb001
        { 
	      $this->db->set('mq001', $this->uri->segment(4));
	      $this->db->where('mq003', '33');
          $this->db->where('mq001', $this->uri->segment(4));		  
	...
	function ajaxpurq04a34($seg1)    //ajax 查詢一筆 顯示用 進貨單別 tb001
        { 
	      $this->db->set('mq001', $this->uri->segment(4));
	      $this->db->where('mq003', '34');
          $this->db->where('mq001', $this->uri->segment(4));		  
		...
		function ajaxpurq04a35($seg1)    //ajax 查詢一筆 顯示用 退貨單別 tb001
        { 
	      $this->db->set('mq001', $this->uri->segment(4));
	      $this->db->where('mq003', '35');
          $this->db->where('mq001', $this->uri->segment(4));		  
	      ...
	      if ($query->num_rows() > 0) 
		   ...
		   return $result;   
			...
			function ajaxpurq04a36($seg1)    //ajax 查詢一筆 顯示用 退貨單別 tb001
        { 
	      $this->db->set('mq001', $this->uri->segment(4));
	      $this->db->where('mq003', '5B');
          $this->db->where('mq001', $this->uri->segment(4));		  
	...
	function ajaxpurq04a32($seg1)    //ajax 查詢一筆 顯示用 核價單別 tb001
        { 
	      $this->db->set('mq001', $this->uri->segment(4));
	      $this->db->where('mq003', '32');
          $this->db->where('mq001', $this->uri->segment(4));		  
