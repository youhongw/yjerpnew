...
class Invq02a_model extends CI_Model {
	...
	function search($limit, $offset, $sort_by, $sort_order)     //欄位表頭排序流覽資料
	  { 
	    ...
	    $sort_columns = array('mb001', 'mb002', 'mb003', 'mb004', 'mb005', 'mb006','mb025','create_date');
	    $sort_by = (in_array($sort_by, $sort_columns)) ? $sort_by : 'mb001';  //檢查排序欄位是否在 table 內
	    $query = $this->db->select('mb001, mb002, mb003, mb004, mb005, mb006,mb025, create_date')
	                      ->from('invmb')
	...
	//	$this->db->like('title', 'match', 'after'); 
	    ...
	    $sort_by = (in_array($sort_by, $sort_columns)) ? $sort_by : 'mb001';  //檢查排序欄位是否在 table 內
	    $query = $this->db->select('mb001, mb002, mb003, mb004,mb025')
	                      ->from('invmb')
	...
	function findf($limit, $offset, $sort_by, $sort_order)    //查詢多筆進階查詢 mysql 
	  ...
	  //$seq5='';$seq51='';$seq7='';$seq71='';		  
	    ...
	    $seq1 = " mb001, mb002, mb003, mb004, mb005, mb006, create_date FROM `invma` ";
	...
	function excelnewf()           //轉excel檔,一筆以上
	    ...
	    $seq4=$this->input->post('mb004c'); 
	    $sql = " SELECT mb001,mb002,mb003,mb004,mb005,mb006,create_date FROM invma WHERE mb001 >= '$seq1'  AND mb001 <= '$seq2' AND mb002 >= '$seq3' AND mb002 <= '$seq4' "; 
	...
	function printfd()           //印明細表一筆以上
	    ...
	    $seq3=$this->input->post('mb003c'); 
	    $seq4=$this->input->post('mb004c'); 
	    $sql = " SELECT * FROM invma WHERE mb001 >= '$seq1'  AND mb001 <= '$seq2' AND mb002 >= '$seq3' AND mb002 <= '$seq4' "; 
            $query = $this->db->query($sql);
	    $ret['rows'] = $query->result();
    ...
    function ajaxinvq02a($seg1)    //ajax 查詢一筆 顯示用 品號6
          { 	              
	    $this->db->set('mb001', $this->uri->segment(4));
	    $this->db->where('mb001', $this->uri->segment(4));	
	    ...
	    if ($query->num_rows() > 0) 
		 {
		   ...
		   foreach ($query->result() as $row)
          {
               $result=$row->mb002.';'.$row->mb003.';'.$row->mb004.';'.$row->mb072.';'.$row->mb025;
            }
		   return $result;   
		 }
	  }
	  function ajaxinvqa1($seg1)    //ajax 查詢一筆 顯示用 品號6
          { 	              
	    $this->db->set('mb001', $this->uri->segment(4));
	    $this->db->where('mb001', $this->uri->segment(4));	
	    $query = $this->db->get('invmb');
	    ...
	    if ($query->num_rows() > 0) 
		 {
		   ...
		   foreach ($query->result() as $row)
          {
               $result=$row->mb002.';'.$row->mb003.';'.$row->mb004;
            }
		   return $result;   
		 }
	  }
