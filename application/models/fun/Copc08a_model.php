...
class Copc08a_model extends CI_Model {
	...
	function search($limit, $offset, $sort_by, $sort_order,$otg001,$otg002,$otg004,$otg042)     //欄位表頭排序流覽資料
	  { 
	  ...
	  /*   $this->db->select('tc001, tc002, tc003, tc004,c.ma002 as tc004disp');
		 ...
		 $this->db->join('coptd as b', 'a.tc001 = b.td001 and a.tc002=b.td002 and b.td016="N" ','left'); 				  
		 $this->db->join('copma as c', 'a.tc004 = c.ma001 ','left'); 
		 $this->db->where('a.tc027', 'Y'); 		*/	
		 
        /*   $this->db->select('b.tc001, b.tc002, b.tc003, b.tc004,c.ma002 as tc004disp,$otg001 as otg001,$otg002 as otg002,$otg004 as otg004,$otg042 as otg042 ');
	     $this->db->from('coptd as a');
		 $this->db->join('coptc as b', 'a.td001 = b.tc001 and a.td002=b.tc002   ','inner'); 				  
		 $this->db->join('copma as c', 'b.tc004 = c.ma001  ','left');
         $this->db->where('b.tc004', $otg004);		 
		 $this->db->order_by($sort_by, $sort_order);
		 $this->db->limit($limit, $offset);
		 $this->db->distinct(); */
		 
		 $sql2 =" select distinct b.tc001, b.tc002, b.tc003, b.tc004,c.ma002 as tc004disp,$otg001 as otg001,$otg002 as otg002,$otg004 as otg004,$otg042 as otg042
                  from coptd as a
                  inner join coptc as b on a.td001 = b.tc001 and a.td002=b.tc002 
                  left join copma as c on 	b.tc004 = c.ma001
                  where b.tc004='$otg004' 
	  ...
	  //  echo var_dump($ret['rows']);exit;
	   //  echo var_dump($otg004);exit; ->where('b.tc027', 'Y')
		 
	    $this->db->select('COUNT(*) as count', FALSE)  //筆數查詢,如果設為FALSE不會使用反引號保護你的字段或者表名
	                      ->from('coptd as a')
						  ->join('coptc as b', 'a.td001 = b.tc001 and a.td002=b.tc002 and b.tc027="Y"  ','inner')
						  ->join('copma as c', 'b.tc004 = c.ma001  ','left')
                          ->where('b.tc004', $otg004);						  
		$this->db->distinct();				  
		$query = $this->db->get();				    
	    $num = $query->result();		
	...
	//	$this->db->like('title', 'match', 'after'); 
	  ...
	  /*   $this->db->select('tc001, tc002, tc003, tc004,c.ma002 as tc004disp');
	     ...
	     $this->db->from('coptd as a');
		 $this->db->join('coptc as b', 'a.td001 = b.tc001 and a.td002=b.tc002 and b.tc027="Y" and a.td016="N" ','inner'); 				  
		 $this->db->join('copma as c', 'b.tc004 = c.ma001 ','left'); 
	...
	function findf($limit, $offset, $sort_by, $sort_order)    //查詢多筆進階查詢 mysql 
	  ...
	  //$seq5='';$seq51='';$seq7='';$seq71='';		  
	    ...
	    $seq1 = " tc001, tc002, tc003, tc004, tc005, tc006, tc007, tc008, tc009, tc010, create_date FROM `coptc` ";
	...
	function delmutif()   //選取刪除多筆 
      ...
      function ajaxcopc08a($seg1)    //ajax 查詢一筆 不更新網頁 廠別 
          { 	              
