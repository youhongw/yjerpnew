...
class Bomc02a_model extends CI_Model {
	...
	function selbrowse($num,$offset)   //查詢 table 表所有資料
      {            
	    $this->db->select('md001, md002, md003, md004, md005, md006, md007, md008, md009, md010, create_date');
        $this->db->from('bommd');
        $this->db->order_by('md001', 'ASC');                //排序  單欄
	...
	function search($limit, $offset, $sort_by, $sort_order)     //欄位表頭排序流覽資料
	  { 
	    ...
	    $sort_columns = array('md001', 'md002', 'md003', 'md004', 'md005', 'md006', 'md007', 'md008', 'md009', 'md010','create_date');
	    $sort_by = (in_array($sort_by, $sort_columns)) ? $sort_by : 'md002';  //檢查排序欄位是否在 table 內
	   /*  $this->db->select('md001, md002, md003, md004,c.ma002 as md004disp');
	     $this->db->from('bommd as a');
		 $this->db->join('purtd as b', 'a.md001 = b.td001 and a.md002=b.td002 and b.td016="N"  ','inner'); 	//沒有不要出現			  
		 $this->db->join('purma as c', 'a.md004 = c.ma001 ','inner'); 
		 $this->db->where('a.md014', 'Y');
		 $this->db->order_by($sort_by, $sort_order);
		 $this->db->limit($limit, $offset);
		 $this->db->distinct();  */
		 if(!isset($ta015)) { $ta015=1; } 
		 if($this->uri->segment(5)>0) {$ta015=$this->uri->segment(5); } 
		// if( $ta015>0 ) {
		// $ta015=$this->uri->segment(5); } 
		 
		 $this->db->select('a.md001, a.md002, a.md003, a.md004,a.md006,b.mb002 as md003disp,b.mb003 as md003disp1,b.mb004 as md003disp2');
	     $this->db->from('bommd as a');
		 $this->db->join('invmb as b', 'a.md003 = b.mb001 ','left'); 
	    ...
	    $this->db->select('COUNT(*) as count', FALSE)  //筆數查詢,如果設為FALSE不會使用反引號保護你的字段或者表名
	                      ->from('bommd as a')
						  ->where('a.md001', $this->uri->segment(4));  
		//$this->db->distinct();				  
		$query = $this->db->get();				    
	    $num = $query->result();		
	...
	//	if ($this->uri->segment(4) == "md002") { $seq6 = $this->uri->segment(6); }
    ...
    /*	 $this->db->select('md001, md002, md003, md004,c.ma002 as md004disp');
		 $this->db->from('bommd as a');
		 $this->db->join('purtd as b', 'a.md001 = b.td001 and a.md002=b.td002 and b.td016="N" ','left'); 				  
		 $this->db->join('purma as c', 'a.md004 = c.ma001 ','left'); 
		 $this->db->like($seq4, $seq6, 'after');                   					 
		 $this->db->order_by($sort_by, $sort_order);
		 $this->db->limit($limit, $offset);		 
		$this->db->distinct();	*/
          $this->db->select('b.md001, b.md002, b.md003, b.md004,c.ma002 as md004disp');
	     $this->db->from('purtd as a');
		 $this->db->join('bommd as b', 'a.td001 = b.md001 and a.td002=b.md002 and b.md014="Y" and a.td016="N" ','inner'); 	//沒有不要出現			  
		 $this->db->join('purma as c', 'b.md004 = c.ma001 ','inner'); 
		 $this->db->like($seq4, $seq6, 'after');   
		 $this->db->order_by($sort_by, $sort_order);
		 $this->db->limit($limit, $offset);
		 $this->db->distinct(); 
	    ...
	    $ret['rows'] = $query->result();
	     ...
	     $array = array('md001' => '1', 'md002' => $seq6 );
	...
	function findf($limit, $offset, $sort_by, $sort_order)    //查詢多筆進階查詢 mysql 
	  ...
	  //$seq5='';$seq51='';$seq7='';$seq71='';		  
	    ...
	    $seq1 = " md001, md002, md003, md004, md005, md006, md007, md008, md009, md010, create_date FROM `bommd` ";
	...
	function delmutif()   //選取刪除多筆 
      ...
      function ajaxbomc02a($seg1)    //ajax 查詢一筆 不更新網頁 廠別 
          { 	              
