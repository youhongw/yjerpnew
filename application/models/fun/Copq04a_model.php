...
class Copq04a_model extends CI_Model {
	...
	function selbrowse($num,$offset)   //查詢 table 表所有資料
      {            
        ...
        $this->db->from('copml');
        $this->db->order_by('ml001', 'ASC');                //排序  單欄
	   // $this->db->order_by('ml001 asc, ml002 asc');    //排序  單欄以上 asc 由小至大 desc預設由大至小
	...
	//	$array = array('ml001' => '1', 'ml002 >=' => $seq6, 'ml002 <=' => $seq6 );
	//	$this->db->like('title', 'match', 'after'); 
	 ...
	 //    $array = array('ml001' => '1', 'ml002' => $seq6 );
	    $query = $this->db->select('COUNT(*) as count', FALSE)  //筆數查詢,如果設為FALSE不會使用反引號保護你的字段或者表名
	                      ->from('copml')
						   ->like($seq4, $seq6, 'after');  
					//	  ->where('ml001','1');
	 ...
	 function ajaxcopq04a1($seg1)    //ajax 查詢一筆 顯示用 客戶類別6
          { 	              
	  ...
	  function ajaxcopq04a($seg1)    //ajax 查詢一筆 顯示用 客戶類別6
          { 	              
	    $this->db->set('ml001', $this->uri->segment(4));
	    $this->db->where('ml001', $this->uri->segment(4));	
	    $query = $this->db->get('copml');
	...
	function findf($limit, $offset, $sort_by, $sort_order)    //查詢多筆進階查詢 mysql 
	  ...
	  //$seq5='';$seq51='';$seq7='';$seq71='';		  
	    ...
	    $seq1 = " ml001, ml002, ml003, ml004, ml005, ml006, create_date FROM `invma` ";
	...
	function insertf()    //新增一筆
	    ...
	    $exist = $this->invq01a_model->selone1($this->input->post('ml001'),$this->input->post('ml002'));
	    if ($exist)
	         {
		    return 'exist';
		 } 
	    ...
	    $data = array( 
            ...
            $exist = $this->invq01a_model->selone2($this->input->post('ml003c'),$this->input->post('ml004c'));
		    if ($exist)
		        {
			  return 'exist';
		        }         
	...
	function excelnewf()           //轉excel檔,一筆以上
	    ...
	    $seq4=$this->input->post('ml004c'); 
	    $sql = " SELECT ml001,ml002,ml003,ml004,ml005,ml006,create_date FROM invma WHERE ml001 >= '$seq1'  AND ml001 <= '$seq2' AND ml002 >= '$seq3' AND ml002 <= '$seq4' "; 
	...
	function printfd()           //印明細表一筆以上
	    ...
	    $seq3=$this->input->post('ml003c'); 
	    $seq4=$this->input->post('ml004c'); 
	    $sql = " SELECT * FROM invma WHERE ml001 >= '$seq1'  AND ml001 <= '$seq2' AND ml002 >= '$seq3' AND ml002 <= '$seq4' "; 
            $query = $this->db->query($sql);
	    $ret['rows'] = $query->result();
	...
	function updatef()   //更改一筆
	    ...
	    $ml002=$this->input->post('ml002');
            ...
            $this->db->where('ml001', $ml001);
	    $this->db->where('ml002', $ml002);
