...
class Actq02a_model extends CI_Model {
	...
	function filterf65($limit, $offset , $sort_by  , $sort_order)    //篩選多筆        
     ...
     function ajaxactq02a($seg1)    //ajax 查詢一筆 顯示用 傳票單別 tb001
        { 
	      $this->db->set('mq001', $this->uri->segment(4));
	      $this->db->where('mq003  >=', '9');
		 ...
		 function ajaxactq02a61($seg1)    //ajax 查詢一筆 顯示用 結帳單別 tb001
        { 
	      $this->db->set('mq001', $this->uri->segment(4));
	  ...
	  function ajaxactq02a63($seg1)    //ajax 查詢一筆 顯示用 收款單別 tb001
        { 
	      $this->db->set('mq001', $this->uri->segment(4));
