...
class Invq81a_model extends CI_Model {
	...
	//	$this->db->like('title', 'match', 'after'); 
	    ...
	    $sort_by = (in_array($sort_by, $sort_columns)) ? $sort_by : 'md001';  //檢查排序欄位是否在 table 內
	    $query = $this->db->select('md001, md002, md003, md004 ,create_date')
	                      ->from('invmd')
	...
	function findf($limit, $offset, $sort_by, $sort_order)    //查詢多筆進階查詢 mysql 
	  ...
	  //$seq5='';$seq51='';$seq7='';$seq71='';		  
	    ...
	    $seq1 = " md001, md002, md003, md004, md005, md006, md007, md008, md009, md010, create_date FROM `invmd` ";
	...
	function delmutif()   //選取刪除多筆 
     ...
     function ajaxinvq81a($seg1)    //ajax 查詢一筆 顯示用 請購公司
        { 	              
	...
	function search_hp($limit, $offset, $sort_by, $sort_order, $tc004, $td004, $row_count){//換算單位
	    ...
	    $query = $this->db->select('a.md001, a.md002, a.md003,a.create_date')
	                    ->from('invmd as a')
						->where('a.md001',$td004);
	    $ret['rows'] = $query->get()->result();
		...
		$sort_order = (substr($sort_order,0,3) == 'asc') ? 'asc' : 'desc';
	    $sort_columns = array('a.md001', 'a.md002', 'a.md003','a.create_date');
		if($sort_by == "create_date")
			$sort_by = "a.".$sort_by;
		else
			$sort_by = "b.".$sort_by;
	    $sort_by = (in_array($sort_by, $sort_columns)) ? $sort_by : 'md001';  //檢查排序欄位是否在 table 內幣別,付款條件,課稅別,業務員,收款業務
	    $query = $this->db->select('a.md001,a.md002,a.md003, a.create_date')
	                    ->from('invmd as a')
		                ->order_by($sort_by, $sort_order)
		                ->limit($limit, $offset)
		...
		//先取有其產品的主單 //檢查排序欄位是否在 table 內幣別,付款條件,課稅別,業務員,收款業務
	    $query = $this->db->select('a.md001, a.md002, a.md003')
	                    ->from('invmd as a')
						->where('a.md001',$td004);
	    $ret['rows'] = $query->get()->result();
		$query = $this->db->select('COUNT(*) as count' ,FALSE)
