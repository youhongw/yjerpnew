...
class Copq03a_model extends CI_Model {
	...
	function __construct()
	  ...
	  { 
	    ...
	    function searchb1($limit, $offset, $sort_by, $sort_order)     //欄位表頭排序流覽資料 (全部)
	  { 
	    $sort_order = (substr($sort_order,0,3) == 'asc') ? 'asc' : 'desc';
	    $sort_columns = array('mq001', 'mq002', 'mq003', 'mq004', 'mq005', 'mq006','create_date');
	    $sort_by = (in_array($sort_by, $sort_columns)) ? $sort_by : 'mq001';  //檢查排序欄位是否在 table 內
	...
	function ajaxpcopq03a21($seg1)    //ajax 查詢一筆 顯示用 報價單別 tb001
        { 
      ...
      function ajaxpcopq03a22($seg1)    //ajax 查詢一筆 顯示用 訂單單別 tb001
        { 
	      $this->db->set('mq001', $this->uri->segment(4));
	 ...
	 function ajaxpcopq03a221($seg1)    //ajax 查詢一筆 顯示用 訂單單別 tb001
        { 
	      $this->db->set('mq001', $this->uri->segment(4));
	      $this->db->where('mq003', '22');
          $this->db->where('mq001', $this->uri->segment(4));		  
     ...
     function ajaxpcopq03a23($seg1)    //ajax 查詢一筆 顯示用 銷貨單別 tb001
        { 
	      $this->db->set('mq001', $this->uri->segment(4));
	      $this->db->where('mq003', '23');
          $this->db->where('mq001', $this->uri->segment(4));		  
	      $query = $this->db->get('cmsmq');
     ...
     function ajaxpcopq03a24($seg1)    //ajax 查詢一筆 顯示用 退貨單別 tb001
        { 
	      $this->db->set('mq001', $this->uri->segment(4));
	      $this->db->where('mq003', '24');
          $this->db->where('mq001', $this->uri->segment(4));		  
	      $query = $this->db->get('cmsmq');
