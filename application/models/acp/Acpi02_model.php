...
class Acpi02_model extends CI_Model {
	...
	function search($limit, $offset, $sort_by, $sort_order)     
	    { 
	     ...
	     $sort_columns = array('ta001', 'ta002', 'b.ma002','ta003', 'ta004','ta004disp',  'ta028','ta029','create_date');
	     $sort_by = (in_array($sort_by, $sort_columns)) ? $sort_by : 'ta001';  //檢查排序欄位是否在 table 內
	     $query = $this->db->select('a.ta001, a.ta002,b.ma002, a.ta003, a.ta004,b.ma002 as ta004disp, a.ta028, a.ta029,a.ta026,a.create_date')
	                       ->from('acpta as a')
	...
	function construct_sql($limit = 15, $offset = 0, $func = "")
	  {
		...
		if(isset($_SESSION['acpi02']['search']['where'])){
			if($where){$where .= " and ";}
			$where .= $_SESSION['acpi02']['search']['where'];
		}
		...
		if(isset($_SESSION['acpi02']['search']['order'])){
			if($order){$order .= " , ";}
			$order .= $_SESSION['acpi02']['search']['order'];
		}
		
		if(!isset($_SESSION['acpi02']['search']['order']) && $default_order){
			if($order){$order .= " , ";}
			$order .= $default_order;
		}
		/* order end */
		...
		/* Data SQL */
		$query = $this->db->select('a.ta001,c.mq002, a.ta002,a.ta034, a.ta003, a.ta004, b.ma002,a.ta006,(a.ta028+a.ta029) as ta2829, a.ta028, a.ta029, a.ta030, a.ta026,a.create_date')
	                       ->from('acpta as a')
						   ->join('purma as b', 'a.ta004 = b.ma001','left')
		...
		$ret['data'] = $query->get()->result();
		//儲存sql 語法
		$_SESSION['acpi02']['search']['sql'] = $this->db->last_query();
		...
		//儲存where與order
		$_SESSION['acpi02']['search']['where'] = $where;
		$_SESSION['acpi02']['search']['order'] = $order;
		$_SESSION['acpi02']['search']['offset'] = $offset;
	...
	function construct_view($data){
		...
		$_SESSION['acpi02']['search']['view'] = $view_array;
		$_SESSION['acpi02']['search']['index'] = $index_array;
		//echo "<pre>";var_dump($_SESSION['acpi02']['search']['view']);exit;
	}	
	//查詢前置單據用 (看資料用)   
	...
	function selone($seq1,$seq2)    
        {
		  $this->db->select('a.* ,c.mq002 AS ta001disp, d.mb002 AS ta005disp,e.mf002 AS ta008disp, g.na003 AS ta039disp,
		...
		$this->db->join('cmsmq as c', 'a.ta001 = c.mq001 and c.mq003="71" ','left');
	    $this->db->join('cmsmb as d', 'a.ta005 = d.mb001 ','left');
		$this->db->join('cmsmf as e', 'a.ta008 = e.mf001 ','left');
		$this->db->join('cmsna as g ', 'a.ta039 = g.na002 and g.na001= "1" ','left');
		...
		$this->db->select('b.*,c.ma002 as tb013disp,d.me002 as tb014disp')
			->from('acptb as b')
			->join('actma as c', 'b.tb013 = c.ma001 ','left')   //科目
			->join('cmsme as d', 'b.tb014 = d.me001 ','left')   //部門
			->where('b.tb001', $seq1)
			->where('b.tb002', $seq2);
		$query = $this->db->get();
	...
	function ajaxchkno1($seg1)    
        { 	              
	      ...
	      $this->db->where('ta034', $this->uri->segment(5));
	...
	function findf($limit, $offset, $sort_by, $sort_order)     
         {            		
	      ...
	      $seq1 = "ta001, ta002, ta003, ta004, ta013, ta018,ta016,ta015,ta010,ta011,ta021,ta028,ta029,ta026, create_date FROM `acpta` ";
        ...
        if ($seq7!='') {$seq9=" ORDER BY ".$seq7;$seq33=$seq7;}
		 //下一頁不會亂跳
		if(@$_SESSION['acpi02_sql_term']){$seq32 = $_SESSION['acpi02_sql_term'];}
		if(@$_SESSION['acpi02_sql_sort']){$seq33 = $_SESSION['acpi02_sql_sort'];}
		
         $sort_order = (substr($sort_order,0,3) == 'asc') ? 'asc' : 'desc';
	     $sort_columns = array('ta001', 'ta002', 'b.ma002','ta003', 'ta004', 'ta013', 'ta018','ta007','ta008','ta010','ta011','ta021','ta028','ta029','create_date');
	     $sort_by = (in_array($sort_by, $sort_columns)) ? $sort_by : 'ta001';  //檢查排序欄位是否在 table 內
	     $query = $this->db->select('ta001, ta002, ta003, ta004,b.ma002, ta004,b.ma002 as ta004disp, ta013,ta018,ta016,ta015,ta011,ta021,ta028,ta029,ta026, a.create_date')
	                       ->from('acpta as a')
						   ->join('purma as b', 'a.ta004 = b.ma001 ','left')
		                   ->where($seq32)
			               ->order_by($seq33)
	...
	function filterf1($limit, $offset , $sort_by  , $sort_order)          
	    {    
	      ...
	      $sort_order = (substr($sort_order,0,3) == 'asc') ? 'asc' : 'desc';
	      $sort_columns = array('ta001', 'ta002', 'b.ma002','ta003', 'ta004', 'ta004disp', 'ta028','ta029','ta026','create_date');
          $sort_by = (in_array($sort_by, $sort_columns)) ? $sort_by : 'ta001';  //檢查排序欄位是否為 table
	      $this->db->select('ta001, ta002, ta003,b.ma002, ta004,b.ma002 as  ta004disp, ta028,ta029,ta026, a.create_date');
	      $this->db->from('acpta as a');
	...
	function selone1($seg1)    
        {
	      $this->db->where('ta001', $this->input->post('acpq01a71'));
    ...
    function selone1d($seg1)    
        {
	      $this->db->where('tb001', $this->input->post('acpq01a71'));
	...
	function insertf()    //新增一筆 檔頭  acpta
        { 
			 ...
			 preg_match_all('/\d/S',$this->input->post('ta015'), $matches);  //處理日期字串
			 $ta015 = implode('',$matches[0]);
			 preg_match_all('/\d/S',$this->input->post('ta019'), $matches);  //處理日期字串
			 $ta019 = implode('',$matches[0]);
			 preg_match_all('/\d/S',$this->input->post('ta020'), $matches);  //處理日期字串
			 $ta020 = implode('',$matches[0]);
			 preg_match_all('/\d/S',$this->input->post('ta032'), $matches);  //處理日期字串
			 $ta032 = implode('',$matches[0]);
			 preg_match_all('/\d/S',$this->input->post('ta034'), $matches);  //處理日期字串
			 $ta034 = implode('',$matches[0]);
			 
			 preg_match_all('/\d/S',$this->input->post('ta040'), $matches);  //處理日期字串
			 $ta040 = implode('',$matches[0]);
			 preg_match_all('/\d/S',$this->input->post('ta041'), $matches);  //處理日期字串
			 $ta041 = implode('',$matches[0]);
			 
			 $ta001=$this->input->post('ta001');
			 $ta002=$this->input->post('ta002');
			 $ta002no=$ta002;   //明細用再新增一筆時加1
			 //檢查資料是否已存在 若存在加1
			  while($this->acpi02_model->selone1($ta001,$ta002)>0){
				$ta002 = $this->check_title_no($ta001,$ta034);
				$ta002no=$ta002;
			}
	     $data = array( 
		...
		// 新增明細 acptb
		   ...
		   foreach($order_product as $key => $val){
		        if($val['tb003'] && $val['tb004']&& $val['tb005']){
				        extract($val);
						preg_match_all('/\d/S',$tb008, $matches);  //處理日期字串
			            $tb008 = implode('',$matches[0]);
						$data = array( 
							'company' => $this->session->userdata('syscompany'),
							'creator' => $this->session->userdata('manager'),
							'usr_group' => 'A100',
						...
						foreach($val as $k=>$v){
							if($k!="tb001"&&$k!="tb002"&&$k!="tb013disp"&&$k!="tb014disp"){//主鍵不用更改以及其他外來鍵庫別名稱
							    if($k=="tb003") {$data[$k] = $vtb003;} else {$data[$k] = $v;}
							}
						}
					$this->db->insert('acptb', $data);
	...
	function auto_print(){
		...
		$this->db->from('cmsmq');
		$this->db->where("mq001",$this->input->post('ta001'));	
		$query = $this->db->get();
		$tmp = $query->result();
		if(@$tmp[0]->mq016=="Y"){
		    //  echo "<script>window.open('printbb/".$this->input->post('ta001')."/".$this->input->post('ta002').".html', '_blank','menubar=no,status=no,scrollbars=no,top=0,left=0,toolbar=no,width=800,height=600');</script>";
		}
	}	 		
	//查複製資料是否重複	 
	...
	function printfd()          
        {
		  ...
		  b.tb010,b.tb011,b.tb012,b.tb015,b.tb016,b.tb018
	...
	function printfc()   
      {           
        $this->db->select('a.* ,c.mq002 AS ta001disp, d.mb002 AS ta005disp,e.mf002 AS ta008disp, g.na003 AS ta039disp,
		  ,h.ma002 AS ta004disp,b.company, b.creator, b.usr_group, b.create_date, b.modifier, b.modi_date, b.flag, b.tb001, b.tb002, b.tb003, b.tb004, b.tb005,
		  b.tb006, b.tb007, b.tb008, b.tb009, b.tb010, b.tb011, b.tb012,b.tb013, b.tb014,b.tb015, b.tb016, b.tb017, b.tb018,i.ma003 as tb013disp');
		 
        $this->db->from('acpta as a');	
        $this->db->join('acptb as b', 'a.ta001 = b.tb001  and a.ta002=b.tb002 ','left');		
		$this->db->join('cmsmq as c', 'a.ta001 = c.mq001 and substring(c.mq003,1,1)="7" ','left');
	    $this->db->join('cmsmb as d', 'a.ta005 = d.mb001 ','left');
		$this->db->join('cmsmf as e', 'a.ta008 = e.mf001 ','left');
		$this->db->join('cmsna as g ', 'a.ta039 = g.na002 and g.na001= "1" ','left');
		$this->db->join('purma as h', 'a.ta004 = h.ma001 ','left');
		$this->db->join('actma as i', 'a.ta013 = i.ma001 ','left');
	...
	function updatef()   
        {
          ...
          $data = array(			
			...
			//echo var_dump($this->input->post('ta008'));var_dump($ta002);exit;
			
            $this->db->where('ta001', $ta001);
			$this->db->where('ta002', $ta002);
            $this->db->update('acpta',$data);  
			...
			if(!is_array($order_product)){$order_product=array();}
					...
					$this->db->delete('acptb'); //刪除明細 1060809
		...
		//	$this->db->flush_cache();  
			...
			foreach($order_product as $key => $val){
				...
				if($this->seldetail($ta001,$ta002,$val['tb003'])>0){
					...
					foreach($val as $k=>$v){
						if($k!="tb001"&&$k!="tb002"&&$k!="tb013disp" && $k!="tb014disp"){//主鍵不用更改以及其他外來鍵庫別名稱 tb013日期等別處理
							if($k=="tb003") {$data[$k] = $vtb003;} else {$data[$k] = $v;}
						}
					}
					$this->db->where('tb001', $ta001);
					...
					$this->db->update('acptb',$data);//更改一筆
					$mtb003 = (int) $vtb003+10;
			        $vtb003 =  (string)$mtb003;
				}else{
					if($val['tb003'] && $val['tb004']&& $val['tb005']){
	    ...
	    if (!empty($_POST['selected'])) 
	         {
                foreach($_POST['selected'] as $check) 
			    {
				 ...
				 if ($seq3 != 'Y') { 
				 //進貨單更新碼 Y 改 N TG015 
	$sql9 =" update purtg as c,(select tb001,tb002,tb005,tb006,tg015 from acptb as b,purtg as c
                   where  tb005=tg001 AND tb006=tg002 AND tb001='$seq1'  AND tb002='$seq2' AND
                        tg013='Y' AND tg015='Y' AND tb004='1'
                ) d
               set c.tg015='N'
               where d.tb005=c.tg001 and d.tb006=c.tg002  " ; 
$this->db->query($sql9);
		//退貨單更新碼 Y 改 N Ti020
	$sql8 =" update purti as c,(select tb001,tb002,tb005,tb006,ti020 from acptb as b,purti as c
                   where  tb005=ti001 AND tb006=ti002 AND tb001='$seq1'  AND tb002='$seq2' AND
                        ti013='Y' AND ti020='Y' AND tb004='2'
                ) d
               set c.ti020='N'
...
$this->db->query($sql8);
                  ...
                  $this->db->delete('acptb'); $this->session->set_userdata('msg1',"未付款已刪除"); }
	...
	//取單號 最大值加1
	function check_title_no($ta001,$ta034){
		preg_match_all('/\d/S',$ta034, $matches);  //處理日期字串
		$ta034 = implode('',$matches[0]);
		$this->db->select('MAX(ta002) as max_no')
			->from('acpta')
			->where('ta001', $ta001)
		//	->where('ta034', $ta034);
			->like('ta034', $ta034, "after");
			
		$query = $this->db->get();
		$result = $query->result();
		
	    if (!$result[0]->max_no){return $ta034."001";}
