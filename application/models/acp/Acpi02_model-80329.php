...
class Acpi02_model extends CI_Model {
	...
	function insertf()    //新增一筆 檔頭  acpta
		...
		//	 if ($this->input->post('ta018')!='1') {$ta019=round($this->input->post('ta019'));}
		    if ($this->input->post('ta015')=='') {$ta015=$this->input->post('ta015');} else {$ta015=substr($this->input->post('ta015'),0,4).substr($this->input->post('ta015'),5,2).substr($this->input->post('ta015'),8,2);} 
			if ($this->input->post('ta019')=='') {$ta019=$this->input->post('ta019');} else {$ta019=substr($this->input->post('ta019'),0,4).substr($this->input->post('ta019'),5,2).substr($this->input->post('ta019'),8,2);} 
			if ($this->input->post('ta020')=='') {$ta020=$this->input->post('ta020');} else {$ta020=substr($this->input->post('ta020'),0,4).substr($this->input->post('ta020'),5,2).substr($this->input->post('ta020'),8,2);} 
		    if ($this->input->post('ta040')=='') {$ta040=$this->input->post('ta040');} else {$ta040=substr($this->input->post('ta040'),0,4).substr($this->input->post('ta040'),5,2).substr($this->input->post('ta040'),8,2);} 
		    if ($this->input->post('ta041')=='') {$ta041=$this->input->post('ta041');} else {$ta041=substr($this->input->post('ta041'),0,4).substr($this->input->post('ta041'),5,2).substr($this->input->post('ta041'),8,2);} 
		   if ($this->input->post('ta032')=='') {$ta032=$this->input->post('ta032');} else {$ta032=substr($this->input->post('ta032'),0,4).substr($this->input->post('ta032'),5,2);} 
	     ...
	     $data = array( 
	      ...
	      $exist = $this->acpi02_model->selone1($this->input->post('acpq01a71'),$this->input->post('ta002'));
	      if ($exist)
	         {
		      return 'exist';
		     } 
		...
		// 新增明細 acptb
            ...
            // 新增明細 acptb  主檔 acpta 重計算合計金額 數量
			    $ta037=0;$ta038=0;$ta028=0;$ta029=0;$ta022b=0;				
			    $n = '0';
				$tb003='1000';
		...
		//	while ($_POST['order_product'][  $n  ]['tb004']) {		
	     ...
	     // $exist = $this->acpi02_model->selone1d($this->input->post('purq04a34'),$this->input->post('ta002'));
		  ...
		  $this->db->insert('acptb', $data_array); }
		       $ta037=$ta037+$_POST['order_product'][ $n  ]['tb015'];
			  $ta038=$ta038+$_POST['order_product'][ $n  ]['tb016'];
			...
			//重新計算貨款 acpta
		  $sql = " UPDATE acpta set ta037='$ta037',ta038='$ta038',ta028='$ta028',ta029='$ta029' WHERE ta001 = '$ta001'  AND ta002 = '$ta002'  "; 
     ...
     function auto_print(){
		...
		$this->db->from('cmsmq');
		$this->db->where("mq001",$this->input->post('acpq01a71'));	
		$query = $this->db->get();
		$tmp = $query->result();
		if($tmp[0]->mq016=="Y"){
			echo "<script>window.open('printbb/".$this->input->post('acpq01a71')."/".$this->input->post('ta002').".html', '_blank','menubar=no,status=no,scrollbars=no,top=0,left=0,toolbar=no,width=800,height=600');</script>";
		}
	}	 		
	//查複製資料是否重複	 
	...
	function updatef()   
		...
		//	$this->db->flush_cache();  
			// 新增明細 acptb  主檔 acpta 重計算合計金額 數量
			    $ta028=0;$ta029=0;$ta037=0;$ta038=0;$ta022b=0;
			    $n = '0';		
				$tb003='1000';
		//	while ($_POST['order_product'][  $n  ]['tb004']) {
			while (isset($_POST['order_product'][  $n  ]['tb004'])) {
