...
class Moci01_model extends CI_Model
{
	...
	{
		$this->session->set_userdata('moci01_search', "display_search/" . $offset);
		...
		if (isset($_SESSION['moci01']['search']['where'])) {
			if ($where) {
				$where .= " and ";
			}
			$where .= $_SESSION['moci01']['search']['where'];
		}
		...
		if (isset($_SESSION['moci01']['search']['order'])) {
			if ($order) {
				$order .= " , ";
			}
			$order .= $_SESSION['moci01']['search']['order'];
		}

		if (!isset($_SESSION['moci01']['search']['order']) && $default_order) {
			if ($order) {
				$order .= " , ";
			}
			$order .= $default_order;
		...
		$query = $this->db->query(" SELECT MQ001,MQ002 from CMSMQ WHERE MQ003='51' ORDER BY MQ001 ");
		...
		// $ret['data'] = $query->get()->result();
		//儲存sql
		$_SESSION['moci01']['search']['sql'] = $this->db->last_query();
		...
		//儲存where與order
		$_SESSION['moci01']['search']['where'] = $where;
		$_SESSION['moci01']['search']['order'] = $order;
		$_SESSION['moci01']['search']['offset'] = $offset;

		return $ret;
	}
	//建構SQL字串
	function construct_sql_moci01($limit = 15, $offset = 0, $func = "")
	{
		$this->session->set_userdata('moci01_search', "display_search/" . $offset);
		if (@session_status() == PHP_SESSION_NONE) {
			session_start();
	...
	function construct_sql1_moci01($limit = 15, $offset = 0, $func = "")
	{
		$this->session->set_userdata('moci01_search', "display_search/" . $offset);
		if (session_status() == PHP_SESSION_NONE) {
			session_start();
		...
		// if ($this->uri->segment(3, 0) == "display_child1_moci01") {
		// 	unset($_SESSION['moci01']['search']);
		// }
		...
		/* Data SQL */
		$sql = " SELECT MQ001,MQ002 from CMSMQ WHERE MQ003='51' ORDER BY MQ001 ";
		if ($where) {
			$sql = " SELECT MQ001,MQ002 from CMSMQ WHERE MQ003='51' and " . $where;
		}
		...
		//  MySQL 轉 MSSQL 時limit的替代方式 end----------------

		//儲存sql
		$_SESSION['moci01']['search']['sql'] = $this->db->last_query();

		/* Num SQL*/
		// $ret['num'] = count($query->result());
	...
	function construct_sqla($limit = 15, $offset = 0, $func = "")
	{
		...
		/* Data SQL */
		$sql = " select DISTINCT(TA001)   FROM SFCTA where TA006='$offset' ";
	...
	function check_pnoa($pnoa)
	{
		$sql21 = " select MQ001,MQ002
			           FROM  CMSMQ
					   where MQ001='$pnoa'				   
					    ";
		...
		if (count($ret['num']) > 0) {
			return mb_convert_encoding($ret['num'][0]->MQ002, "utf-8", "big-5");
		}
		return '查無單別';
	}

	function check_sfci_no($pnoa)
	{
		$sql21 = " select MQ001,MQ002
			           FROM  CMSMQ
					   where MQ001='$pnoa' and MQ003='51'				   
					    ";
		$query = $this->db->query($sql21);
		$ret['num'] = $query->result();
	...
	function check_sfcb_no($pnoa, $pnob)
	{
		$sql21 = " select DISTINCT(TA001)   
						FROM SFCTA 
					where TA006='$pnoa'	AND TA001='$pnob'			   
					    ";
		$query = $this->db->query($sql21);
		$ret['num'] = $query->result();

		if (count($ret['num']) > 0) {
			return $ret['num'][0]->TA001;
		}
		return 'N';
	}

	function construct_sql_new_moci01($limit = 15, $offset = 0, $func = "")
	{
		$this->session->set_userdata('moci01_search', "display_search/" . $offset);
		if (@session_status() == PHP_SESSION_NONE) {
			session_start();
	...
	function insertf()
	{
		...
		$exist = $this->moci01_model->selone1($this->input->post('mq001'));
		if ($exist) {
			return 'exist';
		}
	...
	function copyf()           //複製一筆
	{
		...
		$exist = $this->moci01_model->selone2($this->input->post('mq002c'));
		if ($exist) {
			return 'exist';
		}
	...
	function printfd()
	{
		...
		$sql = " SELECT * FROM cmsmq WHERE mq001 >= '$seq1'  AND mq001 <= '$seq2' 
         AND mq003 >= '51'  AND mq003 <= '5B'		";
		$query = $this->db->query($sql);
		$ret['rows'] = $query->result();
		$seq32 = "mq001 >= '$seq1'  AND mq001 <= '$seq2' AND mq003 >= '51'  AND mq003 <= '5B'  ";
		$query = $this->db->select('COUNT(*) as count', FALSE)  //筆數查詢,如果設為FALSE不會使用反引號保護你的字段或者表名
			->from('cmsmq')
			->where($seq32);
		$num = $query->get()->result();
	...
	/*==以下AJAX處理區域==*/
	function ajaxmoci01($seg1)    //ajax 查詢一筆 顯示用 庫別6
	{
		$this->db->set('mq001', $this->uri->segment(4));
		$this->db->where('mq001', $this->uri->segment(4));
		$query = $this->db->get('cmsmq');
