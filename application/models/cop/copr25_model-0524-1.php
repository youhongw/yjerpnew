...
class Copr25_model extends CI_Model {
	...
	function printfd()          
		...
		//  $seq3=$this->input->post('dateo');   //日期 
		  ...
		  //取銷貨資料
			$sql = "select `th004`, `th005`, `th006`, `th009`,substring(tg003 ,1,6) as gp_tg003, round(sum(th008)) as th008 from copth as a 
					LEFT JOIN coptg as b on a.th002 = b.tg002 and a.th001=b.tg001
					WHERE substring(tg003 ,1,6) >= '".$seq3."'	AND substring(tg003 ,1,6) <= '".$seq4."'";
				if(@$seq5)
					$sql .= " AND `th004` >= '".$seq5."'";
				if(@$seq6)
					$sql .= " AND `th004` <= '".$seq6."'";
				$sql .= " GROUP BY th004`, `th005`, `th006`, `th009`,`gp_tg003` ORDER BY `th004` , `gp_tg003` ";
			$result = mysql_query($sql);
			$data = array();
			$row_data = array();
			$get_need = array('th004','th005','th006','th009','th008','gp_tg003');
			$total = 0;
			if(@$result){
				...
				foreach($row_data as $key => $val){
					...
					$sale_data[$val['th004']]['record'][$val['gp_tg003']] = $val['th008'];
				}
				foreach($retums_data as $key => $val){
					$sale_data[$key]['name'] = $val['name'];
					$sale_data[$key]['th006'] = $val['th006'];
					$sale_data[$key]['th009'] = $val['th009'];
		  ...
		  //取銷退資料
			$sql = "select `tj004`, `tj005`, `tj006`, `tj008`,substring(ti003 ,1,6) as gp_ti003,round(sum(tj007)) as tj007 from coptj as a
			        LEFT JOIN copti as b on a.tj002 = b.ti002 and a.tj001=b.ti001
					WHERE substring(ti003 ,1,6) >= '".$seq3."'	AND substring(ti003 ,1,6) <= '".$seq4."'";
				if(@$seq5)
					$sql .= " AND `tj004` >= '".$seq5."'";
				if(@$seq6)
					$sql .= " AND `tj004` <= '".$seq6."'";
				$sql .= " GROUP BY `tj004`, `tj005`, `tj006`, `tj008`,`gp_ti003` ORDER BY `tj004` , `gp_ti003` ";
			$result = mysql_query($sql);
			$data = array();
			$row_data = array();
			$get_need = array('tj004','tj005','tj006','tj008','tj007','gp_ti003');
			$total = 0;
			if(@$result){
				while($row = mysql_fetch_array($result)){
					foreach($get_need as $val){
				...
				foreach($row_data as $key => $val){
					...
					$retums_data[$val['tj004']]['record'][$val['gp_ti003']] = $val['tj007'];
				}
