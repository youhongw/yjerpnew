...
class copi08_model extends CI_Model {
	...
	function findf($limit, $offset, $sort_by, $sort_order)     
		...
		if(@$_SESSION['admi05_sql_sort']){$seq33 = $_SESSION['admi05_sql_sort'];}
	     ...
	     $sort_by = (in_array($sort_by, $sort_columns)) ? $sort_by : 'tg001';  //檢查排序欄位是否在 table 內
	     $query = $this->db->select('tg001, tg002, tg003,b.ma002, tg004,b.ma002 as tg004disp, tg005, tg006,tg007,tg008,tg010,tg011,tg013,tg025,tg024,tg042, a.create_date')
	                       ->from('coptg as a')
						   ->join('copma as b', 'a.tg004 = b.ma001 ','left') //客戶代號
		                   ->where($seq32)
			               ->order_by($seq33)
	...
	function insertf()    //新增一筆 檔頭  coptg
		...
		//	 if ($this->input->post('tg018')!='1') {$tg019=round($this->input->post('tg019'));}
	     ...
	     $data = array( 
	      ...
	      $exist = $this->copi08_model->selone1($this->input->post('copq03a23'),$this->input->post('tg002'));
	      if ($exist)
	         {
		      return 'exist';
		     } 
		...
		// if (!isset($_POST['order_product'][  $n  ]['th004']) ) { $n='250'; } 
	     ...
	     $exist = $this->copi08_model->selone1d($this->input->post('copq03a23'),$this->input->post('tg002'),$th003);
		...
		if ($_POST['order_product'][  $n  ]['th004']!='') {
		  ...
		  // 訂單已交數量  結案碼
			 ...
			 $th016=$_POST['order_product'][ $n  ]['th016'];
			 $th008=$_POST['order_product'][ $n  ]['th008'];
			 $existb = $this->copi08_model->selone3d($th014,$th015,$th016);
			  $data_add = array( 
	             'company' => $this->session->userdata('syscompany'),
	             'creator' => $this->session->userdata('manager'),
		         'usr_group' => 'A100',
		 ...
		 $query = $this->db->query($sql9);} 
		  ...
		  //庫存增加減少 品號,庫別, 數量
			 ...
			 $th007=$_POST['order_product'][ $n  ]['th007'];
			 $th008=$_POST['order_product'][ $n  ]['th008'];
			 $exista = $this->copi08_model->selone2d($th004,$th007);
			  $data_add = array( 
	             'company' => $this->session->userdata('syscompany'),
	             'creator' => $this->session->userdata('manager'),
		         'usr_group' => 'A100',
	...
	function updatef()   
		...
		//	while ($_POST['order_product'][  $n  ]['th004']) {
		 ...
		 $query = $this->db->query($sql);	} 
		   ...
		   //重新計算貨款 coptg
		  $sql = " UPDATE coptg set tg013='$th035',tg025='$th036',tg045='$th037',tg046='$th038',tg033='$th008b' WHERE tg001 = '$tg001'  AND tg002 = '$tg002'  "; 
		 $query = $this->db->query($sql);		   
