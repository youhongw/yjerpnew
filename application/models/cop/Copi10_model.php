...
class Copi10_model extends CI_Model {
	...
	function search($limit, $offset, $sort_by, $sort_order)     
	    { 
	     ...
	     $query = $this->db->select('a.tg001,c.mq002,c.mq002 as tg001disp, a.tg002, a.tg042, a.tg004,b.ma002 ,  a.tg013, a.tg025,a.tg024,a.create_date')
	                       ->from('coptg3 as a')
						   ->join('copma as b', 'a.tg004 = b.ma001 ','left') //客戶代號
						   ->join('cmsmq as c', 'a.tg001 = c.mq001 ','left') //單別
	...
	function selone($seq1,$seq2)    
        ...
        $this->db->from('coptg3 as a');	
        $this->db->join('copth3 as b', 'a.tg001 = b.th001  and a.tg002=b.th002 ','left');	//單身		
	...
	function findf($limit, $offset, $sort_by, $sort_order)     
         {            		
	      ...
	      $seq11 = "SELECT COUNT(*) as count  FROM `coptg3` ";
	      $seq1 = "tg001, tg002, tg003, tg004, tg005, tg006,tg007,tg13,tg025,tg042, a.create_date FROM `coptg3` ";
		...
		//下一頁不會亂跳
			if(@$_SESSION['copi10_sql_term']){$seq32 = $_SESSION['copi10_sql_term'];}
		if(@$_SESSION['copi10_sql_sort']){$seq33 = $_SESSION['copi10_sql_sort'];}
         $sort_order = (substr($sort_order,0,3) == 'asc') ? 'asc' : 'desc';
	     $sort_columns = array('tg001', 'tg002', 'tg003', 'tg004', 'tg005', 'tg006','tg007','tg008','tg013','tg025','tg042','a.create_date');
	     $sort_by = (in_array($sort_by, $sort_columns)) ? $sort_by : 'tg001';  //檢查排序欄位是否在 table 內
	     $query = $this->db->select('tg001, tg002, tg003, tg004,b.ma002 as tg004disp, tg005, tg006,tg007,tg008,tg010,tg011,tg013,tg025,tg024,tg042, a.create_date')
	                       ->from('coptg3 as a')
						   ->join('copma as b', 'a.tg004 = b.ma001 ','left') //客戶代號
		                   ->where($seq32)
			               ->order_by($seq33)
	...
	function filterf1($limit, $offset , $sort_by  , $sort_order)          
	    {    
	      ...
	      $this->db->select('a.tg001, a.tg002, a.tg042, a.tg004,b.ma002 , a.tg007, a.tg013,a.tg025,a.tg024, a.create_date');
	      $this->db->from('coptg3 as a');
	...
	//新增一筆 檔頭  coptg3	
	function insertf()    //新增一筆 檔頭  coptg3
        {
		...
		//	 if ($this->input->post('tg018')!='1') {$tg019=round($this->input->post('tg019'));}
	     ...
	     $data = array( 
	      ...
	      $exist = $this->copi10_model->selone1($this->input->post('copq03a23'),$this->input->post('tg002'));
	      if ($exist)
	         {
		      return 'exist';
		     } 
             $this->db->insert('coptg3', $data);
			
		// 新增明細 copth3
				//		$this->db->flush_cache();  
		// 新增明細 coptg3  主檔 copth3 重計算合計金額 數量 庫存數量
			    $tg013=0;$tg025=0;$tg033=0;$tg045=0;$tg046=0;$tg033b=0;	
			    $n = '0';
				$th003='1000';
		//前置單据		1050408
		IF ($this->uri->segment(3)!='copybefore') {
		if (!isset($_POST['order_product'][  $n  ]['th004']) ) { $n='0'; } } 
		
		// if (!isset($_POST['order_product'][  $n  ]['th004']) ) { $n='250'; } 
	     ...
	     $exist = $this->copi10_model->selone1d($this->input->post('copq03a23'),$this->input->post('tg002'),$th003);
		
		if ($_POST['order_product'][  $n  ]['th004']!='') {
		  $this->db->insert('copth3', $data_array); }
		  // 訂單已交數量  結案碼
		     $th014=$_POST['order_product'][ $n  ]['th014'];
			 $th015=$_POST['order_product'][ $n  ]['th015'];
			 $th016=$_POST['order_product'][ $n  ]['th016'];
			 $th008=$_POST['order_product'][ $n  ]['th008'];
			 $existb = $this->copi10_model->selone3d($th014,$th015,$th016);
			  $data_add = array( 
	             'company' => $this->session->userdata('syscompany'),
	             'creator' => $this->session->userdata('manager'),
		         'usr_group' => 'A100',
		 ...
		 $query = $this->db->query($sql);	
		  $sql9 = " UPDATE coptd set td016='Y' WHERE td001 = '$th014'  AND td002 = '$th015'  AND td003 = '$th016' AND td009>=td008 "; 
		 $query = $this->db->query($sql9);} 
		  ...
		  //庫存增加減少 品號,庫別, 數量
			 ...
			 $th007=$_POST['order_product'][ $n  ]['th007'];
			 $th008=$_POST['order_product'][ $n  ]['th008'];
			 $exista = $this->copi10_model->selone2d($th004,$th007);
			  $data_add = array( 
	             'company' => $this->session->userdata('syscompany'),
	             'creator' => $this->session->userdata('manager'),
		         'usr_group' => 'A100',
		 ...
		 //   }
			 ...
			 $n =  (string)$num;
			  ...
			  $data_array = array( 
				...
				if ($_POST['order_product'][  $n  ]['th004']>='0') {
			$this->db->insert('copth3', $data_array);}
			 $tg013=$tg013+ $_POST['order_product'][ $n  ]['th035'];
		  $tg025=$tg025+ $_POST['order_product'][ $n  ]['th036'];
		  $tg045=$tg045+ $_POST['order_product'][ $n  ]['th037'];
		  ...
		  $sql = " UPDATE coptg3 set tg013='$tg013',tg025='$tg025',tg033='$tg033',tg045='$tg045',tg046='$tg046' WHERE tg001 = '$tg001'  AND tg002 = '$tg002'  "; 
		  $query = $this->db->query($sql);	
				return true;			
		 }
	function auto_print(){
		...
		$tmp = $query->result();
		if($tmp[0]->mq016=="Y"){
			echo "<script>window.open('printbb/".$this->input->post('copq03a23')."/".$this->input->post('tg002').".html', '_blank','menubar=no,status=no,scrollbars=no,top=0,left=0,toolbar=no,width=800,height=600');</script>";
		}
	}	 
	//查複製資料是否重複	 
    ...
    function copyf()           
        {
         ...
         //   if ($query->num_rows() != 1) { return 'exist'; }
            ...
            $seq1=$this->input->post('tg001c');    //主鍵一筆檔頭coptg3
            ...
            $exist = $this->copi10_model->selone2($this->input->post('tg001c'),$this->input->post('tg002c'));
		    if ($exist)
		      {
			    return 'exist';
		      }         
             $this->db->insert('coptg3', $data);      //複製一筆  
          ...
          //  if ($query->num_rows() != 1) { return 'exist'; }
			...
			$seq1=$this->input->post('tg001c');    //主鍵一筆明細copth3
			$seq2=$this->input->post('tg002c'); 
              $i=0;
            while ($i<$num) {	
			  $data_array = array( 
             ...
             $this->db->insert('copth3', $data_array);      //複製一筆 
             $i++;			 
            }
             return true;		
	   }
	...
	function excelnewf()           
	  ...
	  //    $sql = " SELECT tg001,tg002,tg024,tg004,tg011,tg003,create_date FROM coptg3 WHERE tg001 >= '$seq1'  AND tg001 <= '$seq2' AND  tg002 >= '$seq3'  AND tg002 <= '$seq4'  "; 
         $sql = " SELECT a.tg001,a.tg002,a.tg003,a.tg004,c.ma002 as tg004disp,b.th003,b.th004,b.th005,b.th006,b.th009,b.th008,
		  b.th012,b.th013
		  FROM coptg3 as a
		  LEFT JOIN copth3 as b ON a.tg001=b.th001 and a.tg002=b.th002 
		  LEFT JOIN copma as c ON a.tg004=c.ma001 
		  WHERE  tg001 >= '$seq1'  AND tg001 <= '$seq2' AND tg002 >= '$seq3'  AND tg002 <= '$seq4'  "; 
	//	  FROM coptg3 as a, copth3 as b WHERE tg001=th001 and tg002=th002 and  tg001 >= '$seq1'  AND tg001 <= '$seq2' AND tg002 >= '$seq3'  AND tg002 <= '$seq4'  "; 
		 $query = $this->db->query($sql);
	...
	function printfd()          
	    ...
	    //  $seq4=$this->input->post('tg002c');
		  ...
		  b.th011,b.th012,b.th013,b.th016,a.tg045,a.tg046
		  FROM coptg3 as a
		  LEFT JOIN copth3 as b ON a.tg001=b.th001 and a.tg002=b.th002 
		  LEFT JOIN copma as c ON a.tg004=c.ma001 
...
order by a.tg001,a.tg002,b.th003 "; 
	...
	function printfd1()   
       {           
        ...
        $this->db->from('coptg3 as a');	
        $this->db->join('copth3 as b', 'a.tg001 = b.th001  and a.tg002=b.th002 ','left');		
	...
	function updatef()   
        {
          ...
          $data = array(			
            ...
            $this->db->where('tg001', $this->input->post('copq03a23'));
			$this->db->where('tg002', $this->input->post('tg002'));
            $this->db->update('coptg3',$data);                   //更改一筆
			
			//刪除明細
			$this->db->where('th001', $this->input->post('copq03a23'));
			$this->db->where('th002', $this->input->post('tg002'));
            $this->db->delete('copth3'); 
		...
		//	while ($_POST['order_product'][  $n  ]['th004']) {
		 ...
		 $query = $this->db->query($sql);	} 
		   ...
		   //重新計算貨款 coptg3
		  $sql = " UPDATE coptg3 set tg013='$th035',tg025='$th036',tg045='$th037',tg046='$th038',tg033='$th008b' WHERE tg001 = '$tg001'  AND tg002 = '$tg002'  "; 
		 $query = $this->db->query($sql);		   
	    ...
	    if (!empty($_POST['selected'])) 
	         {
                foreach($_POST['selected'] as $check) 
			    {
				 ...
				 if ($seq3!='Y') {
					 ...
					 //由銷貨單找客戶訂單 		  
		 $query81 = $this->db->query("SELECT th014,th015,th016   FROM copth3 as a 
	...
	$sql95 =" update coptd as c,(select th014,th015,th016,td009,td016,th008 from copth3 as b,coptd as c
			...
			$this->db->query($sql95);   $num =  (int)$i + 1;
			 $i =  (string)$num; 
			   ...
			   //庫存增加減少  (找本張出貨單刪除時庫存+加回)
			    $query82 = $this->db->query("SELECT th004,th007,th008   FROM copth3 as a 
	   ...
	   foreach ($query82->result() as $row)
			...
			while (isset($th004[$i])) {
                        ...
                        $vth007=$th007[$i];
                        $vth008=$th008[$i];
         $sql83 = " UPDATE invmc set mc007=mc007+'$vth008' WHERE mc001 = '$vth004'  AND mc002 = '$vth007'  "; 
		 $query = $this->db->query($sql83);	 
			$num =  (int)$i + 1;
			 $i =  (string)$num; 
                  ...
                  $this->db->delete('copth3'); $this->session->set_userdata('msg1',"未結帳已刪除"); }
