...
class Copi09_model extends CI_Model {
	...
	function selone1($seg1)    
        {
	      $this->db->where('ti001', $this->input->post('copq03a24'));
    ...
    function selone1d($seg1,$seg2,$seg3)    
        {
	      $this->db->where('tj001', $this->input->post('copq03a24'));
		  $this->db->where('tj002', $this->input->post('ti002'));
	...
	function insertf()    //新增一筆 檔頭  copti
		...
		//	 if ($this->input->post('ti018')!='1') {$ti019=round($this->input->post('ti019'));}
		 //營業稅率, 匯率  
		       $ti001=$this->input->post('copq03a24');
	     ...
	     $data = array( 
                 ...
                 'ti007' => $this->input->post('cmsq02a'),
                 'ti008' => $this->input->post('cmsq06a'),
                 'ti009' => $this->input->post('ti009'),
                 ...
                 'ti017' =>substr($this->input->post('ti017'),0,4).substr($this->input->post('ti017'),5,2).substr(rtrim($this->input->post('ti017')),8,2),
                 'ti018' => 'N',
                 'ti019' => $this->input->post('ti019'),
                 'ti020' => $this->input->post('ti020'),
                 'ti021' => $this->input->post('ti021'),
				 'ti022' => $this->input->post('cmsq09a31'),
                 ...
                 'ti030' => $this->input->post('cmsq09a32'),
				 'ti031' => $this->input->post('ti031'),
				 'ti032' => $this->input->post('ti032'),
		         'ti033' => substr($this->input->post('ti033'),0,4).substr($this->input->post('ti033'),5,2),
				 'ti034' => substr($this->input->post('ti034'),0,4).substr($this->input->post('ti034'),5,2).substr(rtrim($this->input->post('ti034')),8,2),
				 'ti035' => $this->input->post('cmsq09a32'),
		         'ti036' => $this->input->post('ti036'),
				 'ti037' => $this->input->post('ti037'),
				 'ti038' => $this->input->post('ti038'),
		         'ti039' => $this->input->post('cmsq21a2'),
	      ...
	      $exist = $this->copi09_model->selone1($this->input->post('copq03a24'),$this->input->post('ti002'));
	      if ($exist)
	         {
		      return 'exist';
		     } 
		...
		// 新增明細 coptj
				//		$this->db->flush_cache();  
		// 新增明細 copti  主檔 coptj 重計算合計金額 數量 庫存數量
			    $ti010=0;$ti011=0;$ti037=0;$ti038=0;$ti029=0;$ti029b=0;	
			    $n = '0';
				$tj003='1000';
		
		 if (!isset($_POST['order_product'][  $n  ]['tj004']) ) { $n='30'; } 
		...
		//	while ($_POST['order_product'][  $n  ]['tj004']) {		
		    while (isset($_POST['order_product'][  $n  ]['tj004'])) {
			   ...
			   $data_array = array( 
		         ...
		         'modi_date' => '',
		         'flag' => 0,
                 'tj001' => $this->input->post('copq03a24'),
		         'tj002' => $this->input->post('ti002'),
		         'tj003' =>  $tj003,
                 ...
                 'tj014' =>  $_POST['order_product'][ $n  ]['tj014'],
                 'tj015' =>  $_POST['order_product'][ $n  ]['tj015'],
			     'tj016' =>  $_POST['order_product'][ $n  ]['tj016'],
				 'tj017' =>  $_POST['order_product'][ $n  ]['tj017'],
				 'tj018' =>  $_POST['order_product'][ $n  ]['tj018'],
				 'tj019' =>  $_POST['order_product'][ $n  ]['tj019'],
				 'tj020' =>  $_POST['order_product'][ $n  ]['tj020'],
				 'tj023' =>  $_POST['order_product'][ $n  ]['tj023'],
				 'tj028' =>  $_POST['order_product'][ $n  ]['tj028'],
				 'tj029' =>  $_POST['order_product'][ $n  ]['tj029'],
				 'tj030' =>  $_POST['order_product'][ $n  ]['tj030'],
				
				 'tj031' =>  $_POST['order_product'][ $n  ]['tj031'],
				 'tj032' =>  $_POST['order_product'][ $n  ]['tj032'],
				 'tj033' =>  $_POST['order_product'][ $n  ]['tj033'],
				 'tj034' =>  $_POST['order_product'][ $n  ]['tj034']
				 
                );   
						 
	     $exist = $this->copi09_model->selone1d($this->input->post('copq03a24'),$this->input->post('ti002'),$tj003);
		
		if ($_POST['order_product'][  $n  ]['tj004']!='') {
		  $this->db->insert('coptj', $data_array); }
		  // 訂單已交數量
		     $tj018=$_POST['order_product'][ $n  ]['tj018'];
			 $tj019=$_POST['order_product'][ $n  ]['tj019'];
			 $tj020=$_POST['order_product'][ $n  ]['tj020'];
			 $tj007=$_POST['order_product'][ $n  ]['tj007'];
			 $existb = $this->copi09_model->selone3d($tj018,$tj019,$tj020);
			  $data_add = array( 
	             'company' => $this->session->userdata('syscompany'),
	             'creator' => $this->session->userdata('manager'),
		         'usr_group' => 'A100',
			   ...
			   if ($_POST['order_product'][  $n  ]['tj015']!='') {
			 if ($existb){			  
         $sql = " UPDATE coptd set td009=td009-'$tj007' WHERE td001 = '$tj018'  AND td002 = '$tj019'  AND td003 = '$tj020' "; 
		 $query = $this->db->query($sql);	} 
		  ...
		  //庫存增加減少 品號,庫別, 數量
			 ...
			 $tj013=$_POST['order_product'][ $n  ]['tj013'];
			 $tj007=$_POST['order_product'][ $n  ]['tj007'];
			 $exista = $this->copi09_model->selone2d($tj004,$tj013);
			  $data_add = array( 
	             'company' => $this->session->userdata('syscompany'),
	             'creator' => $this->session->userdata('manager'),
		         'usr_group' => 'A100',
         ...
         $sql = " UPDATE invmc set mc007=mc007+'$tj007' WHERE mc001 = '$tj004'  AND mc002 = '$tj013'  "; 
		 $query = $this->db->query($sql);	} 
			  }
			  
		  $ti010=$ti010+ $_POST['order_product'][ $n  ]['tj031'];
		  $ti011=$ti011+ $_POST['order_product'][ $n  ]['tj032'];
		  $ti037=$ti037+ $_POST['order_product'][ $n  ]['tj033'];
		  $ti038=$ti038+ $_POST['order_product'][ $n  ]['tj034'];
		  $ti029=$ti029+ $_POST['order_product'][ $n  ]['tj007'];
		  ...
		  if ($exist)
			{
             return 'exist';
          	...
          	//重新計算貨款 coptj  幣別, 匯率整張計算金額x匯率
			    $ti011=round($ti010*$ti036,0);
		        if ($this->input->post('ti013')=='1') {$ti010=$ti010-$ti011;}
		        if ($this->input->post('ti013')>'1') {$ti010=$ti010;}
			  $ti037=round($ti010*$ti009,0);
			  $ti038=round($ti011*$ti009,0);
		  $sql = " UPDATE copti set ti010='$ti010',ti011='$ti011',ti037='$ti037',ti038='$ti038',ti029='$ti029' WHERE ti001 = '$ti001'  AND ti002 = '$ti002'  "; 
		  $query = $this->db->query($sql);	
				return true;			
		 }
		 function auto_print(){
		...
		$this->db->from('cmsmq');
		$this->db->where("mq001",$this->input->post('copq03a24'));	
		$query = $this->db->get();
		$tmp = $query->result();
		if($tmp[0]->mq016=="Y"){
			echo "<script>window.open('printbb/".$this->input->post('copq03a24')."/".$this->input->post('ti002').".html', '_blank','menubar=no,status=no,scrollbars=no,top=0,left=0,toolbar=no,width=800,height=600');</script>";
		}
	}	 
	//查複製資料是否重複	 
	...
	function updatef()   
		...
		//	$this->db->flush_cache();  
			// 新增明細 coptj b原數量
			    $tj035=0;$tj031=0;$tj032=0;$tj033=0;$tj034=0;$tj007b=0;
			    $n = '0';		
				$tj003='1000';
		//	while ($_POST['order_product'][  $n  ]['tj004']) {
			while (isset($_POST['order_product'][  $n  ]['tj004'])) {
             ...
             //    'tj007' => $_POST['order_product'][ $n  ]['tj007'],
				 ...
				 if ($_POST['order_product'][  $n  ]['tj004']!='') {
		     $this->db->insert('coptj', $data_array); }
			  $tj031=$tj031+$_POST['order_product'][ $n  ]['tj031'];
			  $tj032=$tj032+$_POST['order_product'][ $n  ]['tj032'];
			  $tj033=$tj033+$_POST['order_product'][ $n  ]['tj033'];
			  $tj034=$tj034+$_POST['order_product'][ $n  ]['tj034'];
			  $tj007b=$tj007b+$_POST['order_product'][ $n  ]['tj007'];
			 ...
			 $tj020=$_POST['order_product'][ $n  ]['tj020'];
			 $tj007=$_POST['order_product'][ $n  ]['tj007'];
			 $tj007a=$_POST['order_product'][ $n  ]['tj007a'];
			 $existb = $this->copi09_model->selone3d($tj018,$tj019,$tj020);
			  $data_add = array( 
	             'company' => $this->session->userdata('syscompany'),
	             'creator' => $this->session->userdata('manager'),
			   ...
			   if ($_POST['order_product'][  $n  ]['tj018']!='') {
			 if ($existb){			  
         $sql = " UPDATE coptd set td009=td009+'$tj007'-'$tj007a' WHERE td001 = '$tj018'  AND td002 = '$tj019'  AND td003 = '$tj020' "; 
		 $query = $this->db->query($sql);	} 
			  }
			 //庫存增加減少
			 $tj004=$_POST['order_product'][ $n  ]['tj004'];
			 ...
			 $tj007=$_POST['order_product'][ $n  ]['tj007'];
			 $tj007a=$_POST['order_product'][ $n  ]['tj007a'];
			 $exista = $this->copi09_model->selone2d($tj004,$tj008);
			  $data_add = array( 
	             'company' => $this->session->userdata('syscompany'),
	             'creator' => $this->session->userdata('manager'),
		         'usr_group' => 'A100',
         ...
         $sql = " UPDATE invmc set mc007=mc007+'$tj007'-'$tj007a' WHERE mc001 = '$tj004'  AND mc002 = '$tj008'  "; 
		 $query = $this->db->query($sql);	} 
			  }
		...
		//	 while ($_POST['order_product'][  $n  ]['tj004']) {
		   ...
		   //重新計算貨款 copti
		  $sql = " UPDATE copti set ti010='$tj031',ti011='$tj032',ti037='$tj033',ti038='$tj034',ti029='$tj007b' WHERE ti001 = '$ti001'  AND ti002 = '$ti002'  "; 
		 $query = $this->db->query($sql);		   
