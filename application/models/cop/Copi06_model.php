...
class Copi06_model extends CI_Model {
	...
	function construct_sql($limit = 15, $offset = 0, $func = "")
	  {
		...
		/* Data SQL */
		$query = $this->db->select('a.tc001,c.mq002, a.tc002, a.tc003, a.tc004, b.ma002,a.tc006, a.tc029, a.tc030,a.create_date')
	                       ->from('coptc as a')
						   ->join('copma as b', 'a.tc004 = b.ma001','left')
	...
	function findf($limit, $offset, $sort_by, $sort_order)     
		...
		if(@$_SESSION['copi06_sql_sort']){$seq33 = $_SESSION['copi06_sql_sort'];}
	     ...
	     $sort_by = (in_array($sort_by, $sort_columns)) ? $sort_by : 'tc001';  //檢查排序欄位是否在 table 內
	     $query = $this->db->select('tc001,mq002 as tc001disp,mq002, tc002,b.ma002, tc003, tc004,tc004 as tc004disp, tc005, tc006,tc007,tc008,tc010,tc011,tc012,tc029,tc030, a.create_date')
	                       ->from('coptc as a')
						   ->join('copma as b', 'a.tc004 = b.ma001','left')
						   ->join('cmsmq as c', 'a.tc001 = c.mq001','left')
		                   ->where($seq32)
	...
	//複製一筆	
    function copyf($tc001o,$tc002o,$tc001c,$tc039)           
        {	
			$user = $this->session->userdata('sysuser');
			
			$sql_chk = "SELECT COUNT(*) AS NUM FROM COPTC WHERE TC001 = '$tc001o' AND TC002 = '$tc002o'";
			$query_chk = $this->db->query($sql_chk);
			$num = $query_chk->result();
			$NUM1 = $num[0] -> NUM; //輸入的單號是否存在
			$tc039_d = substr($tc039,0,4)-1911 .substr($tc039,5,2).substr($tc039,8,2);
			$tc039_1 = substr($tc039,0,4) .substr($tc039,5,2).substr($tc039,8,2);
			
			$sql = "
			SELECT CASE  WHEN MAX(TC002) IS NULL THEN '$tc039_d' + '001' ELSE MAX(TC002)+ 1 END AS TC002

		    FROM COPTC WHERE TC001 = '$tc001c' AND CREATE_DATE > '20210101' AND TC002 LIKE '$tc039_d%'";
			
			//echo var_dump($sql);exit;

			$query = $this->db->query($sql);
			$tc002 = $query->result();
			$TC002_N = $tc002[0] -> TC002; //取出最大單號+1
			
			//echo var_dump($TC002_N);exit;
			
			$sql1 = "INSERT INTO COPTC (COMPANY,CREATOR,USR_GROUP,CREATE_DATE,MODIFIER,MODI_DATE,FLAG,TC001,TC002,TC003,TC004,TC005,TC006,TC007,TC008,TC009,TC010,TC011,TC012,TC013,TC014,TC015,TC016,TC017,TC018,TC019,TC020,TC021,TC022,TC023,TC024,TC025,TC026,TC027,TC028,TC029,TC030,TC031,TC032,TC033,TC034,TC035,TC036,TC037,TC038,TC039,TC040,TC041,TC042,TC043,TC044,TC045,TC046,TC047,TC048,TC049,TC050,TC051)
			SELECT COMPANY,'$user','A100',convert(varchar, getdate(), 112),'','','1','$tc001c','$TC002_N','$tc039_1',TC004,TC005,TC006,TC007,TC008,TC009,TC010,TC011,TC012,TC013,TC014,TC015,TC016,TC017,TC018,TC019,TC020,TC021,TC022,TC023,TC024,TC025,TC026,'N','0',TC029,TC030,TC031,TC032,TC033,TC034,TC035,TC036,TC037,TC038,'$tc039_1','',TC041,TC042,TC043,TC044,TC045,TC046,TC047,TC048,TC049,TC050,TC051 FROM COPTC WHERE TC001 = '$tc001o' AND TC002 = '$tc002o'
			";
			

			$query1 = $this->db->query($sql1);
			
			$sql2 = "INSERT INTO COPTD (COMPANY,CREATOR,USR_GROUP,CREATE_DATE,MODIFIER,MODI_DATE,FLAG,TD001,TD002,TD003,TD004,TD005,TD006,TD007,TD008,TD009,TD010,TD011,TD012,TD013,TD014,TD015,TD016,TD017,TD018,TD019,TD020,TD021,TD022,TD023,TD024,TD025,TD026,TD027,TD028,TD029,TD030,TD031,TD032,TD033,TD034,TD035,TD036)
			SELECT COMPANY,'$user','A100',convert(varchar, getdate(), 112),'','','1','$tc001c','$TC002_N',TD003,TD004,TD005,TD006,TD007,TD008,TD009,TD010,TD011,TD012,TD013,TD014,TD015,'N',TD017,TD018,TD019,TD020,'N',TD022,TD023,TD024,TD025,TD026,TD027,TD028,TD029,TD030,TD031,TD032,TD033,TD034,TD035,TD036 FROM COPTD WHERE TD001 = '$tc001o' AND TD002 = '$tc002o'
			";
