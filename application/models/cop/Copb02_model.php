...
class Copb02_model extends CI_Model {
	...
	function search($limit, $offset, $sort_by, $sort_order)     
	    { 
	     ...
	     $sort_columns = array('td001', 'td002', 'td003', 'td004', 'td011', 'td019','td020','create_date');
	     $sort_by = (in_array($sort_by, $sort_columns)) ? $sort_by : 'td001';  //檢查排序欄位是否在 table 內
	     $query = $this->db->select('td001, td002, td003, td004,td005,td006, td007,td008,td009,td013,td016,create_date')
	                       ->from('coptd')
	...
	function selone($seq1,$seq2)    
        ...
        $this->db->join('coptd as b', 'a.td001 = b.td001  and a.td002=b.td002 ','left');	//單身	
	...
	function findf($limit, $offset, $sort_by, $sort_order)     
        ...
        if ($seq7!='') {$seq9=" ORDER BY ".$seq7;$seq33=$seq7;}
		 //下一頁不會亂跳
		if(@$_SESSION['copb02_sql_term']){$seq32 = $_SESSION['copb02_sql_term'];}
		if(@$_SESSION['copb02_sql_sort']){$seq33 = $_SESSION['copb02_sql_sort'];}
		
         $sort_order = (substr($sort_order,0,3) == 'asc') ? 'asc' : 'desc';
	     $sort_columns = array('td001', 'td002', 'td003', 'td004', 'td005', 'td006','td007','td008','td010','td011','td009','td013','td016','create_date');
	     $sort_by = (in_array($sort_by, $sort_columns)) ? $sort_by : 'td001';  //檢查排序欄位是否在 table 內
	     $query = $this->db->select('td001, td002, td003, td004, td005, td006,td007,td008,td010,td011,td009,td013,td016, create_date')
	                       ->from('coptd')
		                   ->where($seq32)
			               ->order_by($seq33)
			              //->order_by($sort_by, $sort_order)
	...
	function filterf1($limit, $offset , $sort_by  , $sort_order)          
	    {    
	      ...
	      $sort_order = (substr($sort_order,0,3) == 'asc') ? 'asc' : 'desc';
	      $sort_columns = array('td001', 'td002', 'td003', 'td004', 'td005', 'td006','td007','td008','td009','td013','td016','create_date');
          $sort_by = (in_array($sort_by, $sort_columns)) ? $sort_by : 'td001';  //檢查排序欄位是否為 table
	      $this->db->select('a.td001, a.td002, a.td003, a.td004, a.td005,a.td006,a.td007,a.td008,a.td009,a.td013,a.td016, a.create_date');
	      $this->db->from('coptd as a');
	...
	function insertf()    //新增一筆 檔頭  coptc
		...
		//	 if ($this->input->post('td018')!='1') {$td019=round($this->input->post('td019'));}
	     ...
	     $data = array( 
	      ...
	      $exist = $this->copb02_model->selone1($this->input->post('copq03a22'),$this->input->post('td002'));
	      if ($exist)
	         {
		      return 'exist';
		     } 
		...
		//	while ($_POST['order_product'][  $n  ]['td004']) {
	      ...
	      $exist = $this->copb02_model->selone1d($this->input->post('copq03a22'),$this->input->post('td002'),$td003);
    ...
    function copyf()           
        {
         ...
         //   if ($query->num_rows() != 1) { return 'exist'; }
            ...
            $exist = $this->copb02_model->selone2($this->input->post('td001c'),$this->input->post('td002c'));
		    if ($exist)
		      {
			    return 'exist';
		      }         
          ...
          //  if ($query->num_rows() != 1) { return 'exist'; }
			...
			$seq1=$this->input->post('td001c');    //主鍵一筆明細coptd
			$seq2=$this->input->post('td002c'); 
              $i=0;
            while ($i<$num) {	
			  $data_array = array( 
	...
	function excelnewf()           
        {			
		  ...
		  FROM coptc as a,coptd as b,copma as c WHERE td001=td001 and td002=td002 and td004=ma001 and td001 >= '$seq1'  AND td001 <= '$seq2' AND  td002 >= '$seq3'  AND td002 <= '$seq4'  "; 
          $query = $this->db->query($sql);
	...
	function printfd1()   
       {           
        ...
        $this->db->from('coptc as a');	
        $this->db->join('coptd as b', 'a.td001 = b.td001  and a.td002=b.td002 ','left');		
