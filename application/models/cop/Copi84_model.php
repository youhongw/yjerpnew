...
class copi84_model extends CI_Model {
	...
	function findf($limit, $offset, $sort_by, $sort_order)     
       {            		
         ...
         if ($seq7!='') {$seq9=" ORDER BY ".$seq7;$seq33=$seq7;}
		  //下一頁不會亂跳
		if(@$_SESSION['copi84_sql_term']){$seq32 = $_SESSION['copi84_sql_term'];}
		if(@$_SESSION['copi84_sql_sort']){$seq33 = $_SESSION['copi84_sql_sort'];}
	...
	function filterf1($limit, $offset , $sort_by  , $sort_order)    //篩選多筆        
	   {    
	    ...
	    $sort_order = (substr($sort_order,0,3) == 'asc') ? 'asc' : 'desc';
	    $sort_columns = array('mm001', 'mm002', 'mm003', 'mm004', 'mm005', 'mm006','mm007','create_date');
	...
	function insertf()   
       {
	    ...
	    $exist = $this->copi84_model->selone1($this->input->post('mm001'));
	    if ($exist)
	      {
		    return 'exist';
		  } 
    ...
    function copyf()           //複製一筆
       {
	    ...
	    $data = array( 
            ...
            $exist = $this->copi84_model->selone2($this->input->post('mm002c'));
		    if ($exist)
		      {
			   return 'exist';
		      }         
