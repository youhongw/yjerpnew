...
class Copr26_model extends CI_Model {
	...
	function printfd()          
		...
		//  $seq3=$this->input->post('dateo');   //日期 
		  ...
		  //取銷貨資料
			$sql = "select b.`tg004`, c.`ma002`, substring(b.tg042 ,1,6) as gp_tg003, round(sum(th013)) as th013 from copth as a 
				LEFT JOIN coptg as b on a.th002 = b.tg002 and a.th001=b.tg001
				LEFT JOIN copma as c on b.tg004 = c.ma001
					WHERE substring(b.tg042 ,1,6) >= '".$seq3."' AND substring(b.tg042 ,1,6) <= '".$seq4."'";
			if(@$seq5){
				$sql .= "AND b.`tg004` >= '".$seq5."'";
			}
			if(@$seq6){
				$sql .= " AND b.`tg004` <= '".$seq6."'";
			}
			$sql .= " GROUP BY `tg004`,`ma002`,`gp_tg003` ORDER BY b.`tg004` , `gp_tg003` ";
			$result = mysql_query($sql);
			$data = array();
			$row_data = array();
			$get_need = array('tg004','ma002','gp_tg003','th013');
			$total = 0;
			if(@$result){
				...
				foreach($row_data as $key => $val){
					if (array_key_exists($val['tg004'], $sale_data)) {
						$sale_item_count ++;
					}
					$sale_data[$val['tg004']]['name'] = $val['ma002'];
					$sale_data[$val['tg004']]['record'][$val['gp_tg003']] = $val['th013'];
				}
			}else{
				$sale_data = Array();$sale_item_count = 0;
			}
		  //取銷退資料
			$sql = "select b.`ti004`, c.`ma002`,substring(ti034 ,1,6) as gp_ti003, round(sum(tj012)) as tj012  
			    from coptj as a 
				LEFT JOIN copti as b on a.tj002 = b.ti002 and a.tj001=b.ti001
				LEFT JOIN copma as c on b.ti004 = c.ma001
					WHERE substring(b.ti034 ,1,6) >= '".$seq3."'	AND substring(b.ti034 ,1,6) <= '".$seq4."'";
				if(@$seq5)
					$sql .= " AND b.`ti004` >= '".$seq5."'";
				if(@$seq6)
					$sql .= " AND b.`ti004` <= '".$seq6."'";
				$sql .= " GROUP BY `ti004`,`ma002`,`gp_ti003` ORDER BY b.`ti004` , `gp_ti003` ";
			$result = mysql_query($sql);
			$data = array();
			$row_data = array();
			$get_need = array('ti004','ma002','gp_ti003','tj012');
			$total = 0;
			if(@$result){
				while($row = mysql_fetch_array($result)){
					foreach($get_need as $val){
				...
				foreach($row_data as $key => $val){
					if (array_key_exists($val['ti004'], $retums_data)) {
						$retums_item_count ++;
					}
					$retums_data[$val['ti004']]['name'] = $val['ma002'];
					$retums_data[$val['ti004']]['record'][$val['gp_ti003']] = $val['tj012'];
				}
