...
class Copr25_model extends CI_Model {
	...
	function selbrowse($num,$offset)   
        {            
	      $this->db->select('tg001, tg002, tg042, tg004, tg0011, tg0019,tg020, create_date');
          $this->db->from('coptg');
	...
	function selone($seq1,$seq2)    
        {
		  ...
		  b.th030, b.th035, b.th036, b.th037, b.th008,  b.th033,i.mc002 as th007disp');
	...
	function findf($limit, $offset, $sort_by, $sort_order)     
         {            		
	      ...
	      $seq1 = "tg001, tg002, tg042, tg004, tg005, tg006,tg007,tg13,tg025,tg042, create_date FROM `coptg` ";
        ...
        if ($seq7!='') {$seq9=" ORDER BY ".$seq7;$seq33=$seq7;}
         $sort_order = (substr($sort_order,0,3) == 'asc') ? 'asc' : 'desc';
	     $sort_columns = array('tg001', 'tg002', 'tg042', 'tg004', 'tg005', 'tg006','tg007','tg008','tg013','tg025','tg042','create_date');
	     $sort_by = (in_array($sort_by, $sort_columns)) ? $sort_by : 'tg001';  //檢查排序欄位是否在 table 內
	     $query = $this->db->select('tg001, tg002, tg042, tg004, tg005, tg006,tg007,tg008,tg010,tg011,tg013,tg025,tg042, create_date')
	                       ->from('coptg')
		                   ->where($seq32)
			               ->order_by($seq33)
			              //->order_by($sort_by, $sort_order)
	...
	function filterf1($limit, $offset , $sort_by  , $sort_order)          
	    {    
	      ...
	      $sort_order = (substr($sort_order,0,3) == 'asc') ? 'asc' : 'desc';
	      $sort_columns = array('tg001', 'tg002', 'tg042', 'tg005', 'tg021', 'tg031','tg019','create_date');
          $sort_by = (in_array($sort_by, $sort_columns)) ? $sort_by : 'tg001';  //檢查排序欄位是否為 table
	      $this->db->select('tg001, tg002, tg042, tg005, tg021, tg031,tg019, create_date');
	      $this->db->from('coptg');
	...
	function insertf()    //新增一筆 檔頭  coptg
		...
		//	 if ($this->input->post('tg018')!='1') {$tg019=round($this->input->post('tg019'));}
	     ...
	     $data = array( 
	      ...
	      $exist = $this->posr03_model->selone1($this->input->post('copq03a23'),$this->input->post('tg002'));
	      if ($exist)
	         {
		      return 'exist';
		     } 
		...
		//	while ($_POST['order_product'][  $n  ]['th004']) {		
	     ...
	     $exist = $this->posr03_model->selone1d($this->input->post('copq03a23'),$this->input->post('tg002'),$th003);
		...
		if ($_POST['order_product'][  $n  ]['th004']!='') {
		  ...
		  // 訂單已交數量
			 ...
			 $th016=$_POST['order_product'][ $n  ]['th016'];
			 $th008=$_POST['order_product'][ $n  ]['th008'];
			 $existb = $this->posr03_model->selone3d($th014,$th015,$th016);
			  $data_add = array( 
	             'company' => $this->session->userdata('syscompany'),
	             'creator' => $this->session->userdata('manager'),
		         'usr_group' => 'A100',
		 ...
		 $query = $this->db->query($sql);	} 
		  ...
		  //庫存增加減少 品號,庫別, 數量
			 ...
			 $th007=$_POST['order_product'][ $n  ]['th007'];
			 $th008=$_POST['order_product'][ $n  ]['th008'];
			 $exista = $this->posr03_model->selone2d($th004,$th007);
			  $data_add = array( 
	             'company' => $this->session->userdata('syscompany'),
	             'creator' => $this->session->userdata('manager'),
		         'usr_group' => 'A100',
		 ...
		 $query = $this->db->query($sql);	} 
		  ...
		  $tg046=$tg046+ $_POST['order_product'][ $n  ]['th008'];
    ...
    function copyf()           
        {
         ...
         //   if ($query->num_rows() != 1) { return 'exist'; }
		    if ($query->num_rows() == 1) 
		       {
                ...
                $tg042=$row->tg042;$tg004=$row->tg004;$tg005=$row->tg005;$tg006=$row->tg006;$tg007=$row->tg007;$tg008=$row->tg008;$tg009=$row->tg009;$tg010=$row->tg010;
	        ...
	        $data = array( 
		           ...
		           'modi_date' => ' ',
		           'flag' => 0,
		           'tg001' => $seq1,'tg002' => $seq2,'tg042' => $tg042,'tg004' => $tg004,'tg005' => $tg005,'tg006' => $tg006,'tg007' => $tg007,'tg008' => $tg008,'tg009' => $tg009,'tg010' => $tg010,
            ...
            $exist = $this->posr03_model->selone2($this->input->post('tg001c'),$this->input->post('tg002c'));
		    if ($exist)
		      {
			    return 'exist';
		      }         
          ...
          //  if ($query->num_rows() != 1) { return 'exist'; }
		    if ($query->num_rows() >= 1) 
		       {
				 ...
				 $th008[$i]=$row->th008;$th039[$i]=$row->th039;$th040[$i]=$row->th040;$th041[$i]=$row->th041;
				 $i++;
			    endforeach;
		       }   
            ...
            while ($i<$num) {	
			  $data_array = array( 
			    ...
			    'th033' => $th033[$i],'th034' => $th034[$i],'th035' => $th035[$i],'th036' => $th036[$i],'th037' => $th037[$i],'th008' => $th008[$i],'th039' => $th039[$i],
	...
	function excelnewf()           
        {			
          ...
          $sql = " SELECT a.tg001,a.tg002,a.tg042,a.tg004,c.ma002 as tg004disp,b.th003,b.th004,b.th005,b.th006,b.th009,b.th008,
		  b.th012,b.th013
		  FROM postb as a
	...
	function printfd()          
		...
		//  $seq3=$this->input->post('dateo');   //日期 
		  ...
		  //取銷貨資料
			$sql = "select `th004`, `th005`, `th006`, `th009`,substring(tg042 ,1,6) as gp_th002, round(sum(th008)) as th008 
			        from copth as a 
				    LEFT JOIN coptg as b on a.th002 = b.tg002 and a.th001=b.tg001 
					WHERE  substring(tg042 ,1,6) >= '".$seq3."'	AND substring(tg042 ,1,6) <= '".$seq4."'";
				if(@$seq5)
					$sql .= " AND `th004` >= '".$seq5."'";
				if(@$seq6)
					$sql .= " AND `th004` <= '".$seq6."'";
				$sql .= " GROUP BY `th004`, `th005`, `th006`, `th009`,`gp_th002` ORDER BY `th004` , `gp_th002` ";
			//$result = mysql_query($sql);
			$result = $this->db->query($sql) ;
			$data = array();
			$row_data = array();
			$get_need = array('th004','th005','th006','th009','gp_th002','th008');
			$total = 0;
			if(@$result){
			//	while($row = mysql_fetch_array($result)){
				...
				// echo "<pre>";var_dump($row_data);exit;
				
				foreach($row_data as $key => $val){
					if (array_key_exists($val['th004'], $sale_data)) {
						$sale_item_count ++;
					}
					$sale_data[$val['th004']]['name'] = $val['th005'];
					$sale_data[$val['th004']]['th006'] = $val['th006'];
					$sale_data[$val['th004']]['th009'] = $val['th009'];
					$sale_data[$val['th004']]['record'][$val['gp_th002']] = $val['th008'];
				}
			}else{
				$sale_data = Array();$sale_item_count = 0;
			}
		  //取銷退資料
			$sql = "select `tj004`, `tj005`, `tj006`, `tj008`, substring(ti034 ,1,6) as gp_tj002, sum(tj034) as tj034 
			        from coptj as a
                    LEFT JOIN copti as b on a.tj002 = b.ti002 and a.tj001=b.ti001 					
					WHERE substring(ti034 ,1,6) >= '".$seq3."'	AND substring(ti034 ,1,6) <= '".$seq4."'";
				if(@$seq5)
					$sql .= " AND `tj004` >= '".$seq5."'";
				if(@$seq6)
					$sql .= " AND `tj004` <= '".$seq6."'";
				$sql .= " GROUP BY `tj004`, `tj005`, `tj006`, `tj008`,`gp_tj002` ORDER BY `tj004` , `gp_tj002` ";
			//$result = mysql_query($sql);
			$result = $this->db->query($sql) ;
			$data = array();
			$row_data = array();
			$get_need = array('tj004','tj005','tj006','tj008','gp_tj002','tj034');
			$total = 0;
			if(@$result){
				//while($row = mysql_fetch_array($result)){
				foreach ($result->result_array() as $row) {		
				...
				foreach($row_data as $key => $val){
					if (array_key_exists($val['tj004'], $retums_data)) {
						$retums_item_count ++;
					}
					$retums_data[$val['tj004']]['name'] = $val['tj005'];
					$retums_data[$val['tj004']]['tj006'] = $val['tj006'];
					$retums_data[$val['tj004']]['tj008'] = $val['tj008'];
					$retums_data[$val['tj004']]['record'][$val['gp_tj002']] = $val['tj034'];
				}
			  
				foreach($retums_data as $key => $val){
					$sale_data[$key]['name'] = $val['name'];
					$sale_data[$key]['th006'] = $val['tj006'];
					$sale_data[$key]['th009'] = $val['tj008'];
	...
	function updatef()   
		...
		//	$this->db->flush_cache();  
			// 新增明細 copth b原數量
			    $th035=0;$th036=0;$th037=0;$th008=0;$th008b=0;
			    $n = '0';		
				$th003='1000';
		//	while ($_POST['order_product'][  $n  ]['th004']) {
			while (isset($_POST['order_product'][  $n  ]['th004'])) {
		     ...
		     $this->db->insert('copth', $data_array); }
			  ...
			  $th008=$th008+$_POST['order_product'][ $n  ]['th008'];
		...
		//	 while ($_POST['order_product'][  $n  ]['th004']) {
		   ...
		   //重新計算貨款 coptg
		  $sql = " UPDATE coptg set tg013='$th035',tg025='$th036',tg031='$th045',tg046='$th008',tg033='$th008b' WHERE tg001 = '$tg001'  AND tg002 = '$tg002'  "; 
		 $query = $this->db->query($sql);		   
