...
class Copr20_model extends CI_Model {
	...
	function insertf()    //新增一筆 檔頭  coptg
		...
		//	 if ($this->input->post('tg018')!='1') {$tg019=round($this->input->post('tg019'));}
	     ...
	     $data = array( 
	      ...
	      $exist = $this->copr20_model->selone1($this->input->post('copq03a23'),$this->input->post('tg002'));
	      if ($exist)
	         {
		      return 'exist';
		     } 
		...
		//	while ($_POST['order_product'][  $n  ]['th004']) {		
	     ...
	     $exist = $this->copr20_model->selone1d($this->input->post('copq03a23'),$this->input->post('tg002'),$th003);
		...
		if ($_POST['order_product'][  $n  ]['th004']!='') {
		  ...
		  // 訂單已交數量
			 ...
			 $th016=$_POST['order_product'][ $n  ]['th016'];
			 $th008=$_POST['order_product'][ $n  ]['th008'];
			 $existb = $this->copr20_model->selone3d($th014,$th015,$th016);
			  $data_add = array( 
	             'company' => $this->session->userdata('syscompany'),
	             'creator' => $this->session->userdata('manager'),
		         'usr_group' => 'A100',
		 ...
		 $query = $this->db->query($sql);	} 
		  ...
		  //庫存增加減少 品號,庫別, 數量
			 ...
			 $th007=$_POST['order_product'][ $n  ]['th007'];
			 $th008=$_POST['order_product'][ $n  ]['th008'];
			 $exista = $this->copr20_model->selone2d($th004,$th007);
			  $data_add = array( 
	             'company' => $this->session->userdata('syscompany'),
	             'creator' => $this->session->userdata('manager'),
		         'usr_group' => 'A100',
    ...
    function copyf()           
        {
         ...
         //   if ($query->num_rows() != 1) { return 'exist'; }
            ...
            $exist = $this->copr20_model->selone2($this->input->post('tg001c'),$this->input->post('tg002c'));
		    if ($exist)
		      {
			    return 'exist';
		      }         
	...
	function printfd()          
		...
		/*   $this->db->select('a.* ,c.mq002 AS tc001disp, d.mb002 AS tc007disp,e.mf002 AS tc008disp, f.mv002 AS tc006disp,g.na003 AS tc014disp,
		  ...
		  $this->db->select('a.tc003,a.tc004,h.ma002 AS tc004disp,b.td004, b.td005,b.td006,b.td010,sum(b.td008) as td008,sum(b.td009) as td009,sum(b.td012) as td012 '); 
        $this->db->from('coptc as a');	
        $this->db->join('coptd as b', 'a.tc001 = b.td001  and a.tc002=b.td002 ','left');	//單身	
		$this->db->join('cmsmq as c', 'a.tc001 = c.mq001 and c.mq003="22" ','left');  //單別
	    $this->db->join('cmsmb as d', 'a.tc007 = d.mb001 ','left');    //廠別
		...
		$this->db->where('b.td004 >=', $seq5); 
	    $this->db->where('b.td004 <=', $seq6); 
		$this->db->group_by(array("tc004", "tc004disp", "td004", "td005", "td006", "td010")); 
		$this->db->order_by('tc004 , td004');
