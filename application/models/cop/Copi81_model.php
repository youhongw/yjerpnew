...
class Copi81_model extends CI_Model {
	...
	function selbrowse($num,$offset)   
        {            
	      $this->db->select('ma001, ma002, ma003, ma004, ma005, ma006,ma028,ma009,ma011,ma013, create_date');
          $this->db->from('copma1');
	...
	function search($limit, $offset, $sort_by, $sort_order)     //欄位表頭排序流覽資料
	    { 
	     ...
	     $query = $this->db->select('ma001, ma002, ma003, ma004, ma005, ma006, ma028, ma009, ma011, ma013,create_date')
	                       ->from('copma1')
	...
	function selone($seg1)    
       {
		$this->db->select('a.*,r.mf002 as ma014disp, s.me002 as ma015disp,t.mv002 as ma016disp,b.mr003 as ma017disp,c.mr003 as ma018disp
							 ...
							 ,x.mo006 as ma069disp,w.mo006 as ma070disp, n.mr003 as ma076disp, o.mr003 as ma077disp, p.mr003 as ma078disp, q.mv002 as ma085disp');
        $this->db->from('copma1 as a');
	//	$this->db->where('ma001', $this->uri->segment(4)); 
		...
		$this->db->join('copma1 as i', 'a.ma056 = i.ma001','left');
		$this->db->join('purma as j', 'a.ma057 = j.ma001','left');
		$this->db->join('purma as k', 'a.ma058 = k.ma001','left');
		$this->db->join('copma1 as l', 'a.ma065 = l.ma001','left');
	...
	function findf($limit, $offset, $sort_by, $sort_order)     
        {            		
	     //$seq5='';$seq51='';$seq7='';$seq71='';		  
	      $seq11 = "SELECT COUNT(*) as count  FROM `copma1` ";
	      $seq1 = "ma001, ma002, ma003, ma004, ma005, ma006,ma007,ma08,ma009,ma011,ma013, create_date FROM `copma1` ";
        ...
        if ($seq7!='') {$seq9=" ORDER BY ".$seq7;$seq33=$seq7;}
         $sort_order = (substr($sort_order,0,3) == 'asc') ? 'asc' : 'desc';
	     $sort_columns = array('ma001', 'ma002', 'ma003', 'ma004', 'ma005', 'ma006','ma007','ma028','ma009','ma011','ma013','create_date');
	     $sort_by = (in_array($sort_by, $sort_columns)) ? $sort_by : 'ma001';  //檢查排序欄位是否在 table 內
	     $query = $this->db->select('ma001, ma002, ma003, ma004, ma005, ma006,ma007,ma028,ma009,ma011,ma013, create_date')
	                       ->from('copma1')
		                   ->where($seq32)
			               ->order_by($seq33)
			              //->order_by($sort_by, $sort_order)
	...
	function insertf()    
        {
	      ...
	      $exist = $this->copi81_model->selone1($this->input->post('ma001'));
	      if ($exist)
	         {
		      return 'exist';
		     } 
            return  $this->db->insert('copma1', $data);
        }
    ...
    function copyf()           
          {
		    ...
		    if ($query->num_rows() == 1) 
		       {
			    ...
			    $ma091=$row->ma091;$ma092=$row->ma092;$ma093=$row->ma093;$ma094=$row->ma094;$ma200=$row->ma200;$ma201=$row->ma201;
			endforeach;
		       }   
	        ...
	        $data = array( 
                   ...
                   'ma091' => $ma091,'ma092' => $ma092,'ma093' => $ma093,'ma094' => $ma094,'ma200' => $ma200,'ma201' => $ma201
				   );
            $exist = $this->copi81_model->selone2($this->input->post('ma001c'));
		    if ($exist)
		      {
			    return 'exist';
		      }         
            return $this->db->insert('copma1', $data);      //複製一筆  
        }	
	...
	function excelnewf()           
        {			
		  ...
		  $seq5=$this->input->post('ma003o');    
	      $seq6=$this->input->post('ma003c');
	      $sql = " SELECT ma001,ma002,ma006,ma016,ma028,ma005,create_date FROM copma1 WHERE ma001 >= '$seq1'  AND ma001 <= '$seq2'
          AND ma016 >= '$seq3'  AND ma016 <= '$seq4' AND ma028 >= '$seq5'  AND ma028 <= '$seq6'		  ORDER BY ma001 "; 
	...
	function printfd()           
        {
	      ...
	      $seq6=$this->input->post('ma003c');
		  $seq7=$this->input->post('ma004o');    //查詢一筆以上
	      $seq8=$this->input->post('ma004c');
	      $sql = " SELECT * FROM copma1 WHERE ma001 >= '$seq1'  AND ma001 <= '$seq2' AND ma016 >= '$seq3'  AND ma016 <= '$seq4' AND ma028 >= '$seq5'  AND ma028 <= '$seq6' AND ma201 >= '$seq7'  AND ma201 <= '$seq8'  "; 
          $query = $this->db->query($sql);
	      $ret['rows'] = $query->result();
          $seq32 = "ma001 >= '$seq1'  AND ma001 <= '$seq2' AND ma016 >= '$seq3'  AND ma016 <= '$seq4' AND ma028 >= '$seq5'  AND ma028 <= '$seq6'  AND ma201 >= '$seq7'  AND ma201 <= '$seq8'  ";	
	      $query = $this->db->select('COUNT(*) as count', FALSE)  //筆數查詢,如果設為FALSE不會使用反引號保護你的字段或者表名
		                ->from('copma1')
		                ->where($seq32);
	      $num = $query->get()->result();		
	...
	function updatef()   
        {
            ...
            $this->db->where('ma001', $this->input->post('ma001'));
            $this->db->update('copma1',$data);                   //更改一筆
