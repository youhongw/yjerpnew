...
class Mymr01_model extends CI_Model {
	...
	function search($limit, $offset, $sort_by, $sort_order)     
	    { 
	     ...
	     $sort_columns = array('uid','ta001', 'ta002', 'ta003', 'ta026','ta031',  'ta033','ta034','ta035');
	     $sort_by = (in_array($sort_by, $sort_columns)) ? $sort_by : 'ta002';  //檢查排序欄位是否在 table 內
	     $query = $this->db->select('a.ta001, a.ta002, a.ta003, a.ta026,ta031, a.ta033, a.ta034,a.ta035')
	                       ->from('mymta as a')
	...
	function selonebefore($seq1,$seq2)    
        {
		  $this->db->select('a.* ,c.mq002 AS ta001disp, d.mb002 AS ta010disp,e.mf002 AS ta005disp, f.mv002 AS ta011disp,g.na003 AS ta027disp,
		  ,h.ma002 AS ta004disp,b.company, b.creator, b.usr_group, b.create_date, b.modifier, b.modi_date, b.flag, b.td001, b.td002, b.td003, b.td004, b.td005,
		  b.td006, b.td007, b.td008, b.td009, b.td010, b.td011, b.td012, b.td014,i.mc002 as td007disp');
		 
        $this->db->from('purtc as a');	
        $this->db->join('purtd as b', 'a.ta001 = b.td001  and a.ta002=b.td002 ','left');		
		$this->db->join('cmsmq as c', 'a.ta001 = c.mq001 and c.mq003="34" ','left');
	    $this->db->join('cmsmb as d', 'a.ta010 = d.mb001 ','left');
		$this->db->join('cmsmf as e', 'a.ta005 = e.mf001 ','left');		
		$this->db->join('cmsmv as f ', 'a.ta011 = f.mv001 and f.mv022 = " " ','left');
		$this->db->join('cmsna as g ', 'a.ta027 = g.na002 and g.na001= "1" ','left');
		...
		$this->db->order_by('ta001 , ta002 ,b.td003');
	...
	function selone($seq1)    
        ...
        $this->db->from('mymta as a');	
       
		$this->db->where('a.ta002', $this->uri->segment(5)); 
	 
		$this->db->order_by('ta002');
	...
	function ajaxchkno1($seg1)    
        { 	              
	      ...
	      $this->db->where('ta016', $this->uri->segment(5));
		  $query = $this->db->get('mymta');
	      if ($query->num_rows() > 0) 
		     {
		     $res = $query->result();
		     foreach ($query->result() as $row)
	...
	function findf($limit, $offset, $sort_by, $sort_order)     
         {            		
	      ...
	      $seq11 = "SELECT COUNT(*) as count  FROM `mymta` ";
	      $seq1 = "ta001, ta002, ta003, ta004, ta005, ta015,ta016,ta023,ta026,ta031,ta033,ta034,ta035 FROM `mymta` ";
        ...
        if ($seq7!='') {$seq9=" ORDER BY ".$seq7;$seq33=$seq7;}
         $sort_order = (substr($sort_order,0,3) == 'asc') ? 'asc' : 'desc';
	     $sort_columns = array('ta001', 'ta002', 'ta003', 'ta004', 'ta005', 'ta015','ta016','ta023','ta031','ta032','ta033');
	     $sort_by = (in_array($sort_by, $sort_columns)) ? $sort_by : 'ta001';  //檢查排序欄位是否在 table 內
	     $query = $this->db->select('ta001, ta002, ta003, ta004, ta005, ta015,ta016,ta023,ta026,ta031,ta033,ta034,ta035')
	                       ->from('mymta as a')						  
		                   ->where($seq32)
			               ->order_by($seq33)
			              //->order_by($sort_by, $sort_order)
	...
	function filterf1($limit, $offset , $sort_by  , $sort_order)          
	    {    
	      ...
	      $sort_order = (substr($sort_order,0,3) == 'asc') ? 'asc' : 'desc';
	      $sort_columns = array('ta001', 'ta002', 'ta003', 'ta004', 'ta004disp', 'ta011','ta012','create_date');
          $sort_by = (in_array($sort_by, $sort_columns)) ? $sort_by : 'ta001';  //檢查排序欄位是否為 table
	      $this->db->select('ta001, ta002, ta003, ta004, ta005, ta015,ta016,ta023,ta026,ta031,ta033,ta034,ta035');
	      $this->db->from('mymta as a');
	...
	function selone1($seg1)    
        {
	      $this->db->where('ta001', $this->input->post('acpq01a73'));
    ...
    function selone1d($seg1)    
        {
		  ...
		  $this->db->where('td002', $this->input->post('ta002'));
	...
	//新增一筆 檔頭  mymta	
	function insertf()    //新增一筆 檔頭  mymta
        {
		...
		//	 if ($this->input->post('ta018')!='1') {$ta019=round($this->input->post('ta019'));}
		     $ta001=$this->input->post('acpq01a73');
	     ...
	     $data = array( 
                 ...
                 'ta016' => substr($this->input->post('ta016'),0,4).substr($this->input->post('ta016'),5,2).substr(rtrim($this->input->post('ta016')),8,2),
                 'ta017' => $this->input->post('ta017'),
                 'ta018' => $this->input->post('ta018')
                 
                );
         
	      $exist = $this->mymr01_model->selone1($this->input->post('acpq01a73'),$this->input->post('ta002'));
	      if ($exist)
	         {
		      return 'exist';
		     } 
             $this->db->insert('mymta', $data);
			
		// 新增明細 acptd
				//		$this->db->flush_cache(); 
            // 新增明細 acptd  主檔 mymta 重計算合計金額 數量
			    $ta011=0;$ta012=0;$ta013=0;$ta014=0;$ta022b=0;				
			    $n = '0';
				$td003='1000';
		
		
		if (!isset($_POST['order_product'][  $n  ]['td005']) ) { $n='15'; }  
		...
		//	while ($_POST['order_product'][  $n  ]['td004']) {		
	     ...
	     // $exist = $this->mymr01_model->selone1d($this->input->post('purq04a34'),$this->input->post('ta002'));
		  if ($_POST['order_product'][  $n  ]['td005'] >'0') {
		  $this->db->insert('acptd', $data_array); }
		    if ($_POST['order_product'][  $n  ]['td004'] =='1') {
		      $ta011=$ta011+$_POST['order_product'][ $n  ]['td014'];
			  $ta013=$ta013+$_POST['order_product'][ $n  ]['td015'];}
			else {  
			  $ta012=$ta012+$_POST['order_product'][ $n  ]['td014'];
			  $ta014=$ta014+$_POST['order_product'][ $n  ]['td015']; }
			...
			//重新計算貨款 mymta
		  $sql = " UPDATE mymta set ta011='$ta011',ta012='$ta012',ta013='$ta013',ta014='$ta014' WHERE ta001 = '$ta001'  AND ta002 = '$ta002'  "; 
    ...
    function copyf()           
        {
         ...
         //   if ($query->num_rows() != 1) { return 'exist'; }
		    if ($query->num_rows() == 1) 
		       {
				...
				$ta017=$row->ta017;$ta018=$row->ta018;
			endforeach;
		       }   
		  
            $seq1=$this->input->post('ta001c');    //主鍵一筆檔頭mymta
            ...
            $exist = $this->mymr01_model->selone2($this->input->post('ta001c'),$this->input->post('ta002c'));
		    if ($exist)
		      {
			    return 'exist';
		      }         
             $this->db->insert('mymta', $data);      //複製一筆  
			
			//複製一筆明細
			$this->db->where('td001', $this->input->post('ta001o'));
			$this->db->where('td002', $this->input->post('ta002o'));
	        $query = $this->db->get('acptd');
	        $exist = $query->num_rows();
            if (!$exist)
	          {
          ...
          //  if ($query->num_rows() != 1) { return 'exist'; }
			...
			$seq1=$this->input->post('ta001c');    //主鍵一筆明細acptd
			$seq2=$this->input->post('ta002c'); 
              $i=0;
            while ($i<$num) {	
			  $data_array = array( 
	...
	function excelnewf()           
	  ...
	  //    $sql = " SELECT ta001,ta002,ta024,ta004,ta011,ta003,create_date FROM mymta WHERE ta001 >= '$seq1'  AND ta001 <= '$seq2' AND  ta002 >= '$seq3'  AND ta002 <= '$seq4'  "; 
     
	   $sql = " SELECT a.*
		  FROM mymta as a WHERE  ta002 >= '$seq1'  AND ta002 <= '$seq2'   "; 
		 $query = $this->db->query($sql);
	...
	function printfd()          
        {
		  ...
		  $seq3=$this->input->post('ta002o');    
	      $seq4=$this->input->post('ta002c');
	      $sql = " SELECT a.ta001,a.ta002,a.ta003,a.ta004,b.td001,b.td002,b.td003,b.td004,b.td005,b.td006,b.td007,b.td008,b.td009,
		  b.td010,b.td011,b.td012,b.td014,b.td015,b.td018
		  FROM mymta as a, acptd as b WHERE ta001=td001 and ta002=td002 and  ta001 >= '$seq1'  AND ta001 <= '$seq2' AND ta002 >= '$seq3'  AND ta002 <= '$seq4'  "; 
          $query = $this->db->query($sql);
	      $ret['rows'] = $query->result();
	...
	function printfd1()   
       {           
        ...
        $this->db->from('mymta as a');	
        $this->db->join('acptd as b', 'a.ta001 = b.td001  and a.ta002=b.td002 ','left');		
	...
	function printfc()   
      {           
		...
		$this->db->where('a.ta002', $this->input->post('ta001o')); 
	...
	function updatef()   
        {
          ...
          $data = array(			
		        ...
		        'flag' => $this->input->post('flag')+1,		       
                 ...
                 'ta015' => $this->input->post('ta015'),	
                 'ta016' => substr($this->input->post('ta016'),0,4).substr($this->input->post('ta016'),5,2).substr(rtrim($this->input->post('ta016')),8,2),
                 'ta017' => $this->session->userdata('manager'),
                 'ta018' => $this->input->post('ta018')
                 
                );
            $this->db->where('ta001', $this->input->post('acpq01a73'));
			$this->db->where('ta002', $this->input->post('ta002'));
            $this->db->update('mymta',$data);                   //更改一筆
		...
		//	while ($_POST['order_product'][  $n  ]['td004']) {
			while (isset($_POST['order_product'][  $n  ]['td005'])) {
			   $data_array = array( 
			     ...
			     'td009' =>  substr($_POST['order_product'][ $n  ]['td009'],0,4).substr($_POST['order_product'][ $n  ]['td009'],5,2).substr($_POST['order_product'][ $n  ]['td009'],8,2),
				 'td010' =>  $_POST['order_product'][ $n  ]['td010'],
                 'td011' =>  $_POST['order_product'][ $n  ]['td011'],
				 'td012' =>  $_POST['order_product'][ $n  ]['td012'],
				 'td013' =>  $_POST['order_product'][ $n  ]['td013'],
