...
class ipsr04_model extends CI_Model {
	...
	function insertf()    //新增一筆 檔頭  epsth
		...
		//	 if ($this->input->post('ma018')!='1') {$ma019=round($this->input->post('ma019'));}
	     ...
	     $data = array( 
	      ...
	      $exist = $this->ipsr04_model->selone1($this->input->post('copq03a23'),$this->input->post('ma002'));
	      if ($exist)
	         {
		      return 'exist';
		     } 
		...
		//	while ($_POST['order_product'][  $n  ]['tf004']) {		
	     ...
	     $exist = $this->ipsr04_model->selone1d($this->input->post('copq03a23'),$this->input->post('ma002'),$tf003);
		...
		if ($_POST['order_product'][  $n  ]['tf004']!='') {
		  ...
		  // 訂單已交數量
			 ...
			 $tf016=$_POST['order_product'][ $n  ]['tf016'];
			 $tf008=$_POST['order_product'][ $n  ]['tf008'];
			 $existb = $this->ipsr04_model->selone3d($tf014,$tf015,$tf016);
			  $data_add = array( 
	             'company' => $this->session->userdata('syscompany'),
	             'creator' => $this->session->userdata('manager'),
		         'usr_group' => 'A100',
		 ...
		 $query = $this->db->query($sql);	} 
		  ...
		  //庫存增加減少 品號,庫別, 數量
			 ...
			 $tf007=$_POST['order_product'][ $n  ]['tf007'];
			 $tf008=$_POST['order_product'][ $n  ]['tf008'];
			 $exista = $this->ipsr04_model->selone2d($tf004,$tf007);
			  $data_add = array( 
	             'company' => $this->session->userdata('syscompany'),
	             'creator' => $this->session->userdata('manager'),
		         'usr_group' => 'A100',
    ...
    function copyf()           
        {
         ...
         //   if ($query->num_rows() != 1) { return 'exist'; }
            ...
            $exist = $this->ipsr04_model->selone2($this->input->post('ma001c'),$this->input->post('ma002c'));
		    if ($exist)
		      {
			    return 'exist';
		      }         
	...
	function printfd()          
	    ...
	    //  $seq4=$this->input->post('tc002c');
		  ...
		  $seq5=$this->input->post('tc002o');   //品號 
	      $seq6=$this->input->post('tc002c');
		   $seq7=$this->input->post('tc003o');   //品號 
	      $seq8=$this->input->post('tc003c');
		     if ($seq2=='') {$seq2='zz';}
		  if ($seq4=='') {$seq4='zz';}
		  if ($seq6=='') {$seq6='zz';}
		  if ($seq8=='') {$seq8='zz';}
		$this->db->select('a.*,b.tb001,b.tb002,b.tb003,b.tb016,c.ti001,c.ti002,c.ti003,c.ti008,c.ti010,c.ti055,c.ti056,c.ti027,c.ti014,c.ti029,c.ti017,d.mb002 as ta036disp'); 
        $this->db->from('ipsta as a');	
		$this->db->join('ipstb as b','a.ta001=b.tb001 and a.ta002=b.tb002','left');
		$this->db->join('ipsti as c','b.tb004=c.ti004 and b.tb005=c.ti005 and b.tb006=c.ti006','left');
		$this->db->join('cmsmb as d','a.ta036=d.mb001 ','left');
		$this->db->where('a.ta001 >=', $seq1); 
	    $this->db->where('a.ta001 <=', $seq2); 
		$this->db->where('a.ta008 >=', $seq3); 
	    $this->db->where('a.ta008 <=', $seq4);
        $this->db->where('a.ta005 >=', $seq5); 
	    $this->db->where('a.ta005 <=', $seq6); 
         $this->db->where('a.ta004 >=', $seq7); 
	    $this->db->where('a.ta004 <=', $seq8); 				
		$this->db->group_by(array("ta001", "ta002")); 
		$this->db->order_by('a.ta001 , a.ta002');
		
		$query = $this->db->get();		  
		
	      $ret['rows'] = $query->result();  
          $seq32 = "ta001 >= '$seq1'  AND ta001 <= '$seq2' and ta008 >= '$seq3'  AND ta008 <= '$seq4'   ";	
	      $query = $this->db->select('COUNT(*) as count', FALSE)  //筆數查詢,如果設為FALSE不會使用反引號保護你的字段或者表名
		              ->from('ipsta as a')
					  ->join('ipstb as b','a.ta001=b.tb001 and a.ta002=b.tb002','left')
		              ->where($seq32);
	      $num = $query->get()->result();		
	      $ret['num_rows'] = $num[0]->count;		
	      return $ret;
