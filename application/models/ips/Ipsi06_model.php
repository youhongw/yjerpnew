...
class Ipsi06_model extends CI_Model {
	...
	{
		$this->session->set_userdata('ipsi06_search',"display_search/".$offset);
		...
		if(isset($_SESSION['ipsi06']['search']['where'])){
			if($where){$where .= " and ";}
			$where .= $_SESSION['ipsi06']['search']['where'];
		}
		...
		if(isset($_SESSION['ipsi06']['search']['order'])){
			if($order){$order .= " , ";}
			$order .= $_SESSION['ipsi06']['search']['order'];
		}
		
		if(!isset($_SESSION['ipsi06']['search']['order']) && $default_order){
			if($order){$order .= " , ";}
			$order .= $default_order;
		}
		/* order end */
		...
		$ret['data'] = $query->get()->result();
		//儲存sql
		$_SESSION['ipsi06']['search']['sql'] = $this->db->last_query();
		...
		//儲存where與order
		$_SESSION['ipsi06']['search']['where'] = $where;
		$_SESSION['ipsi06']['search']['order'] = $order;
		$_SESSION['ipsi06']['search']['offset'] = $offset;
	...
	function findf($limit, $offset, $sort_by, $sort_order)     
       {            		
         ...
         if ($seq7!='') {$seq9=" ORDER BY ".$seq7;$seq33=$seq7;}
		 	 //下一頁不會亂跳
		if(@$_SESSION['ipsi06_sql_term']){$seq32 = $_SESSION['ipsi06_sql_term'];}
		if(@$_SESSION['ipsi06_sql_sort']){$seq33 = $_SESSION['ipsi06_sql_sort'];}
	...
	function insertf()   
       ...
       //    }
	    ...
	    $exist = $this->ipsi06_model->selone1($this->input->post('mq001'));
	    if ($exist)
	      {
		    return 'exist';
		  } 
    ...
    function copyf()           //複製一筆
       {
	    ...
	    $data = array( 
            ...
            $exist = $this->ipsi06_model->selone2($this->input->post('mq002c'));
		    if ($exist)
		      {
			   return 'exist';
		      }         
	...
	function printfd()          
       {
	    ...
	    $seq2=$this->input->post('mq002c'); 
	//	if ($seq1<='A1') {$seq1='A1';}
	//	if ($seq2>='A299') {$seq2='A299';}
	    $sql = " SELECT * FROM cmsmq WHERE mq001 >= '$seq1'  AND mq001 <= '$seq2' and mq003 >= 'A1'  AND mq003 <= 'A2zzz' "; 
        $query = $this->db->query($sql);
	    $ret['rows'] = $query->result();
        $seq32 = "mq001 >= '$seq1'  AND mq001 <= '$seq2' and mq003 >= 'A1'  AND mq003 <= 'A2zzz' ";	
	    $query = $this->db->select('COUNT(*) as count', FALSE)  //筆數查詢,如果設為FALSE不會使用反引號保護你的字段或者表名
		                  ->from('cmsmq')
		                  ->where($seq32);
	    $num = $query->get()->result();		
	...
	/*==以下AJAX處理區域==*/
	 function ajaxipsi06($seg1)    //ajax 查詢一筆 顯示用 庫別6
          { 	              
	    $this->db->set('mq001', $this->uri->segment(4));
	    $this->db->where('mq001', $this->uri->segment(4));	
	    $query = $this->db->get('cmsmq');
	...
	function lookup1($keyword){     
	  ...
	  $this->db->from('cmsmq'); 
      $this->db->where('mq003 >=', 'B');
      $this->db->where('mq003 <=', 'BZZZ');	  
