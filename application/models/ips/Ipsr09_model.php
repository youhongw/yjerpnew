...
class ipsr09_model extends CI_Model {
	...
	function insertf()    //新增一筆 檔頭  epsth
		...
		//	 if ($this->input->post('ma018')!='1') {$ma019=round($this->input->post('ma019'));}
	     ...
	     $data = array( 
	      ...
	      $exist = $this->ipsr09_model->selone1($this->input->post('copq03a23'),$this->input->post('ma002'));
	      if ($exist)
	         {
		      return 'exist';
		     } 
		...
		//	while ($_POST['order_product'][  $n  ]['tf004']) {		
	     ...
	     $exist = $this->ipsr09_model->selone1d($this->input->post('copq03a23'),$this->input->post('ma002'),$tf003);
		...
		if ($_POST['order_product'][  $n  ]['tf004']!='') {
		  ...
		  // 訂單已交數量
			 ...
			 $tf016=$_POST['order_product'][ $n  ]['tf016'];
			 $tf008=$_POST['order_product'][ $n  ]['tf008'];
			 $existb = $this->ipsr09_model->selone3d($tf014,$tf015,$tf016);
			  $data_add = array( 
	             'company' => $this->session->userdata('syscompany'),
	             'creator' => $this->session->userdata('manager'),
		         'usr_group' => 'A100',
		 ...
		 $query = $this->db->query($sql);	} 
		  ...
		  //庫存增加減少 品號,庫別, 數量
			 ...
			 $tf007=$_POST['order_product'][ $n  ]['tf007'];
			 $tf008=$_POST['order_product'][ $n  ]['tf008'];
			 $exista = $this->ipsr09_model->selone2d($tf004,$tf007);
			  $data_add = array( 
	             'company' => $this->session->userdata('syscompany'),
	             'creator' => $this->session->userdata('manager'),
		         'usr_group' => 'A100',
    ...
    function copyf()           
        {
         ...
         //   if ($query->num_rows() != 1) { return 'exist'; }
            ...
            $exist = $this->ipsr09_model->selone2($this->input->post('ma001c'),$this->input->post('ma002c'));
		    if ($exist)
		      {
			    return 'exist';
		      }         
	...
	function printfd()          
	    ...
	    //  $seq4=$this->input->post('tc002c');
		  ...
		  if ($seq4=='') {$seq4='zz';}
		  if ($seq6=='') {$seq6='zz';}
		$this->db->select('a.*,b.th001,b.th002,b.th003,b.th008,c.ti001,c.ti002,c.ti003,c.ti005,c.ti008,c.ti010,c.ti014,c.ti015,c.ti020,c.ti015,c.ti055,c.ti056,c.ti057,d.mb002 as tg005disp,e.ma002 as tg006disp'); 
		...
		$this->db->join('purma as e','a.tg006=e.ma001 ','left');
