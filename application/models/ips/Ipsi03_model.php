...
class ipsi03_model extends CI_Model {
	...
	function construct_sql($limit = 15, $offset = 0, $func = "")
	  {
		...
		if(isset($_SESSION['ipsi03']['search']['where'])){
			if($where){$where .= " and ";}
			$where .= $_SESSION['ipsi03']['search']['where'];
		}
		...
		if(isset($_SESSION['ipsi03']['search']['order'])){
			if($order){$order .= " , ";}
			$order .= $_SESSION['ipsi03']['search']['order'];
		}
		
		if(!isset($_SESSION['ipsi03']['search']['order']) && $default_order){
			if($order){$order .= " , ";}
			$order .= $default_order;
		}
		/* order end */
		...
		$query = $this->db->select('a.*,c.mq002, b.ma002')
	                       ->from('ipstc as a')
						   ->join('purma as b', 'a.tc005 = b.ma001','left')
		...
		$query = $this->db->select('a.*,c.mq002,b.ma002')
	                       ->from('ipstc as a')
						   ->join('purma as b', 'a.tc029 = b.ma001','left')
						   ->join('cmsmq as c', 'a.tc001 = c.mq001','left')
			               ->order_by($order)
			               ->limit($limit, $offset);
		if($where){
		...
		$ret['data'] = $query->get()->result();
		//儲存sql 語法
		$_SESSION['ipsi03']['search']['sql'] = $this->db->last_query();
		
		/* Num SQL 1060803*/ 
		$query = $this->db->select('COUNT(*) as total_num')
			->from('ipstc as a')
			->join('copma as b', 'a.tc029 = b.ma001','left')
			->join('cmsmq as c', 'a.tc001 = c.mq001','left');
		if($where){
			$query->where($where);
		}
		...
		//儲存where與order
		$_SESSION['ipsi03']['search']['where'] = $where;
		$_SESSION['ipsi03']['search']['order'] = $order;
		$_SESSION['ipsi03']['search']['offset'] = $offset;
	...
	function construct_view($data){
		...
		$_SESSION['ipsi03']['search']['view'] = $view_array;
		$_SESSION['ipsi03']['search']['index'] = $index_array;
		//echo "<pre>";var_dump($_SESSION['ipsi03']['search']['view']);exit;
	}
	
	//查詢一筆 修改用   
	function selone($seg1, $seg2) {
		$this->db->select('a.* ,c.mq002 AS tc001disp,f.mb002 AS tc009disp,e.ma002 as tc029disp,d.mf002 as tc028disp,
		   g.ma002 as tc127disp,b.company, b.creator, b.usr_group, b.create_date, b.modifier, b.modi_date, b.flag, b.td001, b.td002, b.td003, b.td004, b.td005
		  ');
		 
        $this->db->from('ipstc as a');	
        $this->db->join('ipstd as b', 'a.tc001 = b.td001  and a.tc002=b.td002 and a.tc003=b.td003 ','left');	//單身	
		$this->db->join('cmsmq as c', 'a.tc001 = c.mq001  ','left');  //單別copi03
		$this->db->join('cmsmf as d', 'a.tc028 = d.mf001 ','left');		//幣別cmsi06
		$this->db->join('purma as e', 'a.tc029 = e.ma001 ','left');  //廠商代號
		$this->db->join('cmsmb as f', 'a.tc009 = f.mb001 ','left');  //廠別
		$this->db->join('cmsma as g', 'a.tc127 = g.ma001 ','left');  //銀行
	//	$this->db->join('cmsmc as i', 'b.td007 = i.mc001 ','left');   //庫別
		$this->db->where('a.tc001', $seg1); 
	    $this->db->where('a.tc002', $seg2); 
		$this->db->order_by('tc001 , tc002 ,tc003');
		...
		$this->db->select('b.*')
			->from('ipstd as b')
		//	->join('invmb as c', 'b.td010 = c.mb001 ','left')   //品號
			//->join('cmsmc as d', 'b.td007 = d.mc001 ','left')   //庫別
			->where('b.td001', $seg1)
			->where('b.td002', $seg2);
		$query = $this->db->get();
	...
	function selone_old($seq1,$seq2)  { 
        ...
        $this->db->from('ipstc as a');	
        $this->db->join('ipstd as b', 'a.tc001 = b.td001  and a.tc002=b.td002 ','left');	//單身	
	...
	function lookup($keyword){     
      $this->db->select('mb001, mb002, mb003,mb004')->from('ipstc');  
	...
	function findf($limit, $offset, $sort_by, $sort_order)     
         {            		
	      ...
	      $seq11 = "SELECT COUNT(*) as count  FROM `ipstc` ";
	      $seq1 = "tc001, tc002, tc003, tc004, tc004 as tc004disp,tc005, tc006,tc007,tc08,tc010,tc011,tc012,tc029,tc030, create_date FROM `ipstc` ";
		...
		if(@$_SESSION['ipsi03_sql_term']){$seq32 = $_SESSION['ipsi03_sql_term'];}
		if(@$_SESSION['ipsi03_sql_sort']){$seq33 = $_SESSION['ipsi03_sql_sort'];}
	...
	function selone1($seg1,$seg2,$seg3)    
        {
	      ...
	      $query = $this->db->get('ipstc');
	      return $query->num_rows() ;
	    }
		
	//查新增資料是否重複 (單身)	
    function selone1d($seg1,$seg2,$seg3,$seg4)    
        {
	      $this->db->where('td001', $seg1);
		  $this->db->where('td002', $seg2);
		  $this->db->where('td003', $seg3);
		  $this->db->where('td004', $seg4);
	      $query = $this->db->get('ipstd');
	      return $query->num_rows() ;
	    }  	
 		
	//新增一筆 檔頭  ipstc	
	function insertf()    //新增一筆 檔頭  ipstc
        {
		    //刪日期 / 符號
		     preg_match_all('/\d/S',$this->input->post('tc004'), $matches);  //處理日期字串
			 $tc004 = implode('',$matches[0]);
			 preg_match_all('/\d/S',$this->input->post('tc025'), $matches);  //處理日期字串
			 $tc025 = implode('',$matches[0]);
			 preg_match_all('/\d/S',$this->input->post('tc126'), $matches);  //處理日期字串
			 $tc126 = implode('',$matches[0]);
			 preg_match_all('/\d/S',$this->input->post('tc130'), $matches);  //處理日期字串
			 $tc130 = implode('',$matches[0]);
			 preg_match_all('/\d/S',$this->input->post('tc133'), $matches);  //處理日期字串
			 $tc133 = implode('',$matches[0]);
			 preg_match_all('/\d/S',$this->input->post('tc135'), $matches);  //處理日期字串
			 $tc135 = implode('',$matches[0]);
			 preg_match_all('/\d/S',$this->input->post('tc136'), $matches);  //處理日期字串
			 $tc136 = implode('',$matches[0]);
						   
			 $tc001=$this->input->post('tc001');
			 $tc002=$this->input->post('tc002');
			 $tc003=$this->input->post('tc003');
			 $tc003no=$tc003;   //明細用再新增一筆時加1
			 //檢查資料是否已存在 若存在加1
			  while($this->ipsi03_model->selone1($tc001,$tc002,$tc003)>0){
				$tc003 = $this->check_title_no1($tc001,$tc002);
				$tc003no=$tc003;
			}
	     ...
	     $data = array( 
                 ...
                 'tc111' => $this->input->post('tc111'),
				 'tc112' => $this->input->post('tc112'),
				 'tc113' => $this->input->post('tc113'),
                 'tc114' => $this->input->post('tc114'),
                 'tc115' => $this->input->post('tc115'),
                 'tc116' => $this->input->post('tc116'),
				 'tc117' => $this->input->post('tc117'),
                 'tc118' => $this->input->post('tc118'),
                 'tc119' => $this->input->post('tc119'),
                 'tc120' => $this->input->post('tc120'),
                 'tc121' => $this->input->post('tc121'),
				 'tc122' => $this->input->post('tc122'),
				 'tc123' => $this->input->post('tc123'),
				 'tc124' => $this->input->post('tc124'),
				 'tc125' => $this->input->post('tc125'),
				 'tc126' => $tc126,
				 'tc127' => $this->input->post('tc127'),
                 'tc128' => $this->input->post('tc128'),
                 'tc129' => $this->input->post('tc129'),
                 'tc130' => $tc130,
                 'tc131' => $this->input->post('tc131'),
				 'tc132' => $this->input->post('tc132'),
				 'tc133' => $tc133,
				 'tc134' => $this->input->post('tc134'),
				 'tc135' => $tc135,
				 'tc136' => $tc136,
				 'tc137' => $this->input->post('tc137')
				
               );
	    
             $this->db->insert('ipstc', $data);
    ...
    function copyf()           
        {
         ...
         //   if ($query->num_rows() != 1) { return 'exist'; }
            ...
            $seq1=$this->input->post('tc001c');    //主鍵一筆檔頭ipstc
            ...
            $exist = $this->ipsi03_model->selone1($seq1,$seq2);  //檢查單頭是否重複
		    if ($exist)
		      {
			    return 'exist';
		      }         
             $this->db->insert('ipstc', $data);      //複製一筆  
          ...
          //  if ($query->num_rows() != 1) { return 'exist'; }
			...
			$seq1=$this->input->post('tc001c');    //主鍵一筆明細ipstd
			$seq2=$this->input->post('tc002c'); 
              $i=0;
            while ($i<$num) {	
			  $data_array = array( 
             ...
             $this->db->insert('ipstd', $data_array);      //複製一筆 
             $i++;			 
            }
             return true;		
	   }
	...
	function excelnewf()           
        {			
		  ...
		  FROM ipstc as a,ipstd as b,copma as c WHERE tc001=td001 and tc002=td002 and tc004=ma001 and tc001 >= '$seq1'  AND tc001 <= '$seq2' AND  tc002 >= '$seq3'  AND tc002 <= '$seq4'  "; 
	...
	function printfd()          
        {
		  ...
		  FROM ipstc as a,ipstd as b,copma as c
	...
	function printfd1()   
       {           
        ...
        $this->db->from('ipstc as a');	
        $this->db->join('ipstd as b', 'a.tc001 = b.td001  and a.tc002=b.td002 ','left');		
	...
	function updatef()   
        {
          ...
          $data = array(			
            ...
            $this->db->update('ipstc',$data);                   //更改一筆
	...
	function deletef($seg1)      
        { 
          ...
          $this->db->delete('ipstc'); 
		  $this->db->where('td001', $this->uri->segment(4));
		  $this->db->where('td002', $this->uri->segment(5));
          $this->db->delete('ipstd'); 
	    ...
	    if (!empty($_POST['selected'])) 
	         {
                foreach($_POST['selected'] as $check) 
			    {
			      $seq[$x] = $check; 
		    	      ...
		    	      $seq2;
					  //只要有一筆Y就不能刪除
					$query6c = $this->db->query("SELECT UPPER(td016) as td0161 FROM ipstd WHERE td001='$seq1' AND td002='$seq2' AND ( UPPER(td016)='Y' or td009>0 ) ");         
				...
				if ($vtd0161 != 'Y') {	  
				  ...
				  $this->db->delete('ipstd'); $this->session->set_userdata('msg1',"未出貨已刪除"); }
	...
	//取單號 最大值加1
	function check_title_no($ipsi06,$tc008){
		preg_match_all('/\d/S',$tc008, $matches);  //處理日期字串
		$tc008 = implode('',$matches[0]);
		//echo var_dump($tc070);exit;
		
		$this->db->select('MAX(tc002) as max_no')
			->from('ipstc')
			->where('tc001', $ipsi06)
			->where('tc008', $tc008);
		//	->like('tc070', $tc070, "after");
			
		$query = $this->db->get();
		$result = $query->result();
		//echo var_dump($tc001.$tc070);exit;
		
	    if (!$result[0]->max_no){return $tc008."001";}
		
		return $result[0]->max_no+1;
	}
	//取單號 最大值加1
	function check_title_no1($ipsi02,$tc002){
		//preg_match_all('/\d/S',$tc008, $matches);  //處理日期字串
		//$tc008 = implode('',$matches[0]);
		//echo var_dump($tc070);exit;
		
		$this->db->select('MAX(tc003) as max_no')
			->from('ipstc')
			->where('tc001', $ipsi02)
			->where('tc002', $tc002);
		//	->like('tc070', $tc070, "after");
