...
/*	function selone($seg1, $seg2) {
		$this->db->select('a.te001,a.te002,a.te003,a.te004 , e.ma002 AS te003disp,f.mb002 AS te004disp,
		  b.*');
		 
        $this->db->from('ipste as a');	
        $this->db->join('ipstf as b', 'a.te001 = b.tf001  and a.te002=b.tf002 ','left');	//單身	
		//$this->db->join('cmsmq as c', 'a.te001 = c.mq001  ','left');  //單別copi03
		//$this->db->join('cmsmf as d', 'a.te016 = d.mf001 ','left');		//幣別cmsi06
		$this->db->join('purma as e', 'a.te003 = e.ma001 ','left');  //廠商代號
		$this->db->join('cmsmb as f', 'a.te004 = f.mb001 ','left');  //廠別
		//$this->db->join('cmsma as g', 'a.te006 = g.ma001 ','left');  //銀行
	  ...
	  //  $this->db->where('a.te002', $seg2); 
		...
		$this->db->select('b.*')
			->from('ipstf as b')
		//	->join('invmb as c', 'b.tf010 = c.mb001 ','left')   //品號
		//	->join('cmsmc as d', 'b.tf007 = d.mc001 ','left')   //庫別
			->where('b.tf001', $seg1);
		//	->where('b.tf002', $seg2);
		$query = $this->db->get();
	...
	function findf($limit, $offset, $sort_by, $sort_order)     
		...
		if(@$_SESSION['ipsi04_sql_sort']){$seq33 = $_SESSION['ipsi04_sql_sort'];}
	     ...
	     $query = $this->db->select('a.*,c.mq002,b.ma002')
	                       ->from('ipste as a')
						   ->join('purma as b', 'a.te005 = b.ma001','left')
						   ->join('cmsmq as c', 'a.te001 = c.mq001','left')
		                   ->where($seq32)
			               ->order_by($seq33)
			              ->limit($limit, $offset);
	     $ret['rows'] = $query->get()->result();
...
/*	function insertf()    //新增一筆 檔頭  ipste
		...
		/*	 $te002=$this->input->post('te002');
			 $te002no=$te002;   //明細用再新增一筆時加1
			 //檢查資料是否已存在 若存在加1
			  while($this->ipsi04_model->selone1($te001,$te002)>0){
				$te002 = $this->check_title_no($te001,$te039);
				$te002no=$te002;
			} */
		...
		// 新增明細 ipstf  
		   ...
		   foreach($order_product as $key => $val){
		        if($val['tf002'] && $val['tf003'] && $val['tf004']){
				        extract($val);
					//	preg_match_all('/\d/S',$tf013, $matches);  //處理日期字串
			         //   $tf013 = implode('',$matches[0]);
						$data = array( 
							'company' => $this->session->userdata('syscompany'),
							'creator' => $this->session->userdata('manager'),
							'usr_group' => 'A100',
						...
						foreach($val as $k=>$v){
							if($k!="tf001"&&$k!="tf002"&&$k!="tf003"&&$k!="tf004"){//主鍵不用更改以及其他外來鍵庫別名稱
							   // if($k=="tf003") {$data[$k] = $vtf003;} else {$data[$k] = $v;}
							   $data[$k] = $v;
							}
						}
					$this->db->insert('ipstf', $data);
					$mtf003 = (int) $vtf003+10;
			        $vtf003 =  (string)$mtf003;
				}
			}
		 } */
			...
			//	preg_match_all('/\d/S',$tf013, $matches);  //處理日期字串
			 //   $tf013 = implode('',$matches[0]);
				if($this->seldetail($te001,$val['tf002'],$val['tf003'],$val['tf004'])>0){
					...
					foreach($val as $k=>$v){
						if($k!="tf001"&&$k!="tf002"&&$k!="tf003"&&$k!="tf004"){//主鍵不用更改以及其他外來鍵庫別名稱 tf013日期等別處理
							//if($k=="tf003") {$data[$k] = $vtf003;} else {$data[$k] = $v;}
							$data[$k] = $v;
						}
					}
