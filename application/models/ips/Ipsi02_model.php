...
class ipsi02_model extends CI_Model {
	...
	function construct_sql($limit = 15, $offset = 0, $func = "")
	  {
		...
		if(isset($_SESSION['ipsi02']['search']['where'])){
			if($where){$where .= " and ";}
			$where .= $_SESSION['ipsi02']['search']['where'];
		}
		...
		if(isset($_SESSION['ipsi02']['search']['order'])){
			if($order){$order .= " , ";}
			$order .= $_SESSION['ipsi02']['search']['order'];
		}
		
		if(!isset($_SESSION['ipsi02']['search']['order']) && $default_order){
			if($order){$order .= " , ";}
			$order .= $default_order;
		}
		/* order end */
		...
		$query = $this->db->select('a.*,c.mq002, b.ma002')
	                       ->from('ipsta as a')
						   ->join('purma as b', 'a.ta005 = b.ma001','left')
		...
		$ret['data'] = $query->get()->result();
		//儲存sql 語法
		$_SESSION['ipsi02']['search']['sql'] = $this->db->last_query();
		...
		//儲存where與order
		$_SESSION['ipsi02']['search']['where'] = $where;
		$_SESSION['ipsi02']['search']['order'] = $order;
		$_SESSION['ipsi02']['search']['offset'] = $offset;
	...
	function construct_view($data){
		...
		$_SESSION['ipsi02']['search']['view'] = $view_array;
		$_SESSION['ipsi02']['search']['index'] = $index_array;
		//echo "<pre>";var_dump($_SESSION['ipsi02']['search']['view']);exit;
	}
	
	//查詢一筆 修改用   
	function selone($seg1, $seg2) {
		$this->db->select('a.* ,c.mq002 AS ta001disp, d.mf002 AS ta016disp, e.ma002 AS ta005disp,f.mb002 AS ta036disp,
		  g.ma002 as ta006disp,
		  ...
		  i.mc002 as tb007disp');
		 
        $this->db->from('ipsta as a');	
        $this->db->join('ipstb as b', 'a.ta001 = b.tb001  and a.ta002=b.tb002 ','left');	//單身	
		$this->db->join('cmsmq as c', 'a.ta001 = c.mq001  ','left');  //單別copi03
		$this->db->join('cmsmf as d', 'a.ta016 = d.mf001 ','left');		//幣別cmsi06
		$this->db->join('copma as e', 'a.ta005 = e.ma001 ','left');  //廠商代號
		$this->db->join('cmsmb as f', 'a.ta036 = f.mb001 ','left');  //廠別
		$this->db->join('cmsma as g', 'a.ta006 = g.ma001 ','left');  //銀行
		...
		$this->db->select('b.*,c.mb002 as tb010disp,c.mb003 as tb010disp1,c.mb004 as tb010disp2,d.mc002 as tb007disp')
			->from('ipstb as b')
			->join('invmb as c', 'b.tb010 = c.mb001 ','left')   //品號
			->join('cmsmc as d', 'b.tb007 = d.mc001 ','left')   //庫別
			->where('b.tb001', $seg1)
			->where('b.tb002', $seg2);
		$query = $this->db->get();
	...
	function lookup($keyword){     
      $this->db->select('mb001, mb002, mb003,mb004')->from('ipsta');  
	...
	function findf($limit, $offset, $sort_by, $sort_order)     
         {            		
	      ...
	      $seq11 = "SELECT COUNT(*) as count  FROM `ipsta` ";
	      $seq1 = "ta001, ta002, ta003, ta004, ta004 as ta004disp,ta005, ta006,ta007,ta08,ta010,ta011,ta012,ta029,ta030, create_date FROM `ipsta` ";
		...
		if(@$_SESSION['ipsi02_sql_term']){$seq32 = $_SESSION['ipsi02_sql_term'];}
		if(@$_SESSION['ipsi02_sql_sort']){$seq33 = $_SESSION['ipsi02_sql_sort'];}
    ...
    function selone1d($seg1,$seg2,$seg3)    
        {
	      ...
	      $query = $this->db->get('ipstb');
	      return $query->num_rows() ;
	    }  	
 		
	//新增一筆 檔頭  ipsta	
	function insertf()    //新增一筆 檔頭  ipsta
        {
		    //刪日期 / 符號
		     preg_match_all('/\d/S',$this->input->post('ta008'), $matches);  //處理日期字串
			 $ta008 = implode('',$matches[0]);
			 ...
			 preg_match_all('/\d/S',$this->input->post('ta045'), $matches);  //處理日期字串
			 $ta045 = implode('',$matches[0]);
						   
			 $ta001=$this->input->post('ta001');
			 $ta002=$this->input->post('ta002');
			 $ta002no=$ta002;   //明細用再新增一筆時加1
			 //檢查資料是否已存在 若存在加1
			  while($this->ipsi02_model->selone1($ta001,$ta002)>0){
		        ...
		        if($val['tb003'] && $val['tb004']){
					...
					//	preg_match_all('/\d/S',$tb013, $matches);  //處理日期字串
			         //   $tb013 = implode('',$matches[0]);
						...
						foreach($val as $k=>$v){
							if($k!="tb001"&&$k!="tb002"&&$k!="tb007disp"){//主鍵不用更改以及其他外來鍵庫別名稱
							    if($k=="tb003") {$data[$k] = $vtb003;} else {$data[$k] = $v;}
							}
						}
					$this->db->insert('ipstb', $data);
    ...
    function copyf()           
        {
         ...
         //   if ($query->num_rows() != 1) { return 'exist'; }
            ...
            $seq1=$this->input->post('ta001c');    //主鍵一筆檔頭ipsta
            ...
            $exist = $this->ipsi02_model->selone1($seq1,$seq2);  //檢查單頭是否重複
		    if ($exist)
		      {
			    return 'exist';
		      }         
             $this->db->insert('ipsta', $data);      //複製一筆  
          ...
          //  if ($query->num_rows() != 1) { return 'exist'; }
			...
			$seq1=$this->input->post('ta001c');    //主鍵一筆明細ipstb
			$seq2=$this->input->post('ta002c'); 
              $i=0;
            while ($i<$num) {	
			  $data_array = array( 
             ...
             $this->db->insert('ipstb', $data_array);      //複製一筆 
             $i++;			 
            }
             return true;		
	   }
	...
	function excelnewf()           
        {			
		  ...
		  FROM ipsta as a,ipstb as b,copma as c WHERE ta001=tb001 and ta002=tb002 and ta004=ma001 and ta001 >= '$seq1'  AND ta001 <= '$seq2' AND  ta002 >= '$seq3'  AND ta002 <= '$seq4'  "; 
	...
	function printfd()          
        {
		  ...
		  FROM ipsta as a,ipstb as b,copma as c
	...
	function printfd1()   
       {           
        ...
        $this->db->from('ipsta as a');	
        $this->db->join('ipstb as b', 'a.ta001 = b.tb001  and a.ta002=b.tb002 ','left');		
	...
	function updatef()   
        {
          ...
          $data = array(			
            ...
            $this->db->update('ipsta',$data);                   //更改一筆
			...
			if(!is_array($order_product)){$order_product=array();}
					...
					$this->db->delete('ipstb'); //刪除明細 1060809
			...
			//	preg_match_all('/\d/S',$tb013, $matches);  //處理日期字串
			 //   $tb013 = implode('',$matches[0]);
				if($this->seldetail($ta001,$ta002,$val['tb003'])>0){
					...
					foreach($val as $k=>$v){
						if($k!="tb001"&&$k!="tb002"&&$k!="tb007disp"){//主鍵不用更改以及其他外來鍵庫別名稱 tb013日期等別處理
							if($k=="tb003") {$data[$k] = $vtb003;} else {$data[$k] = $v;}
						}
					}
					$this->db->where('tb001', $ta001);
					...
					$this->db->update('ipstb',$data);//更改一筆
					$mtb003 = (int) $vtb003+10;
			        $vtb003 =  (string)$mtb003;
				}else{
					if($val['tb003'] && $val['tb004']){
	...
	function deletef($seg1)      
        { 
          ...
          $this->db->delete('ipsta'); 
		  $this->db->where('tb001', $this->uri->segment(4));
		  $this->db->where('tb002', $this->uri->segment(5));
          $this->db->delete('ipstb'); 
	    ...
	    if (!empty($_POST['selected'])) 
	         {
                foreach($_POST['selected'] as $check) 
			    {
			      $seq[$x] = $check; 
		    	      ...
		    	      $seq2;
					  //只要有一筆Y就不能刪除
					$query6c = $this->db->query("SELECT UPPER(tb016) as tb0161 FROM ipstb WHERE tb001='$seq1' AND tb002='$seq2' AND ( UPPER(tb016)='Y' or tb009>0 ) ");         
				...
				if ($vtb0161 != 'Y') {	  
				  ...
				  $this->db->delete('ipstb'); $this->session->set_userdata('msg1',"未出貨已刪除"); }
	...
	//取單號 最大值加1
	function check_title_no($ipsi06,$ta008){
		preg_match_all('/\d/S',$ta008, $matches);  //處理日期字串
		$ta008 = implode('',$matches[0]);
	    ...
	    if (!$result[0]->max_no){return $ta008."001";}
