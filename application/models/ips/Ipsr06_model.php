...
class ipsr06_model extends CI_Model {
	...
	function insertf()    //新增一筆 檔頭  epsth
		...
		//	 if ($this->input->post('ma018')!='1') {$ma019=round($this->input->post('ma019'));}
	     ...
	     $data = array( 
	      ...
	      $exist = $this->ipsr06_model->selone1($this->input->post('copq03a23'),$this->input->post('ma002'));
	      if ($exist)
	         {
		      return 'exist';
		     } 
		...
		//	while ($_POST['order_product'][  $n  ]['tf004']) {		
	     ...
	     $exist = $this->ipsr06_model->selone1d($this->input->post('copq03a23'),$this->input->post('ma002'),$tf003);
		...
		if ($_POST['order_product'][  $n  ]['tf004']!='') {
		  ...
		  // 訂單已交數量
			 ...
			 $tf016=$_POST['order_product'][ $n  ]['tf016'];
			 $tf008=$_POST['order_product'][ $n  ]['tf008'];
			 $existb = $this->ipsr06_model->selone3d($tf014,$tf015,$tf016);
			  $data_add = array( 
	             'company' => $this->session->userdata('syscompany'),
	             'creator' => $this->session->userdata('manager'),
		         'usr_group' => 'A100',
		 ...
		 $query = $this->db->query($sql);	} 
		  ...
		  //庫存增加減少 品號,庫別, 數量
			 ...
			 $tf007=$_POST['order_product'][ $n  ]['tf007'];
			 $tf008=$_POST['order_product'][ $n  ]['tf008'];
			 $exista = $this->ipsr06_model->selone2d($tf004,$tf007);
			  $data_add = array( 
	             'company' => $this->session->userdata('syscompany'),
	             'creator' => $this->session->userdata('manager'),
		         'usr_group' => 'A100',
    ...
    function copyf()           
        {
         ...
         //   if ($query->num_rows() != 1) { return 'exist'; }
            ...
            $exist = $this->ipsr06_model->selone2($this->input->post('ma001c'),$this->input->post('ma002c'));
		    if ($exist)
		      {
			    return 'exist';
		      }         
	...
	function printfd()          
	    ...
	    //  $seq4=$this->input->post('tc002c');
		  ...
		  if ($seq6=='') {$seq6='zz';}
		  if ($seq8=='') {$seq8='zz';}
		$this->db->select('a.*,d.mb002 as tg005disp,b.th001,b.th002,b.th003,c.ti001,c.ti002,c.ti003,c.ti010,c.ti014,c.ti017,c.ti020,c.ti022,c.ti023,c.ti027,c.ti028,c.ti029,c.ti055,c.ti056,c.ti057'); 
        $this->db->from('ipstg as a');	
		$this->db->join('ipsth as b','a.tg001=b.th001 and a.tg002=b.th002','left');
		$this->db->join('ipsti as c','a.tg001=c.ti001 and a.tg002=c.ti002','left');
		$this->db->join('cmsmb as d','a.tg005=d.mb001 ','left');
		$this->db->where('a.tg001 >=', $seq1); 
	    $this->db->where('a.tg001 <=', $seq2); 
		$this->db->where('a.tg003 >=', $seq3); 
	    $this->db->where('a.tg003 <=', $seq4); 
		$this->db->where('a.tg006 >=', $seq5); 
	    $this->db->where('a.tg006 <=', $seq6);
		$this->db->where('a.tg004 >=', $seq7); 
	    $this->db->where('a.tg004 <=', $seq8);
		$this->db->group_by(array("tg001", "tg002")); 
		$this->db->order_by('a.tg001 , a.tg002');
		...
		$query = $this->db->get();		  
	      ...
	      $query = $this->db->select('COUNT(*) as count', FALSE)  //筆數查詢,如果設為FALSE不會使用反引號保護你的字段或者表名
		              ->from('ipstg as a')
					  ->join('ipsth as b','a.tg001=b.th001 and a.tg002=b.th002','left')
		              ->where($seq32);
	      $num = $query->get()->result();		
	      $ret['num_rows'] = $num[0]->count;		
	      return $ret;
