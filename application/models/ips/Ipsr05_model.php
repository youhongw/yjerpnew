...
class ipsr05_model extends CI_Model {
	...
	function insertf()    //新增一筆 檔頭  epsth
		...
		//	 if ($this->input->post('ma018')!='1') {$ma019=round($this->input->post('ma019'));}
	     ...
	     $data = array( 
	      ...
	      $exist = $this->ipsr05_model->selone1($this->input->post('copq03a23'),$this->input->post('ma002'));
	      if ($exist)
	         {
		      return 'exist';
		     } 
		...
		//	while ($_POST['order_product'][  $n  ]['tf004']) {		
	     ...
	     $exist = $this->ipsr05_model->selone1d($this->input->post('copq03a23'),$this->input->post('ma002'),$tf003);
		...
		if ($_POST['order_product'][  $n  ]['tf004']!='') {
		  ...
		  // 訂單已交數量
			 ...
			 $tf016=$_POST['order_product'][ $n  ]['tf016'];
			 $tf008=$_POST['order_product'][ $n  ]['tf008'];
			 $existb = $this->ipsr05_model->selone3d($tf014,$tf015,$tf016);
			  $data_add = array( 
	             'company' => $this->session->userdata('syscompany'),
	             'creator' => $this->session->userdata('manager'),
		         'usr_group' => 'A100',
		 ...
		 $query = $this->db->query($sql);	} 
		  ...
		  //庫存增加減少 品號,庫別, 數量
			 ...
			 $tf007=$_POST['order_product'][ $n  ]['tf007'];
			 $tf008=$_POST['order_product'][ $n  ]['tf008'];
			 $exista = $this->ipsr05_model->selone2d($tf004,$tf007);
			  $data_add = array( 
	             'company' => $this->session->userdata('syscompany'),
	             'creator' => $this->session->userdata('manager'),
		         'usr_group' => 'A100',
    ...
    function copyf()           
        {
         ...
         //   if ($query->num_rows() != 1) { return 'exist'; }
            ...
            $exist = $this->ipsr05_model->selone2($this->input->post('ma001c'),$this->input->post('ma002c'));
		    if ($exist)
		      {
			    return 'exist';
		      }         
	...
	function printfd()          
	    ...
	    //  $seq4=$this->input->post('tc002c');
		  ...
		  if ($seq4=='') {$seq4='zz';}
		  if ($seq6=='') {$seq6='zz';}
		$this->db->select('a.te001,a.te002,a.te003,a.te004,b.*,e.mc002 as tf005disp,c.ma002 as te003disp,d.mb002 as te004disp'); 
        $this->db->from('ipste as a');	
		$this->db->join('ipstf as b','a.te001=b.tf001','left');
		$this->db->join('purma as c','a.te003=c.ma001','left');
		$this->db->join('cmsmb as d','a.te004=d.mb001','left');
		$this->db->join('cmsmc as e','b.tf005=e.mc001','left');
		$this->db->where('a.te001 >=', $seq1); 
	    $this->db->where('a.te001 <=', $seq2); 
		$this->db->where('a.te002 >=', $seq3); 
	    $this->db->where('a.te002 <=', $seq4); 
		$this->db->where('a.te003 >=', $seq5); 
	    $this->db->where('a.te003 <=', $seq6); 
		$this->db->group_by(array("te001", "te002")); 
		$this->db->order_by('a.te001 , a.te002');
		...
		$query = $this->db->get();		  
	      ...
	      $query = $this->db->select('COUNT(*) as count', FALSE)  //筆數查詢,如果設為FALSE不會使用反引號保護你的字段或者表名
		              ->from('ipste as a')
					  ->join('ipstf as b','a.te001=b.tf001','left')
		              ->where($seq32);
	      $num = $query->get()->result();		
	      $ret['num_rows'] = $num[0]->count;		
	      return $ret;
