...
class ipsi01_model extends CI_Model {
	...
	function selbrowse($num,$offset)   
       {            
	    $this->db->select('ma001, ma002, ma003, ma004 create_date');
        $this->db->from('ipsma');
	...
	function search($limit, $offset, $sort_by, $sort_order)     
	  { 
	    ...
	    $sort_columns = array('ma001', 'ma002', 'ma003', 'ma004','create_date');
	...
	{
		$this->session->set_userdata('ipsi01_search',"display_search/".$offset);
		...
		if(isset($_SESSION['ipsi01']['search']['where'])){
			if($where){$where .= " and ";}
			$where .= $_SESSION['ipsi01']['search']['where'];
		}
		...
		if(isset($_SESSION['ipsi01']['search']['order'])){
			if($order){$order .= " , ";}
			$order .= $_SESSION['ipsi01']['search']['order'];
		}
		
		if(!isset($_SESSION['ipsi01']['search']['order']) && $default_order){
			if($order){$order .= " , ";}
			$order .= $default_order;
		}
		/* order end */
		...
		$ret['data'] = $query->get()->result();
		//儲存sql
		$_SESSION['ipsi01']['search']['sql'] = $this->db->last_query();
		...
		//儲存where與order
		$_SESSION['ipsi01']['search']['where'] = $where;
		$_SESSION['ipsi01']['search']['order'] = $order;
		$_SESSION['ipsi01']['search']['offset'] = $offset;
	...
	function construct_sql_body($limit = 15, $offset = 0, $func = "")
	{
		...
		if(isset($_SESSION['ipsi01_body']['search']['where'])){
			if($where){$where .= " and ";}
			$where .= $_SESSION['ipsi01_body']['search']['where'];
		}
		...
		if(isset($_SESSION['ipsi01_body']['search']['order'])){
			if($order){$order .= " , ";}
			$order .= $_SESSION['ipsi01_body']['search']['order'];
		}
		
		if(!isset($_SESSION['ipsi01_body']['search']['order']) && $default_order){
			if($order){$order .= " , ";}
			$order .= $default_order;
		}
		/* order end */
		...
		$ret['data'] = $query->get()->result();
		//儲存sql
		$_SESSION['ipsi01_body']['search']['sql'] = $this->db->last_query();
		...
		//儲存where與order
		$_SESSION['ipsi01_body']['search']['where'] = $where;
		$_SESSION['ipsi01_body']['search']['order'] = $order;
		$_SESSION['ipsi01_body']['search']['offset'] = $offset;
	...
	function findf($limit, $offset, $sort_by, $sort_order)     
       {            		
	     ...
	     $seq11 = "SELECT COUNT(*) as count  FROM `ipsma` ";
	     $seq1 = " ma001, ma002, ma003, ma004,  create_date FROM `ipsma` ";
         ...
         if ($seq7!='') {$seq9=" ORDER BY ".$seq7;$seq33=$seq7;}
		  //下一頁不會亂跳
		if(@$_SESSION['ipsi01_sql_term']){$seq32 = $_SESSION['ipsi01_sql_term'];}
		if(@$_SESSION['ipsi01_sql_sort']){$seq33 = $_SESSION['ipsi01_sql_sort'];}
	...
	function filterf1($limit, $offset , $sort_by  , $sort_order)    //篩選多筆        
	   {    
	    ...
	    $this->db->select('ma001, ma002, ma003, ma004,ma005, create_date');
	    $this->db->from('ipsma');
	...
	function insertf()   
       {
	    ...
	    $data = array( 
		          ...
		          'ma003' => $this->input->post('actq03a'),
		          'ma004' => $this->input->post('ma004'),
		          'ma005' => $this->input->post('ma005')
                      );
         
	    $exist = $this->ipsi01_model->selone1($this->input->post('ma001'));
	    if ($exist)
	      {
		    return 'exist';
		  } 
           return  $this->db->insert('ipsma', $data);
       }
    ...
    function copyf()           //複製一筆
       {
	    ...
	    $data = array( 
            ...
            $exist = $this->ipsi01_model->selone2($this->input->post('ma002c'));
		    if ($exist)
		      {
			   return 'exist';
		      }         
              return $this->db->insert('ipsma', $data);      //複製一筆  
       }	
	...
	function excelnewf()           
       {			
	    ...
	    $seq2=$this->input->post('ma002c');
	    $sql = " SELECT ma001,ma002,ma004,ma003,create_date FROM ipsma WHERE ma001 >= '$seq1'  AND ma001 <= '$seq2' ORDER BY ma001 "; 
	...
	function printfd()          
       {
	    $seq1=$this->input->post('ma001c');    //查詢一筆以上
	    $seq2=$this->input->post('ma002c'); 
	    $sql = " SELECT * FROM ipsma WHERE ma001 >= '$seq1'  AND ma001 <= '$seq2'  "; 
        $query = $this->db->query($sql);
	    $ret['rows'] = $query->result();
	...
	function updatef()   //更改一筆
          {
            ...
            $this->db->update('ipsma',$data);                   //更改一筆
	...
	/*==以下AJAX處理區域==*/
	 function ajaxipsi01($seg1)    //ajax 查詢一筆 顯示用 費用6
          { 	              
	    $this->db->set('ma001', $this->uri->segment(4));
	    $this->db->where('ma001', $this->uri->segment(4));	
	    $query = $this->db->get('ipsma');
	...
	function lookup_old($select_col=array(),$search_col=array(),$keyword=array(),$limit=10){
		...
		if($sel_col == ""){$sel_col = "*";}
		$this->db->select($sel_col)->from('ipsma');
	...
	//ajax 下拉視窗查詢類 google 下拉 明細 費用單身(catcomplete)
	...
	//ajax 下拉視窗查詢類 google 下拉 明細 費用單身(check)
