...
class Admi05_model extends CI_Model {
	...
	function search($limit, $offset, $sort_by, $sort_order)     //欄位表頭排序流覽資料
	  { 
	    ...
	    $sort_columns = array('mg001', 'mg002', 'b.mb002','mg003','mg004','mg005','mg006','mg007','mg008', 'a.create_date');
	    $sort_by = (in_array($sort_by, $sort_columns)) ? $sort_by : 'mg001';  //檢查排序欄位是否在 table 內
	    $query = $this->db->select('mg001, mg002,b.mb002, mg003,mg004,mg005,mg006,mg007,mg008,  a.create_date')
	                      ->from('admmg as a')
						  ->join('admmb as b', 'a.mg002 = b.mb001 ','left')
	...
	function findf($limit, $offset, $sort_by, $sort_order)    //查詢多筆進階查詢 mysql 
	  ...
	  //$seq5='';$seq51='';$seq7='';$seq71='';		  
	   ...
	   //  $seq7=$this->session->userdata('find07');
		...
		//echo "<pre>";var_dump($seq33);var_dump($_SESSION['admi05_sql_sort']);exit;
             $sort_order = (substr($sort_order,0,3) == 'asc') ? 'asc' : 'desc';
	...
	function filterf1($limit, $offset , $sort_by  , $sort_order)    //篩選多筆        
	  {    
	    ...
	    $this->db->select('mg001, mg002,b.mb002, mg003,mg004,mg005,mg006,mg007,mg008, a.create_date');
	    $this->db->from('admmg as a');
		$this->db->join('admmb as b', 'a.mg002 = b.mb001 ','left');
	...
	function insertf()    //新增一筆
		...
		$mg6=$this->input->post('mg611').$this->input->post('mg612').$this->input->post('mg613').$this->input->post('mg614').$this->input->post('mg615').$this->input->post('mg616').
	       $this->input->post('mg617').$this->input->post('mg618').$this->input->post('mg619').$this->input->post('mg620').$this->input->post('mg621').$this->input->post('mg622').$this->input->post('mg623').$this->input->post('mg624');
		$mg7=$this->input->post('mg711').$this->input->post('mg712').$this->input->post('mg713').$this->input->post('mg714').$this->input->post('mg715').$this->input->post('mg716').
	       $this->input->post('mg717').$this->input->post('mg718').$this->input->post('mg719').$this->input->post('mg720').$this->input->post('mg721').$this->input->post('mg722').$this->input->post('mg723').$this->input->post('mg724');
        $mg8=$this->input->post('mg811').$this->input->post('mg812').$this->input->post('mg813').$this->input->post('mg814').$this->input->post('mg815').$this->input->post('mg816').
	       $this->input->post('mg817').$this->input->post('mg818').$this->input->post('mg819').$this->input->post('mg820').$this->input->post('mg821').$this->input->post('mg822').$this->input->post('mg823').$this->input->post('mg824');
	    ...
	    $exist = $this->admi05_model->selone1($this->input->post('mg001'),$this->input->post('admq02a'));   //查詢資料是否重複
	    if ($exist)
	         {
		    return 'exist';
		 } 
     ...
     function selone2($seg1,$seg2)    //查copy複製資料是否重複
	   ...
	   //	$this->db->where('mg002', $this->input->post('mg003c'));
	    ...
	    $data = array( 
         ...
         //   $exist = $this->admi05_model->selone2($this->input->post('mg002c'),$mg002);
		 //   if ($exist)
		//        {
		//	  return 'exist';
		  //      }         
