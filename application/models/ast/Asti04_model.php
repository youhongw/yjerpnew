...
class Asti04_model extends CI_Model {
	...
	function construct_sql($limit = 15, $offset = 0, $func = "")
	  {
		...
		if(isset($_SESSION['asti04']['search']['where'])){
			if($where){$where .= " and ";}
			$where .= $_SESSION['asti04']['search']['where'];
		}
		...
		if(isset($_SESSION['asti04']['search']['order'])){
			if($order){$order .= " , ";}
			$order .= $_SESSION['asti04']['search']['order'];
		}
		
		if(!isset($_SESSION['asti04']['search']['order']) && $default_order){
			if($order){$order .= " , ";}
			$order .= $default_order;
		}
		/* order end */
		...
		$ret['data'] = $query->get()->result();
		//儲存sql 語法
		$_SESSION['asti04']['search']['sql'] = $this->db->last_query();
		...
		//儲存where與order
		$_SESSION['asti04']['search']['where'] = $where;
		$_SESSION['asti04']['search']['order'] = $order;
		$_SESSION['asti04']['search']['offset'] = $offset;
	...
	function construct_view($data){
		...
		$_SESSION['asti04']['search']['view'] = $view_array;
		$_SESSION['asti04']['search']['index'] = $index_array;
		//echo "<pre>";var_dump($_SESSION['asti04']['search']['view']);exit;
	}
	...
	function selone($seg1) {
		...
		$this->db->select('b.*, b.tb002 as asti02, c.mb002 as asti02_mb002, c.mb003 as asti02_mb003, c.mb011 as asti02_mb011, c.mb016 as asti02_mb012, c.mb016 as asti02_mb016, c.mb020 as asti02_mb020')
			->from('asttb as b')
			->join('astmb as c','b.tb002 = c.mb001','left')
			->where('b.tb001', $seg1);
		$query = $this->db->get();
	...
	function findf($limit, $offset, $sort_by, $sort_order)     
         {            		
	      ...
	      $seq11 = "SELECT COUNT(*) as count  FROM `astta` ";
	      $seq1 = "ta001, ta002, ta003, ta004, ta005, ta006, ta007, ta008, create_date FROM `astta` ";
		...
		if(@$_SESSION['asti04_sql_term']){$seq32 = $_SESSION['asti04_sql_term'];}
		if(@$_SESSION['asti04_sql_sort']){$seq33 = $_SESSION['asti04_sql_sort'];}
		
         $sort_order = (substr($sort_order,0,3) == 'asc') ? 'asc' : 'desc';
	     $sort_columns = array('ta001', 'ta002', 'ta003', 'ta004', 'ta005', 'ta006', 'ta007',' ta008', 'a.create_date');
	     $sort_by = (in_array($sort_by, $sort_columns)) ? $sort_by : 'ta001';  //檢查排序欄位是否在 table 內
	     $query = $this->db->select('ta001, ta002, ta003, ta004, ta005, ta005, ta006, ta007, ta008, a.create_date')
	                       ->from('astta as a')
		                   ->where($seq32)
			               ->order_by($seq33)
			              ->limit($limit, $offset);
	...
	function insertf()    //新增一筆 檔頭  coptc
        {
		    //刪日期 / 符號
			 ...
			 //檢查資料是否已存在 若存在加1
			//  while($this->asti04_model->selone1($tc001)>0){
	     ...
	     $data = array( 
		         ...
		         'flag' => 0,
                 'ta001' => $ta001,
		         'ta002' => $this->input->post('puri01'),  //保險公司
		         'ta003' => $this->input->post('puri01disp'),
		         'ta004' => $ta004,    
		         'ta005' => $ta005,    
		...
		// 新增明細 coptd  
		   ...
		   foreach($order_product as $key => $val){
		        if($val['asti02']){
						...
						foreach($val as $k=>$v){
							if($k!="tb001"&&$k!="asti02"&&$k!="asti02_mb002"&&$k!="asti02_mb003"&&$k!="asti02_mb012"&&$k!="asti02_mb011"&&$k!="asti02_mb016"&&$k!="asti02_mb020"){//主鍵不用更改以及其他外來鍵庫別名稱
							    $data[$k] = $v;
							}
						}
					$this->db->insert('asttb', $data);
    ...
    function copyf()           
        {
         ...
         //   if ($query->num_rows() != 1) { return 'exist'; }
            ...
            $exist = $this->asti04_model->selone1($seq1,$seq2);  //檢查單頭是否重複
		    if ($exist)
		      {
			    return 'exist';
		      }         
	...
	function updatef()   
        {
          ...
          $data = array(			
            ...
            $this->db->update('astta',$data);                   //更改一筆
			...
			if(!is_array($order_product)){$order_product=array();}
				...
				$this->db->delete('asttb'); //刪除明細 1060809
			...
			foreach($order_product as $key => $val){
				...
				//preg_match_all('/\d/S',$td013, $matches);  //處理日期字串
			    //$td013 = implode('',$matches[0]);
				if($this->seldetail($ta001,$val['asti02'])>0){
					$data = array(
						'modifier' => $this->session->userdata('manager'),
						'modi_date' => date("Ymd"),
						'flag'  => $flag,
					...
					foreach($val as $k=>$v){
						if($k!="asti02"&&$k!="asti02_mb002"&&$k!="asti02_mb003" && $k!="asti02_mb012" && $k!="asti02_mb011" && $k!="asti02_mb016" && $k!="asti02_mb020"){//主鍵不用更改以及其他外來鍵庫別名稱 td013日期等別處理
							$data[$k] = $v;
						}
					}
					$this->db->where('tb001', $ta001);
					$this->db->where('tb002', $val['asti02']);
					//$this->db->where('td003', $vtd003);
					$this->db->update('asttb',$data);//更改一筆
					//$mtd003 = (int) $vtd003+10;
			        //$vtd003 =  (string)$mtd003;
				}else{
					if($val['asti02']){
						...
						foreach($val as $k=>$v){
							if($k!="asti02"&&$k!="asti02_mb002"&&$k!="asti02_mb003" && $k!="asti02_mb012" && $k!="asti02_mb011" && $k!="asti02_mb016" && $k!="asti02_mb020"){//主鍵不用更改以及其他外來鍵庫別名稱
								$data[$k] = $v;
							}
						}
						$this->db->insert('asttb', $data);
