...
class Asti13_model extends CI_Model {
	...
	function construct_sql($limit = 15, $offset = 0, $func = "")
	  {
		...
		if(isset($_SESSION['asti13']['search']['where'])){
			if($where){$where .= " and ";}
			$where .= $_SESSION['asti13']['search']['where'];
		}
		...
		if(isset($_SESSION['asti13']['search']['order'])){
			if($order){$order .= " , ";}
			$order .= $_SESSION['asti13']['search']['order'];
		}
		
		if(!isset($_SESSION['asti13']['search']['order']) && $default_order){
			if($order){$order .= " , ";}
			$order .= $default_order;
		}
		...
		$ret['data'] = $query->get()->result();
		//儲存sql 語法
		$_SESSION['asti13']['search']['sql'] = $this->db->last_query();
		...
		//儲存where與order
		$_SESSION['asti13']['search']['where'] = $where;
		$_SESSION['asti13']['search']['order'] = $order;
		$_SESSION['asti13']['search']['offset'] = $offset;
	...
	{
		$this->session->set_userdata('asti13_asti14_search',"display_search/".$offset);
		if (session_status() == PHP_SESSION_NONE) {
			session_start();
		}
		...
		$default_where = "";//在這裡塞入一些預設條件，如不顯示離職員工等等
		$default_order = "tg001 asc";//在這裡塞入一些預設排序
		...
		if(isset($_SESSION['asti13_asti14_body']['search']['where'])){
			if($where){$where .= " and ";}
			$where .= $_SESSION['asti13_asti14_body']['search']['where'];
		}
		...
		if(isset($_SESSION['asti13_asti14_body']['search']['order'])){
			if($order){$order .= " , ";}
			$order .= $_SESSION['asti13_asti14_body']['search']['order'];
		}
		
		if(!isset($_SESSION['asti13_asti14_body']['search']['order']) && $default_order){
			if($order){$order .= " , ";}
			$order .= $default_order;
		}
		/* order end */
		...
		/* Data SQL */
		$query = $this->db->select('a.tg001, a.tg002, a.tg003, b.te008, a.tg004, a.tg005, a.tg006, a.tg008,c.mb002,c.mb003,e.me002,f.mv002')
			->from('asttg as a')
			->join('astte as b','a.tg001 = b.te001 and a.tg002 = b.te002','left')
	 ...
	 //	echo "<pre>";var_dump($this->db);exit;
		$ret['data'] = $query->get()->result();
		//儲存sql
		$_SESSION['asti13_asti14_body']['search']['sql'] = $this->db->last_query();
		...
		//儲存where與order
		$_SESSION['asti13_asti14_body']['search']['where'] = $where;
		$_SESSION['asti13_asti14_body']['search']['order'] = $order;
		$_SESSION['asti13_asti14_body']['search']['offset'] = $offset;
	...
	function construct_view($data){
		...
		$_SESSION['asti13']['search']['view'] = $view_array;
		$_SESSION['asti13']['search']['index'] = $index_array;
	...
	function selone($seg1, $seg2) {
		...
		$this->db->select('c.*, d.mb001 as asti02, d.mb002 as asti02_mb002, d.mb003 as asti02_mb003, c.tg004 as asti02_asti13, e.me002 as asti02_asti13_mc002disp, c.tg005 as asti02_asti13_mc003, f.mv002 as asti02_asti13_mc003disp, g.ma001 as puri01,g.ma002 as puri01_ma002') 
			->from('asttg as c')
			->join('astmb as d','c.tg003 = d.mb001','left')
			->join('cmsme as e','c.tg004 = e.me001','left')
	    	->join('cmsmv as f','c.tg005 = f.mv001','left')
	    	->join('purma as g','c.tg007 = g.ma001','left')
			->where('c.tg001', $seg1)
			->where('c.tg002', $seg2);
		$query = $this->db->get();
	...
	function findf($limit, $offset, $sort_by, $sort_order)     
		...
		if(@$_SESSION['asti13_sql_term']){$seq32 = $_SESSION['asti13_sql_term'];}
		if(@$_SESSION['asti13_sql_sort']){$seq33 = $_SESSION['asti13_sql_sort'];}
	...
	function insertf()    //新增一筆 檔頭  asttc
        {
		    //刪日期 / 符號
			 ...
			 $te001=$this->input->post('asti03_asti13');
		...
		//      $vtd003='1010';   //流水號重新排序
		   foreach($order_product as $key => $val){
		        if($val['asti02'] && $val['asti02_asti13'] && $val['asti02_asti13_mc003']){
				        extract($val);
						...
						foreach($val as $k=>$v){
							if($k!="asti02"&&$k!="asti02_mb002"&&$k!="asti02_mb003"&&$k!="asti02_asti13"&&$k!="asti02_asti13"&&$k!="asti02_asti13_mc002disp"&&$k!="asti02_asti13_mc003"&&$k!="asti02_asti13_mc003disp"&&$k!="puri01"&&$k!="puri01_ma002"&&$k!="tg012"){//主鍵不用更改以及其他外來鍵庫別名稱
							    $data[$k] = $v;
							}
						}
						
					$this->db->insert('asttg', $data);
	...
	function excelnewf()           
        {			
	      ...
	      $this->db->select('a.te001,a.te002,a.te008,c.tg003,d.mb002,d.mb003,e.me002,f.mv002,c.tg006,c.tg008') 
					->from('astte as a')
					->join('cmsmq as b','a.te001 = b.mq001','left')
					->join('asttg as c','a.te001 = c.tg001 and a.te002 = c.tg002','left')
					->join('astmb as d','c.tg003 = d.mb001','left')
					->join('cmsme as e','c.tg004 = e.me001','left')
					->join('cmsmv as f','c.tg005 = f.mv001','left')
					->join('purma as g','c.tg007 = g.ma002','left')
					->where('a.te001 >=',$seq1)
					->where('a.te001 <=',$seq2)
					->where('a.te002 >=',$seq3)
					->where('a.te002 <=',$seq4);
	...
	function printfd()          
        {
		  ...
		  $this->db->select('a.*,b.mq002 as te001disp,c.tg001 ,c.tg002, c.tg003, d.mb001 as asti02, d.mb002 as asti02_mb002, d.mb003 as asti02_mb003, c.tg004 as asti02_asti13, e.me002 as asti02_asti13_mc002disp,
							 c.tg005 as asti02_asti13_mc003, f.mv002 as asti02_asti13_mc003disp, g.ma002 as puri01_ma002, c.tg006, c.tg007, c.tg008, c.tg009, c.tg010, c.tg011 ,c.tg012') 
					->from('astte as a')
					->join('cmsmq as b','a.te001 = b.mq001','left')
					->join('asttg as c','a.te001 = c.tg001 and a.te002 = c.tg002','left')
					->join('astmb as d','c.tg003 = d.mb001','left')
	...
	function printfc()   
      {           				
        ...
        $this->db->from('astte as a');
		$this->db->join('cmsmq as b','a.te001 = b.mq001','left');
		$this->db->join('asttg as c','a.te001 = c.tg001 and a.te002 = c.tg002','left');
		$this->db->join('astmb as d','c.tg003 = d.mb001','left');
		$this->db->join('cmsme as e','c.tg004 = e.me001','left');
		$this->db->join('cmsmv as f','c.tg005 = f.mv001','left');
		$this->db->join('purma as g','c.tg007 = g.ma001','left');
		...
		$this->db->order_by('te001 , te002 ,c.tg003');
	...
	function updatef()   
        {
          ...
          $data = array(			
			...
			if(!is_array($order_product)){$order_product=array();}
		            $this->db->where('tg001', $te001);
		            $this->db->where('tg002', $te002);
					$this->db->delete('asttg'); //刪除明細 1060809
			...
			foreach($order_product as $key => $val){
				...
				preg_match_all('/\d/S',$tg012, $matches);  //處理日期字串
			    $tg012 = implode('',$matches[0]);
				if($this->seldetail($te001,$te002,$val['asti02'],$val['asti02_asti13'],$val['asti02_asti13_mc003'])>0){
					$data = array(
						'modifier' => $this->session->userdata('manager'),
						'modi_date' => date("Ymd"),
						'tg003' => $asti02,
					...
					$this->db->where('tg001', $te001);
					$this->db->where('tg002', $te002);
					$this->db->where('tg003', $val['asti02']);
					$this->db->where('tg004', $val['asti02_asti13']);
					$this->db->where('tg005', $val['asti02_asti13_mc003']);
					$this->db->update('asttg',$data);//更改一筆
	...
	function deletedetailf($seg1,$seg2,$seg3,$seg4,$seg5)
        { 
		  ...
		  $this->db->where('tg004', $seg4);
	      $this->db->where('tg005', $seg5);
          $this->db->delete('asttg'); 
	      if ($this->db->affected_rows() > 0)
              {
                return TRUE;
