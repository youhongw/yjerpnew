...
class Asti03_model extends CI_Model {
	...
	//新增暫存view表方法construct_view 上一筆,下一筆 2017.04.1
	...
	function lookupd_catcomplete_asti13($keyword){     
      $this->db->select('mq001,mq002,mq003')->from('cmsmq');  
      $this->db->like('mq001',urldecode(urldecode($this->uri->segment(4))),'after');
      $this->db->where('mq003','C8');
      $this->db->limit('10');		
      $query = $this->db->get(); 
      return $query->result();
    }
	...
	function lookupd_check_asti13($keyword){     
      $this->db->select('mq001,mq002,mq003')->from('cmsmq');  
      $this->db->where('mq001',urldecode(urldecode($this->uri->segment(4))));
      $this->db->where('mq003','C8');
      $this->db->limit('10');		
	...
	//ajax 下拉視窗查詢類 google 下拉 明細 固定資產參數取得單別(catcomplete)	
	function lookupd_catcomplete_asti14($keyword){     
      $this->db->select('mq001,mq002,mq003')->from('cmsmq');  
      $this->db->like('mq001',urldecode(urldecode($this->uri->segment(4))),'after');
      $this->db->where('mq003','C9');
      $this->db->limit('10');		
      $query = $this->db->get(); 
      return $query->result();
    }
	...
	//ajax 下拉視窗查詢類 google 下拉 明細 固定資產參數取得單別(check)	
	function lookupd_check_asti14($keyword){     
      $this->db->select('mq001,mq002,mq003')->from('cmsmq');  
      $this->db->where('mq001',urldecode(urldecode($this->uri->segment(4))));
      $this->db->where('mq003','C9');
      $this->db->limit('10');		
