...
class Asti10_model extends CI_Model {
	...
	function construct_sql($limit = 15, $offset = 0, $func = "")
	  {
		...
		if(isset($_SESSION['asti10']['search']['where'])){
			if($where){$where .= " and ";}
			$where .= $_SESSION['asti10']['search']['where'];
		}
		...
		if(isset($_SESSION['asti10']['search']['order'])){
			if($order){$order .= " , ";}
			$order .= $_SESSION['asti10']['search']['order'];
		}
		
		if(!isset($_SESSION['asti10']['search']['order']) && $default_order){
			if($order){$order .= " , ";}
			$order .= $default_order;
		}
		/* order end */
		...
		$ret['data'] = $query->get()->result();
		//儲存sql 語法
		$_SESSION['asti10']['search']['sql'] = $this->db->last_query();
		...
		//儲存where與order
		$_SESSION['asti10']['search']['where'] = $where;
		$_SESSION['asti10']['search']['order'] = $order;
		$_SESSION['asti10']['search']['offset'] = $offset;
	...
	function selone($seg1, $seg2) {
		...
		$this->db->select('b.td003 as asti02_asti10, b.td004 as asti02_asti10_mc003, b.td005, b.td008, b.td010, e.me002 as asti02_asti10_mc002disp, f.mv002 as asti02_asti10_mc003disp')
			->from('asttd as b')
	...
	function findf($limit, $offset, $sort_by, $sort_order)     
         {            		
	      ...
	      $seq1 = "tc001, tc002, tc027, tc004, tc005, tc006, tc007, tc008, tc009, tc010, tc013, create_date FROM `asttc` ";
		...
		if(@$_SESSION['asti10_sql_term']){$seq32 = $_SESSION['asti10_sql_term'];}
		if(@$_SESSION['asti10_sql_sort']){$seq33 = $_SESSION['asti10_sql_sort'];}
		// if ($seq32==null) {$seq32 =" and a.create_date >='0' ";}
         $sort_order = (substr($sort_order,0,3) == 'asc') ? 'asc' : 'desc';
	     $sort_columns = array('a.tc001', 'b.mq002', ' a.tc002', 'a.tc027','a.tc004', 'c,mb002', 'a.tc005','a.tc006', 'a.tc007', 'a.tc008', 'a.tc009', 'a.tc010', 'a,tc013','a.create_date');
	     $sort_by = (in_array($sort_by, $sort_columns)) ? $sort_by : 'mb001';  //檢查排序欄位是否在 table 內
	     $query = $this->db->select('a.tc001, b.mq002 as tc001disp, a.tc002, a.tc027, a.tc004, c.mb002 as tc004disp, a.tc005, a.tc006, a.tc007, a.tc008, a.tc009, a.tc010, a.tc013, a.create_date')
	                       ->from('asttc as a')
						   ->join('cmsmq as b', 'a.tc001 = b.mq001','left')
						   ->join('astmb as c', 'a.tc004 = c.mb001','left')
						   ->where('b.mq003','C5')
	...
	function insertf()    //新增一筆 檔頭  asttc
        {
		    //刪日期 / 符號
			 ...
			 $tc001=$this->input->post('asti03_asti10');
		...
		//      $vtd003='1010';   //流水號重新排序
		   foreach($order_product as $key => $val){
		        if($val['asti02_asti10'] && $val['asti02_asti10_mc003']){
				        extract($val);
						...
						$data = array( 
							...
							'td002' => $tc002,
							'td003' => $asti02_asti10,
							'td004' => $asti02_asti10_mc003,
							'td010' => $body_td010
						);
						foreach($val as $k=>$v){
							if($k!="asti02_asti10"&&$k!="asti02_asti10_mc002disp"&&$k!="asti02_asti10_mc003"&&$k!="asti02_asti10_mc003disp"){//主鍵不用更改以及其他外來鍵庫別名稱
							    $data[$k] = $v;
							}
						}
	...
	function updatef()   
        {
          ...
          $data = array(			
			...
			foreach($order_product as $key => $val){
				...
				//preg_match_all('/\d/S',$td013, $matches);  //處理日期字串
			    //$td013 = implode('',$matches[0]);
				if($this->seldetail($tc001,$tc002,$val['asti02_asti10'],$val['asti02_asti10_mc003'])>0){
					$data = array(
						'modifier' => $this->session->userdata('manager'),
						'modi_date' => date("Ymd"),
						'td003' => $asti02_asti10,
					...
					foreach($val as $k=>$v){
						if($k!="asti02_asti10"&&$k!="asti02_asti10_mc002disp"&&$k!="asti02_asti10_mc003"&&$k!="asti02_asti10_mc003disp"){//主鍵不用更改以及其他外來鍵庫別名稱 td013日期等別處理
							$data[$k] = $v;
						}
					}
					$this->db->where('td001', $tc001);
					$this->db->where('td002', $tc002);
					$this->db->where('td003', $val['asti02_asti10']);
					$this->db->where('td004', $val['asti02_asti10_mc003']);
