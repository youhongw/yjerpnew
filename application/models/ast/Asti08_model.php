...
class Asti08_model extends CI_Model {
	...
	function construct_sql($limit = 15, $offset = 0, $func = "")
	  {
		...
		if(isset($_SESSION['asti08']['search']['where'])){
			if($where){$where .= " and ";}
			$where .= $_SESSION['asti08']['search']['where'];
		}
		...
		if(isset($_SESSION['asti08']['search']['order'])){
			if($order){$order .= " , ";}
			$order .= $_SESSION['asti08']['search']['order'];
		}
		
		if(!isset($_SESSION['asti08']['search']['order']) && $default_order){
			if($order){$order .= " , ";}
			$order .= $default_order;
		}
		/* order end */
		...
		$ret['data'] = $query->get()->result();
		//儲存sql 語法
		$_SESSION['asti08']['search']['sql'] = $this->db->last_query();
		...
		//儲存where與order
		$_SESSION['asti08']['search']['where'] = $where;
		$_SESSION['asti08']['search']['order'] = $order;
		$_SESSION['asti08']['search']['offset'] = $offset;
	...
	function construct_view($data){
		...
		$_SESSION['asti08']['search']['view'] = $view_array;
		$_SESSION['asti08']['search']['index'] = $index_array;
	...
	function construct_sql_body($limit = 15, $offset = 0, $func = "")
	{
		...
		if(isset($_SESSION['asti08_body']['search']['where'])){
			if($where){$where .= " and ";}
			$where .= $_SESSION['asti08_body']['search']['where'];
		}
		...
		if(isset($_SESSION['asti08_body']['search']['order'])){
			if($order){$order .= " , ";}
			$order .= $_SESSION['asti08_body']['search']['order'];
		}
		
		if(!isset($_SESSION['asti08_body']['search']['order']) && $default_order){
			if($order){$order .= " , ";}
			$order .= $default_order;
		}
		/* order end */
		...
		$ret['data'] = $query->get()->result();
		//儲存sql
		$_SESSION['asti08_body']['search']['sql'] = $this->db->last_query();
		...
		//儲存where與order
		$_SESSION['asti08_body']['search']['where'] = $where;
		$_SESSION['asti08_body']['search']['order'] = $order;
		$_SESSION['asti08_body']['search']['offset'] = $offset;
	...
	function selone($seg1, $seg2) {
		...
		$this->db->select('b.td003 as asti02_asti08, b.td004 as asti02_asti08_mc003, b.td005, b.td008, b.td010, e.me002 as asti02_asti08_mc002disp, f.mv002 as asti02_asti08_mc003disp')
			->from('asttd as b')
	...
	function findf($limit, $offset, $sort_by, $sort_order)     
		...
		if(@$_SESSION['asti08_sql_term']){$seq32 = $_SESSION['asti08_sql_term'];}
		if(@$_SESSION['asti08_sql_sort']){$seq33 = $_SESSION['asti08_sql_sort'];}
	...
	function insertf()    //新增一筆 檔頭  asttc
        {
		    //刪日期 / 符號
			 ...
			 $tc001=$this->input->post('asti03_asti08');
			 $tc002=$this->input->post('tc002');
			 if (!isset($body_td010)) {$body_td010='';} else  {$body_td010=$this->input->post('td010');}
		...
		//      $vtd003='1010';   //流水號重新排序
		   foreach($order_product as $key => $val){
		        if($val['asti02_asti08'] && $val['asti02_asti08_mc003']){
				        extract($val);
						...
						$data = array( 
							...
							'td002' => $tc002,
							'td003' => $asti02_asti08,
							'td004' => $asti02_asti08_mc003,
							'td010' => $body_td010
						);
						foreach($val as $k=>$v){
							if($k!="asti02_asti08"&&$k!="asti02_asti08_mc002disp"&&$k!="asti02_asti08_mc003"&&$k!="asti02_asti08_mc003disp"){//主鍵不用更改以及其他外來鍵庫別名稱
							    $data[$k] = $v;
							}
						}
	...
	function printfd()          
        {
	      ...
	      $seq4=$this->input->post('tc002c');
		  if (!isset($seq1)) {$seq1='';}
		  if (!isset($seq2)) {$seq2='zz';}
		  if (!isset($seq3)) {$seq3='';}
		  if (!isset($seq4)) {$seq4='zz';}
	...
	function updatef()   
        {
          ...
          $data = array(			
			...
			foreach($order_product as $key => $val){
				...
				//preg_match_all('/\d/S',$td013, $matches);  //處理日期字串
			    //$td013 = implode('',$matches[0]);
				if($this->seldetail($tc001,$tc002,$val['asti02_asti08'],$val['asti02_asti08_mc003'])>0){
					$data = array(
						'modifier' => $this->session->userdata('manager'),
						'modi_date' => date("Ymd"),
						'td003' => $asti02_asti08,
					...
					foreach($val as $k=>$v){
						if($k!="asti02_asti08"&&$k!="asti02_asti08_mc002disp"&&$k!="asti02_asti08_mc003"&&$k!="asti02_asti08_mc003disp"){//主鍵不用更改以及其他外來鍵庫別名稱 td013日期等別處理
							$data[$k] = $v;
						}
					}
					$this->db->where('td001', $tc001);
					$this->db->where('td002', $tc002);
					$this->db->where('td003', $val['asti02_asti08']);
					$this->db->where('td004', $val['asti02_asti08_mc003']);
