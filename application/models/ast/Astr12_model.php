...
class astr12_model extends CI_Model {
	...
	function insertf()    //新增一筆 檔頭  astmb
		...
		//	 if ($this->input->post('mb018')!='1') {$mb019=round($this->input->post('mb019'));}
	     ...
	     $data = array( 
	      ...
	      $exist = $this->astr12_model->selone1($this->input->post('copq03a23'),$this->input->post('mb002'));
	      if ($exist)
	         {
		      return 'exist';
		     } 
		...
		//	while ($_POST['order_product'][  $n  ]['th004']) {		
	     ...
	     $exist = $this->astr12_model->selone1d($this->input->post('copq03a23'),$this->input->post('mb002'),$th003);
		...
		if ($_POST['order_product'][  $n  ]['th004']!='') {
		  ...
		  // 訂單已交數量
			 ...
			 $th016=$_POST['order_product'][ $n  ]['th016'];
			 $th008=$_POST['order_product'][ $n  ]['th008'];
			 $existb = $this->astr12_model->selone3d($th014,$th015,$th016);
			  $data_add = array( 
	             'company' => $this->session->userdata('syscompany'),
	             'creator' => $this->session->userdata('manager'),
		         'usr_group' => 'A100',
		 ...
		 $query = $this->db->query($sql);	} 
		  ...
		  //庫存增加減少 品號,庫別, 數量
			 ...
			 $th007=$_POST['order_product'][ $n  ]['th007'];
			 $th008=$_POST['order_product'][ $n  ]['th008'];
			 $exista = $this->astr12_model->selone2d($th004,$th007);
			  $data_add = array( 
	             'company' => $this->session->userdata('syscompany'),
	             'creator' => $this->session->userdata('manager'),
		         'usr_group' => 'A100',
    ...
    function copyf()           
        {
         ...
         //   if ($query->num_rows() != 1) { return 'exist'; }
            ...
            $exist = $this->astr12_model->selone2($this->input->post('mb001c'),$this->input->post('mb002c'));
		    if ($exist)
		      {
			    return 'exist';
		      }         
