...
class Asti07_model extends CI_Model {
	...
	function construct_sql($limit = 15, $offset = 0, $func = "")
	  {
		...
		if(isset($_SESSION['asti07']['search']['where'])){
			if($where){$where .= " and ";}
			$where .= $_SESSION['asti07']['search']['where'];
		}
		...
		if(isset($_SESSION['asti07']['search']['order'])){
			if($order){$order .= " , ";}
			$order .= $_SESSION['asti07']['search']['order'];
		}
		
		if(!isset($_SESSION['asti07']['search']['order']) && $default_order){
			if($order){$order .= " , ";}
			$order .= $default_order;
		}
		/* order end */
		...
		$ret['data'] = $query->get()->result();
		//儲存sql 語法
		$_SESSION['asti07']['search']['sql'] = $this->db->last_query();
		...
		//儲存where與order
		$_SESSION['asti07']['search']['where'] = $where;
		$_SESSION['asti07']['search']['order'] = $order;
		$_SESSION['asti07']['search']['offset'] = $offset;
	...
	function construct_view($data){
		...
		$_SESSION['asti07']['search']['view'] = $view_array;
		$_SESSION['asti07']['search']['index'] = $index_array;
	...
	function findf($limit, $offset, $sort_by, $sort_order)     
         {            		
	      ...
	      $seq1 = "tc001, tc002, tc004, tc006, tc027, create_date FROM `asttc` ";
		...
		if(@$_SESSION['asti07_sql_term']){$seq32 = $_SESSION['asti07_sql_term'];}
		if(@$_SESSION['asti07_sql_sort']){$seq33 = $_SESSION['asti07_sql_sort'];}
		
         $sort_order = (substr($sort_order,0,3) == 'asc') ? 'asc' : 'desc';
	     $sort_columns = array('a.tc001', 'b.mq002', 'a.tc002', 'a.tc027','a.tc004', 'c.mb002', 'a.tc006','a.create_date');
	     $sort_by = (in_array($sort_by, $sort_columns)) ? $sort_by : 'tc001';  //檢查排序欄位是否在 table 內
	     $query = $this->db->select('a.tc001,b.mq002, a.tc002, a.tc027, a.tc004, c.mb002, a.tc006,a.tc007, a.create_date')
	                       ->from('asttc as a')
						   ->join('cmsmq as b', 'a.tc001 = b.mq001','left')
						   ->join('astmb as c', 'a.tc004 = c.mb001','left')
		                   ->where($seq32)
	...
	function insertf()    //新增一筆 檔頭  coptc
        {
		    //刪日期 / 符號
			 ...
			 $tc001=$this->input->post('asti03_asti07');
			 $tc002=$this->input->post('tc002');
			 $tc002no=$tc002;   //明細用再新增一筆時加1
			 //檢查資料是否已存在 若存在加1
			  while($this->asti07_model->selone1($tc001,$tc002)>0){
	...
	function excelnewf()           
        {			
	      ...
	      $sql = " SELECT a.tc001, b.mq002 as tc001disp, a.tc002, a.tc027, a.tc004, c.mb002 as tc004disp, c.mb003 as tc004disp2, a.tc006, a.tc010, a.tc023, a.tc015, a.tc013
		  FROM asttc as a,cmsmq as b,astmb as c WHERE a.tc001=b.mq001 and a.tc004=c.mb001 and b.mq003='C2' and tc001 >= '$seq1'  AND tc001 <= '$seq2' AND  tc002 >= '$seq3'  AND tc002 <= '$seq4'  "; 
	...
	function printfd()          
        {
		  ...
		  WHERE a.tc001 = b.mq001 and a.tc004 = c.mb001 and b.mq003 = 'C2' and tc001 >= '$seq1'  AND tc001 <= '$seq2' AND tc002 >= '$seq3'  AND tc002 <= '$seq4'  "; 
          $query = $this->db->query($sql);
	      $ret['rows'] = $query->result();
