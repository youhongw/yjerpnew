...
class Asti12_model extends CI_Model {
	...
	function construct_sql($limit = 15, $offset = 0, $func = "")
	  {
		...
		if(isset($_SESSION['asti12']['search']['where'])){
			if($where){$where .= " and ";}
			$where .= $_SESSION['asti12']['search']['where'];
		}
		...
		if(isset($_SESSION['asti12']['search']['order'])){
			if($order){$order .= " , ";}
			$order .= $_SESSION['asti12']['search']['order'];
		}
		
		if(!isset($_SESSION['asti12']['search']['order']) && $default_order){
			if($order){$order .= " , ";}
			$order .= $default_order;
		}
		...
		$ret['data'] = $query->get()->result();
		//儲存sql 語法
		$_SESSION['asti12']['search']['sql'] = $this->db->last_query();
		...
		//儲存where與order
		$_SESSION['asti12']['search']['where'] = $where;
		$_SESSION['asti12']['search']['order'] = $order;
		$_SESSION['asti12']['search']['offset'] = $offset;
		
		return $ret;
	}
	
	function construct_sql_body_asti14($limit = 15, $offset = 0, $func = "")
	{
		$this->session->set_userdata('asti12_asti14_search',"display_search/".$offset);
		if (session_status() == PHP_SESSION_NONE) {
			session_start();
		}
		...
		$default_where = "";//在這裡塞入一些預設條件，如不顯示離職員工等等
		$default_order = "tf001 asc";//在這裡塞入一些預設排序
		...
		if(isset($_SESSION['asti12_asti14_body']['search']['where'])){
			if($where){$where .= " and ";}
			$where .= $_SESSION['asti12_asti14_body']['search']['where'];
		}
		...
		if(isset($_SESSION['asti12_asti14_body']['search']['order'])){
			if($order){$order .= " , ";}
			$order .= $_SESSION['asti12_asti14_body']['search']['order'];
		}
		
		if(!isset($_SESSION['asti12_asti14_body']['search']['order']) && $default_order){
			if($order){$order .= " , ";}
			$order .= $default_order;
		}
		/* order end */
		...
		/* Data SQL */
		$query = $this->db->select('a.tf001, a.tf002, a.tf003, b.te008, a.tf004, a.tf005, a.tf006, a.tf008,c.mb002,c.mb003,e.me002,f.mv002')
			->from('asttf as a')
			->join('astte as b','a.tf001 = b.te001 and a.tf002 = b.te002','left')
			->join('astmb as c','a.tf003 = c.mb001','left')
			->join('cmsme as e','a.tf004 = e.me001','left')
			->join('cmsmv as f','a.tf005 = f.mv001','left')
			->where('b.te005','C7')
			->order_by($order);
	 ...
	 //	echo "<pre>";var_dump($this->db);exit;
		$ret['data'] = $query->get()->result();
		//儲存sql
		$_SESSION['asti12_asti14_body']['search']['sql'] = $this->db->last_query();
		...
		//儲存where與order
		$_SESSION['asti12_asti14_body']['search']['where'] = $where;
		$_SESSION['asti12_asti14_body']['search']['order'] = $order;
		$_SESSION['asti12_asti14_body']['search']['offset'] = $offset;
	...
	function construct_view($data){
		...
		$_SESSION['asti12']['search']['view'] = $view_array;
		$_SESSION['asti12']['search']['index'] = $index_array;
	...
	function construct_sql_body($limit = 15, $offset = 0, $func = "")
	{
		...
		if(isset($_SESSION['asti12_body']['search']['where'])){
			if($where){$where .= " and ";}
			$where .= $_SESSION['asti12_body']['search']['where'];
		}
		...
		if(isset($_SESSION['asti12_body']['search']['order'])){
			if($order){$order .= " , ";}
			$order .= $_SESSION['asti12_body']['search']['order'];
		}
		
		if(!isset($_SESSION['asti12_body']['search']['order']) && $default_order){
			if($order){$order .= " , ";}
			$order .= $default_order;
		}
		/* order end */
		...
		$ret['data'] = $query->get()->result();
		//儲存sql
		$_SESSION['asti12_body']['search']['sql'] = $this->db->last_query();
		...
		//儲存where與order
		$_SESSION['asti12_body']['search']['where'] = $where;
		$_SESSION['asti12_body']['search']['order'] = $order;
		$_SESSION['asti12_body']['search']['offset'] = $offset;
	...
	function selone($seg1, $seg2) {
		...
		$this->db->select('c.*,c.tf003 as asti02,c.tf004 as asti02_asti12,c.tf005 as asti02_asti12_mc003,
		              d.mb002 as asti02_mb002,d.mb003 as asti02_mb003,c.tf104 as asti02a_asti12,c.tf105 as asti02a_asti12_mc003,
		              e.me002 as asti02_asti12_mc002disp, f.mv002 as asti02_asti12_mc003disp,g.me002 as asti02a_asti12_mc002disp, h.mv002 as asti02a_asti12_mc003disp,
					  c.tf007 as puri01_ma002,c.tf107 as puri01a_ma002') 
			->from('asttf as c')
			->join('astmb as d','c.tf003 = d.mb001','left')
			->join('cmsme as e','c.tf004 = e.me001','left')
	    	->join('cmsmv as f','c.tf005 = f.mv001','left')
			->join('cmsme as g','c.tf104 = g.me001','left')
	    	->join('cmsmv as h','c.tf105 = h.mv001','left')
			->where('c.tf001', $seg1)
			->where('c.tf002', $seg2);
		$query = $this->db->get();
	...
	function findf($limit, $offset, $sort_by, $sort_order)     
         {            		
          ...
          if (trim($this->input->post('find005'))!='' || $this->input->post('find005')!=null)
		    {
		...
		if(@$_SESSION['asti12_sql_term']){$seq32 = $_SESSION['asti12_sql_term'];}
		if(@$_SESSION['asti12_sql_sort']){$seq33 = $_SESSION['asti12_sql_sort'];}
	...
	function insertf()    //新增一筆 檔頭  asttc
        {
		    //刪日期 / 符號
			 ...
			 $te001=$this->input->post('asti03_asti12');
		...
		//      $vtd003='1010';   //流水號重新排序
		   foreach($order_product as $key => $val){
		        if($val['asti02'] && $val['asti02_asti12'] && $val['asti02_asti12_mc003']){
				        extract($val);
						...
						$data = array( 
							...
							'tf003' => $asti02,
							'tf004' => $asti02_asti12,
							'tf005' => $asti02_asti12_mc003,
							'tf006' => $tf006,
							'tf007' => $puri01_ma002,
							'tf008' => $tf008,
							'tf009' => $tf009,
							'tf104' => $asti02a_asti12,
							'tf105' => $asti02a_asti12_mc003,
							'tf107' => $puri01a_ma002
						);
						foreach($val as $k=>$v){
							if($k!="asti02"&&$k!="asti02_asti12"&&$k!="asti02_asti12_mc003"){//主鍵不用更改以及其他外來鍵庫別名稱
							  //  $data[$k] = $v;
							}
						}
						
					$this->db->insert('asttf', $data);
	...
	function excelnewf()           
        {			
	      ...
	      $this->db->select('a.te001,a.te002,a.te008,c.tf003,d.mb002,d.mb003,e.me002,f.mv002,c.tf006,c.tf008') 
					->from('astte as a')
					->join('cmsmq as b','a.te001 = b.mq001','left')
					->join('asttf as c','a.te001 = c.tf001 and a.te002 = c.tf002','left')
					->join('astmb as d','c.tf003 = d.mb001','left')
					->join('cmsme as e','c.tf004 = e.me001','left')
					->join('cmsmv as f','c.tf005 = f.mv001','left')
					->join('purma as g','c.tf007 = g.ma002','left')
					->where('a.te001 >=',$seq1)
					->where('a.te001 <=',$seq2)
					->where('a.te002 >=',$seq3)
					->where('a.te002 <=',$seq4);
	...
	function printfd()          
        {
		  ...
		  $this->db->select('a.*,b.mq002 as te001disp,c.tf001 ,c.tf002, c.tf003, d.mb001 as asti02, d.mb002 as asti02_mb002, d.mb003 as asti02_mb003, c.tf004 as asti02_asti12, e.me002 as asti02_asti12_mc002disp,
							 c.tf005 as asti02_asti12_mc003, f.mv002 as asti02_asti12_mc003disp, g.ma002 as puri01_ma002, c.tf006, c.tf007, c.tf008, c.tf009, c.tf010, c.tf011 ,c.tf012') 
					->from('astte as a')
					->join('cmsmq as b','a.te001 = b.mq001','left')
					->join('asttf as c','a.te001 = c.tf001 and a.te002 = c.tf002','left')
					->join('astmb as d','c.tf003 = d.mb001','left')
	...
	function printfc()   
      {           				
        ...
        $this->db->from('astte as a');
		$this->db->join('cmsmq as b','a.te001 = b.mq001','left');
		$this->db->join('asttf as c','a.te001 = c.tf001 and a.te002 = c.tf002','left');
		$this->db->join('astmb as d','c.tf003 = d.mb001','left');
		$this->db->join('cmsme as e','c.tf004 = e.me001','left');
		$this->db->join('cmsmv as f','c.tf005 = f.mv001','left');
		$this->db->join('purma as g','c.tf007 = g.ma001','left');
		...
		$this->db->order_by('te001 , te002 ,c.tf003');
	...
	function updatef()   
        {
          ...
          $data = array(			
			...
			if(!is_array($order_product)){$order_product=array();}
		            ...
		            $this->db->where('tf002', $te002);
					$this->db->delete('asttf'); //刪除明細 1060809
					
		    //$vtd003='1010';   //流水號重新排序
			foreach($order_product as $key => $val){
				extract($val);
				
				if($this->seldetail($te001,$te002,$val['asti02'],$val['asti02_asti12'],$val['asti02_asti12_mc003'])>0){
					$data = array(
						'modifier' => $this->session->userdata('manager'),
						'modi_date' => date("Ymd"),
						    'tf003' => $asti02,
				...
				//	echo "<pre>";var_dump($data);exit;
					$this->db->where('tf001', $te001);
					$this->db->where('tf002', $te002);
					$this->db->where('tf003', $val['asti02']);
				//	$this->db->where('tf004', $val['asti02_asti12']);
				//	$this->db->where('tf005', $val['asti02_asti12_mc003']);
					$this->db->update('asttf',$data);//更改一筆

					//$mtd003 = (int) $vtd003+10;
			        //$vtd003 =  (string)$mtd003; && $val['asti02_asti12'] && $val['asti02_asti12_mc003']
				}else{
					if($val['asti02'] ){
						$data = array( 
							'company' => $this->session->userdata('syscompany'),
	...
	function check_tf006($tf006){
		$this->db->select('mc004')
				->from('astmc')
				->where('mc001');
	}
}
