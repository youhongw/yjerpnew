...
class Asti11_model extends CI_Model {
	...
	function construct_sql($limit = 15, $offset = 0, $func = "")
	  {
		...
		if(isset($_SESSION['asti11']['search']['where'])){
			if($where){$where .= " and ";}
			$where .= $_SESSION['asti11']['search']['where'];
		}
		...
		if(isset($_SESSION['asti11']['search']['order'])){
			if($order){$order .= " , ";}
			$order .= $_SESSION['asti11']['search']['order'];
		}
		
		if(!isset($_SESSION['asti11']['search']['order']) && $default_order){
			if($order){$order .= " , ";}
			$order .= $default_order;
		}
		/* order end */
		...
		/* Data SQL */
		$query = $this->db->select('a.*, b.mq002 as tc001disp, c.mb002 as tc004disp')
	                       ->from('asttc as a')
						   ->join('cmsmq as b','a.tc001 = b.mq001','left')
						   ->join('astmb as c','a.tc004 = c.mb001','left')
						   ->where('b.mq003','C3')
			               ->order_by($order);
		if($where){
			$query->where($where);
		...
		$ret['data'] = $query->get()->result();
		//儲存sql 語法
		$_SESSION['asti11']['search']['sql'] = $this->db->last_query();
		...
		//儲存where與order
		$_SESSION['asti11']['search']['where'] = $where;
		$_SESSION['asti11']['search']['order'] = $order;
		$_SESSION['asti11']['search']['offset'] = $offset;
	...
	function construct_view($data){
		...
		$_SESSION['asti11']['search']['view'] = $view_array;
		$_SESSION['asti11']['search']['index'] = $index_array;
		//echo "<pre>";var_dump($_SESSION['asti11']['search']['view']);exit;
	}
	...
	function construct_sql_body($limit = 15, $offset = 0, $func = "")
	{
		...
		if(isset($_SESSION['asti11_body']['search']['where'])){
			if($where){$where .= " and ";}
			$where .= $_SESSION['asti11_body']['search']['where'];
		}
		...
		if(isset($_SESSION['asti11_body']['search']['order'])){
			if($order){$order .= " , ";}
			$order .= $_SESSION['asti11_body']['search']['order'];
		}
		
		if(!isset($_SESSION['asti11_body']['search']['order']) && $default_order){
			if($order){$order .= " , ";}
			$order .= $default_order;
		}
		/* order end */
		...
		$ret['data'] = $query->get()->result();
		//儲存sql
		$_SESSION['asti11_body']['search']['sql'] = $this->db->last_query();
		...
		//儲存where與order
		$_SESSION['asti11_body']['search']['where'] = $where;
		$_SESSION['asti11_body']['search']['order'] = $order;
		$_SESSION['asti11_body']['search']['offset'] = $offset;
	...
	function selone($seg1, $seg2) {
		$this->db->select('a.*,c.mq002 as tc001disp,d.mb002 as tc004disp, d.mb003 as tc004disp2, d.mb011 as tc004disp3');
        $this->db->from('asttc as a');		
		$this->db->join('cmsmq as c','a.tc001 = c.mq001','left');
		$this->db->join('astmb as d','a.tc004 = d.mb001','left');
		...
		$this->db->select('b.td003 as asti02_asti11, b.td004 as asti02_asti11_mc003, b.td005, b.td008, b.td010, e.me002 as asti02_asti11_mc002disp, f.mv002 as asti02_asti11_mc003disp')
			->from('asttd as b')
			->join('cmsme as e','b.td003 = e.me001','left')
			->join('cmsmv as f','b.td004 = f.mv001','left')
			->where('b.td001', $seg1)
			->where('b.td002', $seg2);
		$query = $this->db->get();
	...
	function findf($limit, $offset, $sort_by, $sort_order)     
         {            		
	      ...
	      $seq1 = "tc001, tc002, tc003, tc004, tc005, tc006, tc007, tc008, tc010, tc013, create_date FROM `asttc` ";
		...
		if(@$_SESSION['asti11_sql_term']){$seq32 = $_SESSION['asti11_sql_term'];}
		if(@$_SESSION['asti11_sql_sort']){$seq33 = $_SESSION['asti11_sql_sort'];}
		
         $sort_order = (substr($sort_order,0,3) == 'asc') ? 'asc' : 'desc';
	     $sort_columns = array('a.tc001', 'b.mq002', ' a.tc002', 'a.tc003','a.tc004', 'c,mb002', 'a.tc005','a.tc006', 'a,tc007', 'a,tc008', 'a,tc010', 'a,tc013','a.create_date');
	     $sort_by = (in_array($sort_by, $sort_columns)) ? $sort_by : 'mb001';  //檢查排序欄位是否在 table 內
	     $query = $this->db->select('a.tc001, b.mq002 as tc001disp, a.tc002, a.tc003, a.tc004, c.mb002 as tc004disp, a.tc005, a.tc006, a.tc007, a.tc008, a.tc010, a.tc013, a.create_date')
	                       ->from('asttc as a')
						   ->join('cmsmq as b', 'a.tc001 = b.mq001','left')
						   ->join('astmb as c', 'a.tc004 = c.mb001','left')
						   ->where('b.mq003','C3')
	...
	function insertf()    //新增一筆 檔頭  asttc
        {
		    //刪日期 / 符號
			 ...
			 preg_match_all('/\d/S',$this->input->post('tc027'), $matches);  //處理日期字串
			 $tc027 = implode('',$matches[0]);
			 
			 $tc001=$this->input->post('asti03_asti11');
			 $tc002=$this->input->post('tc002');
			 $body_td010=$this->input->post('asti11');
			 //$tc002no=$tc002;   //明細用再新增一筆時加1
			 //檢查資料是否已存在 若存在加1
			//  while($this->asti11_model->selone1($tc001,$tc002)>0){
	     ...
	     $data = array( 
		         ...
		         'tc002' => $tc002,
		         'tc003' => $tc003,
		         'tc004' => $this->input->post('asti02'),  //資產編號
		...
		//      $vtd003='1010';   //流水號重新排序
		   foreach($order_product as $key => $val){
		        if($val['asti02_asti11'] && $val['asti02_asti11_mc003']){
				        extract($val);
						...
						$data = array( 
							...
							'td002' => $tc002,
							'td003' => $asti02_asti11,
							'td004' => $asti02_asti11_mc003,
							'td010' => $body_td010
						);
						foreach($val as $k=>$v){
							if($k!="asti02_asti11"&&$k!="asti02_asti11_mc002disp"&&$k!="asti02_asti11_mc003"&&$k!="asti02_asti11_mc003disp"){//主鍵不用更改以及其他外來鍵庫別名稱
							    $data[$k] = $v;
							}
						}
					$this->db->insert('asttd', $data);
    ...
    function astyf()           
        {
         ...
         //   if ($query->num_rows() != 1) { return 'exist'; }
            ...
            $exist = $this->asti11_model->selone1($seq1,$seq2);  //檢查單頭是否重複
		    if ($exist)
		      {
			    return 'exist';
		      }         
	...
	function excelnewf()           
        {			
	      ...
	      $this->db->select('a.tc001, c.mq002 as tc001disp, a.tc002, a.tc014, a.tc027, a.tc004, d.mb002 as tc004disp, d.mb003 as tc004disp2, b.td005, a.tc008, e.me002 as td003disp, f.mv002 as td004disp, b.td007')
					->from('asttc as a')
					->join('asttd as b','a.tc001 = b.td001 and a.tc002 = b.td002','left')
					->join('cmsmq as c','a.tc001 = c.mq001','left')
					->join('astmb as d','a.tc004 = d.mb001','left')
					->join('cmsme as e','b.td003 = e.me001','left')
					->join('cmsmv as f','b.td004 = f.mv001','left')
					->where('a.tc001 >= '.$seq1.' and a.tc001 <= '.$seq2)
					->where('c.mq003','C3')
					->where('a.tc002 >= '.$seq3.' and a.tc002 <= '.$seq4); 
		  $query = $this->db->get();
	...
	function printfd()          
        {
		  ...
		  $seq3=$this->input->post('tc002o');    
	      $seq4=$this->input->post('tc002c');
	      $sql = " SELECT a.*, b.*, c.mq002 as tc001disp, d.mb002 as tc004disp, d.mb003 as tc004disp2, e.me002 as td003disp, f.mv002 as td004disp
		  FROM asttc as a,asttd as b,cmsmq as c, astmb as d, cmsme as e, cmsmv as f
		  WHERE a.tc001=b.td001 and a.tc002=b.td002 and a.tc001=c.mq001 and a.tc004 = d.mb001 and b.td003=e.me001 and b.td004=f.mv001 and c.mq003 = 'C3' and tc001 >= '$seq1'  AND tc001 <= '$seq2' AND tc002 >= '$seq3'  AND tc002 <= '$seq4' ORDER BY `a`.`tc002` ASC "; 
          $query = $this->db->query($sql);
	      $ret['rows'] = $query->result();
		  
          $seq32 = "a.tc001 >= '$seq1'  AND a.tc001 <= '$seq2' AND a.tc002 >= '$seq3'  AND a.tc002 <= '$seq4'  ";	
	      $query = $this->db->select('COUNT(*) as count', FALSE)  //筆數查詢,如果設為FALSE不會使用反引號保護你的字段或者表名
		              ->from('asttc as a')
					  ->join('cmsmq as b','a.tc001 = b.mq001','left')
		              ->where('b.mq003','C3')
	...
	function printfc()   
      {           
        $this->db->select('a.*, b.*, c.mq002 as tc001disp, d.mb002 as tc004disp, d.mb003 as tc004disp2, d.mb004 as tc004disp3, e.me002 as td003disp, f.mv002 as td004disp');
        $this->db->from('asttc as a');	
        $this->db->join('asttd as b', 'a.tc001 = b.td001  and a.tc002=b.td002 ','left');	//單身	
		$this->db->join('cmsmq as c', 'a.tc001 = c.mq001','left');  //單據性質	
		$this->db->join('astmb as d', 'a.tc004 = d.mb001','left');  //資產編號	
		$this->db->join('cmsme as e', 'b.td003 = e.me001','left');  //部門	
		$this->db->join('cmsmv as f', 'b.td004 = f.mv001','left');  //保管人	
		$this->db->where('a.tc001', $this->input->post('tc001o')); 
		$this->db->where('c.mq003', 'C3'); 
	...
	function updatef()   
        {
			...
			$tc001=$this->input->post('asti03_asti11');
			 $tc002=$this->input->post('tc002');
			 $body_td010=$this->input->post('asti02');
          $data = array(			
            ...
            $this->db->update('asttc',$data);                   //更改一筆
			...
			if(!is_array($order_product)){$order_product=array();}
					...
					$this->db->delete('asttd'); //刪除明細 1060809
			...
			foreach($order_product as $key => $val){
				...
				//preg_match_all('/\d/S',$td013, $matches);  //處理日期字串
			    //$td013 = implode('',$matches[0]);
				if($this->seldetail($tc001,$tc002,$val['asti02_asti11'],$val['asti02_asti11_mc003'])>0){
					$data = array(
						'modifier' => $this->session->userdata('manager'),
						'modi_date' => date("Ymd"),
						'td003' => $asti02_asti11,
					...
					foreach($val as $k=>$v){
						if($k!="asti02_asti11"&&$k!="asti02_asti11_mc002disp"&&$k!="asti02_asti11_mc003"&&$k!="asti02_asti11_mc003disp"){//主鍵不用更改以及其他外來鍵庫別名稱 td013日期等別處理
							$data[$k] = $v;
						}
					}
					$this->db->where('td001', $tc001);
					$this->db->where('td002', $tc002);
					$this->db->where('td003', $val['asti02_asti11']);
					$this->db->where('td004', $val['asti02_asti11_mc003']);
					$this->db->update('asttd',$data);//更改一筆
					//$mtd003 = (int) $vtd003+10;
			        //$vtd003 =  (string)$mtd003;
				}else{
					if($val['asti02_asti11'] && $val['asti02_asti11_mc003']){
	...
	//取單號 最大值加1
	function check_title_no($asti03_asti11,$tc027){
		preg_match_all('/\d/S',$tc027, $matches);  //處理日期字串
		$tc027 = implode('',$matches[0]);
		$this->db->select('MAX(tc002) as max_no')
			->from('asttc')
			->where('tc001', $asti03_asti11)
		//	->where('tc039', $tc039);
			->like('tc027', $tc027, "after");
			
		$query = $this->db->get();
		$result = $query->result();
		
	    if (!$result[0]->max_no){return $tc027."001";}
		
		return $result[0]->max_no+1;
	}
	
	//建構SQL字串 新增純粹以sql做查詢的方法
	function sql_printchart($limit = 5, $offset = 0, $func = "")
	{
		$this->db->select('tc001,tc002,tc005')
				->from('asttc')
				->order_by('tc005','DESC')
		        ->limit(5)
				->where('tc014','C3');
