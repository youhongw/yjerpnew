...
class Asti01_model extends CI_Model {
	...
	function construct_sql($limit = 15, $offset = 0, $func = "")
	  {
		...
		$default_order = "ma001 asc,ma002 desc";//在這裡塞入一些預設排序
	...
	function selone($seg1) {
		$this->db->select('a.*, b.ma003 as acti03disp, c.ma003 as acti03adisp, d.ma003 as acti03bdisp');
	...
	function selone_old($seq1,$seq2)  { 
		$this->db->select('a.*, a.ma003 as acti03disp, a.ma004 as acti03adisp, a.ma005 as acti03bdisp');
        $this->db->from('astma as a');	
	...
	function insertf()    //新增一筆 檔頭  coptc
        {
		    //刪日期 / 符號
			 ...
			 //檢查資料是否已存在 若存在加1
			  //while($this->asti01_model->selone1($tc001,$tc002)>0){
	     ...
	     $data = array( 
	         ...
	         $exist = $this->asti01_model->CheckRepeat($ma001);   //查詢資料是否重複
	        if ($exist) {
		     return 'exist';
		    } 
             $this->db->insert('astma', $data);
