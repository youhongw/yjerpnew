...
class Asti09_model extends CI_Model {
	...
	function construct_sql($limit = 15, $offset = 0, $func = "")
	  {
		...
		if(isset($_SESSION['asti09']['search']['where'])){
			if($where){$where .= " and ";}
			$where .= $_SESSION['asti09']['search']['where'];
		}
		...
		if(isset($_SESSION['asti09']['search']['order'])){
			if($order){$order .= " , ";}
			$order .= $_SESSION['asti09']['search']['order'];
		}
		
		if(!isset($_SESSION['asti09']['search']['order']) && $default_order){
			if($order){$order .= " , ";}
			$order .= $default_order;
		}
		/* order end */
		...
		/* Data SQL */
		$query = $this->db->select('a.*, b.mq002 as tc001disp, c.mb002 as tc004disp, d.ma002 as tc017disp')
	                       ->from('asttc as a')
						   ->join('cmsmq as b','a.tc001 = b.mq001','left')
						   ->join('astmb as c','a.tc004 = c.mb001','left')
						   ->join('copma as d','a.tc017 = d.ma001','left')
						   ->where('b.mq003','C4')
			               ->order_by($order);
		if($where){
			$query->where($where);
		...
		$ret['data'] = $query->get()->result();
		//儲存sql 語法
		$_SESSION['asti09']['search']['sql'] = $this->db->last_query();
		...
		//儲存where與order
		$_SESSION['asti09']['search']['where'] = $where;
		$_SESSION['asti09']['search']['order'] = $order;
		$_SESSION['asti09']['search']['offset'] = $offset;
	...
	function construct_view($data){
		...
		$_SESSION['asti09']['search']['view'] = $view_array;
		$_SESSION['asti09']['search']['index'] = $index_array;
		//echo "<pre>";var_dump($_SESSION['asti09']['search']['view']);exit;
	}
	...
	function construct_sql_body($limit = 15, $offset = 0, $func = "")
	{
		...
		if(isset($_SESSION['asti09_body']['search']['where'])){
			if($where){$where .= " and ";}
			$where .= $_SESSION['asti09_body']['search']['where'];
		}
		...
		if(isset($_SESSION['asti09_body']['search']['order'])){
			if($order){$order .= " , ";}
			$order .= $_SESSION['asti09_body']['search']['order'];
		}
		
		if(!isset($_SESSION['asti09_body']['search']['order']) && $default_order){
			if($order){$order .= " , ";}
			$order .= $default_order;
		}
		/* order end */
		...
		$ret['data'] = $query->get()->result();
		//儲存sql
		$_SESSION['asti09_body']['search']['sql'] = $this->db->last_query();
		...
		//儲存where與order
		$_SESSION['asti09_body']['search']['where'] = $where;
		$_SESSION['asti09_body']['search']['order'] = $order;
		$_SESSION['asti09_body']['search']['offset'] = $offset;
	...
	function selone($seg1, $seg2) {
		$this->db->select('a.*,c.mq002 as tc001disp,d.mb002 as tc004disp, d.mb003 as tc004disp2, d.mb011 as tc004disp3, g.ma002 as tc017disp, h.mf002 as tc019disp');
        $this->db->from('asttc as a');		
		$this->db->join('cmsmq as c','a.tc001 = c.mq001','left');
		$this->db->join('astmb as d','a.tc004 = d.mb001','left');
		$this->db->join('copma as g','a.tc017 = g.ma001','left');
		$this->db->join('cmsmf as h','a.tc019 = h.mf001','left');
		...
		$this->db->select('b.td003 as asti02_asti09, b.td004 as asti02_asti09_mc003, b.td005, b.td008, b.td010, e.me002 as asti02_asti09_mc002disp, f.mv002 as asti02_asti09_mc003disp')
			->from('asttd as b')
	...
	function findf($limit, $offset, $sort_by, $sort_order)     
         {            		
	      ...
	      $seq1 = "tc001, tc002, tc003, tc004, tc017, tc019, tc020, tc005, tc013, create_date FROM `asttc` ";
		...
		if(@$_SESSION['asti09_sql_term']){$seq32 = $_SESSION['asti09_sql_term'];}
		if(@$_SESSION['asti09_sql_sort']){$seq33 = $_SESSION['asti09_sql_sort'];}
		// echo var_dump($seq32);exit;
		 
         $sort_order = (substr($sort_order,0,3) == 'asc') ? 'asc' : 'desc';
	     $sort_columns = array('a.tc001', 'b.mq002', ' a.tc002', 'a.tc003','a.tc004', 'c,mb002', 'a.tc017', 'd.ma002','a.tc019', 'a.tc020', 'a.tc005', 'a.tc013','a.create_date');
	     $sort_by = (in_array($sort_by, $sort_columns)) ? $sort_by : 'mb001';  //檢查排序欄位是否在 table 內
	     $query = $this->db->select('a.tc001, b.mq002 as tc001disp, a.tc002, a.tc003, a.tc004, c.mb002 as tc004disp, a.tc017, d.ma002 as tc017disp, a.tc019, a.tc020, a.tc005, a.tc013, a.create_date')
	                       ->from('asttc as a')
						   ->join('cmsmq as b', 'a.tc001 = b.mq001','left')
						   ->join('astmb as c', 'a.tc004 = c.mb001','left')
						   ->join('copma as d', 'a.tc017 = d.ma001','left')
	...
	function insertf()    //新增一筆 檔頭  asttc
        {
	     ...
	     $data = array( 
                 ...
                 'tc017' => $this->input->post('copi01'),  //客戶代號
                 'tc018' => $this->input->post('copi01disp'),
                 'tc019' => $this->input->post('cmsi06'),	
                 'tc020' => $this->input->post('exchange_rate'),
		...
		//      $vtd003='1010';   //流水號重新排序
		   foreach($order_product as $key => $val){
		        if($val['asti02_asti09'] && $val['asti02_asti09_mc003']){
				        extract($val);
						...
						$data = array( 
							...
							'td002' => $tc002,
							'td003' => $asti02_asti09,
							'td004' => $asti02_asti09_mc003,
							'td010' => $body_td010
						);
						foreach($val as $k=>$v){
							if($k!="asti02_asti09"&&$k!="asti02_asti09_mc002disp"&&$k!="asti02_asti09_mc003"&&$k!="asti02_asti09_mc003disp"){//主鍵不用更改以及其他外來鍵庫別名稱
							    $data[$k] = $v;
							}
						}
    ...
    function astyf()           
        {
         ...
         //   if ($query->num_rows() != 1) { return 'exist'; }
            ...
            $exist = $this->asti09_model->selone1($seq1,$seq2);  //檢查單頭是否重複
		    if ($exist)
		      {
			    return 'exist';
		      }         
	...
	function printfd()          
        {
		  ...
		  $seq3=$this->input->post('tc002o');    
	      $seq4=$this->input->post('tc002c');
	      if (!isset($seq3) ) {$seq3='0';}
		  $this->db->select('a.*, b.*, c.mq002 as tc001disp, d.mb002 as tc004disp, d.mb003 as tc004disp2,e.me002 as td003disp, f.mv002 as td004disp')
					->from('asttc as a')
					->join('asttd as b','a.tc001 = b.td001 and a.tc002 = b.td002','left')
					->join('cmsmq as c','a.tc001 = c.mq001','left')
					->join('astmb as d','a.tc004 = d.mb001','left')
	...
	function updatef()   
        {
          ...
          $data = array(			
			...
			foreach($order_product as $key => $val){
				...
				//preg_match_all('/\d/S',$td013, $matches);  //處理日期字串
			    //$td013 = implode('',$matches[0]);
				if($this->seldetail($tc001,$tc002,$val['asti02_asti09'],$val['asti02_asti09_mc003'])>0){
					$data = array(
						'modifier' => $this->session->userdata('manager'),
						'modi_date' => date("Ymd"),
						'td003' => $asti02_asti09,
					...
					foreach($val as $k=>$v){
						if($k!="asti02_asti09"&&$k!="asti02_asti09_mc002disp"&&$k!="asti02_asti09_mc003"&&$k!="asti02_asti09_mc003disp"){//主鍵不用更改以及其他外來鍵庫別名稱 td013日期等別處理
							$data[$k] = $v;
						}
					}
					$this->db->where('td001', $tc001);
					$this->db->where('td002', $tc002);
					$this->db->where('td003', $val['asti02_asti09']);
					$this->db->where('td004', $val['asti02_asti09_mc003']);
