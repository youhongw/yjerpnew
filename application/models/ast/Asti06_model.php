...
class Asti06_model extends CI_Model {
	...
	function construct_sql($limit = 15, $offset = 0, $func = "")
	  {
		...
		if(isset($_SESSION['asti06']['search']['where'])){
			if($where){$where .= " and ";}
			$where .= $_SESSION['asti06']['search']['where'];
		}
		...
		if(isset($_SESSION['asti06']['search']['order'])){
			if($order){$order .= " , ";}
			$order .= $_SESSION['asti06']['search']['order'];
		}
		
		if(!isset($_SESSION['asti06']['search']['order']) && $default_order){
			if($order){$order .= " , ";}
			$order .= $default_order;
		}
		/* order end */
		...
		/* Data SQL */
		$query = $this->db->select('a.*,b.mq002,c.mb002')
	                       ->from('asttc as a')
		...
		$ret['data'] = $query->get()->result();
		//儲存sql 語法
		$_SESSION['asti06']['search']['sql'] = $this->db->last_query();
		...
		//儲存where與order
		$_SESSION['asti06']['search']['where'] = $where;
		$_SESSION['asti06']['search']['order'] = $order;
		$_SESSION['asti06']['search']['offset'] = $offset;
	...
	function construct_view($data){
		...
		$_SESSION['asti06']['search']['view'] = $view_array;
		$_SESSION['asti06']['search']['index'] = $index_array;
		//echo "<pre>";var_dump($_SESSION['copi06']['search']['view']);exit;
	}
	
	//查詢一筆 修改用   
	function selone($seg1, $seg2) {
		$this->db->select('a.*,b.mq002 as tc001disp, c.mb002 as tc004disp, c.mb003 as tc004disp2');
        $this->db->from('asttc as a');	
        $this->db->join('cmsmq as b','a.tc001 = b.mq001','left');	
        $this->db->join('astmb as c','a.tc004 = c.mb001','left');	
		$this->db->where('a.tc001', $seg1); 
	    $this->db->where('a.tc002', $seg2); 
		$this->db->order_by('a.tc001 , a.tc002');
	...
	function findf($limit, $offset, $sort_by, $sort_order)     
         {            		
	      ...
	      $seq1 = "tc001, tc002, tc004, tc007, tc027, create_date FROM `asttc` ";
		...
		if(@$_SESSION['asti06_sql_term']){$seq32 = $_SESSION['asti06_sql_term'];}
		if(@$_SESSION['asti06_sql_sort']){$seq33 = $_SESSION['asti06_sql_sort'];}
		
         $sort_order = (substr($sort_order,0,3) == 'asc') ? 'asc' : 'desc';
	     $sort_columns = array('a.tc001', 'b.mq002', 'a.tc002', 'a.tc027','a.tc004', 'c.mb002', 'a.tc007','a.create_date');
	     $sort_by = (in_array($sort_by, $sort_columns)) ? $sort_by : 'tc001';  //檢查排序欄位是否在 table 內
	     $query = $this->db->select('a.tc001,b.mq002, a.tc002, a.tc027, a.tc004, c.mb002, a.tc007, a.create_date')
	                       ->from('asttc as a')
						   ->join('cmsmq as b', 'a.tc001 = b.mq001','left')
						   ->join('astmb as c', 'a.tc004 = c.mb001','left')
						   ->where('b.mq003','C1')
	...
	function insertf()    //新增一筆 檔頭  coptc
        {
		    //刪日期 / 符號
			 ...
			 $tc001=$this->input->post('asti03_asti06');
			 $tc002=$this->input->post('tc002');
			 $tc002no=$tc002;   //明細用再新增一筆時加1
			 //檢查資料是否已存在 若存在加1
			  while($this->asti06_model->selone1($tc001,$tc002)>0){
				$tc002 = $this->check_title_no($tc001,$tc027);
				$tc002no=$tc002;
			}
		//	var_dump($tc002no);exit;
	...
	function excelnewf()           
        {			
	      ...
	      $sql = " SELECT a.tc001, c.mb002 as tc001disp, a.tc002, a.tc027, a.tc004, c.mb002 as tc004disp, c.mb003 as tc004disp2, a.tc007, a.tc009, a.tc010, a.tc011, a.tc023, a.tc015 , a.tc013
		  FROM asttc as a, cmsmq as b, astmb as c WHERE a.tc001=b.mq001 and a.tc004=c.mb001 and b.mq003='C1' and tc001 >= '$seq1'  AND tc001 <= '$seq2' AND  tc002 >= '$seq3'  AND tc002 <= '$seq4'  "; 
	...
	function printfd()          
        {
		  ...
		  $seq3=$this->input->post('tc002o');    
	      $seq4=$this->input->post('tc002c');
	      $sql = " SELECT a.*, b.mq002 as tc001disp, c.mb002 as tc004disp, c.mb003 as tc004disp2
		  FROM asttc as a, cmsmq as b, astmb as c
		  WHERE a.tc001 = b.mq001 AND a.tc004 = c.mb001 AND b.mq003 = 'C1' AND a.tc001 >= '$seq1'  AND a.tc001 <= '$seq2' AND a.tc002 >= '$seq3' AND a.tc002 <= '$seq4'  "; 
          $query = $this->db->query($sql);
	      $ret['rows'] = $query->result();
	...
	//印單據筆  
        ...
        $this->db->from('asttc as a');	
		$this->db->join('cmsmq as b', 'a.tc001 = b.mq001','left');  //單據性質
		$this->db->join('astmb as c', 'a.tc004 = c.mb001','left');  //資產編號
		$this->db->where('a.tc001', $this->uri->segment(4)); 
	    $this->db->where('a.tc002', $this->uri->segment(5)); 
