...
class posr02_model extends CI_Model {
	...
	function insertf()    //新增一筆 檔頭  postc 
        {
		 ...
		 $data = array( 
	      ...
	      $exist = $this->posr02_model->selone1($this->input->post('posq02a'),$this->input->post('tc002'));
	      if ($exist)
	         {
		      return 'exist';
		     } 
		...
		//	while ($_POST['order_product'][  $n  ]['td004']) {
	      ...
	      $exist = $this->posr02_model->selone1d($this->input->post('posq02a'),$this->input->post('tc002'),$td003);
    ...
    function copyf()           
        {
         ...
         //   if ($query->num_rows() != 1) { return 'exist'; }
            ...
            $exist = $this->posr02_model->selone2($this->input->post('tc001c'),$this->input->post('tc002c'));
		    if ($exist)
		      {
			    return 'exist';
		      }         
	...
	function excelnewf()           
        {			
		 ...
		 // $seq1=substr($this->input->post('tc003'),0,4).substr($this->input->post('tc003'),5,2).substr(rtrim($this->input->post('tc003')),8,2)
		/*  if(!$seq1) $seq1 = date("Y-m-d");
		  preg_match_all('/\d/S',$seq1, $matches);  //處理日期字串
		  $seq1 = implode('',$matches[0]);               //   WHERE substring(`tb002`,1,8) = '$seq1' */
		  
		  $sql = " SELECT b.ta001,b.ta002,b.ta003,b.ta004,b.ta005,b.ta006,b.ta007,b.ta008,b.ta009,b.ta010,
		  b.ta011,b.ta012,b.ta013,b.ta014,b.ta015,b.ta016,a.tb016,a.tb003,a.tb004,a.tb005,a.tb006,a.tb008,
		  a.tb007,a.tb010,a.tb011,a.tb015,a.tb013,a.tb014
		  FROM postb as a left join posta as b on a.tb001=b.ta001 and a.tb002=b.ta002
		  WHERE b.ta003 = '$seq1' 
		  order by  b.ta004,a.tb002,a.tb003 "; 
