...
class Posr03_model extends CI_Model {
	...
	function printfd()          
		...
		//  $seq3=$this->input->post('dateo');   //日期 
		  ...
		  $this->db->select('a.tb003,a.tb005,a.tb004,a.tb006,a.tb007,a.tb009,sum(a.tb010) as tb010,sum(a.tb016) as tb016,(sum(a.tb016)/sum(a.tb010)) as avg_price'); 
		  $this->db->from('postb as a');
		  $this->db->join('posta as b', 'a.tb001 = b.ta001 and a.tb002 = b.ta002 ','left');
		  $this->db->where('substring(b.ta002 ,1,8) >=', $seq3); 
	      $this->db->where('substring(b.ta002 ,1,8) <=', $seq4);
		  $this->db->group_by('tb005');
		  $this->db->order_by('tb005');
		  $query = $this->db->get();
		  $ret['rows'] = $query->result();
		  
          $seq32 = "substring(b.ta002 ,1,8) >= '$seq3' AND substring(b.ta002 ,1,8) <= '$seq4' ";	
	      $query = $this->db->select('sum(a.tb010) as count,sum(a.tb016) as totle_money,', FALSE)  //筆數查詢,如果設為FALSE不會使用反引號保護你的字段或者表名
		              ->from('postb as a')
					  ->join('posta as b', 'a.tb001 = b.ta001 and a.tb002 = b.ta002 ','left')
		              ->where($seq32);
	      $num = $query->get()->result();
	      $ret['num_rows'] = $num[0]->count;
