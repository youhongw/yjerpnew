...
class posi03_model extends CI_Model {
	...
	function search($limit, $offset, $sort_by, $sort_order)     
	  { 
	    ...
	    $query = $this->db->select('mb001, mb002, mb003, mb004, mb005, mb006,mb007,mb008, create_date')
	                      ->from('posmb')
	...
	function findf($limit, $offset, $sort_by, $sort_order)     
       {            		
	     ...
	     $seq11 = "SELECT COUNT(*) as count  FROM `posmb` ";
	     $seq1 = " mb001, mb002, mb003, mb004, mb005, mb006,mb007, create_date FROM `posmb` ";
		...
		if(@$_SESSION['admi05_sql_sort']){$seq33 = $_SESSION['admi05_sql_sort'];}
         ...
         $sort_order = (substr($sort_order,0,3) == 'asc') ? 'asc' : 'desc';
	     $sort_columns = array('mb001', 'mb002', 'mb003', 'mb004', 'mb005', 'mb006','mb007','mb008','create_date');
	     $sort_by = (in_array($sort_by, $sort_columns)) ? $sort_by : 'mb001';  //檢查排序欄位是否在 table 內
	     $query = $this->db->select('mb001, mb002, mb003, mb004, mb005, mb006,mb007,mb008, create_date')
	                       ->from('posmb')
		                   ->where($seq32)
	...
	function insertf()   
       {
		   ...
		   if ($this->input->post('mb002')>'0') {$vmb002=substr($this->input->post('mb002'),0,4).substr($this->input->post('mb002'),5,2);} else {$vmb002='';}
		   if ($this->input->post('mb003')>'0') {$vmb003=substr($this->input->post('mb003'),0,4).substr($this->input->post('mb003'),5,2);} else {$vmb003='';}
	    $data = array( 
	    ...
	    $exist = $this->posi03_model->selone1($this->input->post('cmsq11a'),$vmb002,$this->input->post('mb004'));
	    if ($exist)
	      {
		    return 'exist';
		  } 
           return  $this->db->insert('posmb', $data);
       }
    ...
    function copyf()           //複製一筆
       {
	    ...
	    $data = array( 
            ...
            $exist = $this->posi03_model->selone2($this->input->post('mb002c'));
		    if ($exist)
		      {
			   return 'exist';
		      }         
              return $this->db->insert('posmb', $data);      //複製一筆  
       }	
	...
	function excelnewf()           
       {			
	    ...
	    $seq2=$this->input->post('mb002c');
	    $sql = " SELECT mb001,mb002,mb004,mb005,mb006,mb007,mb008,mb009,create_date FROM posmb WHERE mb001 >= '$seq1'  AND mb001 <= '$seq2'  "; 
	...
	function printfd()          
       {
	    $seq1=$this->input->post('mb001c');    //查詢一筆以上
	    $seq2=$this->input->post('mb002c'); 
	    $sql = " SELECT * FROM posmb WHERE mb001 >= '$seq1'  AND mb001 <= '$seq2'  "; 
        $query = $this->db->query($sql);
	    $ret['rows'] = $query->result();
	...
	function updatef()   //更改一筆
          {
		   ...
		   if ($this->input->post('mb003')>'0') {$vmb003=substr($this->input->post('mb003'),0,4).substr($this->input->post('mb003'),5,2);} else {$vmb003='';}
            ...
            $this->db->where('mb001', $this->input->post('cmsq11a'));
	        $this->db->where('mb002', $vmb002);
			 $this->db->where('mb004', $this->input->post('mb004'));
            $this->db->update('posmb',$data);                   //更改一筆
