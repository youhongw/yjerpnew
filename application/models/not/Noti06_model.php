...
class Noti06_model extends CI_Model {
	...
	{
		$this->session->set_userdata('noti06_search',"display_search/".$offset);
		...
		if(isset($_SESSION['noti06']['search']['where'])){
			if($where){$where .= " and ";}
			$where .= $_SESSION['noti06']['search']['where'];
		}
		...
		if(isset($_SESSION['noti06']['search']['order'])){
			if($order){$order .= " , ";}
			$order .= $_SESSION['noti06']['search']['order'];
		}
		
		if(!isset($_SESSION['noti06']['search']['order']) && $default_order){
			if($order){$order .= " , ";}
			$order .= $default_order;
		}
		/* order end */
		...
		/* Data SQL */
		$query = $this->db->select('mq001, mq002, mq003, create_date')
			->from('cmsmq')
			->like('mq003','8','after')
			->order_by($order);
		if($where){
		...
		$ret['data'] = $query->get()->result();
		//儲存sql
		$_SESSION['noti06']['search']['sql'] = $this->db->last_query();
		...
		//儲存where與order
		$_SESSION['noti06']['search']['where'] = $where;
		$_SESSION['noti06']['search']['order'] = $order;
		$_SESSION['noti06']['search']['offset'] = $offset;
	...
	function lookupd_check($keyword){  
      ...
      $this->db->select('mq001, mq002, mq003');
	  $this->db->from('cmsmq');  
	  $this->db->like('mq003', '8','after');	
	...
	function insertf()   
       ...
       //    }
	    ...
	    $exist = $this->noti06_model->selone1($this->input->post('mq001'));
	    if ($exist)
	      {
		    return 'exist';
		  } 
    ...
    function copyf()           //複製一筆
       {
	    ...
	    $data = array( 
            ...
            $exist = $this->noti06_model->selone2($this->input->post('mq002c'));
		    if ($exist)
		      {
			   return 'exist';
		      }         
