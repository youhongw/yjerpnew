...
class Noti01_model extends CI_Model {
	...
	function construct_sql2($limit = 15, $offset = 0, $func = "",$sma001)
	{
		...
		$query = $this->db->select('a.ma001, a.ma002,a.ma004, b.ma046, a.create_date, b.ma001 as copi01')
			...
			->order_by($order);
	//1070726	$query->where('b.ma001',$sma001);
		if($where){
			$query->where($where);
		}
	...
	function construct_sql_body($limit = 15, $offset = 0, $func = "")
	{
		...
		if(isset($_SESSION['noti01_body']['search']['where'])){
			if($where){$where .= " and ";}
			$where .= $_SESSION['noti01_body']['search']['where'];
		}
		...
		if(isset($_SESSION['noti01_body']['search']['order'])){
			if($order){$order .= " , ";}
			$order .= $_SESSION['noti01_body']['search']['order'];
		}
		
		if(!isset($_SESSION['noti01_body']['search']['order']) && $default_order){
			if($order){$order .= " , ";}
			$order .= $default_order;
		}
		/* order end */
		...
		$ret['data'] = $query->get()->result();
		//儲存sql
		$_SESSION['noti01_body']['search']['sql'] = $this->db->last_query();
		...
		//儲存where與order
		$_SESSION['noti01_body']['search']['where'] = $where;
		$_SESSION['noti01_body']['search']['order'] = $order;
		$_SESSION['noti01_body']['search']['offset'] = $offset;
	...
	function printfc()   
      {           
          $this->db->select('a.* ,c.mq002 AS ma001disp, d.me002 AS ma004disp, e.mb002 AS ma010disp, f.mv002 AS ma012disp,
		  b.company, b.creator, b.usr_group, b.create_date, b.modifier, b.modi_date, b.flag, b.mb001, b.mb002, b.mb003, b.mb004, b.mb005,
		...
		$this->db->join('notme as d', 'a.ma004 = d.me001 ','left');
	...
	function lookup3($keyword,$smb002){     
      $this->db->select('a.ma001, a.ma002,b.ma046,b.ma001');
	  $this->db->from('notma as a');  
