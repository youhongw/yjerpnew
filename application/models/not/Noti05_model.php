...
class Noti05_model extends CI_Model {
	...
	function selbrowse($num,$offset)   
        {            
	      $this->db->select('te001,te002,te003');
          $this->db->from('notte');
          //$this->db->order_by('id', 'DESC');                //排序  單欄
	      $this->db->order_by('te001 desc');    //排序  單欄以上 asc 由小至大 desc預設由大至小
	...
	function search($limit, $offset, $sort_by, $sort_order)     
	    { 
	     ...
	     $sort_columns = array('te001', 'te002', 'te003');
	...
	{
		$this->session->set_userdata('noti05_search',"display_search/".$offset);
		...
		$default_order = "te001 asc";//在這裡塞入一些預設排序
		...
		if(isset($_SESSION['noti05']['search']['where'])){
			if($where){$where .= " and ";}
			$where .= $_SESSION['noti05']['search']['where'];
		}
		...
		if(isset($_SESSION['noti05']['search']['order'])){
			if($order){$order .= " , ";}
			$order .= $_SESSION['noti05']['search']['order'];
		}
		
		if(!isset($_SESSION['noti05']['search']['order']) && $default_order){
			if($order){$order .= " , ";}
			$order .= $default_order;
		}
		/* order end */
		...
		$ret['data'] = $query->get()->result();
		//儲存sql
		$_SESSION['noti05']['search']['sql'] = $this->db->last_query();
		...
		//儲存where與order
		$_SESSION['noti05']['search']['where'] = $where;
		$_SESSION['noti05']['search']['order'] = $order;
		$_SESSION['noti05']['search']['offset'] = $offset;
	...
	function selone($seq1,$seq2)    
        {
		  $this->db->select('* ');
        $this->db->from('notte');
		$this->db->where('te001', urldecode($this->uri->segment(4))); 
	 //   $this->db->where('a.mf002', $this->uri->segment(5)); 
		$this->db->order_by('te001');
	...
	function findf($limit, $offset, $sort_by, $sort_order)     
         {            		
	      ...
	      $seq11 = "SELECT COUNT(*) as count  FROM `notte` ";
	      $seq1 = "te001,te002,te003 FROM `notte` ";
        ...
        if ($seq7!='') {$seq9=" ORDER BY ".$seq7;$seq33=$seq7;}
	     ...
	     $sort_columns = array('te001', 'te002', 'te003');
	     $sort_by = (in_array($sort_by, $sort_columns)) ? $sort_by : 'te001';  //檢查排序欄位是否在 table 內
	     $query = $this->db->select('te001,te002,te003,te004,te007,create_date')
	                       ->from('notte')
		                   ->where($seq32)
			               ->order_by($seq33)
			              //->order_by($sort_by, $sort_order)
	...
	function filterf1($limit, $offset , $sort_by  , $sort_order)          
	    {    
	      ...
	      $this->db->select('te001,te002,te003,te004,te007,create_date');
	      $this->db->from('notte');
	      $this->db->like($sort_by, $seq4, 'after');
	      $this->db->order_by($sort_by, $sort_order);
	      //$this->db->order_by('te001 asc, mf002 asc');
	...
	//新增一筆 檔頭  notte	
	function insertf()    //新增一筆 檔頭  notte
        {
	     ...
	     $data = array( 
			...
			'te006' => $this->input->post('te006'),
			'te007' => $this->input->post('te007'),
			'te008' => $this->input->post('te008'),
			'te009' => $this->input->post('te009')
		);
         
	      $exist = $this->noti05_model->selone1($this->input->post('te001'));
	      if ($exist)
	         {
		      return 'exist';
		     } 
             $this->db->insert('notte', $data);
    ...
    function copyf()           
		    ...
		    if ($query->num_rows() == 1) 
		       {
                ...
                $te002=$row->te002;$te003=$row->te003;
				
			endforeach;
		       }   
		  
            $seq1=$this->input->post('te001c');    //主鍵一筆檔頭notte
		//	$seq2=$this->input->post('mf002c');    
	        $data = array( 
		           ...
		           'modi_date' => ' ',
		           'flag' => 0,
		           'te001' => $seq1,'te002' => $te002,'te003' => $te003
                   );
				   
            $exist = $this->noti05_model->selone2($this->input->post('te001c'));
		    if ($exist)
		      {
			    return 'exist';
		      }         
             $this->db->insert('notte', $data);      //複製一筆  
	...
	function excelnewf()           
        {			
	      ...
	      $sql = "SELECT te001,te002,te003 FROM notte WHERE te001 >= '$seq1'  AND te001 <= '$seq2'  "; 
	...
	function printfd()          
	   ...
	   //   $seq4=$this->input->post('mf002c');
	      $sql = " SELECT * FROM notte WHERE te001 >= '$seq1'  AND te001 <= '$seq2'   "; 
          $query = $this->db->query($sql);
	      $ret['rows'] = $query->result();
          $seq32 = "te001 >= '$seq1'  AND te001 <= '$seq2'   ";	
	      $query = $this->db->select('COUNT(*) as count', FALSE)  //筆數查詢,如果設為FALSE不會使用反引號保護你的字段或者表名
		              ->from('notte')
		              ->where($seq32);
	      $num = $query->get()->result();		
	...
	function printfd1()   
       {           
        ...
        $this->db->from('notte as a');	
        $this->db->join('notmb as b', 'a.te001 = b.mb001   ','left');
		$this->db->where('a.te001', $this->uri->segment(4)); 
	  
		$this->db->order_by('a.te001 , b.mb002');
	...
	function printfc()   
      {           
          $this->db->select('a.* ,c.mq002 AS te001disp, d.te002 AS mf004disp, e.mb002 AS mf010disp, f.mv002 AS mf012disp,
		  b.company, b.creator, b.usr_group, b.create_date, b.modifier, b.modi_date, b.flag, b.mb001, b.mb002, b.mb003, b.mb004, b.mb005,
		  b.mb006, b.mb007, b.mb011, b.mb009, b.mb017, b.mb018, b.mb012');
		 
        $this->db->from('notte as a');	
        $this->db->join('notmb as b', 'a.te001 = b.mb001  and a.mf002=b.mb002 ','left');		
		$this->db->join('notmq as c', 'a.te001 = c.mq001 and c.mq003="31" ','left');
		$this->db->join('notme as d', 'a.mf004 = d.te001 ','left');
		...
		$this->db->order_by('te001 , mf002 ,b.mb003');
	...
	function updatef()   
        {
          ...
          $data = array(			
            ...
            $this->db->where('te001', $this->input->post('te001'));
		//	$this->db->where('mf002', $this->input->post('mf002'));
            $this->db->update('notte',$data);                   //更改一筆
			return true;
        }
