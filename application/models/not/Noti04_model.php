...
class Noti04_model extends CI_Model {
	...
	function search($limit, $offset, $sort_by, $sort_order)     
	    { 
	     ...
	     $query = $this->db->select('a.tc001, a.tc002, a.tc003,a.tc008, a.tc004, b.ma002,  a.tc029, a.tc030,a.create_date')
	                       ->from('coptc as a')
	...
	function construct_sql($limit = 15, $offset = 0, $func = "")
	  {
		...
		if(isset($_SESSION['noti04']['search']['where'])){
			if($where){$where .= " and ";}
			$where .= $_SESSION['noti04']['search']['where'];
		}
		...
		if(isset($_SESSION['noti04']['search']['order'])){
			if($order){$order .= " , ";}
			$order .= $_SESSION['noti04']['search']['order'];
		}
		
		if(!isset($_SESSION['noti04']['search']['order']) && $default_order){
			if($order){$order .= " , ";}
			$order .= $default_order;
		}
		/* order end */
		...
		/* Data SQL */
		$query = $this->db->select('a.tc001,a.tc002,a.tc003,a.tc008,a.tc004,a.tc012,a.tc013,a.tc014,a.tc008,a.create_date')
	                       ->from('nottc as a')
			               ->order_by($order);
		if($where){
			$query->where($where);
		...
		$ret['data'] = $query->get()->result();
		//儲存sql 語法
		$_SESSION['noti04']['search']['sql'] = $this->db->last_query();
		...
		//儲存where與order
		$_SESSION['noti04']['search']['where'] = $where;
		$_SESSION['noti04']['search']['order'] = $order;
		$_SESSION['noti04']['search']['offset'] = $offset;
	...
	function construct_sql_body($limit = 15, $offset = 0, $func = "")
	{
		...
		if(isset($_SESSION['noti04_body']['search']['where'])){
			if($where){$where .= " and ";}
			$where .= $_SESSION['noti04_body']['search']['where'];
		}
		...
		if(isset($_SESSION['noti04_body']['search']['order'])){
			if($order){$order .= " , ";}
			$order .= $_SESSION['noti04_body']['search']['order'];
		}
		
		if(!isset($_SESSION['noti04_body']['search']['order']) && $default_order){
			if($order){$order .= " , ";}
			$order .= $default_order;
		}
		/* order end */
		...
		/* Data SQL */
		$query = $this->db->select('tc001, tc003, tc005')
			->from('nottc')
			->where('tc012','1')
			->or_where('tc012','9')
			->order_by($order);
		...
		$ret['data'] = $query->get()->result();
		//儲存sql
		$_SESSION['noti04_body']['search']['sql'] = $this->db->last_query();
		...
		//儲存where與order
		$_SESSION['noti04_body']['search']['where'] = $where;
		$_SESSION['noti04_body']['search']['order'] = $order;
		$_SESSION['noti04_body']['search']['offset'] = $offset;
	...
	function construct_view($data){
		...
		$_SESSION['noti04']['search']['view'] = $view_array;
		$_SESSION['noti04']['search']['index'] = $index_array;
		//echo "<pre>";var_dump($_SESSION['noti04']['search']['view']);exit;
	}
	
	//查詢一筆 修改用   
	function selone($seg1) {
		$this->db->select('*,b.ma002 as tc013disp, c.mf002 as tc002disp,d.ma002 as noti01adisp, e.me002 as tc015disp, f.mv002 as tc016disp, g.ma003 as tc020disp, h.ma003 as tc022disp, j.ma002 as tc025disp , j.ma004 as tc025disp2, k.ma002 as tc026disp, a.tc008 as noti01a, a.tc009 as noti01adisp2');
		 
        $this->db->from('nottc as a');
		$this->db->join('copma as b','a.tc013 = b.ma001','left');
		$this->db->join('cmsmf as c','a.tc002 = c.mf001','left');
		$this->db->join('notma as d','a.tc008 = d.ma001','left');
		$this->db->join('cmsme as e','a.tc015 = e.me001','left');
		$this->db->join('cmsmv as f','a.tc016 = f.mv001','left');
		$this->db->join('actma as g','a.tc020 = g.ma001','left');
		$this->db->join('actma as h','a.tc022 = h.ma001','left');
		$this->db->join('nottd as i','a.tc001 = i.td001','left');
		$this->db->join('notma as j','a.tc025 = j.ma001','left');
		$this->db->join('purma as k','a.tc026 = k.ma001','left');
	...
	function findf($limit, $offset, $sort_by, $sort_order)     
         {            		
	      ...
	      $seq11 = "SELECT COUNT(*) as count  FROM `nottc as a` ";
	      $seq1 = "tc001, tc002, tc003, tc004, tc012, tc013,tc014, create_date FROM `nottc as a` ";
		...
		if(@$_SESSION['noti04_sql_term']){$seq32 = $_SESSION['noti04_sql_term'];}
		if(@$_SESSION['noti04_sql_sort']){$seq33 = $_SESSION['noti04_sql_sort'];}
		
         $sort_order = (substr($sort_order,0,3) == 'asc') ? 'asc' : 'desc';
	     $sort_columns = array('tc001', 'tc002', 'tc003','tc008', 'tc004', 'tc012', 'tc013','tc014', 'a.create_date');
	     $sort_by = (in_array($sort_by, $sort_columns)) ? $sort_by : 'tc001';  //檢查排序欄位是否在 table 內
	     $query = $this->db->select('tc001, tc002, tc003,tc008, tc004, tc012, tc013,tc014, a.create_date')
	                       ->from('nottc as a')
		                   ->where($seq32)
			               ->order_by($seq33)
			              ->limit($limit, $offset);
	...
	function filterf1($limit, $offset , $sort_by  , $sort_order)          
	    {    
	      ...
	      $this->db->select('a.tc001, a.tc002, a.tc003,a.tc008, a.tc004,b.ma002,  a.tc029,a.tc030, a.create_date');
	      $this->db->from('coptc as a');
	...
	function insertf()    //新增一筆 檔頭  coptc
        {
		    //刪日期 / 符號
			 ...
			 preg_match_all('/\d/S',$this->input->post('tc005'), $matches);  //處理日期字串
			 $tc005 = implode('',$matches[0]);			 
			 preg_match_all('/\d/S',$this->input->post('tc007'), $matches);  //處理日期字串
			 $tc007= implode('',$matches[0]);
			   
			 /*$tc001=$this->input->post('copi03');
			 $tc002=$this->input->post('tc002');
			 $tc002no=$tc002;   //明細用再新增一筆時加1
			 //檢查資料是否已存在 若存在加1
			  while($this->noti04_model->selone1($tc001,$tc002)>0){
	     ...
	     $data = array( 
		         ...
		         'tc002' => $this->input->post('cmsi06'),  //幣別
		         'tc003' => $this->input->post('tc003'),
		         'tc004' => $tc004,    
		         'tc005' => $tc005,    
		         'tc006' => $this->input->post('tc006'),    
                 'tc007' => $tc007,    
                 'tc008' => $this->input->post('noti01a'),  //付款銀行 
                 'tc009' => $this->input->post('noti01adisp2'),  //付款帳號
                 'tc010' => $this->input->post('tc010'),  //票據種類		
                 'tc011' => $this->input->post('tc011'),
                 'tc012' => $this->input->post('tc012'),
                 'tc013' => $this->input->post('copi01'),  //客戶稱號
                 'tc014' => $this->input->post('copi01disp2'),  //客戶簡稱
                 'tc015' => $this->input->post('cmsi05'),	//部門代號
                 'tc016' => $this->input->post('cmsi09'),  //業務員代號
                 
                 'tc017' => $this->input->post('acri03'),  //收款單別
                 'tc018' => $this->input->post('acri03disp'),  //收款單號
                 'tc019' => $this->input->post('acri03disp2'), //收款序號
                 'tc020' => $this->input->post('acti03'),  //票據科目
				 'tc021' => $this->input->post('tc021') , 
				
                 'tc022' => $this->input->post('acti03a'), //對方科目

				 'tc024' => $this->input->post('tc024'),
				 'tc025' =>$this->input->post('tc025'),
                 'tc026' =>$this->input->post('tc026'),
                 'tc027' =>$this->input->post('exchange_rate')
                );
		...
		// 新增明細 coptd  
		   ...
		   /*foreach($order_product as $key => $val){
			...
			$data2 = array(
				...
				'td001' => $this->input->post('tc001'),
				'td002' => '1010',
				'td003' => $tc004,
				'td004' => '1',
				'td005' => $this->input->post('tc027'),
				'td006' => '',
				'td007' => '',
				'td008' => 'N'
			);
			
			 $this->db->insert('nottd', $data2);
		 }
    ...
    function copyf()           
        {
         ...
         //   if ($query->num_rows() != 1) { return 'exist'; }
            ...
            $exist = $this->noti04_model->selone1($seq1);  //檢查單頭是否重複
		    if ($exist)
		      {
			    return 'exist';
		      }         
	...
	function updatef()   
        {
          ...
          $data = array(			
			...
			if(!is_array($order_product)){$order_product=array();}
					...
					$this->db->delete('nottd'); //刪除明細 1060809
					
		    $vtd002='1010';   //流水號重新排序
			foreach($order_product as $key => $val){
				extract($val);
				preg_match_all('/\d/S',$td003, $matches);  //處理日期字串
			    $td003 = implode('',$matches[0]);
				$std004 = substr($td004,0,1);
				if($this->seldetail($tc001,$td002)){
					$data = array(
						'modifier' => $this->session->userdata('manager'),
						'modi_date' => date("Ymd"),
						'td001' => $tc001,
					...
					foreach($val as $k=>$v){
						if($k!="td001"&&$k!="td003"&&$k!="td004"){//主鍵不用更改以及其他外來鍵庫別名稱 td013日期等別處理
							if($k=="td002") {$data[$k] = $vtd002;} else {$data[$k] = $v;}
						}
					}
					$this->db->where('td001', $tc001);
					$this->db->where('td002', $vtd002);
					$this->db->update('nottd',$data);//更改一筆
					$mtd002 = (int) $vtd002+10;
			        $vtd002 =  (string)$mtd002;
				}else{
					if($val['td003'] && $val['td004']){
						$data = array( 
							'company' => $this->session->userdata('syscompany'),
						...
						foreach($val as $k=>$v){
							if($k!="td001"&&$k!="td003"&&$k!="td004"){//主鍵不用更改以及其他外來鍵庫別名稱
								if($k=="td002") {$data[$k] = $vtd002;} else {$data[$k] = $v;}
							}
						}
						$this->db->insert('nottd', $data);
						$mtd002 = (int) $vtd002+10;
			            $vtd002 =  (string)$mtd002;
					}
				}
	...
	function lookupd_body($keyword){     
		$this->db->select('tc001, tc003, tc005, tc012');
		$this->db->from('nottc');
		$this->db->where('tc001',urldecode(urldecode($this->uri->segment(4))));
		$where = "(`tc012`='1' OR `tc012`='9')";
		$this->db->where($where);
		$query = $this->db->get(); 
		return $query->result();
    }  		
