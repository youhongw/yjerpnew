...
class Notb04_model extends CI_Model {
	...
	function batchaf()           
        {
			preg_match_all('/\d/S',$this->input->post('ta034c'), $matches);  //處理日期字串
			 $ta034c = implode('',$matches[0]);
   ...
   /*  $sql="select mb001,ma003,ma019, 0 as mb0041, 0 as mb0051,mb004,mb005,mb006,mb007,0 as mb008,0 as mb009 
	...
	//	   if ($tb005>='' and $vdail==0) {$this->db->insert('actmb', $data1);}
		   ...
		   if ( $mvtb005a<>$vtb005  or $vdail==0 )  {
                $exist = $this->notb04_model->selone1d($vyy,$vmm,$vtb005);
	            if ($exist)
