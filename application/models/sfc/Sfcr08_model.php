...
class Sfcr08_model extends CI_Model {
	...
	function insertf()    //新增一筆 檔頭  cmsmq
		...
		//	 if ($this->input->post('mq018')!='1') {$mq019=round($this->input->post('mq019'));}
	     ...
	     $data = array( 
	      ...
	      $exist = $this->sfcr08_model->selone1($this->input->post('copq03a23'),$this->input->post('mq002'));
	      if ($exist)
	         {
		      return 'exist';
		     } 
		...
		//	while ($_POST['order_product'][  $n  ]['th004']) {		
	     ...
	     $exist = $this->sfcr08_model->selone1d($this->input->post('copq03a23'),$this->input->post('mq002'),$th003);
		...
		if ($_POST['order_product'][  $n  ]['th004']!='') {
		  ...
		  // 訂單已交數量
			 ...
			 $th016=$_POST['order_product'][ $n  ]['th016'];
			 $th008=$_POST['order_product'][ $n  ]['th008'];
			 $existb = $this->sfcr08_model->selone3d($th014,$th015,$th016);
			  $data_add = array( 
	             'company' => $this->session->userdata('syscompany'),
	             'creator' => $this->session->userdata('manager'),
		         'usr_group' => 'A100',
		 ...
		 $query = $this->db->query($sql);	} 
		  ...
		  //庫存增加減少 品號,庫別, 數量
			 ...
			 $th007=$_POST['order_product'][ $n  ]['th007'];
			 $th008=$_POST['order_product'][ $n  ]['th008'];
			 $exista = $this->sfcr08_model->selone2d($th004,$th007);
			  $data_add = array( 
	             'company' => $this->session->userdata('syscompany'),
	             'creator' => $this->session->userdata('manager'),
		         'usr_group' => 'A100',
    ...
    function copyf()           
        {
         ...
         //   if ($query->num_rows() != 1) { return 'exist'; }
            ...
            $exist = $this->sfcr08_model->selone2($this->input->post('mq001c'),$this->input->post('mq002c'));
		    if ($exist)
		      {
			    return 'exist';
		      }         
	...
	function printfd()          
        {
		  ...
		  $this->db->select('a.ta001,a.ta002,a.ta006,a.ta007,a.ta012,a.ta014,a.ta015,a.ta026,a.ta027,a.ta028,b.*,c.mw002 as tk004disp'); 
