...
class Sfci17a_model extends CI_Model
{
	...
	function construct_sql($limit = 10, $offset = 0, $func = "")
	{
		...
		if ($this->uri->segment(3, 0) == "display_sql") {
			unset($_SESSION['sfci17a']['search']);
		}
		...
		$default_order = "da001 asc"; //在這裡塞入一些預設排序
		...
		if (isset($_SESSION['sfci17a']['search']['where'])) {
			if ($where) {
				$where .= " and ";
			}
			$where .= $_SESSION['sfci17a']['search']['where'];
		}
		...
		if ($order) {
			if ($order == 'da001 asc' || $order == 'da001 desc')
				$order = ' ';
			else
				$order .= " , ";
		}
		...
		if ($this->uri->segment(5)) {
			$seq1 = $where;
			$_SESSION['sfci17a']['search']['seq1'] = $where;
		} else {
			if (isset($_SESSION['sfci17a']['search']['seq1']))
				$seq1 = $_SESSION['sfci17a']['search']['seq1'];
			else
				$seq1 = '';
		}
		...
		$sql = " SELECT a.*,b.MW002 as da013dis FROM molda as a 
						left join CMSMW as b on a.da013 = b.MW001
					WHERE  $where and da001 IN 
			(SELECT TOP $limit da001 FROM molda WHERE $where and da001 NOT IN
				(SELECT TOP $offset  da001 FROM molda WHERE $where ORDER BY $order)
			ORDER BY $order)
		 ORDER BY $order 
		 ";
		if ($where == "") {
			$sql = " SELECT a.*,b.MW002 as da013dis FROM molda as a 
							left join CMSMW as b on a.da013 = b.MW001
						WHERE da001 IN 
				(SELECT TOP $limit da001 FROM molda WHERE da001 NOT IN
					(SELECT TOP $offset da001 FROM molda ORDER BY $order)
				ORDER BY $order)
			ORDER BY $order
			";
		}
		...
		if ($this->uri->segment(3) == "display_search" && ($seq1)) {
			$sql = " SELECT a.*,b.MW002 as da013dis FROM molda as a 
						left join CMSMW as b on a.da013 = b.MW001
					WHERE $seq1 and $where and da001 IN 
			(SELECT TOP $limit da001 FROM molda WHERE $seq1 and $where and da001 NOT IN
				(SELECT TOP $offset  da001 FROM molda WHERE $seq1 and $where ORDER BY $order)
			ORDER BY $order)
		 ORDER BY $order 
		 ";
			if ($where == "") {
				$sql = " SELECT a.*,b.MW002 as da013dis FROM molda as a 
							left join CMSMW as b on a.da013 = b.MW001
						WHERE $seq1 and da001 IN 
				(SELECT TOP $limit da001 FROM molda WHERE $seq1 and da001 NOT IN
					(SELECT TOP $offset da001 FROM molda WHERE $seq1 ORDER BY $order)
				ORDER BY $order)
			ORDER BY $order
			";
			}
		...
		//儲存sql 語法
		$_SESSION['sfci17a']['search']['sql'] = $this->db->last_query();

		$sql = " SELECT count(*) as total_num from molda 
					LEFT JOIN CMSMW ON da013 = MW001 
						WHERE $where ";
		if ($where == "") {
			$sql = " SELECT count(*) as total_num from molda 
						LEFT JOIN CMSMW ON da013 = MW001 
		...
		//儲存where與order
		$_SESSION['sfci17a']['search']['where'] = $where;
		$_SESSION['sfci17a']['search']['order'] = $order;
		$_SESSION['sfci17a']['search']['offset'] = $offset;
	...
	function insertf()
	{
		...
		$sql = " INSERT INTO dbo.molda (creator, create_date, flag, da001, da002, da003, da004, da005, da006, da007, da008, da009, da010, da011, da012, da013, da014, da015, da016)
VALUES ('$creator', '$vtoday', '0', '$vda001', '$vda002', '$vda003', '$vda004', '$vda005', '$vda006', '$vda007', '$vda008', '$vda009', '$vda010', '$vda011', '$vda012', '$vda013', '$vda014', '$vda015', '$vda016'); ";
