...
class Sfcp03_model extends CI_Model
{
	...
	function construct_sql($limit = 10, $offset = 0, $func = "")
	{
		...
		$_SESSION['sfcp03']['search']['sql'] = $this->db->last_query();
	...
	function construct_sql_k($limit = 10, $offset = 0, $func = "")
	{
		...
		$sql21 = " SELECT a.*,b.MB002,b.MB003,b.MB004 FROM sfcpk as a	
	...
	{
		$sql98 = " select a.*,b.MB002 as pk001dis FROM sfcpk as a	
						left join INVMB as b on a.pk001 = b.MB001  
						where pk001 = '$seq1'  ";
	...
	{
		$sql98 = " select * from dbo.sfcpk where pk001 = '$seq1' ";
		$query = $this->db->query($sql98);
	...
	function insertf_k()
	{
		...
		preg_match_all('/\d/S',$this->input->post('pk006'), $matches);  //處理日期字串
			 $pk006 = implode('',$matches[0]);
			 $vpk006=$pk006;
		//$vpk006 = trim($this->input->post('pk006'));
		$vpk005 = iconv("utf-8", "BIG5", $vpk005);
         
		$sql = " INSERT INTO dbo.sfcpk (creator, create_date, flag, pk001, pk002, pk003, pk004, pk005, pk006)
	...
	function updatef_k()
	{
		$modifier = $this->session->userdata('sysuser');
		$vtoday = date('Ymd');
		$vpk001 = trim($this->input->post('pk001'));
		$vpk002 = trim($this->input->post('pk002'));
		$vpk003 = trim($this->input->post('pk003'));
		$vpk004 = trim($this->input->post('pk004'));
		$vpk005 = trim($this->input->post('pk005'));
		...
		$sql = " UPDATE sfcpk 
					...
					, pk006='$vpk006' where pk001 = '$vpk001'  ";
	...
	function delmutif_k()
	{
		...
		if (!empty($_POST['selected'])) {
			foreach ($_POST['selected'] as $check) {
				...
				$relust = $this->db->query(" DELETE FROM dbo.sfcpk WHERE pk001='$seq1' ; ");
			}
		}
		// if ($this->db->affected_rows() > 0) {
		// 	return TRUE;
