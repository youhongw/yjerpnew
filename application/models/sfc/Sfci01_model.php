...
class sfci01_model extends CI_Model
{
	...
	{
		$this->session->set_userdata('sfci01_search', "display_search/" . $offset);
		...
		if (isset($_SESSION['sfci01']['search']['where'])) {
			if ($where) {
				$where .= " and ";
			}
			$where .= $_SESSION['sfci01']['search']['where'];
		}
		...
		if (isset($_SESSION['sfci01']['search']['order'])) {
			if ($order) {
				$order .= " , ";
			}
			$order .= $_SESSION['sfci01']['search']['order'];
		}

		if (!isset($_SESSION['sfci01']['search']['order']) && $default_order) {
			if ($order) {
				$order .= " , ";
			}
			$order .= $default_order;
		...
		// $sql = " SELECT MQ001,MQ002 from CMSMQ WHERE MQ003='D4' ORDER BY MQ001 ";
		// if ($where) {
		// 	$sql = " SELECT MQ001,MQ002 from CMSMQ WHERE MQ003='D4' and " . $where;
		// }

		$sql = " SELECT MQ001,MQ002  FROM  CMSMQ WHERE MQ003='D4' and  $where and MC001 IN 
			(SELECT TOP $limit MQ001 FROM CMSMQ WHERE MQ003='D4' and $where and MQ001 NOT IN
				(SELECT TOP $offset  MQ001 FROM CMSMQ WHERE MQ003='D4' and $where ORDER BY MQ001)
			ORDER BY MQ001)
		 ORDER BY MQ001 
		 ";
		if ($where == "") {
			$sql = " SELECT MQ001,MQ002  FROM  CMSMQ WHERE MQ003='D4' and MQ001 IN 
				(SELECT TOP $limit MQ001 FROM CMSMQ WHERE MQ003='D4' and MQ001 NOT IN
					(SELECT TOP $offset MQ001 FROM CMSMQ WHERE MQ003='D4' ORDER BY MQ001)
				ORDER BY MQ001)
			ORDER BY MQ001
			";
		}
		...
		//儲存sql
		$_SESSION['sfci01']['search']['sql'] = $this->db->last_query();
		...
		$sql = " SELECT count(*) as total_num from CMSMQ WHERE MQ003='D4' and $where ";
		if ($where == "") {
			$sql = " SELECT count(*) as total_num from CMSMQ WHERE MQ003='D4'";
		}
		$query = $this->db->query($sql);
		$ret['num'] =  $query->result()[0]->total_num;

		//儲存where與order
		$_SESSION['sfci01']['search']['where'] = $where;
		$_SESSION['sfci01']['search']['order'] = $order;
		$_SESSION['sfci01']['search']['offset'] = $offset;
	...
	function construct_sqlm($limit = 15, $offset = 0, $func = "")
	{
		...
		if ($this->uri->segment(3, 0) == "clear_sqlm") {
			unset($_SESSION['sfci01']['search']);
		}
		if ($this->uri->segment(3, 0) == "display") {
			unset($_SESSION['sfci01']['search']);
		...
		$sql = " SELECT *  FROM  CMSMQ WHERE (MQ003='D1' or MQ003='D2' or MQ003='D3') and $where and MQ001 IN 
			(SELECT TOP $limit MQ001 FROM CMSMQ WHERE (MQ003='D1' or MQ003='D2' or MQ003='D3') and $where and MQ001 NOT IN
				(SELECT TOP $offset  MQ001 FROM CMSMQ WHERE (MQ003='D1' or MQ003='D2' or MQ003='D3') and $where ORDER BY MQ001)
			ORDER BY MQ001)
		 ORDER BY MQ001 
		 ";
		if ($where == "") {
			$sql = " SELECT *  FROM  CMSMQ WHERE (MQ003='D1' or MQ003='D2' or MQ003='D3') and MQ001 IN 
				(SELECT TOP $limit MQ001 FROM CMSMQ WHERE (MQ003='D1' or MQ003='D2' or MQ003='D3') and MQ001 NOT IN
					(SELECT TOP $offset MQ001 FROM CMSMQ WHERE (MQ003='D1' or MQ003='D2' or MQ003='D3') ORDER BY MQ001)
				ORDER BY MQ001)
			ORDER BY MQ001
			";
		}
		...
		// $ret['num'] = $query->get()->result();
		// $ret['num'] = $ret['num'][0]->total_num;
		$sql = " SELECT count(*) as total_num from CMSMQ WHERE (MQ003='D1' or MQ003='D2' or MQ003='D3') and $where ";
		if ($where == "") {
			$sql = " SELECT count(*) as total_num from CMSMQ WHERE (MQ003='D1' or MQ003='D2' or MQ003='D3')";
		}
		$query = $this->db->query($sql);
		$ret['num'] =  $query->result()[0]->total_num;
	...
	function findf($limit, $offset, $sort_by, $sort_order)
	{
		...
		if (@$_SESSION['admi05_sql_term']) {
			$seq32 = $_SESSION['admi05_sql_term'];
		}
		if (@$_SESSION['admi05_sql_sort']) {
			$seq33 = $_SESSION['admi05_sql_sort'];
		}
	...
	function insertf()
	{
		...
		$exist = $this->sfci01_model->selone1($this->input->post('mq001'));
		if ($exist) {
			return 'exist';
		}
	...
	function copyf()           //複製一筆
	{
		...
		$exist = $this->sfci01_model->selone2($this->input->post('mq002c'));
		if ($exist) {
			return 'exist';
		}
	...
	/*==以下AJAX處理區域==*/
	function ajaxsfci01($seg1)    //ajax 查詢一筆 顯示用 庫別6
	{
		$this->db->set('mq001', $this->uri->segment(4));
		$this->db->where('mq001', $this->uri->segment(4));
		$query = $this->db->get('cmsmq');
	...
	function lookup1($keyword)
	{
		...
		$this->db->from('cmsmq');
		$this->db->where('mq003 >=', 'D');
		$this->db->where('mq003 <=', 'DZZZ');
	...
	{
		$mq001 = urldecode($this->uri->segment(4));
		// $this->db->select('mq001, mq002');
		// $this->db->from('cmsmq');
		// $this->db->where('mq003 >=', 'D');
		// $this->db->where('mq003 <=', 'DZZZ');
		...
		$sql98 = " SELECT MQ001,MQ002 FROM CMSMQ WHERE MQ003='D4' and MQ001='$mq001' ";
		$query = $this->db->query($sql98);

		return $query->result();
	}
	function lookup2m($keyword)
	{
		$mq001 = urldecode($this->uri->segment(4));

		$sql98 = " SELECT MQ001,MQ002 FROM CMSMQ WHERE (MQ003='D1' or MQ003='D2' or MQ003='D3')	 and MQ001='$mq001' ";
		$query = $this->db->query($sql98);
