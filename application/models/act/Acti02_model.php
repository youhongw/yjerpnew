...
class Acti02_model extends CI_Model {
	...
	{
		$this->session->set_userdata('acti02_search',"display_search/".$offset);
		...
		if(isset($_SESSION['acti02']['search']['where'])){
			if($where){$where .= " and ";}
			$where .= $_SESSION['acti02']['search']['where'];
		}
		...
		if(isset($_SESSION['acti02']['search']['order'])){
			if($order){$order .= " , ";}
			$order .= $_SESSION['acti02']['search']['order'];
		}
		
		if(!isset($_SESSION['acti02']['search']['order']) && $default_order){
			if($order){$order .= " , ";}
			$order .= $default_order;
		}
		/* order end */
		...
		$ret['data'] = $query->get()->result();
		//儲存sql
		$_SESSION['acti02']['search']['sql'] = $this->db->last_query();
		...
		//儲存where與order
		$_SESSION['acti02']['search']['where'] = $where;
		$_SESSION['acti02']['search']['order'] = $order;
		$_SESSION['acti02']['search']['offset'] = $offset;
	...
	function insertf()   
       ...
       //    }
	    ...
	    $exist = $this->acti02_model->selone1($this->input->post('mq001'));
	    if ($exist)
	      {
		    return 'exist';
		  } 
    ...
    function copyf()           //複製一筆
       {
	    ...
	    $data = array( 
            ...
            $exist = $this->acti02_model->selone2($this->input->post('mq002c'));
		    if ($exist)
		      {
			   return 'exist';
		      }         
	...
	function lookup1($keyword){     
	  ...
	  $this->db->from('cmsmq'); 
      $this->db->where('mq003 >=', '91');
      $this->db->where('mq003 <=', '93');	  
