...
class Actb04_model extends CI_Model {
	...
	//	if ($vtb005!=$tb005 and $vdail >0 and $tb004==1)  {$tb0071=$tb007;$tb0072=0;$tb0151=$tb015;$tb0152=0;$tb0071c=$tb007c;$tb0072c=0;} else {$tb0072=$tb007;$tb0071=0;$tb0152=$tb015;$tb0151=0;$tb0072c=$tb007c;$tb0071c=0;} 
 	...
 	//0618	 if ($vtb005==$tb005 and  $vdail==0  and $tb004==1 )   {$tb0071=$tb007;$tb0072=0;$tb0151=$tb015;$tb0152=0;$tb0071c=$tb007c;$tb0072c=0;} else {$tb0072=$tb007;$tb0071=0;$tb0152=$tb015;$tb0151=0;$tb0072c=$tb007c;$tb0071c=0;}   //vtc0021 單號+1 
		 ...
		 if ($mvtb005!=$tb005 and  $vdail>0  and $tb004==1 )  {$tb0071=$tb007;$tb0072=0;$tb0151=$tb015;$tb0152=0;$tb0071c=$tb007c;$tb0072c=0;} 
		 if ($mvtb005!=$tb005 and $vdail>0 and $tb004!=1)   {$tb0072=$tb007;$tb0071=0;$tb0152=$tb015;$tb0151=0;$tb0072c=$tb007c;$tb0071c=0;}
		
    	if ($mvtb005==$tb005 and  $vdail>0  and $tb004==1 )   {$tb0071=$tb0071+$tb007;$tb0072=$tb0072+0;$tb0151=$tb0151+$tb015;$tb0152=$tb0152+0;$tb0071c=$tb0071c+$tb007c;$tb0072c=0;} 
		 if ($mvtb005==$tb005 and $vdail>0 and $tb004!=1)     {$tb0072=$tb0072+$tb007;$tb0071=$tb0071+0;$tb0152=$tb0152+$tb015;$tb0151=$tb0151+0;$tb0072c=$tb0072c+$tb007c;$tb0071c=$tb0071c+0;}
	...
	//	   if ($tb005>='' and $vdail==0) {$this->db->insert('actmb', $data1);}
		   ...
		   if ( $mvtb005a<>$vtb005  or $vdail==0 )  {
	            ...
	            if ($exist)
		           ...
		           }else {$this->db->insert('actmba', $data2);}
...
//計算 試算表檔餘額 上期+本期=期未餘額 actmba  mba014, mba015  0623-ok
    function batchdf()           
        {
			$n=0;
			$vyy=substr($this->input->post('ta034c'),0,4);
  ...
  //計算 損益表檔餘額 actmba 本期 8,9 mba016, 14,15累計餘額 mba017  
   ...
   // } 
	//1040630 加上 3219 上期損益
      //科目代號 3218 本期損益	  1040812 都加判斷年月 mbb2002=$vmm	    ACTMC 參數檔      
...
//    加入資產負債表		加本期損益
		  ...
		  WHERE  substring(mbb2003,1,2) = '1 '  and mbb2002=$vmm  ");         
		foreach ($query311->result() as $row)
		 ...
		 $query312 = $this->db->query("SELECT mbb2001,mbb2002,mbb2003 ,mbb2016,mbb2017 FROM actmbb2 
		  WHERE  substring(mbb2003,1,2) = '2 '  and mbb2002=$vmm  ");         
		foreach ($query312->result() as $row)
		 ...
		 $query313 = $this->db->query("SELECT mbb2001,mbb2002,mbb2003 ,mbb2016,mbb2017 FROM actmbb2 
		  WHERE  substring(mbb2003,1,2) = '3 '  and mbb2002=$vmm  ");         
	...
	//		 truncate table actmbc1;
		...
		//負債	
		 ...
		 //計算合併 actmc  1750  資產與負價合併一個檔案 actmbc
    ...
    WHERE A.`idc`=B.`idb`   "; 
		...
		foreach ($query214->result() as $row)
		  ...
		  foreach ($query1999b->result() as $row)
			...
			//清除資產負債左右少的部份
             $query918 = $this->db->query("SELECT count(*) as mc8 from actmbc1 ");         
		  foreach ($query918->result() as $row)
			...
			$query919 = $this->db->query("SELECT count(*) as mc9 from actmbc2 ");  
		  foreach ($query919->result() as $row)
			...
			if ($mc8>=$mc9) {$sql999 = " UPDATE actmbc  set mbc007='', mbc008=''   WHERE  idc>='$mc9'   "; 
		       $this->db->query($sql999);	}
		    if ($mc8<$mc9) {$sql999 = " UPDATE actmbc  set mbc003='', mbc004=''   WHERE  idc>='$mc8'   "; 
		       $this->db->query($sql999);	} 
