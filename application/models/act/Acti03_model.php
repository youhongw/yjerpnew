...
class Acti03_model extends CI_Model {
	...
	function construct_sql($limit = 15, $offset = 0, $func = "")
	{
		...
		/* Data SQL */
		$query = $this->db->select('ma001, ma002, ma003, ma004,ma008,ma018, create_date')
			->from('actma')
			->order_by($order);
		if($where){
			$query->where($where);
	...
	function construct_sql_body($limit = 15, $offset = 0, $func = "")
	{
		...
		if(isset($_SESSION['acti03_body']['search']['where'])){
			if($where){$where .= " and ";}
			$where .= $_SESSION['acti03_body']['search']['where'];
		}
		...
		if(isset($_SESSION['acti03_body']['search']['order'])){
			if($order){$order .= " , ";}
			$order .= $_SESSION['acti03_body']['search']['order'];
		}
		
		if(!isset($_SESSION['acti03_body']['search']['order']) && $default_order){
			if($order){$order .= " , ";}
			$order .= $default_order;
		}
		/* order end */
		...
		/* Data SQL */
		$query = $this->db->select('ma001, ma003, ma008')
			->from('actma')
			->order_by($order);
		...
		$ret['data'] = $query->get()->result();
		//儲存sql
		$_SESSION['acti03_body']['search']['sql'] = $this->db->last_query();
		...
		//儲存where與order
		$_SESSION['acti03_body']['search']['where'] = $where;
		$_SESSION['acti03_body']['search']['order'] = $order;
		$_SESSION['acti03_body']['search']['offset'] = $offset;
	...
	function selbrowse($num,$offset)   
       {            
	    $this->db->select('ma001, ma002,ma003, ma003, ma004, ma005, ma006, create_date');
        $this->db->from('actma');
	...
	function search($limit, $offset, $sort_by, $sort_order)     
	  { 
	    ...
	    $query = $this->db->select('ma001,ma002, ma003, ma007, ma008, ma009, ma011, create_date')
	                      ->from('actma')
	...
	function selone()    
       { 
		 $this->db->select('a.*, c.mf002 as ma018disp,b.ma003 as ma002disp');	
		 $this->db->from('actma as a');
		 $this->db->join('actma as b', 'a.ma002 = b.ma001','left');
		 $this->db->join('cmsmf as c', 'a.ma018 = c.mf001','left');
	     //$this->db->set('ma001', $this->uri->segment(4)); 
	...
	function findf($limit, $offset, $sort_by, $sort_order)     
       {            		
         ...
         if ($seq7!='') {$seq9=" ORDER BY ".$seq7;$seq33=$seq7;}
		  //下一頁不會亂跳
		if(@$_SESSION['acti03_sql_term']){$seq32 = $_SESSION['acti03_sql_term'];}
		if(@$_SESSION['acti03_sql_sort']){$seq33 = $_SESSION['acti03_sql_sort'];}
	...
	//篩選多筆    
	function filterf1($limit, $offset , $sort_by  , $sort_order='asc')    //篩選多筆        
	   {    
	    ...
	    $this->db->select('ma001, ma002, ma003, ma007, ma008, ma009, ma011, create_date');
	    $this->db->from('actma');
	    $this->db->like($sort_by, $seq4, 'after');
	    $this->db->order_by($sort_by, $sort_order);
	   // $this->db->order_by('ma001 asc');
	...
	function lookup1($keyword){     
      $this->db->select('ma001, ma003,ma018');
	  $this->db->from('actma');  
