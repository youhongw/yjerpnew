...
class epsi02_model extends CI_Model {
	...
	{
		$this->session->set_userdata('epsi02_search',"display_search/".$offset);
		...
		if(isset($_SESSION['epsi02']['search']['where'])){
			if($where){$where .= " and ";}
			$where .= $_SESSION['epsi02']['search']['where'];
		}
		...
		if(isset($_SESSION['epsi02']['search']['order'])){
			if($order){$order .= " , ";}
			$order .= $_SESSION['epsi02']['search']['order'];
		}
		
		if(!isset($_SESSION['epsi02']['search']['order']) && $default_order){
			if($order){$order .= " , ";}
			$order .= $default_order;
		}
		/* order end */
		...
		$ret['data'] = $query->get()->result();
		//儲存sql
		$_SESSION['epsi02']['search']['sql'] = $this->db->last_query();
		...
		//儲存where與order
		$_SESSION['epsi02']['search']['where'] = $where;
		$_SESSION['epsi02']['search']['order'] = $order;
		$_SESSION['epsi02']['search']['offset'] = $offset;
	...
	function construct_sql_body($limit = 15, $offset = 0, $func = "")
	{
		...
		if(isset($_SESSION['epsi02_body']['search']['where'])){
			if($where){$where .= " and ";}
			$where .= $_SESSION['epsi02_body']['search']['where'];
		}
		...
		if(isset($_SESSION['epsi02_body']['search']['order'])){
			if($order){$order .= " , ";}
			$order .= $_SESSION['epsi02_body']['search']['order'];
		}
		
		if(!isset($_SESSION['epsi02_body']['search']['order']) && $default_order){
			if($order){$order .= " , ";}
			$order .= $default_order;
		}
		/* order end */
		...
		$ret['data'] = $query->get()->result();
		//儲存sql
		$_SESSION['epsi02_body']['search']['sql'] = $this->db->last_query();
		...
		//儲存where與order
		$_SESSION['epsi02_body']['search']['where'] = $where;
		$_SESSION['epsi02_body']['search']['order'] = $order;
		$_SESSION['epsi02_body']['search']['offset'] = $offset;
	...
	function findf($limit, $offset, $sort_by, $sort_order)     
       {            		
	     ...
	     $seq1 = " ma001, ma002, ma003, ma004,  create_date FROM `epsma` ";
         ...
         if ($seq7!='') {$seq9=" ORDER BY ".$seq7;$seq33=$seq7;}
		  //下一頁不會亂跳
		if(@$_SESSION['epsi02_sql_term']){$seq32 = $_SESSION['epsi02_sql_term'];}
		if(@$_SESSION['epsi02_sql_sort']){$seq33 = $_SESSION['epsi02_sql_sort'];}
	...
	function insertf()   
       {
	    ...
	    $exist = $this->epsi02_model->selone1($this->input->post('ma001'));
	    if ($exist)
	      {
		    return 'exist';
		  } 
           return  $this->db->insert('epsma', $data);
       }
    ...
    function copyf()           //複製一筆
       {
	    ...
	    $data = array( 
            ...
            $exist = $this->epsi02_model->selone2($this->input->post('ma002c'));
		    if ($exist)
		      {
			   return 'exist';
		      }         
              return $this->db->insert('epsma', $data);      //複製一筆  
       }	
	...
	function excelnewf()           
       {			
	    ...
	    $seq2=$this->input->post('ma002c');
	    $sql = " SELECT ma001,ma002,ma004,ma003,create_date FROM epsma WHERE ma001 >= '$seq1'  AND ma001 <= '$seq2' ORDER BY ma001 "; 
	...
	function printfd()          
       {
	    $seq1=$this->input->post('ma001c');    //查詢一筆以上
	    $seq2=$this->input->post('ma002c'); 
	    $sql = " SELECT * FROM epsma WHERE ma001 >= '$seq1'  AND ma001 <= '$seq2'  "; 
        $query = $this->db->query($sql);
	    $ret['rows'] = $query->result();
	...
	/*==以下AJAX處理區域==*/
	 function ajaxepsi02($seg1)    //ajax 查詢一筆 顯示用 費用6
          { 	              
	    $this->db->set('ma001', $this->uri->segment(4));
	    $this->db->where('ma001', $this->uri->segment(4));	
	    $query = $this->db->get('epsma');
	...
	function lookup_old($select_col=array(),$search_col=array(),$keyword=array(),$limit=10){
		...
		if($sel_col == ""){$sel_col = "*";}
		$this->db->select($sel_col)->from('epsma');
