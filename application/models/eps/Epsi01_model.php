...
class epsi01_model extends CI_Model {
	...
	{
		$this->session->set_userdata('epsi01_search',"display_search/".$offset);
		...
		if(isset($_SESSION['epsi01']['search']['where'])){
			if($where){$where .= " and ";}
			$where .= $_SESSION['epsi01']['search']['where'];
		}
		...
		if(isset($_SESSION['epsi01']['search']['order'])){
			if($order){$order .= " , ";}
			$order .= $_SESSION['epsi01']['search']['order'];
		}
		
		if(!isset($_SESSION['epsi01']['search']['order']) && $default_order){
			if($order){$order .= " , ";}
			$order .= $default_order;
		}
		/* order end */
		...
		$ret['data'] = $query->get()->result();
		//儲存sql
		$_SESSION['epsi01']['search']['sql'] = $this->db->last_query();
		...
		//儲存where與order
		$_SESSION['epsi01']['search']['where'] = $where;
		$_SESSION['epsi01']['search']['order'] = $order;
		$_SESSION['epsi01']['search']['offset'] = $offset;
	...
	function insertf()   
       ...
       //    }
	    ...
	    $exist = $this->epsi01_model->selone1($this->input->post('mq001'));
	    if ($exist)
	      {
		    return 'exist';
		  } 
    ...
    function copyf()           //複製一筆
       {
	    ...
	    $data = array( 
            ...
            $exist = $this->epsi01_model->selone2($this->input->post('mq002c'));
		    if ($exist)
		      {
			   return 'exist';
		      }         
	...
	function printfd()          
       {
	    $seq1=$this->input->post('mq001c');    //查詢一筆以上
	    $seq2=$this->input->post('mq002c'); 
	    $sql = " SELECT * FROM cmsmq WHERE mq001 >= '$seq1'  AND mq001 <= '$seq2' and mq003 >= 'B1'  AND mq003 <= 'B4ZZ'  "; 
        $query = $this->db->query($sql);
	    $ret['rows'] = $query->result();
        $seq32 = "mq001 >= '$seq1'  AND mq001 <= '$seq2'  and mq003 >= 'B1'  AND mq003 <= 'B4ZZ' ";	
	    $query = $this->db->select('COUNT(*) as count', FALSE)  //筆數查詢,如果設為FALSE不會使用反引號保護你的字段或者表名
		                  ->from('cmsmq')
		                  ->where($seq32);
	    $num = $query->get()->result();		
	...
	/*==以下AJAX處理區域==*/
	 function ajaxepsi01($seg1)    //ajax 查詢一筆 顯示用 庫別6
          { 	              
	    $this->db->set('mq001', $this->uri->segment(4));
	    $this->db->where('mq001', $this->uri->segment(4));	
	    $query = $this->db->get('cmsmq');
