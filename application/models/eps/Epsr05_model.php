...
class epsr05_model extends CI_Model {
	...
	function insertf()    //新增一筆 檔頭  epsth
		...
		//	 if ($this->input->post('ma018')!='1') {$ma019=round($this->input->post('ma019'));}
	     ...
	     $data = array( 
	      ...
	      $exist = $this->epsr05_model->selone1($this->input->post('copq03a23'),$this->input->post('ma002'));
	      if ($exist)
	         {
		      return 'exist';
		     } 
		...
		//	while ($_POST['order_product'][  $n  ]['th004']) {		
	     ...
	     $exist = $this->epsr05_model->selone1d($this->input->post('copq03a23'),$this->input->post('ma002'),$th003);
		...
		if ($_POST['order_product'][  $n  ]['th004']!='') {
		  ...
		  // 訂單已交數量
			 ...
			 $th016=$_POST['order_product'][ $n  ]['th016'];
			 $th008=$_POST['order_product'][ $n  ]['th008'];
			 $existb = $this->epsr05_model->selone3d($th014,$th015,$th016);
			  $data_add = array( 
	             'company' => $this->session->userdata('syscompany'),
	             'creator' => $this->session->userdata('manager'),
		         'usr_group' => 'A100',
		 ...
		 $query = $this->db->query($sql);	} 
		  ...
		  //庫存增加減少 品號,庫別, 數量
			 ...
			 $th007=$_POST['order_product'][ $n  ]['th007'];
			 $th008=$_POST['order_product'][ $n  ]['th008'];
			 $exista = $this->epsr05_model->selone2d($th004,$th007);
			  $data_add = array( 
	             'company' => $this->session->userdata('syscompany'),
	             'creator' => $this->session->userdata('manager'),
		         'usr_group' => 'A100',
    ...
    function copyf()           
        {
         ...
         //   if ($query->num_rows() != 1) { return 'exist'; }
            ...
            $exist = $this->epsr05_model->selone2($this->input->post('ma001c'),$this->input->post('ma002c'));
		    if ($exist)
		      {
			    return 'exist';
		      }         
	...
	//	  FROM epsth as a, copth as b WHERE ma001=th001 and ma002=th002 and  ma001 >= '$seq1'  AND ma001 <= '$seq2' AND ma002 >= '$seq3'  AND ma002 <= '$seq4'  "; 
		 $query = $this->db->query($sql);
	...
	function printfd()          
	    ...
	    //  $seq4=$this->input->post('tc002c');
		  ...
		  $seq5=$this->input->post('tc003o');   //品號 
	      $seq6=$this->input->post('tc003o');
		         if ($seq2=='') {$seq2='zz';}
		  if ($seq4=='') {$seq4='zz';}
		  if ($seq6=='') {$seq6='zz';}
		  $this->db->select('a.*,b.ti004,b.ti005,b.ti006,b.ti007,b.ti008,b.ti011,b.ti013,b.ti016,b.ti018,c.ma002 as th004disp'); 
        $this->db->from('epsth as a');	
        $this->db->join('epsti as b','a.th001=b.ti001 and a.th002=b.ti002','left');
		$this->db->join('purma as c','a.th004=c.ma001','left');
		$this->db->where('a.th002 >=', $seq1); 
	    $this->db->where('a.th002 <=', $seq2); 
		$this->db->where('a.th017 >=', $seq3); 
	    $this->db->where('a.th017 <=', $seq4); 
		$this->db->where('a.th004 >=', $seq5); 
	    $this->db->where('a.th004 <=', $seq6); 
		$this->db->group_by(array("th001", "th002")); 
		$this->db->order_by('a.th001 , a.th002');
		
		$query = $this->db->get();		  
		
	      $ret['rows'] = $query->result();  
          $seq32 = "th001 >= '$seq1'  AND th001 <= '$seq2' and ti003 >= '$seq3'  AND ti003 <= '$seq4'   ";	
	      $query = $this->db->select('COUNT(*) as count', FALSE)  //筆數查詢,如果設為FALSE不會使用反引號保護你的字段或者表名
		              ->from('epsth as a')
					  ->join('epsti as b','a.th001=b.ti001 and a.th002=b.ti002','left')
		              ->where($seq32);
	      $num = $query->get()->result();		
	      $ret['num_rows'] = $num[0]->count;		
	      return $ret;
	...
	function printfc()   
      {           
		  ...
		  b.th006, b.th007, b.th008, b.th009, b.th010, b.th011, b.th012,b.th013, b.th014,b.th016,b.th017,b.th018,b.th019,b.th031,i.ti002 as th007disp,j.ma002 as ma005disp');
	...
	function updatef()   
		...
		//	 while ($_POST['order_product'][  $n  ]['th004']) {
		   ...
		   //重新計算貨款 epsth
		  $sql = " UPDATE epsth set ma013='$th035',ma025='$th036',ma031='$th045',ma046='$th038',ma033='$th008b' WHERE ma001 = '$ma001'  AND ma002 = '$ma002'  "; 
		 $query = $this->db->query($sql);		   
