...
class Epsr03_model extends CI_Model {
	...
	function insertf()    //新增一筆 檔頭  epsma
		...
		//	 if ($this->input->post('ma018')!='1') {$ma019=round($this->input->post('ma019'));}
	     ...
	     $data = array( 
	      ...
	      $exist = $this->epsr03_model->selone1($this->input->post('copq03a23'),$this->input->post('ma002'));
	      if ($exist)
	         {
		      return 'exist';
		     } 
		...
		//	while ($_POST['order_product'][  $n  ]['th004']) {		
	     ...
	     $exist = $this->epsr03_model->selone1d($this->input->post('copq03a23'),$this->input->post('ma002'),$th003);
		...
		if ($_POST['order_product'][  $n  ]['th004']!='') {
		  ...
		  // 訂單已交數量
			 ...
			 $th016=$_POST['order_product'][ $n  ]['th016'];
			 $th008=$_POST['order_product'][ $n  ]['th008'];
			 $existb = $this->epsr03_model->selone3d($th014,$th015,$th016);
			  $data_add = array( 
	             'company' => $this->session->userdata('syscompany'),
	             'creator' => $this->session->userdata('manager'),
		         'usr_group' => 'A100',
		 ...
		 $query = $this->db->query($sql);	} 
		  ...
		  //庫存增加減少 品號,庫別, 數量
			 ...
			 $th007=$_POST['order_product'][ $n  ]['th007'];
			 $th008=$_POST['order_product'][ $n  ]['th008'];
			 $exista = $this->epsr03_model->selone2d($th004,$th007);
			  $data_add = array( 
	             'company' => $this->session->userdata('syscompany'),
	             'creator' => $this->session->userdata('manager'),
		         'usr_group' => 'A100',
    ...
    function copyf()           
        {
         ...
         //   if ($query->num_rows() != 1) { return 'exist'; }
            ...
            $exist = $this->epsr03_model->selone2($this->input->post('ma001c'),$this->input->post('ma002c'));
		    if ($exist)
		      {
			    return 'exist';
		      }         
	...
	function printfd()          
	    ...
	    //  $seq4=substr($this->input->post('datec'),0,4).substr($this->input->post('datec'),5,2).substr(rtrim($this->input->post('datec')),8,2);
	      ...
	      $seq6=$this->input->post('invq02a1');
		   if ($seq2=='') {$seq2='zz';}
		  if ($seq4=='') {$seq4='zz';}
		  $this->db->select('a.mb001,a.mb002,a.mb003,b.*,c.mb002 as mb001disp,c.mb003 as mb001disp1'); 
        $this->db->from('epsmb as a');	
        $this->db->join('epsmc as b','a.mb001=b.mc001 and a.mb002=b.mc002','left');
		$this->db->join('invmb as c','a.mb001=c.mb001','left');
		$this->db->where('a.mb001 >=', $seq1); 
	    $this->db->where('a.mb001 <=', $seq2); 
		$this->db->where('b.mc003 >=', $seq3); 
	    $this->db->where('b.mc003 <=', $seq4); 
		...
		$query = $this->db->get();		  
	      ...
	      $ret['rows'] = $query->result();  
          $seq32 = "mb001 >= '$seq1'  AND mb001 <= '$seq2' and mc003 >= '$seq3'  AND mc003 <= '$seq4'   ";	
	      $query = $this->db->select('COUNT(*) as count', FALSE)  //筆數查詢,如果設為FALSE不會使用反引號保護你的字段或者表名
		              ->from('epsmb as a')
