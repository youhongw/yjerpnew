...
class epsr17_model extends CI_Model {
	...
	function insertf()    //新增一筆 檔頭  epsth
		...
		//	 if ($this->input->post('ma018')!='1') {$ma019=round($this->input->post('ma019'));}
	     ...
	     $data = array( 
	      ...
	      $exist = $this->epsr17_model->selone1($this->input->post('copq03a23'),$this->input->post('ma002'));
	      if ($exist)
	         {
		      return 'exist';
		     } 
		...
		//	while ($_POST['order_product'][  $n  ]['tf004']) {		
	     ...
	     $exist = $this->epsr17_model->selone1d($this->input->post('copq03a23'),$this->input->post('ma002'),$tf003);
		...
		if ($_POST['order_product'][  $n  ]['tf004']!='') {
		  ...
		  // 訂單已交數量
			 ...
			 $tf016=$_POST['order_product'][ $n  ]['tf016'];
			 $tf008=$_POST['order_product'][ $n  ]['tf008'];
			 $existb = $this->epsr17_model->selone3d($tf014,$tf015,$tf016);
			  $data_add = array( 
	             'company' => $this->session->userdata('syscompany'),
	             'creator' => $this->session->userdata('manager'),
		         'usr_group' => 'A100',
		 ...
		 $query = $this->db->query($sql);	} 
		  ...
		  //庫存增加減少 品號,庫別, 數量
			 ...
			 $tf007=$_POST['order_product'][ $n  ]['tf007'];
			 $tf008=$_POST['order_product'][ $n  ]['tf008'];
			 $exista = $this->epsr17_model->selone2d($tf004,$tf007);
			  $data_add = array( 
	             'company' => $this->session->userdata('syscompany'),
	             'creator' => $this->session->userdata('manager'),
		         'usr_group' => 'A100',
    ...
    function copyf()           
        {
         ...
         //   if ($query->num_rows() != 1) { return 'exist'; }
            ...
            $exist = $this->epsr17_model->selone2($this->input->post('ma001c'),$this->input->post('ma002c'));
		    if ($exist)
		      {
			    return 'exist';
		      }         
	...
	function printfd()          
	    ...
	    //  $seq4=$this->input->post('tc002c');
	      ...
	      $seq6=$this->input->post('tc002c');
		   if ($seq2=='') {$seq2='zz';}
		  if ($seq4=='') {$seq4='zz';}
		  $this->db->select('a.*,b.ti003'); 
        $this->db->from('coptj as a');	
		$this->db->join('copti as b','a.tj001=b.ti001 and a.tj002=b.ti002 ','left');
		$this->db->where('a.tj002 >=', $seq1); 
	    $this->db->where('a.tj002 <=', $seq2); 
		$this->db->where('b.ti003 >=', $seq3); 
	    $this->db->where('b.ti003 <=', $seq4); 
	//	$this->db->group_by(array("tf001", "tf002")); 
		$this->db->order_by('a.tj001 , a.tj002');
		
		$query = $this->db->get();		  
		
	      $ret['rows'] = $query->result();  
          $seq32 = "tj002 >= '$seq1'  AND tj002 <= '$seq2'  AND ti003 >= '$seq3'  AND ti003 <= '$seq4'  ";	
	      $query = $this->db->select('COUNT(*) as count', FALSE)  //筆數查詢,如果設為FALSE不會使用反引號保護你的字段或者表名
		              ->from('coptj as a')
					  ->join('copti as b','a.tj001=b.ti001 and a.tj002=b.ti002 ','left')
		              ->where($seq32);
	      $num = $query->get()->result();		
	      $ret['num_rows'] = $num[0]->count;		
	      return $ret;
