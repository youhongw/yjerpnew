...
class epsr15_model extends CI_Model {
	...
	function insertf()    //新增一筆 檔頭  epsth
		...
		//	 if ($this->input->post('ma018')!='1') {$ma019=round($this->input->post('ma019'));}
	     ...
	     $data = array( 
	      ...
	      $exist = $this->epsr15_model->selone1($this->input->post('copq03a23'),$this->input->post('ma002'));
	      if ($exist)
	         {
		      return 'exist';
		     } 
		...
		//	while ($_POST['order_product'][  $n  ]['tf004']) {		
	     ...
	     $exist = $this->epsr15_model->selone1d($this->input->post('copq03a23'),$this->input->post('ma002'),$tf003);
		...
		if ($_POST['order_product'][  $n  ]['tf004']!='') {
		  ...
		  // 訂單已交數量
			 ...
			 $tf016=$_POST['order_product'][ $n  ]['tf016'];
			 $tf008=$_POST['order_product'][ $n  ]['tf008'];
			 $existb = $this->epsr15_model->selone3d($tf014,$tf015,$tf016);
			  $data_add = array( 
	             'company' => $this->session->userdata('syscompany'),
	             'creator' => $this->session->userdata('manager'),
		         'usr_group' => 'A100',
		 ...
		 $query = $this->db->query($sql);	} 
		  ...
		  //庫存增加減少 品號,庫別, 數量
			 ...
			 $tf007=$_POST['order_product'][ $n  ]['tf007'];
			 $tf008=$_POST['order_product'][ $n  ]['tf008'];
			 $exista = $this->epsr15_model->selone2d($tf004,$tf007);
			  $data_add = array( 
	             'company' => $this->session->userdata('syscompany'),
	             'creator' => $this->session->userdata('manager'),
		         'usr_group' => 'A100',
    ...
    function copyf()           
        {
         ...
         //   if ($query->num_rows() != 1) { return 'exist'; }
            ...
            $exist = $this->epsr15_model->selone2($this->input->post('ma001c'),$this->input->post('ma002c'));
		    if ($exist)
		      {
			    return 'exist';
		      }         
	...
	function printfd()          
	    ...
	    //  $seq4=$this->input->post('tc002c');
		  ...
		  $this->db->select('a.*,b.te006,b.te007,b.te009,b.te010,b.te012,c.ma002 as te010disp'); 
        $this->db->from('epstd as a');	
		$this->db->join('epste as b','a.td001=b.te001 and a.td002=b.te002','left');
		$this->db->join('copma as c','b.te010=c.ma001','left');
		$this->db->where('a.td002 >=', $seq1); 
	    $this->db->where('a.td002 <=', $seq2); 
		$this->db->where('a.td004 >=', $seq3); 
	    $this->db->where('a.td004 <=', $seq4); 
		$this->db->where('a.td016 >=', $seq5); 
	    $this->db->where('a.td016 <=', $seq6);
		$this->db->where('a.td005 >=', $seq7); 
	    $this->db->where('a.td005 <=', $seq8);
		$this->db->group_by(array("td001", "td002")); 
		$this->db->order_by('a.td001 , a.td002,b.te003');
		
		$query = $this->db->get();		  
		
	      $ret['rows'] = $query->result();  
          $seq32 = "td002 >= '$seq1'  AND td002 <= '$seq2' and td004 >= '$seq3'  AND td004 <= '$seq4'   ";	
	      $query = $this->db->select('COUNT(*) as count', FALSE)  //筆數查詢,如果設為FALSE不會使用反引號保護你的字段或者表名
		              ->from('epstd as a')
					  ->join('epste as b','a.td001=b.te001 and a.td002=b.te002','left')
					  ->join('copma as c','b.te010=c.ma001','left')
		              ->where($seq32);
	      $num = $query->get()->result();		
	      $ret['num_rows'] = $num[0]->count;		
	      return $ret;
