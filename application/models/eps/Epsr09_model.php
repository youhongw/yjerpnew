...
class epsr09_model extends CI_Model {
	...
	function insertf()    //新增一筆 檔頭  epsth
		...
		//	 if ($this->input->post('ma018')!='1') {$ma019=round($this->input->post('ma019'));}
	     ...
	     $data = array( 
	      ...
	      $exist = $this->epsr09_model->selone1($this->input->post('copq03a23'),$this->input->post('ma002'));
	      if ($exist)
	         {
		      return 'exist';
		     } 
		...
		//	while ($_POST['order_product'][  $n  ]['tf004']) {		
	     ...
	     $exist = $this->epsr09_model->selone1d($this->input->post('copq03a23'),$this->input->post('ma002'),$tf003);
		...
		if ($_POST['order_product'][  $n  ]['tf004']!='') {
		  ...
		  // 訂單已交數量
			 ...
			 $tf016=$_POST['order_product'][ $n  ]['tf016'];
			 $tf008=$_POST['order_product'][ $n  ]['tf008'];
			 $existb = $this->epsr09_model->selone3d($tf014,$tf015,$tf016);
			  $data_add = array( 
	             'company' => $this->session->userdata('syscompany'),
	             'creator' => $this->session->userdata('manager'),
		         'usr_group' => 'A100',
		 ...
		 $query = $this->db->query($sql);	} 
		  ...
		  //庫存增加減少 品號,庫別, 數量
			 ...
			 $tf007=$_POST['order_product'][ $n  ]['tf007'];
			 $tf008=$_POST['order_product'][ $n  ]['tf008'];
			 $exista = $this->epsr09_model->selone2d($tf004,$tf007);
			  $data_add = array( 
	             'company' => $this->session->userdata('syscompany'),
	             'creator' => $this->session->userdata('manager'),
		         'usr_group' => 'A100',
    ...
    function copyf()           
        {
         ...
         //   if ($query->num_rows() != 1) { return 'exist'; }
            ...
            $exist = $this->epsr09_model->selone2($this->input->post('ma001c'),$this->input->post('ma002c'));
		    if ($exist)
		      {
			    return 'exist';
		      }         
	...
	function printfd()          
	    ...
	    //  $seq4=$this->input->post('tc002c');
		  ...
		  $seq7=substr($this->input->post('dateo2'),0,4).substr($this->input->post('dateo2'),5,2).substr(rtrim($this->input->post('dateo2')),8,2);
	      $seq8=substr($this->input->post('datec2'),0,4).substr($this->input->post('datec2'),5,2).substr(rtrim($this->input->post('datec2')),8,2);
		  
		  
		  $seq9=$this->input->post('tc002o');   //品號 
	      $seq10=$this->input->post('tc002c');
		         if ($seq2=='') {$seq2='zz';}
		  if ($seq4=='') {$seq4='zz';}
		  if ($seq6=='') {$seq6='zz';}
		  if ($seq8=='') {$seq8='zz';}
        ...
        $this->db->where('a.tf009 >=', $seq5); 
	    $this->db->where('a.tf009 <=', $seq6); 
        $this->db->where('a.tf011 >=', $seq7); 
	    $this->db->where('a.tf011 <=', $seq8); 
        $this->db->where('a.tf002 >=', $seq9); 
	    $this->db->where('a.tf002 <=', $seq10); 		
		...
		$query = $this->db->get();		  
	      ...
	      $ret['rows'] = $query->result();  
          $seq32 = "tf001 >= '$seq1'  AND tf001 <= '$seq2'    ";	
	      $query = $this->db->select('COUNT(*) as count', FALSE)  //筆數查詢,如果設為FALSE不會使用反引號保護你的字段或者表名
		              ->from('epstf as a')
