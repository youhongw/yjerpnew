...
class ajsi15_model extends CI_Model {
	...
	function search($limit, $offset, $sort_by, $sort_order)     
	    { 
	      ...
	      $query = $this->db->select(' a.*,b.mq002 as mb002disp,c.mq002 as mb003disp ')
	                       ->from('ajsmb a')
						   ->join('cmsmq b','a.mb002 = b.mq001 and b.mq003="58" ','left')
	...
	function selone($seg1)    
        { 
          ...
          $this->db->join('cmsmq as b', 'a.mb002 = b.mq001 and b.mq003="58" ','left');	//單別	
	...
	function findf($limit, $offset, $sort_by, $sort_order)   
        {            		
         ...
         $seq7="mb001 ";
		  ...
		  //下一頁不會亂跳
		if(@$_SESSION['ajsi15_sql_term']){$seq32 = $_SESSION['ajsi15_sql_term'];}
		if(@$_SESSION['ajsi15_sql_sort']){$seq33 = $_SESSION['ajsi15_sql_sort'];}
	...
	function insertf()    
	    ...
	    $exist = $this->ajsi15_model->selone1($this->input->post('mb001'),$this->input->post('puri04'));   //查詢資料是否重複
	    if ($exist)
	     {
		  return 'exist';
		 } 
    ...
    function copyf()           
        {
	      ...
	      $data = array( 
           ...
           $exist = $this->ajsi15_model->selone2($this->input->post('mb002c'));
		   if ($exist)
		    {
			  return 'exist';
		    }         
	...
	{
		$this->session->set_userdata('ajsi15_search',"display_search/".$offset);
		...
		if(isset($_SESSION['ajsi15']['search']['where'])){
			if($where){$where .= " and ";}
			$where .= $_SESSION['ajsi15']['search']['where'];
		}
		...
		if(isset($_SESSION['ajsi15']['search']['order'])){
			if($order){$order .= " , ";}
			$order .= $_SESSION['ajsi15']['search']['order'];
		}
		
		if(!isset($_SESSION['ajsi15']['search']['order']) && $default_order){
			if($order){$order .= " , ";}
			$order .= $default_order;
		}
		/* order end */
		...
		$ret['data'] = $query->get()->result();
		//儲存sql
		$_SESSION['ajsi15']['search']['sql'] = $this->db->last_query();
		...
		//儲存where與order
		$_SESSION['ajsi15']['search']['where'] = $where;
		$_SESSION['ajsi15']['search']['order'] = $order;
		$_SESSION['ajsi15']['search']['offset'] = $offset;
