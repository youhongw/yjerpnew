...
class ajsi16_model extends CI_Model {
	...
	function findf($limit, $offset, $sort_by, $sort_order)   
        {            		
         ...
         $seq7="mb001 ";
		  ...
		  //下一頁不會亂跳
		if(@$_SESSION['ajsi16_sql_term']){$seq32 = $_SESSION['ajsi16_sql_term'];}
		if(@$_SESSION['ajsi16_sql_sort']){$seq33 = $_SESSION['ajsi16_sql_sort'];}
	...
	function insertf()    
	    ...
	    $exist = $this->ajsi16_model->selone1($this->input->post('mb001'),$this->input->post('puri04'));   //查詢資料是否重複
	    if ($exist)
	     {
		  return 'exist';
		 } 
    ...
    function copyf()           
        {
	      ...
	      $data = array( 
           ...
           $exist = $this->ajsi16_model->selone2($this->input->post('mb002c'));
		   if ($exist)
		    {
			  return 'exist';
		    }         
	...
	{
		$this->session->set_userdata('ajsi16_search',"display_search/".$offset);
		...
		if(isset($_SESSION['ajsi16']['search']['where'])){
			if($where){$where .= " and ";}
			$where .= $_SESSION['ajsi16']['search']['where'];
		}
		...
		if(isset($_SESSION['ajsi16']['search']['order'])){
			if($order){$order .= " , ";}
			$order .= $_SESSION['ajsi16']['search']['order'];
		}
		
		if(!isset($_SESSION['ajsi16']['search']['order']) && $default_order){
			if($order){$order .= " , ";}
			$order .= $default_order;
		}
		/* order end */
		...
		$ret['data'] = $query->get()->result();
		//儲存sql
		$_SESSION['ajsi16']['search']['sql'] = $this->db->last_query();
		...
		//儲存where與order
		$_SESSION['ajsi16']['search']['where'] = $where;
		$_SESSION['ajsi16']['search']['order'] = $order;
		$_SESSION['ajsi16']['search']['offset'] = $offset;
