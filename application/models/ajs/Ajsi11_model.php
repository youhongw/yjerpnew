...
class ajsi11_model extends CI_Model {
	...
	function search($limit, $offset, $sort_by, $sort_order)     
	    { 
	      ...
	      $query = $this->db->select(' a.*,b.mq002 as mb002disp,c.mq002 as mb003disp ')
	                       ->from('ajsmb a')
						   ->join('cmsmq b','a.mb002 = b.mq001 and b.mq003="71" ','left')
	...
	function selone($seg1)    
        { 
          ...
          $this->db->join('cmsmq as b', 'a.mb002 = b.mq001 and b.mq003="71" ','left');	//單別	
	...
	function findf($limit, $offset, $sort_by, $sort_order)   
        {            		
         ...
         $seq7="mb001 ";
		  ...
		  //下一頁不會亂跳
		if(@$_SESSION['ajsi11_sql_term']){$seq32 = $_SESSION['ajsi11_sql_term'];}
		if(@$_SESSION['ajsi11_sql_sort']){$seq33 = $_SESSION['ajsi11_sql_sort'];}
	...
	function selone1($seq1)    
        {
	      ...
	      $this->db->where('mb001', $this->input->post('mb001')); 
		   $this->db->where('mb002', $this->input->post('acpq01a71'));
	      $query = $this->db->get('ajsmb');
	...
	function insertf()    
	    ...
	    $data = array( 
		              ...
		              'mb002' => $this->input->post('acpq01a71'),
	    ...
	    $exist = $this->ajsi11_model->selone1($this->input->post('mb001'),$this->input->post('acpq01a71'));   //查詢資料是否重複
	    if ($exist)
	     {
		  return 'exist';
		 } 
    ...
    function copyf()           
        {
	      ...
	      $data = array( 
           ...
           $exist = $this->ajsi11_model->selone2($this->input->post('mb002c'));
		   if ($exist)
		    {
			  return 'exist';
		    }         
	...
	{
		$this->session->set_userdata('ajsi11_search',"display_search/".$offset);
		...
		if(isset($_SESSION['ajsi11']['search']['where'])){
			if($where){$where .= " and ";}
			$where .= $_SESSION['ajsi11']['search']['where'];
		}
		...
		if(isset($_SESSION['ajsi11']['search']['order'])){
			if($order){$order .= " , ";}
			$order .= $_SESSION['ajsi11']['search']['order'];
		}
		
		if(!isset($_SESSION['ajsi11']['search']['order']) && $default_order){
			if($order){$order .= " , ";}
			$order .= $default_order;
		}
		/* order end */
		...
		$ret['data'] = $query->get()->result();
		//儲存sql
		$_SESSION['ajsi11']['search']['sql'] = $this->db->last_query();
		...
		//儲存where與order
		$_SESSION['ajsi11']['search']['where'] = $where;
		$_SESSION['ajsi11']['search']['order'] = $order;
		$_SESSION['ajsi11']['search']['offset'] = $offset;
