...
class cmsi06a_model extends CI_Model {
	...
	function selbrowse($num,$offset)   //查詢 table 表所有資料
      {            
	   ...
	   // $this->db->order_by('mf001 asc, mb002 asc');    //排序  單欄以上 asc 由小至大 desc預設由大至小
	...
	//	$array = array('mf001' => '1', 'mb002 >=' => $seq6, 'mb002 <=' => $seq6 );
	//	$this->db->like('title', 'match', 'after'); 
	    $sort_order = (substr($sort_order,0,3) == 'asc') ? 'asc' : 'desc';
	    $sort_columns = array('mf001', 'mf002', 'mg003');
	    $sort_by = (in_array($sort_by, $sort_columns)) ? $sort_by : 'mf001';  //檢查排序欄位是否在 table 內
	    $query = $this->db->select('mf001, mf002, mg003')
	                      ->from('cmsmf as a')
						  ->join('cmsmg as b', 'a.mf001 = b.mg001','left')
	    ...
	    $ret['rows'] = $query->get()->result();
	     ...
	     $array = array('mf001' => '1', 'mb002' => $seq6 );
	    $query = $this->db->select('COUNT(*) as count', FALSE)  //筆數查詢,如果設為FALSE不會使用反引號保護你的字段或者表名
	                      ->from('cmsmf')
						  ->join('cmsmg as b', 'a.mf001 = b.mg001','left')
						   ->like($seq4, $seq6, 'after');  
	...
	function findf($limit, $offset, $sort_by, $sort_order)    //查詢多筆進階查詢 mysql 
	  ...
	  //$seq5='';$seq51='';$seq7='';$seq71='';		  
	    ...
	    if (trim($this->input->post('find007'))!='') 
	     ...
	     $sort_columns = array('mf001', 'mf002');
	     $sort_by = (in_array($sort_by, $sort_columns)) ? $sort_by : 'mf001';  //檢查排序欄位是否在 table 內
	     $query = $this->db->select('mf001, mf002')
	                       ->from('cmsmf')
		               ->where($seq32)
			       ->order_by($seq33)
			     //->order_by($sort_by, $sort_order)
	...
	//	$this->db->where('mf001','1');
      ...
      function ajaxcmsi06a($seg1)    //ajax 查詢一筆 不更新網頁 廠別 
          { 	              
