...
class Cmsi11_model extends CI_Model {
	...
	{
		$this->session->set_userdata('cmsi11_search',"display_search/".$offset);
		...
		if(isset($_SESSION['cmsi11']['search']['where'])){
			if($where){$where .= " and ";}
			$where .= $_SESSION['cmsi11']['search']['where'];
		}
		...
		if(isset($_SESSION['cmsi11']['search']['order'])){
			if($order){$order .= " , ";}
			$order .= $_SESSION['cmsi11']['search']['order'];
		}
		
		if(!isset($_SESSION['cmsi11']['search']['order']) && $default_order){
			if($order){$order .= " , ";}
			$order .= $default_order;
		}
		/* order end */
		...
		/* Data SQL */
		$query = $this->db->select('ma001, ma002, ma003, ma004,ma013, create_date')
			->from('taxma')
			->order_by($order);
		if($where){
			$query->where($where);
		...
		$ret['data'] = $query->get()->result();
		//儲存sql
		$_SESSION['cmsi11']['search']['sql'] = $this->db->last_query();
		...
		//儲存where與order
		$_SESSION['cmsi11']['search']['where'] = $where;
		$_SESSION['cmsi11']['search']['order'] = $order;
		$_SESSION['cmsi11']['search']['offset'] = $offset;
	...
	function selone()    
       { 
		 $this->db->select('taxma.* ');	
		 $this->db->from('taxma');
	     $this->db->where('ma001', $this->uri->segment(4));
		// $this->db->join('actma as a', 'taxma.ma004 = a.ma001','left');
		 $query = $this->db->get();
	...
	function findf($limit, $offset, $sort_by, $sort_order)     
       {            		
	     ...
	     $seq11 = "SELECT COUNT(*) as count  FROM `taxma` ";
	     $seq1 = " ma001, ma002, ma003, ma004,  create_date FROM `taxma` ";
	...
	function insertf()   
       {
	    ...
	    $exist = $this->cmsi11_model->selone1($this->input->post('ma001'));
	    if ($exist)
	      {
		    return 'exist';
		  } 
           return  $this->db->insert('taxma', $data);
       }
    ...
    function copyf()           //複製一筆
       {
	    ...
	    $data = array( 
            ...
            $exist = $this->cmsi11_model->selone2($this->input->post('ma002c'));
		    if ($exist)
		      {
			   return 'exist';
		      }         
              return $this->db->insert('taxma', $data);      //複製一筆  
       }	
	...
	function excelnewf()           
       {			
	    ...
	    $seq2=$this->input->post('ma002c');
	    $sql = " SELECT ma001,ma002,ma003,ma004,create_date FROM taxma WHERE ma001 >= '$seq1'  AND ma001 <= '$seq2' ORDER BY ma001 "; 
	...
	function printfd()          
       {
	    $seq1=$this->input->post('ma001c');    //查詢一筆以上
	    $seq2=$this->input->post('ma002c'); 
	    $sql = " SELECT * FROM taxma WHERE ma001 >= '$seq1'  AND ma001 <= '$seq2'  "; 
        $query = $this->db->query($sql);
	    $ret['rows'] = $query->result();
	...
	function updatef()   //更改一筆
          {
            ...
            $this->db->update('taxma',$data);                   //更改一筆
	...
	/*==以下AJAX處理區域==*/
	 function ajaxcmsi11($seg1)    //ajax 查詢一筆 顯示用 部門6
          { 	              
	    $this->db->set('ma001', $this->uri->segment(4));
	    $this->db->where('ma001', $this->uri->segment(4));	
	    $query = $this->db->get('taxma');
	...
	function lookup_old($select_col=array(),$search_col=array(),$keyword=array(),$limit=10){
		...
		if($sel_col == ""){$sel_col = "*";}
		$this->db->select($sel_col)->from('taxma');
