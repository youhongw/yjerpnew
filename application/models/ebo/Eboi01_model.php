...
class Eboi01_model extends CI_Model {
	...
	function findf($limit, $offset, $sort_by, $sort_order)    
	    ...
	    $seq11 = "SELECT COUNT(*) as count  FROM `eboml` ";
	    $seq1 = " ml001, ml002, ml003, ml004, ml005, ml006, create_date FROM `eboml` ";
	...
	function insertf()    
	    ...
	    $exist = $this->eboi01_model->selone1($this->input->post('ml001'),$this->input->post('ml002'));
	    if ($exist)
	        {
		    return 'exist';
		} 
            return  $this->db->insert('eboml', $data);
        }
    ...
    function copyf()          
        ...
        $exist = $this->eboi01_model->selone2($this->input->post('ml003c'),$this->input->post('ml004c'));
		if ($exist)
		   {
		    return 'exist';
		   }         
            return $this->db->insert('eboml', $data);      //複製一筆  
        }
	...
	function excelnewf()           
	    ...
	    $seq4=$this->input->post('ml004c'); 
	    $sql = " SELECT ml001,ml002,ml003,ml004,ml005,ml006,create_date FROM eboml WHERE ml001 >= '$seq1'  AND ml001 <= '$seq2' AND ml002 >= '$seq3' AND ml002 <= '$seq4' "; 
	...
	function printfd()           
	    ...
	    $seq3=$this->input->post('ml003c'); 
	    $seq4=$this->input->post('ml004c'); 
	    $sql = " SELECT * FROM eboml WHERE ml001 >= '$seq1'  AND ml001 <= '$seq2' AND ml002 >= '$seq3' AND ml002 <= '$seq4' "; 
        $query = $this->db->query($sql);
	    $ret['rows'] = $query->result();
	...
	function updatef()   
          {
			...
			$this->load->vars($data);
            $this->db->update('eboml',$data);                   //更改一筆
