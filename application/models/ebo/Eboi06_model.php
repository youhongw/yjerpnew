...
class Eboi06_model extends CI_Model {
	...
	function selone($seq1,$seq2)    
        ...
        $this->db->from('ebomm as a');	
        $this->db->join('ebomn as b', 'a.mc001 = b.md001 ','left');		
	...
	function findf($limit, $offset, $sort_by, $sort_order)     
         {            		
	      ...
	      $seq11 = "SELECT COUNT(*) as count  FROM `ebomm` ";
	      $seq1 = "mc001, mc002, mc003, mc004, mc005,mc006, mc010, create_date FROM `ebomm`  ";
        ...
        if ($seq7!='') {$seq9=" ORDER BY ".$seq7;$seq33=$seq7;}
			 //下一頁不會亂跳
		if(@$_SESSION['eboi06_sql_term']){$seq32 = $_SESSION['eboi06_sql_term'];}
		if(@$_SESSION['eboi06_sql_sort']){$seq33 = $_SESSION['eboi06_sql_sort'];}
	...
	//新增一筆 檔頭  ebomm	
	function insertf()    //新增一筆 檔頭  ebomm
        {
	     ...
	     $data = array( 
	      ...
	      $exist = $this->eboi06_model->selone1($this->input->post('invq02a'));
	      if ($exist)
	         {
		      return 'exist';
		     } 
             $this->db->insert('ebomm', $data);
		...
		//	while ($_POST['order_product'][  $n  ]['md004']) {
	      ...
	      $exist = $this->eboi06_model->selone1d($this->input->post('invq02a'));
		   if ($_POST['order_product'][  $n  ]['md003'] !='') {
		   $this->db->insert('ebomn', $data_array);}
    ...
    function copyf()           
		...
		//	$this->db->where('mc002', $this->input->post('mc002o'));
         ...
         //   if ($query->num_rows() != 1) { return 'exist'; }
            ...
            $seq1=$this->input->post('mc001c');    //主鍵一筆檔頭ebomm
		//	$seq2=$this->input->post('mc002c');    
            ...
            $exist = $this->eboi06_model->selone2($this->input->post('mc001c'));
		    if ($exist)
		      {
			    return 'exist';
		      }         
             $this->db->insert('ebomm', $data);      //複製一筆  
		...
		//	$this->db->where('md002', $this->input->post('mc002o'));
          ...
          //  if ($query->num_rows() != 1) { return 'exist'; }
			...
			$seq1=$this->input->post('mc001c');    //主鍵一筆明細ebomn
		//	$seq2=$this->input->post('mc002c'); 
              $i=0;
            while ($i<$num) {	
			  $data_array = array( 
             ...
             $this->db->insert('ebomn', $data_array);      //複製一筆 
             $i++;			 
            }
             return true;		
	   }
	...
	function excelnewf()           
	  ...
	  //    $sql = " SELECT mc001,mc002,mc003,mc004,c.ma002 as mc004disp,mc005,md003,md004,md005,md006,md009,md010 FROM ebomm as a,ebomn as b,purma c
	//	  WHERE mc001=md001 AND mc002=md002 AND mc004=ma001 AND  mc001 >= '$seq1'  AND mc001 <= '$seq2' AND mc002 >= '$seq3'  AND mc002 <= '$seq4'  ";  
           $sql = " SELECT mc001,c.mb002 as mc001disp,c.mb003 as mc001disp1,c.mb004 as mc001disp2,mc004,md003,md002
		           ,d.mb002 as md003disp ,d.mb003 as md003disp1 ,d.mb004 as md003disp2,md006,md007,md008 FROM ebomm as a,ebomn as b,invmb c, invmb d
	...
	function printfd()          
	    ...
	    //  $seq4=$this->input->post('mc002c');
	      $sql = " SELECT mc001,mc002,mc003,mc004,c.mb002 as mc001disp,c.mb003 as mc001disp1,c.mb004 as mc001disp2,mc005,md003,md004,md005,md006,md009,md010 
		            md002,md007,md008,d.mb002 as md003disp ,d.mb003 as md003disp1 ,d.mb004 as md003disp2 FROM ebomm as a,ebomn as b,invmb c, invmb d
		  WHERE a.mc001=b.md001 AND  a.mc001=c.mb001 AND b.md003=d.mb001 AND  mc001 >= '$seq1'  AND mc001 <= '$seq2'   "; 
          $query = $this->db->query($sql);
	      $ret['rows'] = $query->result();
	...
	function printfd1()   
       {           
        ...
        $this->db->from('ebomm as a');	
        $this->db->join('ebomn as b', 'a.mc001 = b.md001  and a.mc002=b.md002 ','left');		
	...
	function updatef()   
		...
		//	$this->db->where('mc002', $this->input->post('mc002'));
            $this->db->update('ebomm',$data);                   //更改一筆
			
			//刪除明細
			$this->db->where('md001', $this->input->post('invq02a'));
			//$this->db->where('md002', $this->input->post('mc002'));
            $this->db->delete('ebomn'); 
