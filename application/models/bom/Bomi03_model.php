...
class Bomi03_model extends CI_Model {
	...
	{
		$this->session->set_userdata('bomi03_search',"display_search/".$offset);
		...
		if(isset($_SESSION['bomi03']['search']['where'])){
			if($where){$where .= " and ";}
			$where .= $_SESSION['bomi03']['search']['where'];
		}
		...
		if(isset($_SESSION['bomi03']['search']['order'])){
			if($order){$order .= " , ";}
			$order .= $_SESSION['bomi03']['search']['order'];
		}
		
		if(!isset($_SESSION['bomi03']['search']['order']) && $default_order){
			if($order){$order .= " , ";}
			$order .= $default_order;
		}
		/* order end */
		...
		$ret['data'] = $query->get()->result();
		//儲存sql
		$_SESSION['bomi03']['search']['sql'] = $this->db->last_query();
		...
		//儲存where與order
		$_SESSION['bomi03']['search']['where'] = $where;
		$_SESSION['bomi03']['search']['order'] = $order;
		$_SESSION['bomi03']['search']['offset'] = $offset;
		
		return $ret;
	}
	//建構SQL字串 單據
	function constructa_sql($limit = 10, $offset = 0, $func = "",$docnum)
	{
		$this->session->set_userdata('bomi03_search',"display_search/".$offset);
		if (session_status() == PHP_SESSION_NONE) {
			session_start();
	...
	//建構SQL字串 單據
	function constructb_sql($limit = 10, $offset = 0, $func = "",$docnum)
	{
		$this->session->set_userdata('bomi03_search',"display_search/".$offset);
		if (session_status() == PHP_SESSION_NONE) {
			session_start();
	...
	function insertf()   
       ...
       //    }
	    ...
	    $exist = $this->bomi03_model->selone1($this->input->post('mq001'));
	    if ($exist)
	      {
		    return 'exist';
		  } 
    ...
    function copyf()           //複製一筆
       {
	    ...
	    $data = array( 
            ...
            $exist = $this->bomi03_model->selone2($this->input->post('mq002c'));
		    if ($exist)
		      {
			   return 'exist';
		      }         
	...
	function printfd()          
       {
	    $seq1=$this->input->post('mq001c');    //查詢一筆以上
	    $seq2=$this->input->post('mq002c'); 
	    $sql = " SELECT * FROM cmsmq WHERE mq001 >= '$seq1'  AND mq001 <= '$seq2' and mq003>='41' and mq003<='44' "; 
        $query = $this->db->query($sql);
	    $ret['rows'] = $query->result();
        $seq32 = "mq001 >= '$seq1'  AND mq001 <= '$seq2' and mq003>='41' and mq003<='44' ";	
	    $query = $this->db->select('COUNT(*) as count', FALSE)  //筆數查詢,如果設為FALSE不會使用反引號保護你的字段或者表名
		                  ->from('cmsmq')
		                  ->where($seq32);
	    $num = $query->get()->result();		
