...
class Bomi02a_model extends CI_Model
{
	...
	function construct_sql($limit = 15, $offset = 0, $func = "")
	{
		...
		if (isset($_SESSION['bomi02a']['search']['where'])) {
			if ($where) {
				$where .= " and ";
			}
			$where .= $_SESSION['bomi02a']['search']['where'];
		}
		...
		if (isset($_SESSION['bomi02a']['search']['order'])) {
			if ($order) {
				$order .= " , ";
			}
			$order .= $_SESSION['bomi02a']['search']['order'];
		}

		if (!isset($_SESSION['bomi02a']['search']['order']) && $default_order) {
			if ($order) {
				$order .= " , ";
			}
			$order .= $default_order;
		...
		//儲存sql 語法
		$_SESSION['bomi02a']['search']['sql'] = $this->db->last_query();
		...
		//儲存where與order
		$_SESSION['bomi02a']['search']['where'] = $where;
		$_SESSION['bomi02a']['search']['order'] = $order;
		$_SESSION['bomi02a']['search']['offset'] = $offset;
	...
	function construct_view($data)
	{
		...
		$_SESSION['bomi02a']['search']['view'] = $view_array;
		$_SESSION['bomi02a']['search']['index'] = $index_array;
		//echo "<pre>";var_dump($_SESSION['bomi02a']['search']['view']);exit;
	}
	//查詢修改用 (看資料用)   
...
VALUES ('$company', '$creator', '$usr_group', '$vtoday', '0', '$mc001', '$mc002', '$mc004', '$mc009', '$mc010'); ";
		...
		if (isset($order_product)) {
			...
			foreach ($order_product as $key => $val) {
				if ($val['md002'] && $val['md003']) {
					...
					$sql97 = " select * from dbo.bommd where md001='$mc001' and md003='$md003' ";
		...
		/*
         ...
         //   if ($query->num_rows() != 1) { return 'exist'; }
           ...
           $seq1=$this->input->post('mc001c');    //主鍵一筆檔頭bommc
            ...
            $exist = $this->bomi02a_model->selone2($seq1);
		    if ($exist)
		      {
			    return 'exist';
		      }         
	...
	function delmutif()
	{
		...
		if (!empty($_POST['selected'])) {
			foreach ($_POST['selected'] as $check) {
				...
				$querydelmd = $this->db->query(" DELETE FROM bommd WHERE md001='$seq1' ");
			}
		}
		return $querydelmd;
		// if ($this->db->affected_rows() > 0) {
	...
	function check_detail_num($mz001)
	{
		...
		$sql98 = " select * from bommd where md001='$mz001' ";
		$query = $this->db->query($sql98);
