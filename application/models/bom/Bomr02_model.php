...
class Bomr02_model extends CI_Model {
	...
	function get_bom_list_out($temp_list){
		...
		//1080213 33,80 $result = mysql_query($sql);
	...
	function get_bom_list($temp_list)          //get_bom_list  遞廻$m_count,$base_bom_list,$row_temp,$vmd003
    {
		...
		$sql = "select a.md001,a.md002,a.md003,a.md004,a.md006,a.md016,b.mb002,b.mb003,b.mb025 from bommd as a,invmb as b  where a.md003=b.mb001 and a.md001 = '".$temp_list['md001']."' order by a.md001,a.md002 ";
		// 1080213 $result = mysql_query($sql);
		$result = $this->db->query($sql);
	...
	//	1080214 while($row_temp = mysql_fetch_array($result)){
			foreach ($result->result_array() as $row_temp){
      
			$temp_list["bom_list"][$count_temp] = $this->get_item_data($row_temp['md003']);
	...
	function get_item_data($temp_it_id){
		...
		// 1080213  $result = mysqli_query($sql);
		 $result = $this->db->query($sql);
		//1080213 $temp_row = mysqli_fetch_array($result);
	//	 $query = $result->result_array();
	//	echo "<pre>";var_dump($temp_row);exit;
		//$query = $this->db->query($sql);
		//$temp_row = $query->result();
         	
		//echo "<pre>";var_dump($temp_row);exit;
		// $temp_row = $result->row_array();
		 foreach ($result->result_array() as $temp_row)
          {
			  $temp_row['md001'] = $temp_row['mb001'];
		  }
		//echo "<pre>";var_dump($temp_row['mb001']);exit;
		//foreach($temp_row as $row ){	
      //   echo "<pre>";var_dump($row->mb001);exit;		
		//$temp_row['md001'] = $temp_row['mb001'];
		//echo "<pre>";var_dump($temp_row['md001']);exit;
	//	$temp_row['md001'] = $temp_row['mb001'];
		return $temp_row;
	}
