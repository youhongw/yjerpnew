...
class Admi02_model extends CI_Model {
	...
	function construct_view($data){
		...
		$_SESSION['admi02']['search']['view'] = $view_array;
		$_SESSION['admi02']['search']['index'] = $index_array;
		//echo "<pre>";var_dump($_SESSION['admi02']['search']['view']);exit;
		
	}
	//查詢一筆 修改用	
	function selone()  {  
		 $this->db->select('a.*,b.me002 as mf004disp,c.me002 as mf007disp');	
		 $this->db->from('barmb as a');	
	...
	function findf($limit, $offset, $sort_by, $sort_order)   { 
	     $seq11 = "SELECT COUNT(*) as count  FROM `barmb` ";
	     $seq1 = " mf001, mf002, mf003,mf004,mf005,mf006,mf007,  create_date FROM `barmb` ";
	...
	function insertf()   {  
	     ...
	     $exist = $this->admi02_model->CheckRepeat($this->input->post('mf001'));   //查詢資料是否重複
	     if ($exist) {
		     return 'exist';
		    } 
			
        return  $this->db->insert('barmb', $data);
     }
	...
	function excelnewf()  {	  
	     ...
	     $sql = " SELECT mf001,mf002,mf003,mf004,mf005,mf007,create_date FROM barmb WHERE mf001 >= '$seq1' AND mf001 <= '$seq2'  "; 
	...
	function printfd()  {  
	     ...
	     $seq2=$this->input->post('mf002c');
	     $sql = " SELECT a.*,b.me002 as mf004disp,c.me002 as mf007disp FROM barmb as a left join admme as b on a.mf004=b.me001 left join cmsme as c on a.mf007=c.me001  WHERE mf001 >= '$seq1'  AND mf001 <= '$seq2'  "; 
         $query = $this->db->query($sql);
