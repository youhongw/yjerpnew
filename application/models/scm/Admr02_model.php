...
class Admr02_model extends CI_Model {
	...
	function construct_sql($limit = 15, $offset = 0, $func = "")
	{
		...
		if(isset($_SESSION['admr02']['search']['where'])){
			if($where){$where .= " and ";}
			$where .= $_SESSION['admr02']['search']['where'];
		}
		...
		if(isset($_SESSION['admr02']['search']['order'])){
			if($order){$order .= " , ";}
			$order .= $_SESSION['admr02']['search']['order'];
		}
		
		if(!isset($_SESSION['admr02']['search']['order']) && $default_order){
			if($order){$order .= " , ";}
			$order .= $default_order;
		}
		/* order end */
		...
		$ret['data'] = $query->get()->result();
		//儲存sql
		$_SESSION['admr02']['search']['sql'] = $this->db->last_query();
		...
		//儲存where與order
		$_SESSION['admr02']['search']['where'] = $where;
		$_SESSION['admr02']['search']['order'] = $order;
		$_SESSION['admr02']['search']['offset'] = $offset;
	...
	function construct_view($data){
		...
		$_SESSION['admr02']['search']['view'] = $view_array;
		$_SESSION['admr02']['search']['index'] = $index_array;
		//echo "<pre>";var_dump($_SESSION['admr02']['search']['view']);exit;
	...
	function findf($limit, $offset, $sort_by, $sort_order)   { 
         ...
         if ($seq7!='') {$seq9=" ORDER BY ".$seq7;$seq33=$seq7;}
		  //下一頁不會亂跳
		if(@$_SESSION['admr02_sql_term']){$seq32 = $_SESSION['admr02_sql_term'];}
		if(@$_SESSION['admr02_sql_sort']){$seq33 = $_SESSION['admr02_sql_sort'];}
	...
	function insertf()   {  
		...
		if ($this->input->post('mf001')>'0') {
		  ...
		  { $vno = $this->admr02_model->check_title_no($vdate);}
	     $data = array( 
	                  'company' => '000',
	                  'creator' => 'demo',
		              'usr_group' => 'A100',
	     ...
	     $exist = $this->admr02_model->CheckRepeat($this->input->post('mf001'));   //查詢資料是否重複
	     if ($exist) {
		    return 'exist';
		  }
	...
	function printfd($seq1,$seq2)  {  
	     preg_match_all('/\d/S',$seq1, $matches);  //處理日期字串
			...
			if ($vmm=='01') {$yyy=$yyy-1;$mmm='12';}    //本年$yyy,$mmm
			$yyymm=$yyy.$mmm;
			$yyymm1=substr($seq2,0,6);
		//	echo var_dump($vmm,$yyymm,$yyymm1);exit;
		 ini_set('max_execution_time', 120);
		 $sql9= " DELETE SCMMB ";
		 $this->db->query($sql9);
		// ECHO VAR_DUMP($seq1,$seq2);EXIT;
	     $sql8 = " SELECT A.TG006,D.MV002,C.MA001,C.MA002,SUM(B.TH013*A.TG012) AS AMT FROM COPTG AS A 
...
WHERE  A.TG003>='$seq1' AND A.TG003<='$seq2'  
      AND A.TG023='Y'  AND  SUBSTRING(C.CREATE_DATE,1,6)>='$yyymm'   AND  SUBSTRING(C.CREATE_DATE,1,6)<='$yyymm1'  AND 
      ( C.MA001<>'ACHN002'  AND C.MA001<>'ACHN003'   AND C.MA001<>'AUSA036')
     AND  ( C.MA016<>'6666'and C.MA016<>'96019'and C.MA016<>'97046' and C.MA016<>'98015' )
GROUP BY A.TG006,D.MV002,C.MA001,C.MA002
HAVING SUM(B.TH013)>0
ORDER BY A.TG006,C.MA001
		 "; 
		...
		// echo var_dump($query->result());
		 foreach ($query->result() as $row) {
			 ...
			 $MA004=$row->MA002;
             $MA005=$row->AMT;
             $sql2= "INSERT INTO SCMMB(MA001,MA002,MA003,MA004,MA005) VALUES ('$MA001','$MA002','$MA003','$MA004','$MA005') ";
			 $this->db->query($sql2);			 
		 }
				 
		 $sql3="SELECT MA001,MA002,MA003,MA004,MA005 FROM SCMMB  ORDER BY MA001,MA002   ";
		 
		 $query = $this->db->query($sql3);
	     $ret['rows'] = $query->result();
       
			$seq32 = "	SELECT COUNT(*) as count FROM SCMMB AS A ";
