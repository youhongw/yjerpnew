...
class Admi01_model extends CI_Model {
	...
	function construct_view($data){
		...
		$_SESSION['admi01']['search']['view'] = $view_array;
		$_SESSION['admi01']['search']['index'] = $index_array;
		//echo "<pre>";var_dump($_SESSION['admi01']['search']['view']);exit;
	...
	function findf($limit, $offset, $sort_by, $sort_order)   { 
         ...
         if ($seq7!='') {$seq9=" ORDER BY ".$seq7;$seq33=$seq7;}
		  //下一頁不會亂跳
		if(@$_SESSION['admi01_sql_term']){$seq32 = $_SESSION['admi01_sql_term'];}
		if(@$_SESSION['admi01_sql_sort']){$seq33 = $_SESSION['admi01_sql_sort'];}
	...
	function insertf()   {  
		...
		if ($this->input->post('mf001')>'0') {
		  ...
		  { $vno = $this->admi01_model->check_title_no($vdate);}
	     $data = array( 
	                  'company' => '000',
	                  'creator' => 'demo',
		              'usr_group' => 'A100',
	     ...
	     $exist = $this->admi01_model->CheckRepeat($this->input->post('mf001'));   //查詢資料是否重複
	     if ($exist) {
		    return 'exist';
		  }
