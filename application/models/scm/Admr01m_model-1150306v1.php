...
/*$sql984 = " UPDATE SFCTEM1 SET UDF06=DATEDIFF(
	...
	//1150203 modi
$sql984 = "	UPDATE SFCTEM1 
SET UDF06 = (
    -- 計算 UDF02 的總秒數
    (CAST(LEFT(RIGHT('0000' + LTRIM(RTRIM(UDF02)), 4), 2) AS INT) * 3600 + 
     CAST(RIGHT(LTRIM(RTRIM(UDF02)), 2) AS INT) * 60) 
    - 
    -- 減去 UDF01 的總秒數
    (CAST(LEFT(RIGHT('0000' + LTRIM(RTRIM(UDF01)), 4), 2) AS INT) * 3600 + 
     CAST(RIGHT(LTRIM(RTRIM(UDF01)), 2) AS INT) * 60)
)
WHERE UDF01 < UDF02 
  AND UDF02 <= '2400'
  AND ISNUMERIC(UDF01) = 1 
  AND ISNUMERIC(UDF02) = 1
  -- 額外過濾：確保分鐘部分是合理的 (00-59)，或是特殊的 2400
  AND CAST(RIGHT(LTRIM(RTRIM(UDF01)), 2) AS INT) < 60
  AND (CAST(RIGHT(LTRIM(RTRIM(UDF02)), 2) AS INT) < 60 OR UDF02 = '2400'); ";
...
$sql984a = " UPDATE SFCTEM1 
SET UDF06 = (
    -- 計算 TE028 的總秒數 (例如 2359 -> 23*3600 + 59*60)
    (CAST(LEFT(RIGHT('0000' + LTRIM(RTRIM(TE028)), 4), 2) AS INT) * 3600 + 
     CAST(RIGHT(LTRIM(RTRIM(TE028)), 2) AS INT) * 60) 
    - 
    -- 減去 UDF01 的總秒數
    (CAST(LEFT(RIGHT('0000' + LTRIM(RTRIM(UDF01)), 4), 2) AS INT) * 3600 + 
     CAST(RIGHT(LTRIM(RTRIM(UDF01)), 2) AS INT) * 60)
...
/*$sql984a = " UPDATE SFCTEM1 SET UDF06=UDF06+DATEDIFF(
	...
	//1150203
$sql984a = " UPDATE SFCTEM1 
SET UDF06 = ISNULL(UDF06, 0) + (
    -- 計算 UDF02 的總秒數
    (CAST(LEFT(RIGHT('0000' + LTRIM(RTRIM(UDF02)), 4), 2) AS INT) * 3600 + 
     CAST(RIGHT(LTRIM(RTRIM(UDF02)), 2) AS INT) * 60) 
    - 
    -- 減去 TE029 的總秒數 (在此處即為 0000)
    (CAST(LEFT(RIGHT('0000' + LTRIM(RTRIM(TE029)), 4), 2) AS INT) * 3600 + 
     CAST(RIGHT(LTRIM(RTRIM(TE029)), 2) AS INT) * 60)
)
WHERE UDF01 >= UDF02 
  AND TE029 = '0000'
  AND ISNUMERIC(UDF02) = 1 
  AND ISNUMERIC(TE029) = 1; ";	
	$this->db->query($sql984a);	
//go	
    ...
    foreach ($query2->result() as $row) {
     ...
     /*   echo "<tr style='{$bg_color}'>";
		...
		$sql91 = " INSERT INTO SFCTEM2_TEMP1  
		           (TE065,TE031,TE001,TE002,TE003,TE022,TE023,TE012,
	              DIFFM,STATUS,TE012S,TE012A,UDF03,TE011,
				   UDF05,TE004,DIFFS,TE012B,TE012C,TE012D,TE012E,TE012F,TE012G) VALUES
				  ('$TE065','$TE031','$TE001','$TE002','$TE003','$TE022','$TE023','$TE012',
					'$DIFFM','$STATUS','$TE012S','$TE012A','$UDF03','$TE011',
					'', '','0','0','0','0','0','0','0')
							";					     
