...
class palq01a_model extends CI_Model {
	...
	function search($limit, $offset, $sort_by, $sort_order)     //欄位表頭排序流覽資料
	  { 
	    ...
	    $sort_columns = array('mv001', 'mv002', 'mv003', 'mv004','mv005','mv028',  'create_date');
	    $sort_by = (in_array($sort_by, $sort_columns)) ? $sort_by : 'mv001';  //檢查排序欄位是否在 table 內
	    $query = $this->db->select('mv001, mv002, mv003, mv004,mv005,mv028,mv022,   create_date')
	                      ->from('cmsmv')
						  ->where('mv022',' ')
	...
	function findf($limit, $offset, $sort_by, $sort_order)    //查詢多筆進階查詢 mysql 
	  ...
	  //$seq5='';$seq51='';$seq7='';$seq71='';		  
	    ...
	    if (trim($this->input->post('find007'))!='') 
	     ...
	     $sort_by = (in_array($sort_by, $sort_columns)) ? $sort_by : 'mv001';  //檢查排序欄位是否在 table 內
	     $query = $this->db->select('mv001, mv002, mv003, mv004, mv005, mv006,mv022, create_date')
	                       ->from('invma')
		               ->where($seq32)
			       ->order_by($seq33)
			     //->order_by($sort_by, $sort_order)
