...
class Pali09_model extends CI_Model {
	...
	function search($limit, $offset, $sort_by, $sort_order)     
	    { 
	      ...
	      $sort_columns = array('mi001', 'mi002', 'mi003', 'create_date');
	...
	function selone()    
        { 
		  $this->db->select('*');	
	      $this->db->set('mi001', $this->uri->segment(4));              
	    //$this->db->set('mi002', $this->uri->segment(5));
	      $this->db->where('mi001', $this->uri->segment(4));     
	    //$this->db->where('mi002', $this->uri->segment(5));
	      $query = $this->db->get('palmi');
	...
	function findf($limit, $offset, $sort_by, $sort_order)   
        {            		
	     //$seq5='';$seq51='';$seq7='';$seq71='';		  
	      ...
	      $seq1 = " mi001, mi002, mi003,  create_date FROM `palmi` ";
	...
	function insertf()    
	    ...
	    $exist = $this->pali09_model->selone1($this->input->post('mi001'));   //查詢資料是否重複
	    if ($exist)
	     {
		  return 'exist';
		 } 
    ...
    function copyf()           
        {
          ...
          $seq3=$this->input->post('mi002c');    //主鍵一筆
	      $data = array( 
	                 'company' => $this->session->userdata('syscompany'),
	                 'creator' => $this->session->userdata('manager'),
		             'usr_group' => 'A100',
           ...
           $exist = $this->pali09_model->selone2($this->input->post('mi002c'));
		   if ($exist)
		    {
			  return 'exist';
		    }         
	...
	function excelnewf()           
        {			
	      ...
	      $sql = " SELECT mi001,mi002,mi003,create_date FROM palmi WHERE mi001 >= '$seq1' AND mi001 <= '$seq2'  "; 
	...
	function printfd()           
        {
	      $seq1=$this->input->post('mi001c');    
	      $seq2=$this->input->post('mi002c');
	      $sql = " SELECT * FROM palmi WHERE mi001 >= '$seq1'  AND mi001 <= '$seq2'  "; 
          $query = $this->db->query($sql);
	      $ret['rows'] = $query->result();
		
          $seq32 = "mi001 >= '$seq1'  AND mi001 <= '$seq2'  ";	
	      $query = $this->db->select('COUNT(*) as count', FALSE)  //筆數查詢,如果設為FALSE不會使用反引號保護你的字段或者表名
		                ->from('palmi')
		                ->where($seq32);
	      $num = $query->get()->result();		
