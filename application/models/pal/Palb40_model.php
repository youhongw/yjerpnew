...
//計算 加班單 轉入月出勤  
	function batchaf()           
		...
		//刪除出勤彚總檔 PALYE   暫時用excel 滙入1060105
		   $this->db->where('ye001', $vyy);
		   $this->db->delete('palye'); 
		//新增月出勤彚總 PALTE 暫時用excel 滙入1060105  select mv001,mv004,$vyymm1 from cmsmv where  mv022='' or substr(mv022,0,6)='$vyymm1' "; 
		  $sql2 = "INSERT INTO  palye (create_date,ye001,ye002,ye004,ye005,ye006) 
		  SELECT $date,SUBSTR(tc003,1,4) as tc003,tc001,floor(sum(tc004)) as tc004,round(sum(tc007)/8,2) as tc007,round(sum(tc006)/8,2) as tc006 
          FROM paltc as a,cmsmv as b 
          WHERE  SUBSTR(tc003,1,4)='$vyy' and tc001=mv001 and  mv022='' and mv300<>'N'
    ...
    while($row=mysql_fetch_assoc($result)){
		...
		//日薪  PALMD  MD003
           $sql21 =" update palye as b,(select a.md001,a.md003  from palmd as a  group by a.md001 ) as c
			 ...
			 $this->db->query($sql21);
	    //年全勤  1070109 員林 考勤, 到職1年
          //   $sql22 =" update palye set ye008=5  where ye004+ye005+ye006=0 " ; 
		//	 $this->db->query($sql22);
			 
			 $sql22 =" update palye as a, (select mv001,mv213,mv214 from cmsmv where mv021<='20170101' and mv022='' and mv213='1' and mv214='2') as b
                	   set ye008=5
					   where a.ye002=b.mv001 and a.ye004+a.ye005+a.ye006=0
 					     " ; 
			 $this->db->query($sql22);
		//計算年資
		 $vmv020=0.00;
    ...
    while($row=mysql_fetch_assoc($result)){
        foreach($row as $i=>$v){
            $$i=$v;
				...
				if (substr($mv021,0,4)==$vyy )
				{$vmv022=round( (12-(float)(substr($mv021,4,2)))/12,2);$vmv023=round( (31-(float)(substr($mv021,6,2)))/365,2);} else {$vmv022=round( ((float)(substr($mv021,4,2)))/12,2);$vmv023=round( ((float)(substr($mv021,6,2)))/365,2);}
			//	echo "<pre>";var_dump($vmv021);var_dump($vmv022);exit;
				$vmv020=$vmv021+$vmv022+$vmv023;
