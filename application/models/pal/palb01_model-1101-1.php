	...
	function batchaf($date='')
		...
		//新增月發薪 PALTD
		  $sql2 = "INSERT INTO  paltd (td001,td002,td003,td004,td005,td050,td051,td052,td053,td054) 
		           ...
		           LEFT JOIN cmsmv as b ON a.tc001=b.mv001 and (b.mv022='' or b.mv022 IS NULL) or substr(b.mv022,0,6)='$vyymm1'
				   LEFT JOIN cmsme as c ON a.tc002=c.me001 WHERE a.tc003='$vyymm1' "; 
		  $this->db->query($sql2);
			//echo $sql2;exit;
		   ...
		   $this->db->query($sql21);
		    ...
		    $sql22 =" update palta2 b,(select a.*
			 ...
			 $this->db->query($sql22);
		      ...
		      $sql23 =" INSERT INTO palta2  ( ta001,ta002,ta003,ta004,ta005,ta006,ta007,ta008,ta009,ta010,ta011,ta012) 
                        select a.ta001,a.ta002,'$vyymm1',a.ta004,a.ta005,a.ta006,a.ta007,a.ta008,a.ta009,a.ta010,a.ta011,a.ta012 from palta1 a                
                        where ta001 != a.ta001  " ; 
			 $this->db->query($sql23);
		...
		$sql="select COUNT(a.tc003) from paltc as a LEFT JOIN palmd as b ON tc001=md001
		                                  LEFT JOIN palta2 as c ON tc001=ta001 and ta003='$vyymm1'
		...
		while($row=mysql_fetch_assoc($result)){
			...
			foreach($row as $i=>$v){
			    ...
			    //計算外勞 到職 天數 未滿 183天所得稅扣 0.18 18%, 183天以上0.06 6% 稅率
				 ...
				 $vfood=0;
                if ($mv201=='1') {$vfood=880;} else {$vfood=0;}	
                ...
                //計算上班天數 workday  當月到職, 離職 天數  1050909
			      ...
			      if (substr($mv021,0,6) !=$vyymm1 and substr($mv022,0,6)==$vyymm1) {$workday9=(int)substr($mv022,6,2);}
               ...
               //   $workday=30-(($tc006/8)+($tc007/8)+($tc008/8)+($tc009)+($tc010/8)+($tc011)+($tc012)+($tc013)+($tc014)+($tc015)+($tc016));
				   $workday=30-(($tc006/8)+($tc007/8)+($tc009)+($tc010/8)+($tc011)+($tc012)+($tc013)+($tc015));
			 ...
			 //	  if (substr($mv022,0,6)==$vyymm1) { $workday1=30-substr($mv022,6,2);$workday=$workday+round($workday/7);}	
			       ...
			       if (substr($mv021,0,6)==$vyymm1) { $workday1=$vdd-substr($mv021,6,2)+1;$workday=$workday1-(($tc006/8)+($tc007/8)+($tc008/8)+($tc009)+($tc010/8)+($tc011)+($tc012)+($tc013)+($tc014)+($tc015)+($tc016));}				  
				...
				//	$hhamt4=($md004+$md005+$md006+$md007+$md012)/240;    //皇興 請假
				 ...
				 /*   $damt1=round($hhamt2*($tc015*3*8),0);
					...
					//皇興請假 無條件拾去
					$damt1=round($hhamt4*($tc015*3*8),0);
			...
			/*		$vtd008=round(round($md004/30/8,0)*(($tc015*3*8)+$tc006+($tc007/2)+$tc010),0);
				...
				//應稅免稅時數46 overtime  tc020,tc021 六日加班8小時以上
					...
					if (($vtc017+$vtc018+$vtc019+$vtc020+$vtc021+$vtc022)-$over1amt>0) {
					$over2amt=($vtc017+$vtc018+$vtc019+$vtc020+$vtc021+$vtc022)-$over1amt;} else {$over2amt=0;}
				//其他加減項
				   $vtd029=$ta006+$ta007;
			...
			/*	   if ((substr($mv021,0,6)==$vyymm1) or (substr($mv022,0,6)==$vyymm1)) {$vtd038=$md004-round(($md004/30)*$workday,0)+
				...
				//應領薪資 vtd030  1050909
				  ...
				  if (((substr($mv021,0,6)==$vyymm1) or (substr($mv022,0,6)==$vyymm1))) {
					$vtd030=$zmd004-$vtd008+$zmd005-$vtd009+$zmd006-$vtd010+$zmd007-$vtd011+$zmd008-$vtd012+$zmd009-$vtd013+$zmd010-$vtd014+$zmd011-$vtd015+$zmd012-$vtd016+$vtc017+$vtc018+$vtc019+$vtc020+$vtc021+$vtc022+$vtd029;}
				//應稅所得 vtaxamt1=應領金額-免稅加班金額44-免稅伙食津貼49,   所得稅=vtaxamt1*稅率
				   ...
				   //外勞 1050809 0不扣所得稅
                   if ($mv032=='3') {$vtd037=round(($vtd030-$damt)*$vtax,0);}	
                   if (($mv032=='3') && ($mv038=='0')) {$vtd037=0;}				   
				//個人代扣保費
				    if ($mv200 == 1) {$vtd035=round($vtaxamt1*0.02,0);} else {$vtd035=0;}
				//實領金額 vtd039   1050722
				    $vtd039=$vtd030-$tb004-$ml003-$ml004-$vfood-$vtd037-$vtd038-$damt;
				//轉帳金額
