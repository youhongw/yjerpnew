...
class pali53_model extends CI_Model {
	...
	***/
	...
	function construct_sql($limit = 15, $offset = 0, $func = "")
	{
		...
		$default_order = "tf002 desc,mv021 desc";//在這裡塞入一些預設排序
	...
	function insertf()   
       {
		...
		if ( ($tf006+$tf009)  !=  ($tf010+$tf011+$tf012+$tf013+$tf014+$tf015) )	{return "星期歸類時數有錯, 請檢查明細";}   
	   $data = array( 
   ...
   //     if ($query->num_rows() != 1) { return 'exist'; }
	...
	function excelnewf()           
	  ...
	  $seq1=$this->input->post('tf001o');    //查詢一筆以上
	    ...
	    $sql1 = " SELECT a.tf001,b.mv002 as tf001disp,b.mv004 as tf001disp1,c.me002 as tf001disp2,a.tf002,a.tf003, a.tf004,a.tf005,a.tf006,a.tf007,a.tf008,a.tf009,a.tf010,a.tf011,a.tf012,a.tf013,a.tf014,a.tf015,a.tf016 "; 
	...
	function printfd()          
       {
		...
		/*此段為判斷是否國定假日，其他功能不用，可直接砍掉  順便做一下小計處理*/
		$temp_total = 0;
		$ret['rows'] = array();
		foreach($temp_data as $key => $val){
			...
			$ret['rows'][] = $val;
			$temp_total += $val->tf010+$val->tf011+$val->tf012+$val->tf013+$val->tf014+$val->tf015+$val->tf018+$val->tf019;
			if(!isset($temp_data[$key+1])||$val->tf001!=@$temp_data[$key+1]->tf001){
				$object = new stdClass();
				$object->tf001 = "合計";
				$object->tf001disp = "";
				$object->tf001disp1 = "";
				...
				$object->tf014 = "";
				$object->tf015 = "";
				$object->tf016 = "共加班:".$temp_total."小時";
				$object->tf017 = "";
				$object->tf018 = "";
				$object->tf019 = "";
				$object->holiday = 0;
	...
	function updatef()   //更改一筆
		...
		$tf014=$this->input->post('tf014');
		$tf015=$this->input->post('tf015');
		if ( ($tf006+$tf009)  !=  ($tf010+$tf011+$tf012+$tf013+$tf014+$tf015) )	{echo var_dump('星期歸類時數有錯, 請檢查明細,按上一頁');exit; return "星期歸類時數有錯, 請檢查明細";}   		  
		  $data = array(			
