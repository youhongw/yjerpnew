...
class pali24_model extends CI_Model {
	...
	function findf($limit, $offset, $sort_by, $sort_order)     
       {            		
		...
		if(@$_SESSION['admi05_sql_sort']){$seq33 = $_SESSION['admi05_sql_sort'];}
	     ...
	     $query = $this->db->select('ml001,ml001 as ml001disp,ml001 as ml001disp1,ml002 as ml002disp, ml002, ml003, ml004, ml005,create_date')
	                       ->from('palml')
		                   ->where($seq32)
			               ->order_by($seq33)
			             //->order_by($sort_by, $sort_order)
	...
	function filterf1($limit, $offset , $sort_by  , $sort_order)    //篩選多筆        
	   {    
		...
		$this->db->select('a.ml001,b.mv002 as ml001disp, a.ml002, c.me002 as ml002disp,a.ml003, a.ml004, a.ml005, a.create_date');
	       $this->db->from('palml as a');
			$this->db->join('cmsmv as b', 'a.ml001 = b.mv001 ','left');
			$this->db->join('cmsme as c', 'a.ml002 = c.me001 ','left');
