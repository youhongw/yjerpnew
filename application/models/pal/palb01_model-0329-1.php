	...
	function batchaf($date='')
		...
		//新增月發薪 PALTD
		  $sql2 = "INSERT INTO  paltd (td001,td002,td003,td004,td005,td050,td051,td052,td053,td054) 
				   ...
				   LEFT JOIN cmsme as c ON a.tc002=c.me001 WHERE a.tc003='$vyymm1' and mv021<='$vyymm1'"; 
		  $this->db->query($sql2);
			//echo $sql2;exit;
		...
		while($row=mysql_fetch_assoc($result)){
			...
			foreach($row as $i=>$v){
			    ...
			    //計算外勞 到職 天數 未滿 183天所得稅扣 0.18 18%, 183天以上0.06 6% 稅率
				 ...
				 $vfood=0;
                if ($mv201=='1') {if(!@$vma205){$vfood=880;}else{$vfood=$vma205;}} else {$vfood=0;}	
			...
			/*	   if ((substr($mv021,0,6)==$vyymm1) or (substr($mv022,0,6)==$vyymm1)) {$vtd038=$md004-round(($md004/30)*$workday,0)+
				...
				//勞保健保費依照加保退保日期計算	2017.02.21 Talence Editor
					...
					$ml015_day = substr($ml015,6,2);$ml016_day = substr($ml016,6,2);
					$ml017_day = substr($ml017,6,2);$ml018_day = substr($ml018,6,2);
