...
class Pali22_model extends CI_Model {
	...
	function findf($limit, $offset, $sort_by, $sort_order)   
        {            		
	     //$seq5='';$seq51='';$seq7='';$seq71='';		  
	      ...
	      $seq1 = " mm001, mm002, mm003,  create_date FROM `palmm` ";
	...
	function insertf()    
	    ...
	    $exist = $this->pali22_model->selone1($this->input->post('mm001'));   //查詢資料是否重複
	    if ($exist)
	     {
		  return 'exist';
		 } 
    ...
    function copyf()           
        {
	      ...
	      $data = array( 
           ...
           $exist = $this->pali22_model->selone2($this->input->post('mm002c'));
		   if ($exist)
		    {
			  return 'exist';
		    }         
	...
	function excelnewf()           
        {			
	      ...
	      $sql = " SELECT mm001,mm002,mm003,create_date FROM palmm WHERE mm001 >= '$seq1' AND mm001 <= '$seq2'  "; 
	...
	function printfd()           
        {
	      $seq1=$this->input->post('mm001c');    
	      $seq2=$this->input->post('mm002c');
	      $sql = " SELECT * FROM palmm WHERE mm001 >= '$seq1'  AND mm001 <= '$seq2'  "; 
          $query = $this->db->query($sql);
	      $ret['rows'] = $query->result();
		
          $seq32 = "mm001 >= '$seq1'  AND mm001 <= '$seq2'  ";	
	      $query = $this->db->select('COUNT(*) as count', FALSE)  //筆數查詢,如果設為FALSE不會使用反引號保護你的字段或者表名
		                ->from('palmm')
		                ->where($seq32);
	      $num = $query->get()->result();		
