...
//計算 加班單 轉入月出勤  
	function batchaf()           
		...
		//新增月出勤彚總 PALTE  select mv001,mv004,$vyymm1 from cmsmv where  mv022='' or substr(mv022,0,6)='$vyymm1' "; 
		  ...
		  SELECT $date,SUBSTR(tc003,1,4) as tc003,tc001,floor(sum(tc004)/5) as tc004,round(sum(tc007)/8,2) as tc007,round(sum(tc006)/8,2) as tc006 
          FROM paltc as a,cmsmv as b 
          WHERE  SUBSTR(tc003,1,4)='$vyy' and tc001=mv001 and  mv022=''
