...
class pali26_model extends CI_Model {
	...
	function search($limit, $offset, $sort_by, $sort_order)     
	  { 
	    ...
	    $query = $this->db->select('a.md001,b.mv002 as md001disp, a.md002, c.me002 as md002disp,a.md014, a.md003, a.md013')
	                      ->from('palmd1 as a')
	...
	function findf($limit, $offset, $sort_by, $sort_order)     
       {            		
	     ...
	     $seq11 = "SELECT COUNT(*) as count  FROM `palmd1` ";
	     $seq1 = " md001, md002, md003, md004, md005, md008,md014,md011, create_date FROM `palmd1` ";
		...
		if(@$_SESSION['admi05_sql_sort']){$seq33 = $_SESSION['admi05_sql_sort'];}
	     ...
	     $sort_by = (in_array($sort_by, $sort_columns)) ? $sort_by : 'md001';  //檢查排序欄位是否在 table 內
	     $query = $this->db->select('md001,md001 as md001disp,md001 as md001disp1,md002 as md002disp, md002, md003, md004, md005, md008,md013,md011,md014, create_date')
	                       ->from('palmd1')
		                   ->where($seq32)
			               ->order_by($seq33)
			             //->order_by($sort_by, $sort_order)
	...
	function filterf1($limit, $offset , $sort_by  , $sort_order)    //篩選多筆        
	   {    
		...
		$this->db->select('a.md001,b.mv002 as md001disp, a.md002, c.me002 as md002disp,a.md014, a.md003, a.md013, a.create_date');
	       $this->db->from('palmd1 as a');
			$this->db->join('cmsmv as b', 'a.md001 = b.mv001 ','left');
			$this->db->join('cmsme as c', 'a.md002 = c.me001 ','left');
	...
	function selone1($seg1,$seg2)    
       {
	    ...
	    $this->db->where('md014', $this->input->post('md014')); 	    
	    $query = $this->db->get('palmd1');
	...
	//	$this->db->where('md001',$this->input->post('palq01a'));
	//	$this->db->where('md014',substr($this->input->post('md014'),0,4).substr($this->input->post('md014'),5,2).substr(rtrim($this->input->post('md014')),8,2));
	//	$this->db->delete('palmd1'); 
	    $data = array( 
				  ...
				  'md014' => substr($this->input->post('md014'),0,4).substr($this->input->post('md014'),5,2).substr(rtrim($this->input->post('md014')),8,2),
	    ...
	    $exist = $this->pali26_model->selone1($this->input->post('palq01a'),$this->input->post('md014'));
	    if ($exist)
	      {
		  } 
			$this->db->insert('palmd1', $data);
			unset($data['company']);unset($data['creator']);unset($data['usr_group']);unset($data['create_date']);unset($data['flag']);
			unset($data['md014']);unset($data['md015']);
			$data['modifier'] = $this->session->userdata('manager');$data['modi_date'] = date("Ymd");
			
			$exist1 = $this->pali26_model->selone11($this->input->post('palq01a'));
			if ($exist1) {
			$this->db->where('md001',$this->input->post('palq01a'));
			$this->db->update('palmd',$data); } else {$this->db->insert('palmd',$data);}                   //更改一筆
	...
	//查複製資料是否重複	 
    function selone11($seg2)    
       { 	
		 $this->db->where('md001',$seg2);
    ...
    function copyf()           //複製一筆
       {
	    ...
	    $data = array( 
            ...
            $exist = $this->pali26_model->selone2($this->input->post('md001c'),$seq4);
		    if ($exist)
		      {
			   return 'exist';
		      }         
              return $this->db->insert('palmd1', $data);      //複製一筆   
       }	
	...
	function excelnewf()           
	 ...
	 //   $seq4=$this->input->post('md002c'); 
		...
		$sql2 = " FROM palmd1 as a LEFT JOIN cmsmv as b ON  a.md001=b.mv001 LEFT JOIN cmsme as c ON a.md002=c.me001 "; 
		$sql3 = " WHERE a.md001 >= '$seq1'  AND a.md001 <= '$seq2' AND  a.md014 >= '$seq3'  AND a.md014 <= '$seq4' "; 
		$sql=$sql1.$sql2.$sql3;
	...
	function updatef()   //更改一筆
          {
                ...
                if ($this->input->post('md014')>'0') {$md014=substr($this->input->post('md014'),0,4).substr($this->input->post('md014'),5,2).substr(rtrim($this->input->post('md014')),8,2);}
              else {$md014='';}  			  
		   $data = array(			
            ...
            $this->db->where('md001', $this->input->post('palq01a'));
	        $this->db->where('md014', $md014);
            $this->db->update('palmd1',$data);                   //更改一筆
            if ($this->db->affected_rows() > 0)
              {
                 return TRUE;
              }
