...
class Pali12_model extends CI_Model {
	...
	function selbrowse($num,$offset)   
        {            
	      $this->db->select('mu001, mu002, mu003,  create_date');
          $this->db->from('palmu');
          //$this->db->order_by('id', 'DESC');                //排序  單欄
	      $this->db->order_by('mu001 desc, mu002 desc');    //排序  單欄以上 asc 由小至大 desc預設由大至小
	...
	function selone()    
        { 
		  $this->db->select('*');	
	      $this->db->set('mu001', $this->uri->segment(4));              
	    //$this->db->set('mu002', $this->uri->segment(5));
	      $this->db->where('mu001', $this->uri->segment(4));     
	    //$this->db->where('mu002', $this->uri->segment(5));
	      $query = $this->db->get('palmu');
	...
	function findf($limit, $offset, $sort_by, $sort_order)   
        {            		
	     //$seq5='';$seq51='';$seq7='';$seq71='';		  
	      $seq11 = "SELECT COUNT(*) as count  FROM `palmu` ";
	      $seq1 = " mu001, mu002, mu003,  create_date FROM `palmu` ";
	   ...
	   //  $seq7=$this->session->userdata('find07');
         $seq7="mu001 ";
          ...
          if ($seq7!='') {$seq9=" ORDER BY ".$seq7;$seq33=$seq7;}
		   //下一頁不會亂跳
		if(@$_SESSION['admu05_sql_term']){$seq32 = $_SESSION['admu05_sql_term'];}
		if(@$_SESSION['admu05_sql_sort']){$seq33 = $_SESSION['admu05_sql_sort'];}
	...
	function insertf()    
	    ...
	    $exist = $this->pali12_model->selone1($this->input->post('mu001'));   //查詢資料是否重複
	    if ($exist)
	     {
		  return 'exist';
		 } 
          return  $this->db->insert('palmu', $data);
        }
		
	//查copy複製資料是否重複	 
    function selone2($seg1)    
        {
	     $this->db->where('mu001', $this->input->post('mu002c')); 
	     $query = $this->db->get('palmu');
	     return $query->num_rows() ; 
	    }
    ...
    function copyf()           
        {
          ...
          $seq3=$this->input->post('mu002c');    //主鍵一筆
	      $data = array( 
	                 'company' => $this->session->userdata('syscompany'),
	                 'creator' => $this->session->userdata('manager'),
		             'usr_group' => 'A100',
           ...
           $exist = $this->pali12_model->selone2($this->input->post('mu002c'));
		   if ($exist)
		    {
			  return 'exist';
		    }         
              return $this->db->insert('palmu', $data);      //複製一筆  
          }
	...
	function excelnewf()           
        {			
	      ...
	      $sql = " SELECT mu001,mu002,mu003,create_date FROM palmu WHERE mu001 >= '$seq1' AND mu001 <= '$seq2'  "; 
	...
	function printfd()           
        {
	      $seq1=$this->input->post('mu001c');    
	      $seq2=$this->input->post('mu002c');
	      $sql = " SELECT * FROM palmu WHERE mu001 >= '$seq1'  AND mu001 <= '$seq2'  "; 
          $query = $this->db->query($sql);
	      $ret['rows'] = $query->result();
		
          $seq32 = "mu001 >= '$seq1'  AND mu001 <= '$seq2'  ";	
	      $query = $this->db->select('COUNT(*) as count', FALSE)  //筆數查詢,如果設為FALSE不會使用反引號保護你的字段或者表名
		                ->from('palmu')
		                ->where($seq32);
	      $num = $query->get()->result();		
