...
class Pali13_model extends CI_Model {
	...
	function search($limit, $offset, $sort_by, $sort_order)     
	  { 
	    ...
	    $query = $this->db->select('a.mv001,b.mv002 as mv001disp, a.mv002, c.me002 as mv002disp,a.mv003, a.mv004, a.mv005, d.mu002 as mv006disp, e.my002 as mv004disp')
	                      ->from('palmv as a')
						  ->join('cmsmv as b', 'a.mv001 = b.mv001 ','left')
						  ->join('cmsme as c', 'a.mv002 = c.me001 ','left')
						  ->join('palmu as d', 'a.mv006 = d.mu001 ','left')
		                  ->join('palmy as e', 'a.mv004 = e.my001 ','left')
	...
	function selone($seq1)    
       { 
		 $this->db->select('a.*, b.mv002 as mv001disp,c.me002 as mv002disp, d.mu002 as mv006disp, e.my002 as mv004disp');	
		 ...
		 $this->db->join('palmy as e', 'a.mv004 = e.my001 ','left');
		// $this->db->where('mv001', $this->uri->segment(4));
	...
	function insertf()   
       {
	    ...
	    $data = array( 
		          ...
		          'mv003' => substr($this->input->post('mv003'),0,4).substr($this->input->post('mv003'),5,2).substr(rtrim($this->input->post('mv003')),8,2),
		          'mv004' => $this->input->post('palq17a'),
                  'mv005' => $this->input->post('mv005'),
				  'mv006' => $this->input->post('palq12a'),
				  'mv007' => $this->input->post('mv007')
				   ); 
               $exist = $this->pali13_model->selone1($this->input->post('palq01a'),$mv003);   //查詢資料是否重複
	    if ($exist)
	     {
		  return 'exist';
		 } 
