...
class palr57_model extends CI_Model {
	...
	function printfd()          
        {
	      ...
	      $seq6=$this->input->post('ta002c');
	      $seq7=$this->input->post('mv032');
		  
			$sql = " SELECT a.*,b.*,c.mv202,d.ta013 FROM paltd as a 
				LEFT JOIN paltc as b on a.td001=b.tc001 and a.td005=b.tc003
				LEFT JOIN cmsmv as c on a.td001=c.mv001 
				LEFT JOIN palta as d on a.td001=d.ta001 and a.td005 = d.ta003 
		               WHERE td005='$seq3' and mv032='$seq7' and substr(td050,1,1)>'9' ";
			if(@$seq1){
			...
			if(@$seq5){
				$sql.=" and ( c.mv202 = '".$seq5[0]."' ";
				foreach($seq5 as $key => $val){
					$sql .= " or c.mv202 = '".$val."'";
				}
				$sql.=" ) ";
			}
			$sql .= " GROUP BY td001,td003 ";  
			$query = $this->db->query($sql);
			$data = $query->result();
			
			
			$sql = " SELECT b.ta001,b.ta002,b.ta003,c.mv002,IFNULL(a.ta005,0)+b.ta005 as ta005,IFNULL(a.ta008,0)+b.ta008 as ta008,IFNULL(a.ta010,0)+b.ta010 as ta010,a.ta014,a.ta015,a.ta016,a.ta017,d.td005 
						FROM paltd as d 
						LEFT JOIN palta as a on a.ta001 = d.td001 and a.ta003 = d.td005
						LEFT JOIN cmsmv as c on d.td001 = c.mv001 
						LEFT JOIN palta1 as b on b.ta001 = d.td001 
		            WHERE td005='$seq3' and mv032='$seq7' and substr(td050,1,1)>'9' ";  
			if(@$seq1){
				$sql.=" and d.td001 >= '".$seq1."' ";
			}
			if(@$seq2){
				$sql.=" and d.td001 <= '".$seq2."' ";
			} 
			if(@$seq5){
				$sql.=" and ( c.mv202 = '".$seq5[0]."' ";
				foreach($seq5 as $key => $val){
			...
			foreach($data as $key => $val){
				$val->td039 += $val->td038;
				$val->td038 = 0;
				$ret['rows'][$val->td001] = $val;
				if(isset($data_cost[$val->td001])){
					$ret['rows'][$val->td001]->cost = $data_cost[$val->td001];
				}
			}
        //  $seq32 = "td005='$seq3'  ";	
