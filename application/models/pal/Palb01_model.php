		...
		while($row=mysql_fetch_assoc($result)){
			...
			//   if (substr($mv021,0,6)==$vyymm1 && $workday>30) { $workday=30;}
				...
				//	$hhamt4=($md004+$md005+$md006+$md007+$md012)/240;    //皇興 請假
				 ...
				 /*   $damt1=round($hhamt2*($tc015*3*8),0);
					...
					$damt31=round($hhamt4*($tc202*8/2),0);   //生理假
                    $damt4=round($hhamt*$tc010,0);	
					//if ($tc010==8) {$damt4=$md003;}
					//if ($tc202==1) {echo var_dump($damt31);exit;}
                    $damt=$damt1+$damt2+$damt3+$damt4+$damt31;
				//	if ($tc202==1) {echo var_dump($damt);exit;}
			...
			/*		$vtd008=round(round($md004/30/8,0)*(($tc015*3*8)+$tc006+($tc007/2)+$tc010),0);
				...
				//	$vtd011=round(round($md007/30/8,0)*(($tc015*3*8)+$tc006+($tc007/2)),0);
					...
					//特殊離職去掉全勤 1070903
					//補正含4次沒全勤
				//	if ($md001=="107050") { $vtd012= 1000;}
				//	if ($tc005>=4) { $vtd012= 1000;}
				...
				//當月到當月離不是月底扣全勤1000   1071102
				if (substr($mv021,0,6)==$vyymm1 and (substr($mv022,0,6)==$vyymm1 )) {
				          $vtd012= $zmd008;
				   }  
				 if ($mv001=='107072') {$vtd012= 0;}
				...
				*/
				//	2017.02.14新的加班薪資計算	
					$vtc017=round($hhamt3*$tc017*4/3,0);          //平常
					$vtc018=round($hhamt3*$tc018*5/3,0);          //平常2上
					$vtc019=round($hhamt3*$tc019*4/3,0);          //六加
					$vtc020=round($hhamt3*$tc020*5/3,0);          //六加2-8
				    $vtc021=round($hhamt3*$tc021*1,0);            //假日加 國定假日
					$vtc022=round($hhamt3*$tc022*4/3,0);          //假日加2-8
					$vtc024=round($hhamt3*$tc024*5/3,0);  //六加班8小時以上 先以三-八算法算 正常應為:*2.66
					$vtc025=round($hhamt3*$tc025*5/3,0);  //日加班8小時上 國定假日            
			...
			/*	   if ((substr($mv021,0,6)==$vyymm1) or (substr($mv022,0,6)==$vyymm1)) {$vtd038=$md004-round(($md004/30)*$workday,0)+
				...
				//+全勤獎金-全勤獎金+特別-特別+業務-業務+執照-執照+資歷-資歷
				 ...
				 //外勞6%10710  11月拿掉 1071102
				 if($mv001=='107050' or $mv001=='107054'){
					  $vtax = 0.06;
				  }
				  ...
				  if (($mv032)>='3' && $mv038=='1' && $mv040 ==0.18 ) { $vtax = 0.18;}//如果基本資料有輸入以基本資料為主，除非離境日計算
                   if($mv001=='107033'){
					  $vtax = 0;
				  }
				//應稅所得 vtaxamt1=應領金額-免稅加班金額44-免稅伙食津貼49,   所得稅=vtaxamt1*稅率
				   ...
				   //外勞 1050809 0不扣所得稅
					...
					//1080125
					if ($mv001=='106090') {$vtd037 = round($vtaxamt1*0.05,0);}
					if ($mv040=='0.05') {$vtd037 = round($vtaxamt1*0.05,0);}
				...
				//修改個人代扣補充保費 最低工資不扣二代健保費  如在外投保應稅所得 *0.0191費率,應稅所得
				//  -投保金額X4 =Y   Y*.0191 即應扣二代健保費
					$vtd035 = 0;//歸零
					if($ml007<=0){
						if($mv200>0&&$mv200<1){
				...
				//勞保健保費依照加保退保日期計算	2017.02.21 Talence Editor 當月最後一天離職才扣健保1070831
					...
					//當月最後一天離職才扣健保1070831 td034
					$start_day=$vyymm1.'01';
					if($mv022>=$start_day && $mv022< $last_day ) {$ml004=0;}
