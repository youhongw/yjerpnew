<?php if ( ! defined('BASEPATH')) exit('No direct script access allowed');

class Palr54_model extends CI_Model {
	
	function __construct()
        {
          parent::__construct();      //重載ci底層程式 自動執行父類別
        }
		
	
	//印明細表	
	function printfd()          
        {
		  $seq3=substr($this->input->post('dateo'),0,4).substr($this->input->post('dateo'),5,2);
		  $seq5=$this->input->post('mv202');
		  $seq6=$this->input->post('y_lou');
		  $sql = " SELECT a.*,b.mv002,b.mv202,c.mm002 FROM paltd as a LEFT JOIN cmsmv as b on a.td001=b.mv001 LEFT JOIN palmm as c on b.mv202=c.mm001 
		               WHERE td005='$seq3' ";  
					   
		  if(@$seq5){
			  $sql.=" and ( mv202 = '".$seq5[0]."' ";
			  foreach($seq5 as $key => $val){
				  $sql .= " or mv202 = '".$val."'";
			  }
			  $sql.=" ) ";
		  } 
		  if(@$seq6){
			  $sql .= " and SUBSTR(mv009,2,1) > '9' ";
		  }
		  
		  $sql .= " GROUP BY td001,td003 ORDER BY mv202 ASC ,mv004 ASC ,td001 ASC ";  
		  
		  $query = $this->db->query($sql); 
		  
	      $ret['rows'] = $query->result();
		  
	      $sql = " SELECT COUNT(o.mv202) as count FROM(
		  SELECT a.*,b.mv202 FROM paltd as a LEFT JOIN cmsmv as b on a.td001=b.mv001 
		  WHERE td005='$seq3' "; 
		  if(@$seq5){
			  $sql.=" and ( mv202 = '".$seq5[0]."' ";
			  foreach($seq5 as $key => $val){
				  $sql .= " or mv202 = '".$val."'";
			  }
			  $sql.=" ) ";
		  }
		  if(@$seq6){
			  $sql .= " and SUBSTR(mv009,2,1) > 9";
		  }
		  
		  $sql .= " GROUP BY td001,td003 ) as o ";
		  
		  $query = $this->db->query($sql); 
	      $num = $query->result();
	      $ret['num_rows'] = $num[0]->count;		
	      return $ret;
        }
		
	//印excel	
	function excelfd()          
        {
		  $seq3=substr($this->input->post('dateo'),0,4).substr($this->input->post('dateo'),5,2);
		  $seq5=$this->input->post('mv202');
		  $seq6=$this->input->post('y_lou');
		  $cost_detail=$this->input->post('cost_detail');
		  $sql = " SELECT a.td001,a.td002,a.td007,a.td006,a.td008,a.td009,a.td010,a.td011,a.td012,a.td013
		  ,a.td014,a.td015,a.td016
		  ,a.td017+a.td019 as tol_td017
		  ,a.td018+a.td020 as tol_td018
		  ,a.td021+a.td023 as tol_td021
		  ,a.td022+a.td024 as tol_td022
		  ,a.td025+a.td027 as tol_td025
		  ,a.td026+a.td028 as tol_td026,a.td029,a.td030,a.td031,a.td033,a.td034,a.td037,a.td036
		  ,a.td038,a.td035,a.td039,a.td040,a.td041 ";
		  if($cost_detail == "Y"){
			  $sql .="
				,IFNULL(d.ta004, 0)+IFNULL(e.ta004, 0) as ta004
				,IFNULL(d.ta005, 0)+IFNULL(e.ta005, 0) as ta005
				,IFNULL(d.ta006, 0)+IFNULL(e.ta006, 0) as ta006
				,IFNULL(d.ta007, 0)+IFNULL(e.ta007, 0) as ta007
				,IFNULL(d.ta008, 0)+IFNULL(e.ta008, 0) as ta008
				,IFNULL(d.ta009, 0)+IFNULL(e.ta009, 0) as ta009
				,IFNULL(d.ta010, 0)+IFNULL(e.ta010, 0) as ta010
				,IFNULL(d.ta011, 0)+IFNULL(e.ta011, 0) as ta011
				,IFNULL(d.ta014, 0),IFNULL(d.ta015, 0),IFNULL(d.ta016, 0),IFNULL(d.ta017, 0)
			  ";
			  
		  }
			  
		  $sql .=" FROM paltd as a 
		  LEFT JOIN cmsmv as b on a.td001=b.mv001 
		  LEFT JOIN palmm as c on b.mv202=c.mm001 ";
		  if($cost_detail == "Y"){
			  $sql .=" LEFT JOIN palta as d on a.td005=d.ta003 and a.td001=d.ta001 ";
			  $sql .=" LEFT JOIN palta1 as e on a.td001=e.ta001 ";
		  }
		  
		  $sql .=" WHERE td005='$seq3' ";  
					   
		  if(@$seq5){
			  $sql.=" and ( mv202 = '".$seq5[0]."' ";
			  foreach($seq5 as $key => $val){
				  $sql .= " or mv202 = '".$val."'";
			  }
			  $sql.=" ) ";
		  }
		  if(@$seq6){
			  $sql .= " and SUBSTR(mv009,2,1) > '9' ";
		  }
		  
		  $sql .= " GROUP BY td001,td003 ORDER BY mv202 ASC ,mv004 ASC ,td001 ASC ";
		  
		  $query = $this->db->query($sql); 
		  
		  $ret['rows'] = $query->result_array();
		  
	      return $ret;
        }
		
	//列出類別
	function printcol()
        {
	      $this->db->select('mm001, mm002, mm003');
          $this->db->from('palmm');
	      $this->db->order_by('mm001 ASC, mm002 ASC');    //排序  單欄以上 asc 由小至大 desc預設由大至小
	      $ret['rows']=$this->db->get()->result();
	      $this->db->select('COUNT(*) as count');    //查詢總筆數
	      $this->db->from('palmm');
          $query = $this->db->get();
	      $tmp = $query->result();
	      $ret['num_rows'] = $tmp[0]->count;
	      return $ret;
        }
		
	//選取印單據筆	
	function printfd1()   
       {           
           $this->db->select('a.* ,c.mq002 AS tg001disp, d.me002 AS tg004disp, e.mb002 AS tg010disp, f.mv002 AS tg012disp,
		  b.company, b.creator, b.usr_group, b.create_date, b.modifier, b.modi_date, b.flag, b.mv001, b.mv002, b.mv003, b.mv004, b.mv005,
		  b.mv006, b.mv007, b.mv011, b.mv009, b.mv017, b.mv018, b.mv012,g.mm002');
		 
        $this->db->from('coptg as a');	
        $this->db->join('copth as b', 'a.tg001 = b.mv001  and a.tg002=b.mv002 ','left');		
		$this->db->join('cmsmq as c', 'a.tg001 = c.mq001 and c.mq003="31" ','left');
		$this->db->join('cmsme as d', 'a.tg004 = d.me001 ','left');
	    $this->db->join('cmsmb as e', 'a.tg010 = e.mb001 ','left');
		$this->db->join('cmsmv as f ', 'a.tg012 = f.mv001','left');	
		$this->db->join('palmm as g ', 'f.mv202 = g.mm001','left');		
		$this->db->where('a.tg001', $this->uri->segment(4)); 
	    $this->db->where('a.tg002', $this->uri->segment(5)); 
		$this->db->order_by('tg001 , tg002 ,b.mv003');
		
		$query = $this->db->get();
		$ret['rows'] = $query->result();
		$seq1=$this->uri->segment(4);
		$seq2=$this->uri->segment(5);
		$this->db->where('mv001', $this->uri->segment(4));
		$this->db->where('mv002', $this->uri->segment(5));
	    $query = $this->db->get('copth');	      
	    $num = $query->get()->result();		
	    $ret['num_rows'] = $num[0]->count;
		return $ret;
       }
	   
	 //印單據筆 A4
	function printfc()   
      {           
          $this->db->select('a.* ,c.mq002 AS tg001disp, d.mb002 AS tg010disp,e.mf002 AS tg011disp, f.mv002 AS tg006disp,g.na003 AS tg047disp,
		  ,h.ma002 AS tg004disp,h.ma006 as tg004disp1,h.ma008 as tg004disp2,h.ma005 as tg004disp3,b.company, b.creator, b.usr_group, b.create_date, b.modifier, b.modi_date, b.flag, b.mv001, b.mv002, b.mv003, b.mv004, b.mv005,
		  b.mv006, b.mv007, b.mv008, b.mv009, b.mv010, b.mv011, b.mv012,b.mv013, b.mv014,b.mv016,b.mv017,b.mv018,b.mv019,b.mv031,i.mc002 as mv007disp,j.me002 as tg005disp');
		 
        $this->db->from('coptg as a');	
        $this->db->join('copth as b', 'a.tg001 = b.mv001  and a.tg002=b.mv002 ','left');	//單身	
		$this->db->join('cmsmq as c', 'a.tg001 = c.mq001 and c.mq003="22" ','left');  //單別
	    $this->db->join('cmsmb as d', 'a.tg010 = d.mb001 ','left');    //廠別
		$this->db->join('cmsmf as e', 'a.tg011 = e.mf001 ','left');		//幣別
		$this->db->join('cmsmv as f ', 'a.tg006 = f.mv001 and f.mv022 = " " ','left');  //業務人員
		$this->db->join('cmsna as g ', 'a.tg047 = g.na002 and g.na001= "2" ','left');    //付款條件
		$this->db->join('copma as h', 'a.tg004 = h.ma001 ','left');  //產品代號
		$this->db->join('cmsmc as i', 'b.mv007 = i.mc001 ','left');   //庫別
		$this->db->join('cmsme as j', 'a.tg005 = j.me001 ','left');   //部門
		$this->db->where('a.tg001', $this->input->post('tg001o')); 
	    $this->db->where('a.tg002', $this->input->post('tg002o')); 
		$this->db->order_by('tg001 , tg002 ,b.mv003');
		
		$query = $this->db->get();
	    $result['rows'] = $query->result();
	    if ($query->num_rows() > 0) 
		 {
		 return $result;
		 }
      }
	  
	//印單據筆  半張紙letter1/2 A4half
		function printfb()   
        {           
          $this->db->select('a.* ,c.mq002 AS tg001disp, d.mb002 AS tg010disp,e.mf002 AS tg011disp, f.mv002 AS tg006disp,g.na003 AS tg047disp,
		  ,h.ma002 AS tg004disp,h.ma006 as tg004disp1,h.ma008 as tg004disp2,h.ma005 as tg004disp3,b.company, b.creator, b.usr_group, b.create_date, b.modifier, b.modi_date, b.flag, b.mv001, b.mv002, b.mv003, b.mv004, b.mv005,
		  b.mv006, b.mv007, b.mv008, b.mv009, b.mv010, b.mv011, b.mv012,b.mv013, b.mv014,b.mv016,b.mv017,b.mv018,b.mv019,b.mv031,i.mc002 as mv007disp,j.me002 as tg005disp');
		 
        $this->db->from('coptg as a');	
        $this->db->join('copth as b', 'a.tg001 = b.mv001  and a.tg002=b.mv002 ','left');	//單身	
		$this->db->join('cmsmq as c', 'a.tg001 = c.mq001 and c.mq003="22" ','left');  //單別
	    $this->db->join('cmsmb as d', 'a.tg010 = d.mb001 ','left');    //廠別
		$this->db->join('cmsmf as e', 'a.tg011 = e.mf001 ','left');		//幣別
		$this->db->join('cmsmv as f ', 'a.tg006 = f.mv001 and f.mv022 = " " ','left');  //業務人員
		$this->db->join('cmsna as g ', 'a.tg047 = g.na002 and g.na001= "2" ','left');    //付款條件
		$this->db->join('copma as h', 'a.tg004 = h.ma001 ','left');  //產品代號
		$this->db->join('cmsmc as i', 'b.mv007 = i.mc001 ','left');   //庫別
		$this->db->join('cmsme as j', 'a.tg005 = j.me001 ','left');   //部門
		$this->db->where('a.tg001', $this->uri->segment(4)); 
	    $this->db->where('a.tg002', $this->uri->segment(5)); 
		$this->db->order_by('tg001 , tg002 ,b.mv003');
		
		$query = $this->db->get();
	    $result['rows'] = $query->result();
	    if ($query->num_rows() > 0) 
		 {
		 return $result;
		 }	    		
        }
		
	
}
/* End of file model.php */
/* Location: ./application/model/model.php */
?>