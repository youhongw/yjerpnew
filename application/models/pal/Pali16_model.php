...
class Pali16_model extends CI_Model {
	...
	function search($limit, $offset, $sort_by, $sort_order)     
	    { 
	      ...
	      $sort_columns = array('mo001', 'mo002', 'mo003', 'create_date');
	...
	***/
	...
	{
		$this->session->set_userdata('pali16_search',"display_search/".$offset);
		...
		$default_order = "mo001 desc";//在這裡塞入一些預設排序
		...
		if(!empty($_SESSION['pali16']['search']['where'])){
			if($where){$where .= " and ";}
			$where .= $_SESSION['pali16']['search']['where'];
		}
		...
		if(isset($_SESSION['pali16']['search']['order'])){
			if($order){$order .= " , ";}
			$order .= $_SESSION['pali16']['search']['order'];
		}
		
		if(!isset($_SESSION['pali16']['search']['order']) && $default_order){
			if($order){$order .= " , ";}
			$order .= $default_order;
		}
		/* order end */
		...
		$ret['data'] = $query->get()->result();
		//儲存sql
		$_SESSION['pali16']['search']['sql'] = $this->db->last_query();
		...
		//儲存where與order
		$_SESSION['pali16']['search']['where'] = $where;
		$_SESSION['pali16']['search']['order'] = $order;
		$_SESSION['pali16']['search']['offset'] = $offset;
	...
	function selone()    
        { 
		  $this->db->select('*');	
	      $this->db->set('mo001', $this->uri->segment(4));              
	    //$this->db->set('mo002', $this->uri->segment(5));
	      $this->db->where('mo001', $this->uri->segment(4));     
	    //$this->db->where('mo002', $this->uri->segment(5));
	      $query = $this->db->get('palmo');
	...
	function findf($limit, $offset, $sort_by, $sort_order)   
        {            		
	     //$seq5='';$seq51='';$seq7='';$seq71='';		  
	      ...
	      $seq1 = " mo001, mo002, mo003,mo004,mo005,mo006,  create_date FROM `palmo` ";
	...
	function insertf()    
        {
			$mo007 = implode(",",$this->input->post('mo007'));
	    $data = array( 
	    ...
	    $exist = $this->pali16_model->selone1($this->input->post('mo001'));   //查詢資料是否重複
	    if ($exist)
	     {
		  return 'exist';
		 } 
    ...
    function copyf()           
        {
	      ...
	      $data = array( 
           ...
           $exist = $this->pali16_model->selone2($this->input->post('mo002c'));
		   if ($exist)
		    {
			  return 'exist';
		    }         
	...
	function excelnewf()           
        {			
	      ...
	      $sql = " SELECT mo001,mo002,mo003,mo004,mo005,mo006,create_date FROM palmo WHERE mo001 >= '$seq1' AND mo001 <= '$seq2'  "; 
	...
	function printfd()           
        {
	      $seq1=$this->input->post('mo001c');    
	      $seq2=$this->input->post('mo002c');
	      $sql = " SELECT * FROM palmo WHERE mo001 >= '$seq1'  AND mo001 <= '$seq2'  "; 
          $query = $this->db->query($sql);
	      $ret['rows'] = $query->result();
