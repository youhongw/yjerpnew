...
class Pali40_model extends CI_Model {
	...
	function selbrowse($num,$offset)   
        {            
	      $this->db->select('ya001, ya002, ya003, ya004, ya005, ya006, create_date');
          $this->db->from('palya');
          //$this->db->order_by('id', 'DESC');                //排序  單欄
	      $this->db->order_by('ya001 desc, ya002 desc');    //排序  單欄以上 asc 由小至大 desc預設由大至小
	...
	function ajaxkey($seg1)    
        { 	              
	      $this->db->set('ya001', $this->uri->segment(4));
	...
	function findf($limit, $offset, $sort_by, $sort_order)   
        {            		
	     //$seq5='';$seq51='';$seq7='';$seq71='';		  
	      $seq11 = "SELECT COUNT(*) as count  FROM `palya` ";
	      $seq1 = " ya001, ya002, ya003  create_date FROM `palya` ";
	...
	function filterf1($limit, $offset , $sort_by  , $sort_order)            
	   {    
	      ...
	      $this->db->select('ya001, ya002, ya003, create_date');
	      $this->db->from('palya');
	...
	function insertf()    
	    ...
	    $exist = $this->pali40_model->selone1($this->input->post('ya001'));   //查詢資料是否重複
	    if ($exist)
	     {
		  return 'exist';
		 } 
          return  $this->db->insert('palya', $data);
        }
		
	//查copy複製資料是否重複	 
    function selone2($seg1)    
        {
	     $this->db->where('ya001', $this->input->post('ya002c')); 
	     $query = $this->db->get('palya');
	     return $query->num_rows() ; 
	    }
    ...
    function copyf()           
        {
          ...
          $seq3=$this->input->post('ya002c');    //主鍵一筆
	      $data = array( 
	                 'company' => $this->session->userdata('syscompany'),
	                 'creator' => $this->session->userdata('manager'),
		             'usr_group' => 'A100',
           ...
           $exist = $this->pali40_model->selone2($this->input->post('ya002c'));
		   if ($exist)
		    {
			  return 'exist';
		    }         
              return $this->db->insert('palya', $data);      //複製一筆  
          }
	...
	function excelnewf()           
        {			
	      ...
	      $sql = " SELECT ya001,ya002,ya003,create_date FROM palya WHERE ya001 >= '$seq1' AND ya001 <= '$seq2'  "; 
	...
	function printfd()           
        {
	      $seq1=$this->input->post('ya001c');    
	      $seq2=$this->input->post('ya002c');
	      $sql = " SELECT * FROM palya WHERE ya001 >= '$seq1'  AND ya001 <= '$seq2'  "; 
          $query = $this->db->query($sql);
	      $ret['rows'] = $query->result();
		
          $seq32 = "ya001 >= '$seq1'  AND ya001 <= '$seq2'  ";	
	      $query = $this->db->select('COUNT(*) as count', FALSE)  //筆數查詢,如果設為FALSE不會使用反引號保護你的字段或者表名
		                ->from('palya')
		                ->where($seq32);
	      $num = $query->get()->result();		
