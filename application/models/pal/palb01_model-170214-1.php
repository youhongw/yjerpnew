		...
		while($row=mysql_fetch_assoc($result)){
			...
			foreach($row as $i=>$v){
			 ...
			 //	  if (substr($mv022,0,6)==$vyymm1) { $workday1=30-substr($mv022,6,2);$workday=$workday+round($workday/7);}	
				   ...
				   $workday = $day_end - $day_str - (($tc006/8)+($tc007/8)+($tc008/8)+($tc009)+($tc010/8)+($tc011)+($tc012)+($tc013)+($tc014)+($tc015)+($tc016));
				   //作為之前以請假請就職前日子的補救
				   if($workday<0){$workday=$workday+(($tc006/8)+($tc007/8)+($tc008/8)+($tc009)+($tc010/8)+($tc011)+($tc012)+($tc013)+($tc014)+($tc015)+($tc016));}
			...
			/*		$vtd008=round(round($md004/30/8,0)*(($tc015*3*8)+$tc006+($tc007/2)+$tc010),0);
				...
				//	$vtc019=round($hhamt1*$tc019,0);
					...
					if (substr($mv009,1,1)>'9'){$vtc019=round($vtc019*0.58,0);}
			...
			/*	   if ((substr($mv021,0,6)==$vyymm1) or (substr($mv022,0,6)==$vyymm1)) {$vtd038=$md004-round(($md004/30)*$workday,0)+
				...
				//實領金額 vtd039   1050722
				    $vtd039=$vtd030-$tb004-$ml003-$ml004-$vfood-$vtd037-$vtd038-$vtd035;
				//轉帳金額
