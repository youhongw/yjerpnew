	...
	function batchaf()           
        {
	        ...
	        $seq4=substr($this->input->post('date2c'),0,4).substr($this->input->post('date2c'),5,2).substr($this->input->post('date2c'),8,2);
		//加班是否平衡1070511
		$this->db->select(' a.* ');
		$this->db->from('paltf as a');
		$where= "substring(tf002,1,6)='$vyymm1'  and tf006+tf009!=ifnull(tf010,0)+ifnull(tf011,0)+ifnull(tf012,0)+ifnull(tf013,0)+ifnull(tf014,0)+ifnull(tf015,0)";
		$this->db->where($where);
		$query = $this->db->get();
		
	    if ($query->num_rows() > 0){echo "<script>text('星期歸類時數有錯, 請檢查明細');</script>";return "星期歸類時數有錯, 請檢查明細";echo var_dump('test');exit;}
		...
		//新增月出勤人員 PALTC
		  $sql2 = "INSERT INTO  paltc (tc001,tc002,tc003) select mv001,mv004,$vyymm1 from cmsmv where  (mv022='' or substr(mv022,1,6)>='$vyymm1') and substr(mv021,1,6)<='$vyymm1'"; 
		  $this->db->query($sql2);
		//加班單更新入月出勤 PALTF
           $sql21 =" update paltc as b,(select a.tf001,sum(a.tf010) as tf010,sum(a.tf011) as tf011,sum(a.tf012) as tf012,sum(a.tf013) as tf013,
		                                 sum(a.tf014) as tf014,sum(a.tf015) as tf015,sum(a.tf018) as tf018,sum(a.tf019) as tf019
               ...
               set b.tc017=c.tf010, b.tc018=c.tf011, b.tc019=c.tf012, b.tc020=c.tf013, b.tc021=c.tf014, b.tc022=c.tf015, b.tc024=c.tf018, b.tc025=c.tf019
               where b.tc001=c.tf001 and b.tc003='$vyymm1' " ;
			 $this->db->query($sql21);
		//請假單更新入月出勤	 PALTG
		        $sql22 =" update paltc as b,(select a.tg001,sum(a.tg004) as tg004,sum(a.tg005) as tg005,sum(a.tg006) as tg006,sum(a.tg007) as tg007,
                                         ...
                                         sum(a.tg012) as tg012,sum(a.tg013) as tg013,sum(a.tg014) as tg014,sum(a.tg015) as tg015,sum(a.tg016) as tg016,sum(a.tg201) as tg201,sum(a.tg202) as tg202									 
               ...
               set b.tc004=c.tg004, b.tc005=c.tg005, b.tc006=c.tg006, b.tc007=c.tg007, b.tc008=c.tg008, b.tc009=c.tg009, b.tc010=c.tg010, b.tc011=c.tg011,
			       b.tc012=c.tg012,b.tc013=c.tg013,b.tc014=c.tg014,b.tc015=c.tg015,b.tc016=c.tg016,b.tc201=c.tg201,b.tc202=c.tg202
               where b.tc001=c.tg001 and b.tc003='$vyymm1' " ; 
			 $this->db->query($sql22);
