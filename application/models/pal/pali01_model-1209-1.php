...
class Pali01_model extends CI_Model {
	...
	function search($limit, $offset, $sort_by, $sort_order)     //欄位表頭排序流覽資料
	    { 
	     ...
	     $query = $this->db->select('mv001, mv002, mv003, mv004, mv005, mv006, mv008, mv009, mv012, mv015,mv021,mv211,create_date')
	                       ->from('cmsmv')
						   ->where('mv022',"")
	...
	function findf($limit, $offset, $sort_by, $sort_order)     
		...
		if(@$_SESSION['admi05_sql_sort']){$seq33 = $_SESSION['admi05_sql_sort'];}
         ...
         $sort_order = (substr($sort_order,0,3) == 'asc') ? 'asc' : 'desc';
	     $sort_columns = array('mv001', 'mv002', 'mv003', 'mv004', 'mv005', 'mv006','mv007','mv008','mv009','mv012','mv015','mv021','create_date');
	     $sort_by = (in_array($sort_by, $sort_columns)) ? $sort_by : 'mv001';  //檢查排序欄位是否在 table 內
	     $query = $this->db->select('mv001, mv002, mv003, mv004, mv005, mv006,mv007,mv008,mv009,mv012,mv015,mv021, create_date')
	                       ->from('cmsmv')
		                   ->where($seq32)
			               ->order_by($seq33)
			              //->order_by($sort_by, $sort_order)
	...
	function filterf1($limit, $offset , $sort_by  , $sort_order)           
	    {    
	      ...
	      $sort_order = (substr($sort_order,0,3) == 'asc') ? 'asc' : 'desc';
	      $sort_columns = array('mv001', 'mv002', 'mv003', 'mv004', 'mv005', 'mv006','mv008','mv009','mv011','mv013','mv021','mv0211','create_date');
          $sort_by = (in_array($sort_by, $sort_columns)) ? $sort_by : 'mv001';  //檢查排序欄位是否為 table
			
	      $this->db->select('mv001, mv002, mv003, mv004, mv005, mv006,mv008,mv009,mv012,mv015,mv021,mv211, create_date');
	      $this->db->from('cmsmv');
	...
	function excelnewf()           
        {			
	      ...
	      $sql = " SELECT mv001,mv002,mv004,mv008,mv009,mv012,mv014,mv019,mv020,mv021,mv022,create_date FROM cmsmv WHERE mv001 >= '$seq1'  AND mv001 <= '$seq2' ORDER BY mv001 "; 
