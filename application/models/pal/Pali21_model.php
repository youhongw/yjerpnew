...
class Pali21_model extends CI_Model {
	...
	function search($limit, $offset, $sort_by, $sort_order)     
	    { 
	      $sort_order = (substr($sort_order,0,3) == 'desc') ? 'desc' : 'asc';
	      $sort_columns = array('mk001', 'mk002', 'mk003','mk004', 'create_date');
	      $sort_by = (in_array($sort_by, $sort_columns)) ? $sort_by : 'mk004';  //檢查排序欄位是否在 table 內
	      $query = $this->db->select('mk001, mk002, mk003,mk004,  create_date')
	                       ->from('palmk')
	...
	function ajaxkey($seg1)    
        { 	              
	      $this->db->set('mk001', $this->uri->segment(4));
	...
	function selone()    
        { 
		  $this->db->select('*');	
	      $this->db->set('mk001', $this->uri->segment(4));              
	    //$this->db->set('mk002', $this->uri->segment(5));
	      $this->db->where('mk001', $this->uri->segment(4));     
	...
	function findf($limit, $offset, $sort_by, $sort_order)   
        {            		
	     //$seq5='';$seq51='';$seq7='';$seq71='';		  
	      ...
	      $seq1 = " mk001, mk002, mk003,  create_date FROM `palmk` ";
	   ...
	   //  $seq7=$this->session->userdata('find07');
		...
		if(@$_SESSION['admi05_sql_sort']){$seq33 = $_SESSION['admi05_sql_sort'];}
	      ...
	      $sort_columns = array('mk001', 'mk002', 'mk003','create_date');
	...
	function insertf()    
	    ...
	    $exist = $this->pali21_model->selone1($this->input->post('mk001'));   //查詢資料是否重複
	    if ($exist)
	     {
		  return 'exist';
		 } 
    ...
    function selone2($seg1)    
        {
	     $this->db->where('mk001', $this->input->post('mk002c')); 
	     $query = $this->db->get('palmk');
	     return $query->num_rows() ; 
	    }
    ...
    function copyf()           
        {
          ...
          $seq3=$this->input->post('mk002c');    //主鍵一筆
	      $data = array( 
	                 'company' => $this->session->userdata('syscompany'),
	                 'creator' => $this->session->userdata('manager'),
		             'usr_group' => 'A100',
           ...
           $exist = $this->pali21_model->selone2($this->input->post('mk002c'));
		   if ($exist)
		    {
			  return 'exist';
		    }         
	...
	function excelnewf()           
        {			
	      ...
	      $sql = " SELECT mk001,mk002,mk003,create_date FROM palmk WHERE mk001 >= '$seq1' AND mk001 <= '$seq2'  "; 
	...
	function printfd()           
        {
	      $seq1=$this->input->post('mk001c');    
	      $seq2=$this->input->post('mk002c');
	      $sql = " SELECT * FROM palmk WHERE mk001 >= '$seq1'  AND mk001 <= '$seq2'  "; 
          $query = $this->db->query($sql);
	      $ret['rows'] = $query->result();
		
          $seq32 = "mk001 >= '$seq1'  AND mk001 <= '$seq2'  ";	
	      $query = $this->db->select('COUNT(*) as count', FALSE)  //筆數查詢,如果設為FALSE不會使用反引號保護你的字段或者表名
		                ->from('palmk')
		                ->where($seq32);
	      $num = $query->get()->result();		
