...
class pali18_model extends CI_Model {
	...
	function insertf()   
       {
		if(!@$this->input->post('mw001')){$mw001=1;}else{$mw001=$this->input->post('mw001');}
	    $data = array( 
	...
	function auto_update_level(){
		...
		$query = $this->db->select('company, creator, usr_group, mw001, mw002, mw003, mw004
									, mw005, mw006, mw007, mw008, mw009, mw010, create_date')
			->from('palmw')
			->order_by('mw002','asc');
		...
		foreach($origin_data as $key => $val){
			$insurance = $rates->mr001*$val->mw002/100+$rates->mr002*$val->mw002/100;
			$data = array(
				'company' => $val->company,
				'creator' => $val->creator,
		...
		$temp_ary = array(
		        ...
		        'modi_date' => '',
		        'flag' => 0,
				'mw001' => 0,'mw002' => 0,'mw003' => 0,'mw004' => 0,
				'mw005' => 0,'mw006' => 0,'mw008' => 0,'mw009' => 0,
				'mw007' => 0,'mw010' => ""
			);
		//$this->db->insert('palmp', $temp_ary);
		foreach($new_data as $key => $val){
			$this->db->insert('palmw', $val);$new_count++;
		}
	...
	function get_all_data()     
	  { 
	    $query = $this->db->select('mw001, mw002, mw003, mw004, mw008, mw009, create_date')
	                      ->from('palmw')
		                  ->order_by('mw002', 'asc');
	    $ret['rows'] = $query->get()->result();
   ...
   function lookupd_check($keyword){  
      ...
      $this->db->select('mw001, mw002');
	  $this->db->from('palmw');  
      $this->db->where('mw002',$mw002);
      $query = $this->db->get(); 
      return $query->result();
    }  	
   function ajaxpali18($seg1)    //ajax 查詢一筆 不更新網頁 廠別 
          { 	              
	    $this->db->set('mw002', $this->uri->segment(4));
	    $this->db->where('mw002', $this->uri->segment(4));	
	    $query = $this->db->get('palmw');
