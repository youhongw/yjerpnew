...
class Palr47_model extends CI_Model {
	...
	function printfd()          
        {
		 $seq1=$this->input->post('ta001o');
		  ...
		  $seq5=$this->input->post('mv202');
		   ...
		   $sql = " SELECT a.*,b.*,c.mv202,c.mv036 FROM paltd as a 
		   LEFT JOIN paltf as b on a.td001=b.tf001  and a.td005 = substr(b.tf002,1,6)
		   LEFT JOIN cmsmv as c on a.td001=c.mv001 
		               WHERE substr(c.mv009,2,1) > '9' and substr(td050,1,1)>'9' ";  
			...
			if(@$seq3){
				$sql.=" and b.tf002>= '".$seq3."' ";
			} 
			if(@$seq4){
				$sql.=" and b.tf002<= '".$seq4."' ";
			} 
			if(@$seq5){
	...
	function excelfd()          
        {
		  ...
		  $seq5=$this->input->post('mv202');
		   ...
		   $sql = " SELECT b.tf002,a.td003,a.td004,b.tf001,a.td002,b.tf010,b.tf011,b.tf012,b.tf013
		   ,b.tf014,b.tf015 
		   FROM paltd as a 
		   LEFT JOIN paltf as b on a.td001=b.tf001  and a.td005 = substr(b.tf002,1,6)
		   LEFT JOIN cmsmv as c on a.td001=c.mv001 
