...
class Pali48_model extends CI_Model {
	...
	function selbrowse($num,$offset)   
        {            
	      $this->db->select('yh001, yh002, yh003, yh004, yh005, yh006, create_date');
          $this->db->from('palyh');
	...
	function search($limit, $offset, $sort_by, $sort_order)     
	    { 
	      ...
	      $sort_columns = array('yh001', 'yh002', 'yh005','yh006','yh008','yh009','yh010','yh020','yh036', 'create_date');
	      $sort_by = (in_array($sort_by, $sort_columns)) ? $sort_by : 'yh001';  //檢查排序欄位是否在 table 內
	      $query = $this->db->select('a.*')
	                       ->from('palyh as a')
						   ->join('cmsmv as b', 'a.yh002 = b.mv001 ','left')
	...
	function ajaxkey($seg1)    
        { 	              
	    //  $this->db->set('yh001', $this->uri->segment(4));
	...
	function selone()    
		...
		/*  $this->db->select('*');		     
          ...
          $this->db->where('yh002', $this->uri->segment(5));   		  
	      $query = $this->db->get('palyh'); */
		   $this->db->select('a.*, b.mv002 as yh002disp');
	       $this->db->from('palyh as a');
		   $this->db->join('cmsmv as b', 'a.yh002 = b.mv001 ','left');
		   $this->db->where('yh001', $this->uri->segment(4));
		   $this->db->where('yh002', $this->uri->segment(5));
			$query = $this->db->get();
	      if ($query->num_rows() > 0) 
	...
	function findf($limit, $offset, $sort_by, $sort_order)   
        {            		
	     //$seq5='';$seq51='';$seq7='';$seq71='';		  
	      ...
	      $seq1 = " yh001, yh002, yh003 , create_date FROM `palyh` ";
	   ...
	   //  $seq7=$this->session->userdata('find07');
		...
		if(@$_SESSION['admi05_sql_sort']){$seq33 = $_SESSION['admi05_sql_sort'];}
          ...
          $sort_order = (substr($sort_order,0,3) == 'asc') ? 'asc' : 'desc';
	      $sort_columns = array('yh001', 'yh002', 'yh003','yh004','create_date');
	      $sort_by = (in_array($sort_by, $sort_columns)) ? $sort_by : 'yh001';  //檢查排序欄位是否在 table 內
	      $query = $this->db->select('yh001, yh002,b.mv002 as yh002disp, yh003,yh004,yh007,yh011,yh012,  a.create_date')
	                        ->from('palyh as a')
							->join('cmsmv as b', 'a.yh002 = b.mv001 ','left')
		                   ->where($seq32)
			              ->order_by($seq33)
	...
	function filterf1($limit, $offset , $sort_by  , $sort_order)            
	   {    
	      ...
	      $sort_order = (substr($sort_order,0,3) == 'asc') ? 'asc' : 'desc';
	      $sort_columns = array('yh001', 'yh002', 'yh005','yh006','yh008','yh009','yh010','yh020', 'create_date');
	...
	function insertf()    
	    ...
	    $exist = $this->pali48_model->selone1($this->input->post('yh001'),$this->input->post('palq01a'));   //查詢資料是否重複
	    if ($exist)
	     {
		  return 'exist';
		 } 
          return  $this->db->insert('palyh', $data);
        }
    ...
    function copyf()           
        {
	      $seq1=$this->input->post('yh001c'); 
	      $this->db->where('yh002', $this->input->post('yh001c')); 
	      $query = $this->db->get('palyh');
          ...
          $seq3=$this->input->post('yh002c');    //主鍵一筆
	      $data = array( 
	                 'company' => $this->session->userdata('syscompany'),
	                 'creator' => $this->session->userdata('manager'),
		             'usr_group' => 'A100',
           ...
           $exist = $this->pali48_model->selone2($this->input->post('yh002c'));
		   if ($exist)
		    {
			  return 'exist';
		    }         
              return $this->db->insert('palyh', $data);      //複製一筆  
          }
		  
	//轉excel檔	 
	function excelnewf()           
        {			
	      $seq1=$this->input->post('yh001c');    //查詢一筆以上
	      $seq2=$this->input->post('yh002c'); 
	      $sql = " SELECT yh001,yh002,yh003,yh012,create_date FROM palyh WHERE yh001 >= '$seq1' AND yh001 <= '$seq2'  "; 
	...
	function printfd()           
        {
	      $seq1=$this->input->post('yh001c');    
	      $seq2=$this->input->post('yh002c');
	      $sql = " SELECT a.*,b.mv002 as yh002disp FROM palyh as a left join cmsmv as b on a.yh002=b.mv001 WHERE yh001 >= '$seq1'  AND yh001 <= '$seq2'  "; 
          $query = $this->db->query($sql);
	      $ret['rows'] = $query->result();
		
          $seq32 = "yh001 >= '$seq1'  AND yh001 <= '$seq2'  ";	
	      $query = $this->db->select('COUNT(*) as count', FALSE)  //筆數查詢,如果設為FALSE不會使用反引號保護你的字段或者表名
		                ->from('palyh')
		                ->where($seq32);
	      $num = $query->get()->result();		
	...
	function updatef()   
        {
          ...
          $this->db->where('yh001', $this->input->post('yh001'));
		  $this->db->where('yh002', $this->input->post('palq01a'));
          $this->db->update('palyh',$data);                   
	...
	function deletef($seg1,$seg2)      
        {  
	      ...
	      $this->db->where('yh001', $seg1);
	      $this->db->where('yh002', $seg2);
	...
	function ajaxdata($seg1,$seg2)    
        { 
		  $sql99 = " UPDATE  palyh AS A,  
         (SELECT   yh001,yh002  FROM palyh  where  concat(yh001,yh002) = '$seg2'  ) AS B  
    SET A.`yh036`='$seg1'  
    WHERE A.`yh001`=B.`yh001` and  A.`yh002`=B.`yh002`   "; 	
