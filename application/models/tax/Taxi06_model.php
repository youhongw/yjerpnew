...
class taxi06_model extends CI_Model {
	...
	function construct_sql($limit = 15, $offset = 0, $func = "")
	  {
		...
		if(isset($_SESSION['taxi06']['search']['where'])){
			if($where){$where .= " and ";}
			$where .= $_SESSION['taxi06']['search']['where'];
		}
		...
		if(isset($_SESSION['taxi06']['search']['order'])){
			if($order){$order .= " , ";}
			$order .= $_SESSION['taxi06']['search']['order'];
		}
		
		if(!isset($_SESSION['taxi06']['search']['order']) && $default_order){
			if($order){$order .= " , ";}
			$order .= $default_order;
		}
		/* order end */
		...
		$ret['data'] = $query->get()->result();
		//儲存sql 語法
		$_SESSION['taxi06']['search']['sql'] = $this->db->last_query();
		...
		//儲存where與order
		$_SESSION['taxi06']['search']['where'] = $where;
		$_SESSION['taxi06']['search']['order'] = $order;
		$_SESSION['taxi06']['search']['offset'] = $offset;
	...
	function construct_view($data){
		...
		$_SESSION['taxi06']['search']['view'] = $view_array;
		$_SESSION['taxi06']['search']['index'] = $index_array;
		//echo "<pre>";var_dump($_SESSION['taxi06']['search']['view']);exit;
	}
	...
	function findf($limit, $offset, $sort_by, $sort_order)     
		...
		if(@$_SESSION['taxi06_sql_term']){$seq32 = $_SESSION['taxi06_sql_term'];}
		if(@$_SESSION['taxi06_sql_sort']){$seq33 = $_SESSION['taxi06_sql_sort'];}
	...
	function insertf()    //新增一筆 檔頭  taxmc
        {
		    //刪日期 / 符號
			 ...
			 //檢查資料是否已存在 若存在加1
			  while($this->taxi06_model->selone1($mc200,$mc211)>0){
    ...
    function copyf()           
        {
         ...
         //   if ($query->num_rows() != 1) { return 'exist'; }
            ...
            $exist = $this->taxi06_model->selone1($seq1,$seq2,$seq3);  //檢查單頭是否重複
		    if ($exist)
		      {
			    return 'exist';
		      }         
