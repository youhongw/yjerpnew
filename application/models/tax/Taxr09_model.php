...
class Taxr09_model extends CI_Model {
	...
	function insertf()    //新增一筆 檔頭  coptg
		...
		//	 if ($this->input->post('me018')!='1') {$me019=round($this->input->post('me019'));}
	     ...
	     $data = array( 
	      ...
	      $exist = $this->taxr09_model->selone1($this->input->post('copq03a23'),$this->input->post('me002'));
	      if ($exist)
	         {
		      return 'exist';
		     } 
		...
		//	while ($_POST['order_product'][  $n  ]['th004']) {		
	     ...
	     $exist = $this->taxr09_model->selone1d($this->input->post('copq03a23'),$this->input->post('me002'),$th003);
		...
		if ($_POST['order_product'][  $n  ]['th004']!='') {
		  ...
		  // 訂單已交數量
			 ...
			 $th016=$_POST['order_product'][ $n  ]['th016'];
			 $th008=$_POST['order_product'][ $n  ]['th008'];
			 $existb = $this->taxr09_model->selone3d($th014,$th015,$th016);
			  $data_add = array( 
	             'company' => $this->session->userdata('syscompany'),
	             'creator' => $this->session->userdata('manager'),
		         'usr_group' => 'A100',
		 ...
		 $query = $this->db->query($sql);	} 
		  ...
		  //庫存增加減少 品號,庫別, 數量
			 ...
			 $th007=$_POST['order_product'][ $n  ]['th007'];
			 $th008=$_POST['order_product'][ $n  ]['th008'];
			 $exista = $this->taxr09_model->selone2d($th004,$th007);
			  $data_add = array( 
	             'company' => $this->session->userdata('syscompany'),
	             'creator' => $this->session->userdata('manager'),
		         'usr_group' => 'A100',
    ...
    function copyf()           
        {
         ...
         //   if ($query->num_rows() != 1) { return 'exist'; }
            ...
            $exist = $this->taxr09_model->selone2($this->input->post('me001c'),$this->input->post('me002c'));
		    if ($exist)
		      {
			    return 'exist';
		      }         
	...
	function printfd()          
        {
	     $seq1=$this->input->post('cmsi11');   //公司
	      ...
	      //刪除計算當 taxmi  1071128
		   ...
		   $this->db->delete('taxmi'); 
		  //申報公司
		   $sql1 = "INSERT INTO taxmi (mi001,mi002,mi003,mi004,mi005,mi006,mi007,mi008,mi009) 
		 ...
		 $this->db->where('mi002 ', $seq2);
         $this->db->where('mi001 ', $seq1);	
		$this->db->order_by('mi001 , mi002');
		
		$query = $this->db->get();
	 
	      $ret['rows'] = $query->result();  
          $seq32 = "mi001 = '$seq1' and mi002= '$seq2' ";	
	      $query = $this->db->select('COUNT(*) as count', FALSE)  //筆數查詢,如果設為FALSE不會使用反引號保護你的字段或者表名
		              ->from('taxmi');
		           //   ->where($seq32);
	      $num = $query->get()->result();		
