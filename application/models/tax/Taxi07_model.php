...
class Taxi07_model extends CI_Model {
	...
	function construct_sql($limit = 15, $offset = 0, $func = "")
	  {
		...
		$default_order = "mc002 desc,mc001 desc";//在這裡塞入一些預設排序
		...
		if(isset($_SESSION['taxi07']['search']['where'])){
			if($where){$where .= " and ";}
			$where .= $_SESSION['taxi07']['search']['where'];
		}
		...
		if(isset($_SESSION['taxi07']['search']['order'])){
			if($order){$order .= " , ";}
			$order .= $_SESSION['taxi07']['search']['order'];
		}
		
		if(!isset($_SESSION['taxi07']['search']['order']) && $default_order){
			if($order){$order .= " , ";}
			$order .= $default_order;
		}
		/* order end */
		...
		/* Data SQL */
		$query = $this->db->select('a.*,b.ma002 as mc008disp,c.ma002 as mc009disp')
	                       ->from('taxmc2 as a')
						   ->join('copma as b', 'a.mc008 = b.ma001','left')
						   ->join('purma as c', 'a.mc009 = c.ma001','left')
			               ->order_by($order);
		if($where){
			$query->where($where);
		}
		...
		$ret['data'] = $query->get()->result();
		//儲存sql 語法
		$_SESSION['taxi07']['search']['sql'] = $this->db->last_query();
		...
		//儲存where與order
		$_SESSION['taxi07']['search']['where'] = $where;
		$_SESSION['taxi07']['search']['order'] = $order;
		$_SESSION['taxi07']['search']['offset'] = $offset;
	...
	function construct_view($data){
		...
		$_SESSION['taxi07']['search']['view'] = $view_array;
		$_SESSION['taxi07']['search']['index'] = $index_array;
		//echo "<pre>";var_dump($_SESSION['taxi07']['search']['view']);exit;
	}
	
	//查詢一筆 修改用   
	function selone($seg1) {
		$this->db->select('a.*,b.*,c.ma002 as mc008disp,d.ma002 as mc009disp,e.ma002 as mc001disp
		                  ');
		 
        $this->db->from('taxmc2 as a');	
        $this->db->join('taxmd2 as b', 'a.mc001 = b.md001 ','left');	//單身	
		$this->db->join('copma as c', 'a.mc008 = c.ma001 ','left');  //客戶代號 
	    $this->db->join('purma as d', 'a.mc009 = d.ma001 ','left');    //廠商代號 
		$this->db->join('taxma as e', 'a.mc001 = e.ma001 ','left');  //申報公司 
		$this->db->where('a.mc006', $seg1); 
		$this->db->order_by('mc006 ');
	...
	function selone_old($seq1,$seq2)  { 
        ...
        $this->db->from('taxmc2 as a');	
        $this->db->join('taxmd2 as b', 'a.mc001 = b.md001  and a.mc002=b.md002 ','left');	//單身	
	...
	function lookup($keyword){     
      $this->db->select('mb001, mb002, mb003,mb004')->from('taxmc2');  
	...
	function findf($limit, $offset, $sort_by, $sort_order)     
         {            		
	      ...
	      $seq11 = "SELECT COUNT(*) as count  FROM `taxmc2` ";
	      $seq1 = "mc001, mc002, mc003, mc004, mc004 as mc004disp,mc005, mc006,mc007,mc08,mc010,mc011,mc012,mc029,mc030, create_date FROM `taxmc2` ";
		...
		if(@$_SESSION['taxi07_sql_term']){$seq32 = $_SESSION['taxi07_sql_term'];}
		if(@$_SESSION['taxi07_sql_sort']){$seq33 = $_SESSION['taxi07_sql_sort'];}
		
         $sort_order = (substr($sort_order,0,3) == 'asc') ? 'asc' : 'desc';
	     $sort_columns = array('mc001', 'mc002', 'mc003', 'mc004','b.ma002', 'mc005', 'mc006','mc007','mc008','mc010','mc011','mc012','a.create_date');
	     $sort_by = (in_array($sort_by, $sort_columns)) ? $sort_by : 'mc001';  //檢查排序欄位是否在 table 內
	   //  $query = $this->db->select('mc001, mc002,b.ma002, mc003, mc004,mc004 as mc004disp, mc005, mc007,mc008,mc010,mc011,mc012, a.create_date')
	  //                     ->from('taxmc2 as a')
	//					   ->join('notma as b', 'a.mc003 = b.ma001 ','left')  //銀行代號 noti01
		$query = $this->db->select('a.mc001,a.mc002,a.mc003,b.ma002,b.ma004,a.mc005,a.mc007,a.mc016,c.mc002,a.mc004,a.create_date')
	                       ->from('taxmc2 as a')
						   ->join('notma as b', 'a.mc003 = b.ma001','left')
						   ->join('notmc as c', 'a.mc016 = c.mc001','left')
		                   ->where($seq32)
			               ->order_by($seq33)
			              ->limit($limit, $offset);
	     $ret['rows'] = $query->get()->result();
    ...
    function selone1d($seg1,$seg2)    
        {
	      ...
	      $query = $this->db->get('taxmd2');
	      return $query->num_rows() ;
	    }  	
 		
	//新增一筆 檔頭  taxmc2	
	function insertf()    //新增一筆 檔頭  taxmc2
        {
		    //刪日期 / 符號
		     preg_match_all('/\d/S',$this->input->post('mc002'), $matches);  //處理日期字串
			 $mc002 = implode('',$matches[0]);
			 preg_match_all('/\d/S',$this->input->post('mc003'), $matches);  //處理日期字串
			 $mc003 = implode('',$matches[0]);
			 preg_match_all('/\d/S',$this->input->post('mc007'), $matches);  //處理日期字串
			 $mc007 = implode('',$matches[0]);
			 
			 $mc001=$this->input->post('cmsi11');
			 $mc005=$this->input->post('mc005');
			 $mc006=$this->input->post('mc006');
			// $mc002=$this->input->post('mc002');
			/* $mc002no=$mc002;   //明細用再新增一筆時加1
			 //檢查資料是否已存在 若存在加1
			  while($this->taxi07_model->selone1($mc001,$mc002)>0){
				$mc002 = $this->check_title_no($mc001,$mc039);
				$mc002no=$mc002;
			} */
	     ...
	     $data = array( 
                 ...
                 'mc006' => $this->input->post('mc006'),				 
                 'mc007' => $mc007,  
                 'mc008' => $this->input->post('copi01'),   
                 'mc009' => $this->input->post('puri01'),
		   ...
		   foreach($order_product as $key => $val){
		        if($val['md005'] && $val['md006']){
					...
					/*	preg_match_all('/\d/S',$md013, $matches);  //處理日期字串
						...
						foreach($val as $k=>$v){
							if($k!="md001" && $k!="md002" && $k!="md003" && $k!="md004"){    //主鍵不用更改以及其他外來鍵庫別名稱
							    if($k=="md005") {$data[$k] = $vmd002;} else {$data[$k] = $v;}
							}
						}
					$this->db->insert('taxmd2', $data);
    ...
    function copyf()           
        {
         ...
         //   if ($query->num_rows() != 1) { return 'exist'; }
		    if ($query->num_rows() == 1) 
		       {
				...
				$mc035=$row->mc035;$mc036=$row->mc036;$mc037=$row->mc037;$mc038=$row->mc038;$mc039=$row->mc039;$mc040=$row->mc040;$mc041=$row->mc041;
				$mc042=$row->mc042;$mc043=$row->mc043;$mc044=$row->mc044;$mc045=$row->mc045;$mc046=$row->mc046;$mc047=$row->mc047;
				$mc048=$row->mc048;$mc049=$row->mc049;$mc050=$row->mc050;$mc051=$row->mc051;
			endforeach;
		       }   
		  
            $seq1=$this->input->post('mc001c');    //主鍵一筆檔頭taxmc2
	        ...
	        $data = array( 
				   ...
				   'mc031' => $mc031,'mc032' => $mc032,'mc033' => $mc033,'mc034' => $mc034,'mc035' => $mc035,'mc036' => $mc036,
				   'mc037' => $mc037,'mc038' => $mc038,'mc039' => $mc039,'mc040' => $mc040,'mc041' => $mc041,'mc042' => $mc042,
				   'mc043' => $mc043,'mc044' => $mc044,'mc045' => $mc045,'mc046' => $mc046,'mc047' => $mc047,'mc048' => $mc048,
				   'mc049' => $mc049,'mc050' => $mc050,'mc051' => $mc051
                   );
				   
            $exist = $this->taxi07_model->selone1($seq1,$seq2);  //檢查單頭是否重複
		    if ($exist)
		      {
			    return 'exist';
		      }         
             $this->db->insert('taxmc2', $data);      //複製一筆  
		    ...
		    if ($query->num_rows() >= 1) 
		       {
			     ...
			     $md023[$i]=$row->md023;$md024[$i]=$row->md024;$md025[$i]=$row->md025;$md026[$i]=$row->md026;$md027[$i]=$row->md027;
				 $md028[$i]=$row->md028;$md029[$i]=$row->md029;$md030[$i]=$row->md030;$md031[$i]=$row->md031;$md032[$i]=$row->md032;
				 $md033[$i]=$row->md033;$md034[$i]=$row->md034;$md035[$i]=$row->md035;$md036[$i]=$row->md036;
				 $i++;
			    endforeach;
		       }   
			$seq1=$this->input->post('mc001c');    //主鍵一筆明細taxmd2
			$seq2=$this->input->post('mc002c'); 
              $i=0;
            while ($i<$num) {	
			  $data_array = array( 
		         ...
		         'modi_date' => '',
		         'flag' => 0,
                'md001' => $seq1,'md002' => $seq2,'md003' => $md003[$i],'md004' => $md004[$i],'md005' => $md005[$i],'md006' => $md006[$i],'md007' => $md007[$i],
		         'md008' => $md008[$i],'md009' => $md009[$i],'md010' => $md010[$i],'md011' => $md011[$i],'md012' => $md012[$i],'md013' => $md013[$i],
				 'md014' => $md014[$i],'md015' => $md015[$i],'md016' => $md016[$i],'md017' => $md017[$i],'md018' => $md018[$i],'md019' => $md019[$i],
				 'md020' => $md020[$i],'md021' => $md021[$i],'md022' => $md022[$i],'md023' => $md023[$i],'md024' => $md024[$i],'md025' => $md025[$i],
				 'md026' => $md026[$i],'md027' => $md027[$i],'md028' => $md028[$i],'md029' => $md029[$i],'md030' => $md030[$i],'md031' => $md031[$i],'md032' => $md032[$i],
				 'md033' => $md033[$i],'md034' => $md034[$i],'md035' => $md035[$i],'md036' => $md036[$i]
                ); 
				
             $this->db->insert('taxmd2', $data_array);      //複製一筆 
             $i++;			 
            }
             return true;		
	   }
	...
	function excelnewf()           
        {			
	      ...
	      $sql = " SELECT mc001,mc002,mc039,mc004,ma002 as mc004disp,md003,md004,md005,md006,md010,md008,md011,md012 
		  FROM taxmc2 as a,taxmd2 as b,copma as c WHERE mc001=md001 and mc002=md002 and mc004=ma001 and mc001 >= '$seq1'  AND mc001 <= '$seq2' AND  mc002 >= '$seq3'  AND mc002 <= '$seq4'  "; 
	...
	function printfd()          
        {
		  ...
		  $seq3=$this->input->post('mc002o');    
	      $seq4=$this->input->post('mc002c');
		  preg_match_all('/\d/S',$this->input->post('mc002o'), $matches);  //處理日期字串
			 $seq3 = implode('',$matches[0]);
			preg_match_all('/\d/S',$this->input->post('mc002c'), $matches);  //處理日期字串
			 $seq4 = implode('',$matches[0]); 
			 //  echo var_dump($seq3.$seq4);exit;
	      $sql = " SELECT a.*,b.*,c.ma002 as mc003disp
		  FROM taxmc2 as a left join taxmd2 as b on mc001=md001 left join notma as c on mc003=ma001
	...
	function printfd1()   
       {           
        ...
        $this->db->from('taxmc2 as a');	
        $this->db->join('taxmd2 as b', 'a.mc001 = b.md001  and a.mc002=b.md002 ','left');		
	...
	function printfc()   
      {           
	    ...
	    $this->db->where('a.mc002 >= '.$this->input->post('mc002o').' and a.mc002 <= '.$this->input->post('mc002c')); 
		$this->db->order_by('mc001 , mc002 ,b.md003');
	...
	function updatef()   
        {
          ...
          $data = array(			
            ...
            $this->db->where('mc006', $mc006); //流水號KEY
            $this->db->update('taxmc2',$data);                   //更改一筆
			
			if ($this->input->post()){
				extract($this->input->post());
			}
			if(!is_array($order_product)){$order_product=array();}
		            $this->db->where('md004', $mc006);
					$this->db->delete('taxmd2'); //刪除明細 1060809
					
		    $vmd002='1010';   //流水號重新排序
			foreach($order_product as $key => $val){
				extract($val);
				
				if($this->seldetail($mc001,$val['md006'])>0){
					$data = array(
						'modifier' => $this->session->userdata('manager'),
						'modi_date' => date("Ymd"),
						'flag'  => $flag
					...
					foreach($val as $k=>$v){
						if($k!="md001"){//主鍵不用更改以及其他外來鍵庫別名稱 md013日期等別處理
							if($k=="md002") {$data[$k] = $vmd002;} else {$data[$k] = $v;}
						}
					}
					$this->db->where('md004', $mc006);
					$this->db->where('md005', $vmd002);
					$this->db->update('taxmd2',$data);//更改一筆
					$mmd002 = (int) $vmd002+10;
			        $vmd002 =  (string)$mmd002;
				}else{
					if($val['md005'] && $val['md006']){
    ...
    function seldetail($seg1,$seg2)    
        { 	
			$this->db->where('md005', $seg1);
			$this->db->where('md006', $seg2);
	        $query = $this->db->get('taxmd2');
	        return $query->num_rows() ; 
	    }	
	...
	function deletef($seg1)      
        { 
          ...
          $this->db->delete('taxmc2'); 
		  $this->db->where('md001', $this->uri->segment(4));
          $this->db->delete('taxmd2'); 
	    ...
	    if (!empty($_POST['selected'])) 
	         {
                foreach($_POST['selected'] as $check) 
			    {
				  ...
				  $this->db->delete('taxmd2'); $this->session->set_userdata('msg1',"已刪除");
            }
	    if ($this->db->affected_rows() > 0)
            {
              return TRUE;
	...
	//取單號 最大值加1
	function check_title_no($cmsi11,$mc002,$mc005){
	    ...
	    if (!$result[0]->max_no){return $cmsi11.$mc002.$mc005."001";}
