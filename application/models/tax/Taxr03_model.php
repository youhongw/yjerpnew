...
class Taxr03_model extends CI_Model {
	...
	function insertf()    //新增一筆 檔頭  coptg
		...
		//	 if ($this->input->post('me018')!='1') {$me019=round($this->input->post('me019'));}
	     ...
	     $data = array( 
	      ...
	      $exist = $this->taxr03_model->selone1($this->input->post('copq03a23'),$this->input->post('me002'));
	      if ($exist)
	         {
		      return 'exist';
		     } 
		...
		//	while ($_POST['order_product'][  $n  ]['th004']) {		
	     ...
	     $exist = $this->taxr03_model->selone1d($this->input->post('copq03a23'),$this->input->post('me002'),$th003);
		...
		if ($_POST['order_product'][  $n  ]['th004']!='') {
		  ...
		  // 訂單已交數量
			 ...
			 $th016=$_POST['order_product'][ $n  ]['th016'];
			 $th008=$_POST['order_product'][ $n  ]['th008'];
			 $existb = $this->taxr03_model->selone3d($th014,$th015,$th016);
			  $data_add = array( 
	             'company' => $this->session->userdata('syscompany'),
	             'creator' => $this->session->userdata('manager'),
		         'usr_group' => 'A100',
		 ...
		 $query = $this->db->query($sql);	} 
		  ...
		  //庫存增加減少 品號,庫別, 數量
			 ...
			 $th007=$_POST['order_product'][ $n  ]['th007'];
			 $th008=$_POST['order_product'][ $n  ]['th008'];
			 $exista = $this->taxr03_model->selone2d($th004,$th007);
			  $data_add = array( 
	             'company' => $this->session->userdata('syscompany'),
	             'creator' => $this->session->userdata('manager'),
		         'usr_group' => 'A100',
    ...
    function copyf()           
        {
         ...
         //   if ($query->num_rows() != 1) { return 'exist'; }
            ...
            $exist = $this->taxr03_model->selone2($this->input->post('me001c'),$this->input->post('me002c'));
		    if ($exist)
		      {
			    return 'exist';
		      }         
	...
	function printfd()          
        {
	       $dateo=substr($this->input->post('dateo'),0,4).substr($this->input->post('dateo'),5,2);
		     $datec=substr($this->input->post('datec'),0,4).substr($this->input->post('datec'),5,2);
		  $seq3=$this->input->post('cmsi11');
          $seq4=$this->input->post('tc003o');
          $seq5=$this->input->post('tc003c');   		  
	    ...
	    //  $seq4=substr($this->input->post('datec'),0,4).substr($this->input->post('datec'),5,2).substr(rtrim($this->input->post('datec')),8,2);
		 if ($seq3=='') {$seq3='0';}
		  ...
		  $this->db->select('a.*,ma002 as mc001disp '); 
        $this->db->from('taxmc1 as a');
		$this->db->join('taxma as b','a.mc001=b.ma001','left');
		$this->db->where('a.mc002 >=', $dateo );
		$this->db->where('a.mc002 <=', $datec );
		$this->db->where('a.mc001 >=', $seq3 );
		$this->db->where('a.mc010 >=', $seq4 );
		$this->db->where('a.mc010 <=', $seq5 );
		$this->db->order_by('mc004 , mc010');
		...
		$query = $this->db->get();
	      ...
	      $ret['rows'] = $query->result();  
          $seq32 = "mc002 >= '.$dateo.' and mc002 <= '.$datec.'   ";	
	      $query = $this->db->select('COUNT(*) as count', FALSE)  //筆數查詢,如果設為FALSE不會使用反引號保護你的字段或者表名
		              ->from('taxmc1')
		              ->where($seq32);
	      $num = $query->get()->result();		
