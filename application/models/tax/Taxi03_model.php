...
class Taxi03_model extends CI_Model {
	...
	function construct_sql($limit = 15, $offset = 0, $func = "")
	  {
		...
		/* Data SQL */
		$query = $this->db->select('a.*,b.ma002')
	                       ->from('taxmc1 as a')
						   ->join('taxma as b', 'a.mc001 = b.ma001','left')
			               ->order_by($order);
		if($where){
			$query->where($where);
		}
	...
	//查詢一筆 修改用   
	function selone($seg1, $seg2, $seg3) {
		$this->db->select('a.* , b.*,c.ma002 as mc001disp,d.ma002 as mc022disp');
		 
        $this->db->from('taxmc1 as a');	
        $this->db->join('taxmd1 as b', 'a.mc001 = b.md001  and a.mc002=b.md002 and a.mc006=b.md003','left');	//單身	
		$this->db->join('taxma as c', 'a.mc001 = c.ma001 ','left');	//
		$this->db->join('copma as d', 'a.mc022 = d.ma001 ','left');	//
		$this->db->where('a.mc001', $seg1); 
        $this->db->where('a.mc002', $seg2);
        $this->db->where('a.mc006', $seg3);		
		$this->db->order_by('mc001 , mc002 , mc006,b.md004');
	...
	function selone_old($seq1,$seq2)  { 
        ...
        $this->db->from('taxmc1 as a');	
        $this->db->join('taxmd1 as b', 'a.mc001 = b.md001  and a.mc002=b.md002 ','left');	//單身	
	...
	function lookup($keyword){     
      $this->db->select('mb001, mb002, mb003,mb004')->from('taxmc1');  
	...
	function findf($limit, $offset, $sort_by, $sort_order)     
         {            		
	      ...
	      $seq11 = "SELECT COUNT(*) as count  FROM `taxmc1` ";
	      $seq1 = " *  FROM `taxmc1` ";
    ...
    function selone1d($seg1,$seg2,$seg3)    
        {
	      ...
	      $query = $this->db->get('taxmd1');
	      return $query->num_rows() ;
	    }  	
 		
	//新增一筆 檔頭  taxmc1	
	function insertf()    //新增一筆 檔頭  taxmc1
        {
		    //刪日期 / 符號
		     preg_match_all('/\d/S',$this->input->post('mc002'), $matches);  //處理日期字串
			 $mc002 = implode('',$matches[0]);
			 preg_match_all('/\d/S',$this->input->post('mc007'), $matches);  //處理日期年月字串
			 $mc007 = implode('',$matches[0]);
			  preg_match_all('/\d/S',$this->input->post('mc031'), $matches);  //處理日期年月字串
			 $mc031 = implode('',$matches[0]);
			 
			 $mc001=$this->input->post('mc001');
			 $mc006=$this->input->post('mc006');
			 $mc006no=$mc006;   //明細用再新增一筆時加1
			 //檢查資料是否已存在 若存在加1
			  while($this->taxi03_model->selone1($mc001,$mc002,$mc006)>0){
				$mc006 = $this->check_title_no($mc001,$mc002,$mc006);
				$mc006no=$mc006;
			}
    ...
    function copyf()           
        {
         ...
         //   if ($query->num_rows() != 1) { return 'exist'; }
            ...
            $seq1=$this->input->post('mc001c');    //主鍵一筆檔頭taxmc1
		    ...
		    if ($exist)
             ...
             $this->db->insert('taxmc1', $data);      //複製一筆  
          ...
          //  if ($query->num_rows() != 1) { return 'exist'; }
			...
			$seq1=$this->input->post('mc001c');    //主鍵一筆明細taxmd1
			$seq2=$this->input->post('mc002c'); 
			$seq3=$this->input->post('mc003c');
              $i=0;
            while ($i<$num) {	
             ...
             $this->db->insert('taxmd1', $data_array);      //複製一筆 
             $i++;			 
            }
             return true;		
	   }
	...
	function excelnewf()           
        {	
		  ...
		  FROM taxmc1 as a,taxmd1 as b WHERE mc211=md003  AND  mc211>= '$seq3'  AND mc211 <= '$seq4'  "; 
	...
	function printfd1()   
       {           
        ...
        $this->db->from('taxmc1 as a');	
        $this->db->join('taxmd1 as b', 'a.mc001 = b.md001  and a.mc002=b.md002 ','left');		
	...
	function printfc()   
      {           
        ...
        $this->db->from('taxmc1 as a');	
        $this->db->join('taxmd1 as b', 'a.mc200 = b.md001  and a.mc216=b.md002 and a.mc211=b.md003 ','left');	//單身	
	...
	function updatef()   
        {
          ...
          $data = array(			
			...
			$this->db->where('mc006', $mc006);
            $this->db->update('taxmc1',$data);                   //更改一筆
			...
			if(!is_array($order_product)){$order_product=array();}
					...
					$this->db->where('md002', $mc002);
					$this->db->where('md003', $mc006);
					$this->db->delete('taxmd1'); //刪除明細 1060809
					
		    $vmd004='1010';   //流水號重新排序
			foreach($order_product as $key => $val){
				extract($val);
				
				if($this->seldetail($mc001,$mc002,$mc006,$val['md004'])>0){
					$data = array(
						'modifier' => $this->session->userdata('manager'),
						'modi_date' => date("Ymd"),
						'flag'  => $flag
					...
					$this->db->update('taxmd1',$data);//更改一筆
					$mmd004 = (int) $vmd004+10;
			        $vmd004 =  (string)$mmd004;
				}else{
					if($val['md004'] && $val['md005']){
	...
	function deletef($seg1)      
        { 
          ...
          $this->db->delete('taxmc1'); 
		  $this->db->where('md001', $this->uri->segment(4));
		  $this->db->where('md002', $this->uri->segment(5));
		  $this->db->where('md003', $this->uri->segment(6));
          $this->db->delete('taxmd1'); 
	    ...
	    if (!empty($_POST['selected'])) 
	         {
                foreach($_POST['selected'] as $check) 
			    {
				  ...
				  $this->db->where('mc006', $seq3);
                  $this->db->delete('taxmc1'); 
				  $this->db->where('md001', $seq1);
				  $this->db->where('md002', $seq2);
			      $this->db->where('md003', $seq3);
				  $this->db->delete('taxmd1');
	            }
            }
	    if ($this->db->affected_rows() > 0)
	...
	//取發票單號 最大值加1 資料看有否重複
	function check_vno($mc010){
		
		$this->db->select('mc010 as max_no')
			->from('taxmc1')
			->where('mc010', $mc010);
